...
```{r, child = if (knitr::is_html_output() ) {'./children/RouletteWheel/RouletteWheel-HTML.Rmd'} else { './children/RouletteWheel/RouletteWheel-LaTeX.Rmd'}}
```
...
From Fig.\ \@ref(fig:RouletteWheelHistx) (bottom left panel), a sample mean with this value does not look unusual at all; it would occur rreasonably frequently.
...
This means that the possible values of sample statistics that we could potentially observe have a *distribution* (specifically, a *sampling distribution*).
...
Crucially, the standard error *is* a standard deviation, but is ony used to describe the variation in sampling distributions.
...
(Not all samling distributions are *normal* distributions, however.)
