...
\index{Hypothesis testing!difference between two means}\index{Test statistic!$t$-score}
...
This requires a different method, called *analysis of variance*\index{Analysis of variance} (or ANOVA), the details of which are are beyond the scope of this book.
...
\]
The alternative hypothesis is that not all three means are equal. 
...
Performing an ANOVA using software (Fig.\ \@ref(fig:BMIjamovi)), the $P$-value is shown as $P = 0.00007$.
(The *test statistic* here is an $F$-score;\index{Test statistic!$F$-score} we don't discuss these further.)
The $P$-value in this context means the same as usual (Sect.\ \@ref(AboutPvalues)): there is very strong evidence to support the alternative hypothesis (that the three means are not all the same).

While we know the means are not all the same, we do not know *which* means are different from which other means.
One option might seem to be to compare all *pairs* of groups (i.e., $D$ vs $O$; $D$ vs $N$; $O$ vs $N$) using two-sample $t$-tests.
However, this approach increases the probability of *incorrectly* declaring a difference between the means (a Type\ I error; Sect.\ \@ref(TypeErrors))
...
```{r BMIjamovi, echo=FALSE, fig.cap="The jamovi output for the BMI data", fig.align="center", out.width='55%'}
