...
The mean of this sampling distribution is called the *sampling mean*; the standard deviation of this sampling distribution is called the *standard error*. 
:::


## Sample means have a distribution {#SamplingDistributionMeans}

The sample mean, like all sample statistics, varies from sample to sample (Sect.\ \@ref(MakingDecisionsInResearch)); that is, *sampling variation* exists, so the sample means have a *sampling distribution*.
...
The statistic we observe depends on which of the countless samples is selected.
...
The standard error tells us how much the value of the sample statistic is likely to vary across, across all of the possible samples; see Fig.\ \@ref(fig:StatisticVariesAcrossSamples).
...
```{r StatisticVariesAcrossSamples, fig.align="center", fig.width=6.5, out.width="70%", fig.cap="Describing how the value of the sample statistic varies across all possible samples"}
#
...
In other words, the standard error appears to get *smaller* as the sample sizes get *larger*: the sample statistics show less variation for larger $n$.
...
Hence, the *standard error* (the standard deviation of the sampling normal distributions) will be smaller for sample of size $250$ than for sample of size $15$.
:::
...
Typically, *standard deviations* are used to describe the variation in the values of a quantitative variable for individual observations: how observations vary from *individual to individual*.
...
1. *Why* is 'the standard error of the population proportion' inappropriate?\tightlist  
`r if( knitr::is_html_output() ) {
	longmcq( c(
