...
The previous chapter introduced the decision-making process (Sect.\ \@ref(MakingDecisionsInResearch)) used in research:
...
Furthermore, under certain conditions, the variation of the *sample statistic* (such as the sample mean, etc.) from all possible samples can be described based on the assumption made about the parameter.
...
We see examples of this in this chapter.
...
Sample proportions, like all sample statistics, vary from sample to sample (Sect.\ \@ref(MakingDecisionsInResearch)); that is, *sampling variation* exists, so sample proportions have a *sampling distribution*.
...
\begin{wrapfigure}{R}{.45\textwidth}
  \begin{center}
```{r RouletteWheel, echo=FALSE, cache=FALSE, out.width = "95%", fig.align="center", fig.height=3.0, fig.width=3.50}
# Wheel  
DD <- 10
dd <- 4
...
The proportion of odd spins $\hat{p}$ can vary from sample to sample (sampling variation) for one sample of $n = 15$ spins, as shown by the histogram (Fig.\ \@ref(fig:RouletteWheelHist), top left panel).
...
We observed something that is very unlikely to occur.
This suggests that we observed something highly unusual, or a problem exists with the wheel.
:::
...
The sample mean, like all sample statistics, varies from sample to sample (Sect.\ \@ref(MakingDecisionsInResearch)); that is, *sampling variation* exists, so sample means have a *sampling distribution*.
...
Again, computer simulation can be used to demonstrate what could happen if the wheel was spun $15$ times, over and over again, and the mean of the spun numbers was recorded for each repetition.
...
When $n = 15$, the sample mean $\bar{x}$ indeed varies from sample to sample.
The *shape* of the distribution again is approximately bell shaped.
...
The statistic we observe depends on which one of the countless samples is selected, and hence which value of the sample statistic is observed.

This means that the possible values of sample statistics that we could potentially observe have a *distribution* (specifically, a *sampling distribution*); see Fig.\ \@ref(fig:StatisticVariesAcrossSamples).
...
The sampling mean is the average value of the sample statistic, across all possible samples.
...
The standard error measures the value of the sample statistic is likely to vary across, across all of the possible samples; see Fig.\ \@ref(fig:StatisticVariesAcrossSamples).
...
```{r StatisticVariesAcrossSamples, fig.align="center", fig.width=6.5, fig.height=3.5, out.width="70%", fig.cap="Describing how the value of the sample statistic varies across all possible samples"}
#
...
Figures\ \@ref(fig:RouletteWheelHist) and\ \@ref(fig:RouletteWheelHistx) show that the variation in the values of the sample statistic get smaller for larger sample sizes.
That is, the standard error gets *smaller* as the sample sizes get *larger*: the sample statistics show less variation for larger $n$.
...
Hence, the *standard error* (the standard deviation of the sampling normal distributions) will be smaller for samples of size $250$ than for samples of size $15$.
:::
...
## Chapter summary {#Chap18-Summary}

A *sampling distribution* describes how all possible values of a sample statistic vary from sample to sample.
...
**1.** Because only *sample statistics* vary from sample to sample. 
**2.** The sample size.
**3.** False.
**4.** False.
**5.** False.
