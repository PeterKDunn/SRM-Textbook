...
if( knitr::is_html_output() ) {
  kable(Scenarios,
        ...
        format="html") %>%
  column_spec(column=1:4, width=c("50mm", "30mm", "15mm", "15mm") )  %>%
  row_spec(0, bold=TRUE) %>%  
  kable_styling("striped", full_width = TRUE) %>%
  pack_rows("Mean of one sample", 1, 1, bold = FALSE, italic = TRUE) %>%
  pack_rows("Mean of differences (paired data)", 2, 2, bold = FALSE, italic = TRUE) %>%
  pack_rows("Comparing means in two groups", 3, 3, bold = FALSE, italic = TRUE) %>%
  pack_rows("Comparing odds/percentages in two groups", 4, 4, bold = FALSE, italic = TRUE)
}
if( knitr::is_latex_output() ) {
  kable(Scenarios,
        ...
        caption="Four different scenarios studied so far",
        format="latex") %>%
  #column_spec( column=1:4, width=c("30mm", "40mm", "50mm", "30mm") )  %>%
  kable_styling("striped", full_width=TRUE, font_size=9) %>%
  pack_rows("Mean of one sample",                         start_row=1, end_row=1, bold = FALSE, italic = TRUE) %>%
  pack_rows("Mean of differences (paired data)",          start_row=2, end_row=2, bold = FALSE, italic = TRUE, hline_before=TRUE) %>%
  pack_rows("Comparing odds/percentages in two groups",   start_row=3, end_row=3, bold = FALSE, italic = TRUE, hline_before=TRUE) %>%
  pack_rows("Comparing means in two groups",              start_row=4, end_row=4, bold = FALSE, italic = TRUE, hline_before=TRUE)
}
