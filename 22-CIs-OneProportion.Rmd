...
* the standard deviation, called the *standard error* $\text{s.e.}(\hat{p})$, has the value\ $`r round(se.die, 3)`$.
  (The source of this number will be revealed later, in Equation\ \@ref(eq:StdErrorExampleDieCI).)
...
In general, we do not know if the computed CI the actual value of\ $p$, since the value of\ $p$ is usually unknown.
...
The CI for a single proportion is *statistically validity* if *both* of these are true:
