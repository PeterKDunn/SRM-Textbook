...
$$
where $\text{s.e.}(\hat{p})$ is the *standard error of the sample proportion* (calculated using Equation\ \@ref(eq:StdErrorPknownCI)), and the *multiplier* comes from the $68$--$95$--$99.7$ rule, depending on the level of coverage being sought (i.e., the multiplier is a $z$-score).
...
However, here the value of\ $\hat{p}$ is known, so an interval is created that is likely to contain the value of\ $p$ that produced the observed value of $\hat{p}$ (Fig.\ \@ref(fig:CIrelationshipsCI)).
...
More formally, a CI is an interval which contains the unknown value of the parameter a given percentage of the time (over repeated sampling).
:::
...
Consider a situation where $p = 0.1$ is the population proportion of some result of interest.
...
That is, the values for\ $p$ that may have led to this value of $\hat{p} = 0.1694$ are between\ $0.130$ and\ $0.209$ with $95$% confidence.
...
> In the past month, how often have you used an Artificial Intelligence or digital device (e.g. Chat GPT, smart phone) as an unauthorised aid during an assessment, school assignment, or homework.
