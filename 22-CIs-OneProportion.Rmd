  ...
  for (i in 1:num.sims){
    ...
    if (print_Histo[i]){
      out <- hist( prop.even,
                   ...
                   xlab = expression(Proportion~of~the~25~rolls~showing~an~even~number~hat(italic(p))),
...
```{r RollDiceHistFigCI, fig.align="center", fig.height=3, fig.width=6.5, out.width='70%', fig.cap="The proportion of rolls that are even, $\\hat{p}$, is not the same for every sample of $25$ rolls; it varies around a mean of $p = 0.5$. The solid line is the normal distribution used to model the sampling distribution." }
...
```{r NormalDieTheoryCI, fig.cap="The sampling distribution is an approximate normal distribution with mean \\ $0.5$ and standard error\\ $0.1$; it is a model for how the proportion of even rolls varies when a die is rolled $25$ times.", fig.align="center", fig.width=8.25, fig.height=3.25, out.width='100%'}
pop.p <- 0.5
n <- 25
se.p <- sqrt( pop.p * (1 - pop.p) / n )

par( mar = c(5, 0.05, 0.5, 0.05)) 
...
text(x = 0.08,
     ...
     pos = 3,
     cex = 0.9,
     labels = expression(atop(These~values~of~hat(italic(p)),
                              unlikely~to~be~seen) ) )
arrows(x0 = 0.15,
       x1 = 0,
       y0 = 0.09 * max(out$y),
...
So, about\ $68$% of the time, the proportion of even rolls in a sample of\ $25$ rolls will lie between $0.5 - `r round(se.die, 3)` =  `r round(0.5 - se.die, 3)`$ and $0.5 + `r round(se.die, 3)` =  `r round(0.5 + se.die, 3)`$.
...
```{r CIrelationshipsSI, out.width='73%', fig.align="center", fig.cap="A known value of $p$ produces a range of $\\hat{p}$ values.", fig.width = 7, fig.height=3.75}
...
```{r CIrelationshipsCI, out.width='75%', fig.align="center", fig.cap="The sampling distribution for\\ $\\hat{p}$: many values of $p$ may have produced the observed value of $\\hat{p}$.", fig.width = 7, fig.height=3.75}
par(mar = c(0, 1, 5.0, 1) + 0.25 )     
...
```{r, StatisticalValidityPHat, out.width='95%', fig.align="center", fig.cap="Two proposed sampling distributions. The sampling distribution from many simulated samples is shown in the histogram; the normal model is shown by the solid lines. Left: when the statistical validity conditions are not met. Right: when the statistical validity conditions are met.", fig.height=2.5, fig.width=8, out.width="90%"}
...
@data:Lee2016:Hiccups found that $708$ of $864$\ patients examined with hiccups were male.
