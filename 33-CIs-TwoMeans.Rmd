...
# CIs: comparing two independent means {#CITwoMeans}



```{r, child = if (knitr::is_html_output()) {'introductions/33-CIs-TwoMeans-HTML.Rmd'} else {'introductions/33-CIs-TwoMeans-LaTeX.Rmd'}}
```
...
This is a *between-individuals* comparison,\index{Comparison!between individuals} since different students are in each group.
...
Since the population means are unknown, this parameter is estimated using the statistic $(\bar{x}_P - \bar{x}_C)$.
The differences could be computed in the opposite direction ($\bar{x}_C - \bar{x}_P$).
However, computing differences as the reaction time for phone users, *minus*  the reaction time for non-phone users (controls) probably makes more sense: the differences then refer to the increase in the mean reaction times when students are using phones.
...
* the reaction time for phone users, *minus* the reaction time for non-phone users;
  this measures how much faster the reaction times is for *non-phone users*; or
* the reaction time for non-phone users, *minus* the reaction time for phone users;
  this measures how much faster the reaction times is for *phone users*.
...
However, the *standard error* of the difference between the means does make sense, as the value of  $\bar{x}_P - \bar{x}_C$ varies across all possible samples and so has a sampling distribution.
...
The information can be found, for example, using software\index{Software output!comparing two means} (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)).


```{r ReactionPhoneSummaryjamovi, fig.cap="Software output for the phone reaction time data. The top table includes summaries of the difference between the two means, and the bottom table shows summaries for each group separately.", fig.align="center", out.width='100%'}
...
#      digits=c(2, 0, 2, 3),
        col.names = c("Mean", 
                      ...
                      "Standard deviation", 
                      "Standard error"),
    caption = "Summary statistics for the reaction-time data (reaction times measured in ms).") %>%
		row_spec(0, bold = TRUE)  %>%
...
## Confidence intervals for  $\mu_1 - \mu_2$ {#CIDiffBetweenProportions}
\index{Sampling distribution!comparing two means}

Each sample will comprise different students, and give different reaction times. 
...
$$
	(\bar{x}_P - \bar{x}_C) \pm \big(2 \times \text{s.e.}(\bar{x}_P - \bar{x}_C)\big).
$$
...
> The difference between reactions times is\ $`r round(RT.mn, 2)`$\millis, slower for those *using* a phone (mean: $585.19$\millis; s.e.: $15.85$; $n = 32$) compared to those *not using* a phone (mean: $533.59$\millis; s.e.: $11.55$; $n = 32$), with an *approximate* $95$%\ CI for the difference between mean reaction times from $`r round(RT.mn - 2*RT.se, 2)`$ to $`r round(RT.mn + 2*RT.se, 2)`$\millis.


The plausible values for the difference between the two population means are between\ $`r round(RT.mn - 2*RT.se, 2)`$ to\ $`r round(RT.mn + 2*RT.se, 2)`$\millis (slower for those *using* a phone).
...
In the software output in Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi), this is the second row of the top table (labelled 'Welch's\ $t$').
...
From the output, the $95$%\ CI is from\ $12.32$ to\ $90.87$\millis.
...
* When *both* samples have $n \ge 25$, the CI is statistically valid.
  (If the distribution of a sample is highly skewed, the sample size for that sample may need to be larger.)
* When one or both groups have $25$\ or fewer observations, the CI is statistically valid only if the *populations* corresponding to both comparison groups have an approximate normal distribution.
...
Note that the phone-users group has a large outlier, so the sample size being larger than\ $25$ is probably necessary.
:::
...
$$
as in the second row ('Welch's\ $t$') of the first table in Fig.\ \@ref(fig:SpeedjamoviCI2). 
...
Based on the sample, an approximate $95$%\ CI for the *difference* in mean speeds is $5.68 \pm (2 \times 2.964)$, or from\ $-0.24$ to\ $11.6$\kms.h^$-1$^, higher before the addition of extra signage.
(The negative value refers to a *negative* reduction; that is, an *increase* in speed of\ $0.24$\kms.h^$-1$^.) 

From Fig.\ \@ref(fig:SpeedjamoviCI2), a $95$%\ CI is from\ $-11.6$ to\ $0.23$\kms.h^$-1$^, *lower* before the addition of extra signage (since the *After* information is given first in the second table of Fig.\ \@ref(fig:SpeedjamoviCI2)).
This is equivalent to a $95$%\ CI from\ $-0.23$ to\ $11.6$\kms.h^$-1$^, *higher* before the addition of extra signage.
...
> The reduction in the mean speed after adding signage is\ $5.68$\kms.h^$-1$^ (before mean: $98.02$; s.e.: $2.140$; $n = 38$; after mean: $92.34$; s.e.: $2.051$; $n = 41$), with a $95$%\ CI between\ $-0.24$\ km.h^$-1$^ (i.e., an increase of $0.23$\kms.h^$-1$^) and\ $11.6$\kms.h^$-1$^ (two independent samples).
...
They randomly allocated $32$\ patients into a control group (who drank hot water), and\ $32$ to receive chamomile tea.
...
From Table\ \@ref(tab:TGsummaryTable), the estimate of the mean difference is $\bar{x}_T - \bar{x}_W = 45.74$\mgs.dl^$-1$^.
...
$$
or from\ $28.90$ to\ $62.58$\mgs.dl^$-1$^, with a larger reduction for the tea group.
...
```{r ChamomilePlot, fig.align="center", fig.cap="The reduction in total glucose for the chamomile-tea drinking group, and the control group. The horizontal grey line represented no mean reduction in TG in each group.", out.width='55%', fig.width=4.75, fig.height=3.25}
TGsummary <- array(dim = c(3, 7) )

par(mar = c(5, 6, 4, 2) + 0.1) # Default: c(5, 4, 4, 2) + 0.1.
...
#      ylab = "Reduction in total\nglucose (in mg/dl)",
      ylab = expression( atop(Reduction~"in"~total,
                              glucose~"("*"in"~mg*"."*dl^{-1}*")") ),
      xlab = "Group",
      main = "Reductions in\ntotal glucose",
...
> The difference between the mean *reductions* in TG is\ $45.74$\mgs.dl^$-1$^, greater for the tea-drinking group, with the approximate $95$%\ CI from\ $28.64$ to\ $62.84$\mgs.dl^$-1$^.

This interval has a $95$%\ chance of straddling the difference between the mean reductions in TG.\spacex
The sample sizes are larger than\ $25$ ($n = 32$ for both groups), so the results are statistically valid.
...
[*Dataset*: `Dental`]
@data:woodward:dental recorded the sugar consumption in industrialised countries (mean: $41.8$\kgs/person/year) and non-industrialised countries (mean: $24.6$\kgs/person/year).
...
```{r DentaljamoviCI, fig.cap="Software output for the sugar-consumption data; the Groups refer to whether or not the country is industrialised.", fig.align="center", out.width="95%"}
...
Use the data, and the summary in 
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:SignageSummaryData)'
} else {
   'Exercise\\ \\@ref(exr:CompareQuantDeceleration)'
...
[*Dataset*: `BodyTemp`]
The average internal body temperature is commonly believed to be $37.0^\circ\text{C}$ ($98.6$^o^F).
@data:mackowiak:bodytemp wished to determine the difference between the mean internal body temperature for males and females.
