...
However, the authors identified one threat to internal validity (p. 220): 
...
In a study of the hand-hygiene practices of paramedics [@barr2017self], *self*-reported hand-hygiene practices were very different than what was reported by *peers* (p. 77):
...
In a study of retro-fitting house with energy-saving devices, @giandomenico2022systematic found large discrepancies between saving when observational (cross-sectional) studies were used (12.2%) and when experimental (randomised controlled trial) studies were used (6.2%).
...
1. When interpreting the results of studies, the steps taken to maximize internal validity should be considered. `r if( knitr::is_html_output(exclude = "epub") ) {torf( TRUE)}`
2. If studies are not externally valid, then they are not very useful. `r if( knitr::is_html_output(exclude = "epub") ) {torf( FALSE)}`
3. When interpreting the results of studies, the steps taken to maximize external validity do not need to be considered. `r if( knitr::is_html_output(exclude = "epub") ) {torf( FALSE)}`
...
The study states (p. 9) that 'are limited to the specific study and cannot be easily projected to other similar settings'.

What type of validity is being discussed here?
:::


::: {.exercise #ValidityFarmManagement}
In a study evaluating the study of farm managament practices, @kluger2022combining state (p. 2) stated:
...
A study of how well hospital patients sleep at night [@delaney2018they] set out to 'investigate the perceived duration and quality of patient sleep' (p. 1).
In discussing the study, the researchers state (p. 2):
