...
```{r WaterAcessQual, fig.align="center", fig.cap="The distance to the water source (left), the wait time at the water source (centre), and the water sources (right) for the water-access study.", out.width='100%', fig.width=6.25, fig.height=1.7}

par(mfrow = c(1, 3),
    mar = c(3.75, 0, 4, 0.5) )

WASourceLabels <- names(table(WaterAccess$SourceDistance))
WASourceLabels[2] <- "100 to 1000m"

dotchart( as.numeric(table(WaterAccess$SourceDistance)),
          labels = WASourceLabels,
     xlab = "Number",
     ylab = "",
     main = "Distance to\nwater source",
     xlim = c(0, 60),
...
WAQueueLabels <- names(table(WaterAccess$SourceQueueTime))
WAQueueLabels[1] <- "< 5 mins"
WAQueueLabels[2] <- "5 to 15 mins"
WAQueueLabels[3] <- "> 15 mins"

dotchart( as.numeric(table(WaterAccess$SourceQueueTime)),
          labels = WAQueueLabels,
     xlab = "Number",
     ylab = "",
     main = "Queue wait at\nwater source",
     xlim = c(0, 55),
...
3. Compute the percentages in each level.
:::
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "Social groups for spider monkeys.") %>% 
    kable_styling(font_size = 8)
  out2 <- prepareSideBySideTable(out, 
                                 gap = "\\qquad") 
  out2
...
**2.** True.
**3.** True.
**4.** False. Percentages are proportions multiplied by\ $100$, so similar but different.
:::
`r if (knitr::is_html_output()) '-->'`
