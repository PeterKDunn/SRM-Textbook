...
The rows (or the columns) should list the *levels* of the variable, and these should be *exhaustive* (cover all levels) and *mutually exclusive* (observations belong to only one level).
...
For *nominal* data, the levels of the variables can be displayed in alphabetical order, in order of size, by personal preference, or any other way: use the order most likely to be useful to readers.
For *ordinal* data, the natural order of the levels should almost always be used.
...
The responses to the three questions produce *ordinal* data.
...
Qualitative data can also be *numerically* summarised using proportions, percentages (Sect.\ \@ref(QualitativeProportionsPercentages)) or odds (Sect.\ \@ref(QualOdds)) of observations in each level.
...
Consider the AV data in Table\ \@ref(tab:AVtable1), summarising results from a sample of $n = 400$ respondents.
The *percentage* of respondents that are female is $204\div 400\times 100 = 51$%.
