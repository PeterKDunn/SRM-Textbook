...
Sometimes these graphs are also used for quantitative data (especially discrete data) with a small number of possible options.
...
Dots indicate the number (or percentage) in each category.
...
::: {.example #AVOddsMale name="Odds of males"}
...
```{r PropOdds, fig.align="center", out.width='90%', fig.width=9, fig.height=2.0, fig.cap="Proportions (left) are the number of interest divided by the total number; odds (right) are the number of interest divided by the rest."}
...
2. Compute the mode for each qualitative variable, and the percentage and odds of that level occurring in the data.
:::
...
@lopez2022farmers asked $231$ farmers what they considered to be the advantages and disadvantages of using reclaimed water.
...
1. Compute the percentages for each option.
