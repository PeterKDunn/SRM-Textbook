...
```{r, child = if (knitr::is_html_output()) {'introductions/37-Sample-Size-Estimation-HTML.Rmd'} else {'introductions/37-Sample-Size-Estimation-LaTeX.Rmd'}}
```
...
A [confidence interval](#AboutCIs) is an interval which gives a range of values of the population parameter that could plausibly have produced the observed value of the statistic.\index{Confidence intervals}
All other things being equal, a *larger* sample size gives a *more precise* estimate of the population parameter.\index{Precision}
...
For a given level of confidence (e.g., $95$%), the width of a CI depends on the size of the sample.
...
Suppose we wish to estimate an unknown proportion, and find that $\hat{p} = 0.55$ from a sample of size $n = 25$.
The approximate $95$%\ CI is $0.55 \pm 0.199$ (so the *margin of error* is $0.199$)

If the estimate of $\hat{p} = 0.55$ was found from a sample of size $n = 100$, a more precise estimate should be expected.
...
In each case, the sample size increased by a factor of $4$, but the margin of error was halved.
:::
...
In addition, *how* the sample was obtained is usually more important the the size of the sample: random sample are preferred over non-random samples.


::: {.example #Biochar name="The cost of research"}
A project studied the residual effect of organic biochar compound fertilizers (BCFs) *two years* after application [@farrar2021biochar].
...
The formulas given in this chapter only apply for *forming $95$%\ CIs*, and are very *conservative*: they will probably give samples sizes a bit *too large*, but that is better than being too small.
In any case, larger-than-calculated sample sizes are often used anyway to allow for *drop outs*:\index{Drop outs} animals or plants that die; people who can no longer be contacted; and so on.
...
In Sect.\ \@ref(MeanDiffCI), a CI is computed for the mean weight gain by Cornell University students from Week\ 1 to Week\ 12 (@levitsky2004freshman, @DASL:WeightChange). 
...
## Difference between two means {#SampleSizeDifferenceTwoMeans}
...
Conservatively, the size of the [*simple random sample*](#SRS) needed *for a $95$%\ CI for the difference between two means* with a specified margin-of error is *at least*  
\[
...
::: {.example #SampleSizeReactionTime name="Sample size estimation for difference between means"}
In Sect.\ \@ref(MeansIndSamples), a CI is computed for difference between the reaction times for drivers *using* and *not using* a mobile phone [@data:Strayer2001:phones; @agresti2007statistics].
Suppose we wanted to estimate the difference between the mean reaction times to within $15$\ ms.

Table\ \@ref(tab:PhoneNumerical) shows that $s = 65.36$\ ms for those *not using* a phone, and $s = 89.65$\ ms for those *using* a phone.
The sample-size calculation requires using a common value for the standard deviations in both populations.
To use the formula, we will use $s = 78$ (the mean of the two values) as the common standard deviation, understanding that this may give only give a rough sample size.
Then:  
\[
   2 \times \left( \frac{2 \times 78}{15}\right)^2 = 216.32.
\] 
We would need at least $217$ students in *each* sample.
:::
...
Section\ \@ref(exr:CIOneProportionSnacking) discusses a study of the eating habits of university students in Canada [@data:Mann12017:UniStudents].
...
1. How many paramedics is needed to estimated the mean with an precision of give-or-take $25$\ ml?
...
::: {.exercise #SampleSizeTwoMeansSpeeds}

In Sect.\ \@ref(SpeedSignageCI), a study was described to measures the reduction in mean vehicle speed after installation of additional signage [@ma2019impacts].

1. What would be a useful value to use for the common standard deviation (see Fig.\ \@ref(fig:SpeedjamoviCI2))?
1. Suppose we wish our $95$%\ CI to be give-or-take $5$\ km.h^-1^. 
   How many vehicle speeds would need to be measured?
:::
