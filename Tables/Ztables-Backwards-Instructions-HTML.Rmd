...
However, when working backwards, the tables in Appendix \@ref(ZTablesOnlineBackwards) are used: enter the area to the left in search box under `Area.to.left`, and the corresponding $z$-scores appears under the `z.score` column (see the animation below).
...
When the $z$ scores (which appear in the *margins* of the tables; see Sect. \@ref(UsingZTables)) is known, the *areas* appear in the *body* of the table. 
