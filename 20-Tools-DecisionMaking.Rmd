...
One of the main purposes of the study was to determine contributors to the incidence of diarrhoea in young children.
...
We observe just one of the many possible samples, and hence observe just one of the many possible percentages of children with diaarhea in households *with* livestock.
Similarly, we observe just one of the many possible percentages of children with diaarhea in households *without* livestock.
...
Getting $15$ reds cards out of $15$ (i.e., $\hat{p} = 1$) from a well-shuffled pack seems very unlikely; you probably conclude that the pack is somehow unfair or unshuffled.
...
In other words, there has been 'no change, no difference, no relationship' in the population, depending on the context.
Using this idea, a reasonable assumption can be made about the *population* parameter:

* We *assume* that *no difference* exists between the parameter for two groups in the *population*.
  Any difference between the statistics for the two groups can be explained by sampling variation.,
  For example, we might assume that the percentages of children with diarrhea is the *same* for households that *do* and *do not* keep livestock in the water-access study.
...
* We *assume* there has been *no change* in the value of some population parameter (for example, after using a new method, or studying a different group).
  The statistic might suggest a change, but that is due to sampling variation.
...
so a sample producing $\hat{p} = 1$ almost *never* occurs.
