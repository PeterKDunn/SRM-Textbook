...
One of the main purposes of the study was to determine contributors to the incidence of diarrhea in young children.
...
If a different sample of women was chosen, different values for the two sample percentages would have been produced.
...
2. *Expectation*: Based on this assumption, roughly (but not necessarily *exactly*) equal numbers of red and black cards would be expected in a sample of $15$ cards.
...
For example, if the mean of two separate groups are being compared, we would initially assume *no difference* between the *population* means: any difference observed in the *sample* means would be attributed to chance.
...
Making a decision means answering this question: is it reasonable that you would observe $\hat{p} = 1$ if we assume $p = 0.5$?
Observing $15$ red cards out of $15$ cards is quite rare: it never happened once in the $1000$ simulations above
`r if (knitr::is_latex_output()) {
