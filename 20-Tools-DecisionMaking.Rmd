...
In Sect.\ \@ref(WaterAcessSummary), a study of three rural communities in Cameroon [@lopez2022farmers] and their access to water was described.
...
The sample studied is just one of *countless* possible samples that could have been chosen.
If a different sample was chosen, different values for the two odds (with livestock; without livestock) would have been produced.
...
Since many values for the sample are possible, the possible values of the statistic vary (called *sampling variation*)\index{Sampling variation} and have a *distribution* (called a *sampling distribution*).\index{Sampling distribution}
:::
...
Two reasons could explain why the two *sample* odds are different:

1. The two *population* odds are the *same*; the disparity between the two *sample* odds is due to *sampling variation*.
...
The two possible explanations ('statistical hypotheses') have special names:\index{Hypotheses!statistical}

1. *No difference exists between the population odds*: the difference between the sample odds is simply due to *sampling variation*. 
   This is the *null hypothesis*, or $H_0$.\index{Parameter}\index{Hypotheses!null}
2. *A difference exists between the population odds*.
...
In research, decisions need to be made about *parameters* based on *statistics*.
...
However, sensible decisions *can* be made (and *are* made) about parameters based on statistics.
...
In this sample, $\hat{p} = 1$; that is, *all* $n = 15$ cards are red cards.
...
```{r Draw15Cards, fig.align="center", fig.width=7, fig.height=4.5, out.width = "30%",fig.cap="How likely is it that you would get $15$ red cards in a row from a fair pack?" }
...
3. You then *observed* that *all* $15$ cards were red.
   That is, $\hat{p} = 1$.
4. You were expecting $\hat{p} = 0.5$ approximately, but observed $\hat{p} = 1$.
...
1. *Assumption*:
   ...
   Initially, assume that the *sampling variation* explains any discrepancy between the value of the observed statistic and assumed value of the parameter.
2. *Expectation*:
...
The initial assumption is that the discrepancy between the parameter and the statistic is due to sampling variation.
...
These assumptions about the parameter are called *null hypotheses*,\index{Hypotheses!null} denoted $H_0$.
...
If the *sample* mean is not two minutes, the null hypothesis explains the discrepancy by sampling variation.
...
Since many samples are possible, and every sample is likely to be different (sampling variation), the observed value of the sample statistic depends on which one of the countless possible samples we obtain.
...
How could we find out how often $\hat{p} = 1$ occurs in a sample of $n = 15$?
We could:
...
```{r RollDice10, fig.align = "center", fig.width = 6.5, out.width='75%', fig.cap = "Ten hands of $15$ cards: the sample proportion that is red  (as shown on the right-hand side) varies from hand to hand"}
if (knitr::is_latex_output()){
  set.seed(99999)
  num.in.hand <- 15
  num.sims <- 10
...
```{r HandRedHist1000, fig.align="center", fig.height=3.75,fig.width=6.25, out.width='70%', fig.cap="A histogram of the sample proportion of red cards, in hands of $n = 15$ cards, for $1000$ repetitions. A bell-shaped distribution is drawn over the histogram." }
if (knitr::is_latex_output()){
    set.seed(99030991)
    num.in.hand <- 25
    num.sims <- 1000
...
From a sample of $15$ cards (one of the many samples that are possible), suppose we observe all red cards, so that the statistic is $\hat{p} = 1$ (i.e., $15$ red cards out of $15$ cards).
...
That is, the pack of cards has been manipulated.
...
but $\hat{p} = 0.733$ (and even larger values) *did* occur in the $1000$ simulations.
...
Of course, every possible sample of $85$ children will include different children, and so produce a different sample mean brushing times $\bar{x}$.
...
Decisions are often made by making an *assumption* about the parameter, which leads to an *expectation* of what values of the statistic might occur.
...
2. The claim is based on a sample of $125$ pizzas, for which the sample mean pizza diameter was $\bar{x} = 11.48$ inches. 
   (See Sect.\ \@ref(ToolsDecisionMaking-Intro).)
