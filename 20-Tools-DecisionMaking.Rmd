...
```{r, child = if (knitr::is_html_output()) {'./introductions/20-Tools-DecisionMaking-HTML.Rmd'} else {'./introductions/20-Tools-DecisionMaking-LaTeX.Rmd'}}
```


## Introduction {#ToolsDecisionMaking-Intro}
\index{Decision making}

Section\ \@ref(WaterAcessSummary) described a study of three rural communities in Cameroon [@lopez2022farmers] and their access to water.
One purpose of the study was to determine contributors to the incidence of diarrhea in young children.

In the observed sample, the odds of an incidence of diarrhea in children in households *without* livestock was $0.176$ (Table\ \@ref(tab:WaterAccessCompareQuantTabs)).
However, the odds in households *with* livestock was $0.548$.
In other words, the odds of an incidence of diarrhea in young children was over three times greater in households with livestock than without (i.e., $0.548/0.176 = 3.11$).
That is, a disparity in the odds is seen *in the sample*; but RQs ask about the *population*.

However, the sample studied is just one of *countless* possible samples that could have been chosen.
If a different sample was chosen, different values for the odds of an incident of diarrhea in the two groups (with livestock; without livestock) would have been produced.
...
Since many values for the sample statistic are possible, the possible values of the sample statistic vary (called *sampling variation*)\index{Sampling variation} and have a *distribution* (called a *sampling distribution*).\index{Sampling distribution}
:::


Since we observe just one of the many possible samples, what does the disparity in the sample odds imply about the *population* odds?
Two reasons could explain why the *sample* odds are different:

1. The two *population* odds are the *same*; the disparity between the *sample* odds is due to *sampling variation*.
   That is, we just happen to have---by chance---one of those samples where the disparity between the two odds is noticeable.
   The *sample* odds are not the same only because we have data from one of the many possible samples, and every sample is likely to be different.
2. Alternatively, a disparity between the two *population* odds exists, and the *sample* odds reflect this.

How do we decide which of these explanations is supported by the data?

The two possible explanations ('statistical hypotheses'^[The word 'hypothesis' just means 'a possible explanation'.]) have special names:\index{Hypotheses!statistical}
...
1. *No difference exists between the population parameters*: the difference between the statistics is simply due to *sampling variation*. 
   This is the *null hypothesis*, or $H_0$.\index{Parameter}\index{Hypotheses!null}
2. *A difference exists between the population parameters*.
   This is the *alternative hypothesis*, or $H_1$.\index{Hypotheses!alternative}
...
In research, decisions need to be made about  *population parameters* based on *sample statistics*.
The challenge is that the decision must be made using only one of the many possible samples, and every sample is likely to be different (comprising different individuals from the population), and so each sample will produce different summary *statistics*.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Sampling variation refers to how a sample estimate (a *statistic*) varies across all possible samples, because each sample is different.
:::
...
In this sample, $\hat{p} = 1$; that is, *all* 15 cards are red cards.
...
1. *Assumption*:
   ...
   That is, you assumed the *population proportion* of red cards was $p = 0.5$.
2. *Expectation*:
...
3. *Observation*:
   ...
   That is, $\hat{p} = 1$.
4. *Decision*:
   You were expecting that $\hat{p} = 0.5$ approximately, but the observed value was $\hat{p} = 1$.
...
For this reason, you would probably conclude, based on the evidence, that strong evidence suggests that the pack is not a fair pack.
...
1. *Assumption*:
   ...
   Initially, assume that the *sampling variation* explains any discrepancy between the value of the observed statistic and assumed value of the population parameter.
2. *Expectation*:
...
4. *Decision*:
   If the observed *sample statistic* is:
    ...
    - *likely* to have been observed by chance, the statistic (i.e., the evidence) is *consistent with* the assumption about the *population parameter*, and the assumption may be *correct* (though not certainly correct).
...
2. *Expectation*: Based on that assumption, I would *expect* to find his toothbrush is damp.
...
Then, the evidence seems *consistent* with what I expected to find based on my assumption, so my assumption is probably *true*.
...
However, the evidence certainly suggests the pack is not fair.
:::
...
* Make the *observations* (Sect.\ \@ref(Observation)); and
* Make a *decision* (Sect.\ \@ref(MakeDecision)).
...
The initial assumption is that the difference between the parameter and the statistic is due to sampling variation.
In other words, there has been 'no change, no difference, no relationship' in the population, depending on the context; any change, difference or relationship seen in the sample is due only to sampling variation.

Using this idea, a reasonable assumption can be made about the *population* parameter.\index{Parameter}
For example, when comparing the mean of two groups, we would initially assume *no difference* between the *population* means: any difference observed in the *sample* means would be attributed to chance.
 
These assumptions about the population parameter are called *null hypotheses*.\index{Hypotheses!null}
...
@data:Macgregor1979:BrushingDurationKids recorded the tooth-brushing time for $85$ uninstructed schoolchildren from England.

We initially *assume* the *population* mean tooth-brushing time in the population is two minutes ($H_0$: $\mu = 2$).
If the sample mean is not two minutes, the difference would be explained by sampling variation.
...
\index{Decision making!expectation}\index{Statistics}
...
Since many samples are possible, and every sample is likely to be different (sampling variation), the observed value of the sample statistic depends on which one of the possible samples we obtain.
...
*None* of those hands produced $15$ red cards in $15$ cards.
...
Interestingly, the *shape* of the distribution in
`r if (knitr::is_latex_output()) {
...
These bell-shaped distributions appear in many places, and are studied further in Chap.\ \@ref(SamplingDistributions).
...
From a sample of $15$ cards (one of the many samples that are possible), suppose we observe all red cards, so that the sample statistic is $\hat{p} = 1$ (i.e., $15$ red cards out of $15$ cards).
...
Making a decision means answering this question: is it reasonable to observe $\hat{p} = 1$ when $p = 0.5$?
Observing $15$ red cards out of $15$ cards is rare: it never happened once in the $1000$ simulations above
`r if (knitr::is_latex_output()) {
   '(see Fig.\\ \\@ref(fig:HandRedHist1000)),'
} else {
   '(see the animation above),'
}`
so a sample producing $\hat{p} = 1$ hardly ever occurs.
So based on simulating $1000$ hands, you could conclude that finding $\hat{p} = 1$ *almost never occurs*... *if the assumption of a fair pack was true*.
...
Those tools are discussed in the following chapters:
...
## Chapter summary {#ToolsDecisionMaking-Summary}
...
## Quick review questions {#ToolsDecisionMaking-QuickReview}
...
1. The value of a statistic is likely to be different in every sample.  
`r if( knitr::is_html_output() ) {torf(answer = TRUE )}`
1. *Sampling variation* describes how the value of a *statistic* varies from sample to sample.
`r if( knitr::is_html_output() ) {torf(answer = TRUE )}`
1. The initial assumption is made about the *sample statistic*.  
`r if( knitr::is_html_output() ) {torf(answer = FALSE )}`
1. The variation in statistics from sample to sample is called sampling variation
`r if( knitr::is_html_output() ) {torf(answer = TRUE )}`
...
Suppose you are playing a die-based game, and your opponent rolls a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{6}'
} else {
...
1. Does the claim appear to be supported by, or contradicted by, the data? 
   Why?
3. Would your conclusion change if the sample mean was $\bar{x} = 11.25$ inches?
