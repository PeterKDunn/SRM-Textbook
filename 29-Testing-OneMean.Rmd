...
if( knitr::is_latex_output() ) {
  subBodyT <- as.data.frame(subBodyT)
...
From this, if $\mu$ really was $37.0^\circ$C, the possible values of the sample means across all possible samples can be described (if certain conditions are true; (Sect.\ \@ref(ValiditySampleMeanTest)) using:
...
Given the sampling distribution (Fig.\ \@ref(fig:BodyTempSamplingDist)), use the [68--95--99.7 rule](#def:EmpiricalRule) to determine how often will $\bar{x}$ be **larger** than $37.036^\circ$C just because of sampling variation, if $\mu$ really is $37^\circ$C.
...
In traffic, the usual engineering recommendation is that the safe gap between travelling vehicles (called a 'headway') is *at least* $1.9$s (often conveniently rounded to $2$ seconds in practice).
