...
$z$-scores are calculated using *no sample information* (as with a test for proportions (Chapo. \@ref(TestOneProportion)), while $t$-scores are calculated *using sample information*.
...
3. The **observed statistic** is computed, relative to what is expected using a $t$-score (Fig. \@ref(fig:BodyTempSamplingDistT)): $t = -5.453$.
...
* *If* the calculated $t$-score was $t = -1$, the *two-tailed* $P$-value would be the shaded area in Fig. \@ref(fig:BodyTempPvals) (top panel):
...
* *If* the calculated $t$-score was $t = -2$, the *two-tailed* $P$-value would be the shaded area shown in Fig. \@ref(fig:BodyTempPvals) (bottom panel):
...
```{r BodyTempPvals2, echo=FALSE, fig.cap="Computing $P$-values for the body temperature data", fig.align="center", fig.width=5, fig.height=5}
par( mfrow = c(2, 1))
