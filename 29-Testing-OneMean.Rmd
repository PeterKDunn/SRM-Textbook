...
The average internal body temperature is commonly believed to be $37.0^\circ\text{C}$ ($98.6^\circ$F), based on data over 150 years old [@data:Wunderlich:BodyTemp].
...
A numerical summary, from using software (Fig.\ \@ref(fig:BodyTempjamovi)), shows that:
...
\centering
```{r, out.width='95%', fig.width=5, fig.height = 3}
...
Given the sampling distribution (Fig.\ \@ref(fig:BodyTempSamplingDist)), use the [68--95--99.7 rule](#def:EmpiricalRule) to determine how often will $\bar{x}$ be *larger* than $37.036^\circ$C due to sampling variation, if $\mu$ really is $37$.
...
The value of $\bar{x}$ that is *observed*, however, is $\bar{x} = 36.8051^\circ$C.
...
The value $\bar{x} = 36.8051^\circ\text{C}$ is unusually small: a sample mean this low is very unlikely from a sample of $n = 130$ if $\mu$ really was $37$.
...
The number of standard deviations that $\bar{x} = 36.8051$ is from the mean is therefore  
\[
...
When the value of interest is sample statistic, both measure the number of *standard errors* that a *sample statistic* is from the mean.
The difference is that:

* $t$-scores use a standard error involving sample estimates (as in this chapter).
* $z$-scores *do not* use a standard error involving sample estimates (as with a test for proportions (Chap. \@ref(TestOneProportion)).

$t$-scores are more common than $z$-scores, because the population standard deviation is rarely unknown, and so the sample standard deviation is used to compute the standard error.
...
This $P$-value means that, assuming $\mu = 37.0$, observing a sample mean as low as $36.8051$ just through sampling variation (from a sample size of $n = 130$) is almost *impossible*.
And yet... we did.
...
The *parameter* of interest is $\mu$, the mean speed in the *population* (in km.h^-1^).
...
For samples of size $n = 400$, the mean of this normal distribution has the standard deviation   
\[
...
\]
where (as usual) the value of $\mu$ is from the null hypothesis (which we always assume to be true initially).
...
## Example: recovery times: REMOVE {#RecoveryTimes}
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(RTimeArray,
                       all = TRUE,
                       decDigits = 0),
        format = "latex",
        align = "c",
...
This 'large' $P$-value suggests that a sample mean of $13.29$ days could reasonably have been observed just through sampling variation: there is no evidence to support the alternative hypothesis $H_1$.
...
> Very strong evidence exists in the sample ($t = 11.78$; one-tailed $P < 0.001$) that the population mean IQ in students at Griffith University
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(SleepArray,
                      all = TRUE,
                      decDigits = c(0, 2, 2)),
        format = "latex",
        align = "c",
...
The packaging claimed that the Fun Size bars weigh $12$\ g (on average).
...
if( knitr::is_latex_output() ) {
  T1 <- kable( rbind( surroundMaths(head(QC[1:2], 5),
                                    all = TRUE,
                                    decDigits = 2),
                      c("$\\vdots$", "$\\vdots$") ),
        format = "latex",
    ...
    T2 <- kable( rbind( surroundMaths(head(QC[, 3:4], 5),
                                      all = TRUE,
                                      decDigits = 2),
                         c("$\\vdots$", "$\\vdots$") ),
        format = "latex",
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(DataAndTargets,
                       all = TRUE,
                       decDigits = c(2, 3, 3, 3) ),
        format = "latex",
