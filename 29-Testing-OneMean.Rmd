...
\begin{wrapfigure}[9]{R}{.20\textwidth}
  \begin{center}
...
The average internal body temperature is commonly believed to be $\mu = 37.0^\circ\text{C}$, based on data over 150 years old [@data:Wunderlich:BodyTemp].
...
Since the sample mean $\bar{x}$ is likely to be different for every sample (*sampling variation*), the *sampling distribution* of $\bar{x}$ across all possible samples needs to be described.
Because every sample is different,  $\bar{x}$ will varies, and the *sample* mean $\bar{x}$ probably won't be exactly $37.0^\circ\text{C}$, *even if* $\mu$ is $37.0^\circ\text{C}$.
Two broad reasons could explain why:

1. The *population* mean body temperature *is* $37.0^\circ\text{C}$,  but $\bar{x}$ isn't exactly $37.0^\circ\text{C}$ due to sampling variation; 
   or
...
Data to answer this RQ are available from a comprehensive American study [@data:Shoemaker1996:Temperature]
`r if (knitr::is_latex_output()) {
   '(Table \\@ref(tab:DataBodyTemp)).'
} else {
   '(data below).'
}`
Summarising the data is important, because the data are the means by which the RQ is answered (Fig. \@ref(fig:BodyTempHist));  the internal body temperature of individuals varies from person to person: this is *natural variation*.
...
The question is *why*: can the difference reasonably be explained by sampling variation because every sample produced a different value of $\bar{x}$, or not?
...
```{r BodyTempSamplingDist, echo=FALSE, fig.cap="The distribution of sample mean body temperatures, if the population mean is $37^\\circ$C and $n = 130$.  The grey vertical lines are 1, 2 and 3 standard deviations from the mean.", fig.align="center", fig.height=2.5, fig.width=5}
mn <- 37.0
...
The sampling distributions describes how the sample mean varies; that is, what to *expect* from the sample mean, after *assuming** $\mu = 37.0^\circ\text{C}$.
...
```{r BodyTempSamplingDistT,  echo=FALSE, fig.cap="The sample mean of $\\bar{x}=36.8041^\\circ$C is very unlikely to have been observed if the poulation mean really was $37^\\circ$C, and $n=130$", fig.align="center", fig.width=6, fig.height=2.75}
mn <- 37.0
#These taken from Shoemaker's JSE data file
s <- 0.40732
n <- 130
...
$z$-scores are calculated using *no sample information* (as with a test for proportions (Chap. \@ref(TestOneProportion)), while $t$-scores are calculated *using some sample information*.
$t$-scores are more commonly used than $z$-scores, because almost always the population standard deviation is unknown, and the sample standard deviation is used instead.
\smallskip
...
As seen in Sect. \@ref(OnePropTestP), a $P$-value is used to quantify how unusual the computed $t$-score (or $z$-score) is.
The $P$-value measures how unusual an observation is (if $H_0$ is true), and can be *approximated* using the [68--95--99.7 rule](#def:EmpiricalRule) and a diagram (Sect. \@ref(ApproxP)).
Commonly, software is used for test involving one mean (Sect. \@ref(SoftwareP)).


### Approximate $P$-values using the 68--95--99.7 rule {#ApproxP}

Using the ideas in Sect. \@ref(OnePropTestP), the [68--95--99.7 rule](#def:EmpiricalRule) and a diagram (Sect. \@ref(ApproxP)) can be used to *approximate* a $P$-value.
