...
The population mean internal body temperature of the population will never be known: we would need to measure the internal body temperature of every person alive, and even those not yet born.
The researchers studied a *sample*.

We define the parameter as $\mu$, the population mean internal body temperature (in ^o^C).
...
$$
   \text{$H_0$:\ } \mu = 37.0.
$$
The RQ asks if the *population* mean internal body temperate $\mu$ is *equal* to $37.0$^o^C, or not.
...
**Step\ 2** of the decision-making process is to describe the sampling distribution: the values of the sample mean that might be observed if $\mu = 37$. 
The sample mean *varies* from sample to sample: *sampling variation* exists and is described by the *sampling distribution*.
The sample statistic often varies with a normal distribution (whose standard deviation is called the *standard error*).
...
Hence, if\ $\mu$ really was\ $37.0$, the values of the sample means across all possible samples can be described using:
...
## The process of hypotheses testing: the test statistic $t$ {#Tscores}
...
One of two explanations could explain the discrepancy:

1. The *population* mean body temperature *is*\ $37.0$^o^C.\spacex
   However,\ $\bar{x}$ isn't exactly $37.0$ due to sampling variation; 
2. The *population* mean body temperature *is not*\ $37.0$^o^C.\spacex
   The *sample* mean body temperature reflects this.

These hypotheses can be written more formally as:

1. The *null hypothesis*\ ($H_0$): $\mu = 37.0$; and
2. The *alternative hypothesis*\ ($H_1$): $\mu \ne 37.0$.

The RQ asks if\ $\mu$ is\ $37.0$ or some other value (either smaller or larger than\ $37.0$; i.e., the temperature has *changed*).
...
To answer this RQ, data were collated by @data:Shoemaker1996:Temperature
...
The *observed* value of\ $\bar{x}$ is $\bar{x} = 36.8052$^o^C.\spacex
How likely is it that such a value could occur in our sample by chance (i.e., by sampling variation)?

The value of the observed sample mean can be located on the sampling distribution (Fig.\ \@ref(fig:BodyTempSamplingDistT)).
The value $\bar{x} = 36.8052$^o^C is *extremely* small: a sample mean this low is very unlikely to occur in any sample of $n = 130$ when $\mu = 37.0$.
...
$$
This is like a $z$-score: it measures the number of standard deviations that a value is from the mean.
...
> The sample provides very strong evidence ($t = -5.45$; two-tailed $P < 0.001$) that the population mean body temperature is *not*\ $37.0$^o^C ($\bar{x} = 36.81$; $n = 130$; $95$%\ CI\ from\ $36.73$^o^C to\ $36.88$^o^C).
...
* Some *sample summary information* (including a CI, using details in Chap.\ \@ref(OneMeanConfInterval)): $\bar{x} = 36.81$; $n = 130$; $95$%\ CI from\ $36.73$^o^C to\ $36.88$^o^C.
...
Since the alternative hypothesis asks if the mean is *larger* than\ $100$, the $P$-value will be in the tail corresponding to *larger* IQ scores (i.e., to the right).
...
The usual engineering recommendation is that the safe gap between travelling vehicles in traffic (a 'headway') is *at least* $1.9$\ s (often conveniently rounded to\ $2$\ s when communicated to the public).
...
1. Write a conclusion. 
   (The CI was found in Exercise\ \@ref(exr:CIPizzas).)
...
**4** The population mean headway is *less than*\ $1.9$\ s.
**5.** $0.34$.
**6.** Big
