...
*Assume* that $\mu = 37.0$ (the accepted value); then, we determine if compelling evidence exists to contradict this claim.
The RQ is:

> Is the *population* mean internal body temperature $37.0$^o^C?


## Statistical hypotheses and notation

The decision-making process begins by *assuming* the null hypothesis is true: that $\mu = 37.0$.
...
1. The *population* mean body temperature *is* $37.0$^o^C.
   However, $\bar{x}$ isn't exactly $37.0$ due to sampling variation; 
2. The *population* mean body temperature *is not* $37.0$^o^C.
...
## Computing the value of the test statistic: $t$-tests {#Tscores}
...
The *observed* value of $\bar{x}$ is $\bar{x} = 36.8052$^o^C.
...
```{r BodyTempSamplingDistT, fig.cap="The sample mean of $\\bar{x} = 36.8041^\\circ$C is very unlikely to have been observed in any sample of size $n = 130$, if the population mean really was $37.0^\\circ$C. The standard deviation of this normal distribution is $\\text{s.e.}(\\bar{x}) = 0.035724$.", fig.align="center", fig.width=8, fig.height=2.75, out.width='85%'}
mn <- 37.0
#These taken from Shoemaker's JSE data file
s <- 0.40732
n <- 130
...
One RQ was whether the mean speed of cars on one particular road was the posted speed limit of $90$\ km.h^$-1$^, or whether it was *higher*.
...
They found the mean temperature was $56.6$^o^C, with a standard error of $0.4$^o^C. 
...
1. Use the software output (Fig.\ \@ref(fig:CherryRipes201720182019)) to perform a hypothesis test to determine if the mean weight really is $14$\ g or not.
...
The recommendation is for adolescents to have at least $8$\ hrs of sleep each night.

In the sample of $n = 3\ 972$ individuals, the mean amount of sleep on schools days was $6$\ hrs $43$\ mins (i.e., $403$\ mins), with a standard deviation of $87$\ mins.
On non-school days, the mean amount of sleep was $8$\ hrs $38$\ mins (i.e., $518$\ mins), with a standard deviation of $98$\ mins.

Do Norwegian adolescents appear to meet the guidelines of having '*at least* $8$\ hrs' sleep each night on school days? On non-school days?
:::
