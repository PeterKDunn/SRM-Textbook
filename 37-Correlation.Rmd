...
In this chapter (and the next), the relationship between two quantitative variables is studied *when that relationship is approximately linear* (correlational RQs).
...
A relationship between the variables seems evident.
...
However, only one of the countless possible samples of deer have been studied, so potentially that relationship may not actually be present in the population.
...
A correlational RQ can be asked about the relationship between the variables in the *population*, as measured by the unknown *population* correlation coefficient:
...
```{r RedDeerCorrelationjamovi2, fig.cap="jamovi output for the red-deer data", fig.align="center", out.width='45%'}
...
A scatterplot of $n = 32$ observations (Fig.\ \@ref(fig:CorrelationRemovalEfficiency), left panel) suggests an approximately linear relationship [@DevoreBerk2007].
