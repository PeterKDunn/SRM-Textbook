...
: The *alternative hypothesis* proposes that any difference, change or relationship observed in the sample
is because a difference, change or relationship exists the *population* 
(that is, the difference, change or relationship cannot be explained by sampling variation).
...
[Bias](#def:Bias)
: *Bias* is the tendency of a sample to over- or under-estimate a population quantity. 
...
is when the effects of one treatment carry over into the effects of the next treatment,
for experimental studies (Sect. \@ref(CarryOverEffectExperimental))
or observational studies (Sect. \@ref(CarryOverEffectObservational)).


[Categorical data](#def:QuantitativeData)
: *Cateogorical data* is not *mathematically* numerical data:
it consists of categories or labels (even if those labels are numbers). 
In this book,
categorical data is called *qualitative* data.



[Cheating](#def:Cheating)
: We give the USC definition of *cheating*^[From https://www.usc.edu.au/learn/student-support/academic-and-study-support/online-study-resources/academic-integrity]:
*Cheating* is acting dishonestly to gain an unfair advantage. 
At university, cheating may occur in exams, lab tests and other forms of assessment.
...
: A sample where the population is split into a large number
...
: We give the USC definition of *collusion*^[From https://www.usc.edu.au/learn/student-support/academic-and-study-support/online-study-resources/academic-integrity]:
*Collusion* involves two or more people producing work for which one person will claim ownership. 
At university, collusion happens if you give or receive help 
in completing any form of individual assessment such as assignments and exams.



[Comparison](#def:Comparison)
: The *comparison* in the RQ identifies the small number of distinct subsets of the population between which the outcome is being compared.
...
If we computed the 95\% confidence interval (or CI) from each sample,
about 95\% of the CIs would contain the [*statistic*](#def:Statistic) of interest.
This interval is called a *confidence interval*.  
Alternatively,
the CI can be seen as the range of plausible values for the [parameter](#def:Parameter)
that may have produced the observed sample [statistic](#def:Statistic).
...
: *Confounding* is when a third variable influences the relationship between the response and explanatory variable. 
...
[Data](#def:Data)
: *Data* refers items of information obtained from a study
(such as height of seedlings, or the type of medication given).


[Data set](#def:DataSet)
: A *data set*
refers to a collection of data from a study.
...
if the study methods, materials and context approximate the real situation being studied.
...
: In an *experimental study* (or an *experiment*), the researchers intervene to control the values of the explanatory variables (C) 
that are applied to the individuals. The researchers allocate treatments (i.e., apply the intervention). 


 [Experimenter effect](#def:ObserverEffect)
: The *experimenter effect*
is another name for the *observer effect* in experimental studies
(that is,
...
: An *explanatory variable* is a variable of interest from the individuals in the study which (potentially) causes changes in, 
or is related to, the response variable. 


[External validity](#def:ExternalValidity)
: *Externally validity* refers to the ability to
generalise the results to other groups in the population apart from the sample studied.
...
[Fraud](#def:Fraud)
: We give the USC definition of *fraud*^[From https://www.usc.edu.au/learn/student-support/academic-and-study-support/online-study-resources/academic-integrity]:
*Fraud* can occur by:
...
in experimental studies (Sect. \@ref(HawthorneEffectExperimental))
or observational studies (Sect. \@ref(HawthorneEffectObservational)).
...
In this book,
we have looked at some [specific hypothesis tests](#SelectTest) (hundreds exist:  @kanji2006100!):
...
: Two events are *independent* if the probability of one event doesn't change
depending on whether or not other event has happened.
...
: An *intervention* is a comparison or connection that the researchers impose upon those in the study, intending to change the outcome. 
...
: A sample where individuals are selected, based on the researchers' judgement,
...
(that is, is a confounding variable), 
...
: A sample where large groups are selected using a *simple random sample*,
...
: A *normal distribution* is symmetrical distribution,
with most values in the centre of the distribution.
...
: The *null hypothesis* proposes that any difference, change or relationship observed in the sample
can be explained by sampling variation
(that is, no difference, change or relationship exists the *population*).


[Observational study](#def:ObservationalStudy)
: An *observational study* is one where the researchers do not impose the comparison or connection upon those in the study 
...
in experimental studies (Sect. \@ref(ObserverEffectExperimental))
or observational studies (Sect. \@ref(ObserverEffectExperimental)).


[Odds](#def:Odds)
: The *odds* of some event is the proportion (or percentage, or number) of times that an event happens, 
...
numerically summarising the entire population (or subsets of the population).


 
[$P$-value](#def:Pvalue)
: A $P$-value is the likelihood of observing the sample results (or something even more extreme) 
over repeated sampling, 
under the assumption that the null hypothesis about the population is true.



[Parameter](#def:Parameter)
: A *parameter* is a number describing some feature of a population,
and is usually estimated by a [statistic](#def:Statistic).
...
without having received the treatment or condition,
in experimental studies (Sect. \@ref(PlaceboEffectObservational))
or observational studies (Sect. \@ref(PlaceboEffectObservational)).



[Plagiarism](#def:Plagiarism)
: We give the USC definition of *plagiarism*^[From https://www.usc.edu.au/learn/student-support/academic-and-study-support/online-study-resources/academic-integrity]:
*Plagiarism* occurs when a student uses---without acknowledging---the work of other people. 
In a sense, plagiarism is a form of intellectual theft in which 
ideas, words, information or images are taken without attribution. 
...
* Accidental---for instance, if a student has poor notetaking skills or doesn't know how to reference correctly, 
    and they inadvertently present someone else's ideas and words as their own.


[Population](#def:Population)
: The *population* is the group of individuals (or cases, or subjects if the individuals are people) 
from which the total set of observations of interest could be made,
and to which the results will (hopefully) generalise.

[Precision](#PrecisionAccuracy)
: *Precision* refers to how likely it is that the sample values will be
similar or close together, and not vary much from sample to sample.
...
it consists of categories or labels (even if those labels are numbers). Also called *categorical* data.


[Quantitative data](#def:QualitativeData)
: *Quantitative data* is *mathematically* numerical data:
the numbers themselves have numerical meaning, and it makes sense to be able to perform mathematical operation on them.
...
* The first quartile $Q_1$: A value that
...
* The second quartile $Q_2$: A value that
...
: In a *quasi-experiment*, the researchers (a) allocate treatments to groups of individuals (i.e., do not decide the values of the Comparison/Connection used), 
but (b) do not determine who or what is in those groups.
...
: A representative sample is one where 
the individuals *in* the sample
are not likely to be different the individuals *not in* the sample,
...
[Sample](#def:Sample)
: A *sample* is a subset of the population of interest which is actually studied,
and from which information is collected.


[Sample space](#def:SampleSpace)
: The *sample space* 
is a list of all possible and distinct results after administering a procedure 
whose result is unknown beforehand. is a list of the results after administering a procedure 
...
showing how its value varies from one sample to sample.
...
: *Sampling variation* refers to 
how much a sample estimate (a [*statistic*](#def:Statistic)) is likely to vary from sample to sample,
because each sample is different.


[Scale data](#def:QualitativeData)
: *Scale data* is *mathematically* numerical data:
the numbers themselves have numerical meaning, and it makes sense to be able to perform mathematical operation on them.
Most data that are counted or measured will be quantitative. 
In this book,
scale data is called *quantitative data*.
...
any observation more than three standard deviations from the mean can be considered an outlier.
...
: A sample where the population is split into a small number of large (usually homogeneous) groups called *strata*,
then cases are selected using a *simple random sample* from *each* stratum.



[Statistic](#def:Statistic)
: A *statistic* is a number describing some feature of a sample
(to estimate a population [parameter](#def:Parameter)).
...
various factors are incorporated, perhaps subjectively, to determine the probability of an event.



[Systematic sampling](#SystematicSampling)
: A sample where the first case is *randomly* selected;
then, every $n$th individual is selected.


[Treatments](#def:Treatments)
: *Treatments* are the conditions of interest that those in the study can be exposed to (in the comparison/connection). In experiments, treatments are imposed by researchers. 


[True experiment](#def:TrueExperiment)
: In a true experiment, the researchers (a) allocate treatments to groups of individuals (i.e., decide the values of the Comparison/Connection used), 
and (b) determine who or what is in those groups.
...
: The *unit of analysis* is the 'who' or 'what' about which generalizations and conclusions are made; 
the smallest independent 'who' or 'what' for which information is analysed. Units of analysis should not typically share a common underlying source. 


[Unstandardizing formula](#def:UnstandardizingFormula)
: When the $z$-score is known,
the *unstandardizing formula* determines the corresponding value of the observation $x$.
...
: A *variable* is a single aspect or characteristic associated with each of a group of individuals under consideration, 
