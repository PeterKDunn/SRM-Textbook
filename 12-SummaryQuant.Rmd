...
`r if( knitr::is_html_output() ) {
  'Fig.\\ \\@ref(fig:CycloneDataHTML)' } else {
  'Table\\ \\@ref(tab:CycloneDataLATEX)'
}`
show the number of severe cyclones (also called hurricanes or typhoons) in the Australian region, for each year from 16969 to 2005.

A frequency table can be constructed by binning each discrete value individually (Table\ \@ref(tab:CycloneDataLATEX), left table) or grouped in pairs (Table\ \@ref(tab:CycloneDataLATEX), right table).
:::
...
To display the distribution, the weights can be grouped into clearly-defined weight intervals (Table\ \@ref(tab:BabyBoomTable), left column).
...
A histogram can be constructed from either frequency table in Table\ \@ref(tab:CycloneFreqTables); see
`r if( knitr::is_html_output() ) {
  'below.'
} else {
  'Fig.\\ \\@ref(fig:BBHist1).'
...
```{r BBHistTwo, fig.align="center", fig.width=7.0, fig.height=2.15, out.width='100%', fig.cap="Histograms can be constructed in different ways to manage observations on the boundary of bins. Left: boundary values counted in the higher box. Centre: boundary values counted in the lower box. Right: define boundaries with one more decimal places that the data." }
par( mfrow = c(1, 3))
...
```{r HistBrainFreeze, fig.align="center", fig.width=10.5, fig.height=2.75, out.width='60%', fig.cap="Histogram of the duration of brain-freeze symptoms after drinking ice water. Boundary observations are counted in the lower box." }
par( mfrow = c(1, 2))
...
```{r GorillasStem, fig.align="center", fig.width=7.25, fig.height=2.5, out.width='90%', fig.cap='The stemplot (right) for the gorilla chest-beating data (left)' }
if (knitr::is_latex_output()){
...
are *jittered*\index{Overplotting!jittering} (placed with some added randomness in the vertical direction)\index{Overplotting!stacking} or *stacked* (plotted above each other).
...
```{r FriesBabiesPDF, fig.cap="A dot chart of the baby-weight data (left panel), where similar observations are jittered. Gorilla chest-beating rates (right panel), where similar observations are stacked.", fig.align="center", fig.width=9, fig.height=2.15, out.width='95%'}
if (knitr::is_latex_output()){
...
The population mean distance is estimated by the sampe mean as $\bar{x} = 532/11 = 48.4$\ cm.
:::
...
* influenced by skewness\index{Skewness} and outliers,\index{Outiers} like the mean.
...
Describe the histogram in Fig.\ \@ref(fig:BBHist44BrainFreeze) (right panel).
:::
