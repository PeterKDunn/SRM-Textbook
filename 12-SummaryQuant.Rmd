...
The graphs in this section are appropriate for *continuous* quantitative data, but may sometimes be useful for discrete quantitative data if many values are possible.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The axis displaying the counts (or percentages) should *start from zero*, since the height of the bars visually implies the frequency of those observations.
:::
...
::: {.example #Histograms name="Histograms (discrete data)"}
...
The different bin sizes change the appearance of the distribution.
:::
...
When an observation occurs on a boundary between the boxes, software usually (but not universally) places it in the *higher* box ($2.5$\ kg is in the '$2.5$ to $3.0$\ kg' box, not the '$2.0$ to $2.5$\ kg' box).
That is, boxes *include* the lower limit, but *exclude* the upper limit.
...
```{r BBHist44BrainFreeze, fig.align="center", fig.width=9, fig.height=2.75, out.width='100%', fig.cap="Left: The final histogram for the baby-birth data. Right: Duration of brain-freeze symptoms after drinking ice water" }
par( mfrow = c(1, 2))
...
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:BimodalFaithfulHistoChangeBins), left panel) shows the shape of the distribution depends on bin width.'
} else {
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The axis displaying the counts (or percentages) should *start from zero*, since the distance of the dots from the axis visually implies the frequency of those observations.
:::
