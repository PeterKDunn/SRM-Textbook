...
```{r, child = if (knitr::is_html_output()) {'./introductions/12-SummariseQuant-HTML.Rmd'} else {'./introductions/12-SummariseQuant-LaTeX.Rmd'}}
```
...
Sometimes, quantitative data with very few recorded values are best graphed using different graphs (Sect.\ \@ref(QualitativeGraphs)).
...
```{r GorillasStem, fig.align="center", fig.width=5, fig.cap="The gorilla chest-beating data, for young gorillas (aged under 20). The data are on the left, and the stemplot on the right.", fig.width=10, fig.height=2.75, out.width='100%' }
if (knitr::is_latex_output()){
  #KRILL
...
These can be described in rough terms, or specifically using numerical quantities (as in following sections).
...
Describe the histogram in Fig.\ \@ref(fig:BBHist44BrainFreeze) (right panel), the brain freeze durations.\label{thinkBox:DescribeBrainFreeze}
...
Measurements were taken from 14 young gorillas (Fig.\ \@ref(fig:FriesBabiesPDF), left panel).
:::
...
Consider the daily streamflow at the Mary River (Bellbird Creek) during February again (Example\ \@ref(exm:Averages)):
...
With a sample of size $14$, the data cannot be split into exactly into quartiles.)
The IQR is $Q_3 - Q_1$ = $2.9 - 1.5 = 1.4$,
`r if (knitr::is_latex_output()) {
   'as shown in Fig.\\ \\@ref(fig:IQRKrill).'
} else {
...
```{r IQRKrill, fig.align="center", fig.width=8.75, fig.height=3.5, fig.cap="Left: A boxplot for the chest-beating data for young gorillas", out.width='100%' }
if (knitr::is_latex_output()){
...
```{r SDOutliers, fig.align="center", fig.width=9, fig.height = 3.5, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data", out.width = '99%' }
...
Three observations are identified as outliers using the IQR rule.
:::
...
A large number of households are organised by women in their late 50s.
...
rownames(WaterAccessTable1) <- c("Woman's age (years)",
                                 "Household size",
                                 "Children under $5$")
colnames(WaterAccessTable1) <- c("Sample size",
                                 "Mean",
                                 "Median",
                                 "Std. dev.",
                                 "IQR")
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(WaterAccessTable1, 
                      decDigits = c(0, 1, 1, 2, 2)),
        format = "latex",
        longtable = FALSE,
        escape = FALSE,
...
```{r WaterAcessQuant, fig.align="center", fig.cap="The age of the women, the number of people in the household, and the number of children in the household for the water-access study. (Some data are missing.)", out.width='100%', fig.width=8, fig.height=2.75}
...
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
11. The units of the standard deviation, IQR and the range are the same as for the original data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
:::
...
```{r DeathAgeNHANESHist, fig.cap="Left: Histograms of age at death for female Australians in 2012. Right: The histogram of the direct HDL cholesterol concentration from the NHANES study (large outliers exist but are hard to see).", fig.align="center", fig.width=8.75, fig.height=3.25, out.width='100%'}
par(mfrow = c(1, 2))
...
The histogram of the direct HDL cholesterol concentration from the NHANES study is shown  in Fig.\ \@ref(fig:DeathAgeNHANESHist) (right panel).
...
3. Do you think the weights meet the target weight, on average?
:::
...
knitr::kable( surroundMaths(FriesTab,
                            decDigits = 1),
              format = "latex",
              longtable = FALSE,
              booktabs = TRUE,
...
1. Draw a stemplot, using the numbers as (say) $8.0$, and using the decimal places as the leaves.
