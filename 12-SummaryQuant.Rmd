...
Quantitative data can be summarised by displaying the *distribution* of the data.
...
The distribution of quantitative data can be summarised by numerically computing the average value (Sect.\ \@ref(ComputeAverage)), computing the amount of variation (Sect.\ \@ref(Variation)), describing the shape (Sect.\ \@ref(SummaryShape)), and identifying outliers (Sect.\ \@ref(SummaryOutliers)).



## Frequency tables for quantitative data {#QuantFreqTable}

Quantitative data can be collated in a *frequency table*, or a *tabular summary*, by grouping the variables into appropriate intervals.
...
The baby weights can be summarised as the number of babies within increasing weight categories (Table\ \@ref(tab:BabyBoomTable)).
...
When the categories are of equal width, histograms are essentially a picture of the frequency table (such as Table\ \@ref(tab:BabyBoomTable)).
...
`r if (knitr::is_latex_output()) {
   'in Fig.\\ \\@ref(fig:BimodalFaithfulHistoChangeBins) (left panel).'
} else {
   'below.'
}`
`r if (knitr::is_html_output()) '<!--'`
The effect of changing the bin width can be seen in Fig. \@ref(fig:BimodalFaithfulHistoChangeBins).
`r if (knitr::is_html_output()) '-->'`
`r if (knitr::is_latex_output()) '<!--'`
Try changing the number of bins in the interaction below to see the impact.
`r if (knitr::is_latex_output()) '-->'`
:::
...
```{r BBStem44,  fig.align="center", fig.cap="The final stemplot for the baby-weight data. The data are on the left; the final stemplot on the right.", fig.width=9, fig.height=3, out.width='100%' }
if (knitr::is_latex_output()){
  Wts <- sort(BB$Weight)
...
* Place the next smallest unit (e.g., first decimal of a kilogram) on the right (leaves).
...
In this chapter, methods for summarising quantitative variables numerically are described.
...
(When $n$ is odd, the median *may* or *may not* be included in each of these halves.
We decide *not* to include the median in each half in this situation.)
Hence:
...
An example us given in the next section.
