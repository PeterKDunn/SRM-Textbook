...
Except for very small amounts of data, understanding the data is difficult without using a summary.
Quantitative data can be summarised by displaying the *distribution* of the data, and then describing the average value, the amount of variation, the shape, and identifying outliers.

The *distribution* of the data refers to the values taken by the variable, and how often those values appear.
...
Quantitative data can be collated in a table, or a *tabular summary*, by grouping the variables into appropriate intervals.
...
The data give the weights (in kg) of $44$ babies born in a Brisbane hospital on one day [@mypapers:Dunn:dataset:1999; @data:Steele:BabyBoom].
...
The baby weights can be presented as the number of babies within increasing weight categories (Table\ \@ref(tab:BabyBoomTable)).
The percentages are also added; for example, the percentage of birth over $4.0$\ kg is $1/44 \times 100 = 2.27$%, or about $2$%. 
From the table, most babies in the sample are between $3$ and $4$\ kg at birth.
:::
...
When the categories are of equal width, histograms are essentially a picture of the table of data (such as Table\ \@ref(tab:BabyBoomTable)).
...
`r if (knitr::is_latex_output()) {
   'Figure\\ \\@ref(fig:BBHist1) shows the histogram starting to be built, and Fig.\\ \\@ref(fig:BBHist44) shows the final histogram.'
} else {
...
the weights (in kg) of babies born in a Brisbane hospital on one day [@mypapers:Dunn:dataset:1999; @data:Steele:BabyBoom].
    ...
    for (j in (1:i) ){
      if ( GYoung[j] < 1) {
        if ( j != i) points( stemcounts[1], 
                             3, 
                             pch = as.character(GYoung[j]))
        if (j == i ) text( stemcounts[1], 3, 
                           as.character(GYoung[j]), 
                           col = plotDark, 
                           cex = 1.5, 
                           font = 2)
        stemcounts[1] <- stemcounts[1] + 1
      ...
      if ( (GYoung[j] >= 1) & (GYoung[j] < 2) ) {
        if (j != i ) points( stemcounts[2], 
                             2, 
                             pch = as.character(GYoung[j] - 1) )
        if (j == i ) text( stemcounts[2], 
                           2, 
                           as.character(GYoung[j] - 1), 
                           col = plotDark, 
                           cex = 1.5, 
                           font = 2)
        stemcounts[2] <- stemcounts[2] + 1
      ...
      if ( (GYoung[j] >= 2) & (GYoung[j] < 3) ) {
        if (j != i ) points( stemcounts[3], 
                             1, 
                             pch = as.character(GYoung[j] - 2))
        if (j == i ) text( stemcounts[3], 
                           1, 
                           as.character(GYoung[j] - 2), 
                           col = plotDark, 
                           cex = 1.5, 
                           font = 2)
        stemcounts[3] <- stemcounts[3] + 1
      ...
      if ( (GYoung[j] >= 3) & (GYoung[j] < 4) ) {
        ...
        if (j == i ) text( stemcounts[4], 
                           ...
                           cex = 1.5, 
                           font = 2)
        stemcounts[3] <- stemcounts[4] + 1
      }
      if ( GYoung[j] >= 4 ) {
        if (j != i ) points( stemcounts[5], 
                             0, 
                             pch = as.character(GYoung[j] - 3))
        if (j == i ) text( stemcounts[5], 
                           0, 
                           substr(as.character(GYoung[j]), 3, 3),
                           col = plotDark, 
                           cex = 1.5, 
                           font = 2)
...
```{r KrillStem44, fig.align="center", fig.width=5, fig.cap="The gorilla chest-beating data, for young gorillas (aged under 20). The data are on the left, and the stemplot on the right.", fig.width=10, fig.height=2.75, out.width='100%' }
if (knitr::is_latex_output()){
  #KRILL
  ...
  par(mfrow = c(1, 2),
      mar = c(3.85, 3, 4, 1) + 0.1)
  for (i in (length(GYoung):length(GYoung))){
    # Plot the data
    datagrid.x <- 5
    datagrid.y <- 3
    ...
    plot( expand.grid(1:datagrid.x, 
          ...
          ylim = c(0, 3.3),
          xlim = c(1, 7),
          main = "The chest beat rates per 10 hrs (in order)",
          xlab = "",
          ylab = "",
          axes = FALSE)
    text(  1:datagrid.x, 
    ...
    text(  1:datagrid.x, 
           rep(1, 5), 
           c( as.character( format(GYoung[11:14], nsmall = 1) ), "") )
    ...
    plot( expand.grid(1:stemgrid.x, 0:stemgrid.y), 
          type = "n",
          xlab = "1 | 3 means 1.3 beats per 10 hours",
          ylab = "",
          main = "Stemplot of young gorillas' chest beats",
          xlim = c(0, 11),
          ylim = c(-1.2, 3.2),
          axes = FALSE)
  ...
  for (j in 1:length(GYoung) ){
    if ( GYoung[j] < 1 ) {
      points( stemcounts[1], 3, 
              pch = substr(as.character(GYoung[j]), 3, 3))
      stemcounts[1] <- stemcounts[1] + 1
    }
    ...
    if ( (GYoung[j] >= 3) & (GYoung[j] < 4) ) {
      points( stemcounts[4], 0, 
              pch = substr( as.character(format(GYoung[j], nsmall = 1)), 3, 3))
      stemcounts[4] <- stemcounts[4] + 1
    }
    if ( GYoung[j] >= 4 ) {
...
(*Jittering* is another an option to avoid overplotting; see Example\ \@ref(exm:DotsChartsQuant2).)
:::


```{r FriesBabiesPDF, fig.cap="Gorilla chest-beating rates (left panel), where similar observations are stacked. A dot chart of the baby-weight data (right panel), using jittering, rather than stacking.", fig.align="center", fig.width=9, fig.height=2.5, out.width='95%'}
if (knitr::is_latex_output()){
...
These can be described in rough term, or specifically using numerical quantities (as in following sections).
...
'Average' can refer to either means, medians or other measures of centre.
...
To demonstrate ideas, consider a small dataset for answering this descriptive RQ: 'For gorillas aged under 20, what is the average chest-beating rate?'
...
Measurements were taken from milk from 14 young gorillas (shown in Fig.\ \@ref(fig:FriesBabiesPDF), left panel).
:::
...
```{r MeansFigLATEX, fig.align="center", fig.width=4.25, fig.height=4.5,fig.cap="Two ways to understand the mean. Left: The mean is the balance point of the data; right: the mean is the value such that the positive and negative distances sum to zero", out.width = c('51%', '3%', '44%'), fig.show='hold' }
...
For the chest-beating data (Fig.\ \@ref(fig:FriesBabiesPDF), left panel), an *estimate* of the population mean (i.e., the sample mean) chest-beating rate is found by summing all $n = 14$ observations and dividing by $n$:  
\[
   \overline{x} 
   = \frac{\sum x}{n} 
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For the chest-beating data (Fig.\ \@ref(fig:FriesBabiesPDF), left panel), what is the value of $\mu$?\label{thinkBox:ValueOfMu}
...
The *sample mean* rate can be given to two decimal places: $\bar{x} = 5.25$%.
:::
...
To find a sample median for the chest-beating data (Fig.\ \@ref(fig:FriesBabiesPDF), left panel)), first arrange the data *in numerical order* (Table\ \@ref(tab:GYoungSorted)).
...
The sample median is between $1.7$ (ordered observation\ $7$) and $1.7$ (ordered observation\ $8$).
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(GYoung.sort,
                      decDigits = 1),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
if( knitr::is_html_output() ) {
  ...
  if ( knitr::is_html_output()) {
   add_header_above(out,
                    header = c("Chest-beating rate, per 10 h" = 14), 
                    bold = TRUE, 
		    align = "c")
  } else {
    out
...
* If the sample size $n$ is *even* (such as the chest-beating example), the median is halfway between the two middle numbers, when the observations are ordered. 
...
The *mean* daily streamflow is $1\ 123$\ ML, and the *median* daily streamflow is $146.1$\ ML.
Which is 'best' for measuring the average streamflow?

A dot chart of the daily streamflow (Fig.\ \@ref(fig:DailyStreamflow), using jittering) shows that the data are *very* highly right-skewed, with many *very* large outliers (presumably during flood events); the extreme outliers are clear in the tabular summary too (Table\ \@ref(tab:DailyStreamflowTab)).
About $86$% of the observations are *less* than the mean. but $50$% the values are less than the median (by definition).
...
\]
The sample median percentage butterfat is $1.7$ beats per $10$\ h, with a *range* of $3.7$ beats per $10$\ h.
:::
...
if (knitr::is_html_output()) {
  ...
  Beat.quantiles <- quantile(GYoung)
out <- boxplot( list(Older = GYoung), 
...
text( 1.53, Beat.quantiles[5], 
      "Max. value", 
      pos = 1, 
      cex = 0.75)
text( 1.53, Beat.quantiles[4], 
      "75% smaller than this", 
      pos = 3, 
      cex = 0.75)
text( 1.53, Beat.quantiles[3], 
      "Median", 
      pos = 3, 
      cex = 0.75)
text( 1.53, Beat.quantiles[2], 
      "25% smaller than this", 
      pos = 3, 
      cex = 0.75)
text( 1.53, Beat.quantiles[1], 
      "Min. value", 
      pos = 1, 
      cex = 0.75)

text( 0.65, Beat.quantiles[4], 
      expression(italic(Q)[3]), 
      pos = 3, 
      cex = 0.75)
text( 0.65, Beat.quantiles[3], 
      expression(italic(Q)[2]), 
      pos = 3, 
      cex = 0.75)
text( 0.65, Beat.quantiles[2], 
...
For the chest-beating data (Table\ \@ref(tab:GYoungSorted)):

*  $75$% of the observations are smaller than about $2.9$.
   This is $Q_3$, the *third quartile*.
*  $50$% of the observations are smaller than about $1.7$.
   This is $Q_2$, the *second quartile* or the *median*.
*  $25$% of the observations are smaller than about $1.5$.
   This is $Q_1$, the *first quartile*.

(We say 'about' these values, as exact values can only be found when the sample size is a multiple of $4$.
With a sample of size $14, as we have here, the data cannot be split into exactly into quantiles.)
The IQR is $Q_3 - Q_1$ = $2.9 - 1.5 = 1.4$,
`r if (knitr::is_latex_output()) {
   'as shown in Fig.\\ \\@ref(fig:IQRKrill) (right panel).'
} else {
...
```{r IQRKrill, fig.align="center", fig.width=8.75, fig.height=3.5, fig.cap="Left: A boxplot for the krill-egg data (Treatment group); right: Computing the IQR for the same data", out.width='100%' }
if (knitr::is_latex_output()){
  ...
  stripchart( GYoung,
              method = "jitter",
              main = "A boxplot of the chest-beating rates\nof young gorillas (under 20 years)",
              las = 1,
              pch = 19,
              #col = "grey",
              axes = FALSE,
...
#       labels = rep("", 5) )
  ...
  #             xlim = c(0.2, 1.75),
  #             ylim = c(0, 5),
  #             ylab = "Chest-beats per 10 h")
  # axis(side = 2, 
  #      las = 1)
  # abline( h = Beat.quantiles[ c(2, 4)], 
  #         col = "grey",
  #         lty = 2,
  #         lwd = 1)
  # 
  ...
  # 
  # axis(side = 4, # This places the tick marks
  #      at = Beat.quantiles[ c(2, 4)],
  #      labels = rep("", 2) )
  # mtext(Beat.quantiles[ c(2, 4)],
  #       side = 4,
  #       at = Beat.quantiles[c(2, 4)],
  #       las = 1,
  #       line = 1,
...
By this definition, the first quartile $Q_1$ is the $25$th percentile, the second quartile $Q_2$ is the $50$th percentile (and the median), and the third quartile $Q_3$ is the $75$th percentile.
...
```{r PermBoxHist, fig.align="center", fig.cap="A dot chart and histogram for the permeability data", fig.width=5, fig.height=4}
#par (mfrow = c(2, 1))
...
* *The outlier is clearly a mistake* (e.g., an age of $222$):
  If the mistake cannot be fixed (e.g., the completed survey form is no longer available), the observation can be *deleted*.
...
::: {.exercise}
Perhaps the energy consumption data, before and after insulation. Use of on e those (of the diffs).
Or place in WITHIN chapter; could do bu hand.
:::
