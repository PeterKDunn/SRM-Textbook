...
Two different groups of snakes are studied (a between-individuals comparison\index{Comparison!between individuals}), and the data are shown 
...
For the garter-snake data, the samples sizes exceed $25$ ($41$ and $35$), so the test is statistically valid.
...
$$
(Recall that we initially assume that $\mu_B - \mu_A = 0$ from the null hypothesis.)
...
> Moderate evidence exists in the sample ($t = 1.92$; one-tailed $P = 0.030$) that mean speeds have reduced after the addition of extra signage (before: mean speed: $98.02$\ km.h^$-1$^; $n = 38$; standard deviation: $13.19$\ km.h^$-1$^; after:  mean speed: $92.34$\ km.h^$-1$^; $n = 41$; standard deviation: $13.13$\ km.h^$-1$^; 95% CI for the difference: $-0.24$ to $11.6$\ km.h^$-1$^).
...
@data:barrett:echinacea studied the effectiveness of echinacea to treat the common cold, and compared the mean duration of the cold for participants treated with echinacea or a placebo.
...
```{r Dentaljamovi, fig.cap="Software output for the sugar-consumption data.", fig.align="center", out.width="90%"}
...
@data:Becker1991:BHADP compared the access to health promotion (HP) services for people with and without a disability in southwestern of the
`r if (knitr::is_latex_output()) {
  '\\USAperiod'
} else {
  'USA.'
...
Use the software output in Fig.\ \@ref(fig:BodyTempGenderjamoviHT) to perform this test and make a conclusion.
