...
Some software (e.g., jamovi) gives results from *two* similar hypothesis tests.
In this book, we will always use the second row of information: the 'Welch's $t$'\index{Welch's test} row in the output.
...
In many cases, the information in both rows are similar anyway.
:::
...
* When *both* samples have $n \ge 25$, the test is statistically valid.
  (If a sample is highly skewed, the sample size for that sample may need to be larger.)
...
One option might seem to be to compare all possible *pairs* of groups (i.e., $\mu_D$ vs $\mu_O$; $\mu_D$ vs $\mu_N$; $\mu_O$ vs $\mu_N$) using three two-sample $t$-tests.
...
Whether the mean speed reduction of $5.68$\ km.h^$-1$^ has *practical importance* is a different issue.
