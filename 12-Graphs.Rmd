...
Notice that observations are *stacked* on top of each other when  multiple observations are the same; for example, two orders of fries were $133$\ g.
...
\centering
```{r KrillEggsDoubleStem, fig.width=4.0, fig.height=3, out.width='100%'}
...
```{r CementBuildBoxplot, fig.cap="Comparing three push-out values for three dental cements", fig.align="center", fig.height=2.75, fig.width=4.0, fig.show='hold', out.width=c('49%', '49%')}
...
Their 'pre' IgE level is about $5500$ $\mu$g/L, and their 'post' IgE level is about $4500$\ $\mu$g/L, a *change* (or more descriptively, a *reduction*)  of about $1000$\ $\mu$g/L.
...
```{r NHANESplots, fig.cap="Some plots from the NHANES data. Top panels: the response variables and some extraneous variables; bottom panels: the explanatory variable and some extraneous variables", fig.align="center", fig.height=5, fig.width=6.5, out.width='100%'}
par( mfrow = c(2, 2),
...
**1.** A pie chart is **not suitable**. 
The three browsers are not *mutually exclusive* (people can use more than one) nor *exhaustive* (some people may use other browsers). 
**2.** A pie chart is **not suitable**: the percentages are not parts of a whole.
