...
```{r BBStem1, fig.align="center", fig.width=5, fig.cap="Starting to make the stemplot for the baby-weight data: the first 4 observations added" }
if (knitr::is_latex_output()){
...
```{r BBStem44,  fig.align="center", fig.width=5, fig.cap="The final stemplot for the baby-weight data" }
if (knitr::is_latex_output()){
...
```{r KrillStem44, fig.align="center", fig.width=5, fig.cap="The final stemplot for the krill data" }
if (knitr::is_latex_output()){
#KRILL
...
A study examined the serving size of fries at McDonald's [@data:Wetzel2005:McDonalds], and produced the dot chart in Fig \@ref(fig:Fries) (based on @data:Wetzel2005:McDonalds, Fig. 2).

The mass of fries is almost always *under* the target, and often substantially so.
An alternative way to look at these data is to measure the *percentage* that each serving is in relation to the target serving (Fig. \@ref(fig:FriesPC)), where 100% means the serving size was exactly the target weight.
:::



```{r Fries,fig.height=2.5, fig.width=5.5, fig.cap="Mass measurements for large orders of french fries", fig.align="center"}
...
```{r FriesPC,  fig.height=2.5, fig.width=5.5, fig.cap="Percentage variation from target mass, for large orders of french fries", fig.align="center"}
...
```{r BBHist1, fig.align="center", fig.width=5, fig.height=6, fig.cap="Starting to make the histogram for the baby-birth data: the first 6 observations added" }
if (knitr::is_latex_output()){
  Wts <-sort( round(BB$Weight/100, 0) )
  Wtskg <- Wts/10
  DP2 <- function(x){format(round(x, 1), nsmall = 1) }
...
```{r BBHist44, fig.align="center", fig.width=5, fig.height=6, fig.cap="The final histogram for the baby-birth data" }
if (knitr::is_latex_output()){
  Wts <-sort( round(BB$Weight/100, 0) )
  Wtskg <- Wts/10
  DP2 <- function(x){format(round(x, 1), nsmall = 1) }
...
```{r SpiderMonkeys, fig.cap="Dot chart of spider monkey family groups", fig.align="center", fig.width=6}
...
```{r IndependenceBar, fig.cap="Diagnoses of participants", fig.align="center", fig.width=6}
...
```{r IndependencePie, fig.cap="Severity of diagnoses of participants", fig.align="center", fig.height=3.5}
...
Consider the pie chart (using data in @data:andersen:1977) in the top panel of Fig. \@ref(fig:LCgraphs).
...
```{r LCgraphs, fig.cap="Graphs from a study of hospital admission of children with asthma", fig.align="center", fig.width=5, fig.height=8}
data(danishlc)
...
```{r 3Dcharts, fig.cap="Four different graphs for the same data", fig.height=7, fig.align="center"}
data(NHANES)
...
```{r KrillEggsDoubleStem, fig.cap="The number of eggs from krill, for control and treatment groups", fig.align="center", fig.width=5.5}
...
A study examining lower-limb amputation incidence in Australia (based on @data:Dillon2017:Geographic) produced the graphs in Figs. \@ref(fig:AmputationAustraliaF) and \@ref(fig:AmputationAustraliaM).
...
```{r AmputationAustraliaF, fig.cap="Age-adjusted incidence of lower limb amputations in Australia, from August 2007 to December 2011: females. Numbers are incidents per 100 000."}
...
```{r AmputationAustraliaM, fig.cap="Age-adjusted incidence of lower limb amputations in Australia, from August 2007 to December 2011: males. Numbers are incidents per 100 000."}
...
```{r DataInsulationGraphPlots, fig.cap="Two plots of the energy consumption data. The dotted line in the top panel shows the one home where energy consumption increased.", fig.align="center", fig.height=7, fig.width=5}
par( mfrow=c(2,1))
...
```{r BarchartTruncated1, fig.align="center", fig.width=5, fig.cap="The bar chart, without truncating the vertical axis" }
if (knitr::is_latex_output()){
  draw.bar( ylim.lower.vec[1] )
}
```

```{r BarchartTruncated55, fig.align="center", fig.width=5, fig.cap="The bar chart, after truncating the vertical axis" }
...
The NHANES study is an observational study, where confounding is a potential problem (Sect. \@ref(ExtraneousVariables)).
...
```{r NHANESplots, fig.cap="Some plots from the NHANES data", fig.align="center", fig.height=6.5, fig.width=5}
par( mfrow = c(2, 2))
