...
```{r IndependenceBar, fig.cap="Diagnoses of participants: numbers (left panel) and percentges (right panel)", fig.align="center", out.width='100%', fig.height=3, fig.width=11}
...
```{r LCgraphsHTML, fig.cap="Graphs of deaths from lung cancer in Fredericia (Denmark) during 1968 and 1971 (inclusive)", fig.align="center", fig.width=5, fig.height=8}
if (knitr::is_html_output()) {
...
For *quantitative* data, the tabular summary effectively produces a histogram as a table (Example \@ref(BabyBoomTable)).
...
```{r KrillEggsDoubleStem, fig.cap="The number of eggs from krill, for treatment and control groups", fig.align="center", fig.width=7.0}
...
```{r TwoDDotchart, fig.height=6, out.width='100%',fig.cap="Two variations of a 2-D dot chart for the krill-egg data: stacking (left) and jittering (right)", fig.align="center", fig.width=8.75, fig.height=3}
old.par <- par()
...
```{r BoxplotKrillTreatment, fig.cap="A boxplot for the krill-egg data; the boxplot and dotplot just for the treatment group", fig.width=6, fig.height=6.5, out.width='60%', fig.align="center", }
...
For the *Control* data, the largest observation (31 eggs) is deemed unusually large (explained in Sect. \@ref(OutliersIQRrule)), and then the boxplot is constructed like this (Fig. \@ref(fig:BoxplotKrillControl)):
...
Many variations of these graphs are possible (Sect. \@ref(OtherVariations)): using counts or percentages; variable levels on the vertical or horizontal axis.
...
```{r PossumBarChartsLATEX, fig.height=8, fig.width=8.0, out.width='100%', fig.cap="Three graphs: The number or percentage of possums found as road kill, by sex and season", fig.align="center"}
...
```{r PossumBarChartsHTML, fig.height=10, fig.width=4.0, fig.cap="Three graphs: The number or percentage of possums found as road kill, by sex and season", fig.align="center"}
...
A study of the number of lynx trapped in the Mackenzie River area of Canada [@elton1942ten] each year from 1821 to 1934 produced the data shown in 
`r if (knitr::is_latex_output()) {
   '(Fig. \\@ref(fig:BabyBoomTimeLATEX), right panel).'
} else {
   '(Fig. \\@ref(fig:BabyBoomTimeHTML), bottom panel).'
}`
A regular cycle may be present, where the number of trapping is very large.
:::
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
In the NHANES study [@data:NHANES3:Data], the age and sex of each participant were recorded(among numerous other variables).
Using Fig. \@ref(fig:Bar3D), can you easily determine if more females or more males are in each age group?
:::


```{r Bar3D, fig.cap="A three-dimensional bar chart of the NHANES data", fig.align="center", fig.width=5.0, fig.height=3, out.width="70%"}
...
To demonstrate how graphs display data for answering RQs, consider the following RQ:
...
Graphs are an excellent way to do so (especially with large data sets).
...
```{r NHANESSmokeBarchart, fig.cap="A bar chart of current smoking status for the NHANES data. No response was recorded for many subjects", fig.align="center", fig.height=3.5, fig.width=8.75, out.width='90%'}
par(mfrow = c(1, 2))
...
The NHANES study is an observational study, so confounding is a potential problem (Sect. \@ref(ExtraneousVariables)), so exploring the relationships between the response and extraneous variables, and between the explanatory and extraneous variables, is useful (Fig. \@ref(fig:NHANESplots)).


```{r NHANESplots, fig.cap="Some plots from the NHANES data. Top panels: the response variables and some extraneous variables; bottom panels: the explanatory variable and some extraneous variables", fig.align="center", fig.height=5, fig.width=5, out.width='70%'}
par( mfrow = c(2, 2),
...
GraphTab[2, ] <- c("Boxplot; back-to-back stem; 2-D dot chart", 
                   "Scatterplot")
...
`r if( knitr::is_html_output(exclude = "epub")) {longmcq( c(
   ...
   "A boxplot",
   "A histogram of the differences") )}`
3. To compare the average number of hours spent on social media for people aged over 30, to people aged 30 and under.
`r if( knitr::is_html_output(exclude = "epub")) {longmcq( c(
   "A stacked bar chart",
   "A pie chart", 
   answer = "A boxplot",
