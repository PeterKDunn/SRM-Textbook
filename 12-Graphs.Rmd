...
```{r BBStem44,  fig.align="center", fig.cap="The final stemplot for the baby-weight data. The data are on the left; the final stemplot on the right.", fig.width=10, fig.height=3, out.width='95%' }
if (knitr::is_latex_output()){
...
* Sometimes, data may need suitable rounding before creating the stemplot.
...
* Add an explanation for reading the stemplot.
  For example, the caption for the stemplot for the baby-birth data in Fig. \@ref(fig:BBStem44) says '2 | 6 means 2.6kg', which explains what the stem plot means. 
...
`r if (knitr::is_latex_output()) {
   'Figure \\@ref(fig:KrillStem44) shows the stemplot. (The online version has an animation.)'
} else {
   'The animation below shows how the stemplot is constructed.'
}`
...
*Jittering* is also an option (Sect. \@ref(DotsChartsQuant2)).
:::
...
1. The *average*:
   What is an "average", middle-position or central value?
2. The *variation*: 
...
The baby-birth data displayed in Example \@ref(exm:Histograms) 
`r if( knitr::is_latex_output() ) {
  '(Fig. \\@ref(fig:BBHist44)))'
}`
could be described as:

1. *Average*: A typical, middle-position weight is between about 2.5kg and 3kg.
2. *Variation*: Babies were between about 1.5kg and 4.5kg at birth.
3.*Shape*: A few babies have very lower weights (premature births?).
4. *Outliers*: No unusual values are present.
:::

Describing the *shape* can be difficult, but some terminology may help:

* *Right* (or *positively*) skewed: most data are smaller, with some larger values (to the *right*).
* *Left* (or *negatively*) skewed: most the data are larger, with some smaller values (to the *left*).
...
```{r, BimodalFaithfulHisto, fig.align="center", fig.cap="Histogram of the times between eruptions for the Old Faithful geyser", fig.width=4.5, fig.height=3.5, out.width='50%'}
data(faithful)
...
A back-to-back stemplot allows the two groups to be compared visually (Fig. \@ref(fig:KrillEggsDoubleStem)).
