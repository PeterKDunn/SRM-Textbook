...
## Graphing one quantitative variable {#GraphsOneQuant}
...
BB2 <- data.frame(Gender=factor(BB$Gender, levels=1:2, labels=c("Female", "Male")),
...
if( knitr::is_latex_output() ) {
  kable( head(BB2, 10),
         ...
         longtable=FALSE,
       digits=c(0, 1, 0),
       col.names=c("Gender", "Weight (in kg)", "Minutes since midnight"),
...
if( knitr::is_html_output() ) {
  DT::datatable( BB2,
        ...
        colnames=c("Gender", "Weight (in kg)", "Minutes since midnight"),
...
```{r animation.hook="gifski", cache=TRUE, echo=FALSE, interval=0.85}
  BB <- structure(list(Time = c(5L, 104L, 118L, 155L, 257L, 405L, 407L, 
                                422L, 431L, 708L, 735L, 812L, 814L, 909L, 1035L, 1049L, 1053L, 
                                1133L, 1209L, 1256L, 1305L, 1406L, 1407L, 1433L, 1446L, 1514L, 
                                1631L, 1657L, 1742L, 1807L, 1825L, 1854L, 1909L, 1947L, 1949L, 
...
if (knitr::is_html_output()){
  ...
  for (i in (1:length(Wts))){
    ...
    plot( expand.grid(1:datagrid.x, 1:datagrid.y),
          ...
          axes=FALSE)
    cex.data <- 0.9
    text(  1:datagrid.x, rep(4, 11), DP2(Wts[1:11]/10),  cex=cex.data )
    text(  1:datagrid.x, rep(3, 11), DP2(Wts[12:22]/10), cex=cex.data )
    text(  1:datagrid.x, rep(2, 11), DP2(Wts[23:33]/10), cex=cex.data )
    text(  1:datagrid.x, rep(1, 11), DP2(Wts[34:44]/10), cex=cex.data )
    
    # Now highlight the observation being used
    if (i<=11) {
      points(  i, 4, pch=1, col="red", lwd=3, cex=4 )
    }
    if ( (i>11) & (i<=22)){
      points(  i-11, 3, pch=1, col="red", lwd=3, cex=4 )
    }
    if ( (i>22) & (i<=233)){
      points(  i-22, 2, pch=1, col="red", lwd=3, cex=4 )
    }
    if ( (i>33) ){
      points(  i-33, 1, pch=1, col="red", lwd=3, cex=4 )
    }
    ...
    if (Wts[i] < 20 ) {
      mtext("1", side=2, line=1, las=1, at=4, col="red", font=2, cex=1.5)
      mtext("2", side=2, line=1, las=1, at=3)
      mtext("3", side=2, line=1, las=1, at=2)
      mtext("4", side=2, line=1, las=1, at=1)
    }
    if ( (Wts[i] >=20) & (Wts[i]<30) ){
      mtext("1", side=2, line=1, las=1, at=4)
      mtext("2", side=2, line=1, las=1, at=3, col="red", font=2, cex=1.5)
      mtext("3", side=2, line=1, las=1, at=2)
      mtext("4", side=2, line=1, las=1, at=1)
    }
    if ( (Wts[i] >=30) & (Wts[i]<40) ){
      mtext("1", side=2, line=1, las=1, at=4)
      mtext("2", side=2, line=1, las=1, at=3)
      mtext("3", side=2, line=1, las=1, at=2, col="red", font=2, cex=1.5)
      mtext("4", side=2, line=1, las=1, at=1)
    }
    if ( Wts[i] >= 40 ){
      mtext("1", side=2, line=1, las=1, at=4)
      mtext("2", side=2, line=1, las=1, at=3)
      mtext("3", side=2, line=1, las=1, at=2)
      mtext("4", side=2, line=1, las=1, at=1, col="red", font=2, cex=1.5)
    }
    ...
    for (j in (1:i) ){
      if ( Wts[j]<20) {
        if (j != i) points( stemcounts[1], 4, pch=as.character(Wts[j]-10))
        if (j == i ) text( stemcounts[1], 4, as.character(Wts[j]-10), col="red", cex=1.5, font=2)
        stemcounts[1] <- stemcounts[1] + 1
      }
      
      if ( (Wts[j]>=20) & (Wts[j] < 30) ) {
        if (j != i ) points( stemcounts[2], 3, pch=as.character(Wts[j]-20) )
        if (j == i ) text( stemcounts[2], 3, as.character(Wts[j]-20), col="red", cex=1.5, font=2)
        stemcounts[2] <- stemcounts[2] + 1
      }
      
      if ( (Wts[j]>=30) & (Wts[j] < 40) ) {
        if (j != i ) points( stemcounts[3], 2, pch=as.character(Wts[j]-30))
        if (j == i ) text( stemcounts[3], 2, as.character(Wts[j]-30), col="red", cex=1.5, font=2)
        stemcounts[3] <- stemcounts[3] + 1
      }
      if ( Wts[j]>=40 ) {
        if (j != i ) points( stemcounts[4], 1, pch=as.character(Wts[j]-40))
        if (j == i ) text( stemcounts[4], 1, as.character(Wts[j]-40), col="red", cex=1.5, font=2)
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Stemplots: Baby weights", src = "./Animations/StemPlotBBMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->
...
#      points(  i-33, 1, pch=1, col="red", lwd=3, cex=4 )
    ...
    for (j in (1:44) ){
      ...
      if ( Wts[j]>=40 ) {
        points( stemcounts[4], 1, pch=as.character(Wts[j]-40))
        #if (j == i ) text( stemcounts[4], 1, as.character(Wts[j]-40), col="red", cex=1.5, font=2)
        stemcounts[4] <- stemcounts[4] + 1
      }
    }
...
<!-- ```{r BBStem1, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Starting to make the stemplot for the baby-weight data: the first 4 observations added" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/StemPlotBB4") -->
<!-- } -->
<!-- ``` -->
...
<!-- ```{r BBStem44, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The final stemplot for the baby-weight data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/StemPlotBB44") -->
<!-- } -->
<!-- ``` -->
...
```{r animation.hook="gifski", cache=TRUE, cache=TRUE, echo=FALSE, interval=0.85}
if (knitr::is_html_output()){
#KRILL
  ...
  for (i in (1:length(Eggs))){
    ...
    plot( expand.grid(1:datagrid.x, 1:datagrid.y),
          ...
          ylab="",
          axes=FALSE)
    text(  1:datagrid.x, rep(3, 5), as.character(Eggs[1:5]) )
    text(  1:datagrid.x, rep(2, 5), as.character(Eggs[6:10]) )
    text(  1:datagrid.x, rep(1, 5), as.character(Eggs[11:15]) )
    
    # Now highlight the observation being used
    if (i<=5) {
      points(  i, 3, pch=1, col="red", lwd=3, cex=4 )
    }
    if ( (i>5) & (i<=10)){
    ...
    if (Eggs[i] < 10 ) {
      mtext("0", side=2, line=1, las=1, at=3, col="red", font=2, cex=1.5)
      mtext("1", side=2, line=1, las=1, at=2)
      mtext("2", side=2, line=1, las=1, at=1)
      mtext("3", side=2, line=1, las=1, at=0)
    }
    if ( (Eggs[i] >=10) & (Eggs[i]<20) ){
      mtext("0", side=2, line=1, las=1, at=3)
      mtext("1", side=2, line=1, las=1, at=2, col="red", font=2, cex=1.5)
      mtext("2", side=2, line=1, las=1, at=1)
      mtext("3", side=2, line=1, las=1, at=0)
    }
    if ( (Eggs[i] >=20) & (Eggs[i]<30) ){
      mtext("0", side=2, line=1, las=1, at=3)
      mtext("1", side=2, line=1, las=1, at=2)
      mtext("2", side=2, line=1, las=1, at=1, col="red", font=2, cex=1.5)
      mtext("3", side=2, line=1, las=1, at=0)
    }
    if ( Eggs[i] >= 30 ){
      mtext("0", side=2, line=1, las=1, at=3)
      mtext("1", side=2, line=1, las=1, at=2)
      mtext("2", side=2, line=1, las=1, at=1)
      mtext("3", side=2, line=1, las=1, at=0, col="red", font=2, cex=1.5)
    }
    # Divide the stems and leaves
    abline(v=0, lwd=2)
...
#KRILL
  ...
  for (i in (length(Eggs):length(Eggs))){
    ...
    for (j in 1:length(Eggs) ){
      if ( Eggs[j]<10 ) {
        points( stemcounts[1], 3, pch=as.character(Eggs[j]))
        stemcounts[1] <- stemcounts[1] + 1
      }
      
      if ( (Eggs[j]>=10) & (Eggs[j] < 20) ) {
        points( stemcounts[2], 2, pch=as.character(Eggs[j]-10) )
        stemcounts[2] <- stemcounts[2] + 1
      }
      
      if ( (Eggs[j]>=20) & (Eggs[j] < 30) ) {
        points( stemcounts[3], 1, pch=as.character(Eggs[j]-20))
        stemcounts[3] <- stemcounts[3] + 1
      }
      if ( Eggs[j]>=30 ) {
        points( stemcounts[4], 0, pch=as.character(Eggs[j]-30))
        stemcounts[4] <- stemcounts[4] + 1
      }
    }
}
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Stemplots", src = "./Animations/StemPlotMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->


<!-- ```{r KrillStem44, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The final stemplot for the krill data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/StemPlot15") -->
<!-- } -->
<!-- ``` -->
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/KfkvedIBPjE" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>





### Dot charts {#DotChartsOneVar}
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Histogram: Baby weights", src = "./Animations/BabyboomHistMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->


```{r animation.hook="gifski", cache=TRUE, echo=FALSE, interval=0.55, fig.height=7}
  BB <- structure(list(Time = c(5L, 104L, 118L, 155L, 257L, 405L, 407L, 
                                422L, 431L, 708L, 735L, 812L, 814L, 909L, 1035L, 1049L, 1053L, 
                                1133L, 1209L, 1256L, 1305L, 1406L, 1407L, 1433L, 1446L, 1514L, 
                                1631L, 1657L, 1742L, 1807L, 1825L, 1854L, 1909L, 1947L, 1949L, 
...
if (knitr::is_html_output()) {
  ...
  for (i in (1:length(Wts))){
    ...
    plot( expand.grid(1:datagrid.x, 1:datagrid.y),
          ...
          ylab="",
          axes=FALSE)
    text(  1:datagrid.x, rep(4, 11), DP2(Wts[1:11]/10) )
    text(  1:datagrid.x, rep(3, 11), DP2(Wts[12:22]/10) )
    text(  1:datagrid.x, rep(2, 11), DP2(Wts[23:33]/10) )
    text(  1:datagrid.x, rep(1, 11), DP2(Wts[34:44]/10) )
...
<!-- ```{r BBHist1, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Starting to make the histogram for the baby-birth data: the first 6 observations added" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/BabyboomHist6") -->
<!-- } -->
<!-- ``` -->

<!-- ```{r BBHist44, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The final histogram for the baby-birth data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/BabyboomHist44") -->
<!-- } -->
<!-- ``` -->
...
if (knitr::is_html_output()) {
  ...
  p <- ggplot(HICSdata, 
              aes(x = HICS)) +
              geom_histogram(binwidth=5, color="black", fill="blue", alpha = .2, breaks=seq(0, 40, by=5)) +
              #scale_x_continuous(breaks=seq(0, 40, by=5)) +
              scale_y_continuous(breaks=seq(0, 12, by=2)) +
              labs( x= "Duration (seconds)", y="Number of volunteers", title="Duration of HICS after drinking ice water") +
              theme_light()
  ggplotly(p)
}
```
...
xx1 <- scales::rescale( c( rchisq(300, df=2), 7,7,8,7,7,7,8,7,7,6), to=c(0.01, 4.99) )

xx2 <- scales::rescale( c( rchisq(300, df=2), 7,7,8,7,7,7,8,7,7,6), to=c(0.01, 4.99) )
xx2 <- max(xx1) - xx1

xx3 <- scales::rescale( rnorm(500, mean=10), to=c(0.01, 4.99) )

xxA <- rnorm(500,mean=0, sd=1)
xxB <- rnorm(250,mean=4, sd=1)
xx4 <- scales::rescale( c( xxA[xxA>0], xxB), to=c(0.01, 4.99) )

plotA <- ggplot( data.frame(x=xx1), 
              aes( x=x) ) +
              geom_histogram( binwidth=0.5, color="black", fill="blue", alpha = .2, breaks=seq(0, 5, by=0.5)) +
              #scale_x_continuous(breaks=seq(0, 5, by=0.5)) +
              scale_y_continuous(breaks=seq(0, 140, by=40) ) +
              labs( x= "Observations", y="Number of obs.", title="Skewed right") +
              theme_light() 
plotB <- ggplot( data.frame(x=xx2), 
              ...
              #scale_x_continuous(breaks=seq(0, 5, by=0.5)) +
              scale_y_continuous(breaks=seq(0, 140, by=40) ) +
              labs( x= "Observations", y="Number of obs.", title="Skewed left") +
              theme_light() 
plotC <- ggplot( data.frame(x=xx3), 
              ...
              labs( x= "Observations", y="Number of obs.", title="Bell-shaped") +
              theme_light() 
plotD <- ggplot( data.frame(x=xx4), 
              ...
              #scale_x_continuous(breaks=seq(0, 5, by=0.5)) +
              scale_y_continuous(breaks=seq(0, 120, by=40) ) +
              labs( x= "Observations", y="Number of obs.", title="Bimodal") +
              theme_light() 
...
## Graphing one qualitative variable {#GraphsOneQual}
...
par( mfrow=c(2,1), mar=c(5.5, 5, 4.1, 1))
...
abline(h=seq(0, 1600, by=250), col="white")
...
if( knitr::is_latex_output() ) {
  kable( KrillEggs2,
    ...
    add_header_above(header=c("Treatment group "=2, " Control group"=2), bold=TRUE, align="c") %>%
...
### Control data in cols 13 to 19 (x= 1 to x = 7)
  ...
  lines(c(0.5, 0.5),
        c(0.5, 6.5),
         lwd=2,
         col="grey")
  text(-5, 0, "Treatment data", font=2)
  text( 4, 0, "Control data", font=2)
  ...
  for (j in (1:2) ){
    ...
    for (i in 1:length(Eggs)){
      ...
      if ( Eggs[i]<10) {
        points( stemcounts[1], 6, pch=as.character(Eggs[i]))
        stemcounts[1] <- stemcounts[1] + step
      }

    if ( (Eggs[i]>=10) & (Eggs[i] < 20) ) {
      points( stemcounts[2], 5, pch=as.character(Eggs[i]-10) )
      stemcounts[2] <- stemcounts[2] + step
    }

    if ( (Eggs[i]>=20) & (Eggs[i] < 30) ) {
      points( stemcounts[3], 4, pch=as.character(Eggs[i]-20))
      stemcounts[3] <- stemcounts[3] + step
    }
    if ( ( Eggs[i]>=30 ) & (Eggs[i] < 40) ) {
      points( stemcounts[4], 3, pch=as.character(Eggs[i]-30))
      stemcounts[4] <- stemcounts[4] + step
    }
    if ( ( Eggs[i]>=40 ) & (Eggs[i] < 50) ) {
      points( stemcounts[5], 2, pch=as.character(Eggs[i]-40))
      stemcounts[5] <- stemcounts[5] + step
    }
    if ( ( Eggs[i]>=50 ) & (Eggs[i] < 60) ) {
      points( stemcounts[6], 1, pch=as.character(Eggs[i]-50))
...
stripchart( list(Treatment=Eggs.T, Control=Eggs.C), 
...
boxplot( list(Treatment=Eggs.T, Control=Eggs.C), 
...
```{r BoxplotKrillTreatment, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="A boxplot for the krill-egg data; the boxplot just for the treatment group", fig.width=5, fig.align="center", }
...
text( 1.45, Eggs.quantiles[5], "Maximum value", pos=1, cex=0.75)
text( 1.45, Eggs.quantiles[4], "75% smaller than this", pos=3, cex=0.75)
text( 1.45, Eggs.quantiles[3], "Middle value ('average')", pos=3, cex=0.75)
text( 1.45, Eggs.quantiles[2], "25% smaller than this", pos=3, cex=0.75)
text( 1.45, Eggs.quantiles[1], "Minimum value", pos=1, cex=0.75)
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=1}
library(NHANES)
data(NHANES)
...
## Graphing two quantitative variables {#Scatterplots}
...
## Graphing two qualitative variables
...
text( out[4]+0.6, 70, "F", adj=0)
text( out[4]+0.6, 60, "M", adj=0)
text( out[4]+0.6, 30, "?", adj=0)


text( out[1]-0.8, 110, "F", adj=0)
text( out[1]-0.8,  90, "M", adj=0)
text( out[1]-0.8,  40, "?", adj=0)
```
...
mtext( "?", at=out[1,], side=1, line=0, cex=0.8, padj=0.5)
mtext( "M", at=out[2,], side=1, line=0, cex=0.8, padj=0.5)
mtext( "F", at=out[3,], side=1, line=0, cex=0.8, padj=0.5)
```
...
mtext( "Au", at=out[1,], side=2, line=1, las=1)
mtext( "Wn", at=out[2,], side=2, line=1, las=1)
mtext( "Sp", at=out[3,], side=2, line=1, las=1)
mtext( "Su", at=out[4,], side=2, line=1, las=1)

mtext( "Females", at=14, side=2, line=3, las=3)
mtext( "Males",   at=8, side=2, line=3, las=3)
mtext( "Unknown", at=3, side=2, line=3, las=3)




Percentages <- t(apply(Counts2,1,function(x){ round(x/sum(x)*100,1)}))
...
mtext( "Au", at=out[1,], side=1, line=1, cex=0.8, padj=0.5)
mtext( "Wn", at=out[2,], side=1, line=1, cex=0.8, padj=0.5)
mtext( "Sp", at=out[3,], side=1, line=1, cex=0.8, padj=0.5)
mtext( "Su", at=out[4,], side=1, line=1, cex=0.8, padj=0.5)

mtext( "Unknown", at=mean(out[, 1]), side=1, line=3, cex=1, padj=0.5)
mtext( "Males",   at=mean(out[, 2]), side=1, line=3, cex=1, padj=0.5)
mtext( "Females", at=mean(out[, 3]), side=1, line=3, cex=1, padj=0.5)
...
> The purpose of a graph is to display the information in the
> *simplest* and *clearest* possible way, to help us understand the data.
...
locatn <- barplot(t(AgeD.Gender), 
        ...
        cex.names= 0.8,
        ylim=c(0, 900),
        args.legend=list(x="bottom",bg="white", cex=0.75), 
        ylab="Number",
        xlab="Age group",
        main="Number per group",
        col=c("#FFFAF0", "#5844ff"))
...
legend("top", legend = c("Females", "Males"), #levels(NHANES$Gender),
       bty = "n", xpd=TRUE, ncol=2, cex = 0.9, fill=c( '#FFFAF0', '#5844ff') )
...
<!-- ```{r BabyBoomTime, echo=FALSE, fig.cap="The weight of babies born on 21 December 1997 at a Brisbane hospital, plotted over time", fig.align="center", fig.height=8, fig.width=5} -->
<!-- par( mfrow=c(2, 1)) -->

<!-- plot( (Weight/1000) ~ Time,   -->
<!--      data=BB,  -->
...
if (knitr::is_html_output()) {
  dygraph( data.frame(Time=BB$Minutes, Weight=BB$Weight/1000), 
...
```{r echo=FALSE}
#dygraph( data.frame(Time=BB$Time, Weight=BB$Weight/1000), main="Weights of babies", xlab="Minutes after midnight", ylab="Weights (in kg)") %>%
#  dyRibbon(data=ifelse(BB$Gender==1, 1, 0), palette=c("red", "blue"), top=0.2 ) %>%
#  dyOptions(drawPoints = TRUE, pointSize = 2)
#
```
...
if (knitr::is_html_output()) {
  dygraph(lynx, main = "Number of lynx trappings per year", xlab="Year", ylab="No. trappings") %>% 
...
```{r echo=FALSE, cache=TRUE, animation.hook='gifski', interval=0.2}
library(GLMsData)
data(danishlc)

ylim.lower.vec <- c( rep(0, 5), seq(0, 9.75, by=0.25) )
ylim.lower.vec <- c( ylim.lower.vec, rep( max(ylim.lower.vec), 10))
...
draw.bar <- function( ylim.lower) {
  ...
  out <- barplot( egdata$Cases, 
                  ...
                  main="Number of lung cancer\ncases in Fredericia",
                  xpd=FALSE, # BARS **DON'T** GO outside plotting region
                  ylim=c( ylim.lower, 11.25),
                  axes=FALSE)
  axis(side=1, at=out[, 1], labels=egdata$Age, las=2)
  axis(side=2, at=seq( floor(ylim.lower), 12, by=1), las=1)
  box()
}
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Bar charts: Truncating the vertical axis", src = "./Animations/BarchartTruncated.html", frameborder=0, height=500, width=350)  -->
<!-- ``` -->
...
<!-- ```{r BarchartTruncated1, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The bar chart, without truncating the vertical axis" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/BarchartTruncated1") -->
<!-- } -->
<!-- ``` -->

<!-- ```{r BarchartTruncated55, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The bar chart, after truncating the vertical axis" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/BarchartTruncated55") -->
<!-- } -->
<!-- ``` -->
...
```{r animation.hook="gifski", cache=TRUE, echo=FALSE}
library(NHANES)
data(NHANES)
...
out <- barplot( prop.table(xtabs(~ SmokeNow + Gender, data=NHANES),  margin=2) * 100, 
	ylim=c(0, 100),
	xlab="Gender", 
...
colnames(GraphTab) <- c( "Explanatory: Qualitative", "Explanatory: Quantitative")
rownames(GraphTab) <- c("Response: Qualitative", "Response: Quantitative")

GraphTab[1, ] <- c("Stacked bar chart; side-by-side bar chart", "Back-to-back stem; boxplot; 2-D dot chart")
GraphTab[2, ] <- c("Back-to-back stem; boxplot; 2-D dot chart", "Scatterplot")
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/FMprCwua-ew" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
to determine if the bruise depth could be estimated 
from the recorded the surface temperature at the bruise location.
