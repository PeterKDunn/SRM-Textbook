...
When a qualitative variable is compared between individuals (i.e., *one qualitative variable* and *one quantitative variable*) options for plotting include: 
...
```{r BoxplotKrillTreatment, fig.cap="A boxplot for the krill-egg data; the boxplot and dotplot just for the treatment group. The circled numbers on the left refer to the explantions in-text.", fig.width=6, fig.height=6.5, out.width='60%', fig.align="center" }
...
When a qualitative variable is compared between individuals (i.e., *two qualitative variables*) options for plotting include: 
...
```{r PairedGraphCaseProfileLATEX, fig.cap="The IgE data. Left: A histogram of the differences; right: A case-profile plot. Each line represents one subject, joining that person's pre-intervention score to their post-intervention score", fig.align="center", fig.width=9.25, fig.height=3.5, out.width='100%'}
...
A histogram of difference can not be constructed, and a *case-profile plot* can be used: a plot showing how the response variable changes for each unit of analysis.
...
In both cases, each unit of analysis gets similar measurements.
...
You should be able to construct the graphs by hand, but usually software is used (e.g., jamovi).
...
The *explanatory variable* is 'current smoking status', which could be graphed using a bar chart (Fig.\ \@ref(fig:NHANESHDLHistBar), centre panel) or pie chart, for example.
...
A bar could be added to the bar chart to show the number of non-responses (though it probably isn't necessary): Fig.\ \@ref(fig:NHANESHDLHistBar), right panel.
  
The main RQ involves the *relationship* between average direct HDL cholesterol concentration and current smoking status, so a graph displaying this relationship is needed (Fig.\ \@ref(fig:NHANESboxplot)).
...
boxplot(NHANES$DirectChol ~ NHANES$SmokeNow, 
        ...
        las = 1,
        ylim = c(0, 4),
        ylab = "Direct HDL cholesterol conc. (mmol/L)",
...
plot( as.numeric( factor(Col) ) ~ as.numeric( factor(Spine) ), 
      ...
      main = "Dot chart of carapace colouyr\nagainst spine condition",
      las = 1)
```
