...
* One quantitative variable (Sect.\ \@ref(GraphsOneQuant)). \raggedright
* One qualitative variable (Sect.\ \@ref(GraphsOneQual)).
* Between-individual comparisons with a quantitative response variable (Sect.\ \@ref(GraphOneQualOneQuant)).
...
* Between-individual comparisons with a qualitative response variable (Sect.\ \@ref(TwoQualVars)). \raggedright
* Within-individual comparisons with a quantitative response variable (Sect.\ \@ref(PlotsWithinIndividuals)).
* Connections between two quantitative variables (Sect.\ \@ref(Scatterplots)).

:::
::::::


## One variable: quantitative data {#GraphsOneQuant}
...
Sometimes, quantitative data with very few values are best graphed using graphs for qualitative data (Sect.\ \@ref(GraphsOneQual)).

Three different types of graphs can be used to display the distribution of one quantitative variable:

* *Stemplots* (or *stemplots*) (Sect.\ \@ref(StemAndLeafPlots)):
  Used for small amounts of data; only sometimes useful.
...
if( knitr::is_latex_output() ) {
  T1 <- kable( surroundMaths(BB2[1:5, ],
                             decDigits = c(0, 1, 0)),
               format = "latex",
               longtable = FALSE,
               row.names = FALSE,
...
#    column_spec(3, width = "10mm")
  T2 <- kable( surroundMaths(BB2[6:10, ],
                             decDigits = c(0, 1, 0)),
               format = "latex",
               longtable = FALSE,
               row.names = FALSE,
...
are given to one decimal place of a kilogram, so the whole number of kilograms is placed to the left of the *stem*, and the first decimal place is placed on the right of the stem (as a *leaf*).
...
The first weight, of $1.7$\ kg, is entered with the $1$ to the right of the stem, and the $7$ to the left as the 'leaf': `1 | 7`. 
...
One advantage of using stemplots over other plots is that the original data are retained.
...
* Add an explanation for reading the stemplot.
  For example, the stemplot for the baby-birth data says '$2$\ |\ $6$ means $2.6$\ kg' (rather than, say, $26$\ kg, $0.26$\ kg, or $2$\ lb $6$\ oz).


::: {.example #StemLeafPlots name="Stemplots"}
A study recorded the number of eggs laid for $15$ krill [@data:Greenacre2016:reporting].
The stemplot 
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:KrillStem44)'
} else {
...
Dot charts show the original data on a single (usually horizontal) axis, with each observation represented by a dot.
...
based on @data:Wetzel2005:McDonalds) shows that the mass of fries is almost always *under* the target, and often substantially so.

Observations are *stacked* on top of each other when multiple observations are the same; for example, two orders of fries were $133$\ g.
(*Jittering* is another an option; see Example\ \@ref(exm:DotsChartsQuant2).)
:::


```{r FriesBabiesPDF, fig.cap="Large orders of French fries (left panel), where similar observations are stacked. A dot chart of the baby-weight data (right panel), using jittering, rather than stacking.", fig.align="center", fig.width=9, fig.height=3, out.width='95%'}
if (knitr::is_latex_output()){
...
```{r BBHist1, fig.align="center", fig.width=10, fig.height=3, out.width='95%', fig.cap="Starting to make the histogram for the baby-birth data: the first six observations added" }
if (knitr::is_latex_output()){
  Wts <- sort(BB$Weight)
...
A study of *brain freezes* after consumin cold food drink measured the duration of the brain-freeze symptoms [@data:Mages2017:BrainFreeze].

A histogram of the data (Fig.\ \@ref(fig:BrainFreeze), based on @data:Mages2017:BrainFreeze), shows $11$ people experience symptoms less than $5$\ s in length; nine people experienced symptoms for at least $5$ but less than $10$\ s; and $1$ person experienced symptoms for at least $35$\ s but under $40$\ s.
:::

```{r BrainFreeze, fig.cap="Duration of brain-freeze symptons after drinking ice water", fig.width=5.5, fig.height=3.5, fig.align="center"}
...
After producing a graph for one quantitative variable, the *distribution* should be described:
...
The time between eruptions (Fig.\ \@ref(fig:BimodalFaithfulHistoChangeBins), left panel) is bimodal, with peaks near $55$\ min and $80$\ min.
:::
...
## One variable: qualitative data {#GraphsOneQual}
...
## Comparisons for quantitative variables: between-individuals {#GraphOneQualOneQuant}
...
\centering
```{r KrillEggsDoubleStem, fig.width=5.0, fig.height=3, out.width='100%'}
...
if (knitr::is_html_output()) {
  kable( KrillEggs2,
         ...
         booktabs = TRUE,
         escape = FALSE,
         caption = "The number of eggs from krill, for treatment and control groups",
         linesep = c("", "", "\\addlinespace"),
         align = c("c","c")) %>%
    add_header_above(header = c("Treatment group " = 2, 
                                " Control group" = 2), 
...
```{r TwoDDotchart, fig.height=6, out.width='100%',fig.cap="Two variations of a 2-D dot chart for the krill-egg data to avoid overplotting: stacking (left) and jittering (right)", fig.align="center", fig.width=8.75, fig.height=2.5}
old.par <- par()
...
An example of a boxplot, for the krill data in Example\ \@ref(exm:KrillData), is shown in 
`r if (knitr::is_latex_output()) {
   'Fig.\\ \\@ref(fig:BoxplotKrill) (left panel).'
} else {
   'Fig.\\ \\@ref(BoxplotKrillHTML).'
}`
Understanding boxplots is challenging, so will be discussed again (in more detail) in Sect.\ \@ref(VariationIQR).
...
<!-- SOME LATEX-ONLY PLOTS -->
`r if (knitr::is_html_output()) '<!--'`
```{r BoxplotKrill, fig.cap="A boxplot  for the krill-egg data", fig.align="center",  fig.height=5.25, , fig.width=8.5, out.width='100%'}
...
boxplot( list(Treatment = Eggs.T, 
              Control = Eggs.C), 
            main = "A boxplot of the number\nof eggs laid by krill",
            las = 1,
            pch = 19,
            col = plot.colour,
            ylim = c(-5, 50),
...
points(x = c(0.85,
             rep(0.75, length(Eggs.quantiles) - 1) ),
      y = Eggs.quantiles,
      pch = 1,
      cex = 2.5)
text( x = c( 0.85,
...
To explain boxplots, first focus on just the boxplot for the *Treatment* group.
Boxplots have five horizontal lines; from the top to the bottom of the plot 
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:BoxplotKrill), right panel):'
} else {
   '(Fig.\\ \\@ref(fig:BoxplotKrillTreatmentHTML)):'
}`

1. *Top line*:
   The *largest* number of eggs is $`r KrillT.quantiles[5]`$.
2. *Second line from top*:
   $75$% of observations are smaller than about $`r KrillT.quantiles[4]`$, represented by the line at the top of the central box. 
   This is the *third quartile* ($Q_3$).
3. *Middle line*:
   $50$% of observations are smaller than about $`r KrillT.quantiles[3]`$, represented by the line inside the central box. 
   This is an 'average' value, the *second quartile* ($Q_2$).
4. *Second line from bottom*:
   $25$% of observations are smaller than about $`r KrillT.quantiles[2]`$, represented by the line at the bottom of the central box. 
   This is the *first quartile* ($Q_1$).
5. *Bottom line*:
   The *smallest* number of eggs is $`r KrillT.quantiles[1]`$. 


<!-- SOME HTML ONLY PLOTS -->
`r if (knitr::is_latex_output()) '<!--'`
```{r BoxplotKrillHTML, fig.cap="A boxplot  for the krill-egg data", fig.align="center",  fig.height=5.25, , fig.width=8.5, out.width='100%'}
boxplot( list(Treatment = Eggs.T, 
              Control = Eggs.C), 
            main = "A boxplot of the number\nof eggs laid by krill",
            las = 1,
...
```{r BoxplotKrillTreatmentHTML, fig.cap="A boxplot for the krill-egg data; the boxplot and dotplot just for the treatment group. The circled numbers on the left refer to the explantions in-text.", fig.width=6, fig.height=6.5, out.width='60%', fig.align="center" }
if (knitr::is_html_output()){ 
...
The box for the *Control* group krill is slightly different
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:BoxplotKrill), left panel):'
} else {
   '(Fig.\\ \\@ref(BoxplotKrillHTML)):'
...
* *Second line from top*:
  $75$% of observations (*including* the $31$ eggs) are smaller than about $`r Eggs.quantiles[4]`$, represented by the line at the top of the central box. 
  This is the *third quartile*, or $Q_3$.
* *Middle line*:
  $50$% of observations (*including* the $31$ eggs) are smaller than about $`r Eggs.quantiles[3]`$,  represented by the line in the centre of the central box.
  This is an 'average' value for the data, the *second quartile*, or $Q_2$.
* *Second line from bottom*:
  $25$% of observations (*including* the $31$ eggs) are smaller than about $`r Eggs.quantiles[2]`$,  represented by the line at the bottom of the central box.
  This is the *first quartile*, or $Q_1$.
* *Bottom line*:
...
## Comparisons for qualititative variables: between-individuals {#TwoQualVars}
...
## Comparisons for quantitative variables: within-individuals {#PlotsWithinIndividuals}
...
Sometimes, the within-individuals comparison is measured over many time points, often for just one individual.
A *time plot* can be used, which shows how the variable changes over time.


::: {.example #TimePlots name="Time plots"}
The baby-birth data (Sect.\ \@ref(StemAndLeafPlots)) recorded the time of each birth.
...
if( knitr::is_latex_output() ) {
  ...
  plot( Trappings ~ Year, 
        ...
        type = "b",
        xlab = "Year",
        ylab = "Number of lynx trappings in Mackenzie River\nfrom 1821 to 1934",
        main = "Lynx trappings",
        pch = 19,
        xlim = c(1820, 1935),
        ylim = c(0, 7000),
...
## Connections between two quantitative variables {#Scatterplots}
...
You should be able to construct the graphs by hand, but *usually software is used to produce graphs* (e.g., jamovi).
...
Graphs should focus on clear communication, not trying to appear fancy. 
...
The Population is 'Americans', the Outcome is the 'average direct HDL cholesterol concentration', and the Comparison is 'Between those who currently do and do not smoke'.
...
Data to answer this RQs are found from the American *National Health and Nutrition Examination Survey* (NHANES) [@data:NHANES3:Data; @data:NHANES3].
From the NHANES webpage (http://www.cdc.gov/nchs/nhanes/about_nhanes.htm; emphasis added), this survey:
...
For the NHANES data, the *response variable* is direct HDL cholesterol concentration (quantitative continuous).
The histogram (Fig.\ \@ref(fig:NHANESHDLHistBar), left panel) shows that the direct HDL cholesterol concentration is usually around $1.5$\ mmol/L, and varies between about $0.5$ and $3$\ mmol/L.
The data are slightly skewed right.


```{r NHANESHDLHistBar, fig.cap="Plots from the NHANES data. Left: histogram of the direct HDL cholesterol concetration (large outliers exist but are hard to see); centre: bar chart of current smoking status for the NHANES data; right: bar chart of current smoking status, including missing data", fig.align="center", fig.height = 3.5, fig.width=3.5, out.width=c('33%', '32%', '32%'), fig.show="hold"}

hist(NHANES$DirectChol,
     xlab = "Direct HDL cholesterol conc. (mmol/L)",
     ylab = "Frequency",
     main = "Distribution of direct\nHDL cholesterol concentration",
     las = 1,
     axes = FALSE,
     ylim = c(0, 2500),
     xlim = c(0, 4.5),
...
The *explanatory variable* is 'current smoking status' (qualitative), and is graphed using a bar chart (Fig.\ \@ref(fig:NHANESHDLHistBar), centre panel).
...
A bar could be added to the bar chart to show the number of non-responses (Fig.\ \@ref(fig:NHANESHDLHistBar), right panel).

The RQ involves the *relationship* between average direct HDL cholesterol concentration and current smoking status, so a graph displaying this relationship is needed (Fig.\ \@ref(fig:NHANESboxplot)).


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Based on the plot of the *sample* data, is there a difference in the average HDL cholesterol concentrations between the two smoking groups in the *population*?
:::
    
```{r NHANESboxplot, fig.cap="Boxplot of direct HDL cholesterol concentration against current smoking status, for the NHANES data", fig.align="center", fig.height=4, fig.width=5}
boxplot(NHANES$DirectChol ~ NHANES$SmokeNow, 
        ...
        main = "Direct HDL cholesterol concentration by smoking\nstatus, for the NHANES data"
)
```
...
```{r NHANESplots, fig.cap="Some plots from the NHANES data. Top panels: the response variables and some extraneous variables; bottom panels: the explanatory variable and some extraneous variables", fig.align="center", fig.height=5, fig.width=7.0, out.width='100%'}
par( mfrow = c(2, 2),
     mar = c(5, 5.5, 4, 2) + 0.1 ) # DEFAULT: c(5, 4, 4, 2) + 0.1.

plot( DirectChol ~ Weight, 
      data = NHANES, 
	    xlab = "Weight (in kg)", 
	    ylab = "Direct HDL cholesterol\nconc. (mmol/L)",
      main = "NHANES: HDL cholesterol\nconc. vs Weight",
	    ylim = c(0, 4),
      col = blueTransparent,
	    las = 1)
...
For *one quantitative variable*, possible graphs include a histogram (for medium to large amounts of data), a dot chart (for small to moderate amounts of data), or a stemplot (for small amounts of data, and only useful in some circumstances).

For *one qualitative variable*, possible graphs include a bar chart (usually a good choice), a dot chart (usually a good choice), or a pie chart (only useful in special circumstances, and can be harder to interpret).
...
if( knitr::is_latex_output() ) {
  ...
  column_spec(2, width = "45mm") %>%
  column_spec(3, width = "55mm")
}
...
*Variation*: From about $0$ to about $40$. 
*Shape*: Slightly skewed right. 
*Outliers*: No outliers or unusual observations. The observation between $35$ and $40$ *may* be an outlier, but maybe not (larger sample may have observations between $30$ and $35$).
- Sect. \ref{thinkBox:WhenPiechart}:
**1.** Suitable. 
The browsers are not *mutually exclusive* (people can use more than one) nor *exhaustive* (some people may use other browsers). 
**2.** Not suitable: the percentages are not parts of a whole.
Each individual has information recorded on *two* qualitative variables: (a) state of residence; and (b) whether or not they own an iPhone.
