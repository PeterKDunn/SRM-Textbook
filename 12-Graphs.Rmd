...
* Add an explanation for reading the stemplot.
  For example, the caption for the stemplot for the baby-birth data above says '2 | 6 means 2.6kg', which explains what the stem plot means. 
...
```{r, child = if (knitr::is_html_output())  './children/HTML/histogramChangeBins.Rmd'}
```
...
ozdata <- read.csv("Data/ozdata.csv")
require(gridExtra)
...
if( knitr::is_latex_output() ) {
   kable( cbind(InsulationBeforeAfter$Before, 
                InsulationBeforeAfter$After,
                InsulationBeforeAfter$Diff ),
          format = "latex",
          booktabs = TRUE,
          longtable = FALSE,
          col.names = c("Before", 
...
if( knitr::is_html_output() ) {
     DT::datatable( round(InsulationBeforeAfter, 1),
                    fillContainer = FALSE, # Make more room, so we don't just have ten values
                    options = list(searching = FALSE), # Remove searching: See: https://stackoverflow.com/questions/35624413/remove-search-option-but-leave-search-columns-option
                    caption = "The house insulation data: Energy consumption before and after adding insulation, and the energy saving (all in MWh)")
}
