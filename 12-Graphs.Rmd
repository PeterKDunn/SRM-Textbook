...
But before analysis begins, understanding, describing and summarising the collected sample data is important.
...
* [One qualitative and quantitative variable](#GraphOneQualOneQuant);
* [Two quantitative variables](#Scatterplots);
* [Two qualitative variables](#TwoQualVars); and
...
[The message in the graph should be described](#SummariseData).
...
*Stem-and-leaf plots* (or *stemplots*) are best described and explained using an example.
Consider the data in
`r if( knitr::is_html_output() ) {
  'Fig. \\@ref(fig:BabyBoomDataHTML),'
} else
{
  'Table \\@ref(tab:BabyBoomDataLATEX) (which shows the first 10 of the 44 observations).'
}
`
...
The data set also includes the gender of each baby, and the number of minutes after midnight of the birth.
...
The weights (quantitative) are given to one decimal place of a kilogram, so the whole number of kilograms is placed to the left (as a *stem*), and the first decimal place is placed on the right (as a *leaf*).
...
* Within each row, the observations are *ordered* within each stem so patterns can be seen.
...
An alternative graph shows the *percentage* of each serving relative to the target; see
`r if (knitr::is_latex_output()) {
...
A dot chart (Fig. \@ref(fig:BabyBoomDot)) shows that most babies were born between 3 and 4kg.
:::
...
Consider the weights (in kg) of babies born in a Brisbane hospital in one day [@mypapers:Dunn:dataset:1999].
...
A study of 'headache attributed to ingestion or inhalation of a cold stimulus' (HICS), commonly called a *brain freeze* after eating cold food (e.g., ice cream) or drinking a cold drink, measured the duration of the brain freeze [@data:Mages2017:BrainFreeze].

A histogram of the data (Fig \@ref(fig:BrainFreeze), based on @data:Mages2017:BrainFreeze, Fig. 2b), shows 11 people experience HICS symptoms less than 5 seconds in length.
In addition, nine people experienced symptoms for at least 5 but less than 10 seconds, and 1 person experienced symptoms for at least 35 seconds but under 40 seconds.
:::
...
Graphs are constructed to help understand the data.
After producing a graph for one quantitative variable, describe:
...
4. Any *outliers* (observations unusually large or small) or unusual features:
   Are there any very unusual observations, or anything else of interest?

Describing the *shape* can be difficult, but some terminology may help:

* *Right* (or *positively*) skewed: the bulk of the data is smaller, with some larger values (to the *right*).
* *Left* (or *negatively*) skewed: the bulk of the data is larger, with some smaller values (to the *left*).
...
```{r, BimodalFaithfulHisto, fig.align="center", fig.cap="Histogram of the times between eruptions for the Old Faithful geyser", fig.width=4.5, fig.height=3.5}
data(faithful)
...
For the baby-weight data in, for example, Fig. \@ref(fig:BabyBoomDot):
...
* No *outliers* or anything unusual are apparent.
:::


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Describe the histogram in Fig. \@ref(fig:BrainFreeze), the brain freeze durations.
...
For qualitative data, graphs show how often each level of the variable occurs.
...
barplot( Number, 
         ...
         las = 1,
         xlim = c(0, 12),
         main = "Diagnoses of participants",
...
This picture actually conveys only one piece of information ("69% of patients had a less severe injury"), so a graph of any kind is probably unnecessary.

The pie chart colours are used in the bar chart in Example \@ref(exm:IndependenceBarText): they refer to the severity of the diagnoses.
This is called *encoding* extra information into the bar chart.
:::

   
```{r IndependencePie, fig.cap="Severity of diagnoses of participants", fig.align="center", fig.height=2.5, out.width='50%'}
...
* Pie charts are hard to read: In general, humans are better at comparing *lengths* (as in bar and dot charts) than *angles* (as in pie charts) [@data:Friel:Graphs].
...
The pie chart displays the number of lung cancer deaths in Fredericia (Denmark) during 1968 and 1971 inclusive, for various age groups (qualitative).
...
Adding an artificial third dimension usually makes the message hard to see [@siegrist1996use].
...
Back-to-back stem-and-leaf plots are two stem-and-leaf plots (Sect. \@ref(StemAndLeafPlots)) sharing the same stems; one group has the leaves emerging left-to-right from the stem, and the second group has the leaves emerging right-to-left from the stem.
...
A study of krill [@data:Greenacre2016:reporting] produced the data in Table \@ref(tab:KrillDataTable).
A back-to-back stem-and-leaf plot allows the two groups to be compared visually (Fig. \@ref(fig:KrillEggsDoubleStem)).
...
```{r TwoDDotchart, fig.height=6, out.width='100%',fig.cap="Two variations of a 2-D dot chart for the krill-egg data: stacking (left) and jittering (right)", fig.align="center", fig.width=8, fig.height=3}
old.par <- par()
...
Understanding boxplots can be difficult, so boxplots will be discussed again in Sect. \@ref(VariationIQR).
...
```{r BoxplotKrillTreatment, fig.cap="A boxplot for the krill-egg data; the boxplot and dotplot just for the treatment group", fig.width=6, fig.height=6.5, fig.align="center", }
...
For the *Control* data, the largest observation (31 eggs) is deemed unusually large (explained in Sect. \@ref(OutliersIQRrule)), and then the boxplot is constructed like this:
...
Clearly, decimal numbers of eggs (such as $Q_2 = `r Eggs.quantiles[2]`$) is not observable; however, with `r length(Eggs.C)` observations, finding exactly the value for which 25% of observations are smaller is not possible.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Boxplots summarise data with only five numbers, so a lot of details of the distribution is lost.
...
Consider the boxplot of the age of the Americans
`r if (knitr::is_latex_output()) {
   ' from 1999--2004, shown in Fig. \\@ref(fig:BoxplotAge). (The online version has an animation.)'
} else {
   ' from 1999--2004. The animation below shows how the boxplot of the age of the Americans in the sample is constructed.'
}`
...
In Fig. \@ref(fig:CementBoxplot) (based on @data:Silva2016:rootcanal), the three cements are very different regarding their push-out forces.
:::


```{r CementBoxplot, fig.cap="Comparing three push-out values for three cements", fig.align="center", fig.height=3, fig.width=5.5}
...
As with any graph, describing what the graph tells us is important, because the purpose of a graph is to display the information in the clearest, simplest possible way.
...
```{r ScatterAIS, results ='hide', fig.cap="Scatterplot of SSF against percentage body fat for male AIS athletes", fig.width=4.5, fig.height=4, fig.align="center"}
data(AIS)
...
```{r PossumStackedBarChart, fig.height=3.75, fig.width=4.5, fig.align="center", fig.cap="The number of possums found as road kill, by sex and season"}
...
```{r PossumSideBySideBarChart, fig.height=3.25, fig.width=5, fig.align="center", fig.cap="The number of possums found as road kill, by sex and season"}
colnames(Counts2) <- c("?", "M", "F")
...
```{r PossumDotCharts, fig.height=5, fig.width=5.0, fig.cap="A dot chart of the number of possums found as road kill, by sex and season", fig.align="center"}
colnames(Counts) <- c("Unknown", 
                      "Males", 
                      "Females")
...
Many variations of these bar charts are possible, such as using:
...
A table is also possible, of either counts (Table \@ref(tab:PossumRoadkillNumber)) or percentages (Table \@ref(tab:PossumRoadkillPercentages)).


```{r PossumBarCharts, fig.height=10, fig.width=4.0, fig.cap="Three graphs: The number or percentage of possums found as road kill, by sex and season", fig.align="center"}
par( mfrow = c(3, 1))
colnames(Counts2) <- c("?", 
                       "M", 
                       "F")
...
Usually the best graph is one previously described, but sometimes the best graph is something different, perhaps unique.
Always remember the driving principle:

> The purpose of a graph is to display the information in the clearest, simplest possible way, to help the reader understand the message(s) in the data.

Importantly, a graph is needed that helps answer the research question.
...
plotA <- ggplot() + geom_polygon(data = ozdata, 
  ...
  annotate("text", x = 133, y = -20, label = "NT", col = grey(0.4)) +
  annotate("text", x = 145, y = -22, label = "Qld", col = grey(0.4)) +
  annotate("text", x = 121, y = -25, label = "WA", col = grey(0.4)) +
  annotate("text", x = 150, y = -31, label = "NSW", col = grey(0.4)) +
  annotate("text", x = 145, y = -36, label = "Vic", col = grey(0.4)) +
  annotate("text", x = 150, y = -43, label = "Tas", col = grey(0.4)) +
  annotate("text", x = 133, y = -29, label = "SA", col = grey(0.4)) +
...
Then a *case-profile plot* can be used: a plot that show how the response variable changes for each unit of analysis.
...
A study of children with atopic asthma [@data:Lothian2006:Whey] included a graph showing the immunogoblin E concentrations (IgE) before and after an intervention (Fig. \@ref(fig:PairedGraphCaseProfile)).
...
Consider the person in the case-profile plot whose line is at the top in Fig. \@ref(fig:PairedGraphCaseProfile).
...
These reductions could be computed for each person (Table \@ref(tab:IgEChanges)), and a histogram constructed based on these IgE *reductions*s, with one reduction for each person (Fig. \@ref(fig:PairedGraphHistDiffs)).
...
The difference (or *saving*) in energy consumption for each house can be graphed using a histogram, stemplot, or dot chart (Fig. \@ref(fig:DataInsulationGraphPlots), bottom panel).
...
Sometimes researchers prepare graphs with a focus on appearing fancy or clever, rather than focusing on clear communication.
One way people try to be fancy is to use a third dimension when it is not needed.
...
```{r Bar3D, fig.cap="A three-dimensional bar chart of the NHANES data", fig.align="center", fig.width=5.5, fig.height=4, out.width="85%"}
...
Given the purpose of a graph, communicating what the graph displays should be clear: Graphs need to be clear and well-labelled with captions.
...
One common optical illusion is produceed when the frequency (or percentage) axis on a bar chart  does not start at zero.
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
As with any study, possible confounding variables should be considered.
...
In this study, we cannot collect primary data.
However, data to answer this (and other) RQs are obtained from the American *National Health and Nutrition Examination Survey* (NHANES) [@data:NHANES3:Data; @data:NHANES3].
...
To answer any RQ, exploring and understanding the data is important. 
Graphs are a great way to do so (especially with large data sets).
...
```{r NHANESSmokeBarchart, fig.cap="A bar chart of current smoking status for the NHANES data. No response was recorded for many subjects", fig.align="center", fig.height=3.5, fig.width=4.75}
...
The NHANES study is an observational study, so confounding is a potential problem (Sect. \@ref(ExtraneousVariables)), so exploring the relationships between the response and extraneous variables, and between the explanatory and extraneous variables, is useful.
Some useful plots are shown in Fig. \@ref(fig:NHANESplots).
...
1. To compare the number of hours of sunlight exposure per day for female and male teachers.
`r if( knitr::is_html_output(exclude = "epub")) {longmcq( c(
   "A histogram of the differences", 
   answer = "A boxplot",
   "A stacked bar chart",
   "A histogram") )}`
2. To compare the proportion of trees with koalas in them, for trees more than 10 metres tall with trees 10 metres or shorter.
`r if( knitr::is_html_output(exclude = "epub")) {longmcq( c(
   "A pie chart", 
   answer = "A stacked bar chart",
   "A boxplot",
   "A histogram of the differences") )}`
3. To compare the number of hours spend on social media for people aged over 30, to people aged 30 and under.
`r if( knitr::is_html_output(exclude = "epub")) {longmcq( c(
   "A stacked bar chart",
   "A pie chart", 
   answer = "A boxplot",
