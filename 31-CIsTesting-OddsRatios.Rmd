...
(These values were also found in Sect.\ \@ref(ValidityChiSq).)
...
Those are the *expected* counts if the proportions (or odds) was exactly the same in each group (Table\ \@ref(tab:MealsDataTableExpected)), as stated by\ $H_0$.
...
As noted in Sect.\ \@ref(ValidityChiSq), *usually, you do not have to compute these expected values*, as software can produce the *expected* counts (e.g.,  Fig.\ \@ref(fig:UniMealsTestExpectedjamovi)).
...
The rainfall at Emerald (Queensland) was recorded for Augusts between\ 1889 to\ 2002 inclusive [@mypapers:dunnsmyth:glms], where the monthly average\ SOI was positive, and when the SOI was non-positive (that is, zero or negative), as shown in Table\ \@ref(tab:SOItableAnalysis).
...
The data are shown again in Table\ \@ref(tab:BirdsData), and the software output in Fig.\ \@ref(fig:PetBirdsjamovi).
...
1. Use the software output (Fig.\ \@ref(fig:PetBirdsjamovi), left panel) to find a $95$%\ CI for the odds ratio, making sure to describe the odds ratio carefully.
...
1. Determine the $\chi^2$-value and perform a hypothesis to answer the RQ, using the output in Fig.\ \@ref(fig:B12jamoviOutput).
