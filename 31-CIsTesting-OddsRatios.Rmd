...
@chen2023triage investigated the relationship between body temperature of patients admitted to hospital following a heart attack, with a subsequent heart attack (HA) while in hospital (Table\ \@ref(tab:HAttackData)).


1. Of patients with a low body temperature, what *percentage* had an in-hospital HA? \tightlist
`r if (knitr::is_html_output()) '(to *one* decimal place; **do not** add the percentage symbol)'`
`r if( knitr::is_html_output() ) {fitb(num = TRUE, tol = 0.01, answer = 14.8)}`
1. What are the *odds* that a patient with a low body temperature had an in-hospital HA?
`r if( knitr::is_html_output() ) {fitb(num = TRUE, tol = 0.001, answer = 0.174)}`
...
`r if( knitr::is_html_output() ) {torf(answer=FALSE)}`
1. True or false: Since the CIs for the difference between the proportions has a *negative* value, the CI is not statistically valid.
`r if( knitr::is_html_output() ) {torf(answer=FALSE)}`
:::
...
1. What *percentage* of small farms are adopters of the environmental practices???? \tightlist
`r if (knitr::is_html_output()) '(**do not** add the percentage symbol)'`
`r if( knitr::is_html_output() ) {fitb(num = TRUE, tol = 0.001, answer = 75)}`
1. What are the *odds* that a small farm adopts the environmental practices?????
`r if( knitr::is_html_output() ) {fitb(num = TRUE, tol = 0.001, answer = 3)}`
1. FIX THIS ONE: The OR in the output is given as $2.233$.
   What does this OR *mean*????????  
`r if( knitr::is_html_output() ) {mcq(
  c(answer = 1.71,
    0.68,
    4.28)  )}`
1. True or false: The CI means that the sample OR is likely to be between\ $1.41$ and\ $3.84$.  
`r if( knitr::is_html_output() ) {torf(answer = FALSE)}`
1. What is the $\chi^2$-value? \tightlist
`r if( knitr::is_html_output() ) {mcq(
