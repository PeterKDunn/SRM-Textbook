...
Hence, the difference between the two proportions, and the OR, varies between samples.
...
For this reason, software output only will be used for finding the CI for the OR, and the sampling distribution not discussed.
In other words, the CI is taken directly from software for ORs.
...
(If one of the qualitative variable has two levels,\index{Levels} the RQ may be able to be worded in terms of odds or proportions.)
...
```{r TurtleNestsGraphs, fig.show="hold", fig.cap="Bar chart for the turtle-nesting data.", fig.width=6, fig.height=3.2, fig.align="center", out.width="50%"}
par(xpd = TRUE,
...
The conclusion could also be written in terms of odds:
...
@chen2023triage investigated the relationship between body temperature of patients admitted to hospital following a heart attack (HA), and a subsequent HA while in hospital (Table\ \@ref(tab:HAttackData)).
