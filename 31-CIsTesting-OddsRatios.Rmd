...
Confirm that the corresponding test is not statistically valid.
:::
...
1. Write the hypothesis for comparing the *proportions*, for those with and without post-graduate study.
1. Use the software output to conduct the test.
1. Use the software output to write down the corresponding CI.
...
1. Based on the output, how is the difference between the two proportions defined?
1. Write the hypothesis for comparing the proportions, for farmers with small and large farms.
1. Use the software output to conduct the test.
1. Use the software output to write down the corresponding CI.
...
1. Compute the *difference between the proportion* of crashes involving a pedestrian from\ 2011 to\ 2015.
...
1. Write down the $95$%\ CI for the difference between the proportions using the output.
1. Interpret the CIs.
1. Conduct a hypothesis tests to compare the two proportions.
1. Confirm that the odds ratio in the output is correct.
1. Use the software output to write down a $95$%\ CI for the odds ratio, and interpret what it means.
1. Using the software output in Fig.\ \@ref(fig:PetBirdsjamovi), perform a hypothesis test to determine if the odds of having a pet bird is the same for people with and without lung cancer.
1. Are the CIs and tests statistically valid?
...
1. Compute the difference between the proportions of wet Augusts, for months with a positive SOI compared to months with a non-positive SOI.
1. Compute the standard error needed to compute the CI for the difference in proportions.
1. Compute the standard error needed to conduct the hypothesis to compare the proportions.
1. Explain *why* the two standard errors have slightly different values.
1. Compute an approximate $95$%\ CI for the difference between the two proportions.
...
1. Interpret the CIs.
1. Use the software output to write down, then interpret, a $95$%\ CI for the odds ratio.
...
1. Compute the odds of a fissure $1\,800$\hs and $3\,000$\hs, and hence the odds ratio.
1. Compute an approximate $95$%\ CI for the difference between the proportions.
1. Write down the $95$%\ CI for the difference between the proportions.
1. Interpret the CIs.
1. Use the software output (Fig.\ \@ref(fig:TurbinesOutput)) to write down, then interpret, a $95$%\ CI for the odds ratio.
1. Use the software output to test for a relationship.
1. Are the CIs and tests statistically valid?
:::
...
@witmer2020preliminary compared various types of repellents (including bear faeces) to stop bears damaging trees in an Idaho forest.
...
[*Dataset*: `B12Diet`]
@data:Gammon2012:B12 examined B12\ deficiencies in 'predominantly overweight/obese women of South Asian origin living in Auckland', some of whom were on a vegetarian diet and some of whom were on a non-vegetarian diet (Table\ \@ref(tab:B12Data)).
One RQ was:
