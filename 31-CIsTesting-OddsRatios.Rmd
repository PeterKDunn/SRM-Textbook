...
if( knitr::is_latex_output() ) {
  ...
  OR2Notation[2, ] <- c( "$\\text{Odds}_1$", 	
                         "$\\text{Odds}_2$",
                         "$\\text{Odds ratio} = \\text{Odds}_1/\\text{Odds}_2$")
...
\begin{equation}
\text{s.e.}(\hat{p}_1 - \hat{p}_2) = \sqrt{ \text{s.e.}(\hat{p}_1)^2 + \text{s.e.}(\hat{p}_2s)^2},
...
The standard error of the sample proportion for each sample is computed using this common proportion\ $\hat{p}$, using the same idea as in Eq.\ \@ref(eq:stderrorphat):
\begin{align*}
...
\index{Sampling distribution!odds ratio}\index{Odds ratio}
...
Software\index{Software output!comparing two odds (odds ratio)}\index{Confidence intervals!compare two odds (odds ratio)} (Fig.\ \@ref(fig:EatingGraphs), right panel) gives the sample OR as\ $5.94$, and the (exact) $95$%\ CI as\ $1.35$ to\ $26.1$.
...
are all relative to those in the\ $51$ to\ $65$ age group (hence, no odds ratio is given for the $51$ to\ $65$ age group, which is the *reference level*).\index{Reference level}
...
Using Eq.\ \@ref(eq:SmallestExpectedCount), compute
$$
\frac{(\text{Smallest row total})\times(\text{Smallest column total})}{\text{Overall total}}
=
\frac{46\times 43}{289}
...
To compute a confidence interval (CI) for the difference between two proportions, compute the difference between the two sample proportions, $\hat{p}_1 - \hat{p}_2$, and identify the sample sizes\ $n_1$ and\ $n_2$.
...
$$
where $\text{s.e.}(\hat{p}_1)$ and $\text{s.e.}(\hat{p}_2)$ are the standard errors of Groups\ $1$ and\ $2$ (Eq.\ \@ref(eq:stderrorphat)).
