...
Since two groups are being compared, subscripts are used to distinguish between the statistics for the two groups; say, Groups\ $1$ and\ $2$ in general (Table\ \@ref(tab:ORHT)).
...
The parameter can be either a difference between two population proportions, or a population OR.
...
Hence, the difference between the two proportions, and the OR, will vary between samples.
This means that both the difference between the two proportions, and the OR, have sampling distributions.


## Comparing proportions: confidence intervals
\index{Confidence intervals!comparing two proportions|(}\index{Sampling distribution!compare two proportions}\index{Difference between proportions}
...
The *sampling distribution of the difference between two sample proportions*\ $\hat{p}_1$ and\ $\hat{p}_2$ is (when the appropriate conditions are met; Sect.\ \@ref(ValidityChiSq)) described by:
...
* with a standard deviation of $\text{s.e.}(\hat{p}_L - \hat{p}_N) = 0.04282954$.
...
This approximate\ CI is very similar to the (exact) CI from software (Fig.\ \@ref(fig:EatingGraphs)).\index{Software output!compare two proportions}
We write:

> The difference between the proportions of students eating most meals at home is\ $0.1490$, higher for those living with their parents ($0.963$; $n = 52$) that those not living with their parents ($0.814$; $n = 129$), with the $95$%\ CI from\ $0.0633$ to\ $0.235$.
...
## Comparing proportions: $z$-test {#CompareTwoProportions}
...
$$
similar to Equation\ \@ref(eq:SEtwoproportionsCI).
...
## Comparing odds: confidence interval for an odds ratio
\index{Sampling distribution!odds ratio}\index{Odds ratio}\index{Confidence intervals!compare two odds (odds ratio)|(}

A CI can be formed for the difference between the two proportions, and a CI can also be formed for the OR.
...
For this reason, we will only use software output for finding the CI for the OR, and not discuss the sampling distribution directly.
In other words, we will read the CI directly from software for ORs.
...
:::
\index{Confidence intervals!compare two odds (odds ratio)|)}
...
## Comparing odds: $\chi^2$-test {#CompareTwoOdds}
...
From Table\ \@ref(tab:MealsDataTable), $157$\ students out of $183$ ate most meals off-campus, so that $157\div 183 = 0.8579$ of students in the entire sample ate most of their meals off-campus (the same value found in Equation\ \@ref(eq:OverallP)).
...
In this context, the test-statistic is the 'chi-squared' statistic, written $\chi^2$.\index{Test statistic!$\chi^2$-score}
...
::: {style="display: flex;"}
The Greek letter $\chi$ is pronounced 'kie', as in **ki**te (*not* 'chi' as in **Chi**na).
...
However, a quick check for the statistical validity is to compute the *smallest* expected value, using
\begin{equation}
...
The alternative hypothesis encompasses many possibilities: for example, that the four odds are *all* different from each other, or that the odds for the first two age groups (which are the same) are different from for the other two (which are the same).
...
Using Equation\ \@ref(eq:SmallestExpectedCount), compute
$$
  \frac{(\text{Smallest row total})\times(\text{Smallest column total})}{\text{Overall total}}
  =
...
The corresponding hypotheses can be written using proportions, after defining\ $p$ as the proportion of nests that are *infected*:

* $H_0$: $p_N - p_R =   0$ (equivalent to $p_N =   p_R$); and
* $H_1$: $p_N - p_R \ne 0$ (equivalent to $p_N \ne p_R$),

where\ $N$ refers to **N**atural nests, and\ $R$ to **R**elocated nests.
The parameter is the difference between the population proportion.
...
The parameter is the OR of infection, comparing natural to relocated nests.
...
Either way, there is no evidence that relocating the nest (to protect them from tidal inundation) changes the risk of infection.
...
(Odds could be produced in the numerical summary also, but ORs are trickier since they require comparing pairs of odds.)
...
The hypotheses are usually worded in terms of associations (or *relationships*):
...
> The sample provides very strong evidence ($\chi^2 = 23.585$; $3\times 3$ table) of an association between reproductive status and overall health of female burros ($n = 315$).
...
Then compute the standard error, which quantifies how much the value of $\hat{p}_1 - \hat{p}_2$ varies across all possible samples, is
$$
...
To test a hypothesis about a difference between two population proportions $p_1 - p_2$:

* write the null hypothesis\ ($H_0$) and the alternative hypothesis\ ($H_1$).
* initially *assume* the value of $(p_1 - p_2)$ in the null hypothesis to be true.
* then, describe the *sampling distribution*, which describes what to *expect*  from the difference between the sample proportions based on this assumption: under certain statistical validity conditions, the difference between the sample proportions vary with:
   ...
   *  having a standard deviation of $\displaystyle \text{s.e.}(\hat{p}_1 - \hat{p}_2)$.
