...
A stacked bar chart would be useful for comparing proportions.
...
> The difference between the proportions of students eating most meals at home is\ $0.1490$, higher for those living with their parents ($0.963$; $n = 52$) than those not living with their parents ($0.814$; $n = 129$), with the $95$%\ CI from\ $0.0633$ to\ $0.235$.
...
The value of the OR is the same as teh value computed manually.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
You *do not* have to compute the expected counts\index{Expected counts} explicitly (software does it in the background).
...
(If one of the qualitative variable has two levels,\\index{Qualitative data!levels} the RQ may be able to be worded in terms of odds or proportions.)
...
@candan2021first compared the odds of a nest being infected, between relocated nests (relocated due to the risk of tidal inundation), and non-relocated nests
`r if (knitr::is_latex_output()) {
...
Here, the parameter is the OR of infection, comparing natural to relocated nests.
