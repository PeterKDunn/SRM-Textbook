...
# CIs; mean differences (paired data) {#PairedCI}
...
The data are summarised graphically (Fig.\ \@ref(fig:FloweringPlots)) and numerically (Table\ \@ref(tab:FloweringSummary)).
and software output (Fig.\ \@ref(fig:Floweringjamovi)) are repeated here.
...
(This study was seen in Sects.\ \@ref(ChamomileTea-Paired-HT) and\ \@ref(ChamomileTea-TwoMeans-HT).)
...
For the two groups, approximate $95$%\ CI for the *reduction* in TG are (using the standard errors in Sect.\ \@ref(ChamomileTea-Paired-HT)):
...
[*Dataset*: `Fruit`]
(This study was also seen in Exercise\ \@ref(exr:MeanDiffGrowingSquashHT).)
...
[*Dataset*: `Captopril`]
(This study was also seen in Exercise\ \@ref(exr:TestPairedMeansCaptopril).)
...
(This study was also seen in Exercise\ \@ref(exr:TestPairedMeansTasteOfBroccoli).)
...
::: {.exercise #CIPairedMeansFerritin}
[*Dataset*: `Ferritin]
(This study was also seen in Exercise\ \@ref(exr:TestPairedMeansFerritin).)
...
`r if (knitr::is_latex_output()) {
   ...
   'in Exercise\\ \\@ref(exr:TestPairedMeansFerritin),'
}`
construct a $95$% CI to estimate the reduction in ferritin content.
:::
