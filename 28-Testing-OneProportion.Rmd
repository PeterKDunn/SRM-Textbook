...
If I rolled the *fair* die, I'd know what to expect: each face would appear about one-sixth of the time (using [classical probability](#ProbClassical)).
...
If the die was fair, I would expect about one-sixth of rolls to produce a `r include_graphics("Dice/die1.png", dpi=1500)`... but not necessarily *exactly* one-sixth of the time, due to *sampling variation*.
But by initially assuming the proportion of ones would be about $1/6$, it is possible to describe what values of the *sample* proportion would be expected from all possible rolls of the fair die.

This is the [decision-making process](#DecisionMaking).
Initially, make an assumption about the population: assume that the die is fair (I have no evidence against this), and hence that the *population* proportion of rolling a `r include_graphics("Dice/die1.png", dpi=1500)` is $p = 1/6$, or approximately $p = 0.16667$.
Then, compare what is seen in practice (the sample proportion, $\hat{p}$) to how the sample proportions might be distributed due to sampling variation.

If the sample proportion of rolls that are `r include_graphics("Dice/die1.png", dpi=1500)` is not *exactly* $0.1666...$, two possibilities exist:
...
Earlier (Sect. \@ref(SamplingDistributionKnownp)), the [sampling distribution of a sample proportion](#def:SamplingDistProp) was given.
For an assumed value of $p$, the sample proportion $\hat{p}$ across all possible samples is expected to vary, and this variation is described by
...
So, if $p$ really was $0.1666...$, and if [certain conditions are met](#ValidityProportionsTest), the possible values of the sample proportion that could be expected across all possible samples of size $100$ would be described using:
...
One way to measure how far the sample proportion $\hat{p} = 0.41$ is from the population proportion $p = 0.166\dots$ in 100 rolls is to use a $z$-score, since the sampling distribution (Fig. \@ref(fig:RollsSixesSD)) has an approximate normal distribution.
The $z$-score is
...
$P$-values can be approximated using the 68--95--99.7 rule with a diagram (Sect. \@ref(ApproxPProportion)), or more precisely using the $z$-tables (App. \@ref(ZTablesOnlineBackwards); see Sect. \@ref(Z-Score-Forestry)).
...
For *two-tailed* $P$-values, the $P$-value is the combined area in the two tails; for *one-tailed* $P$-values, the $P$-value is the area in one tail.
For example:
...
```{r, OnePropTestP, fig.cap="The two-tailed P-value is the combined area in the two tails of the distribution; left panel: when $z = 1$; right panel: when $z = 2$", fig.width=10, fig.height=3, out.width='90%', fig.align="center"}
par(mfrow = c(1, 2),
    mar = c(4, 1, 4, 1) + 0.1)
...
The $P$-value is a little smaller than 0.32.
...
The $P$-value is a little larger than 0.05.
Clearly, from what the $P$-value means, a $P$-value is always between 0 and 1.

In this example, where the $z$-score is 6.53, the tail area is *very* small, and zero to four decimal places.


```{r OnePropTestP2, echo=FALSE, fig.cap="The two-tailed P-value is the combined area in the two tails of the distribution; left panel: when $z = 1.2$; right panel: when $z = 1.9$", fig.align="center", fig.width=10, fig.height=3, out.width='90%'}
par( mfrow = c(1, 2))
...
* 'Big' $P$-values mean that the sample statistic (i.e., $\bar{p}$) could reasonably have occurred through sampling variation in one of the possible samples, if the assumption made about the parameter (stated in $H_0$) was true: 
   The data *do not* contradict the assumption in $H_0$.
* 'Small' $P$-values mean that the sample statistic (i.e., $\hat{p}$) is unlikely to have occurred through sampling variation in one of the possible samples, if the assumption made about the parameter (stated in $H_0$) was true: 
...
```{r PvaluesBigSmall, echo=FALSE, fig.cap="The strength of evidence: P-values. As the $z$-score becomes larger, the $P$-value becomes smaller, and the evidence is greater to support the alternative hypothesis.", fig.height = 2.5, fig.width=7, out.width='80%', fig.align="center", dev=if (is_latex_output()){"pdf"}else{"png"}}
...
* An answer to the RQ.
  Since the null hypothesis is assumed to be true, the onus is on the evidence to support the alterative hypothesis.
  Hence, conclusions are worded in terms of how much evidence exists to support the *alternative* hypothesis.
* The evidence used to reach that conclusion (such as the $z$-score and $P$-value, including if it is a one- or two-tailed $P$-value).
...
2. **Expectation**: 
   The sampling distribution describes what values to expect from the sample statistic across all possible sample,  *if* the null hypothesis is true: under certain circumstances, the sample proportions will vary with an approximate normal distribution around a mean of $\mu = 0.1666...$ with a standard deviation of $\text{s.e.}(\hat{p}) = 0.0372678$.
3. **Observation**: 
...
* $H_1$: $p < 0.10$ (one-tailed, since the RQ is whether the target is 10% or *lower*, from the RQ).
