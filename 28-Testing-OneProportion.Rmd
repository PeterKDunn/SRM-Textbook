...
So I could roll both dice, and see how often a `r include_graphics("Dice/dice6.png", dpi=2000)` (for example) actually appeared.
...
On a fair die, rolling a `r include_graphics("Dice/dice6.png", dpi=2000)` *exactly* one-sixth of the time is unlikely, but it probably should be close.
This is called [*sampling variation*](#SamplingVariation).
So, when we see that the sample proportion of `r include_graphics("Dice/dice6.png", dpi=2000)`s is not *exactly* $0.1666...$ two possible explanations are:
...
When the proportion of rolls that show a `r include_graphics("Dice/dice6.png", dpi=2000)` is $p = 0.1666...$, what values of the *sample* proportion are reasonable to expect?
...
In 100 rolls of one of the dice, I actually observed 41 rolls that were a `r include_graphics("Dice/dice1.png", dpi=2000)`, sample proportion of $\hat{p} = 41/100 = 0.41$, which is almost impossible *if the die was fair*.
