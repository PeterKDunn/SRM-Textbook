...
\begin{wrapfigure}{R}{.27\textwidth}
  \begin{center}
    \includegraphics[width=.24\textwidth]{Illustrations/LoadedDice.png}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
If I rolled the *fair* die, I'd expect that each face would appear *approximately* (but not exactly) one-sixth of the time (using classical probability; Sect.\ \@ref(ProbClassical)).
...
By initially assuming the population proportion of ones would be $1/6$, the possible values of the *sample* proportion from all possible rolls of the fair die could be determined; that is, the *sampling distribution* could be described.
...
* The *population* proportion *really is* $1/6$, and the *sample* proportion is not exactly $p = 1/6$ due to sampling variation; or
* The *population* proportion *really is not* $1/6$; that is, the *sample* proportion is not exactly $p = 1/6$ because the die is not fair.
...
The null hypothesis proposes that the difference between the hypothesised population proportion and the observed sample proportion is due to *sampling variation*.

Notice that the hypotheses always propose values for the unknown *population* proportion (the *parameter* $p$).
Proposing values for the *known* sample proportion (i.e., the statistic $\hat{p}$) is silly: we *know* the value of $\hat{p}$.
...
In Chap.\ \@ref(CIOneProportion), the sampling distribution of a sample proportion was given.
Since hypothesis testing begins by making an *assumption* about the parameter (here, assuming that $p = 1/6$), 
the sampling distribution can be described as follows (see Sect.\ \@ref(SamplingDistributionKnownp)).
