...
```{r, child = if (knitr::is_html_output()) {'./introductions/HT-OneProp-HTML.Rmd'} else {'./introductions/HT-OneProp-LaTeX.Rmd'}}
```
...
The alternative hypothesis may or may not be true... but the evidence (data) available strongly supports the alternative hypothesis.
...
1. **Assumption**: Write the *null hypothesis* about the *parameter* (based on the RQ): $H_0$: $p = 01.666...$.  
...
3. **Observation**: Compute the $z$-score: $z = 6.53$.
4. **Consistency?**: Determine if the data are consistent with the assumption, by computing the $P$-value. 
...
* $H_1$: $p < 0.10$ (one-tailed, since the research question is whether the target is 10% or lower).
...
\end{align*}
This is a *very* large (and *negative*) $z$-score, so expect a *very* small $P$-value from using the 68--95--99.7 rule, so there is very strong evidence to support the alternative hypothesis.
We write:
...
> There is *very* strong evidence ($P < 0.001$; $z = 4.325$) that the interactions were not won equally between each species ($\hat{p} = 0.8222$ won by green-backed tits; $n = 45$; approximate 95% CI: 0.708 to 0.936).
...
ONE or more needed for exercises in tutorial book too.
