...
\begin{wrapfigure}[10]{R}{.25\textwidth}
  ...
  \vspace{-50pt} % This removes some white space
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
\end{align*}
(Remember that the standard deviation of the distribution in Fig.\ \@ref(fig:RollsSixesSD) is the standard error: the amount of variation in the sample proportions.)
...
$P$-values are also reported by software.
...
Since the $P$-value is a probability (albeit a probability of something quite specific), so is a value between $0$ and $1$. 
...
#  par( mar = c(0.1, 0.1, 0.1, 0.1) ) # Number of margin lines on each side
  ...
  zList <- c( 1.25, # Two-tailed P-value: 10% -1.645
              2.3 ) # Two-tailed P-value: 1% -2.576
...
The *conclusion* is that very strong evidence exists that $p$ is *not* $1/6$, based on the evidence.
