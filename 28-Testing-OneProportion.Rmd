...
Suppose I rolled the *fair* die.
I'd expect that each face would appear *approximately* (but not exactly) one-sixth of the time (using classical probability; Sect.\ \@ref(ProbClassical)).
So, I could roll one die, and see how often a `r include_graphics("Dice/die1.png", dpi=1500)` (for example) actually appeared.
...
> For this die, is the proportion of rolls that show a `r include_graphics("Dice/die1.png", dpi=1500)` equal to $1/6$?
...
Sometimes the value of $\hat{p}$ would be a little smaller than $1/6$, and sometimes a little larger.
...
Then, reasonable values of the sample proportion to expect from all possible samples can be compared to the observed value of $\hat{p}$ from just one of those possible samples.

If the sample proportion of rolls that are `r include_graphics("Dice/die1.png", dpi=1500)` is not *exactly* $1/6$, two explanations exist:

* The *population* proportion *really is* $p = 1/6$, and the *sample* proportion $\hat{p}$ is not exactly $1/6$ due to sampling variation; or
* The *population* proportion *really is not* $p = 1/6$; that is, the *sample* proportion $\hat{p}$ is not exactly $1/6$ because the die is not fair.
...
The *null hypothesis* proposes that the reason for difference between the hypothesised population proportion and the observed sample proportion is due to *sampling variation*.
The *alternative hypothesis* proposes that the value of the population proportion in the null hypothesis is incorrect (i.e., the die is not fair), which explains the observed value of the sample proportion.
...
*Hypothesis testing always begins by assuming the null hypothesis is true*, and hence that $p = 1/6$ here. 
In Chap.\ \@ref(CIOneProportion), the sampling distribution of a sample proportion was given, so the sampling distribution can be described as follows (see Sect.\ \@ref(SamplingDistributionKnownp)).


::: {.definition #DEFSamplingDistributionPhat name="Sampling distribution of a sample proportion when $p$ is unknown"}
The *sampling distribution of the sample proportion* is (when certain conditions are met; Sect.\ \@ref(ValidityProportionsTest)) described by
...
\end{equation}
where $n$ is the size of the sample, and $p$ is the assumed value of the population proportion (from $H_0$).
...
Similarly, the standard deviation of this distribution is denoted $\text{s.e.}(\hat{p})$, the standard deviation of all possible values of the statistic $\hat{p}$.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The notation $\text{s.e.}{\hat{p}}$ is the *standard error of the sample proportion*, and denotes 'the standard deviation of the proportions computed from all possible samples of a given size\ $n$'.
:::
...
* The formula for a hypothesis test uses the *population proportion* $p$ from the null hypothesis (see Eq.\ \@ref(eq:StdErrorPknownTest)), since hypothesis testing *assumes that the null hypothesis is true*, and hence the value of $p$ is known.
...
Values of $\hat{p}$ between about $12$% and $20$% would seem to occur reasonably frequently when $p = 1/6$.
Values of $\hat{p}$ larger than $0.25$ look unlikely when $n = 100$; values less than $0.10$ also looks quite unlikely, but not impossible.
A value above $0.3$, or lower than $0.05$, looks almost impossible.


```{r, RollsSixesSD, fig.height=3.25, fig.width=9, out.width='90%', fig.align="center", fig.cap="The sampling distribution, showing the distribution of the sample proportion of 1s when the population proportion is $1/6$, in $50$ die rolls"}
p <- 1/6
n <- 100
mu <- p
...
\end{align*}
(Remember that the standard deviation of the distribution in Fig.\ \@ref(fig:RollsSixesSD) is the standard error: the amount of variation in the sample statistic.)

In this context, the $z$-score is called a *test statistic*.
It tells us that the observed sample proportion is more than six standard deviations from the mean, which is *highly unusual* according to the [68--95--99.7 rule](#def:EmpiricalRule).
...
For *two-tailed* $P$-values, the $P$-value is the combined area in the two tails.
...
The 68--95--99.7 can be used to determine *approximate* $P$-values only.
For example:
...
```{r, OnePropTestP, fig.cap="The two-tailed P-value is the combined area in the two tails of the distribution; left panel: if $z = 1$ (or $z = -1$); right panel: if $z = 2$ (or $z = -2$). (The one-tailed $P$-values are half the two-tailed $P$-values: $P = 0.16$ and $P = 0.025$.)", fig.width=10, fig.height=3, out.width='90%', fig.align="center"}
par(mfrow = c(1, 2), 
    mar = c(4, 1, 4, 1) + 0.1)
...
out <- plotNormal(mu = 0,
           sd = 1,
           main = expression( atop(The~italic(P)*"-value"~when~italic(z)==1.2*":",
...
out <- plotNormal(mu = 0,
           sd = 1,
           main = expression( atop(The~italic(P)*"-value"~when~italic(z)==1.9*":",
...
Using the tables of areas under normal distributions (`r if ( knitr::is_html_output()) { 'Appendix\\ \\@ref(ZTablesOnline).'} else {'Appendices\\ \\@ref(ZTablesNEG) and \\@ref(ZTablesPOS)'}`), we can be more precise when computing the $P$-values, using the ideas from Sect.\ \@ref(ExactAreasUsingTables).
...
In this context, the $P$-value is the probability of observing the value of $\hat{p} = 0.41$ (or more extreme), just through sampling variation (chance) if $p = 1/6$.
...
> The sample provides very strong evidence ($z = 6.53$; two-tailed $P < 0.001$) that the proportion of sixes is not $1/6$ ($\hat{p} = 0.41$; $n = 100$ rolls) in the population.
...
1. *Assumption*: 
   Write the *null hypothesis* and *alternative hypothesis* about the *parameter* (based on the RQ): 
   * $H_0$: $p = 1/6$, and 
...
2. *Expectation*: 
   ...
   Under certain circumstances, the sample proportions will vary with an approximate normal distribution around a mean of $p = 1/6$ with a standard deviation of $\text{s.e.}(\hat{p}) = 0.0372678$.
3. *Observation*: 
   Compute the $z$-score: $z = 6.53$, a measure of the distance between the assumed population value, and the observed sample value.
4. *Consistency?*: 
...
The *conclusion* is that very strong evidence exists that $p$ is *not* $1/6$.
...
* $H_0$: $p = 0.10$ (i.e, assume the target is met, and the difference between $p$ and $\hat{p}$ is due to sampling variation); and
...
Either way, the evidence very strongly supports the alternative hypothesis.
We write:

> Very strong evidence exists in the sample ($z = -4.51$; one-tailed $P < 0.001$) that the population proportion is less than the target of $p = 0.10$ (Korean sample proportion: $\hat{p} = 0.0566$; $n = 972$; approximate $95$% CI from $0.042$ to $0.071$).
:::
...
This condition ensures that the *sampling distribution of the sample proportions has an approximate normal distribution* (so that, for example, the [68--95--99.7 rule](#def:EmpiricalRule) can be used).
...
::: {.exercise #StatisticalValidityMeasles}
Is the hypothesis test in Example\ \@ref(exm:POnePropTestMeasles) statistically valid?
:::
...
One study [@loyeung2018experimental] examined if subjects could identify potential placebos at *better* rate than just guessing.
...
In a study of western saw-shelled turtles [@streeting2022optimising], eggs were incubated at $27^\circ$C, and $29$ males and $44$ females hatched.
...
In the 2019/2020 English Premier League (EPL), the home team won $91$ out of $208$ games, while the away team won $67$.
($50$ games were draws.)
(Data from: https://sports-statistics.com/sports-data/soccer-datasets/)
*Ignoring draws*, is there evidence of a home-side advantage (i.e., the home-side winning percentage is greater than $50$%)?
:::
...
In a 1995 study [@koenen1995analysis], $88$ of the $357$ visitors to Las Vegas casinos were smokers.
...
A study of black spiny-tailed iguanas in Florida (an invasive species) compared the snout--vent length (SVL) for  various sizes iguanas [@avery2014invasive].
...
In a study of streetlight preferences of drivers [@davidovic2019drivers], drivers were asked to conduct a series of manoeuvres under $3000$K LED light and then under $4000$K LED lights.
