...
So I could roll both dice, and see how often a `r include_graphics("Dice/die6.png", dpi=1250)` (for example) actually appeared.
...
The [decision-making process](#DecisionMaking) begins by making an assumption about the population; in this case, we assume that the die is fair, and hence that the *population* proportion is $p = 1/6 = 0.1666...$.
With this knowlegde, I can compare what I see in practice to what I expect if the die is fair. (I AND WE!!)

On a fair die, rolling a `r include_graphics("Dice/die6.png", dpi=1250)` *exactly* one-sixth of the time is unlikely, but it probably should be close.
This is called [*sampling variation*](#SamplingVariation).
So, when we see that the sample proportion of `r include_graphics("Dice/die6.png", dpi=1250)`s is not *exactly* $0.1666...$ two possible explanations are:
...
When the proportion of rolls that show a `r include_graphics("Dice/die6.png", dpi=1250)` is $p = 0.1666...$, what values of the *sample* proportion are reasonable to expect?
The answer depends on the sample size.
For example, in *one* roll of a die, rolling a `r include_graphics("Dice/die6.png", dpi=1250)`, and hence finding a sample proportion of $\hat{p} = 1$, is not unreasonable.
However, if we rolled a die 20,000 times, rolling a `r include_graphics("Dice/die6.png", dpi=1250)` with a sample proportion of $\hat{p} = 1$ would be *incredibly* unlikely.
...
For some value of $p$, the *sampling distribution of the sample proportion* is described by
...
That is, how many times are we prepared to roll the die and count how many times a `r include_graphics("Dice/die6.png", dpi=1250)` shows?


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Suppose I wanted to estimate the proportion of ones to within 0.01.
Using the ideas in Chap. \@ref(EstimatingSampleSize), what size sample would I need (i.e., how many rolls of the die)?
...
I decide to use 100 rolls initially.
Then, if $p$ really was $0.1666...$, and if [certain conditions are met](#ValidityProportionsTest), the possible values of the sample proportion can be described using:
...
From this picture, a value of $\hat{p}$ larger than 0.25 looks unlikely; a value less than 0.10 also looks quite unlikley.
A value above 0.3, or lower than 0.05, looks almost impossible.

```{r, RollsSixesSD, fig.cap="The sampling distribution, showing the distribution of the sample proportion of 6s when the *population* proportion is 0.1666..., in 50 die rolls"}
...
In 100 rolls of one of the dice, I actually observed 41 rolls that were a `r include_graphics("Dice/die1.png", dpi=1250)`, sample proportion of $\hat{p} = 41/100 = 0.41$, which is almost impossible *if the die was fair*.
What I observed was almost impossible... but I really did observe it.
A reasonable conclsuion is that the assumption I was making---that the die is fair---is not tenable.


## The test statistic: $z$-score {#OnePropTestStatistic}

One way to quantify how unusual it is to observe a value of $\hat{p} = 0.41$ in 100 rolls is to use $z$-scores, since the sampling distribution (Fig. \@ref(fig:RollsSixesSD)) has a normal distribution.
The $z$-score is

\begin{align*}
   z 
   &= \frac{\text{sample statistic} - \text{population mean}}{\text{standard deviation of statistic}} \\
   &= \frac{\hat{p} - p }{\text{s.e.}(\hat{p})} = \frac{0.41 - 0.1666...}{037267} = 6.53.
\end{align*}
(Remember that the standard deviation of the distribution in Fig. \@ref(fig:RollsSixesSD) is the the standard error: the amount of variation in the sample means.)
...
$P$-values can be approximated using the 68--95--99.7 rule and a diagram (Sect. ???).
...
> The sample provides very strong evidence ($z = 6.53$; two-tailed $P < 0.0013$) that the proportion of sixes is not $1/6$ ($n = 100$ rolls; 41 sixes).
...
The hypothesis test regarding the dice is statistically valid, since the number of...
