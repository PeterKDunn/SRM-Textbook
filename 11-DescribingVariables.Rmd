...
*Quantitative research* uses both *quantitative* and *qualitative* variables, because both can be summarised numerically (Chaps.\ \@ref(SummariseQuantData) and\ \@ref(SummariseQualData) respectively) and analysed numerically.
:::
...
In practice, though, values are rounded to a reasonable number of decimal places.
...
Another approach is to record if each bag of pasta was underweight or not (perhaps using a balance scale).
...
The variables in the dataset `WaterAccess` are described in Tables\ \@ref(tab:WaterAccessQualVariables) and\ \@ref(tab:WaterAccessQualVariables).
...
WATableQual[3, 3] <- "Under 100m; 100m to 1000m; over 1000m"
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(SelfieDeaths,
                      decDigits = c(0, 1)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
