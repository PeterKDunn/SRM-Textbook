...
**In this chapter**, you will learn how to describe the data, because this determines how to proceed with the analysis.
You will learn to:
...
The *variable* is the *description* of what varies, and the *data* are the *values* of the variables that are recorded.
Quantitative variables record quantitative data, and qualitative variables record qualitative data.
...
Quantitative data is often (but not always) measured with measurement units (such as *kg* or *cm*).
...
*Quantitative data* is *mathematically* numerical data: the numbers themselves have numerical meaning, and it makes sense to be able to perform mathematical operations on them.
Most data that are counted or measured are quantitative. 
:::


Be careful: Numerical data are not necessarily quantitative.
*Mathematically numerical data are quantitative*; that is, numbers with numerical *meanings*.


<div style="float:right; width: 222x; border: 1px; padding:10px">
<img src="Illustrations/pexels-carlos-cuadros-979959.jpg" width="200px"/>
</div>


::: {.example #QuantitativePostcodes name="Quantitative data"}
Australian postcodes are numbers, but are *not* quantitative.
...
Rather than numerical postcodes, alphabetic postcodes could have been chosen.
For example, the post code of Caboolture is 4510, but could have been QCAB for instance.
:::
...
Sometimes, the possible values may not have a theoretical upper limit, yet are be still considered 'countable'.
...
These (quantitative) variables are *discrete* (and so record *discrete* quantitative data):
...
These (quantitative) variables are *continuous* (and so record continuous quantitative data):
...
* The *weight* of 6-year-old Australian children.
  ...
  Weights are usually given to the nearest kilogram
...
Sometimes, discrete quantitative data with very large values may be treated as continuous.

:::{.example #DiscreteAsContinuous name="Treating discrete data as continuous"}
Annual income is discrete, since income cannot be paid in fractions of a cent.
...
**Qualitative** data has distinct labels or categories that are not mathematically numerical.
These categories are called the *levels* or the *values* of the variable.
...
Many levels are possible (that is, many possible brands), but could be simplified by defining the levels as 'Huawei', 'Apple', 'Samsung', 'Google' and 'Other'.
:::
...
What features of the data collected from the questions are similar? 
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-tristan-le-1642883.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
The variable 'Age' is *continuous quantitative*, since age could be measured to many decimal places of a second.
...
Sometimes *Age group* is used instead (such as Under 20; 20 to under 30; 30 to under 50; 50 or over).
...
Consider a study to determine if the weight of 500g bags of pasta really is at least 500g.
...
Another approach is to record whether or not each bag of pasta weighed at least 500g (bags are not underweight).
...
In practice, quantitative research requires the use of a computer for producing graphs and completing calculations.
In this book, two statistical software packages (rather than [spreadsheets](#Software-In-Research)) are mentioned for analysis of data: [**jamovi**](#UsingJamovi) and [**SPSS**](#UsingSPSS).

To use these packages (any most other) packages correctly, declaring the variables correctly is important (Table \@ref(tab:TypesOfVars)).
...
colnames(VarTypes) <- c("Type of variable", 
...
To specify the type of variable in jamovi, double-click on the variable name at the top of the data worksheet (Fig. \@ref(fig:VarNamesjamovi)), which produces 
...
* Quantitative *discrete* variables are set as `Continuous` (confusingly),  with the *Data type* as `Integer`.

When the information has been entered, clicking the up-arrow on the top right (Fig. \@ref(fig:VarTypesjamovi)) closes this window.
...
In SPSS, variables are described in the *Variable View* window (*not* the *Data View* window).
Each variable is then described in the *Measure* column (Fig. \@ref(fig:VarTypesSPSS)):
...
Variables should be correctly defined in software.


## Quick revision questions {#Chap11-QuickReview}

A study on the bruising of apples [@doosti2016development] aimed to determine the relationship between the recorded surface temperature of apple, the depth of bruising.
...
1. Are these variables **quantitative** and **continuous**?
...
1. Are these variables **qualitative** and **nominal**?
...
1. A study recorded whether or not people (who were not swimming) were wearing head-protection at the beach. The results were recorded as None; Cap; or Hat. 

   a. Which of the following could be used to describe this variable?
...
`r if( knitr::is_html_output(exclude = "epub") ) {torf( answer = FALSE ) }`
   * Ordinal
`r if( knitr::is_html_output(exclude = "epub") ) {torf( answer = FALSE ) }`
   b. How many *levels* does this variable have?
...
(GMFCS is used to describe the impact of cerebral palsy on their motor function, where *lower* levels means *better* functionality: the [Gross Motor Function Classification System](https://en.wikipedia.org/wiki/Gross_Motor_Function_Classification_System).)
...
A study [@brunton2019fright] recorded the response of kangaroos to drones (one of 'Vigilance', 'No vigilance', 'Flee $<10$m', or 'Flee $>10$m') and the altitude of the drone (30m, 60m, 100m or 120m).
...
**1.**  'The knee-flex angle after treatment' and  'Length of time between arrival at an emergency department, and admission'.
**2.** 'Whether a cyclist is wearing a helmet or not and 'The brand of fertilizer being applied'.

**1a.** Nominal, qualitative.
**1b.** Three levels.
:::
`r if (knitr::is_html_output()) '-->'`
