...
(ref:KStonesRowPercentSmall) *Row percentages* for *small* kidney stones (from Table\ \@ref(tab:KS-Small)). Row *proportions* could also be used.
...
(ref:KStonesColPercentSmall) **Column percentages** for **small** kidney stones (from Table \@ref(tab:KS-Small)). Column *proportions* could also be used.
...
(ref:KStonesRowColPercentSmall) The **small** kidney stones data: left: *row percentages*. Right, *column percentages* (from Table\ \@ref(tab:KS-Small)). Proportions could be used rather than percentages.
...
-   use percentages or counts on one of the axis. 
    (The percentages can be percentages of the total, or within the total for each level of the variable, as in the centre plots in Fig.\ \@ref(fig:QualGraphsStones).)
...
## Summarising the comparison: difference between proportions {#DiffProportions}
\index{Difference in proportions}
...
The difference between these proportions (or percentages) is $0.064$ (or $6.4$ percentage points).
The difference between the proportions is a *statistic*, and the (unknown) difference between the population proportiobs is a *parameter*.


## Summarising the comparison: odds ratios {#OddsRatios}
\index{Odds ratio}
...
The *odds ratio* (often written OR) is the ratio of the odds of an result of interest in one group, compared to the odds of the *same* result in a *different* group:
$$
   \text{Odds ratio} = 
...
{    
  '(Table\\ \\@ref(tab:KSAll)),' }` 
without separating by size?

(ref:KStonesNumbersAll) Numbers for *all* kidney stones combined, without separating by the size of the kidney stone
...
if( knitr::is_latex_output() ) {
  kable(pad(WAKidsTab,
        ...
        col.names = c("in children", 
                      "in children"),
        caption = "Cross-tabulation of having livestock in the household, and children under $5$ years of age having diarrhoea in the household in the last two week") %>%
    row_spec(0, bold = TRUE) %>%
    kable_styling(font_size = 8) %>%
...
The odds ratio (OR) and the difference between the proportions can be used to compare the two different groups.

## Quick revision questions {#CompareQual-QuickReview}

::: {.webex-check .webex-box}
A study [@data:Alley2017:SocialMedia] examined social media use (Table\ \@ref(tab:SocialMedia)), using a representative sample of Queenslanders at least $18$ years of age (from the $2013$ Queensland Social Survey).
...
6.  Compute the *sample difference between the proportions* using social media, comparing *urban* to *rural* residents. `r if( knitr::is_html_output() ) {mcq( c( "1.365 - 1.141 = 0.224",   answer = "0.577 -  0.533 = 0.044",     "0.533 - 0.577 = -0.044"))}`
:::
...
1. Of the *male* patients, what *percentage* had their stroke symptoms missed by the dispatcher?
