...
Assume the study is externally valid: the sample is representative of all Californian home owners.
...
That is, each sample has different individuals, produces different data, and may even suggest different answers to the
...
`r if (knitr::is_latex_output()) {
   'Consider a fair pack of cards (a *population*), where $50$% of cards are red. The percentage of red cards is not the same in every hand (the *sample*) of ten cards.'
} else {
   'The animation below shows how samples vary. We know that $50$% of cards in a fair, shuffled pack (a *population*) are red, but each hand (the *sample*) of ten cards can produce a different percentage of red cards (and not always $50$%).'
}`
...
```{r PrecisionAccuracy, fig.align="center", fig.cap="Precision and accuracy: each dot indicates where a shot lands, and is like a sample estimate of the population value (shown by the central X).", fig.width=5.0, out.width='55%'}
par( mfrow = c(2, 2),
     mar = c(1, 1, 1, 1)/4,
...
It does *not* mean 'haphazard' or 'picking individuals as I see fit'.
...
The students *in* the sample are probably somewhat similar to those *not* in the sample in terms of their typing speeds (there is no obvious reason why they would not be), but we cannot be sure.
:::
...
Sampling students at your university, because it is easier than finding a random sample of all people in your country, will only generalise to students at that university and not to students at *all* universities.
:::
