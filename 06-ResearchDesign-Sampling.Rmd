...
The sample comprises the Californian university students studied by the researchers.
...
Only studying one of countless possible samples raises questions:
...
In a 'population' as small as $100$, the number of possible samples of size $25$ is more than twice the number of people currently living on earth.
:::
...
Some examples of random sampling methods appear in Sects.\ \@ref(SRS) to\ \@ref(Multi-stageSampling), and summarised in Table\ \@ref(tab:TypesOfRandomSampling).
...
* *Voluntary response (self-selecting) samples*:\index{Sampling!non-random!voluntary}\index{Sampling!non-random!self-selecting}
...
### Multi-stage sampling {#Multi-stageSampling}
...
if (knitr::is_html_output()){
  ...
  source("R/showSampleMulti-stage.R")
  showSampleMulti-stage(static = FALSE)  
}  
```
...
Sometimes, ways exist to obtain a non-random sample that is *more likely* to be representative.
...
For the typing study in Example\ \@ref(exm:Typing), only selecting students attending the gym, or only students who at a certain Cafe, is unlikely to be somewhat representative of the student population.
...
*This is not a random sample*, but this sample is likely to comprises a variety of student types.
...
Even a *random* sample of Alaskans produces a biased sample: the sampling frame (Alaskans) does not represent the target population ('Americans').
...
This may be intentional (for example, respondents lie) or non-intentional (for example, the question is poorly written, is personal, or is misunderstood).
...
<!-- For a [*multi-stage sample*](#Multi-stageSampling), we could select some columns at random, then select some pixels in those columns at random. -->
...
::: {.exercise #NotMulti-stageSampling}
