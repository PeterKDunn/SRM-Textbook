...
These can generally be included in the data worksheet (though there may be limit to the length of such data), but cannot be analysed using quantitative methods (as described in this book).

Closed questions are easily included in a data worksheet.
In closed questions where respondents can select one option only, one column is needed for the question, reocrding which option is selected.
...
The first question requires open-ended, text-based answers.
For the second (closed) question, students could select multiple option; the third (closed) question required students to select *one* option from a given list, so each ption needs one column in the data worksheet.
