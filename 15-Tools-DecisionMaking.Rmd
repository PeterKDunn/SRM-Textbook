...
```{r echo = FALSE, animation.hook = "gifski", cache = TRUE, fig.width = 4, interval = 0.25, fig.align = "center", dev=if (is_latex_output()){"png"}else{"pdf"}}
...
```{r DecisionFlow, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=1.5, progress=TRUE, fig.cap="A way to make decisions", fig.align="center", fig.width=6, fig.height=3, dev=if (is_latex_output()){"png"}else{"pdf"}}
if( knitr::is_html_output() ) {
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.25, dev=if (is_latex_output()){"png"}else{"pdf"}}
if (knitr::is_html_output()){
  set.seed(99999)
    num.in.hand <- 15
    num.sims <- 10
...
```{r DiceHist, echo=FALSE, animation.hook = "gifski", cache = TRUE, interval = 0.025, dev=if (is_latex_output()){"png"}else{"pdf"}}
if (knitr::is_html_output()){
  set.seed(9900991)
    num.in.hand <- 25
    num.sims <- 1000
