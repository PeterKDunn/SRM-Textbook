...
1. The *population* means are the *same*.
   The *sample* means are different only because we have data from one of many possible samples, and every sample is likely to be different, so the sample means are different by chance.
...
1. The *population* odds are the same.
   The *sample* odds are different only because we have data from one of many possible samples, and evry sample is likely to be different, so sometimes, the sample odds are different by chance.
...
1. There is *no difference* between the population parameters: the difference is due to *sampling variation*. 
...
The decision-making process begins by assuming the null hypothesis is true, and then the data is examined to see if it contains evidence to support the alternative hypothesis.
However, conclusions drawn about the *population* from the *sample* are never certain, since the sample studied is just one of many possible samples.
...
The difficulty is that the decision is made using one of the many possible sample, and every sample is likely to be different (comprising different individuals from the population), and each sample will produce different summary *statistics*.
This is called *sampling variation*.
...
Of course, getting 15 red cards in a row is *possible*... but very *unlikely*^[In fact, the probability of getting 15 cards *of the same colour* (either red *or* black) is about 0.00034%.].
For this reason, you would probably conclude, based on the evidence, that the pack is not a fair pack.
...
Based on the ideas in the last section, a formal process of decision making in research can be described as in Table \@ref(tab:DMPtable).
To expand:

1. **Assumption**:
   The initial assumption is that the *sampling variation* explains differences in the sample.
   For example: 
   
   - the [*population* parameters](#StatisticsAndParameters) are the same in various groups, but sampling variation explains the difference between the [*sample* statistics](#StatisticsAndParameters);
   - the  [*population* parameter](#StatisticsAndParameters) is some given value, but sampling variation explains why the [*sample* statistics](#StatisticsAndParameters) is not equal to this parameter value.

2. **Expectation**:
   Based on this assumption about the parameter, describe what values of the *sample [statistic](#StatisticsAndParameters)* might reasonably be observed from all possible samples through sampling variation.

3. **Observation**:
   Then, if the observed *sample statistic* is:
...
if( knitr::is_latex_output() ) {
  ...
  DMP[4, 2] <- "\\begin{minipage}[t]{51mm}...\\textit{unlikely} to happen by chance, it \\textbf{contradicts} the assumption.\\end{minipage} \\hfill \\hfill \\begin{minipage}[t]{51mm}...\\textit{likely} to happen by chance, it is \\textbf{consistent with} the assumption.\\end{minipage}"
    kable(DMP, 
          format = "latex",
