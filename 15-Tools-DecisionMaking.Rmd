...
1. The *population* means are the *same*.
   The *sample* means are only different because every sample is likely to be different (each possible sample includes different people), so the sample means are different by chance.
...
However, conclusions drawn about the *population* from the *sample* are never certain.
...
Getting 15 reds cards out of 15 seems very unlikely, so you probably conclude that the pack is unfair.
But importantly, *how* did you reach that decision?
Your unconscious decision-making process may have worked like this:

1. You **assumed**, quite reasonably, that this is a standard, well-shuffled pack of cards, with half the cards red and half the cards black. 
...
DMP[3, ] <- c("Make a reasonable assumption about the value of a *population [parameter](#StatisticsAndParameters)*",
              "Based on this assumption, describe what values of the *sample [statistic](#StatisticsAndParameters)* might reasonably be observed",
...
1. **Assumption**:
   Make a reasonable assumption about the population, such as assuming a value for the *population [parameter](#StatisticsAndParameters)*, or state a value for the *population [parameter](#StatisticsAndParameters)* to be confirmed.
...
```{r DecisionFlow2, echo = FALSE, fig.cap = "A way to make decisions", fig.align = "center", fig.width = 6, fig.height = 3}
if( knitr::is_latex_output() ) {
...
This approach is similar to what we unconsciously use every day.
...
3. **Observation**: When I check later, I observe a *dry* toothbrush.
   The evidence contradicts what I expected to find based on my assumption, so my assumption is probably *false*.
...
1. **Assumption**: I *assume* my son brushed his teeth (because I told him to).
2. **Expectation**: Based on that assumption, I *expect* to find a damp toothbrush when I check later.
3. **Observation**: When I check later, I observe a *damp* toothbrush.
   The evidence seems consistent with what I expected to find based on my assumption, so my assumption is probably *true*.
...
1. **Assumption**: *Assume* the pack is fair and well-shuffled pack of cards: the population proportion of red cards is 0.5 (the value of the *parameter*). 
2. **Expectation**: Based on this assumption, roughly equal (but not necessarily *exactly*) equal numbers of red and black cards would be expected in a sample of 15 cards.  
   The sample proportion of red cards is expected to be close to, but maybe not exactly, 0.5 (the value of the *statistic*). 
...
* We could repeatedly shuffle a pack of cards and deal 15 cards many hundreds or thousands of times, then compute how often we get 15 red cards of out 15 cards.
...
```{r HandRedHist1000, echo=FALSE, fig.align="center", fig.height=4,fig.width=5.5, fig.cap="A histogram of the sample proportion of red cards, in hands of 15 cards, for 1000 repetitions", cache=FALSE }
if (knitr::is_latex_output()){
    set.seed(9900991)
    num.in.hand <- 25
    num.sims <- 1000
    ...
    for (i in 1:num.sims){
      ...
      out <- hist( prop.red,
                   ...
                   main = paste("Histogram of sample proportions\nHand number:", i),
                   xlab = "Proportion of the 15 cards that are red",
                   sub = paste("(For this hand: proportion of cards red: ", 
		               format(round(prop.red[i], 3), nsmall = 3), 
			       ")", 
			       sep = "" ),
                   ylab = "Frequency",
                   right = FALSE,
...
From this histogram, based on simulationing one thousand hands, we could conclude that we would *almost never* find 15 red cards in 15 cards... *if the assumption of a fair pack was true*.
...
Making decisions about population [parameters](StatisticsAndParameters) based on a sample statistic can be difficult:
...
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = TRUE )}`
1. True or false: *Sampling variation* refers to how the value of a *statistic* varies from sample to sample.  
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = TRUE )}`
