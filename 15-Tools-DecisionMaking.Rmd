...
```{r animation.hook = "gifski", cache = FALSE, fig.width = 4, interval = 0.25, fig.align = "center", dev=if (is_latex_output()){"pdf"}else{"png"}}
...
```{r RollDice10, fig.align = "center", fig.width = 6.5, out.width='75%', fig.cap = "Ten hands of 15 cards: The sample proportion that is red varies from hand to hand (as shown on the right-hand side)" , cache = FALSE}
if (knitr::is_latex_output()){
  set.seed(99999)
  num.in.hand <- 15
  num.sims <- 10
...
```{r HandRedHist1000, fig.align="center", fig.height=4,fig.width=5.25, fig.cap="A histogram of the sample proportion of red cards, in hands of 15 cards, for 1000 repetitions", cache=FALSE }
if (knitr::is_latex_output()){
    set.seed(99030991)
    num.in.hand <- 25
    num.sims <- 1000
