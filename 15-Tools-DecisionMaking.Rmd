...
with four *suits*: spaces and clubs (which are both black), 
and heart and diamonds (which are both red).
...
```{r echo = FALSE, animation.hook = "gifski", cache = TRUE, fig.width = 4, interval = 0.25, fig.align = "center"}
...
<!-- ```{r Draw15CardsMovie, echo=FALSE, fig.align="center", fig.cap="Does it seems like this is a fair pack of cards?"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/Draw15CardsMovie.html", frameborder=0, height=470, width=450)  -->
<!-- ``` -->
...
<!-- ```{r Draw15Cards, echo=FALSE, fig.align="center", fig.width=5, fig.cap="How likely is it that you would get 15 red cards in a row from a fair pack?" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/Draw15Cards16") -->
<!-- } -->
<!-- ``` -->
...
```{r DecisionFlow, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=1.5, progress=TRUE, fig.cap="A way to make decisions", fig.align="center", fig.width=6, fig.height=3}
if( knitr::is_html_output() ) {
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.25}
if (knitr::is_html_output()){
  set.seed(99999)
    num.in.hand <- 15
    num.sims <- 10
    ...
    for (i in 1:num.sims){
      ...
      plot( c(1, (num.in.hand+2)), c(1, num.sims),
            ...
            axes=FALSE)
      hand <- sample( c("B", "R"), num.in.hand, replace=TRUE)
      all.hands[i, ] <- hand
      ...
      # Add p-hat heading
      mtext(expression(hat( italic(p) ) ), side=3, line=0, at=num.in.hand+2 )
      ...
      #Add dividing line
      abline(v=num.in.hand+1, col="grey")
      
      # Divide each roll set
      abline(h = (1:i)-0.5, col="grey")
    }
  }
```
...
```{r echo=FALSE, animation.hook = "gifski", cache = TRUE, interval = 0.025}
if (knitr::is_html_output()){
  set.seed(9900991)
    num.in.hand <- 25
    num.sims <- 1000
