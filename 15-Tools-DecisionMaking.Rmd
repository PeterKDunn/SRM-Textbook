...
1. The *population* means are the *same*; the difference is due to sampling variation.
   The *sample* means are different only because we have data from one of many possible samples, and every sample is likely to be different.
...
1. The *population* odds are the same.
   The *sample* odds are different only because we have data from one of many possible samples, and every sample is likely to be different, so sometimes, the sample odds are different by chance.
...
Then the data are examined to see if it presents evidence to support the alternative hypothesis.
However, conclusions drawn about the *population* from the *sample* can never be certain, since the sample studied is just one of many possible samples.
...
```{r Draw15Cards, echo=FALSE, cache=FALSE, fig.align="center", fig.width=7, fig.height=4.5, out.width = "40%",fig.cap="How likely is it that you would get 15 red cards in a row from a fair pack?" }
...
4. Since what you observed ('all red cards') was not like what you were expecting ('about half red cards'), the 15 cards in my hand *contradict* what you were expecting, based on your assumption of a fair pack...  so your assumption of a fair pack is probably wrong.
...
1. **Assumption**:
   Initial assume that the *sampling variation* explains the discrepancy between the observed sample and population.
   For example: 
...
2. **Expectation**:
   Based on this assumption about the parameter, describe what values of the *sample [statistic](#StatisticsAndParameters)* might reasonably be observed from all possible samples that could be observed (due to sampling variation).

3. **Observation**:
   After observing the data from one of the many possible samples, if the observed *sample statistic* is:
...
Since many samples are possible, and every sample is likely to be different (['sampling variation'](#def:SamplingVariation), the value of the sample statistic depends on which one of the possible samples we end up with: the sample statistic is likely to be different for every sample.
...
```{r RollDice10, echo = FALSE, fig.align = "center", fig.width = 5.5, out.width='70%', fig.cap = "Ten hands of 15 cards: The sample proportion that is red varies from hand to hand (as shown on the right-hand side)" , cache = FALSE}
if (knitr::is_latex_output()){
    set.seed(99999)
    num.in.hand <- 15
    num.sims <- 10
    ...
    for (i in 1:num.sims){
      par ( mar = c(0.1, 4.1, 4.1, 0.1)) # DEFAULT is 5.1, 4.1, 4.1, 2.1
