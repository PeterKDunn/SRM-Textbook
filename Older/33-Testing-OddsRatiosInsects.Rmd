...
## Introduction: eating insects {#EatingInsects}

A study examined Australians acceptance to eating insects [@hopkins2023food].
The researchers asked $601$ participants about their levels of education and whether they had previously eaten insects.
The data can be displayed in a two-way table of counts (Table\ \@ref(tab:EatInsectsDataTable)), called a *contingency table*.
...
Every cell in the $2\times 2$ table contain different participants, so the comparison is *between* individuals.
:::


```{r EatInsectsDataTable}
Education <- c("University education", 
               "University education",
               "No university education",
               "No university education")
Education <- ordered(Education,
                     levels = c("University education",
                                "No university education"))
Insects <- c("Yes",
             "No",
             "Yes",
             "No")
Insects <- ordered(Insects,
                   levels = c("Yes",
                              "No"))
Counts <- c(132,
            223,
...
EatInsects.tab <- EatInsects.tab.Counts <- xtabs( Counts ~ Education + Insects,
                                                  data = EatInsects)
...
EatInsects.tab <- cbind( EatInsects.tab, 
                         "Total" = rowSums(EatInsects.tab))
EatInsects.tab <- rbind( EatInsects.tab, 
                         "Total" = colSums(EatInsects.tab))
row.names(EatInsects.tab) <- c("University education",
                               "No university education",
                               "Total")


if( knitr::is_latex_output() ) {
  kable(surroundMaths(EatInsects.tab),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
        escape = FALSE,
        align = "r",
        col.names = c("Previously eaten insects",
                      "Never previously eaten insects",
                      "Total"),
        caption = "Eating insects and level of education"
  ) %>%
...
if( knitr::is_html_output() ) {
  kable(EatInsects.tab,
        format = "html",
        booktabs = TRUE,
        longtable = FALSE,
        align ="r",
        row.names = c("University education",
                      "No university education"),
        col.names = c("Previously eaten insects",
                      "Never previously eaten insects"),
        caption = "Eating insects and level of education"
...
A *graphical summary* is shown in Fig.\ \@ref(fig:EatInsectsBarplot) (left panel).
Odds and percentages are needed to prepare a numerical summary.
The odds of having eaten insects before are:

* $132\div 223 = 0.592$ for those *with* a university education; and
* $81\div 165 = 0.491$ for those *without* a university education.

The odds are computed this way since having eaten insects is Column\ 1 in Table\ \@ref(tab:EatInsectsDataTable).
The corresponding percentages are $132\div 355 \times 100 = 37.2$% and $81/246\times 100 = 32.9$%.  

The odds ratio is $0.592/0.491 = 1.206$, comparing  those with a university education to those without.
The odds ratio is computed this way since having a university education is Row\ 1 in Table\ \@ref(tab:EatInsectsDataTable).
The resulting *numerical summary* appears in Table\ \@ref(tab:EatInsectsNumericalSummaryHT).


::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
Understanding how software computes the odds ratio is important for understanding output.
In a $2\times 2$ table, jamovi output can be interpreted in *either* of these ways (i.e., both are correct):
...
```{r EatInsectsNumericalSummaryHT}
EatInsectsNumericalSummary <- array( dim = c(3, 3))

EatInsectsNumericalSummary[1, ] <- c(format( round(EatInsects.tab.Counts[1,1]/EatInsects.tab.Counts[1,2], 4), nsmall = 3),
                                 round(EatInsects.tab.Counts[1,1]/sum(EatInsects.tab.Counts[1,]) * 100, 1),
                                 sum(EatInsects.tab.Counts[1,]) )
EatInsectsNumericalSummary[2, ] <- c(round(EatInsects.tab.Counts[2,1]/EatInsects.tab.Counts[2,2], 4),
                                 round(EatInsects.tab.Counts[2,1]/sum(EatInsects.tab.Counts[2,]) * 100, 1),
                                 sum(EatInsects.tab.Counts[2,]) )
EatInsectsNumericalSummary[3, ] <- c(round( (EatInsects.tab.Counts[1, 1] / EatInsects.tab.Counts[1, 2] ) / (EatInsects.tab.Counts[2, 1] / EatInsects.tab.Counts[2, 2]), 3), 
                                 NA,
                                 NA)
rownames(EatInsectsNumericalSummary) <- c("University education",
                                          "No university eduication",
                                          "Odds ratio")
    
if( knitr::is_latex_output() ) {
  kable(surroundMaths(EatInsectsNumericalSummary,
                      decDigits = c(3, 1, 0)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
        escape = FALSE,
        align = "r",
        col.names = c("never eaten insects",
                      "never eaten insects", 
                      "size"),
       caption = "The odds and percentage of having previously eaten insects"
  ) %>%
...
if( knitr::is_html_output() ) {
  kable(EatInsectsNumericalSummary,
               format = "html",
               longtable = FALSE,
               booktabs = TRUE,
               align = c("r", "r", "r"),
...
The parameter is the population OR of having eaten insects, comparing those with a university education to those without.
(The parameter could also be the population OR of having a university education, comparing those who have eaten insects to those who have not, but the former is easier understand.)


```{r EatInsectsBarplot, out.width='50%', fig.cap="Bar chart showing the number having previously eaten insects by education level", fig.align="center" }
barplot( t(EatInsects.tab.Counts),
        las = 1,
        legend.text = c("Never eaten insects",
                        "Have eaten insects"),
        ylab = "No. of people",
        xlab = "Education",
        main = "Bar chart of education, and\nhaving previously eaten insects")
```


The RQ can be written in terms of comparing proportions, odds, or odds ratios (but means are not appropriate!); these are all equivalent:

* Is the *population odds ratio* of having eaten insects before, comparing those with a university education to those without,  equal to one?
* Is the *population odds* of having eaten insects before the same for those with a university education to those without?
* Is the *population proportion (or percentage) of having eaten insects before the same for those with a university education to those without?

However, for consistency with the output, writing the RQ in terms of odds or odds ratios is recommended.
Another option for writing the RQ, which sounds less direct but is useful for two-way tables larger than $2\times 2$ (see Sect.\ \@ref(ORTestDumping)), is worded in terms of *relationships* or *associations* between the variables:

* Is there a relationship (in the population) between having eaten insects before, and having a university education?

All of these are equivalent, but the second (comparing the odds in the two groups) is probably the easiest to write that remains consistent with the output.


## Statistical hypotheses and notation

For two-way tables of counts, the *parameter* is the population odds ratio.
As usual, the null hypothesis is that the difference between the sample and population OR is due to sampling variation (the 'no difference, no change, no relationship' position). 
In this context, the hypotheses are (when written in terms of odds):

* $H_0$: The *population* odds of having eaten insects before are *the same* for those with and without a university education.
* $H_1$: The *population* odds of having eaten insects before are *not the same* for those with and without a university education.
...
* $H_0$: In the *population*, there *is no association* between having eaten insects before, and having a university education.
* $H_1$: In the *population*, there *is an association* between having eaten insects before, and having a university education.
...
Assuming that the odds of having eaten insects is the same for both groups (that is, the population OR is one), how would the sample OR be *expected* to vary from sample to sample just because of *sampling variation*?

If the null hypothesis is true, the odds (and the percentage) of having eaten insects before is the same in both groups.
That is, the percentage having eaten insects is the same for those with and without a university education.

Let's consider the implication.
From Table\ \@ref(tab:EatInsectsDataTable), $213$ participants out of $601$ had previously eaten insects; that is,  
\[
   \frac{213}{601} \times 100 = 35.44\%
\]
had previously eaten insects.

If the odds (or percentage) who had previously eaten insects is the same for those with and without a university education, then we'd *expect* $35.44$% of participants in *both* groups to have eaten insects before.
That is, we would *expect*:

* $35.44$% of the $355$ *with* a university education (i.e., $125.812$) to have eaten insects before; and
* $35.44$% of the $246$ *without* a university education (i.e., $87.182$) to have eaten insects before.

Those are the *expected* numbers if the odds (and percentage) was exactly the same in each group, if the null hypothesis (the assumption) was true.
Continuing for all the combinations produces Table\ \@ref(tab:EatInsectsDataTableExpected).



```{r EatInsectsDataTableExpected}
EatInsects.ExpCounts <- chisq.test(EatInsects.tab.Counts)$expected
EatInsects.ExpCounts <- cbind( EatInsects.ExpCounts, 
                               "Total" = round( rowSums(EatInsects.ExpCounts)) )
EatInsects.ExpCounts <- rbind( EatInsects.ExpCounts, 
                               "Total" = round( colSums(EatInsects.ExpCounts)) )
...
if( knitr::is_latex_output() ) {
  ...
  ignore <- array( FALSE, dim = dim(EatInsects.ExpCounts))
  ignore[ 3, 1 ] <- ignore[ 3, 2 ] <- TRUE # This keeps the Total row with no decimals
  tab1 <- surroundMaths(EatInsects.ExpCounts,
                        decDigits = c(3, 3, 0), 
                        ignore = ignore)
  tab1[3, 1] <- paste0("$", as.numeric(tab1[3, 1]), "$")
  tab1[3, 2] <- paste0("$", as.numeric(tab1[3, 2]), "$")
  tab1[2, 1] <- paste0("\\phantom{0}", tab1[2, 1])
    ...
    kable( tab1,
        ...
        digits = 3,
        align = c("l", "l", "c"),
        caption = "Eating insects and university education: Expected counts"
  ) %>%
     column_spec(4, bold = TRUE)  %>%
     row_spec(3, bold = TRUE) %>%
     row_spec(0, bold = TRUE) %>%
...
if( knitr::is_html_output() ) {
  out <- kable(EatInsects.ExpCounts,
               format = "html",
               booktabs = TRUE,
               longtable = FALSE,
               digits = 3,
...
Consider the expected counts in Table\ \@ref(tab:EatInsectsDataTable).
Confirm that the *odds* of having eaten insects before is the same for those with and without a university education.\label{thinkBox:ExpectedOddsSame}

`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
* With university education:  $125.815/229.185 = 0.549$; 
* Without university education: $87.185/158.815 = 0.549$.
...
How do those *expected values* (Table\ \@ref(tab:EatInsectsDataTableExpected)) compare to what was *observed* (Table\ \@ref(tab:EatInsectsDataTable))?

* $125.815$ of $355$ with a university education are *expected* to have previously eaten insects; yet we observed $132$.
* $87.185$ of the $129$ without a university education are *expected* to have previously eaten insects; yet we observed $81$.
...
The [decision-making process](#DecisionMaking) compares what is *expected* if the null hypothesis about the parameter is true (Table\ \@ref(tab:EatInsectsDataTableExpected)) to what is **observed** in the sample (Table\ \@ref(tab:EatInsectsDataTable)).
...
The test-statistic here is a 'chi-squared' statistic, written $\chi^2$.
...
From the software (Fig.\ \@ref(fig:EatInsectTestOutputjamovi)), $\chi^2 = 1.15$.
...
So here, the equivalent $z$-score is about $\sqrt{1.15} = 1.07$, which is very small for a $z$-score: a large $P$-value is expected.

More generally, for two-way tables of any size, $\sqrt{\chi^2 \div \text{df}}$ is like a $z$-score, where df is the 'degrees of freedom' (related to the size of the table^[For those interested: the degrees of freedom in a two-way table is the number of rows of data less one, times the number of columns of data less one. For a $2\times 2$ table, $\text{df} = (2 - 1) \times (2 - 1) = 1$.]), as in the software output.
This allows a $P$-value to be estimated using the 68--95--99.7 rule from the value of the $\chi^2$ statistic.


```{r EatInsectTestOutputjamovi, fig.show="hold", fig.cap="The jamovi output for computing a CI (left) and conducting a test (right) for an OR", fig.align="center", out.width=c("50%", "45%"), fig.show='hold'}
knitr::include_graphics( "jamovi/EatInsects/EatInsects-CI.png")
knitr::include_graphics( "jamovi/EatInsects/EatInsects-Test.png")
```
...
The differences between the observed sample statistic (the sample OR) and the hypothesised population parameter (the population OR of one) is summarised by $\chi^2 = 1.1505$, approximately equivalent to $z = 1.07$.
Using the [68--95--99.7 rule](#def:EmpiricalRule), a large $P$-value is expected.
The two-tailed $P$-value reported by jamovi (Fig.\ \@ref(fig:EatInsectTestOutputjamovi), under the `p` column) is indeed large: $0.283$ to three decimal places.
...
As usual, a large small $P$-value ($0.283$ to three decimals) means [no evidence](#tab:PvaluesInterpretation) exists to supporting $H_1$: no evidence suggests a difference in the *population* odds for those with and without a university education.
We write:

> The *sample* provides no evidence ($\chi^2 = 1.15$; two-tailed $P = 0.283$) that the odds in the *population* of having previously eaten insects is different for those with  (odds: $0.592$) and without (odds: $0.491$) a university education (OR: $1.206$; $95$% CI from $0.856$ to $1.70$).

As seen in Sect.\ \@ref(WordingConclusion), the conclusion includes three components:
(a) The *answer to the RQ*; (b) the *evidence* used to reach that conclusion  ('$\chi^2 = 1.15$; two-tailed $P = 0.283$'); and (c) some *sample summary statistics*  (including the $95$% CI for the odds ratio).

The conclusion also makes clear what the odds and the odds ratio *mean*.
The odds are describing as the 'odds... of having previously eaten insects', and the OR as then comparing these odds between those with and without a university education'.
...
The statistical validity condition refers to the *expected* (not *observed*) counts.
In jamovi, the *expected* counts can be requested to see if this condition is satisfied (Fig.\ \@ref(fig:EatInsectsExpectedjamovi)).


```{r EatInsectsExpectedjamovi, fig.cap="The expected values, as computed in jamovi", fig.align="center", out.width="60%"}
knitr::include_graphics("jamovi/EatInsects/EatInsects-Expected.png")
```

For the insect-eating data, the smallest *expected* count is $87.1847$, which is greater than five, so the CI and the test are statistical validity condition.
...
## Example: pet birds REMOVE {#ExampleHTPetBirds}
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The RQ could also be written in terms of comparing the *percentage* in each groups; in terms of the *odds ratio* being one; or in terms of a *relationship* between the two variables
:::
...
In practice, software computes these expected counts automatically.
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(B12summary,
                      decDigits = c(4, 1, 0)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
...
colnames(DumpTab) <- c("Acceptable", 
                       ...
                       "Not acceptable")
rownames(DumpTab) <- c("Reusable", 
                       "Non-reusable")

if( knitr::is_latex_output() ) {
  kable(surroundMaths(DumpTab),
        format = "latex",
        align = "r",
        longtable = FALSE,
        escape = FALSE,
...
cap.tab <- "Odds and percentage that the opinion listed is held for reusable rubbish; the odds ratios are computed relative to the `Not acceptable' opinion"

if( knitr::is_latex_output() ) {
  kable(surroundMaths(DumpSummary,
                      decDigits = c(3, 3, 1, 0)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
...
Here are the details of the calculations (notice that Column\ 3 is on the bottom of the fraction):
...
**Reusable** goods is called the *Reference level*.
...
Hypotheses are best worded in terms of *relationships* or *associations* between the two variables:

* $H_0$: *No association exists* between the rubbish type and the opinion of kerbside dumping.
* $H_1$: *An association exists* between the rubbish type and the opinion of kerbside dumping.
...
Effectively, the $chi^2$ statistic becomes the parameter: when no relationship exists in the population, $chi^2 = 0$; hence $H_0:$ $\chi^2 = 0$.
...
This suggests very strong evidence in the sample that opinions are not the same for reusable and non-reusable rubbish.
...
All *expected* values exceed $5$ (Fig.\ \@ref(fig:KerbsideChisqjamovi); Fig.\ \@ref(fig:KerbsideChisqSPSS)), even though one *observed* count is less than five.
...
```{r}
knitr::kable( surroundMaths(EV10),
         format = "latex",
         align = "r",
         booktabs = TRUE,
         longtable = FALSE,
...
```
\caption{Software output for the EV study}\label{fig:EVjamoviHT}
\end{minipage}
\end{figure}
...
\fontsize{10}{12}\selectfont
```{r}
knitr::kable( surroundMaths(FliesData),
         format = "latex",
         align = "r",
         booktabs = TRUE,
         longtable = FALSE,
...
```{r SunglassesSPSSOutput, fig.show="hold", fig.cap="SPSS output for the hats data", fig.align="center", out.width="80%"}
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(PhoneTable),
        format = "latex",
        align = "r",
        escape = FALSE,
        longtable = FALSE,
...
- Sect. \ref{thinkBox:ExpectedOddsSame}: With university education: $125.815/229.185 = 0.549$; Without university education: $87.185/158.815 = 0.549$.
