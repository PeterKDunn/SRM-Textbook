...
EatingNumericalSummary[2, ] <- c(round(UniS[1, 2]/UniS[2, 2], 4),
                                 round(UniS[1, 2]/sum(UniS[3, 2]) * 100, 1),
                                 UniS[3,2] )
...
```{r EatingGraphsCI, fig.cap="The uni-student eating data. Left: a side-by-side bar chart. Right: the jamovi output for computing a CI", fig.align="center", fig.height=3.75, fig.width=4, out.width=c("45%", "53%"), fig.show='hold'}
par( xpd = TRUE,
...
Using jamovi\index{Software output!Comparing two odds} to find the OR (Fig.\ \@ref(fig:EatingGraphsCI), right panel), the sample OR is $5.94$, and the (exact) $95$%\ CI is from $1.35$ to $26.1$.
...
Recall the jamovi output can be interpreted in either of these ways:\index{Software output!Odds ratios}
...
For the uni-students eating data, jamovi can be used to compute the *expected* counts (Fig.\ \@ref(fig:UniMealsTestOutputExpectedOnlyjamovi))\index{Software output!Expected counts}
