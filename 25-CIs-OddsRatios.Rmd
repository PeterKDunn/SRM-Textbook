...
```{r, child = if (knitr::is_html_output()) {'introductions/25-CIs-OddsRatios-HTML.Rmd'} else {'introductions/25-CIs-OddsRatios-LaTeX.Rmd'}}
```
...
Another sensible parameter would be the difference between the *proportions* (or *percentages*) in each group, but the odds ratio usually is used as the parameter (for reason beyond the scope of this book, though one reason is that software commonly produces output related to the sample OR).
...
> Among university students, is the odds of students eating most meals off-campus the same for students who *do* and *do not* live with their parents?
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
What graphs would be best for displaying these data?\label{thinkBox:WhatGraph}
...
## Summaries: Comparing odds {#CIOddsRatiosSummaries}

With two qualitative variables, an appropriate numerical summary includes the odds and percentages for the outcome (from each comparison group) and the sample sizes.
...
* The *odds* are the odds of eating most meals *off-campus* (Row 1 of Table \@ref(tab:MealsDataTable)) compared to *on-campus* (Row 2): $52/2 =  26$ (for those living with parents) and $105/24 = 4.375$ (for those not living with parents).
  ...
  Then, the *odds ratio* compares these odds for students living with their parents (Column 1 of Table \@ref(tab:MealsDataTable)) to those *not* living with their parents (Column 2): the OR is $26/4.375 = 5.943$, as in the output (Fig.&nbsp;\@ref(fig:UniMealsTestOutputjamovi)).
...
\smallskip
  ...
  Then, the *odds ratio* compares these odds for students eating most meals off-campus (Row 1 of Table \@ref(tab:MealsDataTable)) to the odds of students eating most meals on-campus (Row 2): the OR is $0.49524/0.083333 = 5.943$, as in the output (Fig.&nbsp;\@ref(fig:UniMealsTestOutputjamovi)).
  
That is, the odds and odds ratios are relative to the *second row* or *second column*.
:::
...
if( knitr::is_latex_output() ) {
  kable(EatingNumericalSummary,
    ...
    kable_styling(font_size = 10) %>%
    row_spec(3, bold = TRUE) %>%
    #row_spec(0, bold = TRUE) %>% Because the headings span two rows, the bolding is wonky
    column_spec(1, bold = TRUE) %>%
    column_spec(1, width = "35mm") %>%
    column_spec(2, width = "33mm") %>%
    column_spec(3, width = "38mm") %>%
    column_spec(4, width = "15mm")
  # Because the headings span two rows, the bolding is wonky:
  # row_spec(0, bold = TRUE)
}
if( knitr::is_html_output() ) {
...
```{r EatingGraphs, echo=FALSE, fig.cap="A plot of the uni-student eating data: A side-by-side bar chart", fig.align="center", fig.height=4, fig.width=7.7, out.width="80%"}
par( xpd = TRUE,
...
For this reason, we will use software output for finding the CI for the odds ratio, and not discuss the sampling distribution directly (see Table \@ref(tab:NotationOddsRatioCI)).
...
```{r NotationOddsRatioCI, echo=FALSE}
...
if( knitr::is_latex_output() ) {
  ...
  OddsRatioNotation[3, 1] <- paste(OddsRatioNotation[3, 1], 
                                 OddsRatioNotation[4, 1])
  OddsRatioNotation[3, 2] <- paste(OddsRatioNotation[3, 2], 
                                 OddsRatioNotation[4, 2])
  OddsRatioNotation[3, 3] <- paste(OddsRatioNotation[3, 3], 
                                 OddsRatioNotation[4, 3])
  OddsRatioNotation[3, 4] <- paste(OddsRatioNotation[3, 4], 
                                 OddsRatioNotation[4, 4])
  OddsRatioNotation[4, ] <- NA
...
Using either jamovi (Fig. \@ref(fig:UniMealsTestOutputjamovi), left panel) or SPSS output (right panel, labelled, obscurely, **Risk Estimate**), the sample OR is 5.94 (as computed), and the (exact) 95% CI is from 1.35 to 26.1.
(The SPSS output shows other information too, which is not relevant here.)


```{r UniMealsTestOutputjamovi, echo=FALSE, fig.show="hold", fig.cap="The jamovi output (left) and SPSS output (right) for computing a CI", fig.align="center", out.width="49%"}
...
Recall that the jamovi and SPSS output can be interpreted in one of these ways:

* Odds are Row 1 divided by Row 2.  
  Then the odds ratio is computed as Column 1 odds divided by Columns 2 odds (i.e., *comparing* Column 1 odds to Column 2 odds); or
...
However, the interest is in comparing the *odds* (or percentage) of crashes involving pedestrians in 2011 and 2015, in the *population*.
...
The *expected* counts are what we would expected to find if there was no relationship between the two variables in the two-way table.
...
In Sect.&nbsp;\@ref(ExpectedValues), we see how to find similar information from SPSS.
...
```{r ExpCrashDataTable, echo=FALSE}
...
Software computes the CI for the *population* odds ratio (Fig.&nbsp;\@ref(fig:PetBirdsCIjamovi)) based on the sample.
...
```{r BirdsGraphs, echo=FALSE, fig.cap="A plot of the pet-birds data", fig.align="center", fig.width=5.5, fig.height=3.5, out.width='55%'}
par(xpd = TRUE,
...
```{r PetBirdsCIjamovi, echo=FALSE, fig.show="hold", fig.cap="jamovi output (left) and SPSS output (right) for the pet-birds data", fig.align="center", out.width="49%"}
...
The CI will be statistically valid if all expected counts exceed 5.
From the jamovi output (Fig.&nbsp;\@ref(fig:PetBirdsCIjamoviExpected)), the expected-counts condition is met.


```{r PetBirdsCIjamoviExpected, echo=FALSE, fig.show="hold", fig.cap="The expected (and observed) counts as computed by jamovi for the pet-birds data", fig.align="center", out.width="80%"}
...
From the jamovi or SPSS output (Fig.&nbsp;\@ref(fig:B12SPSSOutputCIjamovi)), the OR (and 95% CI) is $3.15$ ($1.08$ to $9.24$).
...
```{r B12BarchartsCI, echo=FALSE, fig.cap="A side-by-side barchart comparing the number of women B12 deficient", fig.align="center", fig.width=4.5, fig.height=3, out.width="45%"}
### B12 example
...
```{r B12SPSSOutputCIjamovi, echo=FALSE, fig.show="hold", fig.cap="jamovi output (left) and SPSS output (right) for the B12 data", fig.align="center", out.width="55%"}
...
```{r B12jamoviExpectedCounts, echo=FALSE, fig.show="hold", fig.cap="The expected counts from jamovi for the B12 data", fig.align="center", out.width="70%"}
...
```{r EarInfPSSOutputCI, echo=FALSE, fig.show="hold", fig.cap="SPSS output for the ear-infection data", fig.align="center", out.width="60%"}
knitr::include_graphics( "SPSS/EarInf/EarInf-SPSS-OR.png" )
```
...
```{r TurbinesOutputCI, echo=FALSE, fig.show="hold", fig.cap="jamovi output for the turbine data", fig.align="center", out.width=c("49%", "44%")}
...
\fontsize{10}{12}\selectfont
```{r echo=FALSE}
knitr::kable( t(SOI.tab),
         format = "latex",
         booktabs = TRUE,
         longtable = FALSE,
         #row.names = c("No rain", "Rain"),
         align = c("P{19mm}", "P{14mm}"),
         table.env = "@empty") %>%
   row_spec(0, bold = TRUE)
   #kable_styling(font_size = 10) %>% # CANNOT USE THIS IS THE MINIPAGE
   #column_spec(1, width = "13mm") %>%
...
```{r SunglassesSPSSOutputCI, echo=FALSE, fig.show="hold", fig.cap="SPSS output for the hats data", fig.align="center", out.width="60%"}
knitr::include_graphics( c("SPSS/Sunglasses/SunglassesSPSS-CI.png") )
```
...
- Sect. \ref{thinkBox:WhatGraph}: Stacked bar chart; side-by-side bar chart.
