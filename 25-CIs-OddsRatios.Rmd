...
One interest was the relationship between where they ate, and where the student lived.
...
Eitehr way, the parameter is the population OR, comparing the odds of eating most meals *off*-campus for students living with their parents to students *not* living with their parents.
...
That is, the odds and odds ratios are relative to the **first row** or **first column**.
:::
...
EatingNumericalSummary[1, ] <- c(round(UniS[1, 1]/UniS[2, 1 ], 4),
...
```{r EatingGraphs, echo=FALSE, fig.cap="A plot of the uni-student eating data: A side-by-side bar chart", fig.align="center", fig.height=4, fig.width=7.7, out.width="85%"}
par( xpd = TRUE,
...
Using either jamovi (Fig.&nbsp;\@ref(fig:UniMealsTestOutputjamovi)) or the *second table* of the SPSS output (labelled, obscurely, **Risk Estimate**; Fig.&nbsp;\@ref(fig:UniMealsTestOutputSPSS)), the sample OR is 5.94 (as computed), and the (exact) 95% CI is from 1.35 to 26.1.
(The SPSS output shows other information too, some of which will be useful later.)


```{r UniMealsTestOutputjamovi, echo=FALSE, fig.show="hold", fig.cap="The jamovi output for computing a CI", fig.align="center", out.width="50%"}
...
* Odds are Row 1 to Row 2; then odds ratio compares Column 1 odds to Column 2 odds; or
* Odds are Column 1 to Column 2; then the odds ratio compares Row 1 odds to Row 2 odds.
:::
...
That is, there is a 95% chance that this CI straddles the population OR.
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
Interpreting ORs can be confusing, so care is needed!
:::
...
The statistical validity condition is a tricky to understand (but is explained further in Sect.&nbsp;\@ref(ExpectedValues)).
...
From Table \@ref(tab:MealsDataTable), the *observed* counts are *not* all greater than five (one cell has a count of 2).
However, the statistical validity condition is about the *expected* counts, not the *observed* counts.
...
```{r B12BarchartsCI, echo=FALSE, fig.cap="A side-by-side barchart comparing the number of women B12 deficient", fig.align="center", fig.width=4, fig.height=3}
### B12 example
...
1. The *percentage* of people without post-grad study who would buy an EV in the next 10 years is `r if (knitr::is_html_output()) '(**do not** add the percentage symbol)'`:  
