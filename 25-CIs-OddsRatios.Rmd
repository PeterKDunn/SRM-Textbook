...
One interest was the relationship between eating on-campus, and where the student lived.
...
The comparison here is *between* individuals.
:::


The parameter of interest could be the difference between the *proportions* (or *percentages*) in each group, or the **odds ratio**.
However, for reasons that we can't delve into, usually the [*odds ratio* (OR)](#OddsRatio) is used as the parameter.
One reason is that software produces output related to the sample OR.
...
(Noitce the *fiurst* column is always on top of the fraction.)
