...
> Based on the sample, a $95$% CI for the OR comparing the odds of eating most meals off-campus is $5.94$, with the $95% CI from $1.35$ to $26.1$ (living with parents, compared to *not* living with parents).
...
> The population odds ratio for a crash involving pedestrians (comparing 2011 to 2015) is $0.986$, and has a $95$% chance of being between $0.480$ and $2.018$.
:::
...
A study [@candan2021first] compared the proportion of infected nests between relocated nest (relocated due to the risk of tidal inundation) and non-relocated nests (Table\ \@ref(tab:TurtleNestDataTable))..
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(TurtleData,
                      all = TRUE,
                      decDigits = 0),
        format = "latex",
        booktabs = TRUE,
...
> Based on the sample, the OR of an infected nest, comparing natural nests to relocated nests, is $1.66$ with a $95$% CI from $0.537$ to $5.12$.
...
## Example: pet birds REMOVE?? {#PetBirdsOR}
...
The CI will be statistically valid if all expected counts exceed five.
...
> Based on the sample, the OR of being B12 deficient (comparing vegetarians to *non*-vegetarians) is $3.15$, and the $95$% CI is from $1.08$ to $9.24$.
...
\fontsize{10}{12}\selectfont
```{r}
knitr::kable( surroundMaths(t(SOI.tab),
                            all = TRUE,
                            decDigits = 0),
         format = "latex",
         booktabs = TRUE,
