...
The two groups (live with parents; do not live with parents) contain different students, so the comparison is *between* individuals.
:::
...
(Notice the *first* column is always on top of the fraction.)
...
For consistency with software output, the odds calculation uses the pedestrian data on top of the fraction (Row 1), and the odds ratio is computed using the 2011 data on top of the fraction (i.e., Column 1)
