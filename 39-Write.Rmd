...
Formal guidelines for writing and reporting research exist, for experimental studies [@hopewell2022update] and observational studies [@von2007strengthening], though we will not delve into these specifically.
...
Even journals in the same discipline can have different expectations.
...
The study simply shows a difference in proportions, but does not study *why*.
To learn the reason, provisional drivers could be surveyed.
:::


A high standard of grammar is also expected, including correctly using commonly-confused words (there/their; your/you're; affect/effect; chose/choose; etc.), capitalisation and apostrophes (*not* apostrophe's!).
Ensure correct spelling, punctuation and formatting, and use (but do not rely upon) a spell checker.
...
Ensure you can keep to your allocated time.
...
Slides should *not* contain information that you *read* to the audience; a good presenter adds important details around the structure provided by information on the slides.
...
The *Appendix* may include large tables, images, discussions of technical details, and so on.
...
* *Average*:\index{Average}\index{Mean}\
  in research, 'average' refers to *any* way of measuring the typical value (Sect.\ \@ref(ComputeAverage)) including the mean and the median, but also other measures too.
  Use the specific word 'mean' or 'median' when that is what you intend.
* *Confidence*:\index{Confidence interval}
  In research, 'confidence' is usually used in the phrase 'confidence interval' (Sect.\ \@ref(CIInterpretation)).
  Take care not when using 'confidence' in other contexts.
* *Comparison*: \index[Comparison}
  Distinguishing between a 'within-individuals' and 'between individuals' comparison is important (Sect.\ \@ref(Intervention)).
* *Control*:\index{Control}
  In research, a 'control' refers to a specific situation, and is helpful for maximising internal validity (Def.\ \@ref(def:Control)).
* *Correlation*:\index{Correlation}
  In research, correlation describes the (usually linear) relationship between two *quantitative* variables (Sect.\ \@ref(CorrCoefficients)).
   In general use, 'correlation' is often used to mean 'association'.
* *Estimate*:\index{Estimate}
  In research, 'estimating' usually means to *calculate* a sample estimate for an unknown population parameter.
...
* *Intervention*:\index{Intervention}
  In research, an 'intervention' (Sect.\ \@ref(Intervention)) is when the researchers manipulate the comparison.
* *Normal*:\index{Notrmal distribution}
...
* *Random*:\index{Random}
  In research, 'random' has a specific meaning.
  In general usage, it often means 'haphazard' or 'without planning'.
* *Regression*:\index{Regression}
  In research, 'regression' refers to the mathematical (usually linear) relationship between two quantitative variables (Sect.\ \@ref(Regression)). 
* *Sample*: \index{Sample}
...
* *Significant*:\index{Statistical significance}\index{$P$-values}
...
* $\beta$:\index{Regression parameter}
  In research, $\beta$ refers to the regression parameters (Sect.\ \@ref(Regression-Software)).
