...
* When appropriate, use the correct, technical words (and use them correctly!).
...
Formal guidelines for writing and reporting research exist, for experimental ([CONSORT](http://www.consort-statement.org/)) and observational studies ([STROBE](https://www.strobe-statement.org)).
We will not delve into these specifically, but these websites are useful resources.
...
A series of experimental studies concludes that students believe that "increasing the complexity of their vocabulary" [@oppenheimer2006consequences, p. 139] makes them sound smarter;
that is, students like to use fancy words to sounds clever.
One conclusion of the research was that using 'fancy' language *does not work*: 'needless complexity leads to negative evaluations...' (@oppenheimer2006consequences, p. 151).
Of course, use the fancy word if that is the best wors and the correct word.
...
1. Set up the context and background for the paper.
1. Define the language and definitions used in the study.
1. Allow the reader to become familiar with the theoretical groundwork of the subject.
...
The Results summarise what was found from the data, especially regarding the initial RQ.
The Results section:
...
One aspect of writing well is selecting the best word to use, especially when many words have a different meaning in other scientific disciplines, or compared to every-day use [@richardson2013identification; @dunn2016learning].
If you are unsure of the definitions used in this book, use the Glossary (Appendix \@ref(Glossary)).
Here are some words where care is needed:
...
## Constructing tables, figures and graphs

Good figures and tables can be hard to produce.
However, their purpose should always be to *display the data in the simplest, clearest possible way*.
In general, tables and figures:
...
Figures typically have captions *below*, while tables typiucally havecaptions *above*.
...
\hspace{0.1\textwidth}
\begin{minipage}{0.45\textwidth}%
```{r echo=FALSE, out.width='95%', fig.width=7, fig.height=6}
