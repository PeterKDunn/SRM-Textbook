...
if( knitr::is_latex_output() ) {
  tb1 <- cbind( ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][1:6],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][7:12],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][13:18],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][19:24],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][25:30],
                c(ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][31:32], NA, NA, NA, NA) )
  ...
  T1 <- knitr::kable( pad(tb1,
    ...
    add_header_above( c("Using phone" = 6),
                      bold = TRUE)

  tb2 <- cbind( ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][1:6],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][7:12], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][13:18], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][19:24], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][25:30], 
                c(ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][31:32], NA, NA, NA, NA))
  T2 <- knitr::kable( pad(tb2,
                      ...
                      booktabs = TRUE) %>%
    row_spec(0, bold = TRUE) %>%
    add_header_above( c("Not using phone" = 6),
                      bold = TRUE)
...
A suitable graphical summary of the data is a boxplot (Fig.\ \@ref(fig:PhonePlots)), which shows that the *sample* medians are slightly different, and the IQRs slightly smaller for the phone-using group; one large outlier is present for the phone-using group. 
...
(An error bar chart can be used to compare more than two groups, too.)
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
What is different about the information displayed in the error bar chart (Fig.\ \@ref(fig:PhonePlots)) and the boxplot (Fig.\ \@ref(fig:PhoneErrorbar))?\label{thinkBox:DiffErrorbarBoxplot}
...
Since the difference between the *population* means  $\mu_P - \mu_C$ is unknown, the difference is estimated using the *sample* means; the *statistic* is $\bar{x}_P - \bar{x}_C$.
...
Then, finding a $95$%\ CI for the difference between the mean reaction times is similar to the process used in Chap.\ \@ref(OneMeanConfInterval). 
Almost all CIs have the same form:  
\[
...
Output from jamovi (and other software) shows *two* CIs (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)).
...
> For patients with T2DM, what is the mean difference between the average reductions in TG, between those drinking chamomile tea and those who drink hot water?
...
The approximate $95$%\ CIs for each group can also be computed, and aan error bar chart produced (Fig.\ \@ref(fig:ChamomilePlot)).


```{r ChamomilePlot, fig.align="center", fig.cap="The reduction in total glucose for the chamomile-tea drinking group, and the control group. The horizontal grey line represenetd no mean change in the groups", out.width='45%', fig.width=3.5, fig.height=3.25}
...
Use the jamovi output in Fig.\ \@ref(fig:BodyTempGenderjamovi) to construct an approximate $95$%\ CI appropriate for answering the RQ,.
:::
