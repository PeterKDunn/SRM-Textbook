...
```{r, child = if (knitr::is_html_output()) {'introductions/32-CIs-TwoMeans-HTML.Rmd'} else {'introductions/32-CIs-TwoMeans-LaTeX.Rmd'}}
```
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For those *using* a phone, what is the difference in meaning between the *standard deviation* and the *standard error* in this context?\label{thinkBox:SEsd}
...
```{r SignageErrorBar, fig.cap="Boxplot (left); error bar chart (right) showing the mean speed before and after the addition of extra signage, and the $95$\\% CIs. The vertical scales on the two graphs are different.", fig.align="center", out.width='70%',fig.width=6.0, fig.height=3.25}
data(Speed)
...
The NHANES study records data annually about a sample of Americans.
The data can be used to ask the RQ:
...
EchinaceaInfo[3, ] <- c(0.53, NA, 0.367, NA)
