...
Observational studies can be classified by their *directionality* (Table \@ref(tab:TypesOfObsStudies)):   

* [Backward direction](#BackwardStudies): 
  The values of the response variable are obtained, then the study looks into the past for the explanatory variable values
* [Forward direction](#ForwardStudies): 
  The values of the explanatory variable is obtained, and the study looks to the future for the response variable values
* [Mixed direction](#NondirectionalStudies): 
  The values of the response and explanatory variables are observed at the same time.

Experimental studies always have a forward direction.
Many specific types of observational studies exist (case-control studies; cohort studies; etc.), but we will not delve into these (and the definitions may be ambiguous).
...
colnames(ObsStudies) <- c("Type", 
  ...
  ObsStudies[1, ] <- c("Backward direction", 
                        "Look back to find", 
                        "When study starts",
                        "Sect. \\ref{BackwardsStudies}")
   ObsStudies[2, ] <- c("Forward direction", 
                        "When study starts", 
                        "Look forward to find",
                        "Sect. \\ref{ForwardsStudies}")
   ObsStudies[3, ] <- c("No direction", 
                        "When study starts", 
                        "When study starts", 
                        "Sect. \\ref{NondirectionalStudies}")
...
### Backward directionality {#BackwardsStudies}

In studies with a *backward direction*, the values of the response variable are known (and individuals usually are selected because of this).
The study involves obtaining information about the values of the explanatory variable in the past.
...
### Forward directionality {#ForwardsStudies}

In studies with *forward directionality*, the values of explanatory variable are known when the study begins (and individuals usually are selected because of this).
...
### Non-directionality {#NondirectionalStudies}

In studies with *non-directionality*, the values of the response variable the explanatory variable are gathered at the same time.
These studies are often called *cross-sectional studies*.
...
Observational studies can usually be classified as having a **forward direction**, **backward direction**, or **mixed direction**.
