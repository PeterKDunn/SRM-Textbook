...
* *Nonanalytical studies*, that do *not* study relationships:
...
```{r POCIDescriptive, echo=FALSE, fig.cap="A descriptive study, used to answer a descriptive RQ", fig.align="center", fig.width=6, fig.height=2, out.width='70%'}
...
We do not discuss descriptive studies further, as they can be considered as a special case of observational studies.
...
```{r POCIObservational, echo=FALSE, fig.cap="An observational study, used to answer a relational RQ", fig.align="center", fig.width=6, fig.height=2, out.width='70%'}
showPOCI(addC = TRUE,
         addI = FALSE,
         addArrows = TRUE,
         addY = TRUE,
...
Experimental studies have an [*intervention*](#def:Intervention), and so *experimental studies answer interventional RQs*.
...
```{r POCIExperiment, echo=FALSE, fig.cap="An experimental study, used to answer interventional RQs", fig.align="center", fig.width=6, fig.height=2, out.width='70%'}
showPOCI(addC = TRUE,
         addI = TRUE,
         addY = TRUE, 
         addX = TRUE, 
...
if( knitr::is_latex_output() ) {
   ...
   kable(ExpStudies,
        ...
        caption = "Comparing experimental designs (descriptive studies do not have any comparison or connection (C))",
        align = c("r", "c", "c", "l"))   %>%
...
*True experiments* are commonly used, but conducting true experiments is not always possible.
...
This is still experimental, since the researchers may decide to switch which group receives echinacea; ultimately, the decision is still made by the researchers.
:::
...
She runs the program at one university only, then compares the average amount of drinking per person at two universities (A and B).
What type of study is this: *observational* or *true experimental*?\label{thinkBox:AlcoholProgram}
...
The researchers would need to *find* (not *create*) two existing groups of people (say, from two different suburbs) then decide to allocate one group to take echinacea, and allocate the other group to not take echinacea (Fig. \@ref(fig:QuasiExpStudiesImage)).
:::
...
Many studies reported that the bacteria living in the gut of people on the autism spectrum is different than the bacteria in the gut of people *not* on the autism spectrum (@kang2019long, @ho2020gut), and suggest the bacteria may contribute whether a person is autistic.
...
Other studies [@yap2021autism] suggest the relationship works in reverse:
people on the autism spectrum are more likely to be "picky eaters", contributing to the differences in gut bacteria.
:::
...
The explanatory variable (the amount of soft drink consumed) is measured when the study begins.
...
* may suffer from *recall bias* when people are the individuals: accurately recalling the past can be unreliable.
...
For this reason, experimental studies usually have higher internal validity than observational studies.
...
A review of programs that used double-fortified salt to manage iodine and iron deficiencies examined numerous existing studies.
...
One of many threats to internal validity is that the groups being compared are initially different; for example, if the group receiving echinacea is younger (on average) than the group receiving no medication.
...
The sample comprises the Queensland university students actually studied.
The study is externally valid if the sample is a random sample from the population of all Queensland university students.

The results will not necessarily apply to university students outside of Queensland (though they may), or all Queensland residents.
...
The researcher *did* decide how to allocate the program to University A or University B.
- Sect. \ref{thinkBox:GardenersDirection}: *Backward directionality*: people were identified with an infection, and then the researchers looked *back* at past activities.
