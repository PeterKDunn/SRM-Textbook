...
```{r Bar3D2D, out.width='100%', fig.width=9, fig.height=4.5, fig.align="center", fig.cap="Two plots of the NHANES participants, divided by age group and sex. Left: A three-dimensional bar chart. Right: a side-by-side bar chart"}
...
@data:Solomon2002:Ginko studied using ginko for memory enhancement.
Caregivers blinded\index{Blinding} to the treatment (ginko or placebo)\index{Placebo} were asked to report the impact on subjects' memory.
The bar chart in Fig.\ \@ref(fig:PieSmallCounts) (right panel), for those on the placebo, shows that four of the available categories had zero responses, and one had a very small number of responses (two).
The pie chart (left panel) make the small category difficult to see, and the categories with zero counts are impossible to see.
:::
