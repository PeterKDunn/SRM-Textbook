...
`r if( knitr::is_html_output() ) {torf(answer = FALSE )}`
6. The variation in the value of the statistic from sample to sample is called *sampling variation*.
`r if( knitr::is_html_output() ) {torf(answer = TRUE )}`
...
`r if( knitr::is_html_output() ) {torf(answer = FALSE )}`
8. In the sample, we know exactly what value of\ $\hat{p}$ to expect.  
`r if( knitr::is_html_output() ) {torf(answer = FALSE )}`
