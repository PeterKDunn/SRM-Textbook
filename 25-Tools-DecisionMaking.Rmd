...
Suppose I produce a standard pack of cards, and shuffle well.
I am interested in the event 'the number of red cards when I draw $25$\ cards from the pack, with replacement'.
('With replacement' means that, after drawing a card, I place the card back into the pack, and reshuffle before drawing a new card.)
The pack of cards can be considered the *population* (where the proportion of red cards is $p = 0.5$ for each draw, because sampling is with replacement).
...
Of course, the sample of $25$\ cards in just one of *countless* possible samples that could have been chosen to study.
...
In research, decisions need to be made about parameters, based on one of many possible values of the statistic.
...
In the cards example, obtaining $25$\ reds cards out of\ $25$ (i.e., $\hat{p} = 1$) seems very unlikely; you would probably conclude that the pack is somehow unfair, or that I was cheating somehow.
...
2. Based on that assumption, you *expected* about half the cards in the sample of\ $25$ to be red (i.e., expect\ $\hat{p}$ to be about\ $0.5$).
   You wouldn't necessarily expect *exactly* half red and half black (because of sampling variation), but you'd probably expect that\ $\hat{p}$ would be close to\ $0.5$.
...
Two reasons could explain why the *sample* proportion of red cards in the sample of $25$\ cards ($\hat{p} = 1$)is not equal to the value of the population proportion ($p = 0.5$).

1. *The _population_ proportion of red cards really is $p = 0.5$*, and the *sample* proportion $\hat{p}$ is not equal to $0.5$ only due to *sampling variation*.
...
2. *The _population_ proportion of red cards really isn't $p = 0.5$*, and this is simply reflected in the observed sample proportion.
...
1. The first explanation is the *null hypothesis*, denoted $H_0$.\index{Parameter}\index{Hypotheses!null} 
   This hypothesis states that *the population proportion is $0.5$*; the value of the sample proportion is *not* $0.5$ due to *sampling variation*. 
2. The second explanation is the *alternative hypothesis*, denoted $H_1$.\index{Hypotheses!alternative}
   This hypothesis states that the population proportion is not $0.5$, which is reflected in the value of the sample proportion.
...
and so the statistic is $\hat{p} = 25/25 = 1$.
...
Observing $25$\ red cards out of $25$\ cards is highly unusual, so the chance that our specific sample produced $\hat{p} = 1$ is incredibly unlikely.
...
Figure\ \@ref(fig:SamplingDistCards) shows that $\hat{p} = 0.6$ could reasonably be observed, since there are many possible samples that lead to $\hat{p} = 0.6$, or even higher.
...
Many dental associations recommend brushing teeth for two minutes (i.e., $120\secs$).
@data:Macgregor1979:BrushingDurationKids recorded the toothbrushing time for $85$\ uninstructed schoolchildren from England to assess compliance with these guidelines.

Of course, every possible sample of $85$ children in England will include different children, and so produce a different sample mean brushing time\ $\bar{x}$.
Even if the *population* mean toothbrushing time really was $120\secs$ (i.e., $\mu = 120$), the *sample* mean probably wouldn't be exactly $120\secs$, because of sampling variation.

*Assume* the population mean toothbrushing time is $\mu = 120$; that is, $H_0$ is $\mu = 120$.
*If* this is true, we then could describe what values of the sample statistic $\bar{x}$ could be *expected* from all possible samples.

The study found the mean time spent brushing was $60.3\secs$, with a standard deviation of $23.8\secs$.
To determine if $\bar{x} = 60.3$ is unusual from a sample of $n = 85$, we need to describe the sampling distribution of $\bar{x}$: how sample means are likely to vary for samples of size $85$ when $\mu = 120$.

Using the ideas in Chap.\ \@ref(OneMeanConfInterval), and in Sect.\ \@ref(SamplingDistSampleMean) specifically, the sampling distribution of $\bar{x}$ has an approximate normal distribution, with mean $\mu = 120$ and a standard deviation of $\text{s.e.}(\bar{x}) = 23.8/\sqrt{85} = 2.58\secs$ (shown in Fig.\ \@ref(fig:SamplingDistBrushing)).
...
```{r, SamplingDistBrushing, fig.cap="The sampling distribution for $\\bar{x}$, the mean toothbrushing time in schoolchildren from England. A sample mean of $60.3\\secs$ seems very unlikely.", fig.align="center", fig.width=8.5, fig.height=2.75, out.width='85%'}
