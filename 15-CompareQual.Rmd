...
However, to *compare* Methods\ A and\ B, the odds and percentages (or proportions) of successful results need to be computed for each row separately.
...
- Method\ A: 
  the odds of success is $81/6 = 13.5$: there are $13.5$ more successful procedures than failures for Method\ A.
- Method\ B: 
  the odds of success is $234/36 = 6.5$: there are $6.5$ more successful procedures than failures for Method\ B.
...
Comparing rows (i.e., using row percentages and row odds) seems more intuitive than column percentages here: they compare the success percentage for each method.
:::
...
-   *Method\ A*: the proportion of successful procedures is $\hat{p}_A = 0.931$.
-   *Method\ B*: the proportion of successful procedures is $\hat{p}_B = 0.867$.
...
-   The *odds* in each column compares Row\ 1 counts to Row\ 2 counts.
...
- ORs *greater than*\ $1$ mean the odds of the result is *larger* for the group on top of the division compared to the group on the bottom.
- ORs *equal to*\ $1$ mean the odds of the result is the *same* for both groups (on the top and the bottom of the division).
- ORs *less than*\ $1$ mean the odds of the result is *smaller* for the group on the top of the division compared to the group on the bottom.
...
- *Method\ A*: success proportion for *large* kidney stones: $192/263 = 0.730$.
- *Method\ B*: success proportion for *large* kidney stones: $55/80 = 0.688$.
...
- *Method\ A*: success proportion for *all* kidney stones: $273/350 = 0.780$.
- *Method\ B*: success proportion for *all* kidney stones: $289/350 = 0.826$.
...
- *Method\ A* is more successful for *large* stones ($73.0$% vs\ $68.8$%).
...
```{r SimpsonRulesStones, fig.cap="The size of the stones is associated with both the success percentage and the method.", fig.align="center", fig.height=2.15, out.width='70%'}
...
if( knitr::is_html_output() ) {
  WANumericalSummary[3, 1] <- paste0("Difference: $",
                                     WANumericalSummary[3, 1],
                                     "$")
  WANumericalSummary[3, 2] <- paste0("OR: $",
                                     WANumericalSummary[3, 2],
                                     "$")
...
Suppose the sample OR has a value *smaller* than one.
...
rownames(Dispatchers) <- c("Female patient",
                           "Male patient")
...
* the student type:
each student was classified as a 'soccer player' ($63$\ students), a 'non-soccer athlete' ($96$ students), or a 'non-athlete' ($81$\ students).
