...
* *Avoid leaps of logic, and reaching conclusions unsupported by the evidence*.  
  ...
  For example, a student project found that the proportion of provisional drivers (those yet to get a open licence) was *higher* in the free university car park, compared to paid car parks.  
  \ \    
...
* *Producing reproducible research*.  
  When possible, research should be *reproducible* (Sect.\ \@ref(ReproducibleResearch)).\index{Research!reproducibility}
...
* *Use of artificial intelligence (AI)*.\index{Artificial intelligence}  
  ...
  AI should never be listed as an author, as AI cannot verify accuracy.
...
As a *visual* medium, presentations usually omit technical details and give the audience an overview of the major points and processes; sharing tedious technical details in a presentation is unlikely to produce an engaging presentation. 
Presentations tend to focus on the *why* and the *what* of the research.
...
Some journals require a *structured abstract*, where the *Abstract* contains specific headings.
...
* show how the research fills a gap in existing knowledge, by using a literature review (sometimes a *Literature review* is a separate section).
...
Counts are usually written in words (i.e., 'three') when fewer than ten (or sometimes twelve), and otherwise presented using digits ('12\ 041').
...
* *Control*:\index{Control}
  In research, a 'control' is usually used in the context of a control group (Def.\ \@ref(def:Control)).
...
Some *symbols* may also have different meanings in other disciplines; care is needed when using these symbols:
...
Remember: 'Don’t write so that you *can* be understood; write so that you *can't* be misunderstood', and 'Write what you mean; mean what you write'.
