...
A study can blind the *researcher* to knowing what comparison group the individuals are in.  
...
: The carry-over effect occurs when the influence of one treatment or conditions influences individuals' responses to subsequent treatments or conditions.
...
The $\chi^2$ statistic measures the overall size of the differences between the expected counts and observed counts, over the entire $2\times 2$ table.
...
We studied CIs in some specific situations (there are hundreds more!):
...
[Continuous data](#def:ContinuousData)\index{Data!quantitative continuous|textbf}\index{Variables!discrete quantitative|textbf}
...
: An *extraneous* variable is any variable associated with the response variable, but is not one the explanatory variable. 
...
: The *Hawthorne effect* is the tendency of individuals to change their behaviour if they know (or think) they are being observed, in experimental studies (Sect.\ \@ref(HawthorneEffect)).
...
In this book, we have looked at some [specific hypothesis tests](#SelectTest):
...
: *Jittering* is when a small amount of randomness is added in either the horizontal or vertical direction (or sometimes both) to separate points that would otherwise be over-plotted.,
...
[Multistage sampling](#MultistageSampling)\index{Sampling!multistage|textbf}
...
: *Stacking* is when points are plotted above other points with similar values, to separate points that would otherwise be over-plotted.,
