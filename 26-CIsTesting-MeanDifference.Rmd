...
# CIs and tests: mean differences (paired data) {#PairedCI}
\index{Research question!relational}


<!-- Introductions; easier to separate by format -->
```{r, child = if (knitr::is_html_output()) {'./introductions/26-CIsTesting-MeanDifference-HTML.Rmd'} else {'./introductions/26-CIsTesting-MeanDifference-LaTeX.Rmd'}}
```
...
Each subject has a *pair* of 6MWT measurements, and the study produced *paired data*, the topic of this chapter.\index{Data!paired}\index{Study types!paired}
The data collected for this study are shown
...
In addition, linking the $20$\ms and $30$\ms 6MWD that belong together for each individual patient is not possible,
...
can be treated like a single sample of data (Table\ \@ref(tab:PairedNotation)):
...
> For Thai patients with chronic obstructive pulmonary disease, is the mean difference for 6MWT distance less when  subjects use a\ $20$\ms walkway compared to a\ $30$\ms walkway?
...
can be treated like a single sample of data.
Recapping Sect.\ \@ref(MeanDiffCI), the notation used is:
...
The RQ for the 6MWT study asks if the mean 6MWT is less when subjects use a\ $20$\ms walkway compared to a\ $30$\ms walkway.
The *parameter* is the *population mean difference* in 6MWT, $\mu_d$. 
Earlier (XREF), the differences are computed as the $30$\ms\ distance minus the $20$\ms\ distance.
...
Since the RQ asks specifically if mean 6MWT is *smaller* for a $20$\ms walkway, the alternative hypothesis is *one-tailed* (Sect.\ \@ref(AboutHypotheses)).
...
The differences (i.e., the **DIff.** columns in
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:Data6MWT)'
} else {
   'Sect.\\ \\@ref(PairedIntro)'
...
In addition, the notation when working with paired data is similar to that used when working with the single-mean case too (Table\ \@ref(tab:PairedNotation)):\index{Sampling distribution!mean difference}
...
The sampling distribution, as described in Sect.\ \@ref(def:DEFSamplingDistributionDbar), still applies, where $\mu_d$ is the value given in $H_0$:

* an approximate normal distribution,
* centred around the *sampling mean* whose value is the population mean *difference*\ $\mu_d = 0$ (from $H_0$),
* with a standard deviation, called the standard error of the difference, of 
$\displaystyle\text{s.e.}(\bar{d}) = 3.117$ (from Eq.\ \@ref(eq:StdErrorDifferences)).


The *sample* mean\ 6MWT change will vary, depending on which one of the countless possible samples is randomly obtained, *even if* the mean change in the population is zero.
...
The sample mean difference can be located on the sampling distribution (Fig.\ \@ref(fig:SMWTSamplingDistHT)) by computing the $t$-score:\index{Hypothesis testing!mean difference}\index{Test statistic!t@$t$-score}
$$
	t
...
Software displays the same $t$-score (Fig.\ \@ref(fig:SMWTTestOutput)).
...
Software (Fig.\ \@ref(fig:SMWTTestOutput)) reports that the *two*-tailed $P$-value is less than\ $0.0001$.
Hence, the *one*-tailed $P$-value is less than $0.0001/2 = 0.00005$.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The software clarifies *how* the differences have been computed.
At the left of the output (Fig.\ \@ref(fig:SMWTTestOutput)), the order implies the differences are found as `Distance30` (the $30$\ms walk distance) minus `Distance20` (the $20$\ms walk distance), the same as our definition.
:::
...
> Very strong evidence exists in the sample (paired $t = 7.07$; one-tailed $P < 0.0005$) of a mean reduction in 6MWT for a $20$\ms walkway compared to a $30$\ms walkway (mean reduction: $22.03$\ms; $95$% CI: $15.80$\ms to\ $28.26$\ms; $n = 40$).
...
## Example: soil nitrogen {#PairedTTest-SoilN}
\index{Research question!relational}
...
The approximate $95$%\ CI for the difference is $1.36 \pm (2\times 0.940)$, or from\ $-0.52$ to\ $3.24$ days.
...
The CIs for the mean reduction in TG in each group are:
