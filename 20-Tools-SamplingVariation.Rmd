...
1. Make an *assumption* about the *parameter*.
1. Based on this assumption, describe what values the *statistic* might reasonably be *expected* from all possible samples.
1. *Observe* the sample data from just one of those many possible samples.
...
We saw in Sect.\ \@ref(MakingDecisionsInResearch) that the sample proportion of red cards in a sample of $15$ varied from hand to hand.
Furthermore, the distribution of this proportion was approximately distributed with a bell-shape.
...
The *sampling distribution* often (but not always) has a normal distribution shape.
...
```{r RouletteWheel, fig.cap="A European roulette wheel, with numbers\\ $0$ to\\ $36$.", echo=FALSE, cache=FALSE, out.width = "45%", fig.align="center", fig.width = 3.5, fig.asp=1}
# Wheel  
DD <- 10
dd <- 4
...
The proportion of odd spinsc\ $\hat{p}$ varies from sample to sample (sampling variation), as shown by the histogram (Fig.\ \@ref(fig:RouletteWheelHist), top left panel).
...
Typically, in a reseaech study, *standard deviations* describe the variation in the individuals in a sample: how observations vary from *individual to individual*.
...
1. A study estimated the proportion of children aged under\ $14$ who owned a mobile (cell) phone.
   The researchers wished to report this estimate, with an indication of the precision of that estimate.
:::
