...
Normal distributions represent the theoretical distributions of unknown population distributions, not a sample.
...
(These values may be guided by sample values; e.g., suggesting a mean possum weight of $1000$\ g based on Fig.\ \@ref(fig:HistogramDBPPossums) (right panel) would be silly.)
...
For example, every America has a diastolic blood pressure and so is represented in Fig.\ \@ref(fig:HistogramDBPPossums) (left panel); every possum has a weight and so is represented in Fig.\ \@ref(fig:HistogramDBPPossums) (right panel). 
...
Using the 69--95--99.7 rule, $68$% of Australian women will be between $162 - 7 = 155$\ cm and $162 + 7 = 169$\ cm tall.
Similarly, using the 69--95--99.7 rule, $95$% of Australian women will be between $162 - (2\times 7) = 148$\ cm and $162 + (2\times 7) = 176$\ cm tall.
:::
...
\begin{equation}
   ...
   (\#eq:zscore214)
\end{equation}
that is, about $2.43$ standard deviations *below* the mean.
...
work with $z$-scores to two decimal places, so consider the $z$-score in Sect.\ \@ref(ApproxProbs) as $z = -2.43$.
...
This agrees with the rough answer using the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule): less than $2.5$%.

The general approach to computing probabilities from normal distributions is:

* *Draw a diagram*, and mark on the values of interest.
...
The diameter of specific trees were modelled using
...
This is a different type of problem than before; previously, the *tree diameter* was known, so a $z$-score could be computed, and hence a probability (Fig.\ \@ref(fig:WorkingWithZ), top panel).

However, in Example\ \@ref(exm:MNormalBackwards), the *probability* is known, and a tree diameter is sought.
That is, working 'backwards' is necessary (Fig.\ \@ref(fig:WorkingWithZ), bottom panel), so the $z$-tables need to be used 'backwards' too.
...
The retention time is $41.07$\ hrs.
