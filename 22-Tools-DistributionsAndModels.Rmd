...
The values of the statistic that could be observed from all possible samples can be described.
...
```{r RouletteWheelHistPropMean, results='hide', fig.width=8.5, fig.height=2.5, fig.cap="Sampling distributions for the proportion of red spins (left), and the mean of the numbers after $15$ roulette wheel spins (right) are approximate normal distributions", fig.align="center", out.width="90%"}
p <- 18/37
...
The *normal distributions* shown in Fig.\ \@ref(fig:RouletteWheelHistPropMean) represent how the histogram might appear theoretically, if we used an infinite number of simulations.
The normal distributions are *models* for what might occur in the *population*.
...
Apart from their use in modelling theoretical sampling distributions, many quantitative variables have approximate normal distributions too, when the distribution of the data in the *population* can be approximately modelled by a normal distribution.
...
```{r, fig.width=7.5, fig.height=2.25, out.width='100%', fig.align="center", fig.capt="The $68$--$95$--$99.7$ rule"}
par(mfrow = c(1, 3))

out <- plotNormal(mu = 0, 
                  sd = 1, 
                  main = "68% of observations are\nwithin 1 std dev of the mean",
...
out <- plotNormal(mu = 0, 
                  sd = 1, 
                  main = "95% of observations are\nwithin 2 std devs of the mean",
                  xlab = "Number of std devs from the mean")
shadeNormal(out$x,
            out$y,
            lo = -2,
...
out <- plotNormal(mu = 0, 
                  sd = 1, 
                  main = "99.7% of observations are\nwithin 3 std devs of the mean",
                  xlab = "Number of std devs from the mean")
shadeNormal(out$x,
            out$y,
            lo = -3,
...
Suppose heights of Australian adult females can be modelled with a normal distribution having a mean of $\mu = 162$\ cm, and a standard deviation of $\sigma = 7$\ cm, and (approximately) follow a normal distribution.
<!-- (based on the -->
...
<!-- } -->
Using the $68$--$95$--$99.7$ rule, $68$% of Australian women will be between $162 - 7 = 155$\ cm and $162 + 7 = 169$\ cm tall using this model.
Similarly, $95$% of Australian women will be between $162 - (2\times 7) = 148$\ cm and $162 + (2\times 7) = 176$\ cm tall using this model.
:::
...
For example, suppose heights of Australian adult females can be modelled with a normal distribution having a mean of $\mu = 162$\ cm, and a standard deviation of $\sigma = 7$\ cm (Example\ \@ref(exm:HeightsFemales)).

Using this model, the proportion of Australian adult women *taller* than $169$\ cm can be determined.
...
From earlier, the $z$-score for a height of $169$\ cm is  
\[
   z = \frac{x-\mu}{\sigma} = \frac{169 - 162}{7} = 1,
\]
one standard deviation *above* the mean.
Similarly, the $z$-score for a height of $148$\ cm is  
\[
...
```{r HtsEmpirical, fig.cap="The $68$--$95$--$99.7$ rule and the heights of Australian adult females", fig.align="center", fig.width=7.0, fig.height=2.75, out.width='70%'}
...
Suppose again that heights of Australian adult females can be modelled with a normal distribution with a mean of $\mu = 162$\ cm, and a standard deviation of $\sigma = 7$\ cm (Fig.\ \@ref(fig:HtsEmpirical)).
To find the proportion of women *shorter* than $145$\ cm, first draw the situation (Fig.\ \@ref(fig:HtsExer3)).
...
```{r HtsExer3, fig.cap="What proportion of Australian adult females are shorter than $145$\\ cm?", fig.align="center", fig.width=7.0, fig.height=3.00, out.width='75%'}
...
About $2.5$% of observations are less than $2$ standard deviations below the mean; that is, about $2.5$% of women are shorter than $148$\ cm.
...
Percentages found this way are crude, but often sufficiently precise.
...
Using this approach, more complicated questions can be asked.
...
```{r ZDBH1, fig.cap="What proportion of tree diameters are greater than $5$ inches?", fig.align="center", fig.width=7.0, fig.height=2., out.width='100%'}
DBH.mn <- 8.8
DBH.sd <- 2.7
DBH.x <- 5
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The normal-distribution tables *always* provide area to the *left* of the $z$-score.
...
```{r MatchDiagrams, fig.cap="Scenarios with their corresponding diagrams",  fig.align="center", out.width="100%", fig.height=1.50, fig.width=6.5}
par( mfrow = c(1, 4))
...
* For $5$ inches: $\quad  z = (5 - 8.8)/2.7 = -1.41$ (i.e., below the mean).
* For $11$ inches: $\quad z = (11 - 8.8)/2.7 = 0.81$ (i.e., above the mean).
...
The *largest* $25$% implies large trees, so required diameter is larger than the mean. 
...
A *model* is a way of describing the theoretically distribution of some quantitative variable.
...
## Quick revision questions {#Chap22-QuickReview}
...
The seed diameter was modelled with a normal distribution, with mean $7.5$\ mm and standard deviation of $0.225$\ mm.
...
**2b.** About $3.8$%
**2c.** About $48.7$%.
**2d.** *Smaller* than $7.38$\ mm.
**2e.** *Larger* than $7.21$\ mm
