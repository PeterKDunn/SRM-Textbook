...
```{r, child = if (knitr::is_html_output()) {'./introductions/22-Tools-DistributionsAndModels-HTML.Rmd'} else {'./introductions/22-Tools-DistributionsAndModels-LaTeX.Rmd'}}
```


## Introduction {#Chap18-Intro}

In the decision-making process used in research (Sect.\ \@ref(DecisionMaking)), an *assumption* is made about a parameter of the population.
...
Based on the assumption about the parameter, the values of the statistic that we would reasonably *expect* from all these possible samples can be described.
The challenge is that only one of these countless possible samples is observed.
...
As seen in Chap.\ \@ref(SamplingVariation), sampling distributions are often bell-shaped.
More formally, bell-shaped distributions are called *normal distributions* or *normal models*.\index{Distributions}\index{Normal distribution}

Many sampling distribution can be described by a normal distribution.
That is, the normal model is often used to describe the *sampling distribution*: how a statistic varies.\index{Sampling distribution}
...
In both cases, these sampling distributions had a rough *normal distribution* or *normal model* shape.
This was true for larger numbers of spins also (Figs.\ \@ref(fig:RouletteWheelHist) and\ \@ref(fig:RouletteWheelHistx)).


```{r RouletteWheelHistPropMean, results='hide', fig.width=8.5, fig.height=2.5, fig.cap="Sampling distributions for the proportion of red spins (left), and the mean of the numbers after $15$ roulette wheel spins (right)", fig.align="center", out.width="90%"}
p <- 18/37
...
The sampling distributions in Fig.\ \@ref(fig:RouletteWheelHistPropMean) are not *exactly* normal distributions (only $5\ 000$ sets of $15$ spins are used), but are very close to a normal distribution, and certainly close enough for most purposes.
Many sampling distributions have approximate normal distributions.
In addition, many quantitative variables have approximate normal distributions too, such as heights of people.


```{example NormalExamples, name="Normal distributions of data"}
Many quantitative *variables* have approximate normal distributions too.
...
```{r HistogramDBPPossums, fig.align="center", fig.height = 3, fig.width=8.5, out.width='100%', fig.cap="Two normal distributions: Diastolic blood pressure of $398$ Americans (left); the weight of $83$ male Leadbeater's possums (right).  The solid lines are the model."}
par( mfrow = c(1, 2))
data(Diabetes)
...
Sampling distributions represent theoretical distributions of unknown populations, not the distribution of sample data.
When the sampling distribution is a normal distribution, the mean of the distribution is denoted by $\mu$ and the standard deviation by $\sigma$.
(These values may be *guided* by sample values; e.g., suggesting a mean Leadbeater's possum weight of $1000$\ g based on Fig.\ \@ref(fig:HistogramDBPPossums) (right panel) would be silly.)
...
For any normal distribution:\index{68@$68$--$95$--$99.7$ rule}
...
Using the $68$--$95$--$99.7$ rule, $68$% of Australian women will be between $162 - 7 = 155$\ cm and $162 + 7 = 169$\ cm tall.
Similarly, $95$% of Australian women will be between $162 - (2\times 7) = 148$\ cm and $162 + (2\times 7) = 176$\ cm tall.
:::


## Standardising ($z$-scores) {#z-scores}
\index{zscores@$z$-scores}
...
(Another $16$% are shorter than one standard deviation *below* the mean, or less than $162 - 7 = 155$\ cm in height.)
...
The $z$-score is the *number of standard deviations the observation is away from the mean*, and is also called the *standardised value* or *standard score*.
Note that:
...
```{r HtsEmpirical, fig.cap="The empirical rule and heights of Australian adult females", fig.align="center", fig.width=7.0, fig.height=2.75, out.width='75%'}
...
As seen above, the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule) can be used to approximate percentages under normal distributions.
...
First draw the situation (Fig.\ \@ref(fig:HtsExer3)).
Proceeding as before, we ask 'How many standard deviations from the mean is $145$\ cm?'
Using Equation\ \@ref(eq:zscores), $145$\ cm corresponds to a $z$-score of  
\begin{equation}
...
```{r HtsExer3, fig.cap="What proportion of Australian adult females are shorter than $150$\\ cm?", fig.align="center", fig.width=7.0, fig.height=3.00, out.width='80%'}
...
This case is not covered by the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule), though the rule can be used to make *rough estimates*.
...
So the percentages females shorter than $145$\ cm (that is, even further into the tail of the distribution), will be *smaller* than $2.5$%.
While we don't know the probability exactly, it will be smaller than $2.5$%.

Probabilities found this way are crude, but often serviceable.
More accurate probabilities of 'percentages under the normal curve' are found using tables compiled for this very purpose
...
We now learn how to use these tables, using Example\ \@ref(exm:Height160) (Sect.\ \@ref(ApproxProbs)).
...
`r if (knitr::is_latex_output()) {
   'but only the *hard-copy* tables are explained in this book (see the [online](https://bookdown.org/pkaldunn/SRM-Textbook/tables.html#ZTablesOnline) version of this book for the online tables, and instructions for using the online tables).'
} else {
...
This is consistent with the rough answer using the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule): a value less than $2.5$%.
...
The diameters of a specific type of trees were modelled using
...
* *Deduce* the answer (Fig.\ \@ref(fig:ZDBH1), right panel):
  since the *total* area under the normal distribution is one, the probability of a tree diameter  *greater* than $5$\ inches is $1 - 0.0793 = 0.9207$, or about $92$%.




```{r ZDBH1, fig.cap="What proportion of tree diameters are greater than 6 inches?", fig.align="center", fig.width=7.0, fig.height=2., out.width='100%'}
DBH.mn <- 8.8
DBH.sd <- 2.7
DBH.x <- 5
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The normal-distribution tables *always* provide area to the *left* of the $z$-score that is looked up.
Drawing a picture of the situation is important: it helps visualise getting the answer from what the tables provide.
...
* For $6$ inches: $\quad  z = (5 - 8.8)/2.7 = -1.41$ (below the mean).
* For $11$ inches: $\quad z = (11 - 8.8)/2.7 = 0.81$ (above the mean).

Table\ B can then be used to find the area to the *left* of $z = -1.41$, which is $0.0793$.
Table\ B can also be used to find the area to the *left* of $z = 0.81$, which is $0.791$.
However, neither of these provide the area *between* $z = -1.41$ and $z = 0.81$ (Fig.\ \@ref(fig:ZDBH3)).


```{r ZDBH3, fig.cap="What proportion of tree diameters are between $5$\ and $11$\ inches? The two shaded areas are what we find using the tables with $z = -1.41$ and $z = 0.81$; neither give us the area we seek.", fig.align="center", fig.width=9.5, fig.height = 2.75,out.width='100%'}
...
out0 <- plotNormal(mu,
                   sigma,
                   cex.axis = 0.85,
                   main = expression(What~tables~give~"for"~italic(z)==-1.41),
                   xlab = "Tree diameters (inches)")
shadeNormal(out0$x,
            out0$y,
            col = "white",
...
\index{Normal distribution!using tables backwards}
...
Since an area is given, we need to work 'backwards' (Fig.\ \@ref(fig:DBHBackwards2)), so the $z$-tables need to be used 'backwards' too.
...
A study of methane produced by animals [@huhtanen2016effects] modelled the retention time of food in sheep using a normal distribution, with the mean retention time as $\mu = 42.5$\ hrs, and the standard deviation of the retention time as $\sigma = 3.68$\ hrs.
...
Using this model, what proportion of sheep have a retention time *less than* $40$\ hrs?
:::

A retention time of $40$\ hrs corresponds to a $z$-score of (Fig.\ \@ref(fig:RetentionPlots), top left panel):  
\[
   z = \frac{40 - 42.5}{3.68} = -0.68.
\]
This is a *negative* number, since $40$\ hrs is *below* the mean.
...
About $24.8$% of sheep have a retention times *less* than $40$\ hrs.


::: {.example #Methane2 name="Working with the normal distribution"}
What proportion of sheep have a retention time *greater than* $48$\ hrs (two days)?
:::

A retention time of $48$\ hrs corresponds to a $z$-score of $1.49$.
...
What proportion of sheep have a retention time *between* $40$\ and $48$\ hrs?
:::
...
A retention time of $40$\ hrs corresponds to $z = -0.68$, and, using the normal distribution tables, the area to the *left* of $z = -0.68$ is $0.2483$ (Fig.\ \@ref(fig:RetentionPlots), bottom left panel; hatched area).
...
From the two areas that we know, we *can* find the area that we seek:
...
We know the $z$-score, so the retention value is found using the unstandardising formula: $x = \mu + (z \times \sigma)$.
The retention time is about $41.1$\ hrs.



## Chapter summary {#Chap17-Summary}
...
::: {.exercise #CornSeeds}
In a simulation of methods to coat corn seeds (with fertilizer and crop protection chemicals, etc.), @pasha2016effect modelled the seed diameter as having a normal distribution, with mean $7.5$\ mm and standard deviation of $0.225$\ mm.

1. What is the probability that a seed has a diameter of more than $8$\ mm?\tightlist  
  `r if( knitr::is_html_output() ) {
	longmcq( c(
	   "About 2.22",
	   answer = "About 1.3%",
...
In a study to understand factors influencing preterm births [@snowden2018causal], the gestation length of healthy babies was modelled with a normal distribution, having a mean of $40$ weeks, and a standard deviation of $1.64$ weeks.
Using this model:
...
Based on a survey [@us20112009], they modelled the time at which EVs began charging as having a mean of $5$:$30$pm, with a standard deviation of $2.28$\ hrs.
For this model:
...
- Sect. \ref{thinkBox:MatchForward}: **1**--B; **2**--C; **3**--D; **4**--A.
- \textbf{\textit{Quick Revision} questions:}
**1a.** $`r round(z1, 2)`$. 
**1b.** $`r round(prob1, 2)`$.
**1c.** $`r round(1 - prob1, 2)`$.
**1d.** $`r round(z2, 2)`$.
**1e.** $`r round(prob2, 2)`$.
**1f.** $`r round(1 - prob2, 2)`$.
**2a.** About $1.3$%
**2b.** About $3.8$%
**2c.** About $48.7$%.
**2d.** *Smaller* than about $7.38$\ mm.
**2e.** *Larger* than about $7.21$\ mm
