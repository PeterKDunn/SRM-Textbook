...
If there was *no relationship* between the two variables, students living with or not with their parents would have a similar percentage of meals eaten on-campus.
First see that, overall, the percentage of meals on-campus is $157/183 = 85.79%$ (from Table\ \@ref(tab:MealsDataTable)).
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Compute the expected counts for the number of students eating most meals *on-campus*. vehicles.\label{thinkBox:ExpectedCounts}
:::


*You do not have to compute these expected values*, as software like jamovi can be used to produce the *expected* counts to check statistical validity.
...
(Note that one *observed count* is less than five.)
:::
...
Both the percentage and odds columns, and the odds ratio, suggest a difference between 2011 and 2015, in the *sample*.
