...
Quantitative data can be understood by knowing how often values of the variables appear.
...
The distribution of quantitative data can be summarised by the shape (Sect.\ \@ref(SummaryShape)), and summarised numerically by computing the average value (Sect.\ \@ref(ComputeAverage)), computing the amount of variation (Sect.\ \@ref(Variation)), and identifying outliers (Sect.\ \@ref(SummaryOutliers)).
...
Care is needed when creating frequency tables for continuous quantitative data, since all continuous data are rounded.
...
#    column_spec(3, width = "10mm")
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "The first ten observations (of $44$) of the baby-births data. The `birth time' is the number of minutes since midnight.") %>% 
    kable_styling(font_size = 8)
  out2 <- prepareSideBySideTable(out, 
                                 gap = "\\quad") 
  out2
...
We only consider histograms where the boxes have the same width, which is equivalent.].
The height of the histogram bars indicate the number (or percentage) in each category.
...
When the quantitative variable is discrete, the labels usually are placed on the axis aligned with the centre of the bar (e.g., Example\ \@ref(exm:HistogramsDiscrete)).
...
For example, the histogram shows that there were eight years where three severe cyclones were recorded. 

Notice that the different bin sizes change the appearance of the distribution (Example\ \@ref(exm:BinWidthFaithful)).
:::
...
One way to do this is to create boundaries between bins to one more decimal place than the given data.
...
For large data sets, these choices tend to matter less.
...
`r if( knitr::is_html_output() ) {
  ...
  'Fig.\\ \\@ref(fig:BBHist1) shows the histogram being constructed.'
}`
...
Notice that the choice impacts the look of the histogram.
:::
...
Software usually makes sensible default choices for the number of bins, and width of the bins.
...
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:BimodalFaithfulHistoChangeBins)) shows the shape of the distribution depends on bin width.'
} else {
...
* place the larger digit (e.g., kilograms) on the left (stems).
* place the smaller digit (e.g., first decimal of a kilogram) on the right (leaves).
...
```{r GorillasStem, fig.align="center", fig.width=7.25, fig.height=2.5, out.width='80%', fig.cap='The stemplot (right) for the gorilla chest-beating data (left)' }
if (knitr::is_latex_output()){
...
the points are *jittered*\index{Overplotting!jittering} (placed with some added randomness in the vertical direction)\index{Overplotting!stacking} or *stacked* (plotted above each other).
...
Hence, after producing a graph, the *distribution* of the data should be described using:
...
The average, variation and outliers are usually described numerically, too (as in Sect.\ \@ref(ComputeAverage) to\ Sect.\ \@ref(SummaryOutliers)).
...
Both summarise the same data, yet give *very*  different 'averages'.
...
The unknown population mean is estimated using the sample mean ($\bar{x}$), and every sample can give a different value for $\bar{x}$.
...
`r if (knitr::is_latex_output()) {
   'observations. This is shown in Fig.\\ \\@ref(fig:MeansFigLATEX) (left panel) for the gorilla data.'
} else {
...
The sample median, as estimate of the *population* median, is between $1.7$ (ordered observation\ $7$) and $1.7$ (observation\ $8$).
...
```{r DailyStreamflow, fig.cap="A dot plot of the daily streamflow at Mary River from 1960 to 2017, for February ($n = 1650$). The vertical grey line is the mean value. Many large outliers exist, so the data near zero are all squashed together. Note: values have been jittered in the vertical direction, but overplotting is still present near $0$.\\index{Overplotting}", fig.align="center", out.width='90%', fig.width=9, fig.height=3}
par(mfrow = c(1, 1))
set.seed(183763286)
...
The streamflow data are *very* highly right skewed, which is important for choosing between the mean and the median:
...
*The median is the better measure of average for these data*, though understanding the variation may be more important than understanding the average value.
These data may be better described using percentiles (Sect.\ \@ref(VariationPercentiles)).
...
```{r ShowVar, fig.cap="The standard deviation is related to the sum of the squared-distances from the mean. The chest-beating data are being used.", fig.align="center",fig.width=7.5, fig.height=4.25, out.width='80%'}
source("R/showVar.R") 
showVar() 
```
...
In large data sets, the differences are usually minimal.
:::



```{r QuartilesYgorillas, fig.align="center", fig.width=8.75, fig.height=3.25, fig.cap="A dotchart for the chest-beating data for young gorillas showing the IQR", out.width='100%' }
...
The bat data (Table\ \@ref(tab:BatData)) has $n = 11$.
...
That is, the first quartile $Q_1$ is the $25$th percentile, the second quartile $Q_2$ is the $50$th percentile (and median), and the third quartile $Q_3$ is the $75$th percentile.
...
* *The reason for the outlier is unknown*:
  *Discarding outliers routinely is not recommended*; the outliers are probably real observations that are just as valid as the others.
...
Some observations are missing, explaining the differences in sample sizes.
...
```{r WaterAcessQuant, fig.align="center", fig.cap="The age of the women, the number of people in the household, and the number of children in the household for the water-access study.", out.width='100%', fig.width=7.25, fig.height=2.25}
...
```{r NHANESCherryRipeHist, fig.cap="Left: the histogram of direct HDL cholesterol concentration from the NHANES study (large outliers exist but are hard to see). Right: the weights of `Fun Size' Cherry Ripe chocolate bars.", fig.align="center", fig.width=8.75, fig.height=3.15, out.width='100%'}
par(mfrow = c(1, 2))
...
[*Dataset*: `CherryRipe`]
The histogram of the weights of 'Fun Size' Cherry Ripe chocolate bars after 2016 is shown in Fig.\ \@ref(fig:NHANESCherryRipeHist) (right panel).
...
1. Compute the mean, median, standard deviation and IQR for the heights.
...
2. What is the mean and median number of microplastics found?
...
Describe the histogram in Fig.\ \@ref(fig:HistBrainFreeze).
:::
...
```{r VariationHistograms, echo=FALSE, fig.align="center", fig.cap="Histogram of marks for four classes", out.width='55%', fig.width=5.5, fig.height=4.75, out.width="80%"}
set.seed(2015)
