...
For *discrete* quantitative data, bins are defined to contain single discrete values, or a small number of discrete values.
...
For *continuous* data, care is needed when creating frequency tables, since all continuous data are rounded.
...
give the weights of $44$\ babies born in a hospital on one day [@mypapers:Dunn:dataset:1999; @data:Steele:BabyBoom], plus the gender of each baby, and the number of minutes after midnight of the birth (shown in the order in which the births occurred).
...
Most babies in the sam2ple are between\ $3$ and\ $4$\kgs at birth.
:::
...
```{r HistBrainFreeze, fig.align="center", fig.width=4.5, fig.height=2.65, out.width='45%', fig.cap="Histogram of the duration of brain-freeze symptoms after drinking ice water. Boundary observations are counted in the lower box." }
