...
The unknown population mean $\mu$ is estimated using the sample mean ($\bar{x}$) of $14$\ young gorillas
`r if( knitr::is_html_output() ) {
...
```{r ShowVar, fig.cap="The standard deviation is related to the sum of the squared-distances from the mean. The chest-beating data are used.", fig.align="center",fig.width=8, fig.height=4.25, out.width='90%'}
source("R/showVar.R") 
...
* the *IQR rule*, which is most useful in other situations (Sect.\ \@ref(OutliersIQRrule)).
...
The strategy for managing outliers depends on *why* they occur (@mypapers:dunnsmyth:glms, p.\ 138):
