...
```{r, fig.align="center", fig.cap="Two histograms of the severe-cyclone data", out.width='100%', fig.width=9.5, fig.height=3.25}
par(mfrow = c(1, 2))
...
* the *mean* daily streamflow, which is $1\ 123.2$\ ML.
* the *median* daily streamflow, which is $146.1$\ ML.
...
The sample median, as estimate of the *population* median, is between $1.7$ (ordered observation\ $7$) and $1.7$ (ordered observation\ $8$).
...
A measure of variation *not* affected by skewness\index{Skewness} is the inter-quartile range, or IQR.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The sample IQR *estimates* the population IQR, and every sample is likely to have a different sample IQR.
We usually only ever have one sample.
:::
...
```{r IQROutliers, fig.align="center", fig.width=9, fig.height = 3.5, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data", out.width = '99%' }
...
```{r WaterAcessQuant, fig.align="center", fig.cap="The age of the women, the number of people in the household, and the number of children in the household aged under $5$, for the water-access study.", out.width='100%', fig.width=7, fig.height=2.25}
...
in August from $1995$ to $2000$ (Table\ \@ref(tab:SOIvalues)).
...
The standard deviation for Dataset\ A in Fig.\ \@ref(fig:VariationHistograms) is $s = 2$.
...
```{r FourHistograms, echo=FALSE, fig.cap="Four histograms: where would they be useful?", fig.align="center", out.width='55%', fig.width=5.5, fig.height=4.5, out.width="80%"}
