...
The table gives the numbers and the corresponding percentages.
:::
...
The table also gives percentages in each bin; for example, the percentage of babies over $4.0$\kgs is $1/44 \times 100 = 2.27$%, or about $2$%. 
...
An observation on a boundary between the bins may be placed it in the *higher* box (i.e., $2.5$\kgs is in the '$2.5$ to\ $3.0$\kgs' box, not the '$2.0$ to\ $2.5$\kgs' box): see the left panel.
...
The histogram for the time between eruptions (Fig.\ \@ref(fig:BimodalFaithfulHistoChangeBins), top left panel) is bimodal, with peaks near $55$\mins and $80$\mins.
...
The chest-beating data are given to one decimal place, so the *sample mean* rate can be given as $\bar{x} = 2.22$ beats per $10$\hs.
:::
...
$$
The sample median chest-beating rate is $1.7$ beats per $10$\hs, with a *range* of $3.7$ beats per $10$\hs.
:::
...
$$ 
   s = \sqrt{\frac{20.9636}{14 - 1}}
     ...
     = 1.269876.
$$
The sample mean chest-beating rate is $2.22$ per $10$\hs, with a sample *standard deviation* of $1.27$ per $10$\hs.
:::
