...
Quantitative data can be collated in a *frequency table*\index{Frequency table!quantitative data} by grouping the variables into appropriate intervals ('bins').
The intervals should be *exhaustive* (cover all values) and *exclusive* (observations belong to one and only one category).
While not essential, usually the categories are of equal width.

For *discrete* quantitative data, bins are defined to contain single discrete values, or a small number of discrete values (Example\ \@ref(exm:CyclonesTable)).


:::{.example #CyclonesTable}
The data in
...
show the number of severe cyclones (also called hurricanes or typhoons) in the Australian region, for each year from 1969 to 2005.
...
The table gives the numbers and the correspoinding percentages.
:::
...
CC2tab[, 1] <- c(" $3$ or $4$ cyclones",
                ...
                " $9$ or $10$ cyclones",
                "$11$ cyclones")
colnames(CC2tab) <- c("Num. cyclones", 
                     "Num. of years",
                     "Percentage of years")
...
if( knitr::is_latex_output() ) {
  T1 <- kable( pad(CCtab,
        ...
        row.names = FALSE,
        col.names = c("cyclones",
                      "years",
                      "years")) %>%
    add_header_above( c("Num. of",
                        "Num. of",
                        "Percentage of"),
                      bold = TRUE,
                      line = FALSE) %>%
...
For *continuous* data, care is needed when creating frequency tables, since the data are rounded.
The bins should be defined to ensure no values lie on the border between bins (Example\ \@ref(exm:BabyBoomTable)).
...
give the weights of $44$ babies born in a hospital on one day [@mypapers:Dunn:dataset:1999; @data:Steele:BabyBoom], plus the gender of each baby, and the number of minutes after midnight of the birth.
...
The table also gives percentages in each bin; for example, the percentage of babies over $4.0$\ kg is $1/44 \times 100 = 2.27$%, or about $2$%. 
...
#    column_spec(3, width = "10mm")
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "The first ten observations (of $n = 44$) of the baby-births data: the number of babies born in a Brisbane (Australia) hospital on one specific day. The `birth time' is the number of minutes after midnight.") %>% 
    kable_styling(font_size = 8)
  out2 <- prepareSideBySideTable(out, 
                                 gap = "\\quad") 
  out2
...
if( knitr::is_latex_output() ) {
  kable( pad(BBtab,
        ...
        linesep = c("","","\\addlinespace"),
        row.names = FALSE,
        caption = "The baby-weights data, displayed in a frequency table. The first and last columns shown two different (but equivalent) ways to group the data.") %>%
	kable_styling(font_size = 8) %>%
	row_spec(0, bold = TRUE)
}
if( knitr::is_html_output() ) {
...
Often, discrete quantitative data with very few recorded values are better displayed using the graphs designed for qualitative data (Sect.\ \@ref(QualitativeGraphs)).

Graphs used to display the distribution of one quantitative variable include:

* *Histograms* (Sect.\ \@ref(Histograms)):
  best for moderate to large amounts of data.
* *Stemplots* (Sect.\ \@ref(StemAndLeafPlots)):
  best for small amounts of data; sometimes useful.
* *Dot charts* (Sect.\ \@ref(DotChartsOneVar)):
...
\index{Histogram}\index{Graphs!histogram}
...
The height of the histogram bars indicate the number (or percentage) in each category (often called 'bins').
A histograms is essentially a picture of a frequency table.
...
For example, the left histogram shows there were eight years where three severe cyclones were recorded. 

Notice that the different bin sizes change the appearance of the distribution.
:::
...
hist( Cyclones$Severe,
      ...
      main = "Number severe cyclones in Australian\nregion per year, from 1969 to 2005",
      las = 1)
axis(side = 2, 
     las = 1)
axis(side = 1,
...
One way to do this is to create boundaries between bins to one more decimal place than the given data (as in the final column of Table\ \@ref(tab:BabyBoomTable)).
...
An observation on a boundary between the bins may be placed it in the *higher* box (i.e., $2.5$\ kg is in the '$2.5$ to $3.0$\ kg' box, not the '$2.0$ to $2.5$\ kg' box): see the left panel.
...
```{r BBHistTwo, fig.align="center", fig.width=7.0, fig.height=2.15, out.width='100%', fig.cap="Histograms can be constructed in different ways to manage observations on the boundary of bins. Left: boundary values counted in the higher box. Centre: boundary values counted in the lower box. Right: defining boundaries with one more decimal places than the data may be clearer." }
par( mfrow = c(1, 3))
  ...
  hist( Wts,
        ...
        breaks = seq(1.5, 4.5, by = 0.5),
        col = plot.colour)
  title(sub = "Boundary values: in higher box",
        cex.sub = 1.2,
        font.sub = 2)
  ...
  hist( Wts,
        ...
        breaks = seq(1.5, 4.5, by = 0.5),
        col = plot.colour)
  title(sub = "Boundary values: in lower box",
        cex.sub = 1.2,
        font.sub = 2)
...
A histogram of the data (Fig.\ \@ref(fig:HistBrainFreeze)), shows $11$ people experience symptoms less than $5$\ s in length; nine people experienced symptoms for at least $5$ but less than $10$\ s; and $1$ person experienced symptoms for at least $35$\ s but under $40$\ s.
:::


```{r HistBrainFreeze, fig.align="center", fig.width=4.5, fig.height=2.75, out.width='45%', fig.cap="Histogram of the duration of brain-freeze symptoms after drinking ice water. Boundary observations are counted in the lower box." }
#par( mfrow = c(1, 2))
...
Software tries to use sensible default choices for the number of bins, and width of the bins.
...
::: {.example #BinWidthFaithful name="Bin width"}
A histogram for the time between eruptions [@hardle1991smoothing] of the *Old Faithful* geyser in Yellowstone National Park (USA)
...
```{r BBStem44,  fig.align="center", fig.cap="The completed stemplot for the baby-weight data.", fig.width=6, fig.height=3, out.width='60%' }
if (knitr::is_latex_output()){
  Wts <- sort(BB$Weight)
...
In stemplots, the original data remain visible.
For stemplots:
...
@wright2021chest recorded the number chest-beats by gorillas.
The stemplot 
`r if (knitr::is_latex_output()) {
...
for gorillas aged under $20$\ years shows a lot of variation in the chest-beating rate.
:::
...
```{r GorillasStem, fig.align="center", fig.width=7.25, fig.height=2.5, out.width='80%', fig.cap='The stemplot (right) for the gorilla chest-beating data (shown on the left).' }
if (knitr::is_latex_output()){
...
\index{Dot chart!one quantitative variable}

Dot charts show the data on a single (usually horizontal) axis, with each observation represented by a dot (or other symbol).
Sometimes, observations are identical, or nearly so, and to avoid points being plotted on top of other points (called *overplotting*),\index{Overplotting}
...
```{r FriesBabiesPDF, fig.cap="Left; a dot chart of the baby-weight data, where similar observations are jittered. Right: a dot chart of the gorilla chest-beating rates, where similar observations are stacked.", fig.align="center", fig.width=9, fig.height=2.15, out.width='95%'}
if (knitr::is_latex_output()){
...
Hence, after producing a graph, the *distribution* of the data should be described by noting:
...
The weights of babies (Example\ \@ref(exm:Histograms)) are typically between about $2.5$\ kg and $3$\ kg (the *average*), with most between $1.5$\ kg and $4.5$\ kg (*variation*).
A few babies have very low weights, probably premature births (*shape*).
...
The purpose of describing *sample* data is to understand the *population* that the research question is asking about.
Any numerical quantities that we compute (such as averages) are computed from a sample, even though the whole population is of interest.
...
A ***s**tatistic* is a number describing some feature of a **s**ample (to estimate the unknown value of the population *parameter*).
:::
...
The observed value of the statistic depends on which one of the countless possible samples is (randomly) selected.
...
The *shape* of a distribution can described using some terminology:

* *Right* (or *positively*) skewed: most data are small, with some larger values.\index{Shape!right skewed}\index{Shape!positively skewed}
* *Left* (or *negatively*) skewed: most data are large, with some smaller values.\index{Shape!left skewed}\index{Shape!negatively skewed}
* Symmetric data: \index{Shape!symmetric}
  the left and right sides of the graph are similar.
...
Sometimes, no short descriptions (like above) are suitable.
...
The histogram for the time between eruptions (Fig.\ \@ref(fig:BimodalFaithfulHistoChangeBins), top left panel) is bimodal, with peaks near $55$\ min and $80$\ min.

The weight of babies born oin Brisbane (Fig.\ \@ref(fig:BBHistTwo)) are slightly skewed left.
:::
...
* the *sample mean* (or *sample arithmetic mean*), which estimates the unknown population mean (Sect.\ \@ref(Mean)); and
...
* the *mean* daily streamflow, which is $1\ 123.2$\ ML.\spacex
* the *median* daily streamflow, which is $146.1$\ ML.\spacex

Both give an 'average value' of the same data, yet give *very*  different values.
This implies they measure the 'average' differently, with different meanings.
...
The mean of the population is denoted by $\mu$,\index{Mean!population} and its value is almost always unknown.
...
The unknown population mean is estimated using the sample mean ($\bar{x}$), and every possible sample could give a different value for $\bar{x}$.
...
Alternatively, the mean is the value such that the positive and negative distances (the 'deviatiobs') of the observations from the mean add to 
`r if (knitr::is_latex_output()) {
...
an *estimate* of the population mean (i.e., the sample mean) chest-beating rate is found by summing all $n = 14$ observations and dividing by $n = 14$:
$$
   \overline{x} 
   = \frac{\sum x}{n} 
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The sample mean is usually calculated using software (such as jamovi) for large amounts of data, or a calculator (in *Statistics Mode*) for small amounts of data.
...
The chest-beating data are given to one decimal place, so the *sample mean* rate can be given as $\bar{x} = 2.22$ beats per $10$\ hrs.
:::
...
The population mean distance is estimated by the sample mean as $\bar{x} = 532/11 = 48.4$\ cm.
:::
...
(The median is *not* equal to $(n + 1)\div 2$, and is *not* necessarily halfway between the minimum and maximum values in the data.)
...
The sample median, as estimate of the *population* median\index{Median!population}, is between $1.7$ (ordered observation\ $7$) and $1.7$ (ordered observation\ $8$).
...
Consider the daily streamflow at the Mary River (Bellbird Creek) during February again (Example\ \@ref(exm:Averages)): the *mean* daily streamflow is $1\ 123$\ ML, and the *median* daily streamflow is $146.1$\ ML.\spacex
Which is 'best' for measuring the average streamflow?

For these data, about $86$% of the observations are *less* than the mean, but $50$% of the observations are less than the median (by definition).
The mean is hardly a *central* value.

A dot chart of the daily streamflow (Fig.\ \@ref(fig:DailyStreamflow); jittered)\index{Overplotting!jittering} shows that the data are *very* highly right-skewed, with many *very* large outliers\index{Outliers} (presumably during flood events).


```{r DailyStreamflow, fig.cap="A dot plot of the daily streamflow at Mary River from 1960 to 2017, for February ($n = 1650$). Many large outliers exist, so most data near zero are overplotted. Note: values have been jittered in the vertical direction, but overplotting is still present near $0$.\\index{Overplotting}", fig.align="center", out.width='100%', fig.width=9, fig.height=3}
par(mfrow = c(1, 1),
    mar = c(5, 0.1, 4, 0.1) )
set.seed(183763286)
...
The streamflow data are *extremely* right skewed, which is important for choosing between the mean and the median:
...
For the Mary River data, the large outliers---and because they are so *extreme* and abundant---cause the mean to be so much larger than the median.
*The median is the better measure of average for these data*.
However, understanding the variation may be more important than understanding the average value (Sect.\ \@ref(Variation)), and may even be better described using percentiles (Sect.\ \@ref(VariationPercentiles)).
...
The range is not often used, as it only uses values which are extreme observations.
...
The population standard deviation is denoted by $\sigma$ (the parameter) and is estimated by the sample standard deviation $s$ (the statistic).
The standard deviation is the most commonly-used measure of variation, but is tedious to compute manually.
The sample standard deviation is usually calculated using software\index{Computers and software!statistical} for large amounts of data, or a calculator (in *Statistics Mode*) for small amounts of data.
...
* calculate the *deviation* of each observation $x$ from the sample mean: $x - \bar{x}$;
...
```{r ShowVar, fig.cap="The standard deviation is related to the sum of the squared-distances from the mean. The chest-beating data are being used.", fig.align="center",fig.width=8, fig.height=4.25, out.width='100%'}
source("R/showVar.R") 
showVar() 
```
...
The standard deviation uses the value of $\bar{x}$, so is affected by skewness just like the sample mean.
A measure of variation *not* affected by skewness\index{Skewness} is the inter-quartile range, or IQR.\spacex
...
Since each half has seven observations (an *odd* number), the median of each half is the $(7 + 1)/2 = 4$th value.
Hence:
...
```{r QuartilesYgorillas, fig.align="center", fig.width=8.75, fig.height=3.25, fig.cap="A dotchart (with jittering) for the chest-beating data for young gorillas, showing the IQR.", out.width='100%' }
...
*Percentiles* are similar in principle to quartiles.
In fact, quartiles are a special case of percentiles.
...
This means that the first quartile $Q_1$ is the $25$th percentile, the second quartile $Q_2$ is the $50$th percentile (and median), and the third quartile $Q_3$ is the $75$th percentile.
...
The median (50th percentile) is $146.1$\ ML.\spacex
...
Outliers are 'unusual' observations: those quite different than the bulk of the data (larger or smaller).
...
* *Mild outliers*: observations $1.5\times \text{IQR}$ more unusual than $Q_1$ or $Q_3$ (that are not extreme outliers).\index{Outliers!IQR rule!mild}
:::
...
After all, the outliers were obtained from your study like all other observations; they deserve to be in the data as much as any other observation.
...
* *The reason for the outlier is unknown*:
  ...
  Furthermore, very large datasets are expected to have observations identified as outliers using the above arbitrary rules. 
...
A large number of households are coordinated by women in their late $50$s.
...
```{r WaterAcessQuant, fig.align="center", fig.cap="The age of the women, the number of people in the household, and the number of children in the household aged under $5$ years of age, for the water-access study.", out.width='100%', fig.width=7, fig.height=2.25}
...
[*Dataset*: `NHANES`]
The histogram of the direct HDL cholesterol concentration from the American National Health and Nutrition Examination Survey (NHANES) [@data:NHANES:Rpackage] from $1999$--$2004$ is shown in Fig.\ \@ref(fig:NHANESCherryRipeHist) (left panel).
...
```{r NHANESCherryRipeHist, fig.cap="Left: the histogram of direct HDL cholesterol concentration from the NHANES study (large outliers exist but are hard to see, as the sample size is very large). Right: the weights of `Fun Size' Cherry Ripe chocolate bars.", fig.align="center", fig.width=8.75, fig.height=3.15, out.width='100%'}
par(mfrow = c(1, 2))
...
[*Dataset*: `FriesWt`]
@data:Wetzel2005:McDonalds weighed orders of French fries to determine how they matched the target weight of $171$\ g (Table\ \@ref(tab:FriesWtTable)).
...
1. Compute the sample mean, sample median, sample standard deviation and sample IQR for the heights.
1. What is the value of the population mean, population median, population standard deviation and population IQR for the heights?
...
2. What are the values of the sample mean and sample median number of microplastics found?
3. What are the values of the population mean and population median number of microplastics found?
...
```{r TwoDatasets, fig.height=1.75, fig.width=7.75, out.width='90%', fig.cap="Dotplots of two datasets.", fig.align="center"}
...
1. What proportion of jeans in the sample have a minimum height less than $13.25$\ cm, for men's and women's jeans?
:::


```{r JeansBoxplot, echo=FALSE, fig.cap="Left: the minimum height of the height of front pockets in jeans. Right: the price of different styles of women's jeans.", fig.align="center", fig.width=7.95, fig.height=3, out.width="100%"}
data(Jeans)
...
The corresponding histograms are shown in Fig.\ \@ref(fig:FourHistograms).
...
```{r FourHistograms, echo=FALSE, fig.cap="Four histograms: where would they be useful?", fig.align="center", out.width='55%', fig.width=5.5, fig.height=4.25, out.width="80%"}
