  ...
  for (i in (1:NumPlots)){
    ...
    if (i >= 7 ) {  # DATA + LINE + x1 + y1 + x2 + y2 + RUN
      arrows(x1, 2, x2, 2,
             ...
             col="darkgrey",
             code=3)
      text( mean( c(x1, x2)), 2, paste("Run: ", x2," - ", x1, " = ", x2 - x1, sep=""), pos=3 )  
    }
    
    if (i >= 8 ){ # DATA + LINE + x1 + y1 + x2 + y2 + RUN + RISE
      arrows(0.5, y1, 0.5, y2,
             lwd=2,
             col="darkgrey",
             code=3)
      text( 0.5, mean( c(y1, y2)), paste("Rise: ", y2," - ", y1, " = ", y2-y1, sep=""), pos=4 )  
    }
    
    
    if (i >= 9 ) {  # DATA + LINE + x1 + y1 + x2 + y2 + RUN + RISE + SLOPE
      polygon( c(2.15, 2.15, 4.85, 4.85), c(6, 8, 8, 6), 
               border=NA, # No border
               col="antiquewhite1")
      text(3.5, 7, paste("Slope = Rise/Run = ", y2-y1,"/",x2-x1, " = ", (y2-y1)/(x2-x1)) )
    }
  }
}
```
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Estimating the slope", src = "./Animations/RiseRunMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->
...
<!-- ```{r RiseRun, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Making a guess of the slope, using rise-over-run" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RiseRun9") -->
<!-- } -->
<!-- ``` -->
...
clm <- lm( Total ~ OND, data=cyclones)
abline( coef(clm), lwd=1, col="grey")
grid(lty=2)
```
...
```{r echo=FALSE}
cyclone.lm <- lm(Total ~ OND, data=cyclones)
```
...
<!-- Check that you can find the correct $t$-score (for the slope). -->

<!-- <iframe src="https://h5p.org/h5p/embed/498898" width="1090" height="456" frameborder="0" allowfullscreen="allowfullscreen"></iframe><script src="https://h5p.org/sites/all/modules/h5p/library/js/h5p-resizer.js" charset="UTF-8"></script> -->
<!-- ``` -->
...
that the slope in the population between age of the deer and molar weight
is not zero.
...
if( knitr::is_html_output() ) {
  DT::datatable(ruminant,
              list(scrollX=TRUE, scrollY=TRUE, ordering=FALSE),
...
abline( coef( Punt.lm <- lm( Punt ~ Right, data=punting)), lwd=2, col="grey")
```
...
TAlm <- lm(MAPvalue ~ ThrottleAngle, data=TA)
abline(coef(TAlm), lwd=2, col="grey")
```
...
plot(AirVoids ~ Bitumen, data=Bit,
     pch=19,
