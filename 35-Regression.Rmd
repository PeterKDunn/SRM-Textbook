...
$$
   \hat{y} = 7x + 73.
$$
In bith cases, $b_0 = 73$ and $b_1 = 7$.

The regression equation canbe used to *predict* the *mean* height for any given age\ $x$.
...
* a standard deviation of $\text{s.e.}(b_1) = 0.029$ (from software; Fig.\ \@ref(fig:RedDeerRegressionjamoviSPSSraw)).
...
Software can be used to produce *exact* CIs too (Fig.\ \@ref(fig:RedDeerRegressionjamoviSPSSCI)); the exact $95$% CI is from\ $-0.239$ to\ $-0.124$\gs/year.
...
$$
From the jamovi output (Fig.\ \@ref(fig:CyclonesRegressionjamoviSPSS)), $t = -5.52$ and the $P$-value is small: $P < 0.001$.
We write:
...
1. What are the measurements units for the slope?
`r if( knitr::is_html_output() ) {mcq( c(
  "cm",
  "kg",
  answer = "kg per cm",
  "cm per kg" ))}`
1. What are the measurements units for the intercept?
`r if( knitr::is_html_output() ) {mcq( c(
  "cm",
  answer = "kg",
  "kg per cm",
