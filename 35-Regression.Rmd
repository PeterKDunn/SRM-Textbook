...
1. What mean weight is predicted for a rower who is $180$\cms tall?  
`r if( knitr::is_html_output() ) {
...
`r if( knitr::is_html_output() ) {torf( answer = TRUE)}`
1. True or false: Predicting the mean weight of a $220$\cms-tall rower would be *extrapolation*.
:::
...
`
all of the data are plotted in Fig.\ \@ref(fig:OSAscatter) (left panel).
...
```{r BitumenPlotReg, fig.cap="Air voids in bitumen samples.", fig.align="center", fig.width=4, fig.height=3.35, out.width=c("45%", "50%"), fig.show="hold"}
...
Some of the data collected to answer this RQ are shown
`r if (knitr::is_latex_output()) {
