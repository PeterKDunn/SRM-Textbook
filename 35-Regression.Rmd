...
1. Based on the scatterplot,\tightlist what variable is considered the $y$-variable?  
`r if( knitr::is_html_output() ) {
   longmcq( c(
...
1. Using the 'rise-over-run' idea, what is a rough estimate of the value of the slope?  
...
`r if( knitr::is_html_output() ) {torf( answer = TRUE)}`
1. True or false: Predicting the weight of a $220$\cms-tall rower would be *extrapolation*.
:::
...
Use the software output (Fig.\ \@ref(fig:GorillajamoviHT)) to find the regression line, and perform a hypothesis test for the slope.
Write a conclusion.
...
Use the software output (Fig.\ \@ref(fig:BitumenPlotReg), right panel) to answer these questions.
...
1. Write down the regression equation.
1. Interpret what the regression equation means.
1. Perform a test to determine if there is a relationship between the possum weight and the elevation.
...
2. What else could influence the price of a second-hand Corollas besides the age? 
