...
```{r, child = if (knitr::is_html_output()) {'./introductions/35-Regression-HTML.Rmd'} else {'./introductions/35-Regression-LaTeX.Rmd'}}
```
...
Equation\ \@ref(eq:ExampleRegressionEqn) can be used to make predictions.
For instance, suppose $x = 7$; then the equation gives
$$
  \hat{y} = -4 + (2\times 7) = -4 + 14 = 10.
$$
This means that, when the value of\ $x$ is\ $7$, the observed values of\ $y$ will vary around a predicted mean of $y = 10$.
...
$$
This means that the heights of girls will vary around a mean height of\ $108$\cms; some individual girls aged five will be taller than\ $108$\cms, and some will be shorter than\ $108$\cms.
:::
...
When $x = 0$, the regression line suggests the value of\ $\hat{y}$ is about\ $2$; that is, the value of\ $b_0$ is approximately\ $2$.
...
The corresponding values of\ $y$ change from\ $5$ to\ $17$, a change (specifically, an *increase*) of $17 - 5 = 12$.
So:
$$
   \frac{\text{rise}}{\text{run}} 
   = \frac{17 - 5}{5 - 1}
...
For the red-deer data again (Fig.\ \@ref(fig:RedDeerScatter2)), part of the relevant software output is shown in Fig.\ \@ref(fig:RedDeerRegressionjamoviSPSSraw).

In the output, the values of\ $b_0$ and\ $b_1$ are in the column labelled `Estimate`; the value of the sample *slope* is $b_1 = `r RD.b1`$, and the value of the sample $y$-intercept is $b_0 = `r RD.b0`$.
These are the values of the two *regression coefficients*.\index{Regression!regression coefficients}
...
The regression equation for the red-deer data in Eq.\ \@ref(eq:RedDeerRegressionEqn) is estimated from one of the many possible samples.
This regression equation can be used to make *predictions* of the mean value of\ $y$ for a given value of\ $x$.
...
But in any case, this predicted mean may be nonsense: it is *extrapolating*\index{Extrapolation} beyond the data (the youngest deer in the sample is aged $4.4$\ years).
...
This interpretation of $b_1$ explains the relationship: each extra year of age is associated with the weight of the molars reducing by $0.181$\gs, on average, in male red deer.
...
In the *population*, the intercept is denoted by\ $\beta_0$ and the slope by\ $\beta_1$; in the population, the regression line is
$$
  \hat{y} = \beta_0 + \beta_x.
$$
The values of the paremeters\ $\beta_0$ and\ $\beta_1$ are unknown, and are estimated by the statistics\ $b_0$ and\ $b_1$ respectively.
...
Under certain conditions (Sect.\ \@ref(RegressionStatValidity)), the sampling distributions for\ $b_0$ and\ $b_1$ both have normal distributions.
...
Because of this, forming confidence intervals for\ $\beta_1$ is more common than for\ $\beta_0$, though the ideas below apply similarly to $\beta_0$.
For this reason, we give the sampling distribution for the slope only, but the sampling distribution for the intercept is analogous.
...
Using the standard error reported by software (Fig.\ \@ref(fig;RedDeerRegressionjamoviSPSSCI)), an approximate $95$%\ CI is $-0.181 \pm (2\times 0.029)$, or $-0.181 \pm 0.058$: from\ $-0.239$ to\ $-0.123$\gs/year.

Software can be used to produce *exact* CIs too (Fig.\ \@ref(fig:RedDeerRegressionjamoviSPSSCI)): $-0.239$ to\ $-0.124$\gs/year.
...
## Hypothesis testing for the regression parameters: $t$-tests {#RegressionHT}
...
Assuming the null hypothesis (that $\beta_1 = 0$) is true, the possible values of the sample slope $b_1$ can be described (Def.\ \@ref(def:Beta1SamplingDistn)).
...
\index{Statistical validity!regression parameters}
...
Other options are available if the statistical validity conditions are not met, depending on which conditions are not met.
For example, generalized linear models [@mypapers:dunnsmyth:glms] may be appropriate for non-linear relationships where the variation in the response variable is not constant.
...
1. What \tightlist is the $y$-variable, according to how the scatterplot is constructed?  
`r if( knitr::is_html_output() ) {
   longmcq( c(
     answer = "The weight of the rower",
     "The height of the rower"
) )
} else {
  '**a.**\\ The weight of the rower; **b.**\\ The height of the rower'
}`
...
1. What weight is predicted for a rower who is $180$\cms tall?  
`r if( knitr::is_html_output() ) {
...
1. The standard error of the slope is $0.112$. 
   What is the value of the *test statistic* (to two decimal places) to test if the population slope is zero?
`r if( knitr::is_html_output() ) {fitb( answer = 10.7, num=TRUE, tol=0.05 )}`
1. True or false: The $P$-value for this test will be *very small*.
`r if( knitr::is_html_output() ) {torf( answer = TRUE)}`
1. True or false: The *units* of the slope are kg/cm.
`r if( knitr::is_html_output() ) {torf( answer = TRUE)}`
1. True or false: Making a prediction for the weight of a rower weighing $220$\kgs would be an example of *extrapolation*.
`r if( knitr::is_html_output() ) {torf( answer = TRUE)}`
:::
...
`
and plotted in Fig.\ \@ref(fig:OSAscatter) (left panel).

1. Using the software output (Fig.\ \@ref(fig:OSAscatter), right panel), determine the value of $r$.
1. Compute the value of\ $R^2$, and interpret what this means.
1. Using the software output, write down the values of the intercept and the slope, and hence the regression equation.
...
```{r EDScatterjamovi, fig.align = "center", fig.cap = "The number of emergency department patients, and the number of days since distribution of welfare. Left: scatterplot. Right: software output.", fig.width=3.35, fig.height=3.5, out.width=c("38%", "61%"), fig.show="hold"}
...
[*Dataset*: `Gorillas`]
(This study was seen in Exercise\ \@ref(exr:TwoQuantExercisesGorillas).)
...
Use the software output (Fig.\ \@ref(fig:GorillajamoviHT)) to find the regression line, and perform a hypothesis tests for the slope.
Write a conclusion.
...
1. Predict the mean weight of male possums at an elevation of\ $1000$\ms.
...
[*Dataset*: `Soils`]
(This study was seen in Exercise\ \@ref(exr:CorrelationExerciseSoil).)
...
1. Predict the birth weight of an infant with a chest circumference of\ $30$\cms.
:::
...
1. What is the value of the correlation coefficient?
   Explain.
1. Use the output to write down the regression equation.
1. Interpret the meaning of the intercept and slope.
   Are these both sensible?
   Explain.
1. Conduct a test to determine if a relationship exists between the 6MWT and age in the population.
1. Construct an approximate $95$%\ CI for the slope.
1. Are the test and CI statistically valid?
1. What is the predicted 6MWT distance for a patient who is $60$-years-old?
...
(an Australian online market place), for `Toyota Corolla` in the 'Cars, Vans \& Utes' category.
...
I then produced the scatterplot in Fig.\ \@ref(fig:CorollasPriceAgeYear) (left panel).
...
```{r CorollasPriceAgeYear, echo=FALSE, fig.cap="(ref:CorollasScatter)", fig.align="center", fig.width=9, fig.height=4, out.width='100%'}
data(Corollas)
