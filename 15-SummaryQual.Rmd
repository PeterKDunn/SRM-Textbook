# Summarising qualitative data {#CompareQuantData}
...
However, the ways in which qualitative data are summarised is different than with quantitative data.
...
if (knitr::is_html_output()) {
  ...
  knitr::kable(surroundMaths(AVtable1),
               format = "latex",
               longtable = FALSE,
               escape = FALSE,
               align = c("r", "c", "c", "c"),
...
```{r, AVDotBarPie, fig.align='center', fig.cap="The age group of respondents in the AV study. All the graphs present the same data.", out.width='100%', fig.width=9, fig.height=6.25, fig.show='hold'}
...
The equivalent bar chart makes the comparison easy: clearly the youngest age group has the smallest representation, while the\ $25$ to\ $34$ and the\ $35$ to\ $44$ age groups have the most respondents.
...
## Numerical summaries for quantitative data {#QualitativeNumbers}
...
* The *median* age groups is $35$ to\ $44$ (observations $200$ and $201$ fall into this level).
* The *median* response to the question about driving near AVs is 'Neutral'  (observations $200$ and $201$ fall into this level).
* The *median* response to the question about cycling near AVs is 'Neutral'  (observations $200$ and $201$ fall into this level).
* The *median* response to the question about walking near AVs is 'Neutral'  (observations $200$ and $201$ fall into this level).
:::
...
* The *mean* response to the question about driving near AVs is $3.11$ (slightly safer than 'Neutral').
* The *mean* response to the question about cycling near AVs is $2.83$ (slighty unsafer than 'Neutral').
* The *mean* response to the question about walking near AVs is $3.01$ (very close to 'Neutral').
:::


## Example: student transport to campus
...
The mode means of transport is by bus. 
Medians are not appropriate.
...
The methods of transport are displayed in Fig.\ \@ref(fig:TransportPlots).
The colours in the two plots are related:  white refers to active methods of transport.
The left panel shows the specific methods, and the right panel shows the methods of transport grouped more coarsely.

The *odds* of a students getting to campus using motorised transport is $220 / (284 - 220) = 3.44$; that is, students are $3.44$ times as likely to use motorised transport than to use active transport.
In other words, for every $100$ students that *use active transport*, about $100\times 3.44 = 344$ use motorised transport.

Similarly, the odds that a student walks to campus is $35/(284 - 35) = 0.141$; that is, for every $100$ students that *do not walk* to campus, about $100\times 0.141 = 14.1$ *do walk* to campus.


```{r TransportPlots, fig.align="center", fig.cap="Methods of transport to campus. White refers to active methods of transport; shaded to motorised methods of transport.", out.width='100%', fig.width=8, fig.height=3}
par( mfrow = c(1, 2) )
...
1. Odds compute the ratio of how often an event occurs, to how often it does *not* occur.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
...
A study of spider monkeys [@data:Chapman1990:SpiderMonkeys] examined the types of social groups present (Table\ \@ref(tab:SpiderMonkeysData)).
...
SpiderMonkeyTable <- array( dim = c(7, 2) )
SpiderMonkeyTable[, 2] <- c(8, 3, 2, 15, 1, 23, 48)
