...
ENS[3, ] <- c( paste("\\llap{OR:}", ENS[3, 1]),
               paste("\\llap{Diff.:}", ENS[3, 2]),
               NA)
if( knitr::is_latex_output() ) {
  kable(pad(ENS,
            surroundMaths = TRUE,
...
The table can also include the *difference* between the proportions (or percentages*).
For Table\ \@ref(tab:EatingNumericalSummary), the difference between the two percentages is  
\[
  \hat{p}_P - \hat{p}_N = 0.166 - 0.038 =  0.128,
\]
or $12.8$%.
**HOW TO ADD THIS TO SUMMARY TABLE? OR JUST REMOVE IT?**
...
Each sample of students will comprise different students, giving different proportions and odds. 
...
* with a standard deviation, called the *standard error* of the difference, of $\text{s.e.}(\hat{p}_P - \hat{p}_N) = XXXXXX$.
...
\[
  ...
  = \sqrt{ AAA^2 + BBB^2 } = XXXX,
\]
as in the jamovi output (Fig.\ XXXXX).
...
\[
	0.028 \pm (2 \times XXX),
\]
or $0.028 \pm XXX$ after rounding appropriately.
We write:

> The difference between YYYYY

The plausible values for the difference between the two population proportions are between $11111$ to $2222222$ milliseconds (DIRECTION).
...
A CI can be formed for the difference between the two proportions (as above), and a CI can also be formed for teh odds ratio.
