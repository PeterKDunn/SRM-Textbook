...
# CIs for comparing proportions {#OddsRatiosCI}
...
Another sensible parameter would be the difference between the *proportions* (or *percentages*) in each group, but the odds ratio usually is used as the parameter^[This is *optional*: The difference between two proportions is always a number between $-1$ and $1$, which makes the sampling distribution hard to describe.].
...
From Table\ \@ref(tab:EatingNumericalSummary), define the sample proportion of students eating most meals off-campus as $\hat{p}$, and write $\hat{p}_P$ for the proportion living with parents, and $\hat{p}_N$ for the proportion *not* living with parents.
Then, the difference between the two percentages is  
\[
...
So the *odds ratio* (OR) of eating most meals *off-campus* (the *first* column), comparing students living with parents to students *not* living with parents (second column), is $26 \div 4.375 = 5.943$.
...
ENS[3, ] <- c( paste0("\\llap{OR:\ }$", ENS[3, 1], "$"),
...
```{r EatingGraphsCI, fig.cap="The uni-student eating data. Left: a side-by-side bar chart. Right: the jamovi output for computing a CI for the difference between the proportions, and the odds ratio", fig.align="center", fig.height=3.75, fig.width=4, out.width=c("45%", "53%"), fig.show='hold'}
par( xpd = TRUE,
...
Each sample of students will comprise different students, giving different proportions and odds for both groups (living with, and not living with, parents).
Hence, the difference between the two proportions and the odds ratio will differ between samples.


## CIs for the difference between two proportions

### Sampling distribution for difference between proportions {#DiffPropSamplingDist}

The sample proportions for each group will differ from sample to sample, and the *difference* between the sample proportions will be different for each sample.
The *difference* between the sample proportions varies from sample to sample, and so has a sampling distribution and *standard error*.
...
For the student-eating data, define the sample proportion of student eating most meals off-campus as $\hat{p}$, and write $\hat{p}_P$ for the proportion living with parents, and $\hat{p}_N$ for the proportion *not* living with parents.
Then, the differences between the sample proportions will have:
...
\]
where the individual standard error for each group are computed using Eq. \@ref(eq:stderrorphat).
**CHECK** Seems wrong



### Confidence interval for difference between proportions {#DiffPropCI}
...
\[
	0.149 \pm (2 \times 0.042830),
\]
or $0.149 \pm 0.08566$ after rounding appropriately (i.e., from $0.0633$ to $0.2347$. **CHECK: NOT SAME AS OUTPUT: cont correction? Nor same as R's `prop.test()` either...?**
We write:

> The difference between the proportions of students eating most meals at home, comparig those living with and not with their parents, is 0.149 ($95$% CI: $0.063$ to $0.235$). **AND FLESH OUT**
...
A CI can be formed for the difference between the two proportions (as above), and a CI can also be formed for the odds ratio.
...
** ALSO DIFF BETWEEN TWO PROPS; FIX**

> The difference between ... comparing natural nests (p: $??$; $n = 39$) to relocated nests (p: $???$; $n = 22$), is $10.72$% with a $95$%\ CI from $0.537$ to $5.12$... HIGHER FOR
