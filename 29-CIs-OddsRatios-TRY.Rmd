...
# CIs for comparing qualitative variables {#OddsRatiosCI}
...
However, software commonly compares *rows*, so it makes sense to place the groups to be compared in the rows of the table.
:::
...
Alternatively, and equivalently, the odds of students who eat most meals off-campus can be compared between those who live with their parents and those who do *not* live with their parents.
...
Then, the *difference* between the two proportions is  
\[
  \hat{p}_P - \hat{p}_N = 0.9630 - 0.8140 =  0.1490,
\]
or $14.9$% (as in the software output: Fig.\ \@ref(fig:EatingGraphsCI)).
By this definition, the difference refers to how much greater the proportion eating most meals off-campus is, compared to students living *with* their parents.
...
* $52 \div  2 = 26$    for students     *living with their parents* (Row\ 1).
* $105\div 24 = 4.375$ for students *not living with their parents* (Row\ 2).
...
An appropriate graph (Fig.\ \@ref(fig:EatingGraphsCI), left panel) is a side-by-side\index{Graphs!side-by-side barchart} or stacked bar chart.\index{Graphs!stacked bar chart}
...
```{r EatingGraphsCI, fig.cap="The student-eating data. Left: a side-by-side bar chart. Right: the jamovi output for computing a CI for the difference between the proportions, and for the odds ratio", fig.align="center", fig.height=3.75, fig.width=4, out.width=c("43%", "55%"), fig.show='hold'}
par( xpd = TRUE,
...
Each sample of students will comprise different students, giving different proportions and odds of having most meals off-campus for both groups (living with, and not living with, parents).
...
The *sampling distribution of the difference between two sample proportions* $\hat{p}_A$ and $\hat{p}_B$ is (when the appropriate conditions are met; Sect.\ \@ref(ValidityTwoProportions)) described by:
...
The standard error of the sample proportions for each group are computed using Eq.\ \@ref(eq:stderrorphat) as  
\[
  \text{s.e.}(\hat{p}_L) = 0.025700\qquad\text{and}\qquad\text{s.e.}(\hat{p}_N) = 0.034262.
\]
...
Then, finding a $95$%\ CI for the difference between the proportions is similar to the process used previously, since almost all CIs have the same form:  
\[
...
\[
	(\hat{p}_P - \hat{p}_N) \pm (2 \times \text{s.e.}(\hat{p}_P - \hat{p}_N)).
\]
So, in this case, the approximate $95$%\ CI is  
\[
	0.1490 \pm (2 \times 0.042830),
\]
or $0.149 \pm 0.08566$ after rounding appropriately (i.e., from $0.0633$ to $0.235$).
This approximate CI is very similar to the (exact) CI shown in Fig.\ \@ref(fig:EatingGraphsCI).
We write:

> The difference between the proportions of students eating most meals at home is between $6.33$% and $23.5$, higher for those living with their parents ($96.3$%; $n = 52$) that those not living with their parents ($81.4$%; $n = 129$).

The plausible values for the difference between the two population proportions are between $6.3$% to $23.5$%, larger for those living with parents.
...
While a CI can be formed for the difference between the two proportions, a CI can also be formed for the odds ratio.
...
Using jamovi\index{jamovi output!Comparing two odds}\index{Confidence intervals!odds ratio} to find the OR (Fig.\ \@ref(fig:EatingGraphsCI), right panel), the sample OR is $5.94$, and the (exact) $95$%\ CI is from $1.35$ to $26.1$.
The OR is the same as computed manually, since the OR was computed comparin Row\ 1 to Row\ 2, and Column\ 1 to Column\ 2, as used by the software.
...
Note that this condition is based on the *expected* counts, not the *observed* counts.
...
For the students-eating data, jamovi can be used to compute the *expected* counts (Fig.\ \@ref(fig:UniMealsTestOutputExpectedOnlyjamovi))\index{jamovi output!Expected counts}
...
```{r UniMealsTestOutputExpectedOnlyjamovi, fig.show="hold", fig.cap="The expected counts in jamovi, for the student-eating data", fig.align="center", out.width="70%"}
...
> For Mediterranean loggerhead turtles, what is the difference between the prooportion of infected nests, comparing natural to relocated nests?
...
A graphical summary (Fig.\ \@ref(fig:TurtleNestsGraphs)) and a numerical summary table (Table\ \@ref(tab:TurtleNestsNumericalSummary)).
From the jamovi output (Fig.\ \@ref(fig:TurtleNestsOutputjamovi)), the $95$%\ CI for the odds ratio is from $0.537$ to $5.12$.
Write:
...
In addition, the $95$% CI for the difference between the proportions is from $-0.1361$ to $0.3505$%.
This negative value is *not* a negative proportion; it is a negative *difference* between two proportions.
Specifically, it means that the population proportion of infected nests is larger for relocated nests than natural nests by $0.1361$ (i.e., corresponding to where the odds ratio will be less than one).
Write:
...
\]
where $\text{s.e.}(\hat{p}_A)$ and $\text{s.e.}(\hat{p}_B)$ are the standard errors of Groups\ $A$ and\ $B$.
...
<!-- The figure for LaTeX is in the minipage (combined with data table), so only need show it for the HTML -->
`r if (knitr::is_latex_output()) '<!--'`
```{r CarCrashjamovi, fig.show="hold", fig.cap="jamovi output for the car-crash data", fig.align="center", out.width="50%"}
knitr::include_graphics( "jamovi/CarCrashes/CarCrashes-ORpDiffCI.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
15. Determine if the CI is statistically valid.
:::
...
1. Interpret the CI.
1. Use the jamovi output (Fig.\ \@ref(fig:EarInfPSSOutputCI)) to write down a $95$%\ CI for the odds ratio.
1. Interpret the CI.
1. Is the CI statistically valid (Fig.\ \@ref(fig:EarInfPSSOutputCI))?
:::



```{r EarInfPSSOutputCI, fig.show="hold", fig.cap="jamovi output for the ear-infection data", fig.align="center", out.width=c("42%", "53%"), fig.show='hold'}
...
1. Write down the $95$% CI for the difference between the proportions.
1. Interpret the CI.
1. Use the jamovi output (Fig.\ \@ref(fig:TurbinesOutputCI)) to write down a $95$%\ CI for the odds ratio.
1. Interpret the CI.
...
SOI.tab[1, 1] <- SOI.tab[2, 2]
SOI.tab[2, 2] <- SOI.tab[1, 1]
SOI.tab[1, 2] <- SOI.tab[2, 1]
SOI.tab[2, 1] <- SOI.tab[1, 2]
...
1. Find a $95$%\ CI for the difference between the proportion of wet Augusts, comparing those with positive SOI to those with a non-positive SOI.
1. Find a $95$%\ CI for the OR.
1. Carefully explain what these CIs means.
...
The jamovi output is shown in Fig.\ \@ref(fig:SunglassesSPSSOutputCI). 

1. Find a $95$%\ CI for the difference between the proportion *not* wearing hats, comparing females to males.
1. Find a $95$%\ CI for the OR.
1. Carefully explain what these CIs means.
1. Construct the summary table for the data.
:::
...
```{r SunglassesSPSSOutputCI, fig.show="hold", fig.cap="jamovi output for the hats data", fig.align="center", out.width=c("40%", "56%"), fig.show='hold'}
...
1. Use the software output to find a $95$%\ CI, making sure to describe the odds ratio carefully.
...
```{r B12SPSSOutputCIjamovi, fig.show="hold", fig.cap="jamovi output for the B12 data. Left: the OR, difference between proportions, and confidence interval. Right: expected counts", fig.align="center", out.width=c("48%","51%"), fig.show='hold'}
