...
A sample is then taken, and the value of the sample statistics computed. 
...
In Chap.\ \@ref(SamplingVariation), we saw that the proportion of red spins in 15 spins could vary; similarly, the mean spin from 15 spins could vary (Fig.\ \@ref(fig:RouletteWheelHistPropMean)).
In both cases---and also for larger sample sizes---these sampling distributions had a rough bell-shape, or a *normal distribution*.


```{r RouletteWheelHistPropMean, results='hide', fig.width=8.5, fig.height=3, fig.cap="Sampling distributions for the proportion of red spins (left), and the mean of the numbers after a roulette wheel spins (right)", fig.align="center", out.width="100%"}
p <- 18/37
...
Many variables have approximate normal distributions, such as heights of people.
Many sampling distributions, as in Fig.\ \@ref(fig:RouletteWheelHistPropMean), also have approximate normal distributions.
...
```{r HistogramDBPPossums, fig.align="center", fig.height = 3, fig.width=8, out.width='95%', fig.cap="Two normal distributions: Diastolic blood pressure of $398$ Americans (left); the weight of $83$ male Leadbeater possums (right)"}
par( mfrow = c(1, 2))
data(Diabetes)
...
out <- hist( Possums$Wgt[Possums$Sex=="Male"],
      ...
      xlim = c(100, 170),
      ylim = c(0, 35),
      main = "Weight of male Leadbeater possums")
x <- seq(100, 170, 
         length = 200)
y <- dnorm(x,
...
Normal distributions represent the theoretical distributions of unknown populations.
Any specific sample of data may have variables with *approximate* normal distributions, but normal distributions represent theoretical population distributions.
...
**NOT x-bar and s.**
the values of $\mu$ and $\sigma$ are unknown. 
These are not sample values, but may be guided by sample values (eg silly to suggest a mode for possume weigt as 1000g based on the sample).
...
The normal distribution shown for the diastolic blood pressure (left panel) are so far from $0$ that the XXX XXXXXXX XXXXXXX XXXXXXXX XXX

Similarly, the normal distribution shown for the weights of the possums are so far from $0$ that negative weights are YYY YYYYYY YYYYY YY
...
Again, drawing the situation is helpful (Fig.
\@ref(fig:HtsExer1), right panel).
