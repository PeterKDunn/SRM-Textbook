...
The rows (or the columns) should list the *levels* of the variable (Sect.\ \@ref(def:QualData)), and these should be *exhaustive* (cover all levels) and *exclusive* (observations belong to only one level).
...
### Parameters and statistics {#Statistic2s}
...
Ordinal qualitative data, but *not* nominal qualitative data, can be summarised using *medians*, by finding the location of the response that is in the middle, when the levels from all individuals are placed in order.
...
* The *mean* response to the question about cycling is $2.83$ (slightly unsafer than 'Neutral').
