...
We do not know beforehand what distance your next throw will be, but the *sample space* (the throwing distance) could be anywhere between (say) $0$ and $100$ metres (and some of those distances are very unlikely to occur...).
...
Events are commonly combined using **and** and **or**.
...
* The *percentage* of rolls that expected to be even is $3 \div 6 \times 100 = 50$%.
...
1. Suppose I roll a standard six-sided die. \tightlist
   What is the *probability* that I will roll a number larger than `r include_graphics("Dice/die2.png", dpi=1250)`?
`r if( knitr::is_html_output() ) {mcq( c(
	"0",
	"1/6",
	"2/6",
...
2. Suppose I roll a standard six-sided die. 
   What are the *odds* of rolling a number smaller than `r include_graphics("Dice/die6.png", dpi=1250)`?
`r if( knitr::is_html_output() ) {mcq( c(
	"0",
	"1/6",
	"1/5",
