...
The parameter is\ $\mu_d$, the population mean *reduction* in %N from non-irrigated to irrigated sites; that is, non-irrigated\ %N minus irrigated\ %N.
...
Pairing does not require the *same* unit of analysis to be measured twice, as shown in this study.
Pairing i a good design strategy when the individuals in the pair are very similar for many extraneous variables.
...
The *null* hypothesis is (as always) that 'there is *no mean change* in %N, in the population' (Sect.\ \@ref(AboutHypotheses)):
...
Then, hypotheses are (where $\mu_d$ represent the mean change in TG (in\ mg.dl^$-1$^) after eight weeks):
