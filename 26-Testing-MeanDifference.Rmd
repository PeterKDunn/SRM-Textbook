...
# Tests: mean difference (paired data) {#TestPairedMeans}
\index{Research question!relational}


<!-- Introductions; easier to separate by format -->
```{r, child = if (knitr::is_html_output()) {'./introductions/26-Testing-MeanDifference-HTML.Rmd'} else {'./introductions/26-Testing-MeanDifference-LaTeX.Rmd'}}
```
...
> The irrigated and non-irrigated pairs within each site were within\ $100$\ms of each other and were on the same soil, landform and usually the same farm with the same farm management on both treatments.

Pairing (Sect.\ \@ref(PairedIntro)) is a form of blocking\index{Blocking} (Sect.\ \@ref(ManagingConfounding)) and is used to manage confounding.\index{Confounding!blocking}
The comparison is *within individuals*.
...
The parameter is\ $\mu_d$, the population mean *reduction* in %N from non-irrigated to irrigated sites; that is, the difference is computed as non-irrigated\ %N minus irrigated\ %N.
...
```{r, SoilNGraphs, fig.align="center", fig.cap="The reduction in \\%N when sites are non-irrigated, compared to irrigated (i.e., non-irrigated minus irrigated). Left: a histogram of the differences (boundary values in the lower box). Right: a case-profile plot (solid lines, solid dots for lower percentage N in irrigated sites).", fig.show="hold", out.width='100%', fig.width = 9.25, fig.height = 3.5}
par( mfrow = c(1, 2))
...
are *paired*.
The RQ is a special case of a *repeated-measures RQs* (Sect.\ \@ref(RQsRepeatedMeasures)),\index{Research question!repeated-measures} where each unit of analysis has two observations.
Computing the *differences* or *changes* between the pairs of observations makes sense, since the values for each pair belong to the same unit of analysis (the same site, in this case).
...
For example, each pair of sites recorded in
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:SoilCN)'
} else {
   'the data above'
}`
experience similar climatic condition, have similar soil, and most have the same farm management.
Pairing is a form of blocking\index{Blocking} (Sect.\ \@ref(ManagingConfounding)).
Pairing is a good design strategy when the individuals in the pair are very similar for many extraneous variables.
(For example, the pair may comprise two different people, of the same sex, with similar age, height and weight.)
Pairing sometimes involves taking two measurements from the same individuals (see Sect.\ \@ref(PairedIntro)).
...
The RQ for the soil nitrogen study asks if the mean\ %N *reduces* in the *population* when sites are irrigated.
The *parameter* is the *population mean difference* in %N, $\mu_d$. 

If the differences are defined as the %N for *non-irrigated* sites minus *irrigated* sites, then positive values will occur when non-irrigated sites have a larger\ %N than irrigated sites. 
With this definition of the differences, then, the RQ is asking if the mean difference is zero, or *greater* than zero.
...
The differences (i.e., the **Reduction** columns in
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:SoilCN))'
} else {
...
The *sample* mean\ %N reduction will vary, depending on which one of the countless possible samples is randomly obtained, *even if* the mean reduction in the population is zero.
...
* centred around the sampling mean, whose value\ is $\mu_d$ (from\ $H_0$),
* with a standard deviation (called the *standard error* of $\bar{d}$) of
\begin{equation}
   \text{s.e.}(\bar{d}) = \frac{s_d}{\sqrt{n}},
   (\#eq:StdErrorDifferencesTest)
\end{equation}
where\ $n$ is the number of differences, and\ $s_d$ is the standard deviation of the sample differences.
:::


The value of the *standard error of the differences* here is
$$
  \text{s.e.}(\bar{d}) = \frac{s_d}{\sqrt{n}} = \frac{0.0618}{\sqrt{28}} = 0.0117,
$$
as shown in Fig.\ \@ref(fig:Nitrogenjamovi) (top table).
This sampling distribution of\ $\bar{d}$ is shown in Fig.\ \@ref(fig:PercentageNSamplingDist).



```{r PercentageNSamplingDist, fig.cap="The sampling distribution for $\\bar{d}$ is a normal distribution; it describes how the sample mean reduction in percentage N varies in samples of size $n = 28$ when the population mean reduction is $0$.", fig.align="center", fig.width=9.25, fig.height=2.5, out.width='100%'}
...
text(x = mn,
     y = 0.7 * max(out$y),
     pos = 3,
     labels = expression( italic(z) == 2.41) )

points(x = mn,
       y = 0,
       pch = 19,
       cex = 0.6)
mtext(text =  expression(bar(italic(d)) == 0.0282),
      side = 1,
      at = mn,
      cex = 0.8,
      line = 0,
...
The sample mean difference can be located on the sampling distribution (Fig.\ \@ref(fig:PercentageNSamplingDist)) by computing the $t$-score:\index{Hypothesis testing!mean difference}\index{Test statistic!t@$t$-score}
$$
	t
...
Software displays the same $t$-score (Fig.\ \@ref(fig:Nitrogenjamovi), top table).


A $P$-value determines if the sample data are consistent with the assumption (Table\ \@ref(tab:PvaluesInterpretation)).
Since $t = 2.41$, and since $t$-scores are like $z$-scores, the *one*-tailed $P$-value is smaller than\ $0.025$ (based on the $68$--$95$--$99.7$ rule).\index{68@$68$--$95$--$99.7$ rule}
Software (Fig.\ \@ref(fig:Nitrogenjamovi)) reports that the *two*-tailed $P$-value is\ $0.02279$.
Hence, the *one*-tailed $P$-value is $0.02279/2 = 0.0114$ (indeed smaller than\ $0.025$).
...
> Moderate evidence exists in the sample (paired $t = 2.41$; one-tailed $P = 0.0114$) of a mean reduction in percentage soil N  from non-irrigated to irrigated sites (mean reduction: $0.0282$%; $n = 28$).
...
For a hypothesis test for the mean of paired data, these conditions are similar to those for one sample mean.
...
Provided $n \ge 25$, this will be approximately true *even if* the distribution of the differences in the population does not have a normal distribution.
...
For more than two within-individuals comparisons, *repeated-measures analysis of variance (ANOVA)*\index{Analysis of variance!repeated measures} can be used.
For paired *qualitative* data, McNemar's test\index{McNemar's test} may be suitable.
...
> No evidence exists ($t = 1.45$; two-tailed $P = 0.161$) that the day of first-flowering is different for the encroaching willow and the native skypilot (mean difference: $1.36$ days earlier for willow; $n = 25$).
...
Make sure to interpret the test results consistently with how the differences were defined.
:::
...
They randomly allocated $32$\ patients into a control group (who drank hot water), and another $32$\ patients to receive chamomile tea (p.\ 164):\index{Control group}\index{Blinding!researchers}
...
Did the TG reduce in either group?
That is, are the mean differences in TG non-zero due to sampling variation or not?
...
* \makebox[31mm][l]{Tea-drinking group:}  $H_0$: $\mu_d = 0$\quad \text{vs}\quad$H_1$: $\mu_d \ne 0$.
* \makebox[31mm][l]{Control group:}       $H_0$: $\mu_d = 0$\quad \text{vs}\quad$H_1$: $\mu_d \ne 0$.

To conduct the hypotheses tests, the standard errors for each reduction are needed:
...
Then, the two test statistics are:
$$
...
> There is very strong evidence ($t = 7.19$; two-tailed $P < 0.001$) of a mean change in TG for the chamomile-drinking groups (mean reduction: $38.62$\mgs.dl^$-1$^; $n = 32$), but *no* evidence ($t = -1.10$; two-tailed $P > 0.10$) of a mean change in the hot-water drinking group (mean reduction: $-7.12$\mgs.dl^$-1$^; $n = 32$).
...
However, the two groups ultimately need to be compared.
...
1. What is the approximate value of the two-tailed $P$-value?
...
Use the data (Table\ \@ref(tab:CICaptoprilData)) and the software output (Fig.\ \@ref(fig:CaptoriljamoviHT)) to answer these questions.
...
```{r CaptoriljamoviHT, fig.cap="Software output for the Captopril data.", fig.align="center", out.width="80%", fig.show="hold"}
...
Taste was assessed using a '$100$\mms visual analog scale', where a *higher* score means a *better* taste.
In summary:
