...
A study examined the relationship between where students ate, and where the student lived [@data:Mann12017:UniStudents]..
...
The parameter is the [*odds ratio* (OR)](#OddsRatio); specifically, the odds ratio of eating most meals off-campus, comparing those living with parents to those not living with parents.
...
> Among university students, what are the odds of students eating most meals off-campus comparing students who *do* and *do not* live with their parents?
...
## Constructing confidence intervals using software
\index{Confidence intervals!odds ratio}

Using jamovi to find the OR (Fig.\ \@ref(fig:EatingGraphsCI), right panel), the sample OR is $5.94$ (as computed manually), and the (exact) $95$%\ CI is from $1.35$ to $26.1$.
...
The *expected* counts are what we would expect to find if there was *no relationship* between the two variables in the two-way table (see Example\ \@ref(exm:Sunglasses)).

If there was *no relationship* between the two variables for the student-meals data, students living *with* or *not with* their parents would have a similar percentage of meals eaten on-campus.
First see that the overall percentage of students eating meals on-campus is $157/183\times 100 = 85.79$% (from Table\ \@ref(tab:MealsDataTable)).
...
*You do not have to compute these expected values*, as software like jamovi can be used to produce the *expected* counts to check statistical validity (see Fig.\ \@ref(fig:UniMealsTestOutputExpectedOnlyjamovi)).
...
A study [@candan2021first] compared the odds of infected nests between nest relocated due to the risk of tidal inundation, and non-relocated nests (Table\ \@ref(tab:TurtleNestDataTable)).
...
A study of car crashes in a rural, mountainous county in western China [@wang2020driver] recorded the data in
`r if (knitr::is_latex_output()) {
...
1. Use the output to write down a CI for the odds ratio.
1. Write a conclusion.
1. Use
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:CrashDataTables) (right table)'
} else {
   'Table\\ \\@ref(tab:ExpCrashDataTable)'
}`
   to determine if the CI is statistically valid.
:::
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2), 
                       format = "latex",
                       label = "CrashDataTables",
                       caption = "Types of crashes in different years: data (left), and expected values (right)") %>% 
    kable_styling(font_size = 8)
...
A research study conducted in Brisbane [@data:Dexter2019:SunProtection] recorded the number of people at the foot of the Goodwill Bridge, Southbank, who wore hats between $11$:$30$am to $12$:$30$pm.
...
```{r PetBirdsCIjamovi, fig.show="hold", fig.cap="jamovi output for the pet-birds data", fig.align="center", out.width="55%"}
...
The data appear in Table\ \@ref(tab:B12DataCI), and the jamovi output in Figs.\ \@ref(fig:B12SPSSOutputCIjamovi) and\ \@ref(fig:B12jamoviExpectedCounts).
...
- Sect.\ \ref{thinkBox:ExpectedCounts}: Lives with parents: $7.7$; *does not* live with parents: $18.3$.
