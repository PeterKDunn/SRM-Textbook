...
The parameter is the *odds ratio* (OR); specifically, the odds ratio of eating most meals off-campus, comparing those living with parents to those not living with parents.
...
So the *odds ratio* (OR) of eating most meals *off-campus* (the *first* row), comparing students living with parents to students *not* living with parents, is $26 \div 4.375 = 5.943$.
...
The *expected* counts are what we would expect to find if there was *no relationship* between the two variables in the two-way table (Example\ \@ref(exm:Sunglasses)).
...
A forward-direction observational study in Western Australia [@data:Wallace2017:Sunburn] compared the heights of scars from burns received (Table\ \@ref(tab:ScarsData)).
