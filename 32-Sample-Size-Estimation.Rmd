...
* As seen above, very large sample sizes only slighty improve precision.
...
In a weight-loss study, estimating the weight loss with a precision of\ $1$\gs is far more precise than is necessary: a weight loss of\ $1$\gs has no practical importance, but would require a massive sample size to estimate.\index{Practical importance}
...
* Estimating a difference between two proportions: Sect.\ \@ref(SampleSizeDifferenceTwoProportions).
