...
All other things being equal, a *larger* sample size gives a *more precise* (Sect.\ \@ref(PrecisionAccuracy)) estimate of the parameter;\index{Precision} that is, a *narrower* CI.
After all, that's why larger samples are preferred over smaller samples: they provide more *precise* estimates.
If that was not the case, the smallest, cheapest and easiest possible sample of size one would be sufficient, which is clearly absurd.
...
Using more pots increasing precision, but increases costs and the necessary time to complete the study.
:::
...
In a weight-loss study, estimating the weight loss with a precision of\ $1$\gs is far more precise than is necessary: a weight loss of\ $1$\gs is of no practical importance, but would require a massive sample size to estimate.\index{Practical importance}
...
Formulas are available for computing sample sizes without this restrictions, but are more complicated than that given here.
...
Suppose we wanted to estimate the difference between the proportion of infected nests to within\ $0.15$.

We would compute:
$$
   \frac{2}{0.15^2} = 88.89.
$$ 
We would need to record data from at least\ $89$ natural nests and\ $88$ relocated nests.
:::
...
All other things being equal, making a sample size four times as large results in a confidence interval half as wide.
...
1. Suppose we wish our $95$%\ CI to be give-or-take\ $0.025$; that is, we wish to *halve* the width of the interval above. 
   How many Kenyans would need to be surveyed? 
1. How many *times* as many Kenyans are needed to *halve* the width of the interval?
:::
...
1. how many paramedics are needed to estimate the mean with an precision of give-or-take\ $50$\mLs?
1. how many paramedics are needed to estimate the mean with an precision of give-or-take\ $25$\mLs?
...
To estimate the difference between the proportion of patients having an in-hospital heart attack, between patients with a low body temperature and patients with a high body temperature, to within\ $0.03$, what size samples are needed?
:::


::: {.exercise #SampleSizeSunglasses}
Exercise\ \@ref(exr:TestOddsRatioSunglasses) describes a study comparing the proportion of males and females who wore sunglasses in Brisbane, Australia [@data:Dexter2019:SunProtection].
Suppose we wished to make a similar comparison for people in Auckland, to within\ $0.07$.
How many males and females would be needed?
:::
