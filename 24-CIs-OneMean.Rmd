...
Suppose thousands of people made one sample of $25$ rolls each, and computed the mean for their sample.
...
::: {.definition #ConfidenceIntervalp name="Confidence interval for $\mu$"}
A *confidence interval* (CI) for the unknown value of the population proportion $\mu$ is
\begin{equation}
  \bar{x} \pm ( \text{multiplier} \times \text{s.e.}(\bar{x})), 
  (\#eq:CImu)
\end{equation}
where $( \text{multiplier} \times \text{s.e.}(\bar{x}))$ is the *margin of error*, and
$$
   \text{s.e.}(\hat{p}) 
   =
