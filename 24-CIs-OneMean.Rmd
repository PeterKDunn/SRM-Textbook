...
The quantity $\big( \text{multiplier} \times \text{s.e.}(\bar{x})\big)$ is called the *margin of error*.
:::
...
A range of $\mu$ values that are likely to straddle $\bar{x}$ is given by a CI.
...
Usig software, the exact $95$% CI is from $2.17$ to $4.23$, the same as the approximate CI to one decimal place.
...
* When $n \ge 25$, the test is statistically valid when the distribution of data is *not* highly skewed.
...
@bartareau2017estimating studied American black bears, and found the mean weight of the $n = 185$ male bears was  $\bar{x} = 84.9$\ kg, with a standard deviation of $s = 51.1$\ kg.
...
@data:Macgregor1985:ToothbrushinghYoungAdults studied the brushing time for $60$ young adults (aged 18--22 years old), and found the mean tooth brushing time was $33.0$\ s, with a standard deviation of $12.0$\ s.
