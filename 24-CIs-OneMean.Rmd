...
## Sampling distribution for $\bar{x}$: when $\sigma$ known {#SamplingDistSampleMeanSigmaKnown}
...
Under certain conditions, the values of the sample mean vary with a normal distribution, and this normal distribution has a mean and a standard deviation.
...
## Sampling distribution for $\bar{x}$: when $\sigma$ unknown {#SamplingDistSampleMean}
...
Here the multiplier is *not* actually a $z$-score (but is *like* a $z$-score).
...
* When $n \ge 25$, the test is statistically valid provided the distribution of data is *not* highly skewed.
