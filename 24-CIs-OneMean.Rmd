...
```{r, child = if (knitr::is_html_output()) {'./introductions/24-CIs-OneMean-HTML.Rmd'} else {'./introductions/24-CIs-OneMean-LaTeX.Rmd'}}
```
...
## Sampling distribution for $\bar{x}$: $\sigma$ known {#SamplingDistSampleMeanSigmaKnown}
...
In this chapter, estimating a population mean $\mu$ (the parameter) using the sample mean $\bar{x}$ (the statistic) is studied.
As usual, the sample mean is computed from just one of the many possible samples, and each possible sample is likely to produce a different value of $\bar{x}$.
...
```{r RollDiceHistMeanFig, fig.align="center", fig.width=5, fig.height=2.75, fig.cap="Rolling dice: The mean of 25 rolls, for thousands of repetitions" }
if (knitr::is_latex_output()){
  set.seed(123456789)
  num.sims <- 2000
  num.rolls <- 25
...
Under certain conditions, the values of the sample mean varies with a normal distribution, and this normal distribution has a mean and a standard deviation.
...
However, since the population standard deviation is rarely ever known, let's focus on the case where the value of $\sigma$ is unknown (and estimated by the *sample* standard deviation, $s$).


## Sampling distribution for $\bar{x}$: $\sigma$ unknown {#SamplingDistSampleMean}
...
Furthermore, the values of $\bar{x}$ vary from sample to sample (*sampling variation*), with an approximate normal distribution.
So, the $68$--$95$--$99.7$ rule could be used to construct an approximate $95$% interval for the plausible values of $\mu$ that may have produced the observed values of the sample mean.\index{68@$68$--$95$--$99.7$ rule}
...
In Chap.\ \@ref(CIOneProportion), the multiplier was a $z$-score, and approximate values can be found using the $68$--$95$--$99.7$ rule.
Here the multiplier *is not* a $z$-score (but is *like* a $z$-score).

The multiplier is a $z$-score if the value of the *population* standard deviation is known (e.g., the situation in Sect.\ \@ref(SamplingDistSampleMeanSigmaKnown)).
When $\sigma$ is unknown, and the *sample* standard deviation is used instead, the multiplier is called a $t$-score.
...
This condition ensures that the *sampling distribution of the sample means has an approximate normal distribution* (so that the $68$--$95$--$99.7$ rule can be used).\index{68@$68$--$95$--$99.7$ rule}
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
When $n > 25$ approximately, the *data* do not have to have a normal distribution.
...
Other methods (beyond the scope of this book) are needed to compute a CI for the mean.
:::
...
They found the sample mean cadmium concentration was $\bar{x} = 0.076$\ ppm with a standard deviation of $s = 0.0460$\ ppm, from a sample of $290$ peanuts.
...
@bartareau2017estimating studied of American black bears, and found the mean weight of the $n = 185$ male bears was  $\bar{x} = 84.9$\ kg, with a standard deviation of $s = 51.1$\ kg.
...
::: {.exercise #BagsCI}
@data:Dianat2014:schoolbags studied the weight of the school bags of $586$ children in Grades 6--8 in Tabriz, Iran.
The mean weight was $\bar{x} = 2.8$\ kg with a standard deviation of $s = 0.94$\ kg.
...
They measured the forced expiratory volume (FEV) of $n = 45$ eleven-year-old girls.
...
They found the mean lead concentration at one playground (Memorial Park, Port Pirie, in South Australia) was $6956.41$ micrograms per square metre, with a standard deviation of $7571.74$ micrograms of lead per square metre, from a sample of $n = 58$ wipes taken over a seven-day period.
...
@data:Macgregor1985:ToothbrushinghYoungAdults studied the brushing time for $60$ young adults (aged 18--22 years old), and found the mean brushing time was $33.0$ seconds, with a standard deviation of $12.0$ seconds.
...
@data:Grabosky2016:Trees describe the diameter of *Quercus bicolor* trees planted in a lawn as having a mean of $25.8$\ cm, with a standard error of $0.64$\ cm, from a sample of $19$ trees.
...
@watanabe1995estimation studied $n = 30$ five-year-old children, and found the mean time for the children to eat a cookie was $61.3$\ s, with a standard deviation of $29.4$\ s.
...
12. If we wanted to estimate the population mean diameter to within $1$\ mm (or $0.04$\ inches) with $95$% confidence, what size sample would we need?  
    What is a reasonable level of accuracy with which we could measure the diameter of a pizza?
13. Do you think that, on average, the pizzas do have a mean diameter of $12$\ inches in the population, as Eagle Boy's claim?
    Explain.
:::

```{r PizzaCIjamovi, echo=FALSE, fig.cap="Summary statistics for the diameter of Eagle Boys' large pizzas", fig.align="center", fig.align="center", out.width='40%'}
