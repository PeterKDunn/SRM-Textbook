...
In Sect. \@ref(OddsRatioIntro), a study [@data:Mann12017:UniStudents] was introduced that examined the eating habits of university students.
...
* The *odds* are the odds of eating most meals *off-campus* (Row 1).
  Then, the odds ratio compares these odds for students living with their parents (Column 1) to those *not* living with their parents (Row 2).   
...
* The *odds* are the odds of living with parents (Column 1).
  Then, the odds ratio compares these odds for students eating most meals off-campus (Row 1) to the odds of students eating most meals on-campus (Row 2).  
...
::: {style="display: flex;"}
The Greek letter $\chi$ is pronounced 'ki', as in **ki**te (**not** as in **Chi**na).  
...
Here are some details of the calculations:
...
From the software output, $\chi^2 = 26.318$ and $\text{df} = 2$, so this $\chi^2$ value is approximately equivalent to a $z$-score of 
...
- Question in Sect. \ref{thinkBox:ExpectedOddsSame}: Living with parents: 46.333/7.667 = 6.043$; not living with parents: 110.667/18.333 = 6.036$.
  The odds are the same (the small difference is because the expected counts are only given to three decimal places).
- Question in Sect. \ref{thinkBox:NotCauseCancer}: Because the study *observational*. 
  Confounders may explain the relationship (can you think of one?).
  In addition, maybe having lung cancer means that people seek companionship in the form of a pet.
