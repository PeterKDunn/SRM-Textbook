...
The data (Table\ \@ref(tab:BagsTable)) are given in a $3\times 2$ table of counts.


```{r BagsTable}
BagsTab <- array( dim = c(3, 2))
...
```{r BagsSidebysideBar, fig.cap="A side-by-side bar chart for the shopping-bags data", fig.align="center", out.width="75%", fig.height=4, fig.width=7}
barplot(t(BagsTab), 
        beside = TRUE,
        las = 1,
        ylim = c(0, 140),
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(BagsSummary,
                      decDigits = c(3, 3, 1, 0)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
...
* For those '30 or under': the *odds* that they bring their own bag is $126/138 = 0.913$.
* For those '31 to 40': the *odds* that they bring their own bag is $50/32 = 1.712$.
* For those 'Over 40': the *odds* that they bring their own bag is $41/13 = 3.154$.

Then the *odds ratios* can be computed:

* Comparing the odds of bringin a shopping bag, comparing people '31--40' to people '30 and under': $1.563/0.913 = 1.712$.
* Comparing the odds of bringing a shopping bag, comparing people 'Over 40' to people '30 and under': $3.154/0.913 = 3.455$.

In Table\ \@ref(tab:BasHTNumerical), the odds of bringing a shopping bag are relative to those '30 and under'.

Since Table\ \@ref(tab:BasHTNumerical) has *three* groups to compare, three odds are needed.
...
In Table\ \@ref(tab:BasHTNumerical), the reference level is '30 and under' the *Reference level* (i.e., on the bottom of the fraction when computing the odds ratios).
