...
The four potential components of a quantitative RQ are:
...
Populations comprise *individuals*, sometimes called *cases*.\index{Individuals}\index{Cases}
If the individuals are people, they are sometimes called *subjects*.\index{Subjects}
...
Most RQs seek to establish a relationship in the **P**opulation between the **O**utcome and another attribute of the individuals.
...
The other attribute of the individuals may be a:

* between-individuals comparison (Sect.\ \@ref(ComparisonBetweenInd));
* within-individuals comparison (Sect.\ \@ref(ComparisonWithinInd)); or
* connection (Sect.\ \@ref(Connections)).


### Between-individuals comparisons {#ComparisonBetweenInd}

Some RQs *compare* the outcome between different groups of individuals.
The groups either are naturally different (e.g., different sub-species), or are manipulated by the researchers to be different (e.g., different doses of a drug).
These comparisons are called *between-individuals* comparisons, as they compare the outcome *between* different groups of *individuals*.
Examples include:

* Comparing the average amount of wear in floor boards (O) *between* two *groups* of boards: standard wooden flooring materials, and bamboo flooring.
* Comparing the average heart rates (O) *across* three *groups* of people: those who received no dose of a drug, those who received a daily dose of the drug, and those who received a twice-daily dose of the drug.
...
The different groups have different, unique attributes for which the outcome is compared.
:::


:::{.example #InterventionHimalaya name="Between-individuals comparison"}
...
### Within-individuals comparisons {#ComparisonWithinInd}

Some RQs *compare* the outcome multiple times on the *same* individuals.
These comparisons are called *within-individuals* comparisons, as they compare the outcome *within the same individuals*, not across different groups of individuals.
...
The comparisons may be made when researchers manipulate the individuals between measurements or observations (e.g., recording pulse rate *before* and *after* bein given caffeine to drink), or with researchers not manipulating the individuals (e.g., recording the activity of lizards at 4a.m. and 4p.m.). 
...
Some RQs explore *connections* between the outcome and another attribute of the individuals that can take many various values.
Examples include:

* Connecting the average heart rate (O) with exposure to the amount of caffeine consumption the previous day (C).
...
:::{.example #WithinBetweenConnection name="Comparisons and connections"}
The Wollemi pine was discovered by science in 1994.
@offord2023home studied the growth of these rare plants.

The relationship between the size of trees (using DBH, diameter at breast height; the outcome), and the pH of the soil (the connection) was explored.
Both DBH and pH are recorded for many different trees. 

The relationship between the DBH for each tree at at various times after the planting date was recorded. 
Each tree has the DBH measured over time, for many time points. 
Time is the *within*-individuals comparison. 
:::


:::{.example #WithinBetweenConnections name="Within-individual connections and comparisons"}
Consider studying marathon runners.
A study with a *connection* would record the daily water intake and the daily sun exposure once each for many individuals.

A *within*-individuals comparison would record the daily sun exposure for many individuals over two days: the comparison is *within* each individual.
:::


## The intervention {#Intervention}
\index{Intervention (in a RQ)}

In some studies, the comparison or connection naturally occurs (e.g., age of people), and in some studies they are manipulated by researchers (and called *interventions*).


::: {.definition #Intervention name="Intervention"}
An intervention is present when *researchers* manipulate (or impose) the circumstances of the individuals (whether that is a *comparison or connection*) to determine the impact on the outcome. 
:::


Comparisons or connections that are *not* manipulated by the researchers are called *conditions*.
In other studies, the comparison or connection can be *manipulated* by the researchers, or *imposed* on the individuals by the researchers (e.g., fertilizer dose).
When this occurs, the RQ has an *intervention*.
Comparisons or the connections that are manipulated by the researchers are called *treatments*


Any type of comparison or connection can have an intervention (though they are uncommon with connections).
Within any type of comparison or connection, the *analysis* is the same whether an intervention is used or not.
However, the *interpretation* of the results depend on whether an intervention is used or not.
...
*Descriptive RQs* are the most basic, giving the population to be studied, and the outcome of interest about this population.
They do not explore relationships, so have no comparison or connection.
...
*Relational RQs* state the population, the outcome, and the *between-individuals* comparison.
They may or may not have an intervention.
...
There is *no intervention* since the sex of the individual (the comparison) is *not* manipulated by, or imposed by, the researchers.
...
*Repeated measures RQs* state the population, the outcome, and the *within-individuals* comparison.
They may or may not have an intervention.
...
The comparison is between winter and summer, which cannot be manipulated by researchers, so there is no intervention.
This is a relational, *estimation*-type relational RQ.
:::


Repeated-measures RQs with only two comparisons are often called *paired* studies.
...
*Correlational RQs* state the population, the outcome, and the comparison.
They may, but usually do not, have an intervention.
...
This is a correlational RQ where the *connection* is 'age'.
Age cannot be manipulated by the researchers, so there is no intervention.
...
The first two are *two-tailed RQs*, since they ask about a difference either way: the average zinc intake could be higher for girls or hiher for boys; we are just interested in whether any difference is present.
The last two area *one-tailed RQ*, since they ask specifically about a difference in just one direction: girls greater than boys, or boys greater than girls.
...
The outcome could be *compared* between groups (such as those taking echinacea and those who do not), or *connected* to something else (such as the daily intake of echinacea in milligrams).
...
If the study *did have an intervention*, the subjects would decide for themselves how to treat their cold.
If the study *did not have an intervention*, various interventions could be used; the dose *frequency* or the doses *amounts* could be imposed.
...
This is an one-tailed, interventional RQ.
...
* The *explanatory variable*, which records information to determine the comparison or the connection.\index{Variables!explanatory}
...
An *explanatory variable* is a variable that records the values of the comparison or connection on individuals in the population.
:::
...
Similarly, the values of the explanatory variable measured on the individuals distinguish between the values of the *comparison* (Table\ \@ref(tab:RQsPopulationIndividualsExamplesComparison) and\  \@ref(tab:RQsPopulationIndividualsExamplesComparisonWithin)) or the *connection* (Table\ \@ref(tab:RQsPopulationIndividualsExamplesConnection)) being made.
...
PopInd3[2, ] <- c("Between left and right arms", 
                  "", 
                  "Which arm")
PopInd3[3, ] <- c("Between for legs and hind legs", 
                  "", 
                  "Which legs")
...
PopInd3[2, ] <- c("With diastolic blood pressure", 
                  "", 
                  "Diastolic blood pressure \\emph{individual} person")
PopInd3[3, ] <- c("With the percentage aggregate", 
                  "", 
...
Section\ \@ref(RepeatedMeasuresPaired) discussed a study [@levitsky2004freshman] where the weights of university students were recorded both at the beginning university, and then after $12$ weeks.
The number of *rows* in the data worksheet will equal the number of people in the study, since the person is the unit of analysis.
The data worksheet needs at least two columns (Fig.\ \@ref(fig:DataPrepjamovi), right panel):

* one to record the student's weight at the start of university (say, `????`);
* one to record the student's weight after $12$ weeks at university (say, `???`).
:::

STUDENT WWT< as per earlier example. CHANE SCREEN SHOT
Fig.\ \@ref(fig:DataPrepjamovi) (right panel).
:::


```{r DataPrepjamovi, fig.show="hold", out.width=c('42%', '1%', '46%'), fig.align="center", fig.cap="jamovi prepared for the data, with some data entered, and the variable names as the column headers. Left: between-individuals comparison. Right: Within-individuals comparison.", fig.align="center", fig.width=3, fig.height=4}
knitr::include_graphics( "jamovi/DataPrep-jamovi.png" )
knitr::include_graphics( "OtherImages/SPACER.png" )
knitr::include_graphics( "jamovi/CaptoprilAll/Captopril-Data.png" )
```
...
Comparisons and connections may be *within*- or *between*-individuals. 

RQs may be *descriptive* (with only a P and O), *relational* (with a P, O and between-individuals comparison), repeated measures (with a P, O and within-individuals comparison) or *correlational* (with a P, O and a connection).
Rqs with a comparison or connection may also have an intervention.
...
::: {.exercise #InterventionalRQBetween}
...
Two possible RQs are estimating the difference in composting time when worms were added to waste, and when worms were *not* added and whether the composting times for waste with and without earthworms was the same or not. 
...
\index{Between-individual comparisons}\index{Within-individual changes}
...
In contrast, *within-individual changes* mean that any *changes* (or differences) are studied *within* the *same* individuals: different groups *are not compared*.
:::


Effectively, in a within-individuals study, each unit of analysis acts as its own control.
THAT STATEMENT ONLY MAKES SENSE IF A CONTROL IS DEEINED> ACAN LEAVE THAT TILL CHAP 3 THOUH
