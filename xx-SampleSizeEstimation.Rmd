...
Furthermore, ethics committees often wish to keep sample sizes as small as possible, so that as little harm as possible is done to as few animals and people as possible.

Determining what sample size to use is, therefore, a trade-off between cost, ethics and precision.

In addition, it sometimes is important to begin with a slightly larger sample than required to allow for drop-outs (foe example, anials die, or people withdraw from the study).
...
```{example SampleSizeParamed, name="Sample size estimation"}
