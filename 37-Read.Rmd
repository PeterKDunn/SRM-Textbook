...
Current practice and advances in every evidence-based discipline\index{Evidence-based research} builds on past research, so being able to critique the research is important.
(A *critique* evaluates: identifying what is good, and what can be improved.)
Research is usually communicated formally in *journal articles* or less formally in *presentations* (Chap.\ \@ref(WritingResearch)).
...
For example, other terms used for *explanatory variables*\index{Explanatory variable} include *independent variables*, *predictors*, *regressors*, *covariates* [@dunn2016learning].
...
> The primary outcome of the study was subjective sleep quality, assessed by the Serbian version of the Pittsburgh Sleep Quality Index ( <span style="font-variant:small-caps;">psqi</span>).\smallskip
>
> The mean total  <span style="font-variant:small-caps;">psqi</span> score before intervention was $6.83\pm 2.73$ (bad), while after the intervention the same score was statistically significant reduced to $3.93 \pm 1.68$ (good)...\smallskip
>
...
* *Asking* the RQ:\index{Research question!relational}
  ...
  * The outcome is the (average) 'subjective sleep quality', as measured by  <span style="font-variant:small-caps;">psqi</span> (the operational definition).\index{Definitions!operational}
* *Designing* the study:
  * The sampling method is not stated, but likely to be voluntary-response.
  * The sample size is not given in the Abstract.
* *Analyse* the data:
  * A quantitative variable is being compared *within* individuals, so a paired $t$-test is the likely method of analysis (Chap.\ \@ref(AnalysisPaired)).
* *Report the results*:
...
* The *inclusion* criteria are 'owning and daily usage of mobile phone with Android operating system and <span style="font-variant:small-caps;">amoled</span> screen' (p.\ 336).\index{Inclusion criteria}
...
* The *response variable*\index{Response variable} is the  <span style="font-variant:small-caps;">psqi</span> total score, which ranges from $0$ to $21$; the *Abstract* implies smaller scores are better.
...
* The method of analysis was a paired $t$-test (p.\ 337).
  The $P$-value for the $t$-test is $p < 0.0001$ (p.\ 337), so there is very strong evidence to support a change in mean  <span style="font-variant:small-caps;">psqi</span>.
...
* The data and analysis details are not available, so the research is not *reproducible*.\index{Research!reproducibility}
...
* The researchers compared the students who *remained* in the study with those who *dropped out* of the study (Table\ 1); they found no evidence that those who dropped out and those who stayed were different on the variables studied (i.e., the drop-outs did not introduce a bias).\index{Bias}\index{Drop outs}\index{Bias!non-response}
* The sample size of $n = 30$ suggests the $t$-test is *statistically valid*.\index{Statistical validity (for inference)}
...
> ... being specific to investigate the impact of mobile phones to sleep quality...
...
* Since 'each participant was informed about the detailed plan of the study...' due to ethics requirements,\index{Ethics} (p.\ 336) the participants were not blinded to being in a study, nor the purpose of the study.\index{Blinding}
  ...
  That is, the *Hawthorne effect*\index{Hawthorne effect} and placebo effects\index{Placebo effect} may impact the results.
  This means the change in  <span style="font-variant:small-caps;">psqi</span> may have been because students knew they were in the study, and not due to the reduction in blue light.
...
* Participants for the study were chosen 'on voluntary basis'\index{Sampling!non-random!voluntary} (p.\ 336), so the sample was not a random sample.
  The study may not be *externally valid*,\index{External validity} but the students *in* the study probably would not be very different from students who *did not* volunteer for the study regarding the impact of blue light.
...
* Participants completed the questionnaire pre-intervention, then 'used the app for one month period [and] at the end they completed <span style="font-variant:small-caps;">psqi</span> once again'.
