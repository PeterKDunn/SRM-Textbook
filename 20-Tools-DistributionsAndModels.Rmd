...
out <- hist( sampleP,
             ...
             axes = FALSE,
             col = plot.colour,
             xlab = expression(Sample~proportions~hat(italic(p))), 
             ylab = "",
...
```{r HistogramDBPPossums, fig.align="center", fig.height = 3, fig.width=8.5, out.width='100%', fig.cap="Two normal distributions. Left: diastolic blood pressure of a sample of\ $398$ Americans. Right: the weight of a sample of\ $83$ male Leadbeater's possums.  The solid lines are the approximate normal model for the variable in the population."}
par( mfrow = c(1, 2))
data(Diabetes)
...
```{r EmpiricalRuleDiagram, fig.width=6, fig.height=2.1, out.width='100%', fig.align="center", fig.cap="The $68$--$95$--$99.7$ rule. The shaded regions correspond to the central $68$%, $95$% and $99.7$%."}
par(mfrow = c(1, 3),
    mar = c(4, 0.25, 4, 0.25))
...
```{r EmpiricalRuleHts, fig.width=7.5, fig.height=3.15, out.width='75%', fig.align="center", fig.cap="A model for the height of adult Australian females in the population."}
out <- plotNormal(mu = 162, 
                  sd = 7, 
                  #zlim.hi =  3.75,
                  #zlim.lo = -3.75,
                  main = "A model for the heights of adult Australian\nfemales in the population",
                  xlab = "Heights (in cm)")
```
...
```{r HtsExer1, fig.cap="Left: what proportion of Australian adult females are taller than $169\\cms$? Right: what proportion of Australian adult females are shorter than $148$\\cms?", fig.align="center", fig.width=7.5, fig.height=3.15, out.width='90%'}
HT.mn <- 162
HT.sd <- 7
...
out <- plotNormal(mu = HT.mn, 
                  sd = HT.sd, 
                  ylim = c(0, 0.075),
                  main = "95% of heights are\nwithin two std devs. of the mean",
                  xlab = "Height (in cm)")
shadeNormal(out$x,
            out$y,
            lo = 0,
...
```{r HtsEmpirical, fig.cap="The $68$--$95$--$99.7$ rule and the heights of Australian adult females.", fig.align="center", fig.width=7.15, fig.height=2.75, out.width='65%'}
...
```{r ZDBH3, fig.cap="What proportion of tree diameters are between $5$\ and $11$\ inches? Left: the hatched area is the area to the left of $z = -1.41$. Right: the shaded area is the area to the left of $z = 0.81$. Neither give us the area we seek directly.", fig.align="center", fig.width=9.5, fig.height = 2.75,out.width='100%'}
...
```{r DBHBackwards, fig.cap="Tree diameters: the smallest\\ $3$\\% is shaded. The approximate location of the required $z$-score is drawn.", fig.align="center", out.width='70%', fig.width=6, fig.height=2.65}
par( mar = c(4, 1, 3, 1) )
...
`r if (knitr::is_latex_output()) {
   'Only the hard-copy tables are explained in this book (see the online version of this book for the online tables, and instructions for their use).'
} else {
...
```{r DBHBackwards2, fig.cap="Tree diameters: the largest $25$\\% is the same as the smallest $75$\\%.", fig.align="center", fig.width=6, out.width='65%', fig.height=2.65}
par( mar = c(4, 1, 3, 1) )
...
```{r RetentionTime, fig.cap="Retention times of food in sheep.", fig.align="center",  fig.width=6, out.width='65%', fig.height=2.65}
par( mar = c(4, 1, 3, 1) )
out <- plotNormal(42.5,
                  3.68,
                  xlab = "Retention times (in h)",
...
```{r RetentionPlots, fig.cap="Plots for retention times in sheep.",  fig.align="center", out.width="100%", fig.width=7.5, fig.height=4.25}
par( mfrow = c(2, 2))
...
Probabilities can be computed from normal distributions using *$z$-scores*, and the $68$--$95$--$99.7$ rule or tables.
...
The modelled the gestation length of healthy babies with a normal distribution, having a mean of\ $40$\ weeks, and a standard deviation of\ $1.64$\ weeks.
Using this model, answer these questions.
...
1. What does a negative $z$-score mean in this?
:::
