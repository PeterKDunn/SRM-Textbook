...
The proportions are usually computed within each row, as the Column\ 1 information out of the total. 
...
:::{.example name="Summary information"}
For the data in Table\ \@ref(tab:MealsDataTableHT), the proportion of students eating most mean off-campus is

* for students living with their parents (i.e., Row\ 1): $52/54 = 0.963$;
* for students *not* living with their parents (i.e., Row\ 2): $105/129 = 0.814$.

The difference is therefore $0.963 - 0.814 = 0.149$, as shown in the software output (Fig.\ \@ref(fig:UniMealsTestOutputHTjamovi)).
...
The RQ and the hypotheses can be written as comparing proportions (Sect.\ \@ref(CompareTwoProportions)), comparing odds (Sect.\ \@ref(CompareTwoOdds)), or about odds ratios directly.
...
```{r UniMealsTestOutputHTjamovi, fig.show="hold", fig.cap="The jamovi output for computing a CI and conducting a test", fig.align="center", out.width="60%"}
...
Often more than two proportions need to be compared, or the tables of counts is larger than $2\times 2$.
...
The RQ is answered using a $\chi^2$-test, by extending the ideas in extension in Sect.\ \@ref(CompareTwoOdds).
We demonstrate using an example.


:::{.example #ChiSqLarger name="Larger two-way tables"}
[*Dataset*: `RipsID`]
@diez2023rip studied the people's knowledge of ocean rips (Table\ \@ref(tab:RipTableSummary), left table).
The table is a $4\times 2$ two-way table.
The RQ is

> Is there a relationship (or association) between age group and people's abiity to correctly identify a rip?
...
RipSummary[, 1] <- RipTable[, 1] / rowSums(RipTable)
RipSummary[, 3] <- RipTable[, 1] / RipTable[, 2]
RipSummary[, 4] <- rowSums(RipTable)

RipSummary[1, 2] <- NA
RipSummary[2, 2] <- RipSummary[2, 1] / RipSummary[1, 1]
RipSummary[3, 2] <- RipSummary[3, 1] / RipSummary[1, 1]
RipSummary[4, 2] <- RipSummary[4, 1] / RipSummary[1, 1]
...
T2 <- kable( pad(RipSummary,
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "Identifying rips. Left: the data by age group. Right: a summary table. The odds ratio is relative to the 18 to 24 age group.") %>% 
    kable_styling(font_size = 8)
...
The data can be summarised using the proportions and odds (Table\ \@ref(tab:RipTableSummary)) of correctly identifying rips.
Odds ratios compare *pairs* of odds, so the odds ratios in Table\ \@ref(tab:RipTableSummary) are all relative to those in $18$ to $24$; hence, no odds ratio is given for the $18$ to $24$ age group which acts as the *reference level*.\index{Reference level}
For example, the odds of someone aged $51$ to $65$ correctly identifying a rip is $0.960$ times the odds of someone aged $18$ to $24$ correctly identifying a rip.

Tests involving two-way tables larger than $2\times 2$ use a $\chi^2$-test (Sect.\ \@ref(CompareTwoOdds)).
The hypotheses are written in terms of *associations* between the two qualitative variables:
...
As in Sect.\ \@ref(CompareTwoOdds), the value of $\chi^2$ compares the observed counts in Table\ \@ref(tab:RipTableSummary) to the expected counts (found in the same as in Sect.\ \@ref(ExpectedValues)).
Software computes $\chi^2 = 2.6608$ (Fig. \@ref(fig:Ripsjamovi)).
...
\]
So here, $\text{df} = (4 - 1)\times ( 2 - 1) = 3$, as shown in the output (Fig. \@ref(fig:Ripsjamovi)).
Hence, the equivalent $z$-score is  
\[
  z = \sqrt{2.6608/3} = 0.942,
\]
which is quite small, so we expected a large $P$-value.
Software confirms this (Fig. \@ref(fig:Ripsjamovi)): $P = 0.447$ (recall, the $P$-value is always two-tailed for a $\chi^2$-test).
