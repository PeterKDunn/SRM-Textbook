...
To compare the two proportions, the RQ is:
...
The two *sample* proportions will vary from sample to sample and so have a sampling distribution (Sect.\ \@ref(OnePropTestSamplingDist)); likewise the difference between the proportions will vary from sample to sample, and hence have a sampling distribution.
...
The test statistic is a $z$-score:\index{Test statistic!$z$-score}  
\[
...
Rather than comparing proportions, odds can be compared:
...
In a $2\times 2$ table of counts, the *square root* of the $\chi^2$ value is equivalent to a $z$-score of about $\sqrt{6.934} = 2.633$.
This is the *same* $z$-score produced when comparing two proportions (Sec.\ \@ref(CompareTwoProportions)), and hence the $P$-value will be the same also (as seen in the output; Fig.\ \@ref(fi:UniMealsTestOutputHTjamovi), top table).
...
[*Dataset*: `RipsID`]
@diez2023rip studied the people's knowledge of ocean rips (Table\ \@ref(tab:RipTable)).
...
```{r}
RipTable <- array( dim = c(4, 2) )
RipTable[, 1] <- c(39, 48, 111, 48)
RipTable[, 2] <- c(7, 11, 14, 11)
...
if( knitr::is_latex_output() ) {
  kable( pad(RipTable,
        ...
        escape = FALSE,
        align = "c",
        caption = "IUX I SI  Xiui iu it iuiU I ItIFUYFUY") %>%
     column_spec(1, bold = TRUE) %>%
     row_spec(0, bold = TRUE) %>%
     kable_styling(font_size = 8) %>%
    add_header_above( c(" " = 1, 
...
Tests involving two-way tables larger than $2\times 2$ require a $\chi^2$ test (Sect.\ \@ref(CompareTwoOdds)).
...
The odds and percentage of people in each age group that can correctly identify rips can be computed (SUMMARY TABLE), but this is not always possible (e.g., for a $3\times 4$ table).
Software (Fig. \@ref(fig:Ripsjamovi)) shows that $\chi^2 = 2.6608$.
...
Software confirms this: $P = 0.447$ (recall, the $P$-value is always two-tailed for a $\chi^2$-test).
