...
The recommended setting for the test is usually a walkway of at least $30$\ m.
@saiphoklang2022comparison compared the mean 6MWT when the same subjects used both using $20$\ m and $30$\ m walkways:

> For Thai patients with chronic obstructive pulmonary disease (COPD), what is the mean difference between the 6MWT when subjects use a $20$\ m walkway and a $30$\ m walkway?

The comparison is *within* individuals (Sect.\ \@ref(RQsRepeatedMeasures)); this is a *repeated-measures* study.
Each subject has a *pair* of 6MWT measurements, and the study produces *paired data*, the topic of this chapter.
...
out <- knitr::kables(list(T1, T2),
                     format = "latex",
                     label = "Data6MWT",
                     caption = "The six-minute walk test (6MWT) distance, for walkways of $20$\\ m and $30$\\ m length. These are the first five and the last five of the $50$ total observations. (A negative difference means the $20$\\ m distance is greater than the $30$\\ m distance.)") %>% 
...
if( knitr::is_html_output() ) {
  DT::datatable(SixMWT,
               ...
               colnames = c("Subject", 
                            "Distance (20m walkway)",
                            "Distance (30m walkway)",
                            "Difference"),
               filter = "none",
               options = list(searching = FALSE), # Remove searching: See: https://stackoverflow.com/questions/35624413/remove-search-option-but-leave-search-columns-option
               caption = "The six-minute walk test (6MWT) distance, for walkways of $20$\\ m and $30$\\ m length. (A negative difference means the $20$\\ m distance is greater than the $30$\\ m distance.)")
}
```
...
Since the differences are computed as the $30$\ m distance minus the $20$\ m distance, a negative difference means the $20$\ m distance is greater than the $30$\ m distance.
:::
...
are *paired*.
The RQ is a special case of a *repeated-measures RQs* (Sect.\ \@ref(RQsRepeatedMeasures)), where each unit of analysis has only two observations.
Computing the *differences* or *changes* between the pairs of observations makes sense.
...
come from same person, so sex and age remains the same.
Pairing is a form of blocking (Sect.\ \@ref(ManagingConfounding)), and is a good design strategy when the individuals in the pair are similar for many extraneous variables.
...
* Blood pressure is recorded on the same individuals *before* and *after* receiving a drug.
  The *change* in blood pressure is recorded for each person.
* The number of campers is recorded at many national parks (the 'individuals') on the first weekend in summer, and on the first weekend on winter.
  The *change* in camper numbers for each national park between these time points is recorded.
* The body temperature of dogs (the 'individuals') is measured using rectal and ear thermometers.
  The *difference* between the two recorded temperatures from the thermometers is recorded.
...
For instance, blood pressures can be recorded every thirty minutes over four hours, or temperatures can be compared using three different types of thermometers.
...
For the 6MWT study, the distance is measured for the same subjects for two different walkway distance.
Each subject receives two measurements, and the *change* in the distance walked for each individual is recorded.

Since data are paired, an appropriate graph is *a histogram* of the differences (Chap.\ \@ref(SummariseWithin)); specifically, $30$\ m distance minus the $20$\ m distance.
...
In addition, the link between the 6MWT distances for each subject has been lost.

The *histogram* of the difference in the 6MWT distances makes the difference in individuals' distances easier to see (Fig.\ \@ref(fig:ComparePairedBoxplotsHistogram), right panel).
The histogram also makes it easy to see that some subjects walked further with a $20$\ m walkway, and some further for a $30$\ m walkway.
...
A case-profile plot (Sect.\ \@ref(CaseProfilePlot)) is also appropriate, but is difficult to read for these data as the sample size is large (a line is needed for each unit of analysis).


```{r ComparePairedBoxplotsHistogram, fig.align="center", out.width='90%', fig.cap="Plots of the 6MWT data. Left: graphing the data incorrectly as not paired. Right: a histogram of 6MWT distances changes ($30$\\ m walkway distance minus $20$\\ m walkway distance; the vertical grey line represents no change in distance).", fig.height = 3.25, fig.width = 7.5}
...
boxplot( cbind(SixMWT$Distance20,
         ...
         main = "A boxplot of 6MWT distance\nfor two walkway lengths")
...
The 6MWT distances for each walkway length can be summarised individually (the first two rows of Table\ \@ref(tab:Data6MWT)) using the methods of Chap.\ \@ref(OneMeanConfInterval).
All statistics are slightly different for the two walkway distance; in particular, the $30$\ m walkway distances are slightly larger.
However, since the RQ is about the difference between the distances, a numerical summary of the *differences* is essential.
...
SMWT.DataSummary[2, 4] <- findStdError(SixMWT$Distance20)
...
Every possible sample of $n = 50$ subjects comprises different people, and hence produces different 6MWT distance for $20$\ m and $30$\ m walkways.
For this reason, the 6MWT distance summaries in Table\ \@ref(tab:SMWTSummary) include standard errors.
Since the 6MWT distance vary from sample to sample, the *difference* between the distances for each subject vary from sample to sample too, and also have a *sampling distribution*.

The differences can be treated like a single sample of data (Chap.\ \@ref(OneMeanConfInterval)), and so the sampling distribution for the differences has a similar sampling distribution to that of $\bar{x}$ (provided the conditions are met; Sect.\ \@ref(ValidityPaired)).
In addition, the notation when working with paired data is similar to that used when working with the one-mean case too (Table\ \@ref(tab:PairedNotation)).
...
Notice that the standard deviation of the difference is *not* the difference between the standard deviations for the $20$\ m and $30$\ m distances. 
(The same applies for the standard error and the median.)
Instead, the standard deviation of the list of differences is found (i.e., the column **Diff.** in the data table).
...
$$
or $22.03\pm 6.234$ (so the *margin of error* is $6.234$).
Equivalently, the CI is from $22.03 - 6.234 = 15.796$, up to $22.03 + 6.234 = 28.264$.
We write:

> The mean difference in the 6MWT distances when using a $20$\ m and $30$\ m walkway is $22.03$\ m ($\text{s.e.} = 3.117$; $n = 50$), with an *approximate* $95$%\ CI from $15.80$\ m to $28.26$\ m, further for a $30$\ m walkway.

The CI means that the reasonable values for the population mean difference in 6MTW distances are between $15.80$\ m and $28.26$\ m.
Alternatively, we are $95$% confident that the population mean difference between the 6MWT distances is between $15.80$\ m and $28.26$\ m (further for $30$\ m walkway).
...
Statistical software\index{jamovi output!mean difference} produces *exact* $95$%\ CIs, which may be slightly different than the *approximate* $95$%\ CI (the $68$--$95$--$99.7$ rule gives *approximate* multipliers).
For the 6MWT data, the *approximate* and *exact* $95$%\ CIs are the same to one decimal place (Fig.\ \@ref(fig:SMWTTestOutput)).
We write:

> The mean difference in the 6MWT distances when using a $20$\ m and $30$\ m walkway is $22.03$\ m ($\text{s.e.} = 3.117$; $n = 50$), with an $95$%\ CI from $15.76$\ m to $28.29$\ m further for a $30$\ m walkway.


```{r SMWTTestOutput, fig.cap="The weight-gain data: software output", fig.align="center", out.width="100%", fig.show="hold"}
...
* When $n \ge 25$, the test is statistically valid provided the distribution of differences is *not* highly skewed.
* When $n < 25$, the test is statistically valid only if the data come from a *population* of differences with a normal distribution.

The sample size of $25$ is a rough figure; some books give other values (such as $30$).
Data with severe skewness or large outliers may need a larger sample size for the CI to be statistically valid.

This condition ensures that the *distribution of the sample means has an approximate normal distribution* (so the $68$--$95$--$99.7$ rule can be used).
...
For the SMWT data, the sample size is $n = 50$, so the results are statistically valid.
Neither the differences *in the population*, nor the distances for each walkway length, need to follow a normal distribution.
:::
...
if( knitr::is_latex_output() ) {
  ...
  knitr::kable(pad(TGsummary,
               ...
               align = "c",
               linesep = "",
               caption = "The total glucose (TG; in mg.dl$^{-1}$) for two groups: those who drank chamomile tea, and those who drank hot water (the control group)",
               col.names = c("$n$",
                             "Mean", "Std. dev.", 
                             "Mean", "Std. dev.",
                             "Mean", "Std. dev."),
...
if( knitr::is_latex_output() ) {
 ...
 T1 <- knitr::kable(pad(FruitTab[1:5, ],
  ...
  out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "FruitsData",
                       caption = "The average weight of fruits (in g) in two different years, from $424$ farms. One observation is missing for Farm\ 20.") %>% 
    kable_styling(font_size = 8)
  out2 <- prepareSideBySideTable(out) 
  out2 
}
...
```{r FruitDescriptivesjamovi, fig.cap="Software output for the fruit data", fig.align="center", out.width='70%'}
...
[*Dataset*: `Anorexia`]
Young girls ($n = 29$) with anorexia received cognitive behavioural treatment (@data:hand:handbook), and their weight before and after treatment were recorded.
In summary:
...
The mean weight loss per girls was $`r round(mean( ANCB$After - ANCB$Before), 2)`$ pounds, with a standard deviation of $`r round(sd( ANCB$After - ANCB$Before), 2)`$ pounds
Find an approximate $95$%\ CI for the population mean weight loss. 
Do you think the treatment had any meaningful impact on the mean weight of the girls, based solely on these data?
:::
...
A numerical summary appears is in Table\ \@ref(tab:StressTable).

1. Use the software output in Fig.\ \@ref(fig:StressDescriptivesjamovi) to construct an *approximate* $95$%\ CI for the *increase* in stress as surgery gets closer.
2. Use the software output in Fig.\ \@ref(fig:StressDescriptivesjamovi) to write down the *exact* $95$%\ CI for the *increase* in stress as surgery gets closer.
