...
The Six-Minute Walk Test (6MWT) measures how far subjects can walk in six minutes, and is used as a simple, low-0cost evaluation of fitness and other health-related measures.
The recommended setting for the test is usually a walkway of at least $30$\ m.
@saiphoklang2022comparison measured the 6MWT distance when the same subjects used both $20$\ m and $30$\ m walkways:
...
Each subject receives two measurements, and the *difference* between the distances walked for each individual is recorded.

Since the data are paired, an appropriate graph is *a histogram* of the differences (Chap.\ \@ref(SummariseWithin)); specifically, $30$\ m distance minus the $20$\ m distance.
...
The *histogram* of the differences in the 6MWT distances makes the difference in individuals' distances easier to see (Fig.\ \@ref(fig:ComparePairedBoxplotsHistogram), right panel).
...
A case-profile plot (Sect.\ \@ref(CaseProfilePlot)) is also appropriate, but is difficult to read for these data because of the sample size (a line is needed for each unit of analysis).
...
The 6MWT distances for each walkway length can be summarised individually (the first two rows of Table\ \@ref(tab:SMWTSummary)) using the methods of Chap.\ \@ref(OneMeanConfInterval).
All statistics are slightly different for the two walkway distances; in particular, the mean $30$\ m walkway distance is slightly larger.
...
Every possible sample of $n = 50$ subjects comprises different people, and hence produces different 6MWT distances for $20$\ m and $30$\ m walkways.
...
Instead, the standard deviation of the differences (i.e., the column **Diff.** in the data table) is computed.
...
```{r SMWTTestOutput, fig.cap="The SMWT data: software output", fig.align="center", out.width="100%", fig.show="hold"}
