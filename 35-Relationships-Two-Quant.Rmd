...
Each row in the data set (and each point on the scatterplot) corresponds to a single deer (the units of analysis); two variables (age; molar weight) are measured on each deer.
...
Question in Sect. \ref{thinkBox:DescribeScatterNHANES}: *Form*: curved. *Direction*: not relevant (up, then down). *Variation*: large.
`r webexercises::unhide()`
`r if (knitr::is_latex_output()) '-->'`
:::
