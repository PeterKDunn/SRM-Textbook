...
Each row in the data set (and each point on the scatterplot) corresponds to a single deer (the individual deer are the units of analysis), and two different variables (age; molar weight) are measured on each deer.
...
For the red deer data (Fig. \@ref(fig:RedDeerScatter)), the relationship is approximately linear (form) with a negative direction (*older* deer generally have *less heavy* teeth); the *variation* is... perhaps moderate.
:::
