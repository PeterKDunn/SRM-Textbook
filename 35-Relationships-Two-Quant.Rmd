...
So far, RQs comparing two groups have been studied.
Comparing the mean value of a quantitative variable in two groups was studied in Sect. and .
Comparing the percentage of times an outcome of interest appears in two groups was studied in Sect. and .
In this Chapter(and the next two), the relationship between two quantitative variables is studied.

Our example is a study [@data:Holgate1965:StraightLine] examined the relationship between the age of $n = 78$ male red deer and the weight of their
...
In some cases,  it doesn't matter which is $x$ and which is $y$, such as exploring the relationship between height and weight of red deer (see Example \@ref(exm:CorTestDogs).)
...
`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
* **Form**: may start off straight-ish, but seems curved for larger ages. 
* **Direction**: diameter increases as age increases (on average). 
* **Variation**:small-ish for small ages; large-ish for older trees.
`r webexercises::unhide()`
`r if (knitr::is_latex_output()) '-->'`
:::


```{r LimeTreesScatter, fig.cap="The age and diameter of small-leaved lime trees grown in Russia ($n = 385$)", fig.align="center", fig.show="hold", fig.width=8, fig.height=3.25, out.width='90%'}
data(lime)
par( mfrow = c(1, 2))
