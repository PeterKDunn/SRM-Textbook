...
# Hypothesis tests for the mean difference (paired data) {#TestPairedMeans}
...
include_graphics("Illustrations/pexels-taryn-elliott-4231477.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/hush-naidoo-pA0uoltkwao-unsplash.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
if( knitr::is_latex_output() ) {
  kable(head( diab[DiffExists, c("bp.1d", "bp.2d", "Diff")], 10),
        format="latex",
      booktabs = TRUE,
      longtable=FALSE,
      row.names=FALSE,
...
if( knitr::is_html_output() ) {
  DT::datatable( diab[DiffExists, c("bp.1d", "bp.2d", "Diff")],
                 fillContainer=FALSE, # Make more room, so we don't just have ten values
                 #filter="top", 
                 #selection="multiple", 
                 #escape=FALSE,
...
rownames(Diab.Table) <- c("DBP: First visit", "DBP: Second visit", "Decrease in DBP")
