...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "FruitsData",
                       caption = "The average weight of fruits (in g) in two different years, from $24$ farms. One observation is missing for Farm\ 20. The change is computed as the normal year (2015) minus dry year (2014).") %>% 
    kable_styling(font_size = 8)
  out2 <- prepareSideBySideTable(out) 
  out2 
}
...
```{r FruitDescriptivesjamovi, fig.cap="Software output for the fruit data.", fig.align="center", out.width='90%'}
knitr::include_graphics("jamovi/Fruit/FruitWeight.png")
```
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "FerritinTable",
                       caption = "The ferritin content (in $\\microgs.\\xLs$) for $20$\\ thalassemia patients at the Adelaide Children's Hospital.") %>% 
    kable_styling(font_size = 8)  
  out2 <- prepareSideBySideTable(out, 
                                 gap = "\\quad") 
  out2
...
if( knitr::is_html_output()) {
  kable( pad(head(FR, 10),
         ...
         align = "c",
         longtable = FALSE,
         caption = "The ferritin content (in $\\microgs.\\xLs$) for $20$\\ thalassemia patients at the Adelaide Children's Hospital (first ten observations).",
         booktabs = TRUE)
}
```
...
if( knitr::is_latex_output() ) {
  knitr::kable(pad(StressTab,
               ...
               longtable = FALSE,
               escape = FALSE,
               caption = "The surgery-stress data (in fmol.mL$^{-1}$).",
...
colnames(COVID.summary) <- c("Mean (mins)",
                             "Std dev. (mins)")
rownames(COVID.summary) <- c("Before lockdown",
                             "During lockdown",
                             "Increase")
