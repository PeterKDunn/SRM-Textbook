...
experience similar climatic conditions, have similar soil, and most have the same farm management.
...
Pairing sometimes involves taking two measurements from the *same* individuals.
...
A case-profile plot (Sect.\ \@ref(CaseProfilePlot)) is also appropriate, but is difficult to read for these data because sample size is too large (a line is needed for each of the $50$\ units of analysis).
...
The 6MWT distances for each walkway length can be summarised individually (the first two rows of Table\ \@ref(tab:SMWTSummary)) using the methods of Chap.\ \@ref(OneMeanConfInterval) from software (Fig.\ \@ref(fig:SMWTNumericalOutput)).
...
However, since the RQ is about the difference between the distances, a numerical summary of the *differences* is essential (third row of Table\ \@ref(tab:SMWTSummary), using Fig.\ \@ref(fig:SMWTNumericalOutput)).
...
if( knitr::is_latex_output() ) {
  ...
  kable(pad(SMWT.DataSummary,
        ...
        escape  = FALSE,
        align = "c",
        col.names = c("Mean", "Median", "Standard deviation", "Standard error", "Sample size"),
        digits = 2,
        caption = "The numerical summary of the 6MWT data.") %>%
    row_spec(0, bold = TRUE) 
}
...
```{r SMWTNumericalOutput, fig.cap="The 6MWT data: numerical summary software output for each group, and the CI and test results.", fig.align="center", out.width=c("65%","100%"), fig.show="hold"}
...
> For Thai patients with chronic obstructive pulmonary disease, is the mean difference in 6MWT distance using a\ $30$\ms walkway and a \ $20$\ms walkway *positive*?
...
The sampling distribution, as described in Sect.\ \@ref(def:DEFSamplingDistributionDbar), still applies, where $\mu_d$ is assumed to be the value given in $H_0$:
...
The exact $95$% CI (Fig.\ \@ref(fig:FloweringjamoviHT)) is $-0.579$ to\ $3.30$\ days; the difference is because the approximate CI uses the *approximate* multiplier of\ $2$ fromthe $68$--$95$--$99.7$ rule.
...
So, the following descriptive RQs can be asked:
