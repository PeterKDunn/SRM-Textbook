...
We now know this isn't true... But why did van Helmont reach this crazy conclusion?
Through *observation*.

Spontaneous generation was proposed as a theory to explain what had been *observed*.
...
A more recent example of the scientific process in action is the emergence of the dangers of smoking, which was still being debated into the 1990s:
...
Alternatively, if the evidence supports the hypothesis, the hypothesis is *temporarily* accepted.

Notice that the hypothesis is *temporarily* accepted if it fits the available evidence: contradictory evidence may emerge in the future.
...
The researchers wanted to identify the source of the infection to prevent more infections.

The researchers noticed that many of those infected were gardeners who had recently handled potting mixs, so they wondered if the infection was somehow associated with using potting mix.
They designed a study to test this hypothesis, then collected data from 100 people (25 people *with* the infection,  and 75 similar ('matched') people *without* the infection).
...
The researchers described and summarised their data, then analysed the data to reach an evidence-based conclusion: potting mix was partially responsible for the increase in infection numbers, but other factors were also involved.
...
`r if( knitr::is_html_output() ) {
   "Arrange the steps of research into the correct order:"
}`
`r if( knitr::is_latex_output() ) {
...
The two main types of research are **qualitative** and **quantitative** research, which are different yet complementary (Table \@ref(tab:TypesOfResearch), which is a *gross generalisation*!).
...
The decision to use qualitative, quantitative or mixed methods approaches should depend on the research *problem*, not the skills or knowledge of the researchers.
...
(see @egbue2012barriers).\smallskip
...
The researchers ask each group about their perceived benefits and barriers for adopting EVs.\smallskip

A *quantitative research* study might survey a large number of buyers of EVs and buyers of non-EVs, and ask the buyers' age, sex, and questions about why they purchased their car.
The survey responses could be analysed by numerically summarising, and comparing, the responses for buyers and non-buyers of EVs.\smallskip
...
* **Asking** the question: Chap. \@ref(RQs).
  The research process begins with a question that needs answering.
...
* **Analysing** the data: Chaps. \@ref(MakingDecisions) to \@ref(Regression).
  ...
  Analysis depends on the type of data and the research question.
  A computer is useful for this step.
...
* Spreadsheets *do not leave a record* of how the data have been analysed or prepared; for example, formulas can be very difficult to understand and parse.
  Keeping a record of the analysis, preparation of variables, and other operations with the data are part of good scientific practice (*reproducible research*; see Sect. \@ref(ReproducibleResearch)) [@simons2019reproducible]).
