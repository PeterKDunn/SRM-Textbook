...
*Recipes* even existed; for example, van Helmont [@data:VanHelmont:Transformations; @pasteur1922generations; @data:Latour:PasteurPouchet] gave this
`r if (knitr::is_latex_output()) {
   'recipe'
} else {
...
::: {.definition #Data name="Data"}\index{Data}\index{Dataset}}
...
The researchers classified and summarised their data, then analysed the data to reach an evidence-based conclusion: potting mix was partially, but not solely, responsible for the increase in infection numbers.
...
`r if( knitr::is_html_output() ) {"Arrange these steps into the usual order:"} else {"Try arranging these steps into the usual order, using these terms:  
   *Design*; 
...
Research can be broadly classified as *qualitative* or *quantitative*, which are different yet complementary approaches (Table\ \@ref(tab:TypesOfResearch)).\index{Research!qualitative}\index{Research!quantitative}
...
In quantitative research, typically information about a large group of interest (a *population*) is found from a subset of the population (a *sample*).
...
Suppose we wish to learn about the perceived benefits and barriers to people adopting electric vehicles (EVs).
...
The process is not always linear: researchers may jump from step to step as necessary, and the research usually leads to new questions.
Nonetheless, each step is important:\index{Research!steps}


```{r SixSteps, fig.cap="", fig.align="center", fig.width=5, fig.height=3.75, out.width="45%", fig.cap="The six basic steps in research"}
...
* *Describing* and *summarising* the data (Chaps.\ \@ref(DescribingVars) to\ \@ref(SummariseComments)).
...
* *Reporting* the results (Chaps.\ \@ref(Reading) and\ \@ref(WritingResearch)).
  Communicating the results appropriately, accurately and ethically is important, including limitations.


## Using software in research {#Software-In-Research}index{Software!use in research}

*Statistical* software (such as jamovi, R, SAS, SPSS, etc.) is useful for summarising and analysing data.
...
This book sometimes shows output from the statistical software package jamovi [@Software:jamovi], but using jamovi is *not* essential.
:::
...
These problems emerge for various reasons:
...
* Spreadsheets may *change data* (e.g., reformatting entries as dates) when not appropriate (@ziemann2016gene, @zeeberg2004mistaken).
...
Problems with spreadsheets, as with any software package, are often due to human error, but *spreadsheets make the errors hard to find and fix*.
...
Which of the following are likely to be answered using *quantitative* research?

1. What percentage of the population experience side-effects from a medication?\tightlist
`r if( knitr::is_html_output() ) {mcq( c("Qualitative", answer="Quantitative") )}`
1. What is the average number of roof-top solar panels on domestic properties?
`r if( knitr::is_html_output() ) {mcq( c("Qualitative", answer="Quantitative") )}`
