...
Spontaneous generation was proposed to explain what had been *observed*: when the above process was effected, mice were indeed observed.
...
Scientific research systematically answers questions by understanding the world using *data*; that is, science seeks *evidence-based answers*.
...
`r if( knitr::is_latex_output() ) {
   ...
   *Design*; *Summarise and describe*; *Analyse*; *Ask*; *Collect*; *Report*."
}`
...
```{r SixSteps, fig.cap="", fig.align="center", fig.width=3, out.width="35%", fig.cap="The six basic steps in research. The steps are not always performed in a linearly."}
SixSteps()
```
...
* Spreadsheets may *automatically alter the data* (for example, incorrectly reformatting entries as dates), even when not appropriate [@ziemann2016gene].
...
:::{.example}
@zeeberg2004mistaken found Excel was corrupting data files used in genetic analysis.
They conclude (p. 1):
