...
## Introduction: toatl nitrogen {#Chap29-Intro}
...
A study [@lambie2021microbial] compared carbon (C) and nitrogen (N) in soils from irrigated and non-irrigated intensively grazed pasture sites.
...
The data are *paired* (Sect.\ \@ref(PairedIntro)), the *mean reduction* in total N in irrigated sites is of interest.
One purpose of the study was to determine:

> For intensively grazed pastures sites, is there a mean reduction in percentage nitrogen when sites are irrigated, compared to non-irrigated?
...
The parameter is $\mu_d$, the population mean *reduction* in total N percentage.


::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
Explaining *how* the differences are computed is important.  

The differences could be computed as the *Irrigated N* minus *Non-irrigated N* (the *reduction*), or the *Non-irrigated N* minus the *Irrigated N* (the *increase*).
...
The data should be summarised numerically and graphically, using software (Fig.\ \@ref(fig:Nitrogenjamovi)).
...
The RQ asks if the mean total nitrogen *reduces* in the *population* when sites are irrigated; that is, is the mean difference is zero, or less than zero, if the difference is defined as the total N in irrigated sites minus non-irrigated sites.
...
The *null* hypothesis is 'there is *no change* in total N, in the population':
...
Since the RQ asks specifically if total N *decreases*, the alternative hypothesis will be a [*one-tailed* hypothesis](#AboutHypotheses):

* $H_1$: $\mu_d < 0$ (one-tailed).

This hypothesis says that the mean reduction in the population is *less than* zero.
The alternative hypothesis is *one*-tailed because of the wording of the RQ.
Recall that the differences are defined as total N *reduction*.



```{r, fig.align="center", fig.cap="A histogram of the differences", fig.show="hold", out.width=c("48%","48%"), fig.with = 4, fig.height = 4}
hist(SoilN$Change,
...
for (i in 1:length(SoilN$Change)){
  lines( x = c(1, 2),
         ...
         lty = ifelse(SoilN$Change > 0, 1, 2),
         type = "b",
         pch = ifelse(SoilN$Change > 0, 1, 19))
}
```
...
However, the *sample* N reduction  will vary depending on which one of the many possible samples is randomly obtained, *even if* the mean reduction in the population is zero.
...
```{r WeightGainNormal2, fig.cap="The sampling distribution is a normal distribution; it shows how the sample mean reduction in total N varies in samples of size $n = 28$ when the population mean reduction is $0$", fig.align="center", fig.width=9.0, fig.height=2.5, out.width='95%'}
...
Using Tables also shows that the one-tailed $P$-value is small.
...
At the left of the output, the order implies the differences are found as `TotalCNonI` minus `TotalCI`.
...
> Moderate evidence exists in the sample (paired $t = 2.41$; one-tailed $P = 0.114$) of a mean reduction in percentage N  from non-irrigated to irrigated sites (mean reduction: $0.0282$%; $n = 28$; $95$% CI from $0.0042$% to $0.0522$%).

The wording implies the direction of the differences (by talking of *reductions*).
Statistically validity conditions should be checked; the validity conditions are given again in the next section.
...
For the nitrogen-data used above, the sample size is $n = 28$, so the test is statistically valid.
:::
