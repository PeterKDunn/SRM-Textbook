...
The plot is not linear, and the variation in the FEV increases as children get taller
...
While formulas exist to compute the value of $r$, they tedious to use.
We will used software to compute the value of $r$.
For the red deer data
`r if( knitr::is_latex_output() ) {
  '(Fig. \\@ref(fig:RedDeerScatter)),'
} else {
...
When expressed as a percentage, $R^2$ is the percentage of the variation in $y$ explained by using the linear relationship, rather than using the mean value of $y$ as an estimate of $y$ for all values of $x$.
