...
Consider the plot in Fig. \@ref(fig:LimeTreesScatter2), showing the diameter of small-leaved lime trees of various ages [@data:ForestBiomass2017; @mypapers:dunnsmyth:glms].
The plot is not linear, and the variation in the diameters seems to increase as trees get older.
A correlation coefficient is inappropriate.
:::


```{r LimeTreesScatter2, fig.cap="The age and diameter of small-leaved lime trees grown in Russia ($n = 385$)", fig.align="center", fig.width=5, fig.height=3.5}
data(lime)

plot(DBH ~ Age,
	las = 1,
  main = "Diameter of small-leaved lime trees\nplotted against age",
	xlab = "Age (in years)",
	ylab = "Diameter (in cm) at breast height", 
  cex = 0.8,
	pch = 19,
	xlim = c(0, 140),
	ylim = c(0, 40),
...
The scatterplot in Example \@ref(exm:LimeCor) shows that the relationship is non-linear, as least for larger ages.
In addition, the variation in diameter biomass *increases* for larger ages (for values of $x$ near $20$, the values of $y$ do not vary much, but for values of $x$ near $80$, the values of $y$ vary greatly). 
...
\]
From software, $r = 0.837$ and $P < 0.001$ (two-tailed), based on $n = 30$ dogs.
We write:
...
\centering%
```{r PuntPlotCor2, results='hide', fig.width=4.5, fig.height = 3.5, out.width='90%'}
