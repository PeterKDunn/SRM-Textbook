...
SD <- read.csv("Data/Sanddollars.csv")

SDsub <- subset(SD, SD.temperatures < 35)
...
In these plots, it does not matter which variable used on the horizontal axis and which is used on the vertical axis.
...
Using the jamovi output (Fig. \@ref(fig:GorillaCorrelationjamovi)), determine the value of $r$ and $R^2$.
Perform a hypothesis tests, and make a conclusion about the relationship between the chest beating and size of gorillas.
:::


```{r GorillaCorrelationjamovi, fig.cap="jamovi output for the gorilla data", fig.align="center", out.width=c("48%", "48%"), fig.show="hold"}
...
PS <- read.csv("Data/Possums.csv")

plot(Wgt ~ DEM, 
     data = subset(PS, Sex=="Male"),
     xlab = "Elevation (in m)",
