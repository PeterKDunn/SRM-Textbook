...
Pearson correlation coefficients only apply if the form is approximately *linear* (so checking with a scatterplot first).
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The value of $R^2$ is *never* negative, and is usually multiplied by 100 and hence expressed as a percentage.
:::


::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
Even the value of $R^2$ is never negative, you need to be careful when using your calculator!
With most calculators, entering `-0.5^2` returns `-0.25`.
The calculator interprets your input as meaning `-(0.25^2)`.
\smallskip

You need to enter `(-0.5)^2`, which will give the expected answer of `0.25`.
:::
...
Software conducts the hypotheses test.
...
Since taller dogs would also be expected to be longer, so we may ask:
...
\]
The correlation co-efficient is $r = 0.837$ and, from software, the two-tailed $P < 0.001$, based on $n = 30$ dogs.
We write:
...
No scatterplot is provided, so the test is statistically valid only if the relationship is approximately linear, and if the variation in STAI scores does not vary for different levels of work experience.
