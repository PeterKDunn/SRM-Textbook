...
# CIs and tests: comparing two independent means {#AnalysisTwoMeans}


<!-- Introductions; easier to separate by format -->
```{r, child = if (knitr::is_html_output()) {'./introductions/27-CIsTesting-TwoMeans-HTML.Rmd'} else {'./introductions/27-CIsTesting-TwoMeans-LaTeX.Rmd'}}
```
...
Both groups should be summarised too.
The information can be found using software (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)), and compiled into a table (Table\ \@ref(tab:SnakesNumericalTest)).
...
An error bar chart Sect.\ \@ref(ErrorBarCharts)), which compares the *sample means*, should also be produced (Fig.\ \@ref(fig:SnakesErrorbar), right panel).
...
if( knitr::is_latex_output() ) {
  Diff2Notation[1, ] <- c(	"$n_A$",
                           "$n_B$",
                           NA)
  Diff2Notation[2, ] <- c( "$\\mu_A$", 	
                           "$\\mu_B$",
                           "$\\mu_A - \\mu_B$")
  Diff2Notation[3, ] <- c( "$\\bar{x}_A$",		
                           "$\\bar{x}_B$",
                           "$\\bar{x}_A - \\bar{x}_B$")
  Diff2Notation[4, ] <- c(	"$s_A$", 			
                           "$s_B$",
                           NA)
  Diff2Notation[5, ] <- c(	"$\\displaystyle\\text{s.e.}(\\bar{x}_A) = \\frac{s_A}{\\sqrt{n_A}}$",
...
A useful way to compare the means of two (or more) groups is to display the CIs for the means of the groups being compared in an *error bar chart*.
...
$$
the same value shown in the software output (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)).

The sampling distribution describes how the values of $\bar{x}_N - \bar{x}_C$ vary from sample to sample. 
Then, finding a $95$%\ CI for the difference between the mean SVLs is similar to the process used in Chap.\ \@ref(OneMeanConfInterval), since the sampling distribution has an approximate normal distribution:\index{Confidence intervals!comparing two means}
$$
...
> The difference between mean SVLs is\ $3.52$\cms, shorter for those living in a crayfish region (mean: $34.17$\cms; s.e.: $2.112$; $n = 35$) compared to those *not* living in a crayfish region (mean: $42.57$\cms; s.e.: $1.216$; $n = 41$), with an *approximate* $95$%\ CI for the difference between mean reaction times from $3.52$ to $13.27$\cms.
...
Output from software often shows *two* CIs (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)).
...
Specifically, the information in the first row (`Student's t`) is appropriate when the *population* standard deviations in the two groups *are* the same; the second row (`Welch's t`) is appropriate when the *population* standard deviations in the two groups *are not* necessarily the same.
...
From the output, the $95$%\ CI is from\ $3.511$ to\ $13.28$\cms
...
The *sampling distribution of the difference between two sample means*\ $\bar{x}_A$ and\ $\bar{x}_B$ is (when the appropriate conditions are met; Sect.\ \@ref(ValidityTwoMeansHT)), described by:\index{Sampling distribution!difference between two means}
...
The value of the standard error for the difference between the means is
$$
  \text{s.e.}(\bar{x}_A - \bar{x}_B) = \sqrt{ \text{s.e.}(\bar{x}_A)^2 + \text{s.e.}(\bar{x}_B)^2},
$$
though this value will often be given (e.g., on computer output).
...
* with a standard deviation, called the *standard error of the difference between the means*, of
$$
...
$$
   ...
   The answer agrees with the *second row* of the software output (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)).
...
The software output (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)) gives the $t$-score, but the $t$-score can also be computed using the information in Table\ \@ref(tab:SnakesNumericalTest): \index{Hypothesis testing!difference between two means}\index{Test statistic!t@$t$-score} 
\begin{align*}
   t
  &= 
...
The conclusion contains an *answer to the RQ*, the *evidence* leading to this conclusion ($t = 3.44$; two-tailed $P = 0.0011$), and *sample summary statistics*, including a CI (using the ideas in Chap.\ \@ref(CITwoMeans)).


## Statistical validity conditions{#ValidityTwoMeansHT}
...
Statistical validity can be assessed using these criteria:\index{Statistical validity!difference between two means}
...
For the garter-snakes data, the samples sizes exceed\ $25$ ($41$ and\ $35$), so the test is statistically valid.
The data in each group do not need be normally distributed, since both sample sizes are larger than\ $25$, and the data are not severely skewed (Fig.\ \@ref(fig:SnakesErrorbar), left panel).
:::
...
This requires a different method, called *analysis of variance*\index{Analysis of variance} (or ANOVA).
The details are are beyond the scope of this book.
In this section, a very brief overview of using a one-way ANOVA is given, using an example.
...
An error bar chart can be used to display the three groups (Fig.\ \@ref(fig:BMIerrorbar)).\index{Graphs!error bar charts}\index{Error bar charts}
...
This hypothesis encompasses many possibilities: for example, that the three means are *all* different from each other, or that the first is different than the other two (which are the same).
...
Performing an ANOVA using software (Fig.\ \@ref(fig:BMIjamovi)) gives $P = 0.00007$.
...
However, this approach increases the probability of declaring a false positive (i.e., or making a Type\ I error; Sect.\ \@ref(TypeErrors)):\index{Type\ I error} *incorrectly* declaring a difference between two sets of means.
...
```{r BMIjamovi, echo=FALSE, fig.cap="Software output for the BMI data.", fig.align="center", out.width='55%'}
...
The data are summarised in Table\ \@ref(tab:SignageSummary), where
$$
...
Table\ \@ref(tab:SignageSummary) gives the standard error for estimating this *difference* as $\text{s.e.}(\bar{x}_B - \bar{x}_A) = 2.964$.
Using the summary information in Table\ \@ref(tab:SignageSummary), the $t$-score (using Eq.\ \@ref(eq:tscore)) is
$$
  t
...
> Moderate evidence exists in the sample ($t = 1.92$; one-tailed $P = 0.030$) that mean speeds have reduced after the addition of extra signage (before: mean speed: $98.02$\kms.h^$-1$^; $n = 38$; standard deviation: $13.19$\kms.h^$-1$^; after:  mean speed: $92.34$\kms.h^$-1$^; $n = 41$; standard deviation: $13.13$\kms.h^$-1$^; 95%\ CI for the difference: $-0.24$ to\ $11.6$\kms.h^$-1$^).

Whether the mean speed reduction of $5.68$\kms.h^$-1$^ has *practical importance* is a different issue.\index{Practical importance}
...
This is equivalent to treating the differences for both groups like two separate sets of data in the two-sample test.
...
`r if( knitr::is_html_output() ) { longmcq( c( 
  ...
  "error bar chart") )}`
1. True or false: The difference in population means could be denoted by $\mu_V - \mu_C$, where\ $V$ represent the vegan diet, and\ $C$ represents the conventional diet.  
`r if( knitr::is_html_output() ) { torf( answer = TRUE )}`
1. True or false: The standard error of the difference between the sample means is denoted by $\text{s.e.}(\bar{x}_V) - \text{s.e.}(\bar{x}_C)$.  
...
1. The sample size is missing from the *Difference* row. \tightlist
   What should the sample size be in this row?
`r if( knitr::is_html_output() ) {longmcq( c(
  "46 + 47 = 93",
  "46 - 47 = -1",
  "47 - 46 = 1",
...
1. The standard deviation is missing from the *Difference* row. 
...
1. The standard error is missing from the *Difference* row. 
...
::: {.exercise #TwoMeansCISamplingDistReaction}
Sketch the sampling distribution for the difference between the mean reaction times for students using their phone while driving, and students not using their phone while driving.
:::


::: {.exercise #TwoMeansCISamplingDistSignage}
Sketch the sampling distribution for the difference between the mean speeds before and after adding extra signage, in Sect.\ \@ref(SpeedSignageCI).
:::
...
```{r NHANESTest, fig.cap="Software output for the NHANES data.", fig.align="center", out.width="90%"}
knitr::include_graphics("jamovi/NHANES/NHANES-DirectHDL-Smoke-ALL.png")
```


::: {.exercise #MeansIndSamplesExercisesEchinaceaHT}
@data:barrett:echinacea studied the effectiveness of echinacea to treat the common cold, and compared the mean duration of the cold for participants treated with echinacea or a placebo.\index{Placebo}
...
1. Is the difference likely to be of practical importance?
1. Write the hypotheses to answer the RQ.
1. Compute the $t$-score, and approximate the $P$-value using the $68$--$95$--$99.7$ rule.
1. Write a conclusion.
1. Are the CI and test statistically valid?
...
1. Using the software output (Fig.\ \@ref(fig:DentaljamoviCI)), write down and interpret the CI.
...
1. Construct an appropriate numerical summary from the software output (Fig.\ \@ref(fig:FallFowardTTestjamovi2)).
...
1. Write a conclusion.
1. Are teh CI and  test statistically valid?
:::
...
1. Consider comparing the mean grip strength for the two groups of paramedics.
   ...
   b. Compute an approximate $95$%\ CI for the difference in the mean grip strength for the two groups of paramedics.
   c. Compute the $t$-score for testing if a difference exists between the two types of paramedics.
   d. Approximate the $P$-value using the $68$--$95$--$99.7$ rule.
   e. Discuss the conditions required for statistical validity in this context.
   f. Make a conclusion.
1. Consider comparing the mean number of push-ups completed in one minute. 
   ...
   a. Carefully write down the hypotheses.
   b. Compute an approximate $95$%\ CI for the difference between the number of push-ups for the two groups of paramedics.
   c. Compute the $t$-score for testing if a difference exists between the two types of paramedics.
   d. Approximate the $P$-value using the $68$--$95$--$99.7$ rule.
   e. Discuss the conditions required for statistical validity in this context.
   f. Make a conclusion.
...
1. Determine the mean *gain* in each individuals girls weight using software.
...
Some of the summary information regarding the number decayed, missing and filled teeth (DMFT) is shown in Table\ \@ref(tab:CoeliacDMFTCI).
...
**3.** $0.5197$.
**4.** Some evidence population means are different.
:::
`r if (knitr::is_html_output()) '-->'`
