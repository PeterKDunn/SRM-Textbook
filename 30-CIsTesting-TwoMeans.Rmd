...
No sample size or standard deviation is provided for the differences in Table\ \@ref(SnakesNumericalTest); these make no sense in the context of comparing two means.
:::
...
```{r LimeTreesjamovi, echo=FALSE, fig.cap="Software output for testing hypotheses for the lime-trees data.", fig.align="center", out.width='55%'}
knitr::include_graphics( "jamovi/Lime/LimeANOVA.png")
```
...
Limetab <- array(dim = c(3, 4))

Limetab[, 1] <- tapply(Lime$Foliage, 
                      list(Lime$Origin), 
                      "mean")
Limetab[, 2] <- tapply(Lime$Foliage, 
                      list(Lime$Origin), 
                      "sd")
Limetab[, 3] <- tapply(Lime$Foliage, 
                      list(Lime$Origin), 
                      "findStdError")
Limetab[, 4] <- tapply(Lime$Foliage, 
                      list(Lime$Origin), 
                      "realLength")

rownames(Limetab) <- names(table(Lime$Origin))

if( knitr::is_latex_output() ) {
  kable( pad(Limetab,
             surroundMaths = TRUE,
             targetLength = c(4, 4, 5, 3),
             decDigits = c(2, 2, 3, 0)),
         format = "latex",
         booktabs = TRUE,
         longtable = FALSE,
         ...
         col.names = c("Mean",
                       ...
                       "$n$"),
         align = "c",
         caption = "Foliage biomass of lime trees (in kg) from different origins.") %>%
...
```{r TeaSummaryAnnotated, fig.cap="The chamomile-tea study has two within-individuals comparisons, and a between-individuals comparison (comparing the differences in each group).", fig.align="center", out.width="100%"}
...
```{r Dentaljamovi, fig.cap="Software output for the sugar-consumption data; the Groups refer to whether the country is industrialised (Yes) or not (No).", fig.align="center", out.width="95%"}
...
```{r BodyTempGenderjamoviHT, fig.cap="Software output for the body-temperature data.", fig.align="center", out.width="100%"}
...
knitr::include_graphics( "jamovi/BMI/BMIANOVA.png")
```
