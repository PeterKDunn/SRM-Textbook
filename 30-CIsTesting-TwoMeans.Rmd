...
However, the error bar chart, and these CIs, do not give a CI for the *difference* between the two means, as is relevant to the RQ.
...
$$
though this value will often be *given* (e.g., on computer output) rather than needed to be computed.
:::
...
From the output, the $95$%\ CI for teh difference is from\ $3.51$ to\ $13.28$\cms
...
(This is very similar to the $95$% CI shown in Fig.\ \@ref(fig:SpeedjamoviCI20).)
...
Since a negative reduction is an increase, this is more easily understood as the difference being located between a $0.25$\kms.h^$-1$^ *increase* before the signage was added to a\ $11.64$\kms.h^$-1$^ *reduction* after the signage was added.
...
The total glucose (TG) was measured for each individuals in both groups, both before the intervention and after eight weeks on the intervention.
...
This is equivalent to treating the differences for both groups as the two separate sets of data in the two-sample test.
...
`r if( knitr::is_html_output() ) { longmcq( c( 
  ...
  "error bar chart") )}`
1. True or false: The difference in population is denoted by $\mu_V - \mu_C$, where\ $V$ represent the vegan diet, and\ $C$ represents the conventional diet.  
...
1. The sample size is missing from the *Difference* row. \tightlist
   What is teh sample size for the difference?
`r if( knitr::is_html_output() ) {longmcq( c(
  "46 + 47 = 93",
  "46 - 47 = -1",
  "47 - 46 = 1",
...
[*Dataset*: `NHANES`]
Earlier, the NHANES study (Exercise\ \@ref(exr:CompareQuantExercisesNHANES)) was used to summarise the data used to answer this RQ:
...
```{r NHANESTest, fig.cap="Software output for the NHANES data.", fig.align="center", out.width="95%"}
...
**3.** False.
**4.* False.
**5.** A sample size makes no sense.
**6.** A standard deviation makes no sense
**7.** $0.5197$.
**8.** Some evidence population means are different.
:::
`r if (knitr::is_html_output()) '-->'`
