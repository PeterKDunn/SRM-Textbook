...
Using this definition, the result of rolling a die is a 'random procedure', with outcomes `r include_graphics("Dice/die1.png", dpi=1250)`, `r include_graphics("Dice/die2.png", dpi=1250)`, `r include_graphics("Dice/die3.png", dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)`, `r include_graphics("Dice/die5.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`.
...
The event 'rolling a `r include_graphics("Dice/die1.png", dpi=1250)` is a simple event.
:::
...
* Rolling a `r include_graphics("Dice/die4.png", dpi=1250)`: 
  This *simple event* includes one element of the sample space: `r include_graphics("Dice/die4.png", dpi=1250)`.
* Rolling a even number: 
  This *compound event* includes three elements of the sample space: `r include_graphics("Dice/die2.png", dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`
* Rolling a number larger than `r include_graphics("Dice/die2.png", dpi=1250)`: 
  This *compound event* includes four elements of the sample space: `r include_graphics("Dice/die3.png", dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)`, `r include_graphics("Dice/die5.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`.
:::
...
Events of interest are often combinations of other events, often combined using **and**, **or**, **not**.
Consider two events called\ $A$ and\ $B$.
Then, '$A$ **and**\ $B$' is the event where $A$ and $B$ are *both* true.
'$A$ **or**\ $B$' is the event where $A$ is true, $B$ is true, or are both true.
...
Event $A$ comprises the simple events `r include_graphics("Dice/die2.png",dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`, and event $B$ comprises the simple events `r include_graphics("Dice/die3.png",dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`.

Then, the event '$A$ **and** $B$' includes all events in $A$ and *also* in $B$; that is, '$A$ **and** $B$' comprises the single simple event `r include_graphics("Dice/die6.png", dpi=1250)`.

Event '$A$ **or** $B$' include the events in $A$, the events in $B$, and those in both; that is, '$A$ **or** $B$' comprises the four simple events `r include_graphics("Dice/die2.png", dpi=1250)`, `r include_graphics("Dice/die3.png", dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`.

The event '**not**\ $A$' comprises the three simple events `r include_graphics("Dice/die1.png", dpi=1250)`, `r include_graphics("Dice/die3.png", dpi=1250)` and `r include_graphics("Dice/die5.png", dpi=1250)`.
:::
...
\[
   ...
   = \frac{\text{The number of results that are a \includegraphics[width=0.167in]{Dice/die4.png}}}{\text{The number of possible results}}
   = \frac{1}{6}.
\]
...
<!-- In the LaTeX code: the die images are 208 x 208 or thereabouts. -->
<!-- So, since the in-text uses a dpi of 1250, the equivalent in inches is 209/1250 -->
...
With six equally-likely results (Example\ \@ref(exm:SampleSpaceDie)), each with probability $1/6$ (or, approximately, $16.67$%) of occurring, the probability of rolling a `r include_graphics("Dice/die4.png", dpi=1250)` is $1/6$.
:::
...
The answer can also be expressed as a *percentage*: 'the probability of rolling a `r include_graphics("Dice/die4.png", dpi=1250)` is (approximately) $16.67$%'.\index{Percentages}
The answer could also be interpreted as 'the *expected* proportion of rolls that are a `r include_graphics("Dice/die4.png", dpi=1250)` is (approximately) $0.1667$'.\index{Proportions}
That is, about $16.67$% of a very large number of future rolls are likely to be a `r include_graphics("Dice/die4.png", dpi=1250)`.
...
Consider rolling a standard six-sided die.
There are six equally-likely results (Example\ \@ref(exm:SampleSpaceDie)) each with probability $1/6$ (or $16.67$%) of occurring.
The probability of rolling a `r include_graphics("Dice/die1.png", dpi=1250)` **or** a `r include_graphics("Dice/die2.png", dpi=1250)` is $2/6$ (or $33.33$%).
:::
...
\index{Probability!relaticve frequency approach}
...
The *proportion* of boys born in 2021 is $148\ 636\div 289\ 603 = 0.513$, or about $51.3$% (without using any other information).
...
Based on the earlier information, the *odds* that a new baby will be a boy is *approximately* $0.513\div (1 - 0.513) = 1.053$.
According to the 
`r if (knitr::is_latex_output()) {
...
Table\ \@ref(tab:SkipBreakfast2) comes from a study of Iranian children aged 6--18 years [@data:kelishadi2017:snack].\label{thinkBox:FindProbs}
...
A research study conducted in Brisbane [@data:Dexter2019:SunProtection] recorded the number of people at the foot of the Goodwill Bridge, Southbank, who wore sunglasses between $11$:$30$am to $12$:$30$pm (Table\ \@ref(tab:SunglassesTableProb)).
...
Since the data are found from a single sample, taken at on one day at one location, so we cannot be sure about any conclusions more generally.
:::
...
if( knitr::is_html_output() ) {
  kable(pad(SG.Table,
            ...
            digits = 0),
               ...
               longtable = FALSE,
               align = "c",
        caption = "Females and malea wearing sunglasses") %>%
    row_spec(0, bold = TRUE)
}
```
...
*Event A* is 'rolling a an *even* number', *Event B*  is rolling an *odd* number` and *Event C* is rolling a `r include_graphics("Dice/die2.png", dpi=1250)`.
...
1. What is the *probability* that I will roll a number larger than `r include_graphics("Dice/die2.png", dpi=1250)`? \tightlist
`r if( knitr::is_html_output() ) {mcq( c(
	"0",
	"1/6",
	"2/6",
  ...
  answer = "4/6",
	"5/6",
	"1"))}`
2. What are the *odds* of rolling a number smaller than `r include_graphics("Dice/die6.png", dpi=1250)`?
`r if( knitr::is_html_output() ) {mcq( c(
	"0",
	"1/6",
	"1/5",
...
Table\ \@ref(tab:EdTable) tabulates information about school children in Queensland in 2019 [@mypaper:Dunn:GLM-IEE].
...
**2.** $2/6$.
**3.** True.
**4.** $1/6$ (only the event `r include_graphics("Dice/die2.png", dpi=1250)`)
**5.** $4/6$ (the events `r include_graphics("Dice/die1.png", dpi=1250)`, `r include_graphics("Dice/die2.png", dpi=1250)`, `r include_graphics("Dice/die4.png", dpi=1250)` and `r include_graphics("Dice/die6.png", dpi=1250)`)
**6.** $3/6$ (the events `r include_graphics("Dice/die1.png", dpi=1250)`, `r include_graphics("Dice/die3.png", dpi=1250)` and `r include_graphics("Dice/die5.png", dpi=1250)`)
:::
`r if (knitr::is_html_output()) '-->'`
