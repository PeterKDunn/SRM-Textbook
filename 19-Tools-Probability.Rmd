...
Consider rolling a standard six-sided die.
There are six equally-likely results (Example\ \@ref(exm:SampleSpaceDie)) each with probability $1/6$ (or approximately $16.67$%) of occurring.
...
*Conditional probability* refers to adjusting probabilities when extra information is know.
...
is $1/6$ using the classical approach.
The sample space is 
`r if (knitr::is_latex_output()) {
   '\\Largedice{1}'
} else {
...
However, if we are told that the number rolled is a *odd number*, the sample space is now
`r if (knitr::is_latex_output()) {
   '\\Largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
since no other outcomes are possible, and so the probability or rolling a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
* *If the observed person is female*, the probability that she is wearing sunglasses is $126\div 366 = 0.3443$, and
* *If the observed person is male*, the probability that he is wearing sunglasses is $123\div 386 = 0.3187$.
