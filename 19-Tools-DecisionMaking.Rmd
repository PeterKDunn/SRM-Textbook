...
Every possible sample comprises different households, and the sample results depends on which households are in the studied sample.
...
1. *The two* population *odds are the same*.  
   The disparity between the two *sample* odds is due to *sampling variation*.
   That is, we just happen to have---by chance---one of those samples where the disparity between the two odds is noticeable.
2. *The two* population *odds are different*.  
   The *sample* odds simply reflect this.
...
Is this evidence that the pack of cards is somehow unfair, poorly shuffled, or rigged?
...
```{r Draw15Cards, fig.align="center", fig.width=7, out.width = "35%",fig.cap="How likely is it that you would get $15$ red cards in a row from a fair pack?" }
...
Of course, getting $15$ red cards in a row is *possible*... just very *unlikely*.
...
```{r DecisionFlow2, fig.cap = "A way to make decisions", fig.align="center", out.width='100%', fig.width = 9.25, fig.height = 5}
showDecisionMaking() 
```
...
None of these $1000$ simulations produces a value of $\hat{p}$ close to\ $1$.
...
From our sample of $15$ cards, all cards are red so the statistic is $\hat{p} = 1$ (i.e., $15$ red cards out of $15$ cards).
...
Observing $15$ red cards out of $15$ cards is rare: it never happened once in the $1000$ simulations
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:HandRedHist1000)),'
} else {
...
This would not seem unusual at all.
