...
```{r, child = if (knitr::is_html_output()) {'introductions/23-CIs-OneProportion-HTML.Rmd'} else {'introductions/23-CIs-OneProportion-LaTeX.Rmd'}}
```
...
Of course, the value of the sample proportion in $25$ rolls could be very small or very high by chance, but we wouldn't expect to see that very often.
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=6.5, out.width='65%', fig.cap="The proportion of rolls that are even, $\\hat{p}$, changes from one sample of $25$ rolls to the next sample of $25$ rolls, around a mean of $p = 0.5$. The solid line is the normal distribution used to model the sampling distribution." }
if (knitr::is_latex_output()){
  set.seed(99100991)
  num.rolls <- 25
  num.sims <- 1000
...
Let's pretend for the moment that the population proportion of even rolls on a die is *unknown* (to demonstrate ideas).
...
The unknown value of $p$ could be a little smaller or a little larger than the value of $\hat{p}$; the interval is
$$
...
\end{align*}
The interval, from $0.2414$ to $0.639$, is an interval containing values of $p$ that could have reasonably produced the observed value of $\hat{p}$
`r if (knitr::is_latex_output()) {
...
We can say the interval $0.2414$ to $0.639$ has a $95$% chance of straddling the population proportion\ $p$.
...
```{r pProducingpHatLATEX, fig.width=7.5, fig.height=8, out.width='100%', fig.cap="The CI gives an interval containing values of $p$ that may have produced the observed value of $\\hat{p}$. Here, the CI is $0.241$ to $0.639$, shown as the thick black line under the plots.", fig.align="center"}
if (knitr::is_latex_output()) {
  par(mfrow = c(2, 2),
      xpd = NA,
...
However, in this contrived example, the CI *does* straddle the known value of $p = 0.5$ with $95$% confidence.
...
Since we usually don't know the value of $p$, and we usually only have one sample (and hence one CI), in general *we never know whether the CI computed from the single sample we have straddles the value of $p$ or not*.
...
This is true if certain conditions are met.
If these conditions are met, the confidence interval is *statistically valid*.
