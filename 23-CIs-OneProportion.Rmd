...
$$
Previously, the sampling distribution was used to construct a sampling interval (using the $68$--$95$--$99.7$ rule) that could contain the value of $\hat{p}$.
However, here the value of $\hat{p}$ is known, so instead an interval is created for the unknown value of $p$.
Since the unknown value of $p$ could be a little smaller or a little larger than the value of $\hat{p}$, the interval is
$$
...
\end{align*}
The interval is from $0.2414$ to $0.639$.
This is an interval containing values of $p$ that could have reasonably produced the observed value of $\hat{p}$
`r if (knitr::is_latex_output()) {
...
```{r CIrelationshipsCI, out.width='75%', fig.align="center", fig.cap="A known value of $p$ produces a range of $\\hat{p}$ values.", fig.width = 7, fig.height=3.75}
par(mar = c(1, 1, 5.5, 1) + 0.5 )   
...
A CI is *statistically valid* if the conditions for the underlying mathematical calculations and assumptions to be approximately correct are met, such as the sampling distribution having an approximate normal distribution.
:::
