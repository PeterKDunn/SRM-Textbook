...
The sample proportions would be expected to vary around $p = 0.5$ (the *population proportion*): some larger than $p$ and some smaller than $p$.
...
Suppose a fair die was rolled $25$ times, and this was repeated *thousands* of times, and the proportion of even rolls was recorded for every one of those thousands of sets of $25$ rolls.
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=6.5, out.width='65%', fig.cap="The proportion of rolls that are even, $\\hat{p}$, is not the same for every sample of $25$ rolls; it varies around a mean of $p = 0.5$. The solid line is the normal distribution used to model the sampling distribution." }
if (knitr::is_latex_output()){
  set.seed(99100991)
  num.rolls <- 25
  num.sims <- 1000
...
For instance, values of $\hat{p}$ less than $0.2$, or greater than $0.8$ are unlikely to be observed from a fair die (with $p = 0.5$).


```{r NormalDieTheory, fig.cap="The sampling distribution is an approximate normal distribution; it shows a model of how the proportion of even rolls varies when a die is rolled $25$ times.", fig.align="center", fig.width=8.25, fig.height=3.25, out.width='100%'}
pop.p <- 0.5
n <- 25
...
## Confidence intervals for $p$: for $p$ unknown {#ConfIntPUnknownP}
...
The quantity $\big( \text{multiplier} \times \text{s.e.}(\hat{p})\big)$ is called the *margin of error*.
:::
...
Of course, value of $p$ is usually unknown, so we never know if the CI computed from our single sample contains $p$ or not.
