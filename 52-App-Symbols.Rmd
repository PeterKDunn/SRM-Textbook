...
* If the test-statistic is a $t$-scores, $P$-value can be approximated using
`r if (knitr::is_latex_output()) {
   '(Appendices\\ \\@ref(ZTablesNEG) and\\ \\@ref(ZTablesPOS))' 
} else {
   '(Appendix\\ \\@ref(ZTablesOnline))'
...
For statistics with an approximate normal distribution, *confidence intervals* have the form
$$ 
...
* To *calculate $z$-scores* (Sect.\ \@ref(zScores)), use\index{zscores@$z$-scores} 
$$
...
$$
  $t$-scores are like $z$-scores.\index{tscores@$t$-scores}
