...
* When the sampling distribution is approximately normally distributed, under certain statistical validity conditions, this is indicated by\ \ding{51}.
* For the mean of the sampling distribution (the *sampling mean*)\index{Sampling mean}:

  - for *confidence intervals*, the sampling mean value is unknown.
  - for *hypothesis tests*, the (assumed) sampling mean value is given in the null hypothesis.
...
#                 "$p_1 - p_2$",
                 "$\\hat{p}_1 - \\hat{p}_2$", 
                 "$p_1 - p_2$",
                 "$\\displaystyle \\sqrt{\\text{s.e.}(\\hat{p}_1) + \\text{s.e.}(\\hat{p}_2)}$", 
                  ifelse( knitr::is_latex_output(), "\\ding{52}", "&#10004;"),
...
For statistics that have an approximate normal distributions, **confidence intervals** have the form  
\[ 
...
For statistics that have an approximate normal distributions, the *test statistic* has the form:  
\[
  \frac{\text{statistic} - \text{parameter}}{\text{s.e.}(\text{statistic})}.
\]
This quantity is a $t$-score for most hypothesis tests in this book.
However, if the *standard error* does not involve any sample estimate, this quantity is a $z$-score.
This is the case for a hypothesis test for a *proportion*.
...
* When the sampling distribution for the statistic does not have an approximate normal distribution (e.g., for *odds ratios* and *correlation coefficients*), **this formula does not apply**.
* A hypothesis test about **odds ratios** uses a $\chi^2$ test statistic, whose value is like a $z$-score with a value of  
\[
   \sqrt{\frac{\chi^2}{\text{df}}},
\]
where $\text{df}$ is the 'degrees of freedom', as given in the software output.
...
* To **calculate $z$-scores** (Sect.\ \@ref(zScores)):   
\[
...
* The **degrees of freedom** for a two-way table of counts:  
\[
...
SymbolsTab[1, ] <- c("$s$", 
                     ...
                     "(ref:VariationStdDev)")
SymbolsTab[2, ] <- c("$\\sigma$", 
                     "Population standard deviation", 
                     "(ref:VariationStdDev)")
SymbolsTab[3, ] <- c("$s_d$", 
                     "Sample standard deviaton of differences", 
                     "(ref:VariationStdDev)")
SymbolsTab[4, ] <- c("$\\sigma_d$", 
...
if( knitr::is_latex_output() ) {
  kable( SymbolsTab,
         ...
         escape = FALSE,
         align = c("c", "l", "c"),
         linesep = c("", "\\addlinespace", "",  "\\addlinespace", "\\addlinespace", "", "\\addlinespace", "", "")) %>%  # Else adds a space every 5 lines...
