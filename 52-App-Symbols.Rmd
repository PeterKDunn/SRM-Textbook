...
ParStat[6, ] <- c("$\\mu_1 - \\mu_2$", 
                  "$\\bar{x}_1 - \\bar{x}_2$", 
                  "$\\displaystyle\\text{s.e.}(\\bar{x}_1 - \\bar{x}_2)$: given", 
                  "--")
ParStat[7, ] <- c("Pop. OR", 
                  "Sample OR", 
                  "$\\displaystyle\\text{s.e.}(\\text{sample OR})$: given", 
                  "--")
ParStat[8, ] <- c("$\\rho$", 
                  "$r$", 
                  "", 
...
The sample statistics have an approximate normal distribution (when certain statistical validity conditions are met), where the mean of the distribution is the value of the parameter, and the standard deviation of the distribution is given by the standard error of the parameter.

* The sampling distribution of the sample mean:  
  normal distribution, with mean $\mu_{\bar{x}} = \mu$ and standard deviation $\text{s.e.}(\bar{x})$ as given in the table.
* The sampling distribution of the sample proportion:  
  normal distribution, with mean $\mu_{\hat{p}} = p$ and standard deviation $\text{s.e.}(\hat{p})$ as given in the table.
* The sampling distribution of the sample mean difference:  
  normal distribution, with mean $\mu_{\bar{d}} = \mu$ and standard deviation $\text{s.e.}(\bar{d})$ as given in the table.
* The sampling distribution of the difference between two means:  
  normal distribution, with mean $\mu_{\bar{x}_A - \bar{x}_B} = \mu_A - \mu_B$ and standard deviation $\text{s.e.}(\bar{x}_A - \bar{x}_B)$ as given in the table.
