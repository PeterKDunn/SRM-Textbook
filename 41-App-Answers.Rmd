...
#        escape=FALSE,
      ...
      #            "\\addlinespace", "", "", ""), # Otherwise addes a space after five lines... 
      align= c("r", "r"),
      caption=tab.cap) %>%
    #column_spec(1, bold=TRUE) %>%
    row_spec(0, bold=TRUE) %>%
  pack_rows("Grip strength (in kg)", 2, 4, bold = TRUE, italic = FALSE) %>%
  pack_rows("Push-ups (per minute)", 5, 7, bold = TRUE, italic = FALSE) %>%
    kable_styling(font_size=9)
}
if( knitr::is_html_output() ) {
  out <- kable(ParaTable, 
...
colnames(PhoneTable) <- c("Answer call", "Respond to text", "Reply to email")
rownames(PhoneTable) <- c("Low exposure", "High exposure")
