...
If the wheel is spun (say) 15 times, the *sample* proportion of odd-spins in those 15 spins, denoted $\hat{p}$, will vary.
...
```{r RouletteWheel, echo=FALSE, fig.cap="A European roulette wheel", cache=FALSE, out.width = "25%", fig.align="center", fig.height=2.5,fig.width=2.25}
...
The value of a sample statistic can vary for every possible sample that we could select.
However, we only observe the sample statistic from one sample.
The actual value of the sample statistic that we observe depends on which one of those countless samples we have.
...
Perhaps surprisingly, under certain conditions, the sampling distribution is often a *normal distribution*.
If the sampling distribution is a normal distribution, it is reasonable to ask what the value of the *standard deviation*  of that normal distribution is; that is, how much the value of the sample statistic might vary across all of the possible samples.
...
If every possible sample (of a given size) was found, and the statistic computed from each sample, the standard deviation of these estimates is the *standard error*.
...
In research studies, *standard deviations* are used to describe the variation in individual observations (for *quantitative* data): how observations vary from *individual to individual*.
...
For example, the '[standard error of the sample mean](#SamplingDistributionMeans)' is often written as $\text{s.e.}(\bar{x})$, and the '[standard error of the sample proportion](#SamplingDistributionProportions)' is often written as $\text{s.e.}(\hat{p})$.
:::
...
A **sampling distribution** describes how the value of all possible sample statistic is likely to vary from sample to sample.
...
- Sect. \ref{thinkBox:SpinMean}: From Fig. \ref{fig:RouletteWheelHist} (bottom-left), highly unlikely from a fair wheel.
