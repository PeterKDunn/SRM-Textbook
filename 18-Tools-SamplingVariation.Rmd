...
```{r RouletteWheel, echo=FALSE, fig.cap="A European roulette wheel", cache=FALSE, out.width = "45%", fig.align="center", fig.height=2.75,fig.width=2.5}
...
If the wheel was spun (say) $n = 40$ times (rather than $15$ times), $\hat{p}$ again varies (Fig. \@ref(fig:RouletteWheelHist), top right panel): the values of $\hat{p}$ vary from sample to sample.
...
The value of a sample statistic can vary for every possible sample that we could select, so the actual value of the sample statistic that we observe depends on which one of those countless samples we have.
...
- Sect. \ref{thinkBox:SpinMean}: From Fig. \ref{fig:RouletteWheelHist} (bottom-left panel), highly unlikely from a fair roulette wheel... 
- Sect. \ref{thinkBox:SpinMean} From Fig. \ref{fig:RouletteWheelHists} (bottom-left panel), highly unlikely from a fair roulette wheel...
