...
Graphs are useful this purpose, and a table of the numerical summaries usually are also produced.
...
```{r GorillasDoubleStem, fig.width=7.0, fig.height=3, out.width='75%', fig.cap="Stemplot for the chest-beating rate for gorillas", fig.align="center"}
...
Rather, how the the dots *compare* in the groups is the main feature of interest.
:::
...
The distribution for each group is summarised by five numbers: the minimum value; the first quartieel ($Q_1$); the median ($Q_2$); the third quartile ($Q_3$); and the maximum value.
...
1. *Top line*:
   The *fastest* chest-beating rate (largest value) is $`r CBRyounger.quantiles[5]`$ per $10$\ h.
...
5. *Bottom line*:
   The *slowest* chest-beating rate (smallest value) is $`r CBRyounger.quantiles[1]`$ per $10$\ h. 
...
Computer software has identified this observation as a potential (large) *extreme outlier* using the IQR rule (Sect.\ \@ref(OutliersIQRrule)), and has plotted this point separately.
