...
<!-- ```{r BoxplotGorillasYoungHTML, fig.cap="A boxplot for chest-beating data; the boxplot just for the younger gorillas", fig.width=5, fig.height=4, fig.align="center"} -->
<!-- if (knitr::is_html_output()) { -->
...
<!--              col = plot.colour, -->
<!--              las = 1, -->
<!--              main = "Boxplot of chest-beating rate\n(younger gorillas)", -->
<!--              ylim = c(-1, 6), -->
<!--              ylab = "Number of eggs") -->
<!--     if ( i == 2 ){ -->
<!--       arrows(left, Qs[2], -->
...
The graphs (Fig.\ \@ref(fig:WaterAccessCompareQuantFigs)) and summary (Table\ \@ref(tab:WaterAccessCompareQuantTabs)) show that households in which diarrhea was found in the last two weeks in children had older coordinators, more in the household, and more children under\ $5$ in the household. 
...
```{r WaterAccessCompareQuantFigs, fig.align="center", fig.cap="Some plots for the water access data", out.width = '100%', fig.width=6, fig.height=2.25}
data(WaterAccess)
par( mfrow = c(1, 3),
...
boxplot(Age ~ Diarrhea,
     ...
     las = 1,
     names = c( expression(N*":"~italic(n)==59),
                expression(Y*":"~italic(n)==26))
     )
...
if( knitr::is_latex_output() ) {
  ...
  column_spec(0, bold = TRUE) %>%     # Row headings in bold
    pack_rows("Woman's age (in years)",
              start_row = 1,
              end_row = 3,
              bold = TRUE) %>%
...
Selected answers are available in Sect.\ \@ref(CompareQuantData-Exercises).
