# Relationships: qualitative data comparisons between individuals {#CompareQualData}
...
:::{.example #SmallKidneyStones name="Two-way tables"}
...
Each variable in a two-way table can be analysed separately, using percentage, proportions or odds (Sect.\ \@ref(QualitativeNumbers)).
For example, the two variables in Table\ \@ref(tab:KS-Small) (Method; Result) can be analysed separately, using percentages, proportions or odds (Sect.\ \@ref(QualitativeNumbers)).
For instance:
...
::: {.example #SummaryTableCompareQual name="Large kidney stones"}
...
give the proportion of successes for each *Method*, since the rows represent the counts for Methods\ A and\ B.\index{Proportions}
...
* Method\ A: The odds of success is $81/6 = 13.5$. 
  This means there are $13.5$ more successful procedures than failures for Method\ A.
* Method\ B: The odds of success is $234/36 = 6.5$.
  This means there are $6.5$ more successful procedures than failures for Method\ B.

This shows that the odds of a success is far greater for Method\ A then Method B.
:::
...
::: {.example #KidneyStonesSmallColums name="Comparing by column"}
...
* Successes: The odds of a success coming from Method\ A is $81/234 = 0.346$. 
  This means there are $0.346$ more Method\ A procedures than Method\ B procedures among the successes.
* Failures: The odds of failure coming from Method\ A is $6/36 = 0.167$.
  This means there are $0.167$ more Method\ A procedures than Method\ B procedures among the failures.
...
::: {.example #BarStacked name="Stacked bar charts"}
...
::: {.example #BarSideBySide name="Side-by-side bar charts"}
...
::: {.example #BarSideBySide2 name="Side-by-side bar charts"}
...
Using a computer to create graphs is recommended, and using a computer makes it easy to try different variations to find the grah that best displays the message in the data.
...
\index{Odds ratio!interpreting}
Our computation of the odds ratio is the same used by jamovi, SPSS and many other statistical programs.
...
::: {.example #GorillaComparisonTable name="Numerical summary table"}
...
if( knitr::is_latex_output() ) {
  kable(pad(KidneySmall.Sum,
        ...
        col.names = c("success", 
                      "success", 
                      "size"),
        caption = "Numerical summary of the small kidney-stone data: Odds and percentage of a successesful procedure") %>%
    row_spec(0, bold = TRUE) %>%
    kable_styling(font_size = 8) %>%
...
As for small kidney stones, the *success percentages* can be computed for large kidney stones for Methods\ A and\ B (i.e., row percentages):
...
However, random allocation was not possible in this study, so the researchers used a different method to manage confounding: *recording* the size of the kidney stones; see Sect.\ \@ref(ObsManagingConfounding).\index{Confounding}
...
This is called\index{Simpson's paradox}
`r if (knitr::is_latex_output()) {
   "*Simpson's paradox*."
} else {
...
The comparison in Fig.\ \@ref(fig:WaterAccessQualCompare) includes some categories with small sample sizes, so the percentages shown may not be precise estimates of the population values.
...
1. Compute the *sample proportion* of *urban* residents who use social media.\tightlist
`r if( knitr::is_html_output() ) {mcq( c(
	"416/984 = 0.423",
	answer = "568/984 = 0.577",
	"416/568 = 0.732",
	"568/416 = 1.365"))}`
1. Compute the *sample proportion* of *rural* residents who use social media.
`r if( knitr::is_html_output() ) {mcq( c(
	"78/89 = 0.876",
	"89/78 = 1.141",
	"78/167 = 0.467",
...
Selected answers are available in Sect.\ \@ref(CompareQualData-Answers).
...
1. Compute the *odds* that a wine-only drinker vomited.
...
1. Sketch some graphs to display the data.
1. What is the main message in the data?
...
1. Compute the *proportion* of males who skipped breakfast.
1. Compute the *odds* of a female skipping breakfast.
1. Compute the *odds* of a male skipping breakfast.
1. Compute the *odds ratio* comparing the odds of skipping breakfast, comparing females to males.
1. Interpret this OR.
