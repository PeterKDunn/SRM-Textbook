# Qualitative data: Comparing between individuals {#CompareQualData}
...
## Summarising by rows and columns {#RowPercentages}
...
These percentages are collated in
`r if (knitr::is_latex_output()) {
   'Table\\ \\@ref(tab:KidneyRowColLATEX) (left table).'
} else {
   'Table\\ \\@ref(tab:KS-Small-rowPC).'
...
Rather then comparing methods (in the rows), the procedure results can be compare (i.e., the columns).
...
* either variable can be used as the first division of the data.
...
The numerical summary information for comparig qualitative variables can be collated in a table, or a *tabular summary*.
...
if( knitr::is_latex_output() ) {
  kable(pad(KidneySmall.Sum,
        ...
        col.names = c("success", 
                      "success", 
                      "size"),
        caption = "Numerical summary of the small kidney-stone data: Odds and percentage of successes") %>%
    row_spec(0, bold = TRUE) %>%
    kable_styling(font_size = 10) %>%
...
That seems strange...
Method\ A performs better for small *and* large kidney stones... but Method\ B performs better when size is unknown (i.e., ignoring size).
...
However, random allocation was not possible in this study, so the researchers used a different method to manage confounding: *recording* the size of the kidney stones; see Sect.\ \@ref(ObsManagingConfounding).
...
WAKidsTab <- xtabs( ~ HasLivestock + Diarrhea,
                    data = WAkids)
rownames(WAKidsTab) <- c("No diarrhea reported in household",
                         "Diarrhea reported in household")
...
if( knitr::is_latex_output() ) {
  kable(pad(WAKidsTab,
        ...
        align = "c",
        digits = c(1, 3),
        col.names = c("No diarrhea in children", 
