...
```{r InsulationHistogram, echo=FALSE, fig.cap="A plot of weight changes. Left panel: A histogram (the vertical grey line represents no weight gain). Right panel: Case-profile plot (a dashed line represents a weight gain!!!!!!!!!!!!!!!!!)", fig.align="center", out.width='100%', fig.width=9.5, fig.height = 3.0}
par( mfrow = c(1, 2))
...
WtLoss.DataSummary <- array( dim=c(3, 4))

WtLoss.DataSummary[1, 1] <- mean(SW$InitialWt)
WtLoss.DataSummary[2, 1] <- mean(SW$FinalWt)
WtLoss.DataSummary[3, 1] <- mean(SW$GainWt)

WtLoss.DataSummary[1, 2] <- median(SW$InitialWt)
WtLoss.DataSummary[2, 2] <- median(SW$FinalWt)
WtLoss.DataSummary[3, 2] <- median(SW$GainWt)

WtLoss.DataSummary[1, 3] <- sd(SW$InitialWt)
WtLoss.DataSummary[2, 3] <- sd(SW$FinalWt)
WtLoss.DataSummary[3, 3] <- sd(SW$GainWt)

WtLoss.DataSummary[1, 4] <- IQR(SW$InitialWt)
WtLoss.DataSummary[2, 4] <- IQR(SW$FinalWt)
WtLoss.DataSummary[3, 4] <- IQR(SW$GainWt)

rownames(WtLoss.DataSummary) <- c("Week 1 weight", 
                                  "Week 12 weight", 
                                  "Weight gain")


if( knitr::is_latex_output() ) {
  kable(WtLoss.DataSummary,
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
        align = c("r", "r", "r", "r"),
...
\[
	0.8735 \pm (2 \times 0.1159764),
\]
or $0.8735\pm 0.2319528$ (so the *margin of error* is $0.232$).
Equivalently, the CI is from $0.8735 - 0.23195 = 0.642$, up to $0.8735 + 0.232195 = 1.11$.
We write:

> Based on the sample, an *approximate* 95% CI for the population mean weight gain between Week 1 and 12 is from $0.64$kg to $1.11$kg.
