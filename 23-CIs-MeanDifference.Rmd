...
*Paired data* refers to situations where each observation in one group is uniquely related to, or can be matched sensibly to, an observation in another group.
The two groups are *not* independent (Sect.\ \@ref(Independence)), and finding the differences between individual pair makes sense.

Since individuals can vary substantially, taking two measurements from each individual means that variables (potentially, other *confounding* variables) are held constant for those two observations.
...
A typical example is when a variable of interest is recorded on the same individuals *before* and *after* giving a drug.
However, paired data can arise in many ways:
...
* Heart rate is measured for each individual when are sitting and when standing.
...
* The body temperature of dogs is measured using two different types of thermometers.
  Each dog receives a pair of measurements, and the *difference* between the two recorded temperatures is recorded.
...
* Recording the species richness of numerous natural waterways before and after restoration projects.
  Each waterway has the species richness recorded both before and after, and the change in species richness is recorded for each waterway

Many of these examples can be extended to beyond two measurements.
For instance, the number of visitors at a national park can be record for every weekend in a  season, or temperatures can be compared using three different types of thermometers.
We only study pairs of measurements.

In this book, comparing pairs on a quantitative variable are considered.



:::{.example #PairedUnpaitred, name = "Paired and unpaired data"}
Treating data as paired or not. STD devs, plots

```{r}
data(StudentWt)
apply(StudentWt[, 2:4], 2, "sd")
```
:::


```{r ComparePaired, out.width='100%', fig.cap="Plots of the weight-loss data. Left: Treating the data incorrectly as not paired; centre: a boxplot of the weight gains; right: A case-profile plot of the weight gains.", fig.height = 2.5, fig.width = 7}
...
Prof. David Levitsky (Cornell University) obtained data to answer this (descriptive) RQ (@levitsky2004freshman, @DASL:WeightChange):
...
T1 <- knitr::kable(tb1,
                   ...
                   align = c("r", "c", "c", "r"),
                   linesep = "",
                   col.names = c("Student", "Week 1", "Week 12", "Wt gain"),
                   row.names = FALSE,
                   escape = FALSE,
                   booktabs = TRUE) %>%
...
The Week\ 1 and the Week\ 12 data can both be summarised.
Since the data are differences, a *numerical* summary must also summarise the *differences*, because the RQ is about the differences
`r if (knitr::is_latex_output()) {
   '(Table\\ \\@ref(tab:WeightGainSummary)).'
} else {
   '(see below).'
}`
For the weight-gain data, the appropriate *numerical summary* for paired quantitative data uses means, standard deviations, and so on, as appropriate.
...
A numerical summary of the weight gain (using a computer) gives the sample mean of the differences as $\bar{d} = -0.8735$, the standard deviation of the differences as $s_d = 0.9489378$, and the standard error as  
\[
...
(Notice that the standard deviation for the weight gain is **not** the difference between the Week\ ` and the Week\ 12 standard deviation; instead, it is the standard deviation of the weight gains of individuals.)
...
Many decimal places are used in the working; results will be rounded when reported.
...
> Based on the sample, an *exact* $95$% CI is for the population mean weight gain between Weeks\ 1 and\ 12 is between $0.63$ to $1.09$\ kg


```{r WeightGainTestOutput, fig.cap="The weight-gain data: jamovi output (top) and SPSS output(bottom)", fig.align="center", out.width=c("95%","85%"), fig.show="hold"}
