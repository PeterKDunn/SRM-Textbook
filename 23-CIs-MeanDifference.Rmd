...
The RQ would be a *descriptive RQ*: the *Outcome* is the mean *change* in weight, and the response variable is the weight *change* for each individual student.
...
> In the Colorado Rocky Mountains, what is the mean difference between first-flowering day for the native skypilot and encroaching willow?
...
```{r, FloweringPlots, out.width='90%', fig.width=9, fig.height=3.5, fig.align=TRUE, fig.cap="The flowering-day data. Left: A histogram of the difference between the first-flowering days (skypilot minus willow) ; right: a case-profile plot of days of first flowering (unfilled points and dashed lines indicate earlier dates (smaller values) for willow)"}
par( mfrow = c(1, 2))
...
## Example: green coffee bean extract {#CoffeeExtractCI}

A study of patients with non-alcoholic fatty liver disease (NAFLD) randomly allocated $21$ patients into a control group, and $21$ to receive green coffee extract (GCE) (@hosseinabadi2020effects, p.\ 2):

> Over eight weeks, patients received $2\times 200$\ mg GCE daily (one 200 mg capsule after breakfast and one after lunch), [or] a placebo capsule that was similar to GCE supplement in terms of dosage, color and size...

The fasting blood glucose (FBG) was measured for each individual, in both groups, both before the intervention and after eight weeks on the intervention.
The data are not available so a graphical summary cannot be produced; however, a data summary is given in the article (motivating Table\ \@ref(tab:GCBsummaryTable)).
The following descriptive RQ can be asked for *both* groups:

> For patients with NAFLD, is there a mean reduction in FBG after eight weeks?

For the *green coffee group*, the standard error of the *reduction* in FBG is $\text{s.e.}(\bar{d}) = 13.63/\sqrt{21} = 2.974$, so an approximate $95$% CI for the *reduction* in fasting blood glucose (FBG) is  
\[
  12.50\pm (2\times 2.974), \text{or from $6.55$ to $18.45$ mg.dl$^{-1}$}.
\]
For the *control group*, the standard error of the *reduction* in FBG is $\text{s.e.}(\bar{d}) = 12.543/\sqrt{21} = 2.737$, so an approximate $95$% CI for the *reduction* in fasting blood glucose (FBG) is  
\[
  2.00\pm (2\times 2.737), \text{or from $-3.47$ to $7.47$ mg.dl$^{-1}$}.
\]
The green coffee extract appears to reduce FBG; this is studied further in Sect.\ \@ref(CoffeeExtractHT).


```{r GCBsummaryTable}
GCBsummary <- array(dim = c(3, 7) )

rownames(GCBsummary) <- c("Green coffee beans",
                          "Control",
                          "Difference")
colnames(GCBsummary) <- c("n",
                          "BaselineMean",
                          "BaselinesSD",
                          "PostMean",
                          "PostSD",
...
if( knitr::is_latex_output() ) {
  ...
  knitr::kable(surroundMaths(GCBsummary,
                             decDigits = c(0, 2, 2, 2, 2, 2, 2)),
               format = "latex",
               align = c("r", "r", "r", "c"),
               linesep = "",
               caption = "The fasting blood glucose (in mg.dl$^{-1}$)",
               col.names = c("$n$",
                             "Mean", "Std. dev.", 
                             "Mean", "Std. dev",
                             "Mean", "Std.dev"),
...
> A approx.\ $95$% CI for the mean reduction in FBG is between $6.55$ to $18.45$ mg.dl$^{-1}$ for those taking green coffee extract, and between $-3.47$ to $7.47$ mg.dl$^{-1}$ for the control group.

The intervals have a $95$% chance of straddling the population mean reduction in FBG.
The sample sizes are a bit less than $25$, so the statistical validity is not guaranteed.
