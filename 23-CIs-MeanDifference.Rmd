...
```{r WeightGainHistogram, fig.cap="A histogram of weight changes (the vertical grey line represents no weight gain)", fig.align="center", out.width='60%', fig.width=8.5, fig.height = 3.0}
par( mfrow = c(1, 2))
...
```{r WeightGainNormal, fig.cap="The sampling distribution is a normal distribution; it shows how the sample mean weight gain varies in samples of size $n = 68$", fig.align="center", fig.width=9.0, fig.height=2.75, out.width='95%'}
mn <- mean(SW$GainWt)
n <- length(SW$GainWt)
stdd <- sd(SW$GainWt)
...
```{r}
DiffExists <- complete.cases(diab[, c("DBPfirst", 
                                      "DBPsecond", 
                                      "Diff")])
diabData <- diab[DiffExists, c("DBPfirst", 
                               "DBPfirst", 
                               "Diff")]
...
if( knitr::is_html_output() ) {
  DT::datatable( diab[DiffExists, c("DBPfirst", 
                                    "DBPsecond", 
				    "Diff")],
...
Diab.Table[1, 1] <- mean(diab$DBPfirst[DiffExists], na.rm = TRUE)
Diab.Table[2, 1] <- mean(diab$DBPsecond[DiffExists], na.rm = TRUE)
Diab.Table[3, 1] <- mean(diab$Diff[DiffExists], na.rm = TRUE)

Diab.Table[1, 2] <- sd(diab$DBPfirst[DiffExists], na.rm = TRUE)
Diab.Table[2, 2] <- sd(diab$DBPsecond[DiffExists], na.rm = TRUE)
Diab.Table[3, 2] <- sd(diab$Diff[DiffExists], na.rm = TRUE)

Diab.Table[1, 3] <- se.diab(diab$DBPfirst[DiffExists])
Diab.Table[2, 3] <- se.diab(diab$DBPsecond[DiffExists])
Diab.Table[3, 3] <- se.diab(diab$Diff[DiffExists])

Diab.Table[1, 4] <- n.diab(diab$DBPfirst[DiffExists])
Diab.Table[2, 4] <- n.diab(diab$DBPsecond[DiffExists])
