...
Among many changes experienced, many students will be responsible for their own meals for the first time.
Does their weight increase or decrease, on average?

One approach to answering this question is to obtain some students' weights as they begin university, and then obtain some students' weight at some later time.
If the *same* students have their weights measured at both times, we would be comnparing within individuals (Sect. \@ref(Comparison)).
...
Here, we decide to use weight *gain*.
:::
...
SWlen <- length(SW$Initial)

SW$Student <- paste("Student", 
                       seq(1, SWlen))
...
if( knitr::is_html_output() ) {
  kable( SW,
         ...
         booktabs = TRUE,
         longtable = FALSE,
         col.names = c("Student", "Initial", "After 12 weeks", "Weight gain"),
                       caption = "The student weight-change data: The weight of students in Week 1 at university, in Week 12, and the weight loss (all in kg)") %>% 
  kable_styling(font_size = 10) %>%
  row_spec(0, bold = TRUE)
}
```
...
```{r InsulationHistogram, echo=FALSE, fig.cap="A plot of weight changes. Left panel: A histogram (the vertical grey line represents no weight loss). Right panel: Case-profile plot (a dashed line represents a weight gain!!!!!!!!!!!!!!!!!)", fig.align="center", out.width='100%', fig.width=9.5, fig.height = 3.0}
par( mfrow = c(1, 2))
...
plot( range(c(SW$Initial, SW$Final)) ~ c(1, 2),
      pch = 19,
      las = 1,
      ylim = c(40, 110),
      axes = FALSE,
      type = "n",
      main = "Weight loss from\nWeek 1 to Week 12",
...
for (i in 1:SWlen){
  lines( x = c(1, 2),
         y = c(SW$Initial[i], SW$Final[i]),
         col = ifelse( (SW$Initial[i] - SW$Final[i]) > 0, plotSolid, "grey"),
         lty = ifelse( (SW$Initial[i] - SW$Final[i]) > 0, 1, 1),
         lwd = 2
  )
  points( c(1, 2),
         c(SW$Initial[i], SW$Final[i]),
         pch = 19
  )
}
```
...
`r if (knitr::is_latex_output()) {
   '(Table \\@ref(tab:WeightLossSummary)).'
} else {
   '(see below).'
}`
For the weight-loss data, the appropriate *numerical summary* for paired quantitative data *summarises the differences* using means, standard deviations, and so on, as appropriate.
...
A formal numerical summary table is shown in Table \@ref(tab:WeightLossSummary).


```{r WeightLossSummary, echo=FALSE}
Insulation.DataSummary <- array( dim=c(3, 4))

Insulation.DataSummary[1, 1] <- mean(SW$InitialWt)
Insulation.DataSummary[2, 1] <- mean(SW$FinalWt)
Insulation.DataSummary[3, 1] <- mean(SW$GainWt)

Insulation.DataSummary[1, 2] <- median(SW$InitialWt)
Insulation.DataSummary[2, 2] <- median(SW$FinalWt)
Insulation.DataSummary[3, 2] <- median(SW$GainWt)

Insulation.DataSummary[1, 3] <- sd(SW$InitialWt)
Insulation.DataSummary[2, 3] <- sd(SW$FinalWt)
Insulation.DataSummary[3, 3] <- sd(SW$GainWt)

Insulation.DataSummary[1, 4] <- IQR(SW$InitialWt)
Insulation.DataSummary[2, 4] <- IQR(SW$FinalWt)
Insulation.DataSummary[3, 4] <- IQR(SW$GainWt)

rownames(Insulation.DataSummary) <- c("Week 1 weight", 
                                      "Week 12 weight", 
                                      "Weight gain")
...
if( knitr::is_latex_output() ) {
  kable(Insulation.DataSummary,
        ...
        digits = 2,
        caption = "The mean, median, standard deviation and IQR for the weight-loss data (in kg)") %>%
    row_spec(0, bold = TRUE) %>%
    kable_styling(font_size = 10)
} else {
   kable(Insulation.DataSummary,
...
The study concerns the mean weight loss.
Every possible sample of $n = 68$ students comprises different students, and hence produces different *Week 1* and *Week 12* weights, and hence different weight losses.
As a result, the *sample mean weight loss* will vary from sample to sample, so the mean differences have a *sampling distribution*, and a *standard error*.
...
* centred around the *sampling mean* $\mu_{\bar{d}}$, whose value is the population mean *difference* $\mu_d$,
...
For the weight-loss data, the variation in the sample mean differences $\bar{d}$ is described by (see Table \@ref(tab:NotationOneDiffCI)):

* approximate normal distribution;
* centred around $\mu_{\bar{d}}$;
* with a *standard error* of $\displaystyle\text{s.e.}(\bar{d}) = {0.9489378}\div{\sqrt{68}} = 0.1150756$.

Notice that many decimal places are used in the working here; results will be rounded when reported.
 
 

```{r WeightLossnNormal, echo=FALSE, fig.cap="The sampling distribution is a normal distribution; it shows how the sample mean weight loss varies in samples of size $n = 68$", fig.align="center", fig.width=9.0, fig.height=2.75, out.width='95%'}
mn <- -0.8735
n <- 68
stdd <- 0.9489378
...
\[
	0.8735 \pm (2 \times 0.1150756),
\]
or $0.8735\pm 0.2301512$ (so the *margin of error* is $0.230$).
Equivalently, the CI is from $0.8735 - 0.23015 = 0.643$, up to $0.8735 + 0.23015 = 1.104$.
We write:

> Based on the sample, an *approximate* 95% CI for the population mean weight gain between Week 1 and 12 is from $0.64$kg to $1.10$kg.

The 95% CI is saying that we are reasonably confident that, between Weeks 1 and 12, the mean weight gain is between using $0.64$kg and $1.10$kg.
Alternatively, the plausible values for the mean weight gain in the *population* are between $0.64$kg and $1.10$kg.
...
For the weight-loss data, the sample size is $n = 68$, larger than 25, so the results are statistically valid.
...
```{r InsulationTestOutput, echo=FALSE, fig.cap="The weight-loss data: jamovi output (top) and SPSS output(bottom)", fig.align="center", out.width=c("95%","85%"), fig.show="hold"}
knitr::include_graphics("jamovi/WeightLoss/WeightLoss-PairedTOutput.jpg")
