...
<!-- Case-profile plot not very helpfuk: -->
...
if( knitr::is_latex_output() ) {
  kable(WtGain.DataSummary,
        ...
        digits = 2,
        caption = "The mean, median, standard deviation and standard error for the weight-gain data (in kg)") %>%
    row_spec(0, bold = TRUE) %>%
...
if( knitr::is_latex_output() ) {
  T1 <- kable( bloodS[1:5, ],
               ...
               longtable = FALSE) %>%
    row_spec(0, bold = TRUE)
  T2 <- kable( bloodS[6:10, ],
               format = "latex",
               row.names = FALSE,
               booktabs = TRUE,
               longtable = FALSE) %>%
    row_spec(0, bold = TRUE)
  T3 <- kable( bloodS[11:15, ],
               format = "latex",
               row.names = FALSE,
               booktabs = TRUE,
               longtable = FALSE) %>%
