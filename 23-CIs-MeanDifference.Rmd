...
## Example: competing plants {#PairedCI-SoilN}

Skypilot (*Polemonium viscosum*) is a native alpine wildflower growing in the Colorado Rocky Mountains (USA).
In recent years, a willow shrub (*Salix*) has been encroaching on skypilot territory.
Researchers studied the impacts on this encroachment on pollination on the native skypilot [@kettenbach2017shrub].
One RQ was:

> In the Colorado Rocky Mountains, what is the mean difference between first-flowering time for the native skypilot and encroaching willow?
...
The parameter is $\mu_d$, which we define as the population mean *difference* in day of first flowering.
...
if( knitr::is_latex_output() ) {
 ...
 T1 <- knitr::kable(surroundMaths(Flowering[1:9, 1:2],
                                   decDigits = 0),
                     format = "latex",
                     valign = 't',
                     align = "r",
                     linesep = "",
                     col.names = c("Willow", "Skypilot"),
                     row.names = FALSE,
                     escape = FALSE,
                     booktabs = TRUE) %>%
    row_spec(0, bold = TRUE) %>%
   add_header_above( c( "Day of first flowering" = 2),
                     bold = TRUE)

 T2 <- knitr::kable(surroundMaths( rbind(Flowering[10:17, 1:2],
                                         c(NA, NA)),
                                   decDigits = 0),
                     format = "latex",
                     valign = 't',
  ...
  T3 <- knitr::kable(surroundMaths( rbind(Flowering[18:25, 1:2],
                                          c(NA, NA)),
                                   decDigits = 0),
                     format = "latex",
                     valign = 't',
  ...
  out <- knitr::kables(list(T1, T2, T3),
                       format = "latex",
                       label = "FloweringData",
                       caption = "The Julian dates of first flowering by encroaching willow and native skypilot.") %>% 
    kable_styling(font_size = 10)
  out2 <- prepareSideBySideTable(out) 
  out2 
}
...
```{r, FloweringPlots, out.width='90%', fig.width=9, fig.height=3.5, fig.align=TRUE, fig.cap="The flowering-day data. Left: A histogram of the difference between the first-flowering days; right: a case-profile plot of days of first flowering (??????unfilled points and dashed lines indicate a decrease)"}
par( mfrow = c(1, 2))
...
The CI should be statistically valid as the sample size since $n = 25$.



## Example: soil nitrogen: REMOVE  {#PairedCI-SoilN}
