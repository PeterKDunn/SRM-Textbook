...
```{r InsulationHistogram, echo=FALSE, fig.cap="A plot of the energy savings from the insulation data. Left panel: A histogram (the vertical grey line represents no energy saving). Right panel: Case-profile plot (a dashed line represents an energy increase)", fig.align="center", out.width='85%', fig.height = 3.0}
par( mfrow = c(1, 2))
...
* centred around $\mu_d$;
* with a standard deviation of $\displaystyle\text{s.e.}(\bar{d}) = {1.015655}\div{\sqrt{10}} = 0.3211784$ called the *standard error of the differences*.
...
The *differences* are defined as *during* minus *before*, so that *positive* difference therefore means the *during* value is higher.
Hence, the differences correspond to the *increase* in PA during lockdown (compared to before).
Similarly, a *negative * value means that the *before* value is higher.
The *parameter* of interest is the population mean difference $\mu_d$, the mean increase vigorous PA *during* lockdown compared to *during* the lockdown.
...
\[
  2.68 - (2 \times 3.515018) = -4.3500\qquad\text{to}\qquad 2.68 + (2 \times 3.515018) = 9.7100,
\]
so the approximate 95% CI for the population mean *difference* is from -4.35 to 9.71 minutes.

Notice that one value is *negative*.
This does **not** mean a negative amount of PA (which would make no sense); the CI is for the population mean *increase* during compared to before lockdown.
So, a negative value means that the *before* values are higher than the *during* values on average.

So, the CI means: 

> In the population, the mean difference between the amount of vigorous PA by Spanish health students is between 4.35 minutes more *during* lockdown, and 9.71 minutes more *before* lockdown.
:::
...
```{r InsulationTestOutput, echo=FALSE, fig.cap="The insulation data: jamovi output (top) and SPSS output(bottom)", fig.align="center", out.width="85%", fig.show="hold"}
...
The approximate 95% CI is $\bar{d} \pm (2\times \text{s.e.}(\bar{d}))$:

\[
  7.70 \pm (2\times 3.10)\quad\text{or:}\quad 7.70 \pm 6.20,
\]
