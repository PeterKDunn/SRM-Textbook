...
## Green coffee bean extract {#CoffeeExtractCI}
...
The fasting blood glucose (FBG) was measured for each individual, in both groups, both before the intervention and after the intervention.

> Over eight weeks, patients received 2 × 200 mg GCE daily (one 200 mg capsule after breakfast and one after lunch), and a placebo capsule that was similar to GCE supplement in terms of dosage, color and size containing 200 mg starch as the same manner, in the intervention and control groups, respectively. (on page 2)

The data are not available so a graphical summary cannot be produced; however, the data summary is shown in the article (Table\ \@ref(tab:GCBsummaryTable)).
For the *green coffee group*, the standard error of the *reduction* in FBG is $\text{s.e.}(\bar{d}) = 14.21/\sqrt{21} = 3.10$, so an approximate $95$% CI for the *reduction* in fasting blood glucose (FBG) is  
\[
  12.50\pm (2\times 3.10), \text{or from $6.29$ to $18.70$ mg.dl$^{-1}$}.
\]
For the *control group*, the standard error of the *reduction* in FBG is $\text{s.e.}(\bar{d}) = 13.69/\sqrt{23} = 2.855$, so an approximate $95$% CI for the *reduction* in fasting blood glucose (FBG) is  
\[
  2.00\pm (2\times 2.855), \text{or from $-3.71$ to $7.71$ mg.dl$^{-1}$}.
\]
