...
$$
   ...
   = \frac{\text{The number of rolls that are a $\largedice{4}$}}{\text{The number of possible events from a single roll}} 
   = \frac{1}{6}.
$$
`r if (knitr::is_html_output()) '-->'`
`r if (knitr::is_latex_output()) '<!--'`
$$
   ...
   = \frac{\text{The number of rolls that are a 4}}{\text{The number of possible events from a single roll}} 
   = \frac{1}{6}.
$$
`r if (knitr::is_latex_output()) '-->'`
...
This is the *relative frequency* approach to calculating (future) probabilities: based on past data.
