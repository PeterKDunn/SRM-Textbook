...
To talk about probability, a *random procedure* must be defined first.
...
These are all mutually exclusive\index{Mutually exclusive} (or distinct) results and cover all possible results (exhaustive)\index{Exhaustive}, and the sample space is *discrete*.\index{Quantitative data!continuous}
...
::: {.example #EventsAndProb name="Events"}
...
An *estimate* of the probability that the next birth will be a boy is about $0.513$ (or $51.3$%), based on past data.
...
is $1/6$ using the classical approach, as the sample space has six elements.
...
No other outcomes are possible, and so the probability or rolling a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
Consider these three events about drawing two cards from a fair pack.
*Event\ 1* is 'draw a **Jack** as the first card'; *Event\ 2* is 'draw a $\heartsuit$ as the first card'; and *Event\ 3* is 'draw a $\diamondsuit$ as the second card'.
...
*Event A* is 'drawing a picture card', *Event B*  is 'drawing a card less than a $5$' and *Event C* is 'drawing a $\spadesuit$.
