...
Two events are *independent* if the probability of one event happening is the same, whether the other event has happened.
...
Two events are *independent* if the probability of one event is the same, whether the other event has happened.
:::
...
However, if we are told that the number rolled is an *odd number*, only three elements in the sample space need now be considered, rather than six:
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
1. Is the whether the student is a first nations student of the student approximately independent of the type of school, for female students?
1. Is the whether the student is a first nations student of the student approximately independent of the type of school, for male students?
