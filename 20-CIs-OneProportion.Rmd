...
Suppose a fair die was rolled $25$ times, and this was repeated *thousands* of times (not just ten times as in 
`r if (knitr::is_latex_output()) {
...
The mean of this distribution is called the *sampling mean*, and the standard deviation for this distribution is called the *standard error*, denoted $\text{s.e.}(\hat{p})$ (see Fig.\ \@ref(fig:NormalDieTheory)).
...
* with a standard deviation (called the *standard error* $\text{s.e.}(\hat{p})$) whose value is $`r round(se.die, 3)`$ (where this number comes from will be revealed later, in Eq.\ \@ref(eq:StdErrorExampleDie)).
...
For this example, where $n = 25$ rolls of a die and the population proportion of even rolls is $p = 0.5$, the standard error of the sample proportion is  
\begin{equation}
...
Based on the sampling distribution in Fig.\ \@ref(fig:NormalDieTheory), how often would a value of $\hat{p}$ larger than 0.80 be expected?\label{thinkBox:HowOftenp}
...
Make sure you are using a *proportion* in the formula, not a *percentage* (i.e., using $0.5$ rather than $50$%). 
...
Suppose $11$ of the $n = 25$ rolls produced an even number, so that $\hat{p} = 11/25 = 0.44$ (which is quite reasonable; see Fig.\ \@ref(fig:NormalDieTheory)).
The unknown value of $p$ could be a bit larger than $\hat{p} = 0.44$, or a bit smaller than $\hat{p} = 0.44$.
In other words, the unknown value of $p$ is in the range $\hat{p}$, give-or-take a bit.

Since the sampling distribution has an approximate normal distribution (Def.\ \@ref(def:DEFSamplingDistributionPhat)), the 68--95--99.7 rule can be used to compute the approximate give-or-take amount (using the ideas in Sect.\ \@ref(CIpKnownp)): the give-or-take amount would be $\left(\text{multiplier}\times\text{s.e.}(\hat{p})\right)$.
This means that the interval (referred to above as "$\hat{p}$, give-or-take a bit") would, more precisely, be  
\[
...
```{r, pProducingpHat, out.width='100%', fig.width=6, fig.height=2, fig.align="center", fig.cap="Various possible values for $p$, and the corresponding $95$\\% sampling intervals (i.e., the values of $\\hat{p}$ these are likely to produce)."}
par(mfrow = c(1, 4),
    mar = c(4, 0.3, 4, 0.3) )
...
\end{align*}
So, an approximate $95$% CI is between $0.241 - (2\times 0.01098449) = 0.2190$ and $0.241 + (2\times 0.01098449) = 0.2627$.
The *approximate* $95$% CI is from $0.219$ to $0.263$.
We write

> Based on the sample, the sample proportion of young Canadians reporting sleeping difficulties after consuming energy drinks is $\hat{p} = 0.241$ ($n = 1516$), with an approximate $95$% CI from $0.219$ to $0.263$.
...
> Based on the sample, the sample proportion of koalas that crossed at least one road in the previous $30$ months is $0.353$ ($n = 51$), with an approximate $95$% CI from 0.219$ to $0.263$.

It is plausible that the sample proportion of $\hat{p} = 0.3529$ may have come from a population with a proportion somewhere between $0.219$ and $0.487$.
:::
...
> Based on the sample, the sample proportion of female US college students who drank coffee daily is $\hat{p} = 0.169$ ($n = 360$), with an approximate $95$% CI from $0.130$ to $0.209$.
