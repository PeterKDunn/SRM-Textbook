...
```{r RollDiceHistFig, echo=FALSE, fig.align="center", fig.height=3, fig.width=6.5, out.width='70%', fig.cap="The proportion of rolls that are even change from one sample of 25 rolls to the next sample of 25 rolls" }
if (knitr::is_latex_output()){
  set.seed(990099)
  num.rolls <- 25
  num.sims <- 1000
...
However, the *average value* of the sample proportion can be described by a *mean*, and the amount of variation in the sample proportions can be described by a *standard deviation* (called a *standard error* in this context).

For example, the mean of the sampling distribution is the 'average' value of all possible sample proportions, $\hat{p}$.
:::
...
OneProportionNotation[1, ] <- c("Describing the population",
                          "Proportion of successes $p$")
OneProportionNotation[2, ] <- c("Describing a sample",
                          "Proportion of successes $\\hat{p}$")
OneProportionNotation[3, ] <- c("Describing sample proportions ($\\hat{p}$)",
