...
```{r, child = if (knitr::is_html_output()) {'./introductions/20-CIs-OneProportion-HTML.Rmd'} else {'./introductions/20-CIs-OneProportion-LaTeX.Rmd'}}
```
...
Suppose a fair, six-sided die is rolled $25$ times (Fig.\ \@ref(fig:RouletteWheelHist), top-right panel).
What proportion of the rolls will produce an even number?
That is, what will be the value of *sample proportion* of numbers that are even?

Of course, no-one knows what proportion will be even for any sample of $25$ rolls.
...
Let's simulate rolling a die $25$ times, using just $10$ sets of $25$ rolls, and find the value of $\hat{p}$ for
...
```{r RollDiceFig, fig.align="center", out.width="85%", fig.width=7, fig.height=4, fig.cap="The proportion of rolls that are even $\\hat{p}$ is not the same for every sample of 25 rolls" }
if (knitr::is_latex_output()){
  ...
  plot( c(1, (num.rolls + 2)), 
        ...
        xlab = "",
        ylab = "",
        main = paste("The results from", num.sims, "sets of", num.rolls, "rolls each"),
        axes = FALSE)
  ...
  for (j in 1:i){
    ...
    # Overall P(even): Add some slight background colour under the sample proportions
    polygon( c(num.rolls + 1, 
               num.rolls + 1, 
               num.rolls + 3, 
               num.rolls + 3),
             c(j - 0.5, 
               ...
               j - 0.5),
             border = NA,
             col = ifelse( prop.even[j] > 0.5, 
                           rgb( col1[1], col1[2], col1[3], alpha = 75, max = 255), 
                           rgb( col2[1], col2[2], col2[3], alpha = 75, max = 255) ) )
...
The proportion of even rolls varies from set to set.
For these $10$ sets of $n = 25$ rolls, the proportion of even rolls ranged from $\hat{p} = 0.32$ to $\hat{p} = 0.60$.

The sample proportion of even rolls would be expected to vary around $p = 0.5$ (the *population proportion*), since three of the six faces of the die are even numbers, using the classical approach to probability.
Of course, the sample proportion in $25$ rolls could be very small or very high by chance, but we wouldn't expect to see that very often (Fig.\ \@ref(fig:RouletteWheelHist), top-right panel).

For any sample of $25$ rolls, the value of $\hat{p}$ is unknown until the die are rolled.
The proportion of even rolls varies from sample to sample; that is, the sample proportions exhibit sampling variation, and the *amount* of sampling variation is quantified using a *standard error*.
...
and the proportion of even rolls was recorded for every one of those thousands of sets of  rolls.
These thousands of sample proportions $\hat{p}$, one from every sample of $25$ rolls, could be graphed using a histogram;
`r if (knitr::is_latex_output()) {
   'see Fig.\\ \\@ref(fig:RollDiceHistFig).'
} else {
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=6.5, out.width='65%', fig.cap="The proportion of rolls that are even changes from one sample of 25 rolls to the next sample of 25 rolls" }
if (knitr::is_latex_output()){
  set.seed(99100991)
  num.rolls <- 25
  num.sims <- 1000
...
This is no accident: statistical theory says this will happen (when certain conditions are met: see Sect.\ \@ref(ValidityProportions)).
The mean of this distribution is the *sampling mean*, and its value is $p$; the standard deviation for this distribution is the *standard error*, denoted $\text{s.e.}(\hat{p})$ (see Fig.\ \@ref(fig:NormalDieTheory)).
...
can be determined (Fig.\ \@ref(fig:StatisticVariesAcrossSamples)).
The sampling distribution for $\hat{p}$ is:

* an approximate normal distribution;
* centred around a *sampling mean* whose value is $p = 0.5$ (the population proportion);
* with a standard deviation (called the *standard error* $\text{s.e.}(\hat{p})$) whose value is $`r round(se.die, 3)`$ (where this number comes from will be revealed in Eq.\ \@ref(eq:StdErrorExampleDie)).

This distribution is called a [*sampling distribution*](#def:SamplingDistribution), as discussed in Sect.\ \@ref(SamplingVariationIntro).
...
```{r NormalDieTheory, fig.cap="The normal distribution, showing how the proportion of even rolls varies when a die is rolled $25$ times", fig.align="center", fig.width=8.25, fig.height=3.5, out.width='100%'}
pop.p <- 0.5
n <- 25
...
out <- plotNormal(mu = pop.p, 
                  sd = se.p, 
                  xlab = expression( hat(italic(p))*","~the~sample~proportion~of~even~rolls~out~of~25), 
                  round.dec = 2,
                  xlim.hi = 1,
                  xlim.lo = 0,
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The [parameter](#StatisticsAndParameters) $p$ and the [statistic](#StatisticsAndParameters) $\hat{p}$ are both *proportions*.
...
Almost always, the value of $p$ is unknown.
...
\begin{equation}
   ...
   (\#eq:StdErrorPknown)
\end{equation}
when certain conditions are met (Sect.\ \@ref(ValidityProportions)), where $n$ is the size of the sample, and $p$ is the population proportion.
...
Based on the sampling distribution shown in Fig.\ \@ref(fig:NormalDieTheory), how often would a value of $\hat{p}$ larger than 0.80 be expected?\label{thinkBox:HowOftenp}

`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
Figure\ \@ref(fig:NormalDieTheory) suggests that, while not impossible, 0.80 or greater will be observed rarely.
...
Since the possible values of the sample proportions $\hat{p}$ can be described by an approximate *normal distribution*, the 68--95--99.7 rule (Sect.\ \ @ref(def:EmpiricalRule) can be applied.
For example (see Fig.\ \@ref(fig:NormalDieTheory)), about $68$% of the time, a set of $25$ rolls will have a value of $\hat{p}$ between $0.5$ give-or-take *one* standard deviation (that is, give-or-take `r round(se.die, 3)`) rolls that are even.
So, about $68$% of the time, the proportion of even rolls in a set of $25$ rolls will be between $0.5 - `r round(se.die, 3)` =  `r round(0.5 - se.die, 3)`$ and $0.5 + `r round(se.die, 3)` =  `r round(0.5 + se.die, 3)`$.

Similarly, about $95$% of the time, the proportion of even rolls will be between $0.5$ give-or-take *two* standard deviations, or between $0.5 - (2\times`r round(se.die, 3)`) =  `r round(0.5 - 2*se.die, 3)`$ and $0.5 + (2\times`r round(se.die, 3)`) =  `r round(0.5 + 2*se.die, 3)`$.
...
all ten sets of $25$ rolls (i.e., $100$%) had a sample proportion between $0.30$ and $0.70$.)

Formally, the sample proportion $\hat{p}$ is likely to lie within the interval  
\[
   p \pm (\text{multiplier} \times \text{s.e.}(\hat{p})),
\]
where $\text{s.e.}(\hat{p})$ is the *standard error of the sample proportion* (calculated using Eq.\ \@ref(eq:StdErrorPknown)), and the *multiplier* comes from the 68--95--99.7 rule, depending on the level of confidence we are aiming for.
...
For a $95$% interval, the multiplier is *approximately* $2$, based on the [68--95--99.7 rule](#def:EmpiricalRule):
Approximately $95$% of observations are within *two* standard deviations of the value of $p$ (the mean of the normal distribution in  Fig.\ \@ref(fig:NormalDieTheory)).
That is, the *approximate* $95$% sampling interval is:  
\[
   p \pm (2 \times \text{s.e.}(\hat{p}) ).
\]
For a $90$% sampling interval, either tables or a computer would be used to find the correct multiplier, since the [68--95--99.7 rule](#def:EmpiricalRule) isn't helpful.


## Sampling distribution: unknown proportion  {#SamplingDistributionUnknownp}

In the die example (Sect.\ \@ref(SamplingDistributionKnownp)), the value of $p$ was known.
...
When the value of $p$ is *unknown*, the *sampling distribution of the sample proportion* is described by
...
\begin{equation}
   ...
   (\#eq:stderrorphat)
\end{equation}
when certain conditions are met (Sect.\ \@ref(ValidityProportions)), where $n$ is the size of the sample, and $\hat{p}$ is the sample proportion.
In general, the approximation gets better as the sample size gets larger.
:::
...
Then (from Def.\ \@ref(def:DEFSamplingDistributionPhat)),  
\[
   \text{s.e.}(\hat{p}) = \sqrt{ \frac{ 0.44 \times (1 - 0.44)}{25}} = 0.099277.
\]
(This is very similar to the value of $0.1$, the value of the standard error when the value of $p$ was known; see Sect.\ \@ref(SamplingDistributionKnownp).)

Hence, the sample proportions will vary with an approximate normal distribution (Fig.\ \@ref(fig:NormalDieTheoryUnknownp)), centred around the unknown value of $p$ with a standard deviation of $\text{s.e.}(\hat{p}) = 0.099277$.
...
That is, for some *known* value of $p$, the *unknown* value of $\hat{p}$ is expected to vary between $p - 0.199$ and $p + 0.199$ (Fig.\ \@ref(fig:CIrelationships)).


```{r CIrelationships, out.width='60%', fig.align="center", fig.cap="A known value of $p$ produces a range of $\\hat{p}$ values.", fig.width = 5.5, fig.height=4}
...
So we rephrase the question: What values of $p$ could reasonably be expected to produce our observed value of $\hat{p}$?
Figure\ \@ref(fig:NormalDieTheory) shows that values of $p$ between roughly $0.3$ and $0.6$ could be reasonably expected to produce the value $\hat{p} = 0.44$.
More precisely, a value of $p$ between $0.44 - 0.199$ and $0.44 + 0.199$  could reasonably be expected to produce the observed value of $\hat{p}$ (Fig.\ \@ref(fig:CIrelationshipsP)).


```{r CIrelationshipsP, out.width='75%', fig.align="center", fig.cap="A CI gives a range of possible values of $p$ for which it is reasonable to produce the observed value of $\\hat{p}$. The shaded regions represent regions containing $95$\\% of the values of $\\hat{p}$ for each value of $p$.", fig.width=6.5, fig.height=8}
...
This interval is called a *confidence interval* (or CI), based on ideas from  Sect.\ \@ref(CIpKnownp).

In summary, using $\hat{p} = 0.44$ and $\text{s.e.}(\hat{p}) = 0.0993$, the (approximate) $95$% CI is $0.44 \pm (2 \times 0.0993)$, or from $0.241$ to $0.639$.
This CI straddles the known population proportion of $p = 0.5$, though we would not know this if $p$ was unknown (which is usually the case).
...
```{r RollDiceCIFig, fig.align="center", fig.width=8.5, out.width="100%", fig.cap="About 95\\% of CIs contain the population proportion. We only have one sample."}
if (knitr::is_latex_output()){
  set.seed(99999)
    num.rolls <- 25
    num.sims <- 50
...
About $95$% of those $95$% CIs straddle the population proportion of $p = 0.5$ (shown as solid lines)... but some do not (shown as dashed lines).
Of course, since the value of $p$ is usually unknown, we never know if our CI from a single sample contains $p$ or not.


::: {.definition #ConfidenceInterval name="Confidence interval"}
A *confidence interval* is an interval in which the population [parameter](#def:Parameter)    to be contained, if we found many samples the same way.
If a $95$% confidence interval (or CI) is computed from many samples, about $95$% of the CIs would straddle the *parameter* of interest.
...
:::{.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
Using the 68--985--99.7 rule produces *approximate* multipliers and hence approximate CIs.
In reality, finding the exact multipliers (and hence exact CIs) is more involved.

In this book, we will use multipliers from the 68--95--99.5 rule and create *approximate* CIs.
Except for small sample sizes, the approximations are generally very good.
To form *exact* CIs, software will be used.
:::
...
A study of young Canadians aged 12--24 [@data:Hammond2018:Drinks] found $365$ of the $1516$ respondents reported sleeping difficulties after consuming energy drinks.
The unknown parameter is $p$, the *population* proportion reporting sleeping difficulties.

The sample proportion reporting sleeping difficulties after consuming energy drinks is $\hat{p} = 365/1516 = 0.241$.
The sample proportion varies from one sample of size $n = 1516$ to another; *sampling variation* exists.
The *standard error* (Def.\ \@ref(def:ConfidenceIntervalp)) quantifies how much the sample proportion varies from sample to sample:
\begin{align*}
  \text{s.e.}(\hat{p}) 
  &= \sqrt{\frac{\hat{p}\times(1 - \hat{p})}{n}}\\
...
\end{align*}
or about $0.011$.
So, in samples of size $1516$, an approximate $95$% CI is between $0.241 - (2\times 0.01098449) = 0.2190$ and $0.241 + (2\times 0.01098449) = 0.2627$.
...
A study of koalas [@data:Dexter2018:Koalas] found that $18$ of the $n = 51$ koalas studied in a certain area over $30$ months had crossed at least one road during that time.
...
\]
An approximate $95$% CI is $0.3529 \pm (2 \times 0.06692)$, or $0.3529 \pm 0.1338$.
The *margin of error* is $0.1338$.
Alternatively, the approximate $95$% CI is from $0.219$ to $0.487$ (after rounding appropriately).

The approximate $95$% CI for the population proportion of koalas that crossed at least one road in the last $30$ months is from $0.219$ to $0.487$. 
That is, it is plausible that the sample proportion of $\hat{p} = 0.3529$ may have come from a population with a proportion somewhere between $0.219$ and $0.487$.
:::
...
The CI can also be seen as having a $95$% chance of straddling the value of the parameter
These are close to the correct interpretation.

Commonly, the CI is interpreted as having a $95$% chance of containing the value of population [parameter](#StatisticsAndParameters) $p$.
This is not strictly correct (since the CI either *does* or *does not* contain the value of $p$), but is like a convenience that captures the essence of the correct interpretation.
...
The histogram in Sect.\ \@ref(SamplingDistributionKnownp), shows the proportion of $n = 25$ rolls that were even for many samples; it has an approximate normal distribution.
Because of this, the [68--95--99.7 rule](#def:EmpiricalRule) could be used to determine the multipliers and form the approximate $95$% CIs.
However, the distribution of $\hat{p}$ only has an approximate normal distribution under certain conditions.
Certain conditions must be true for the sampling distribution to be sufficiently normally distributed, or *statistically valid*.
...
Your blood is analysed, and your doctor is emailed the results of the blood test.
...
If this condition is not met, the normal distribution may not approximate the sampling distribution very well, so the 68--95--99.7 rule (used to create the CI) may be inappropriate, and so the CI may also be inappropriate.




:::{.example #StatisticalValidityPHat name="Statistical validity conditions"}
Consider a situation where $p = 0.1$.
A sample of size $n = 10$ is taken, giving $\hat{p} = 0.1$.
The statistical validity conditions *are not* satisfied, so we would expect that the sampling distribution is not well modelled by a normal distribution.
If the sampling distribution was assumed to follow the distribution described in Def.\ \@ref(def:DEFSamplingDistributionPhat), the sampling distribution would look like that in Fig.\ \@ref(fig:StatisticalValidityPHat) (left panel).
A large part of the sampling distribution covers *negative* values for $\hat{p}$, which makes no sense.

In contrast, assume a sample of size $n = 100$ is taken, giving $\hat{p} = 0.1$.
The statistical validity conditions *are* satisfied, so we would expect that the sampling distribution is well modelled by a normal distribution.
The sampling distribution is shown in Fig.\ \@ref(fig:StatisticalValidityPHat) (right panel).
:::


```{r, StatisticalValidityPHat, out.width='100%', fig.align="center", fig.cap="Two proposed sampling distributions. Left: When the statistical validity conditions are not met; right: when the statistical validity conditions are met.", fig.height=3, fig.width=9}
...
out <- plotNormal(mu = 0.1,
                  sd = 0.09486833,
                  round.dec = 2,
                  cex.tickmarks = 0.7,
                  main = expression( atop(Statistical~validity~conditions,
                                          bold(are~not)~met)),
                  xlab = expression(Values~of~hat(italic(p))),
                  las = 2)
abline(v = 0)

lines( out$y[out$x < 0] ~ out$x[out$x < 0], 
       col = "grey", 
       lwd = 2)
...
text(x = -0.15,
     y = 0.5 * max(out$y),
     pos = 3,
     labels = expression(atop(Negative~values,
                              "for"~hat(italic(p))*"!")),
     cex = 0.8)
arrows(x0 = -0.15,
       x1 = -0.06,
...
# Suppose p = 0.1, and we estimate p-hat = 0.1, with n = 100.
# Then s.e.(p-hat) = 0.03
...
In Example\ \@ref(exm:CanadianEnergyDrinks), the approximate $95$% CI was $0.192$ to $0.236$.
This CI for the proportion will be *statistically* valid if the number of youth in the sample who *did* experienced sleeping difficulties *and* the number who *did not* experience sleeping difficulties both exceed $5$.
The numbers are $365$ and $1516 - 365 = 1151$ respectively, both easily exceeding\ $5$%.
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider Example\ \@ref(exm:KoalasCrossingRoads), about koalas crossing roads.
Is the CI likely to be statistically, internally and externally valid?\label{thinkBox:StatValidityKoalas}
...
\]
Always check whether the [statistical validity conditions](#ValidityProportions) are satisfied.
...
A study of $360$ female college students in the United States [@data:Kelpin2018:AlcoholCoffee] found that $61$ drank coffee daily. 
...
The sample proportion has *sampling variation*, measured by the *standard error*  
\[
  \text{s.e.}(\hat{p})
               = \sqrt{ \frac{ 0.16944 \times (1 - 0.16944)}{360}}
               = 0.01977.
\]
An *approximate* $95$% CI is $0.1694 \pm (2 \times 0.01977)$, or $0.1694 \pm 0.03954$ (i.e., the *margin of error* is $0.03954$.)
Equivalently, the approximate $95$% CI is from $0.130$ to $0.209$, after rounding appropriately.

The plausible values for $p$ that may have led to this value of $\hat{p} = 0.1694$ are between $0.130$ and $0.209$.
...
The CI is *externally* valid if the sample is a simple random sample of some population, and the study is internally valid.
The CI is approximately *externally* valid if the sample is representative of some population, and the study is internally valid.
...
1. True or false: $p$ is called a *parameter*.\tightlist  
...
Selected answers are available in Sect.\ \@ref(CIOneProportionAnswer).


::: {.exercise #CIOneProportionSaltIntake}
A study of salt intake in the United Kingdom [@data:Sutherland:SaltIntake] found that $2\ 182$ out of the $6\ 882$ people sampled in 2007 'generally added salt at the table'.
...
A study of the eating habits of university students in Canada [@data:Mann12017:UniStudents] found that $8$ students out of $154$ met the recommendation for eating a sufficient number of servings of grains each day.
...
A study of hiccups [@data:Lee2016:Hiccups] found that, of $864$ patients examined (across different studies) who had hiccups, $708$ were male.
...
A study of how paramedics administer pain medication [@lord2009impact] found that $791$ of patients reporting pain did *not* receive pain relief, out of $1766$ patients in the study  who reported pain.

1. Compute the sample proportion of patient who received pain medication.
1. Find an approximate $95$% CI for the true proportion of patients who receive pain medication.
1. Check if the statistical validity conditions are met or not.
...
- Sect. \ref{thinkBox:HowOftenp}: From Fig.\ \@ref(fig:NormalDieTheory), $0.80$ or greater will rarely be observed.
- Sect. \ref{thinkBox:StatValidityKoalas}: 
*Statistically*: 
The number crossed a road: $18$.
The number *not* crossed a road: $51 - 18 = 33$.
Both exceed $5$; CI is statistically valid.
*Internally*: Insufficient information about *how* the study was conducted.
*Externally*: A random sample of koalas is unlikely; probably not.
