...
Suppose a fair, six-sided die^[Note the language: two *dice*, but one *die*. *Dice* is plural.] is rolled 25 times.
...
The sample proportion will *vary*: *sampling variation* exists.

This sampling variation can be described, which quantifies *how* the sample proportion varies from sample to sample (Sect. \@ref(ExpectationOf)).
...
```{r NormalDieTheory, echo=FALSE, fig.cap="The normal distribution, showing how the proportion of even rolls varies when a die is rolled 25 times", fig.align="center", fig.width=5.75, fig.height=2.5, out.width='75%'}
pop.p <- 0.5
n <- 25
...
```{r NormalDieTheoryUnknownp, echo=FALSE, fig.cap="The normal distribution, showing how the proportion of even rolls varies when a die is rolled 25 times", fig.align="center", fig.width=5, fig.height=2.5, out.width='60%'}
HT.mn <- 175
HT.sd <- 7
...
Though we are pretending the value of $p$ is unknown, the value really is known.
What if the roles of $p$ and $\hat{p}$ were 'reversed'?
Then, about 95% of the values of $p$ are expected to be between $\hat{p} - 0.199$ and $\hat{p} + 0.199$.

Since $\hat{p} = 0.44$, this is equivalent to: About 95% of the values of $p$ are expected to be between $0.24$ and $0.64$.
This interpretation is not quite correct, but the idea seems reasonable.
This is called a *confidence interval* (or CI), based on ideas from  Sect.&nbsp;\@ref(CIpKnownp).
...
This CI straddles the population proportion of $p = 0.5$, though we would not know this if $p$ truly was unknown.

::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
In this case, we know the value of the population parameter: $p = 0.5$.
Usually we do *not* know the value of the parameter:.
After all, that's why would tak a sample; to *estimate* the value of the population proportion.
:::
...
The research article reports the exact 95% CI as from 22% to 48%, very clos to out approximate 95% CI.
:::
...
Hence, $\text{s.e.}(\hat{p}) =  0.01183326$, so the *approximate* 95% CI is:
\[
   \text{from\ } 0.4479049 - (2\times 0.0118332) = 0.424238 \text{\ to\ } 
...
Consider an artificial situation to estimate the proportion of die rolls that show as a **one**.
The population proportion (using the classical approach to probability) is 1/6, or about 0.167.

If we repeatedly rolled a die in sets of $n = 20$ rolls, say 5000 times, the proportion of rolls that showed as **one** could be recorded for each sample of 20 rolls.
...
Then, a histogram of the proportion of **ones** for those sets of 60 rolls could be produced.
...
```{r HistoProportionsConditions, echo=FALSE, fig.width=7, fig.height=3.0, out.width = '85%', fig.align="center", fig.cap="The sampling distribution of the proportion of ones rolled, for sets of 20 rolls (left panel) and sets of 60 rolls (right panel)" }
par( mfrow = c(1, 2),
