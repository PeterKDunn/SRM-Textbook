...
So, what if the roles of $p$ and $\hat{p}$ were 'reversed'?
Then, the values of $p$ that could reasonably be expected to produce the observed value of $\hat{p}$ are between $\hat{p} - 0.199$ and $\hat{p} + 0.199$ (Fig. \@ref(fig:CIrelationshipsCI)).

```{r CIrelationships, echo=FALSE, out.width='60%', fig.align="center", fig.cap="A known value of $p$ produces a range of $\\hat{p}$ values.", fig.width = 5}
...
```{r CIrelationshipsCI, echo=FALSE, out.width='100%', fig.align="center", fig.cap="A CI gives a range of possible values of $p$ for which it is reasonable to produce the observed value of $\\hat{p}$ values", fig.width = 7, fig.height=8}
...
Since $\hat{p} = 0.44$, this is equivalent to saying that we are reasonably sure that a population with a value of $p$ between $0.24$ and $0.64$ could have produced the observed value of $\hat{p} = 0.44$.
...
This CI straddles the known population proportion of $p = 0.5$, though we would not know this if $p$ was unknown (which is not usually the case).
