...
The mean of this distribution is the *sampling mean*, denoted $\mu_{\hat{p}}$; the standard deviation for this distribution is the *standard error*, denoted $\text{s.e.}(\hat{p})$.
...
can even be determined (Fig. \@ref(fig:StatisticVariesAcrossSamples)).
The sampling distribution for $\hat{p}$ is:

* an approximate normal distribution;
* centred around a *sampling mean* denoted by $\mu_{\hat{p}}$, whose value is $p = 0.5$ (the population proportion);
* with a standard deviation (called the *standard error* $\text{s.e.}(\hat{p})$) whose value is $`r round(se.die, 3)`$ (where this number comes from will be [revealed soon](#def:SamplingDistProp)).
...
```{r NormalDieTheory, echo=FALSE, fig.cap="The normal distribution, showing how the proportion of even rolls varies when a die is rolled 25 times", fig.align="center", fig.width=8.25, fig.height=3.5, out.width='100%'}
pop.p <- 0.5
n <- 25
...
out <- plotNormal(mu = pop.p, 
                  ...
                  round.dec = 2,
                  ylim = c(0, 8.5), # To allow room for "Sampling mean"
...
text(x = 0.5,
     y = 1.65 * max(out$y),
     pos = 3,
     labels = expression(Sampling~mean*":"~mu[hat(italic(p))]) )
...
text(x = 0.55,
     y = 0.25 * max(out$y),
     pos = 3,
     labels = expression(Std~error*":"~plain(s.e.)(hat(italic(p)))))
```

::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The [parameter](#StatisticsAndParameters) $p$ and the [statistic](#StatisticsAndParameters) $\hat{p}$ are both *proportions*.

However, the *average value* of the sample proportion can be described by a *sampling mean* $\mu_{\hat{p}}$, and the amount of variation in the sample proportions can be described by a *standard deviation* (called a *standard error* in this context) $\text{s.e.}(\hat{p})$.
...
* an approximate normal distribution,
* centred around the sampling mean $\mu_{\hat{p}}$, whose value is $p$,
...
From the die example, the values of $\hat{p}$ will vary with an approximate normal distribution, centred around $p = 0.5$, and with a standard error of $\text{s.e.}({\hat{p}}) = 0.1$.
...
Since the possible values of the sample proportions $\hat{p}$ can be described by an approximate *normal distribution*, the [68--95--99.7 rule](#def:EmpiricalRule) can be applied\.
For example (see Fig. \@ref(fig:NormalDieTheory)), about 68% of the time, a set of 25 rolls will have between $0.5$ give-or-take *one* standard deviation (that is, give-or-take `r round(se.die, 3)`) rolls that are even.
...
In the die example (Sect.&nbsp;\@ref(SamplingDistributionKnownp)), the sampling distribution for the sample proportion was given, including an equation for computing the standard error for the sample proportion for samples of size $n$, *when the value of $p$ was known*.
...
OneProportionNotation[4, ] <- c("across all possible samples",
                          "with sampling mean $\\mu_{\\hat{p}}$ and standard deviation $\\text{s.e.}(\\hat{p})$")
...
```{r NormalDieTheoryUnknownp, echo=FALSE, fig.cap="The normal distribution, showing how the proportion of even rolls varies when a die is rolled 25 times, assuming the value of $p$ is unknown", fig.align="center", fig.width=8.25, fig.height=3.5, out.width='100%'}
die.mn <- 0.5
die.sd <- 0.099277
...
```{r CIrelationships, echo=FALSE, out.width='60%', fig.align="center", fig.cap="A known value of $p$ produces a range of $\\hat{p}$ values.", fig.width = 5.5, fig.height=4}
...
```{r CIrelationshipsP, echo=FALSE, out.width='75%', fig.align="center", fig.cap="A CI gives a range of possible values of $p$ for which it is reasonable to produce the observed value of $\\hat{p}$. The shaded regions represent the regions containing 95\\% of the values of $\\hat{p}$ for each value of $p$.", fig.width=6.5, fig.height=8}
...
```{r RollDiceCIFig, echo=FALSE, fig.align="center", fig.width=8.5, out.width="100%", fig.cap="About 95\\% of CIs contain the population proportion. We only have one sample."}
if (knitr::is_latex_output()){
  set.seed(99999)
    num.rolls <- 25
    num.sims <- 50
