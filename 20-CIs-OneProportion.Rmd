...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=6.5, out.width='70%', fig.cap="The proportion of rolls that are even changes from one sample of 25 rolls to the next sample of 25 rolls" }
if (knitr::is_latex_output()){
  set.seed(99100991)
  num.rolls <- 25
  num.sims <- 1000
...
As usual, the sample proportion varies from one sample of size $n = 1516$ to another; *sampling variation* exists.
The *standard error* (Def.\ \@ref(def:ConfidenceIntervalp)) quantifies how much the sample proportion is likely to vary from sample to sample:
\begin{align*}
  \text{s.e.}(\hat{p}) 
  &= \sqrt{\frac{\hat{p}\times(1 - \hat{p})}{n}}\\
...
\end{align*}
or about $0.011$.
So, in samples of size $1516$, the approximate $95$% CI (Def.\ \@ref(def:ConfidenceIntervalp)) is between $0.241 - (2\times 0.01098449) = 0.2190$ and $0.241 + (2\times 0.01098449) = 0.2627$.
...
The standard error (Def.\ \@ref(def:ConfidenceIntervalp)) is  
\[
\text{s.e.}(\hat{p})
...
* the number of youth in the sample who experienced sleeping difficulties exceeds\ $5$, which it does (there are $365$); **and**
* the number of youth in the sample who *did not* experience sleeping difficulties exceeds\ $5$, which it does (there are $1516 - 365 = 1151$).
...
Consider estimating the proportion of die rolls that show as a `r include_graphics("Dice/die1.png", dpi=1250)` (clearly, this is contrived as we know the population proportion).
...
Alternatively, we could repeatedly roll a die in sets of $n = 60$ rolls thousands of times, and record the proportion that show as `r include_graphics("Dice/die1.png", dpi=1250)` for each sample of $60$ rolls.

A histogram of the proportion of `r include_graphics("Dice/die1.png", dpi=1250)` for those sets of $60$ rolls could be produced.
...
The *standard error* is, using Eq.\ \@ref(eq:stderrorphat):  
\[
  \text{s.e.}(\hat{p})
