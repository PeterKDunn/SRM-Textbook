...
Describing *how* the sample proportion varies from sample to sample is useful (Sect. \@ref(ExpectationOf)).
...
```{r RollDiceFig, echo=FALSE, fig.align="center", out.width="90%", fig.width=7, fig.height=4, fig.cap="The proportion of rolls that are even change from one sample of 25 rolls to the next sample of 25 rolls" }
...
For example, the mean of the sampling distribution is the 'average' value of the sample proportion.
:::
...
Recall that the *the standard error is the standard deviation of the distribution of a sample statistic*, that measures how much a sample estimate is likely to vary from sample to sample.
In that sense, the standard error of the proportion measures how precisely $\hat{p}$ estimates the population proportion $p$.
...
when [certain conditions are met](#ValidityProportions), where $n$ is the size of the sample, and $p$ is the population proportion.
...
So, about 68% of the time, the proportion of even rolls in a set of 25 rolls will be between:
...
\begin{equation}
   ...
   (\#eq:stderrorphat)
\end{equation}
when [certain conditions are met](#ValidityProportions), where $n$ is the size of the sample, and $\hat{p}$ is the sample proportion.
...
That's why we are taking a sample; to *estimate* the value of the population proportion.
:::
...
\]
is the *standard error* of $\hat{p}$, $\hat{p}$ is the sample proportion, and $n$ is the sample size.
...
\end{align*}
or about $0.011$.
So, in samples of size 1516, the approximate 95% CI (Definition&nbsp;\@ref(def:ConfidenceIntervalp)) is between
...
> Of the 51 koalas, 18 (35.3%) crossed at least one road. 
> The [...] probability of a koala crossing at least one road during the study was 35.3% (95% CI = 22--48%).
>
...
A study of how paramedics administer pain medication [@lord2009impact] found that
...
\[
   \text{from\ } 0.4479049 - (2\times 0.01183326) = 0.424238 \text{\ to\ } 
                 0.4479049 + (2\times 0.01183326) = 0.471571,
\]
...
However, most people would think of our 95% CI as having a 95% chance of containing the value of population [parameter](#StatisticsAndParameters) $p$.
...
```{r HistoProportionsConditions, echo=FALSE, fig.width=7, fig.height=3.5, out.width = '90%', fig.align="center", fig.cap="The sampling distribution of the proportion of ones rolled, for sets of 20 rolls (left panel) and sets of 60 rolls (right panel)" }
par( mfrow = c(1, 2),
...
Then compute the standard error, which quantifies how much the value of $\hat{p}$ varies from one sample to the next:
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
You must use *proportions* in these formulas, **not** *percentages* (that is, values between 0 and 1 (like 0.23) and **not** 23%).
:::
...
The size of this sampling variation is quantified using a *standard error*.
Using \@ref(eq:stderrorphat):
...
Computing the 'plus' and the 'minus' bits, the approximate 95% CI is from $0.1694 - 0.03954 = 0.12986$ to $0.1694 + 0.03954 = 0.20894$.
