...
```{r RollDiceFig, fig.align="center", out.width="90%", fig.width=7, fig.height=4, fig.cap="The proportion of rolls that are even is not the same for every sample of 25 rolls" }
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=6.5, out.width='70%', fig.cap="The proportion of rolls that are even change from one sample of 25 rolls to the next sample of 25 rolls" }
if (knitr::is_latex_output()){
  set.seed(990099)
  num.rolls <- 25
  num.sims <- 1000
...
```{r CIrelationshipsP, out.width='75%', fig.align="center", fig.cap="A CI gives a range of possible values of $p$ for which it is reasonable to produce the observed value of $\\hat{p}$. The shaded regions represent the regions containing 95\\% of the values of $\\hat{p}$ for each value of $p$.", fig.width=6.5, fig.height=8}
...
```{r, child = if (knitr::is_html_output())  './children/HTML/CIWidth.Rmd'}
```
...
```{r HistoProportionsConditions, fig.width=7, fig.height=3.0, out.width = '80%', fig.align="center", fig.cap="The sampling distribution of the proportion of ones rolled, for sets of 20 rolls (left panel) and sets of 60 rolls (right panel)" }
par( mfrow = c(1, 2),
