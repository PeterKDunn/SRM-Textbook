...
```{r RollDice, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.5}
  ...
  set.seed(99999)
    ...
    for (i in 1:num.sims){
      ...
      plot( c(1, (num.rolls+2)), c(1, num.sims),
            ...
            axes = FALSE)
      roll <- sample(1:6, num.rolls, replace=TRUE)
      all.rolls[i, ] <- roll
...
<!-- ```{r RollDice, echo=FALSE, fig.align="center", out.width="85%", fig.cap="The proportion of rolls that is even changes from one sample of 25 rolls to the next sample of 25 rolls"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/RollDiceMovie.html", frameborder=0, height=600, width=520)  -->
<!-- ``` -->

<!-- ```{r RollDiceFig, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The proportion of rolls that are even change from one sample of 25 rolls to the next sample of 25 rolls" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RollDice10") -->
<!-- } -->
<!-- ``` -->
...
<center>
```{r fig.show="animate", RollDiceHistHTML, echo=FALSE, cache = TRUE, animation.hook="gifski", cache=TRUE, interval=0.0005, loop=FALSE}
if (knitr::is_html_output()){
  set.seed(990099)
    num.rolls <- 25
    num.sims <- 1000
    ...
    for (i in 1:num.sims){
      ...
      out <- hist( prop.even,
                   ...
                   sub = paste("(For this set: proportion of rolls even is ", 
...
<!-- ```{r RollDiceHistHTML, echo=FALSE, fig.cap="Rolling 1000 times", fig.align="center"} -->
<!-- #includeHTML("./Animations/RollDiceHistMovie.html") -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/RollDiceHistMovie.html", frameborder=0, height=450, width=350)  -->
<!-- ``` -->


<!-- ```{r RollDiceHistFig, echo=FALSE, fig.align="center", fig.width=5, out.width="85%", fig.cap="The proportion of rolls that are even change from one sample of 25 rolls to the next sample of 25 rolls" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RollDiceHist1000") -->
<!-- } -->
<!-- ``` -->
...
```{lemma Valuesphat, name="Values for $\\hat{p}$"}
...
Fig.&nbsp;\@ref(fig:NormalDieTheory).
How often would a value of $\hat{p}$ larger than 0.80
be expected?
```
...
plot.normZ(HT.mn, HT.sd, 
           xlab.name="Sample proportion of even rolls out of 25", 
           zlim.lo = -4, zlim.hi = 4, 
           shade.lo.z = -5, shade.hi.z = -5,
#           axis.labels=c(" p - 0.298", " p - 0.199", " p - 0.099", " p", " p + 0.099", " p + 0.199", " p + 0.298"),
           axis.labels=c( 	
...
```{r RollDiceCIMovie, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.1}
if (knitr::is_html_output()){
  set.seed(99999)
    num.rolls <- 25
    num.sims <- 50
    ...
    p.ci <- function(roll){ 
      ...
      upper <- mn + 1.96*se
      lower <- mn - 1.96*se
      list(lower=lower, mn=mn, upper=upper)
    }
    ...
    for (i in 1:num.sims){
      ...
      for (j in (1:i)){
        ...
        lines( c(j, j), c(ci.lower[j], ci.upper[j]),
               lwd=2,
               lty=l.ty,
               col=p.col)
        points( j, ci.est[j], pch=20)
      }
    }
  }
```
...
<!-- ```{r RollDiceCIMovie, echo=FALSE, fig.cap="Most CIs straddle the population proportion; here are the CIs for 50 samples", fig.align="center"} -->
<!-- htmltools::tags$iframe(title = "Most CIs straddle the population proportion", src = "./Animations/RollDiceCIMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->


<!-- ```{r RollDiceCIFig, echo=FALSE, fig.align="center", fig.width=5, out.width="85%", fig.cap="About 95\\% of CIs contain the population proportion"} -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RollDiceCI50") -->
<!-- } -->
<!-- ``` -->
...
axis(side=1,
     cex=0.9,
     at = c( seq(0, 0.4, by=0.1), 0.44, seq(0.5, 1, by=0.1)))
     label = c( as.character(seq(0, 0.4, by=0.1)), "", as.character(seq(0.5, 1, by=0.1)))
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "30%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/tin-1568095_640.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "30%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/eriksson-luo-ppgFysfT7tc-unsplash.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
we almost always have only *one* sample,
and since the value of $p$ is unknown
we don't know if *our* CI includes the population parameter or not.

A reasonable alternative interpretation
is:

> The interval gives a range of values of $p$
> that could plausibly 
> (with 95% confidence)
> have given rise to our observed value of $\hat{p}$.

Or we might say that:

> There is a 95% chance that our computed CI
> straddles the population value of $p$.

These alternatives are not entirely correct,
but are reasonable interpretations.

Many people will write---and you will see it written in many places---that
the CI means that there is a 95% chance that the CI contains the population [parameter](##StatisticsAndParameters).
This is not strictly correct, but is common.

I use this analogy:
Most people say the sun rises in the east.
This is incorrect: the sun doesn't *rise* at all.
It *appears* to rise in the east 
...
Similarly, 
most people use the final interpretation above in practice,
...
<!-- ```{r TwoIntervals, echo=FALSE, fig.cap="Sampling intervals (top) and confidence intervals (bottom)", fig.align="center", fig.height=7} -->
<!-- par( mar=c(0.05, 0.05, 0.05, 0.05)) -->
<!-- openplotmat() -->

<!-- pos <- array(NA, dim=c(6, 2)) -->
<!-- pos[1, ] <- c(0.50, 0.95) # p -->
<!-- pos[2, ] <- c(0.20, 0.65) # p-hat range -->
<!-- pos[3, ] <- c(0.80, 0.65) # p-hat range -->

<!-- pos[4, ] <- c(0.50, 0.15)   # p-hat -->
<!-- pos[5, ] <- c(0.20, 0.45)   # p ranges -->
<!-- pos[6, ] <- c(0.80, 0.45)   # p ranges -->
...
<!-- polygon( x=c(0.20, 0.80, 0.5, 0.20), -->
<!--          y=c(0.65, 0.65, 0.95, 0.65), -->
<!--          border=NA, -->
<!--          col=blueTransparent) -->

<!-- polygon( x=c(0.20, 0.80, 0.5, 0.20), -->
<!--          y=c(0.45, 0.45, 0.15, 0.45), -->
<!--          border=NA, -->
<!--          col=blueTransparent) -->
...
<!--             lcol="black", -->
<!--             lwd=2) -->
<!-- straightarrow(to=pos[4,], from=pos[5,],  -->
<!--             lty=3,  -->
<!--             lcol="black", -->
<!--             lwd=2) -->
<!-- straightarrow(to=pos[4,], from=pos[6,],  -->
<!--             lty=3,  -->
<!--             lcol="black", -->
<!--             lwd=2) -->

<!-- # Other in-between lines -->
<!-- for (i in ( (range.from+1):(range.to-1))){ -->
<!--   straightarrow(from=pos[1,], to=c(i/10, pos[2, 2]),  -->
<!--             lty=3,  -->
<!--             lcol="grey", -->
<!--             lwd=1) -->
<!--   straightarrow(to=pos[4,], from=c(i/10, pos[5, 2]),  -->
<!--             lty=3,  -->
<!--             lcol="grey", -->
<!--             lwd=1) -->
...
<!-- text(x = pos[1, 1],  -->
<!--      y = pos[1, 2], -->
<!--          lab=expression(italic(p)), -->
<!--      pos=3 -->
<!--      ) -->
<!-- text(x = pos[4, 1], -->
<!--      y = pos[4, 2], -->
<!--          lab=expression(hat(italic(p))), -->
<!--      pos=1 -->
<!--      ) -->
...
<!--        angle=15) -->
<!-- text(0.5, 0.58, -->
<!--      "Plausible values for p-hat that could come from p") -->
...
<!--        angle=15) -->
<!-- text(0.5, 0.52, -->
<!--      "Plausible values for p that may have produced p-hat") -->




<!-- # MORE TEXT -->
<!-- greyBackground <- rgb(0.1, 0.1, 0.1, alpha=0.3, maxColorValue = 1) -->


<!-- textrect( c(0.5, 0.70), -->
<!--           radx=0.07, -->
<!--           rady=0.043, -->
<!--           box.col=greyBackground, -->
<!--           lcol=greyBackground, -->
<!--           lab="Population\nestimates\nstatistic", -->
<!--           shadow.size=0) -->
<!-- textrect( c(0.5, 0.40), -->
...
> If we took many samples of 1516 Canadian youth,
> and computed the approximate 95% CI for each one,
> about 95% of those CIs would contain the population proportion.
...
> that could plausibly (with 95% confidence) 
> have produced $\hat{p}=0.241$
> is between 0.192 and 0.236. 

In practice,
the CI is usually interpreted as saying:

> There is a 95% chance that the population proportion
> of Canadian youth who have experienced sleeping difficulties 
> after consuming energy drinks
> is between 0.192 to 0.236.

This is not strictly correct, but is commonly used.
```


```{lemma InterpretCIKoalas, name="Interpretation of a CI"}
In Example \@ref(exm:KoalasCrossingRoads) about koalas crossing roads,
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-karolina-grabowska-4226912.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
```{lemma, StatisticalValidityKoalas, name="Koalas crossing roads"}
...
xn <- seq(0, 0.55, length=100)
yn <- dnorm(xn, mean=1/6, sd = sqrt( p*(1-p)/n) )
...
(rather than the $\pm0.018$ found from the sample):
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/-fflEggczG4" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "30%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/gian-cescon-GxQ13MXLTHQ-unsplash.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
1. Find an approximate 95% CI for the true proportion of people 
with hiccups who are male.
