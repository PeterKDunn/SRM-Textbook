...
The value of $p$ in the population that gave rise to the observed sample proportion could be a bit larger than $\hat{p}$, or a bit smaller than $\hat{p}$.
In other words, the value of $p$ that gave rise to the observed sample proportion is $\hat{p}$, give-or-take a bit.
The amount to 'give-or-take' depends on how confident we wish to be that the interval straddles the value of $p$.
This is like the interval in Eq.\ \@ref(eq:CIpKnownp), but creating the interval around $\hat{p}$ rather than around $p$.

This all means that we can create an interval ($\hat{p}$, give-or-take a bit) that contains the values of $p$ likely to have produced the observed value of $\hat{p}$ (Fig.\ \@ref(fig:CIrelationshipsP)).
...
In this contrived example, though, the CI straddles the known value of $p = 0.5$.
...
```{r, out.width='100%', fig.align="center", fig.cap="oiHohohO", fig.width=8, fig.height=3}
source("R/testCI.R")
```
