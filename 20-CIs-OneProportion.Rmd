...
That is, for some *known* value of $p$, the *unknown* value of $\hat{p}$ is expected to vary between $p - 0.199$ and $p + 0.199$ (Fig. \@ref(fig:CIrelationships), left panel).

However, we *know* the value of $\hat{p}$, but the value of $p$ is *unknown*.
So, what if the roles of $p$ and $\hat{p}$ were 'reversed'?
Then, the values of $p$ that could reasonably be expected to produce the observed value of $\hat{p}$ are between $\hat{p} - 0.199$ and $\hat{p} + 0.199$ (Fig. \@ref(fig:CIrelationships), right panel).

```{r CIrelationships, echo=FALSE, out.width='100%', fig.align="center", fig.cap="Left: A known value of $p$ produces a range of $\\hat{p}$ values. Right: A known value of $\\hat{p}$ could reasonably have come from a variety of values of $p$.", fig.width = 10}
par(mfrow = c(1, 2),
    mar = c(5, 4, 5, 1.5) + 0.5 ) 
