...
Mn <- aggregate(MR2$Mean, by=list(MR2$Month), FUN="mean", na.rm=TRUE, simplify=TRUE)
Mdn <- aggregate(MR2$Mean, by=list(MR2$Month), FUN="median", na.rm=TRUE, simplify=TRUE)

MRsummary <- data.frame(Month=month.abb, "Mean" = Mn[, 2], "Median" = Mdn[, 2])
```
...
if( knitr::is_latex_output() ) {
  kable(BFat,
    ...
    add_header_above(header=c("Butterfat percentages"=5), bold=TRUE, align="c") %>%
    kable_styling(font_size=10)
}
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.25, progress=TRUE}
BalanceMean <- function(locate.fulcrum, numberImages, iteration){
  ...
  # Draw ruler
  polygon( Pos[1:4, 1], Pos[1:4, 2], col="lightblue")
  ...
  lines( c( locate.fulcrum, locate.fulcrum),
         ...
         col=grey(0.3))
  points(locate.fulcrum, h.fulcrum, pch=19, cex=0.75)
...
if (knitr::is_html_output()){
    ...
    for (i in 1:length(FL)){ 
      BalanceMean( FL[i], length(FL), iteration=i )
    }
  }
```
...
if (knitr::is_latex_output()){
  FL <- c(   seq(5, 5.5, by= 0.025), 
             ...
             seq(5.2, 5.3, by= 0.025),
             rep(5.25, 10) )
    BalanceMean( FL[49], length(FL),iteration=length(FL) )
}
```



<!-- ```{r BalanceMeansMovie, echo=FALSE, fig.align="center", fig.cap="The mean is the balance point of the data"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/BalanceMeansMovie.html", frameborder=0, height=650, width=520)  -->
<!-- ``` -->

<!-- ```{r BalanceMeansFig, echo=FALSE, fig.align="center", fig.width=4.5, fig.cap="The mean is the balance point of the data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/BalanceMeans49") -->
<!-- } -->
<!-- ``` -->



```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.20}
FindMean <- function(locate.x = 5.25, numberImages, iteration){
  ...
  for (i in 1:length(milk)){
    points( sort(milk)[i], jump, pch=19)
    ...
    # The distances
    text( 7.5, jump, sprintf("%.2f", round(sort(milk)[i] - locate.x, 2)),
          col=ifelse( locate.x - sort(milk)[i] > 0, "red", "green4"),
          adj=1,
          cex=1)
  ...
  abline(v=locate.x, col="grey")
  text(locate.x, -0.4, sprintf("%.2f", locate.x),
       cex=1)
#  text(locate.x, -0.4, "Trying",
#       cex=1)
  
  
  # The sum  
  segments(  x0=7.1, y0=0, x1=7.6)
  text( 7.5, -0.4, sprintf("%.2f", round( sum(sort(milk)- locate.x), 2)),
        adj=1,
        cex=0.95)
  text( 7.65, jump+jump.extra, "Distance",
        adj=1,
        cex=0.95)
  text( 7.5, -0.2, "Sum:",
        adj=1,
...
if (knitr::is_html_output()){
    for (i in 1:length(meanCandidates)){
      FindMean( meanCandidates[i], numberImages=length(meanCandidates), i)
    }
...
if (knitr::is_latex_output()){
  FindMean( meanCandidates[43], numberImages=length(meanCandidates), 43 )
}
```
...
<!-- ```{r PosNegDistMeansMovie, echo=FALSE, fig.align="center", fig.cap="The mean is where the positive and negative distances sum to zero"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/PosNegDistMeansMovie.html", frameborder=0, height=650, width=520)  -->
<!-- ``` -->
...
<!-- ```{r PosNegDistMeansFig, echo=FALSE, fig.align="center", fig.width=4.5, fig.cap="The mean is the value such that the positive and negative distances sum to zero" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/PosNegDistMeans43") -->
<!-- } -->
<!-- ``` -->
...
if( knitr::is_latex_output() ) {
  kable(Eyes,
    ...
    add_header_above(header=c("Corneal thickness"=4), bold=TRUE, align="c") %>%
    kable_styling(font_size=10)
}
...
if( knitr::is_html_output() ) {
  kable(Eyes,
      ...
      caption="The thickness of the cornea (in microns) in eyes affected by glaucoma",
      col.names=rep("", 4)) %>%
    add_header_above(header=c("Corneal thicknesses"=4), bold=TRUE, align="c")
}
```
...
BFat.sort <- matrix( BFat.sort, ncol=5, byrow=TRUE)
...
<!-- The boxplot in  -->
<!-- Fig. \@ref(fig:DailyStreamflow) (bottom panel) -->
<!-- doesn't even look like a boxplot because the outliers are so large -->
...
stripchart(MR2$Mean[MR2$Month==2], 
           ...
           ylim=c(0.2, 1.8), 
           cex=0.5)
abline( v = Mn[2,2], col="grey")




#stripchart(subset(MR2$Mean, MR2$Mean<2000)[MR2$Month==2], 
#           main="Dot plot of daily streamflow at\nMary Creek (Bell River) in Feb.",
#           xlab="Daily streamflow (in ML)",
#           sub="(From 01 October 1959 to 17 January 2019)",
#           method="jitter", 
...
The sample standard deviation $s$ is mostly found
using 
...
ShowVar <- function(){
  ...
  for (i in 1:length(milk)){
    ...
    # The distances
    text( 7.5, jump, sprintf("%.4f", (sort(milk)[i] - locate.x)),
          col=ifelse( locate.x - sort(milk)[i] > 0, "red", "green4"),
          adj=1,
          cex=0.95)
  ...
  # Highlight the mean/mean guess
  abline(v=locate.x, col="grey")
  text(locate.x, jump + jump.extra, sprintf("%.2f", locate.x),
       cex=0.95)
  text(locate.x, jump + 2*jump.extra, "Mean:",
       cex=0.95)
  

    # The sum  
  segments(  x0=7.0, y0=0, x1=8.7)
  
  text( 7.5, -0.15, sprintf("%.4f", sum( (sort(milk)- locate.x))),
        adj=1,
        cex=0.95)
  text( 7.5, jump+jump.extra, "Dist.",
        adj=1,
        cex=0.95)

  text( 8.5, -0.15, sprintf("%.4f",  sum( (sort(milk)- locate.x)^2)),
        adj=1,
        cex=0.95)
  text( 8.5, jump+jump.extra, expression((Dist.)^2),
        adj=1,
        cex=0.95)
...
abline( h = Eggs.quantiles, 
        ...
        lty=2,
        lwd=1)
bxp(out, las=1, add=TRUE, boxfill=plot.colour)

text( 1.53, Eggs.quantiles[5], "Max. value", pos=1, cex=0.75)
text( 1.53, Eggs.quantiles[4], "75% smaller than this", pos=3, cex=0.75)
text( 1.53, Eggs.quantiles[3], "Median", pos=3, cex=0.75)
text( 1.53, Eggs.quantiles[2], "25% smaller than this", pos=3, cex=0.75)
text( 1.53, Eggs.quantiles[1], "Min. value", pos=1, cex=0.75)

text( 0.65, Eggs.quantiles[4], expression(italic(Q)[3]), pos=3, cex=0.75)
text( 0.65, Eggs.quantiles[3], expression(italic(Q)[2]), pos=3, cex=0.75)
text( 0.65, Eggs.quantiles[2], expression(italic(Q)[1]), pos=3, cex=0.75)
```
...
```{r echo=FALSE, animation.hook="gifski", interval=0.75, cache=TRUE}
Eggs.T <- c(0, 0, 1, 1, 3, 8, 8, 12, 18, 21, 26, 30, 35, 48, 50)
  ...
  for (i in (1:6)){
    ...
    if ( i ==2 ){
      arrows(left, Qs[2],
             ...
             angle=angle,
             lwd=2)
      text(mid, Qs[2], label=expression( italic(Q)[1]), pos=3)
    }
    if (i==3){
      arrows(left, Qs[4],
             axis.edge, Qs[4],
             angle=angle,
             lwd=2)
      text(mid, Qs[4], label=expression( italic(Q)[3]), pos=3)
    }
    if (i>=4){
      arrows(left, Qs[2],
             axis.edge, Qs[2],
      ...
      text( mid.r, Qs[3], label="IQR", pos=4)
    }
  }
}
```
...
<!-- ```{r IQRKrillMovie, echo=FALSE, fig.cap="Computing the IQR for the krill-egg data", fig.align="center"} -->
<!-- htmltools::tags$iframe(title = "Computing the IQR for the krill-egg data", src = "./Animations/IQRKrillMovie.html", frameborder=0, height=500, width=350)  -->
<!-- ``` -->
...
<!-- ```{r IQRKrill, echo=FALSE, fig.align="center", fig.width=6, fig.cap="Computing the IQR for the krill Treatment-group data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/IQRKrill4") -->
<!-- } -->
<!-- ``` -->
...
```{r IQRAgeMovie, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.75}
library(NHANES)
data(NHANES)
  ...
  for (i in (1:6)){
    ...
    if ( i ==2 ){
      arrows(left, Qs[2],
             ...
             lwd=2)
      text(mid, Qs[2], label=expression( italic(Q)[1]), pos=3)
      text(mid, Qs[2], label=paste("=",as.character(Qs[2])), pos=1)
    }
    if (i==3){
      arrows(left, Qs[4],
             axis.edge, Qs[4],
             ...
             lwd=2)
      text(mid, Qs[4], label=expression( italic(Q)[3]), pos=3)
      text(mid, Qs[4], label=paste("=", as.character(Qs[4])), pos=1)
    }
    if (i>=4){
      arrows(left, Qs[2],
             axis.edge, Qs[2],
...
<!-- ```{r IQRKrillMovie, echo=FALSE, fig.cap="Computing the IQR for the krill-egg data", fig.align="center"} -->
<!-- htmltools::tags$iframe(title = "Computing the IQR for the krill-egg data", src = "./Animations/IQRKrillMovie.html", height=640, width=520)  -->
<!-- ``` -->
...
<!-- ```{r DeathAgeBoxplot, echo=FALSE, fig.cap="Boxplot of age at death for Australians in 2012. Note that there are many low outliers, so they are overplotted and look odd...", fig.align="center",  fig.height=4, fig.width=5} -->
...
<!-- # out.DeathAges <- boxplot(Age ~ Gender, data=DeathAges, -->
<!-- #    las=1, -->
...
<!-- #    names=c("Females", "Males"), -->
<!-- #    col=c( "mistyrose", "azure2")) -->
<!-- # points(jitter(rep(1, length(out.DeathAges$out))),  -->
<!-- #        out.DeathAges$out) # Plot outliers with jitter -->
<!-- #  -->
<!-- # boxplot(DeathAges$Age, -->
<!-- #        outline=FALSE, -->
<!-- #        ylim=c(min(c(bar$stats,bar$out)), -->
<!-- #               max(c(bar$stats,bar$out)))) -->
<!-- # points(jitter(rep(1, length(bar$out))), -->
<!-- #       bar$out) -->
<!-- ``` -->

<!-- https://stats.stackexchange.com/questions/43721/outliers-for-boxplot-->
...
```{r HeightsModelQUANTMovie, echo=FALSE, animation.hook="gifski", cache=TRUE}
 if (knitr::is_html_output()) {
   set.seed(6000)
  ...
  x <- seq( HT.mn - 4*HT.sd, HT.mn + 4*HT.sd, length=100)
  y <- dnorm( x,  mean=HT.mn, sd=HT.sd)
...
<!-- ```{r HeightsModelQUANTMovie, echo=FALSE, fig.cap="The model for heights", fig.align="center"} -->
<!-- htmltools::tags$iframe(title = "The model for heights of Australian adult males", src = "./Animations/HeightsModelMovie.html", frameborder=0, height=450, width=350)  -->
<!-- ``` -->


<!-- ```{r HeightsModelQUANTFig, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The model for heights of Australian adult males, plus the histogram from one specific sample of Australian adult males" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/HeightsModel2") -->
<!-- } -->
<!-- ``` -->
...
hist(x, 
   ...
   main="Histogram of husbands' hts\n (1980 OPCS; n=199)",
   col="thistle")
axis(side=1, at=seq(150, 200, by=5), las=2)
...
```{r EmpRule, echo=FALSE, animation.hook="gifski", interval=1, cache=TRUE}
empRule <- function(z){
  zseq <- seq(-4, 4, by=0.1)
  dseq <- dnorm(zseq)
...
#        xlab=expression( paste(italic(z), "-value") ),
        ylab="",
        type="l",
        col="blue",
        axes=FALSE)
  ...
  if (z==1) {
    pEmp <- "68%"
    pEdge <- "16%"
#    title( main="One std dev from the z = 1")
...
#    title( main="When z = 3")
  ...
  polygon( x=c( zseq[zSelect],                 rev( zseq[zSelect] ) ),
           ...
           border="blue",
           lwd=2,
           col=rgb(0, 0, 100, alpha=50, maxColorValue = 255))

  # Labelling
  text(   0, max(dseq)/2, pEmp,  pos=3, cex=1.1)
  text(   3.2, max(dseq)/2, pEdge, pos=3)
  text(  -3.2, max(dseq)/2, pEdge, pos=3)
  
  # Arrows
  arrows( 3.2,        0.45*max(dseq), 
          mean( c(4, z) ), (dseq[z==zseq])*0.15,
          angle=15)
  arrows(-3.2,        0.45*max(dseq), 
         mean( c(-4, -z) ), (dseq[z==zseq])*0.15,
         angle=15)
...
if (knitr::is_latex_output()){
  ...
  empRule(2)
  empRule(3)
#  include_graphics(c("./Animations/images/empRule1", 
#                     "./Animations/images/empRule2",
#                     "./Animations/images/empRule3") )
}
```
...
<!-- ```{r EmpRule, echo=FALSE, fig.align="center", fig.cap="The 68--95--99.7 rule: About 95% of observations are within two standard deviations of the mean"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/empRuleMovie.html", frameborder=0, height=550, width=520)  -->
<!-- ``` -->

<!-- ```{r EmpRuleFig, echo=FALSE, fig.align="center", out.width='32%', fig.width=2, fig.show='hold', fig.cap="The 68--95--99.7 rule: About 95\\% of observations are within two standard deviations of the mean" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics(c("./Animations/images/empRule1",  -->
<!--                      "./Animations/images/empRule2", -->
<!--                      "./Animations/images/empRule3") ) -->
...
```{r IQROutlier, echo=FALSE, animation.hook="gifski", interval=1.75, cache=TRUE}
...
plotIQROutliersAnimation <- function(x, i){
  ...
  if ( (i >= 2) & (i <= 6) ){
    ...
    #points( rep(1, 3), x[x>mild], pch=1, col=grey.pch)
    ...
    #points( rep(1, 2), xsort[1:2], pch=8, cex=1.5)
  }
  if ( i == 1) {
    out <- boxplot(x, 
  ...
  if ( i > 2 ){
    abline( h=qx[2], col="grey", lty=2)
    text(1.4, qx[2], expression(Q[1] == 24.66), cex=0.9)
    abline( h=qx[4], col="grey", lty=2)
    text(1.4, qx[4], expression(Q[3] == 50.58), cex=0.9)
    
    arrows(0.75, qx[2], 0.75, qx[4], angle=15, length=0.1, code=3)
    text(0.6, mean(qx[ c(2, 4)]), expression(IQR== 5.92), cex=0.9)
  }
  
  if ( (i > 3) & (i<8) ){
    abline( lwd=2, col="gray", h=extreme )
    text(1.3, mean( c(extreme, qx[5])), "All observations here\nare extreme outliers", col="grey")
  }
  if ( (i == 4) | (i == 5) ) {
    arrows(0.75, qx[4], 0.75, extreme, angle=15, length=0.1, code=3)
    text(0.6, mean( c(qx[4], extreme)), "Three IQRs")
  }
  
  if ( (i > 5) & (i<8) ){
    abline( lwd=2, col="gray", h=mild )
    text(1.3, mean( c(mild, extreme)), "All observations here\nare mild outliers", col="grey")
  }
  if ( (i == 6) | (i == 7) ) {
    arrows(0.75, qx[4], 0.75, mild, angle=15, length=0.1, code=3)
    text(0.6, mean( c(qx[4], mild)), "1.5 IQRs")
  }
  
  
  if ( i == 7 ) {
    out <- boxplot(x, #xsort[ -(1:3)], 
                   add=TRUE, 
                   col=rgb(0, 0, 0, maxColorValue=256, alpha=0.5), 
                   # range=0, 
                   las=1)
    ...
    points( rep(1, 2), xsort[1:2], pch=8, cex=1.5)
    points( 1, xsort[3], pch=1, cex=1.5)
  }
  if ( i==8 ) {
    out <- boxplot(x, #xsort[ -(1:3)], 
                   #                   add=TRUE, 
...
<!-- ```{r IQROutlier, echo=FALSE, fig.align="center", fig.cap="Using the IQR outlier rule"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/IQROutliersMovie.html", frameborder=0, height=580, width=520)  -->
<!-- ``` -->
...
<!-- ```{r IQROutliers, echo=FALSE, fig.align="center", fig.width=6, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/IQROutliers6") -->
<!-- } -->
<!-- ``` -->
...
hist( perm$Perm,
	...
	las=1)
axis(side=1, at=seq(0, 180, by=20), las=2, cex=0.8 )
axis(side = 2, las=1)
...
boxplot( perm$Perm,
  horizontal=TRUE,
	...
	axes=FALSE,
	las=1)
axis(side=1, at=seq(0, 180, by=20), las=2, cex=0.8 ) # las=2: Perpendicular to axes
box()
```
...
if( knitr::is_latex_output() ) {
  kable(SummaryQS, 
      ...
      align=c("r", "c", "c"),
      caption="Summarising quantitative data") %>%
    add_header_above(header=c(" "=1, "For distributions that are:"=2), bold=TRUE, align="c") %>%
	kable_styling(font_size=10)
}
if( knitr::is_html_output() ) {
  kable(SummaryQS, 
...
<!-- ```{r MatchGraphs, echo=FALSE, fig.cap="Match the histograms and the boxplots", fig.align="center", fig.width=5, fig.height=4, out.width='90%'} -->
...
<!-- xlims <- c( 0, 2 ) -->
<!-- hist(dt1, col=plot.colour, main="A", xlim=xlims, las=1, axes=FALSE, xlab=""); axis(side=1) -->
<!-- boxplot(dt2, col="bisque", main="I", ylim=xlims, las=1, range=0, horizontal=TRUE) -->

<!-- hist(dt2, col=plot.colour, main="B", xlim=xlims, las=1, axes=FALSE, xlab=""); axis(side=1) -->
<!-- boxplot(dt1, col="bisque", main="II", ylim=xlims, las=1, range=0, horizontal=TRUE) -->

<!-- hist(dt3, col=plot.colour, main="C", xlim=xlims, las=1, axes=FALSE, xlab=""); axis(side=1) -->
...
<!-- You will need a calculator to answer some of these questions. -->

<!-- <iframe src="https://h5p.org/h5p/embed/187519" width="1090" height="471" frameborder="0" allowfullscreen="allowfullscreen"></iframe><script src="https://h5p.org/sites/all/modules/h5p/library/js/h5p-resizer.js" charset="UTF-8"></script> -->
