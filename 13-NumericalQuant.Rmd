...
* *Variation*: The values range from about 0.5 to 3 mmol/L, but with some larger values.
...
These are both summarising the same data (the 'average' daily streamflow in February), yet give very different answers.
...
The *population* is 'Jersey cows', and the population mean percentage butterfat (denoted $]mu$) from those cows is to be estimated.
...
the maximum value is `r formatC( round(max(MaryRiver$Mean[MaryRiver$Month == "02"], na.rm = TRUE), 0), format="fg", big.mark=',')`ML, more than *one hundred times* larger than the mean of `r formatC(round(MRsummary[2, 2], 1), big.mark=',')`ML).
...
par(mfrow = c(1, 1))
set.seed(183763286)
stripchart(MaryRiver$Mean[MaryRiver$Month == "02"], 
...
out <- hist(MaryRiver$Mean[MaryRiver$Month == "02"],
            breaks = seq(0, 160000,
                         by = 20000),
            right = FALSE,
            plot = FALSE)
...
```{r BoxplotKrillTreatment2, fig.cap="A boxplot for the krill-egg data; the boxplot just for the treatment group", fig.width=5, fig.height=4, fig.align="center"}
...
```{r IQROutliers, fig.align="center", fig.width=5.5, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data" }
if (knitr::is_latex_output()){ 
