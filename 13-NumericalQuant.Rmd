...
* True or false: The IQR measures the amount of variability in a set of data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The mean and the median can both be called the "average".
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The mean and the median are not always the same value.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The range is a measure of variability in a set of data (but it usually too simple to be useful).
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The standard deviation measures the amount of variability in a set of data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: Another name for the median is $Q_2$.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: $Q_3$ is the median of the largest half of the data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The IQR is a useful measure of the amount of variation in data that are skewed.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
* True or false: The IQR is the difference between the first and second quartiles.
