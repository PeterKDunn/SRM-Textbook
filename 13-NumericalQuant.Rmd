...
```{r EmpRule, animation.hook="gifski", interval=1, cache=FALSE, dev=if (is_latex_output()){"pdf"}else{"png"}}
empRule <- function(z){
  zseq <- seq(-4, 4, 
              by = 0.1)
  dseq <- dnorm(zseq)
...
```{r IQROutlier, animation.hook="gifski", interval=1.75, cache=FALSE, dev=if (is_latex_output()){"pdf"}else{"png"}}
