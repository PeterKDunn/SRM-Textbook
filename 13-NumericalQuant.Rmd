...
Estimate the population mean corneal thickness.\label{thinkBox:EstimateMu}
`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For the butterfat data (Table \@ref(tab:BFatData)), what is the *population* median?\label{thinkBox:PopMedian}
...
What is an estimate of the population median?\label{thinkBox:EstimateMedian}}
...
Using the [68--95--99.7 rule](#def:EmpiricalRule), about 99.7% of the husbands are between what heights?\label{thinkBox:HusbandsHts:997}
...
```{r HusbandHistNormalShadeEmpirical, fig.cap="The heights of husbands, showing the 68--95--99.7 rule in use", fig.align="center", fig.width=6.5, fig.height=2.4, out.width='100%'}
par( mfrow = c(1, 3),
     mar = c(5, 2, 2, 1) )
...
Deleting or removing these outliers simply because they are outliers is a *very bad idea*.
...
One strategy for handling outliers depends on the *reason* for the outlier (@mypapers:dunnsmyth:glms, p. 138):

* **The outlier is a mistake**:
  If the outlier is clearly wrong (e.g., an age of 222), it should be *fixed* if possible (e.g., the person may be 22 years old, not 222 years old).
  When the outlier is clearly an error but the correct value is unknown, *deleting* the observation seem sensible.  
  Similarly, if the observations come from some error or mistake in the data collection (e.g., twice as much fertiliser was accidentally applied), the observation can be deleted.
* **The outlier represents a different population**:
  If the outliers come from a different population that the other observations, the outlier may be removed from the analysis.
  ...
  In this case, the outlier can be removed from the analysis since it belongs to a different population ("students aged over 40"), and all the other observations belong to a different population ("students aged under 40").
...
* **There is no known reason for the outlier**:
...
To compile tables of numerical summary information:
...
Table \@ref(tab:YoghurtDataSummary) makes comparing the two groups easier, but Table \@ref(tab:YoghurtDataSummary2) is probably more common (as it allows more variables to be easily included in the table).
:::
...
Using different output (Fig. \@ref(fig:NHANEDescriptivesBySmoking)), Table \@ref(tab:NHANESSummaryQuant) shows the numerical summaries of direct HDL cholesterol for each group.
...
- Sect. \ref{thinkBox:ValueOfMu}: **We do not know!**
We know the value of the *sample* mean, but not the *population* mean.
We have an *estimate* of the value of the population mean by using the sample mean.
- Sect. \ref{thinkBox:EstimateMu}: The **estimate** of $\mu$ is $\bar{x} = 459$ microns.
- Sect. \ref{thinkBox:PopMedian}: **We do not know!**
We know the value of the *sample* median, but not the *population* median.
We only have an *estimate* of the value of the population median.
- Sect. \ref{thinkBox:EstimateMedian}}: With $n = 8$ observations, the median is ordered observation number $(8 + 1)/2 = 4.5$.
This is interpreted as 'between ordered observation numbers 4 and 5'. 
After sorting into increasing order, the two middle numbers (the 4th and 5th) are 464 and 476. 
The median could be *any* number between 464 and 476, but the usual answer would be that the median is $(464 + 476)/2 = 470$. 
The *sample* median is 470 microns; the value of the *population* median remains unknown.
