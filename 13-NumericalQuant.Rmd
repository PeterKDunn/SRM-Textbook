...
The range is not often used, as it is computed only using the two extreme observations, sois highly influenced by outliers.
...
```{r HusbandHistNormal, fig.cap="The heights of husbands have an approximate normal distribution", fig.align="center", fig.height=2.0, fig.width=6.5, out.width='100%'}
data(HusbandWife)

x <- hh <- HusbandWife$Hht/10
...
Since every sample is likely to contain different men, and every sample will produce a slightly different histogram.
...
The solid line represents the possible 'average histogram' from every possible sample from the population.
...
```{r HeightsModelQUANTMovie, animation.hook="gifski", dev=if (is_latex_output()){"pdf"}else{"png"}}
 if (knitr::is_html_output()) {
   set.seed(6000)
...
`r if (knitr::is_latex_output()) {
   'The 68--95--99.7 rule is shown in Fig.\\ \\@ref(fig:EmpRuleFig).'
} else {
...
```{r EmpRule, animation.hook="gifski", interval=1, dev=if (is_latex_output()){"pdf"}else{"png"}}
source("R/showEmpRule.R")
...
```{r EmpRuleFig, fig.align="center", out.width='32%', fig.width=3, fig.height = 2.5, fig.show='hold', fig.cap="The 68--95--99.7 rule: About 95\\% of observations are within two standard deviations of the mean" }
source("R/showEmpRule.R")
...
Similarly, approximately 95% of the husbands in the *population* would have heights between $173.2 - (2\times 6.88) = 159.4$cm and $173.2 + (2\times 6.88) = 187.0$cm.
...
Bell-shaped (normal) distributions are also studied in Chap.\ \@ref(SamplingDistributions).



```{r HusbandHistNormalShadeEmpirical, fig.cap="The heights of husbands, showing the 68--95--99.7 rule in use", fig.align="center", fig.width=6.7, fig.height=2.0, out.width='100%'}
par( mfrow = c(1, 3),
     mar = c(6, 2, 2, 1) )
...
out1 <- plotNormal(mu = mu,
                   ...
                   cex.tickmarks = 1,
                   axis.labels = NULL)
lines( x = rep( mu - sdev, 2),
       y = c(0, max(out1$y) * 0.9),
       lwd = 1,
       col = "grey")
lines( x = rep( mu + sdev, 2),
       y = c(0, max(out1$y) * 0.9),
...
out1 <- plotNormal(mu = mu,
                   ...
                   cex.tickmarks = 1,
                   axis.labels = NULL)
lines( x = rep( mu - 2 * sdev, 2),
       y = c(0, max(out1$y) * 0.6),
       lwd = 1,
       col = "grey")
lines( x = rep( mu + 2 * sdev, 2),
       y = c(0, max(out1$y) * 0.6),
...
out1 <- plotNormal(mu = mu,
                   ...
                   cex.tickmarks = 1,
                   axis.labels = NULL)
lines( x = rep( mu - 3 * sdev, 2),
       y = c(0, max(out1$y) * 0.4),
       lwd = 1,
       col = "grey")
lines( x = rep( mu + 3 * sdev, 2),
       y = c(0, max(out1$y) * 0.4),
...
```{r IQROutliers, fig.align="center", fig.width=9, fig.height = 4.5, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data", out.width = '99%' }
if (knitr::is_latex_output()){ 
  
  par(mfrow = c(1, 2),
      mar = c(0.5, 4, 4, 0.5) + 0.1)
  boxplot(x,
          main = "The original boxplot\nof permeability",
          ylab = "Permeability (in seconds)",
          ylim = c(0, 180), 
          xlim = c(0.5, 1.5),
          las = 1
...
```{r PermBoxHist, fig.align="center", fig.cap="A boxplot and histogram for the permeability data", fig.width=5, fig.height=4.5}
#par (mfrow = c(2, 1))
...
Deleting or removing these outliers simply because they are identified as outliers is a *very bad idea*.
...
* *The outlier is clearly a mistake* (e.g., an age of 222):
  The mistake should be fixed if possible (e.g., the survey form is still available), otherwise the observation can be *deleted*.
...
* *The outlier represents a different population*:
  ...
  If the the next oldest student in the data is aged 38, the outlier can be removed, since it belongs to a different population ('students aged over 40') than the other observations  ('students aged under 40').
...
* *The reason for the outlier is unknown*:
  *Discarding the outliers routinely is not recommended*, as the outliers are probably real observations that are just as valid as all the others.
  Maybe a different analysis is necessary (for example, using medians rather than means).
...
The permeability data (Fig.\ \@ref(fig:PermBoxHist)) has large outliers, but none seem unreasonable.
Removing these from the analysis would be inappropriate.
:::
...
* Shape:
  Slightly skewed right (from Fig.\ \@ref(fig:NHANESboxplot) or\ \@ref(fig:NHANESHDLHist)).
* Outliers: 
  Software identified some outliers (Fig.\ \@ref(fig:NHANESboxplot)), mostly large values.


```{r NHANEDescriptives, fig.cap="Output for direct HDL cholesterol concentration: jamovi (left); SPSS (right)", fig.align="center",  out.width=c('30%', '8%', '60%'), fig.show="hold"}
...
The RQ is about *comparing* the mean direct HDL cholesterol concentration in the two smoking groups, so compiling a table of summaries for each group is useful.
...
```{r NHANEDescriptivesBySmoking, fig.cap="Output for direct HDL cholesterol concentration, by current smoking status: jamovi (left) and SPSS (right)", fig.align="center",fig.show="hold", out.width=c('34%', '4%', '60%')}
...
if( knitr::is_latex_output() ) {
  kable(Fatalities, 
        ...
        align = "c",
        escape = FALSE,
        caption = "Fatalities at amusement rides in the US") %>%
    row_spec(0, bold = TRUE) %>%
    kable_styling(font_size = 10)
}
if( knitr::is_html_output() ) {
...
The histogram of the direct HDL cholesterol concentration from the NHANES study is shown  in Fig.\ \@ref(fig:NHANESHDLHist).
...
```{r MatchHistoBox, fig.align = "center", fig.cap="Match the histogram with the boxplot", out.width = '100%', fig.width = 6.5, fig.height=4}
if( knitr::is_latex_output() ) {
set.seed(20032009)
num <- 400
...
The data are skewed; perhaps use a median.
In practice, a larger sample would be needed before giving a value to use on a specification sheet.
- Sect.\ \ref{thinkBox:CompareSD}:
In Dataset\ B, more observations are closer to the mean, so the average distance would be a smaller number: the standard deviation for Dataset\ B will be **smaller** than 2.
...
Three standard deviations is $3\times6.88 = 20.64$:
About 99.7% of husbands *in the population* are between $(173.2 - 20.64) = 152.6$cm and $(173.2 + 20.64) = 193.8$cm.
