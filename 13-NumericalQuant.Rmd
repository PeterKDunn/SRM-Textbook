...
## The average value {#ComputeAverage}
...
* [the **sample mean** (or *sample arithmetic mean*)](#Mean), which estimates the population mean; and
...
The word 'average' can refer to either means, median r other measures of centre.
Use the precise terms 'mean' or 'median', rather than 'average', when possible!
:::
...
The 'average' daily streamflow in February could be given using either the mean or the median.
...
To demonstrate ideas, consider a small data set for answering this descriptive RQ: "For mature Jersey cows, what is the average percentage butterfat in their milk?"
...
```{r PosNegDistMeansFig, cache=FALSE, fig.align="center", fig.width=5.5, fig.cap="The mean is the value such that the positive and negative distances sum to zero" }
if (knitr::is_latex_output()){
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
A study of eyes [@ehlers1970corneal] aimed to estimate the average thickness of corneas affected by glaucoma.
Using the data (Table \@ref(tab:EyesData)), estimate the population *median* corneal thickness.
...
Some software uses slightly different rules when $n$ is even.
...
Which 'avearge' would be best to use (for example, to quote an average permeability time on a specification sheet)?  
...
The range is not often used, because it uses only the two extreme observations, and so it is highly influenced by outliers.
...
The *standard deviation* is (roughly) the mean distance that observations are from the mean.
...
\]
where $\bar{x}$ is the sample mean, $x$ represents the individual data values, and $n$ is the sample size.
...
```{r ShowVar, fig.cap="The standard deviation is related to the sum of the squared-distances from the mean", fig.align="center",fig.width=6, out.width='70%'}
...
```{r TwoDatasets, cache=FALSE, fig.height=2.0, fig.width=7.75, out.width='100%', fig.cap="Dotplots of two datasets", fig.align="center"}
...
* 50% of the subjects are aged less than about `r Age.quantiles[3]` ($Q_2$, the median): the second quartile $Q_2 = `r Age.quantiles[3]`$, the median of the data set.
...
Understanding the standard deviation rule first requires studying [bell-shaped distributions](#NormalDistributions). 
...
For convenience then, an 'average' of all these possible histogram could be constructed.
For example,
`r if (knitr::is_latex_output()) {
...
The solid line represents the average of many sample histograms from the population.
...
```{r HeightsModelQUANTFig, fig.align="center", fig.width=5, fig.height=3.5, fig.cap="The model for heights of Australian adult males, plus the histogram from one specific sample if size $n=100$ of Australian adult males" }
...
The smoothed histogram represents 100% of the observations; *every* husband in the sample has a height, and so it must represented somewhere on the histogram.
...
```{r EmpRuleFig, echo=FALSE, fig.align="center", out.width='32%', fig.width=2.75, fig.height = 2.5, fig.show='hold', fig.cap="The 68--95--99.7 rule: About 95\\% of observations are within two standard deviations of the mean" }
if (knitr::is_latex_output()){
...
```{r HusbandHistNormalShadeEmpirical, fig.cap="The heights of husbands, showing the 68--95--99.7 rule in use", fig.align="center", fig.width=6.5, fig.height=2.2, out.width='100%'}
par( mfrow = c(1,3),
     mar = c(4, 2, 2, 1) )
