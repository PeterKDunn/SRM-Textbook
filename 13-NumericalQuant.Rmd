...
```{r IQROutliers, fig.align="center", fig.width=7, fig.height = 4.5, fig.cap="Mild and extreme outliers, using the IQR rule, for the permeability data", out.width = '99%' }
if (knitr::is_latex_output()){ 
...
```{r PermBoxHist, fig.align="center", fig.cap="A boxplot and histogram for the permeability data", fig.width=5, fig.height=6}
par (mfrow = c(2, 1))
...
Using the software output (Fig.\ \@ref(fig:NHANEDescriptives)), the direct HDL cholesterol concentration can be summarised numerically:
...
Using different output (Fig.\ \@ref(fig:NHANEDescriptivesBySmoking)), Table\ \@ref(tab:NHANESSummaryQuant) shows the numerical summaries of direct HDL cholesterol concentration for each group, and the difference.


```{r NHANEDescriptivesBySmoking, fig.cap="Output for direct HDL cholesterol concentration, by current smoking status: jamovi (left) and SPSS (right)", fig.align="center",fig.show="hold", out.width=c('33%', '5%', '60%')}
...
This could impact the results (non-response bias; Sect.\ \@ref(SelectionBias)), especially if those who provide data and those who do not are different regarding direct HDL cholesterol concentration.
:::
