...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.25, progress=TRUE, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.20, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
```{r echo=FALSE, animation.hook="gifski", interval=0.75, cache=TRUE, dev=if (is_latex_output()){"pdf"}else{"png"}}
Eggs.T <- c(0, 0, 1, 1, 3, 8, 8, 12, 18, 21, 26, 30, 35, 48, 50)
...
```{r IQRAgeMovie, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.75, dev=if (is_latex_output()){"pdf"}else{"png"}}
library(NHANES)
data(NHANES)
...
```{r HeightsModelQUANTMovie, echo=FALSE, animation.hook="gifski", cache=TRUE, dev=if (is_latex_output()){"pdf"}else{"png"}}
 if (knitr::is_html_output()) {
   set.seed(6000)
...
```{r EmpRule, echo=FALSE, animation.hook="gifski", interval=1, cache=TRUE, dev=if (is_latex_output()){"pdf"}else{"png"}}
empRule <- function(z){
  zseq <- seq(-4, 4, by=0.1)
  dseq <- dnorm(zseq)
...
```{r IQROutlier, echo=FALSE, animation.hook="gifski", interval=1.75, cache=TRUE, dev=if (is_latex_output()){"pdf"}else{"png"}}
