...
# Summarising quantitative data {#NumericalQuant}
...
* *Outliers*: Some large outliers are present (but hard to see on the histogram).
...
* [**P**arameters](#StatisticsAndParameters) numerically describe **p**opulations, and values of parameters are usually unknown.
* [**S**tatistics](#StatisticsAndParameters) numerically describe **s**amples, to estimate unknown parameters.
...
## Tabulating quantitative data {#TabulatinQuant}
...
For the butterfat data (Table\ \@ref(tab:BFatData)), what is the value of $\mu$?\label{thinkBox:ValueOfMu}
...
KrillEggs2 <- cbind( rbind(Eggs.T[1:8],
                           Eggs.T[9:16]), 
                     rbind(Eggs.C[1:8],
                           Eggs.C[9:16])
)
...
if( knitr::is_latex_output() ) {
  ...
  add_header_above(header = c("Treatment group " = 8, 
                              " Control group" = 8), 
	           bold = TRUE, 
		   align = "c") %>%
  kable_styling(font_size = 10)
...
```{r IQRKrill, fig.align="center", fig.width=8, fig.height=3, fig.cap="Left: A boxplot for the krill-egg data (Treatment group); right: Computing the IQR for the same data", out.width='100%' }
if (knitr::is_latex_output()){
...
#knitr::include_graphics("OtherImages/SPACER.png")
  ...
  boxplot( Eggs.T, 
           col = plot.colour,
           las = 1,
           main =  "The IQR for the number\nof eggs laid: Treatment group",
           ylim = c(-5, 50),
           ylab = "Number of eggs")
...
The boxplot for the age of respondents is
`r if (knitr::is_latex_output()) {
   'shown in Fig.\\ \\@ref(fig:IQRAge).'
} else {
   'shown below.'
}`
For these data:
...
```{r  IQRAge, fig.align="center", out.width='50%', fig.height=3, fig.width=4.5, fig.cap="Computing the IQR for the age in the NHANES dataset" }
if (knitr::is_latex_output()){
...
## Comparing quantitative variables {#Summary-Info-Tables}
