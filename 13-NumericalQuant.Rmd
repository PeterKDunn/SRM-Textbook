...
In some cases, the features of the data displayed in the graph can be described *numerically*.
That is the purpose of this chapter: to learn how to summarise *quantitative* data numerically.
...
A number that describes a feature of a *population* is called a [*parameter*](#StatisticsAndParameters).
The values of parameters are usually unknown.
In contrast, a number that describes a feature of a *sample* is called a  [*statistic*](#StatisticsAndParameters).
That is:

* **S**amples are numerically described by [**s**tatistics](#StatisticsAndParameters);
* **P**opulations are numerically described by [**p**arameters](#StatisticsAndParameters).


::: {.definition #Parameter name="Parameter"}
A [**parameter**](#StatisticsAndParameters) is a number describing some feature of a population.
:::


::: {.definition #Statistic name="Statistic"}
A [**statistic**](#StatisticsAndParameters) is a number describing some feature of a sample (to estimate a population *parameter*).
:::


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The RQ identifies the population, but in practice a sample is studied.
...
<div style="float:right; width: 222x; border: 1px; padding:10px">
<img src="Pics/iconmonstr-weather-109-240.png" width="50px"/>
</div>


The average (or *location*, or *centre*, or *typical value*) for *quantitative sample data* can be described in many ways.
...
<div style="float:right; width: 222x; border: 1px; padding:10px">
<img src="Illustrations/river-4221472_640.jpg" width="200px"/>
</div>
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/river-4221472_640.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
Consider the *daily* river flow volume (called 'streamflow') at the Mary River from 01 October 1959 to 17 January 2019, summarised by month in Table \@ref(tab:MRtable)
...
These two common ways of measuring the same thing (the 'average' daily streamflow in February) give very different answers.
Why?
...
### The average: means {#Mean}

The mean of the population is denoted by $\mu$, and its value is almost always unknown.

Instead, the mean of the population is *estimated* by the mean of the sample, denoted by $\bar{x}$ (an $x$ with a line above it).
...
To demonstrate ideas, consider a small data set for answering this descriptive RQ:

> For mature Jersey cows, what is the average percentage butterfat in their milk?

The *population* is 'milk from Jersey cows', and an estimate of the population mean percentage butterfat is sought, denoted $\mu$.
...
Alternatively, the mean is the value such that the positive and negative distances of the observations from the mean add to zero
`r if (knitr::is_latex_output()) {
   '(Fig. \\@ref(fig:PosNegDistMeansFig); again, the online version has an animation.)'
} else {
   ', as in the animation below.'
}`
Both of these explanations seem reasonable for identifying the 'average' of the data.
...
```{r BalanceMeansFig, fig.align="center", fig.width=4.5, fig.cap="The mean is the balance point of the data" }
if (knitr::is_latex_output()){
...
```{r PosNegDistMeansFig, fig.align="center", fig.width=4.5, fig.cap="The mean is the value such that the positive and negative distances sum to zero" }
if (knitr::is_latex_output()){
...
For the Jersey cow data (Example \@ref(exm:JerseyCows)), an *estimate* of the population mean percentage butterfat is found by summing all $n = 10$ observations and dividing by $n$:
\begin{align*}
   \overline{x} 
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
Usually, software (such as jamovi or SPSS) or a calculator (in *Statistics Mode*) is used to compute the sample mean.
However, knowing *how* these quantities are computed is important.
:::


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For the butterfat data (Table \@ref(tab:BFatData)), what is the value of $\mu$, the *population* mean?
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
A study of eyes [@ehlers1970corneal] aimed to estimate the average thickness of corneas affected by glaucoma.
The collected data (in microns) are shown in Table \@ref(tab:EyesData).
...
A useful rule-of-thumb is to round to one or two more significant figures than the original data.
...
### The average: medians {#Median}

The median is a value separating the larger half of the data from the smaller half of the data.
In a data set with $n$ values, the median is *ordered* observation number $\displaystyle \frac{n + 1}{2}$.
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/jersey-cow-4269262_640.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
To find the sample median for the Jersey cow data (Example \@ref(exm:JerseyCows)), first arrange the data *in numerical order* (Table \@ref(tab:JerseyCowsSorted)).
...
Since these values happen to be the same, the sample median is $5.20$ percent.
:::
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For the butterfat data (Table \@ref(tab:BFatData)), what is the *population* median?
...
Using the collected data (Table \@ref(tab:EyesData)), estimate the population *median* corneal thickness.
What is the population median?

`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
With $n = 8$ observations, the median is ordered observation number $(8 + 1)/2 = 4.5$.
This is interpreted as 'between ordered observation numbers 4 and 5'. 

After sorting into increasing order, the two middle numbers (the 4th and 5th) are 464 and 476. 
The median could be *any* number between 464 and 476, but the usual answer would be that the median is $(464 + 476)/2 = 470$. 
...
```{r}
LessThanMean <- sum( MR2$Mean[MR2$Month == 2] < 1123.2, na.rm = TRUE ) / 
...
A dot chart of the daily stream flow (Fig. \@ref(fig:DailyStreamflow)) shows that the data are *very* highly right-skewed, with many *very* large outliers:
the maximum value is `r sprintf( "%.1f", max(MR2$Mean[MR2$Month == 2], na.rm = TRUE))`ML, more than *one hundred times* larger than the mean of `r round(MRsummary[2, 2], 1)`ML).
...
```{r DailyStreamflow, fig.cap="A dot plot of the daily streamflow at Mary River from 1960 to 2017, for February. The vertical grey line is the mean value. Many large outliers exist, so the data near zero are all squashed together", fig.align="center", fig.width=5, fig.height=3}
par(mfrow = c(1, 1))
set.seed(183763286)
...
The streamflow data are very highly skewed (to the right), which is important here:
...
For the Mary River data, the large outliers---and the fact that they are so *extreme* and abundant---result in the mean being substantially influenced by the outliers, which explains why the mean is much so larger than the median.
...
Many ways exist to measure the variation in a data set, including:
...
### Computing variation: Range {#VariationRange}
...
The range is not often used, because only the two extreme observations are used, so it is highly influenced by outliers.
...
For Jersey cow data (Example \@ref(exm:JerseyCows)), the range is:
\[
...
### Computing variation: Standard deviation {#VariationStdDev}

The population standard deviation is denoted by $\sigma$ ('sigma', the [parameter](#StatisticsAndParameters)) and is estimated by the sample standard deviation $s$ (the [statistic](#StatisticsAndParameters)).

The standard deviation is the most commonly-used measure of variation, but is complicated to compute manually (but you don't need to!).
...
The *standard deviation* is (roughly) the mean distance that the observations are away from the mean.
...
**You do not have to use the formula** to calculate $s$, but we will demonstrate to show exactly what $s$ calculates.
The formula is:

\[
	s = \sqrt{ \frac{\sum(x - \bar{x})^2}{n - 1} },
\]
where $\bar{x}$ is the sample mean, $x$ represents the data values, and $n$ is the sample size.
...
For the Jersey cow data (Example \@ref(exm:JerseyCows)), find the *deviations* of each observation from the mean of $5.25$ (Fig. \@ref(fig:ShowVar)).
Then follow the steps outlined.
From Fig. \@ref(fig:ShowVar), the sum of the squared distances is 2.7650.
...
```{r ShowVar, fig.cap="The standard deviation is related to the sum of the squared-distances from the mean", fig.align="center",fig.width=5.5}
...
```{r TwoDatasets, fig.width=5, fig.height=3, fig.cap="Dotplots of two sets of data", fig.align="center"}
...
If you do not get this value, you may be pressing the wrong button on your calculator.
`r webexercises::unhide()`
`r if (knitr::is_latex_output()) '-->'`
:::


### Computing variation: IQR {#VariationIQR}
...
To understand the IQR, understanding *quartiles* first is necessary.
...
For example, a boxplot of the egg-krill data [@data:Greenacre2016:reporting] was shown in Example \@ref(exm:KrillData).
The data are repeated in Table \@ref(tab:KrillDataTable2), and the boxplot in Fig. \@ref(fig:BoxplotKrillTreatment2).
...
*  75% of the observations are smaller than about `r KrillT.quantiles[4]`, represented by the line at the top of the central box.
   This is $Q_3$, or the **third quartile**.
*  50% of the observations are smaller than about `r KrillT.quantiles[3]`, and this is represented by the line in the centre of the central box.
   This is $Q_2$, the **second quartile** or the **median**.
*  25% of the observations are smaller than about `r KrillT.quantiles[2]`, and this is represented by the line at the bottom of the central box.
   This is $Q_1$, the **first quartile**.

The IQR is $Q_3 - Q_1$ = $`r KrillT.quantiles[4]` - `r KrillT.quantiles[2]`$: $\text{IQR} = `r KrillT.quantiles[4] - KrillT.quantiles[2]`$.
`r if (knitr::is_latex_output()) {
...
Consider the NHANES data [@data:NHANES3:Data; @data:NHANES3; @data:NHANES:Rpackage].
The boxplot for the age of respondents in the NHANES data set is
`r if (knitr::is_latex_output()) {
...
* About 75% of the subjects are aged less than about `r Age.quantiles[4]` ($Q_3$): the third  quartile $Q_3 = `r Age.quantiles[4]`$, the median of the *largest half* of the data.
* About 50% of the subjects are aged less than about `r Age.quantiles[3]` ($Q_2$, the median): the second quartile $Q_2 = `r Age.quantiles[3]`$, the median of the data set.
* About 25% of the subjects are aged less than about `r Age.quantiles[2]` ($Q_1$): the first quartile $Q_1 = `r Age.quantiles[2]`$, the median of the *smallest half* of the data.
...
```{r  IQRAge, fig.align="center", fig.width=6, fig.cap="Computing the IQR for the age in the NHANES data set" }
if (knitr::is_latex_output()){
    boxplot( NHANES$Age, 
             col = plot.colour,
             las = 1,
...
### Computing variation: Percentiles {#VariationPercentiles}
...
For the streamflow data at the Mary River (Table \@ref(tab:MRtable)), the February data is highly right-skewed (Fig. \@ref(fig:DailyStreamflow)):
...
Since the standard deviation calculation uses the mean, it is impacted in the same way as the mean by outliers and skewness, so the standard deviation is best used with approximately symmetric data.
...
Describing the skewness numerically is possible; however, in this book the shape will be described just using words (skewed, approximately symmetric, bimodal, etc.) as before (Sect. \@ref(SummariseData)).


::: {.example #Skewness name="Skewness"}
The Australian Bureau of Statistics ([ABS](http://www.abs.gov.au/)) records the  [age at death of Australians](http://www.abs.gov.au/AUSSTATS/subscriber.nsf/log?openagent&33020do008_2012.xls&3302.0&Data%20Cubes&19D242509768F63BCA257C1B000D6ECA&0&2012&07.11.2013&Latest).
...
Outliers are 'unusual' observations: observation quite different (larger or smaller) than the bulk of the data.
Rules for deciding whether or not an observation is 'unusual' are arbitrary.
...
* The [*standard deviation rule*](#OutliersStdDevRule), useful when the data have an approximately symmetric distribution.
* The [*IQR rule*](#OutliersIQRrule), useful in other situations.

Understanding the first rule requires studying [bell-shaped distributions](#NormalDistributions) first. 
[Knowing which rule to use](#CompareOutlierRules) is important.


### Bell-shaped (normal) distributions and the 68--95--99.7 rule {#NormalDistributions}

To begin, identifying outliers will be studied for data approximately symmetrically distributed.
More specifically, bell-shaped symmetric will be studied.
For example, the heights of husbands in the UK [@data:hand:handbook; @data:Marsh1988:ExploringData] have an approximate bell shape (Fig. \@ref(fig:HusbandHistNormal), left panel).
...
Of course, every sample is likely to contain different men, and every sample of men will produce a slightly different histogram.

For convenience then, histograms may be *smoothed*, so that the smoothing produces a shape that represents an 'average' of all these possible sample histograms (in other words, an estimate of how the heights may be distributed in the *population*).
For example,
`r if (knitr::is_latex_output()) {
...
```{r HeightsModelQUANTFig, fig.align="center", fig.width=5, fig.height=4, fig.cap="The model for heights of Australian adult males, plus the histogram from one specific sample if size $n=100$ of Australian adult males" }
...
The smoothed histogram can be considered as representing 100% of the observations; after all, *every* husband in the sample has a height represented somewhere in the histogram.
When we do this, the *areas* under the normal curve are theoretical percentages of the total number.


```{r HusbandHistNormal, warning=FALSE, fig.cap="The heights of husbands have an approximate normal distribution", fig.align="center", fig.height=2.25, fig.width=6.5, out.width='90%'}
hts <- 
...
* The middle 50% of husbands (Fig. \@ref(fig:HusbandHistNormalShade), centre  panel) are between about 168 and 178cm tall.
* The tallest 20% of husbands (Fig. \@ref(fig:HusbandHistNormalShade), right panel) are taller than about 179cm.


```{r HusbandHistNormalShade, fig.cap="The heights of husbands, with certain percentages shaded", fig.align="center", fig.width=6, fig.height=2, out.width='90%'}
par( mfrow = c(1,3),
     mar = c(4, 1, 2, 1) )
...
Importantly, for *any* normal distribution, whatever the mean or standard deviation, the areas under the smoothed curve approximately follow the following important rule.
...
The 68--95--99.7 rule can be used to understand variables that have an approximate normal distribution.
...
Similarly, approximately 95% of the husbands would have heights between $173.2 - (2\times 6.88) = 159.4$cm and $173.2 + (2\times 6.88) = 187.0$cm.


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For the husbands' heights, the sample mean height is $173.2$cm; the sample standard deviation is $6.88$cm.
Using the [68--95--99.7 rule](#def:EmpiricalRule), about 99.7% of the husbands are between what heights?
...
```{r HusbandHistNormalShadeEmpirical, fig.cap="The heights of husbands, showing the 68--95--99.7 rule in use", fig.align="center", fig.width=6.5, fig.height=2.5, out.width='90%'}
par( mfrow = c(1,3),
     mar = c(4, 1, 2, 1) )
...
Although this rule is based on normal distributions, it has proven useful for many approximately-symmetric distributions. 

All rules for identifying outliers are arbitrary, and the standard deviation rule is sometimes given slightly differently.
For example, outliers may be identified as observations more than 2.5 standard deviations away from the mean.
Since all rules for identifying outliers are arbitrary, both rules are acceptable.
...
Measurements of permeability time (the time for water to permeate the sheets) were taken from 81 pieces of material (in seconds).
...
Then, *mild outliers* are shown using a $\circ$, and *extreme outliers* are shown using a $\star$.
:::


You don't need to *do* this (that's what software is for), but you do need to *understand* what the software is doing.
`r if (knitr::is_latex_output()) {
...
* The IQR ruke: for *any distribution*, but primarily for those skewed or with outliers.
...
The boxplot identifies the observations in the histogram that seem to be outliers.
:::
...
In general, deleting or removing these outliers simply because they are outliers is a *very bad idea*.
...
However, there are some exceptions (see @mypapers:dunnsmyth:glms, p. 138):

* **Mistakes**:
  If the outlier is clearly wrong (e.g., an age of 222), you can either *fix* it (the person may be 22 years old, not 222 years old) if possible (and it is not often possible), or you can *delete* it.  
  Similarly, if the observations come from some error or mistake in the data collection (e.g., twice as much fertiliser was accidentally applied), again it can be deleted.
* **A different population is represented**:
  If the outliers come from a different population that the other observations, they  may be removed from the analysis.  
  
  For example, consider a study of how often students exercise.
  One observation is found to be an outlier; closer inspection find that this student is aged 65, but the next oldest student in the data is aged 38.
  In this case, the outlier can be removed from the analysis as it belongs to a different population ("students aged over 40"), and all the other observations belong to a different population ("students aged under 40").
...
* **Unknown reason**:
  ...
  In these situations, the solution is unclear.    
  Discarding the outliers routinely is not recommended, as they are probably real observations that are just as valid as all the others. 
...
The Mary River dataset used in Sect. \@ref(ComputeAverage) has many *extreme* large outliers, but each of them is reasonable. 
They probably correspond to flooding events.
Removing these from the analysis would be silly.
:::


::: {.example #OutliersWhatToDo2 name="Outliers"}
The permeability data (Fig. \@ref(fig:PermBoxHist)) has some *extreme* large outliers.
...
A study [@data:Ejtahed2012:Yoghurt] assessed the effects of probiotic and conventional yoghurt on blood glucose and antioxidant status in Type 2 diabetic patients.
...
Table \@ref(tab:YoghurtDataSummary) makes comparing the two groups easier, but Table \@ref(tab:YoghurtDataSummary2) is the more conventional orientation
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Do  you think a difference exists between the mean BMI in the two groups *in the population*, based on  Tables \@ref(tab:YoghurtDataSummary) and \@ref(tab:YoghurtDataSummary2)?
...
In Sect. \@ref(NHANESGraphs), the NHANES data were introduced (@data:NHANES3:Data, @data:NHANES3, @data:NHANES:Rpackage), and graphs were used to understand the data
...
Using the software output (jamovi: Fig. \@ref(fig:NHANEjamoviDescriptives); SPSS: Fig. \@ref(fig:NHANESPSSDescriptives)), the direct HDL cholesterol can be summarised numerically:
...
Using different output (jamovi: Fig. \@ref(fig:NHANEjamoviDescriptivesBySmoking); SPSS: Fig. \@ref(fig:NHANESPSSDescriptivesBySmoking)), Table \@ref(tab:NHANESSummaryQuant) shows the numerical summaries of direct HDL cholesterol for each group.
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
Notice that information about current smoking status is unavailable for all people in the study.
This could impact the results, especially if those who provide data and those who do not are different regarding direct HDL.
:::


The RQ, as usual, asks about the *population*.
The RQ cannot be answered with certainty, only using one of the many possible samples, since every sample is likely to be different.
...
* **The *population* means are the same**, but the *sample* means are *different* simply because of the people who ended up in this sample.
...
The difficulty, of course, is knowing which of these two reasons ('hypotheses') is the most likely reason for the difference between the sample means.
...
1. True or false: The IQR measures the amount of variability in a set of data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
2. True or false: The mean and the median can both be called the "average".
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
3. True or false: The mean and the median are not always the same value.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
4. True or false: The range is a measure of variability in a set of data (but it usually too simple to be useful).
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
5. True or false: The standard deviation measures the amount of variability in a set of data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
6. True or false: Another name for the median is $Q_2$.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
7. True or false: $Q_3$ is the median of the largest half of the data.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
8. True or false: The IQR is a useful measure of the amount of variation in data that are skewed.
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
9. True or false: The IQR is the difference between the first and second quartiles.
...
The [average monthly SOI](http://www.bom.gov.au/climate/current/soihtm1.shtml) values in August from 1995 to 2000 are shown in Table \@ref(tab:SOIvalues).
...
1. For which data sets would the mean and standard deviation be the appropriate numerical summary?  
