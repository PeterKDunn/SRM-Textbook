...
However, annual incomes are much larger than cents, and vary at scales much greater than cents, and so are usually treated as continuous data.
:::
\index{Quantitative data|)}
...
*Qualitative data* are not *mathematically* numerical data: they comprise mutually exclusive, exhaustive categories or labels.\index{Mutually exclusive}\index{Exhaustive}
:::
...
Alternatively, the variable could be defined as 'Does the person use public transport to commute to work'.
...
Most *statistical* software requires variables to be classified as quantitative or qualitative (and perhaps discrete or continuous; ordinal or nominal).
...
`r if( knitr::is_html_output() ) {torf( answer = TRUE )}`
1. 'School grade is a quantitative discrete variable.
`r if( knitr::is_html_output() ) {torf( answer = FALSE)}`
:::
...
::: {.exercise #NHANES}
The *National Health and Nutrition Examination Survey* (<span style="font-variant:small-caps;">bhadp</span>; @data:NHANES3:Data) survey:

> examines a nationally representative sample of about $5\,000$ persons each year

The following questions, among many others, appear in the 2021--2023 <span style="font-variant:small-caps;">bhadp</span> survey, and are asked about the person selected in the household (SP) to complete the questionnaire:
...
@swinnen2018influence studied using ankle-foot orthoses in children with cerebral palsy.
