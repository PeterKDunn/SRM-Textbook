...
```{r WeightGainNormal2, echo=FALSE, fig.cap="The sampling distribution is a normal distribution; it shows how the sample mean weight gain varies in samples of size $n = 68$ when the population mean gain is $0$", fig.align="center", fig.width=9.0, fig.height=2.5, out.width='95%'}
mn <- mean(SW$GainWt)
n <- length(SW$GainWt)
stdd <- sd(SW$GainWt)
...
```{r WeightGainTestOutputHT, echo=FALSE, fig.cap="The weight-gain data: jamovi output (top) and SPSS output(bottom)", fig.align="center", out.width=c("95%","85%"), fig.show="hold"}
...
```{r CaptoriljamoviHT, echo=FALSE, fig.cap="jamovi (top) and SPSS (bottom) output for the Captoril data", fig.align="center", out.width="80%", fig.show="hold"}
...
if( knitr::is_latex_output() ) {
  T1 <- kable( FR[1:7, ],
               ...
               longtable = FALSE) %>%
    row_spec(0, bold = TRUE)
  T2 <- kable( rbind(FR[8:13, ], c(NA, NA, NA)),
               format = "latex",
               row.names = FALSE,
               col.names = c("Sept.", "March", "Reduction"),
               booktabs = TRUE,
