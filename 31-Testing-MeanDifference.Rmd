...
## Example: chamomile tea {#CoffeeExtractHT}

(This study was seen in Sect.\ \@ref(ChamomileTea-Paired-CI).)
A study of patients with Type\ 2 diabetes mellitus (T2DM) randomly allocated $32$ patients into a control group, and $32$ to receive chamomile tea (@rafraf2015effectiveness, p.\ 164).
Summary data are given in Table\ \@ref(tab:TGsummaryTable).
The following descriptive RQ can be asked for *both* groups:

> For patients with T2DM, is there a mean reduction in TG after eight weeks?

For *both* groups, a hypothesis test can be conducted to see if the evidence supports a mean *reduction* in TG, or whether any reduction is due to sampling variation:  
\[
  \text{$H_0$: $\mu_d = 0$\quad and\quad $H_1$: $\mu_d \ne 0$},
\]
where $\mu_d$ is the population mean change in TG.

For the *tea group*, the mean *reduction* in TG is $38.62$\ mg.dl^-1^, and the standard error is $\text{s.e.}(\bar{d}) = 5.37$ (found in Sect.\ \@ref(ChamomileTea-Paired-CI).)
The test statistic is  
\[
  t = \frac{38.62 - 0}{5.37} = 7.19,
\]
which is a *huge* $t$-score. 
The one-tailed $P$-value will be *very* small (using the 68--95--99.7 rule), and certainly smaller than $0.001$.

For the *control group*, the mean *increase* in TG is $7.12$\ mg.dl^-1^, and the standard error is $\text{s.e.}(\bar{d}) = 6.48$ (found in Sect.\ \@ref(ChamomileTea-Paired-CI).)
The test statistic is  
\[
  t = \frac{7.12 - 0}{6.48} = 1.10,
\]
...
> There is very strong evidence $t = 7.19$; one-tailed $P < 0.001$) of a mean reduction in TG for the chamomile drinkers ($38.62$\ mg.dl^-1^; approx.\ $95$% CI from $6.55$ to $18.45$\ mg.dl^-1^), but *no* evidence ($t = 1.10$; two-tailed $P > 0.10$) for the control group ($-7.12$\ mg.dl^-1^; approx.\ $95$% CI from $-5.48$ to $20.08$\ mg.dl^-1^).

The sample sizes are larger than $25$, so the results are statistically valid.

The two groups appear to show different mean reductions in TG, suggesting there may be evidence that chamomile tea reduces TG compared to the control... but perhaps the difference is simply due to sampling variation.
