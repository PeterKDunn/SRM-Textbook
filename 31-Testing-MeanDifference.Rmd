...
The data are *paired* (Sect.\ \@ref(PairedIntro)), which is a form of blocking (Sect.\ \@ref(ExpManagingConfounding)) and used to manage confounding.
...
The differences here are the percentage N in *non-irrigated* sites minus percentage N in *irrigated* sites.
However, the differences could be computed as the percentage N in *irrigated* sites minus percentage N in *non-irrigated* sites.
...
Since the raw data are available, the data should be summarised graphically (Fig.\ \@ref(fig:SoilNGraphs)) and numerically (Table\ \@ref(tab:SoilNSummary)), using information provided by software (Fig.\ \@ref(fig:Nitrogenjamovi)).
...
The RQ asks if the mean percentage nitrogen *reduces* in the *population* when sites are irrigated.
If the difference is defined as the percentage N in *non-irrigated* sites minus *irrigated* sites, the RQ is asking if the mean difference is zero, or if it is greater than zero.
...
The *null* hypothesis is 'there is *no mean change* in percentage N, in the population' (Sect.\ \@ref(AboutHypotheses)):
...
This hypothesis says that the mean change in the population is *greater than* zero, because of the wording of the RQ, and because of how we defined the differences.
(If the differences had been defined in the opposite way---as 'the percentage N in *irrigated* sites minus *non-irrigated* sites'--- then the alternative hypothesis would be $\mu_d < 0$, which has the same *meaning*.)
...
In other words, the sample mean changes $\bar{d}$ have a sampling distribution.
...
The $t$-score locates the *observed* sample statistic on the sampling distribution (Fig.\ \@ref(fig:PercentageNSamplingDist)).
...
At the left of the output (Fig.\ \@ref(fig:Nitrogenjamovi)), the order implies the differences are found as `NonirrigatedN` minus `IrrigatedN`.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Just saying 'evidence of a difference' is insufficient.
...
This condition ensures that the *distribution of the sample mean differences has an approximate normal distribution* (so that, for example, the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule) can be used).
...
Indeed, the output shows that $P = 0.161$, so there is no evidence of a mean difference in first-flowering day.
We write:
...
## Example: growing squash {#GrowingSquashHT}

(This study was seen in Sect.\ \@ref(GrowingSquashCI).)
...
Since the same farms are used in both years, the data are *paired* (Table\ \@ref(tab:FruitsDataHT)).
Data is missing for Farm\ 20 in the dry year (2014), so there are $n = 23$ differences.

```{r FruitsDataHT, echo=FALSE}
data(Fruit)
...
In Table\ \@ref(tab:FruitsDataHT), the changes refer to the *increase* in fruit weight from 2014 (dry) to 2015 (normal).
...
`r if( knitr::is_latex_output() ) {
  '(Table\\ \\@ref(tab:FruitsDataHT)),'
} else {
  '(see above),'
}`
a numerical summary *must* summarise the differences, as the RQ is about the mea difference.
Each group can be summarised too.
The information is found using software (Fig.\ \@ref(fig:FruitjamoviHT)), and can be compiled into a table (Table\ \@ref(tab:FruitSummaryHT)).
...
```{r FruitjamoviHT, fig.cap="jamovi output for the fruit-weight data", fig.align="center", out.width=c("100%"), fig.show='hold'}
...
```{r, FruitPlotsHT, out.width='90%', fig.width=9, fig.height=3.5, fig.align=TRUE, fig.cap="The fruit-weight data. Left: A histogram of the weight changes on each farm; right: a case-profile plot of the weight changes (unfilled points and dashed lines indicate a decrease)"}
par( mfrow = c(1, 2))
...
Figure\ \@ref(fig:FruitjamoviHT) gives the $95$% CI as $-24.79$ to $20.33$\ g.
...
Since Fig.\ \@ref(fig:FruitPlotsHT) shows the *sample* differences are approximately symmetrically, so the CI should be reasonably statistically valid.
...
\begin{align*}
  \text{Tea group:\qquad}& \text{$H_0$: $\mu_d = 0$\quad vs\quad$H_1$: $\mu_d \ne 0$};\\
  \text{Hot water group:\qquad} & \text{$H_0$: $\mu_d = 0$\quad vs\quad$H_1$: $\mu_d \ne 0$}.
\end{align*}

For the two groups, using the standard errors found in Sect.\ \@ref(ChamomileTea-Paired-CI), the test statistics are  
\[
...
This means that there is evidence that chamomile tea had an impact on TG.
...
This means that there is no evidence that placebo treatment (hot water) had any impact on TG, as one might expect.
...
These interval have a $95$% chance of straddling the difference between the mean reductions in TG.
...
This is studied in Sect.\ \@ref(ChamomileTea-TwoMeans-HT).
...
if( knitr::is_latex_output() ) {
  T1 <- kable( pad(FR[1:7, ],
               ...
               longtable = FALSE) %>%
    row_spec(0, bold = TRUE)
  T2 <- kable( pad( rbind(FR[8:13, ], c(NA, NA, NA)),
                    surroundMaths = TRUE,
                    targetLength = c(4, 4, 5),
                    digits = 0),
               format = "latex",
               ...
               longtable = FALSE) %>%
    row_spec(0, bold = TRUE)
  T3 <- kable( pad(FR[14:20, ],
                   surroundMaths = TRUE,
                   targetLength = c(5, 4, 5),
                   digits = 0),
               format = "latex",
               row.names = FALSE,
               escape = FALSE,
