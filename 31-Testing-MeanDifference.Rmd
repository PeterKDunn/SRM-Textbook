...
## Example: green coffee bean extract {#CoffeeExtractHT}

(This study was seen in Sect.\ \@ref(CoffeeExtractHT).)
A study of patients with non-alcoholic fatty liver disease (NAFLD) randomly allocated $21$ patients into a control group, and $21$ to receive green coffee extract (GCE) [@hosseinabadi2020effects].
Summary data are given in Table\ \@ref(tab:GCBsummaryTable).
...
For *both* groups, a hypothesis test can be conducted to see if the evidence supports a mean *reduction* in FBG, or whether any reduction is due to sampling variation:  
\[
  \text{$H_0$: $\mu_d = 0$\quad and\quad $H_1$: $\mu_d \ne 0$},
\]
where $\mu_d$ is the population mean change in FBG.

For the *green coffee group*, the mean *reduction* of FBG is $12.50$, and the standard error is $\text{s.e.}(\bar{d}) = 2.974$ (found in Sect.\ \@ref(CoffeeExtractHT).)
The test statistic is  
\[
  t = \frac{12.50 - 0}{2.974} = 4.20,
\]
which is a huge $t$-score. The one-tailed $P$-value will be very small (using the 68--95--99.7 rule), and certainly smaller than $0.001$.
...
The test statistic is  
\[
  t = \frac{2.00 - 0}{2.737} = 0.73,
\]
which is a small $t$-score, so the one-tailed $P$-value will be very large (using the 68--95--99.7 rule), and certainly larger than $p = 0.10$.
We conclude:

> There is very strong evidence $t = 4.20$; one-tailed $P < 0.001$) of a mean reduction in FBG for the green coffee extract group ($12.50$\ mg.dl^-1^; approx.\ $95$% CI from $6.55$ to $18.45$\ mg.dl^-1^), but *no* evidence ($t = 0.73$; two-tailed $P > 0.10$) for the control group ($2.00$\ mg.dl^-1^; approx.\ $95$% CI from $-3.47$ to $7.47$\ mg.dl^-1^).

The sample sizes are a bit less than $25$, so statistical validity is not guaranteed.
However, the $P$-values are but the results are  

The two groups appear to show different mean reductions in FBG, suggesting there may be evidence that green coffee extract reduced FBG compared to the control... but perhaps the difference is simply due to sampling variation.
This is studied in TWO SAMPLES.
