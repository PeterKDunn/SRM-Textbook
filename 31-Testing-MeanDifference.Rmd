...
In Chap. \@ref(PairedIntro), data were introduced that studied the mean amount of weight gained between Week 1 and Week 12 when they began university.
We could also ask:
...
Here we will discuss weight *gain* as mentioned in the RQ.
:::


```{r DataWeightChange2, echo=FALSE}
...
```{r WeightGainNormal2, echo=FALSE, fig.cap="The sampling distribution is a normal distribution; it shows how the sample mean weight gain varies in samples of size $n = 68$ when the population mean gain is $0$", fig.align="center", fig.width=9.0, fig.height=2.75, out.width='95%'}
mn <- mean(SW$GainWt)
n <- length(SW$GainWt)
stdd <- sd(SW$GainWt)
...
Software computes the $t$-score too (jamovi: Fig. \@ref(fig:WeightGainTestNumericaljamovi); SPSS: Fig. \@ref(fig:WeightGainTestNumericalSPSS)).
The $t$-score locates the *observed* sample statistic on the sampling distribution.


```{r WeightGainTestNumericaljamovi, echo=FALSE, fig.cap="jamovi output for the insulation data", fig.align="center", out.width="80%"}
knitr::include_graphics("jamovi/WeightGain/WeightGain-PairedTOutput.jpg")
```

```{r WeightGainTestNumericalSPSS, echo=FALSE, fig.cap="SPSS output for the insulation data", fig.align="center", out.width="80%"}
...
Software (jamovi: Fig. \@ref(fig:WeightGainTestNumericaljamovi); SPSS: Fig. \@ref(fig:WeightGainTestNumericalSPSS)) reports that the *two-tailed* $P$-value is less than $0.001$.
...
* **jamovi**: 
  At the left of the output (Fig. \@ref(fig:WeightGainTestNumericaljamovi)), the order implies the differences are found as `Week12` minus `Week1`.
