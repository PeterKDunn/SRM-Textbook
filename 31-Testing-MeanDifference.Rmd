...
`r if( knitr::is_latex_output() ) {
    'Table \\@ref(tab:DataInsulationHT).'
} else {
    'the table below.'
}`
These data were used in Sect. \@ref(PairedCI), where a CI was constructed for the mean energy saving.

For these data, finding the *difference* (or *change*) in energy consumption for each house seems sensible.
...
The parameter is $\mu_d$, the population mean *saving*  (or *reduction*) in energy consumption.
...
The data should be summarised numerically using your calculator or software, such as jamovi or SPSS (Table \@ref(tab:InsulationData)), and graphically (Fig. \@ref(fig:InsulationHistogram)).
...
However, the *sample* mean energy saving will vary depending on which one of the many possible samples is randomly obtained, *even if* the mean saving in the population is zero.
That is, the value of $\bar{d}$ will vary across all possible samples even if $\mu_d = 0$.

The possible values of the sample mean differences can, [under certain conditions](#ValiditySampleMeanDiffTest), be described using an approximate normal distribution.
...
\]
This describes what can be *expected* from the possible values of $\bar{d}$ (Fig. \@ref(fig:EnergySamplingDist)), just through sampling variation if $\mu_d = 0$.


```{r EnergySamplingDist, echo=FALSE, fig.cap="The sampling distribution of sample means, if the energy saving in the population really was zero. The mean of the distribution is $\\text{mean}(\\bar{d}) = 0$ and the starnard deviation $\\text{s.e.}(\\bar{d}) = 0.3212$ ", fig.align="center", fig.width=10, fig.height=3, out.width='90%'}
par( mfrow = c(1, 2))
...
out <- plotNormal(mu = mn0, 
                  sd = se,
                  las = 2,
                  round.dec = 2,
                  main = "", # Main title
                  xlab = "Enery savings") # horizontal axis labels
shadeNormal(out$x, 
            out$y,
            col = plot.colour,
            lo = mn,
...
out <- plotNormal(mu = 0, 
                  ...
                  round.dec = 0,
                  main = "", # Main title
                  xlab = expression(Enery~savings:~italic(t)-scores)) # horizontal axis labels
shadeNormal(out$x, 
            out$y,
            col = plot.colour,
            lo = 1.68,
...
\[
	t
	= \frac{\bar{d} - \text{mean}(\bar{d})}{\text{s.e.}(\bar{d})}
	= \frac{0.54 - 0}{0.3212} = 1.681,
\]
following the ideas in Equation \@ref(eq:tscore).
Software computes the $t$-score too (jamovi: Fig. \@ref(fig:InsulationTestNumericaljamovi); SPSS: Fig. \@ref(fig:InsulationTestNumericalSPSS)).
The $t$-score locates the *observed* sample statistic on the sampling distribution.



```{r InsulationTestNumericaljamovi, echo=FALSE, fig.cap="jamovi output for the insulation data", fig.align="center", out.width="80%"}
knitr::include_graphics("jamovi/InsulationBeforeAfter/InsulationBeforeAfterPairedTOutput.png")
```

```{r InsulationTestNumericalSPSS, echo=FALSE, fig.cap="SPSS output for the insulation data", fig.align="center", out.width="80%"}
...
Since $t = 1.681$, and since $t$-scores are like $z$-scores, the *one-tailed* $P$-value is between 2.5% and 16% based on the [68--95--99.7 rule](#def:EmpiricalRule).
This is a wide, and inconclusive, interval for the $P$-value. 
Using the Tables, the *approximate* *one-tailed* $P$-value is $0.465$... but since the sample size is small, this is probably not a good estimate.

Software gives a more precise $P$-value (jamovi: Fig. \@ref(fig:InsulationTestNumericaljamovi); SPSS: Fig. \@ref(fig:InsulationTestNumericalSPSS)): the *two-tailed* $P$-value is $0.127$, so the *one-tailed* $P$-value is $0.127/2 = 0.0635$.
...
* **jamovi**: 
  At the left of the output (Fig. \@ref(fig:InsulationTestNumericaljamovi)), the order implies the differences are found as `Before` minus `After`.

* **SPSS**: 
  At the left of the output (Fig. \@ref(fig:InsulationTestNumericalSPSS)), the difference is described as `Before - After`.
:::
...
The *one-tailed* $P$-value is $0.0635$, suggesting only slight evidence (Table \@ref(tab:PvaluesInterpretation)) supporting $H_1$.
...
\[
  t = \frac{\bar{d} - \text{mean}(\bar{d})}{\text{s.e.}(\bar{d})} = \frac{2.68 - 0}{3.515018} = 0.76.
\]
...
\[
   t = \frac{\bar{d} - \text{mean}(\bar{d})}{\text{s.e.}(\bar{d})} = \frac{0.77 - 0}{0.073837} = 10.43,
\]
...
\begin{align*}
  t 
  &= \frac{\text{sample statistic} - \text{population mean of all sample statistics}}
...
\[
   t = \frac{ \bar{d} - \mu(\bar{d})}{\text{s.e.}(\bar{d})},
\]
...
'Intention to smoke', and other measures, were assessed both before and after exercise for each subject, using the 10-item quantitative
`r if (knitr::is_latex_output()) {
...
```{r FerritinTable, echo=FALSE, cache=FALSE}
if( knitr::is_latex_output() ) {
  kable( head(FR, 10),
        format = "latex",
        longtable = FALSE,
