...
Chapter\ \@ref(Sampling) discusses how to select a suitable sample to study to enhance the external validity.
...
Apart from being externally validity, well-designed research studies should be *internally valid*: studies that allow the researchers to focus on the relationship of interest between the response and explanatory variables.
Internally valid studies eliminate, or account for, other explanations for the variation in the values of the response variable apart from the explanatory variable.
...
The typing speeds of the many individuals (the *response variable*) will vary: every student in the study recording exactly the same typing speed is highly unlikely.
...
```{r Influences, fig.cap="Other factors can influence the values of the response variable, besides the explanatory variable (i.e., the relationship of interest).", fig.width=7, fig.align="center", fig.height=2.5, fig.width=7.5, out.width='95%'}
...
If nothing else influenced typing speed, every student under $25$ record would the same typing speed every time, and every student $25$ or older would record the same typing speed every time.
...
Using control variables may limit the population, and hence limit the generalisability of the the results.
...
In the typing-speed study, for example, restricting the study to people not wearing glasses would mean the results would apply to a very narrow group of people.
...
This does not necessarily mean the value of the confounding variable *causes* changes in the response or explanatory variables.
:::
...
That is, does the presence of rain change the proportion of people wearing sunglasses?
It also *may* be the case (for example) that males are more likely to walk in the rain than females, so 'whether or not it is raining' *may* be an extraneous variable (but it is not obvious).
:::
...
*extraneous variables* (other variables that aren't of primary interest), and  *chance*.
...
A study of New Zealanders found that people wearing hearing aids were more likely to have grey hair that people *not* wearing hearing aids.
...
A study of school teachers found a relationship between the average number of children plus grandchildren, and having high blood pressure. 
...
5. Whether or not the teacher is very health conscious.
6. Whether or not the teacher has high blood pressure.
7. Whether or not the teacher teaches a health subject.
::::
::::::
