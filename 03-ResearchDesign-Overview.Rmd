...
Apart from being external validity, well-designed research studies should be *internally valid*.
...
```{r InfluencesConfounding, fig.cap="Left: Confounding variables (left) are extraneous variables associated with the response and explanatory variables. In extreme situations as shown here, no real association between exists between the response and explanatory variables; the association is explained by a confounding variable. Right: If the confounding variable is not measured (and so a lurking variable is present), an apparent association does exist between the response and explanatory variables. Usually, confounding is not as extreme as shown in this diagram, and the confounding variable just changes the relationship between response and explanatory variables.", fig.align="center", fig.height=2.15, fig.width=8.25, out.width='100%'}
