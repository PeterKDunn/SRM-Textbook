...
The [Australian Bureau of Statistics (ABS)](http://www.abs.gov.au/ausstats/abs@.nsf/Lookup/4338.0main+features212011-13), however, takes samples of Australians to compute estimates of the heights and other measurements.
...
```{r HeightsModelFig, echo=FALSE, fig.align="center", fig.width=5, fig.height=3, fig.cap="The model for heights of Australian adult males, plus the histogram from one specific sample if size $n=100$ of Australian adult males" }
...
```{r MatchDiagrams, echo=FALSE, fig.cap="Match the diagram with the description", out.width="70%", fig.align="center", fig.width=5.5, fig.height=4}
par( mfrow = c(2, 2))
...
```{r ZDBH3, echo=FALSE, fig.cap="What proportion of tree diameters are between 6 and 11 inches? The two shaded areas given are what we find by using the tables with $z = -1.04$ and $z = 0.81$, but neither give us the area we are seeking", fig.align="center", fig.width=8, fig.height = 2.5,out.width='90%'}
...
Looking carefully at the areas from the tables and the area sought, that area between the two $z$-scores is $0.7910 - 0.1492 = 0.6418$;
`r if (knitr::is_latex_output()) {
...
```{r NormalMiddleFig, echo=FALSE, fig.align="center", fig.height = 3,fig.width=6, fig.cap="Finding the area (probability) between two $z$-scores" }
if (knitr::is_latex_output()){
       plot.norm(RT.mn, 
                 sd = RT.sd, 
                 width = 5, 
...
```{r RetentionTime, echo=FALSE, fig.cap="Retention times", fig.align="center", fig.width=5, fig.height=2.5}
plot.norm(42.5, 
          sd = 3.68, 
          width = 5, 
	        height = 4,
...
1. In Australia, [a premature birth is defined as a birth occuring before 37 weeks](https://www.pregnancybirthbaby.org.au/premature-baby).
   What proportion of births are expected to be premature?
1. According to [Health Direct](https://www.pregnancybirthbaby.org.au/premature-baby), 'Babies born between 32 and 37 weeks may need care in a special care nursery'.
...
IQ scores are [designed to have](https://en.wikipedia.org/wiki/IQ_classification) a mean of 100 and a standard deviation of 15.
[Mensa](https://www.mensa.org/) is a society for people with a high IQ:
...
IQ scores are [designed to have](https://en.wikipedia.org/wiki/IQ_classification) a mean of 100 and a standard deviation of 15.
@data:Zagorsky2016:Blondes reports that 
...
IQ scores are [designed to have](https://en.wikipedia.org/wiki/IQ_classification) a mean of 100 and a standard deviation of 15.
Match the diagram in  ig. \@ref(fig:IQMatchDiagramsForwards) with the meaning.
