...
Then, based on the assumption about the parameter, we describe the values of the statistic that we would *expect* from all possible samples.
...
* No upper limit or lower limit exists, in theory, for the variable.
  Of course, the chance that some of these values occur is essentially zero (such as a male taller than 550cm).
...
Since the [68--95--99.7 rule](#def:EmpiricalRule) applies for all normal distributions, the percentages only depend on how many standard deviations ($\sigma$) a value ($x$) is from the mean ($\mu$).
...
```{r MatchDiagrams, echo=FALSE, fig.cap="Match the diagram with the description",  fig.align="center", out.width="80%", fig.height=4.00}
par( mfrow = c(2, 2))
...
```{r WorkingWithZ, echo=FALSE, fig.cap="Working with $z$-scores. In the tables, the areas (probabilitues) are in the body of the table, and the $z$-scores are in the margins of the table.", fig.align="center", out.width='70%', fig.height=3, fig.width=7}
...
\end{align*}
that is, about 3% of trees have diameters less than about 1.38 inches.
