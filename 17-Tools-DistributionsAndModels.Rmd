...
```{r HeightsModelMovie, echo=FALSE, animation.hook="gifski", cache=TRUE}
...
<!-- ```{r HeightsModelFig2, echo=FALSE, fig.align="center", fig.width=5, fig.cap="The model for heights of Australian adult males, plus the histogram from one specific sample if size $n=100$ of Australian adult males" } -->
<!-- if (knitr::is_latex_output()){ -->
...
plot.norm(HT.mn, HT.sd, 
          ...
          zlim.lo = -4, zlim.hi = 4, 
          type="",
          shade.lo.x=182, shade.hi.x=Inf )
plot.norm(HT.mn, HT.sd, 
          xlab.name="Height (in cm)", 
          zlim.lo = -4, zlim.hi = 4, 
          type="",
          shade.lo.x=-Inf, shade.hi.x=168, 
          new=FALSE )
text(2.5, 0.1, "16%")
text(-2.5, 0.1, "16%")
segments(-1, 0, -1, dnorm(-1), col=grey(0.5))
text(0, 0.1, "68%")
...
plot.norm(HT.mn, HT.sd, 
          ...
          zlim.lo = -4, zlim.hi = 4, 
          type="",
          shade.lo.x=-Inf, shade.hi.x=161 )
plot.norm(HT.mn, HT.sd, 
          xlab.name="Height (in cm)", 
          zlim.lo = -4, zlim.hi = 4, 
          type="",
          shade.lo.x=189, shade.hi.x=Inf, 
          new=FALSE )
text(3, 0.1, "2.5%")
text(-3, 0.1, "2.5%")
segments(2, 0, 2, dnorm(2), col=grey(0.5))
text(0, 0.1, "95%")
...
```{example EmpiricalRule,  name="The 68--95--99.7 rule"}
...
* [**hard copy** tables](#UsingZTables), or
* [**online** tables](#UsingOnlineZTables). 
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE}
  RT.mn <- 8.8
  RT.sd <- 2.7
...
if (knitr::is_html_output()){
  for (i in (1:4)){
    if ( i == 1 ){
      plot.norm(RT.mn, sd=RT.sd, width=5, height=4,
                shade.col="azure2",
...
<!-- ```{r NormalMiddleMovie2, echo=FALSE, fig.align="center", fig.cap="The tree diameter is between 6 and 11 inches"} -->
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/NormalMiddleMovie.html", frameborder=0, height=450, width=350)  -->
<!-- ``` -->

<!-- ```{r NormalMiddleFig, echo=FALSE, fig.align="center", fig.width=6, fig.cap="Finding the area (probability) between two $z$-scores" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/NormalMiddle3") -->
<!-- } -->
<!-- ``` -->
...
curvedarrow(from=pos[1,], to=pos[2,], 
            curve=0.4)
curvedarrow(from=pos[2,], to=pos[3,], 
            curve=0.4)


textrect( pos[1,], lab="x (known)", 
          box.col="white",
          lcol="white",
          shadow.size=0,
          radx=0.04,
          rady=0.04)
textrect( pos[2,], lab="z", 
          box.col="white",
          lcol="white",
          shadow.size=0,
          radx=0.04,
          rady=0.04)
textrect( pos[3,], lab="Area", 
          box.col="white",
          lcol="white",
          shadow.size=0,
          radx=0.04,
...
text( mean(pos[2:3,1]), 0.55, "Using tables")
text( mean(pos[1:2,1]), 0.55, "Using formula")
...
text(0.5, 0.4, "Working backwards with z-scores", font=2)

curvedarrow(from=pos[2,], to=pos[1,], 
            curve=0.4)
curvedarrow(from=pos[3,], to=pos[2,], 
            curve=0.4)

textrect( pos[1,], lab="x", 
          box.col="white",
          lcol="white",
          shadow.size=0,
          radx=0.04,
...
text( mean(pos[2:3,1]), 0.3, "Using tables")
text( mean(pos[1:2,1]), 0.3, "Using formula")
```
