...
In the [decision-making process used in statistics](#DecisionMaking), an **assumption** is made about the population parameter, and then, based on this assumption, the values **expected** from all possible sample statistics can be described.

The **expectations** about the sample statistic are based on how the statistic (such as a sample mean, or a sample proportion, or a sample odds ratio) is [*distributed*](#GraphsOneQuant): what values it can take in different samples, and how often.
...
Under [certain circumstances](#ValidityProportions), many sample statistics have a similarly-shaped distribution: a [*bell-shaped (or normal) distribution*](#NormalDistributions).
...
Indeed, if this model turns out to be poor at describing what appears in these many samples, the [*parameters*](#StatisticsAndParameters) of the model (the values of $\mu$ and $\sigma$) can be adjusted so the model *does* describe the sample data well.
...
From a picture of the situation (Fig. \@ref(fig:HtsExer1)), $175 + 7 = 182$cm is one standard deviation *above* the mean.
Since 68% of values are within one standard deviation of the mean, 32% are outside that range, smaller or larger (Fig. \@ref(fig:HtsExer1)).
...
## Approximating percentages using the 68--95--99.7 rule
...
To find the proportion of men are *shorter* than 160cm, drawing the situation is helpful (Fig. \@ref(fig:HtsExer3)).
:::
...
We now learn how to use these tables, then come back to finish Example \@ref(exm:Height160).
...
`r if (knitr::is_latex_output()) {
   'but only the *hard-copy* tables are explained in this book (see the [online](https://bookdown.org/pkaldunn/SRM-Textbook/tables.html#ZTablesOnline) version for how to use the online tables).'
} else {
