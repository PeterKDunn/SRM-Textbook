...
what values it can take in various samples, and how often.
...
$160$cm corresponds to a $z$-score of 
...
The online tables 
(which work differently to the [hard-copy tables](#UsingZTables))
can be found
`r if (knitr::is_html_output()){
...
In the tables,
enter the value `-2.00` in the search region just under the column labelled `z.score`
...
Like the [online tables](#UsingZTables),
the hard-copy
...
that 160cm is from the mean is found
...
<iframe src="https://learningapps.org/watch?v=ppievv9gc22" style="border:0px;width:100%;height:500px" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>
...
```
  ...
  `r if( knitr::is_html_output(exclude = "epub") ) {
	fitb(z1,
 		num = TRUE,
		tol = 0.01)}`r if (!knitr::is_html_output()) '<!--'``
