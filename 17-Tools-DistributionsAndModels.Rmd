...
arrows(15, max(out$y) * 0.6, 
       12.5, max(out$y) * 0.1, 
       angle = 15, 
       length = 0.15, 
       lwd = 2) # Note: Locations in terms of z-scores
text(15, max(out$y) * 0.6, 
     "Largest 25%", 
     cex = 0.9,
     pos = 3)
...
A retention time of 40 hours corresponds to $z = -0.68$, and, using the normal distribution tables, the area to the *left* of $z = -0.68$ is $0.2483$ (Fig. \@ref(fig:RetentionPlots), bottom left panel; bottom-left to top-right hatching).
But this is not the area that we are seeking...
From earlier, the area to the *left* of $z = 1.49$ is 0.9319 (Fig. \@ref(fig:RetentionPlots), bottom left panel; top-left to bottom-right hatching).
...
```{r IQMatchDiagramsForwards, echo=FALSE, fig.cap="Match the diagram with the description", fig.align="center", out.width="85%", fig.height=3.75, fig.width=6.5}
par( mfrow = c(2, 2),
     mar = c(4, 1, 2, 2) + 0.1)
...
```{r IQMatchDiagramsBackwards, echo=FALSE, fig.cap="Match the diagram with the description", fig.align="center", out.width="85%", fig.height=3.75, fig.width=6.5}
par( mfrow = c(2, 2),
     mar = c(4, 1, 2, 2) + 0.1)
...
Using the unstandardising formula, $x = 50 + (-1.04\times 0.1) = 49.896$.
Bearings less than about 49.90mm in diameter cannot be sold.
