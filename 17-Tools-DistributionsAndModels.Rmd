...
If this model is accurate, the distribution of heights in any *sample*å may be shaped a bit like this... but *sampling variation* will exist, and every sample will be a bit different.
...
* approximatey 68% of values lie within 1 standard deviation of the mean;
* approximatey 95% of values lie within 2 standard deviations  of the mean; and
* approximatey 99.7% of values lie within 3 standard deviations  of the mean.
...
So the percentages males even shorter than 161cm (that is, further into the tail of the distribution), will be a little *less* than 2.5%.
While we don't know the probability exactly, it will be a little smaller than 2.5%.
...
This agrees with the approximate answer using the [68--95--99.7 rule](#def:EmpiricalRule): a little less than 2.5%.
...
Using this model, what is the probability that a randomy-chosen tree has a diameter *greater than* than 6 inches?
:::
...
```{r MatchDiagrams, echo=FALSE, fig.cap="Match the diagram with the description",  fig.align="center", out.width="80%", fig.height=4.25}
par( mfrow = c(2, 2))
...
```{r ZDBH2, echo=FALSE, fig.cap="What proportion of tree diameters are between 6 and 11 inches?", out.width='42.5%', fig.align="center", fig.height=2.75, fig.width=5}
...
```{r WorkingWithZ, echo=FALSE, fig.cap="Working with $z$-scores", fig.align="center", out.width='70%', fig.height=3, fig.width=7}
...
textrect( pos[1, ], 
          ...
          rady = boxY)
textrect( pos[2, ], 
          lab = expression(italic(z)~computed), 
          box.col = plotSolid,
          lcol = plotSolid,
          shadow.size = 0,
          radx = boxX,
...
textrect( pos[1, ], 
          lab = expression(italic(x)~computed), 
          box.col = plotSolid,
          lcol = plotSolid,
          shadow.size = 0,
          radx = boxX,
          rady = boxY)
textrect( pos[2, ], 
          lab = expression(italic(z)~from~Tables), 
          box.col = plotSolid,
          lcol = plotSolid,
          shadow.size = 0,
          radx = boxX,
...
The closest area to 3% in the tables is exactly 0.0030. 
(Sometimes, the exact area cannot be found and we take the value as close as possible.)
So, the corresponding $z$-score is $z = 2.75$.


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
To identify the diameters of the *smallest* 3% of trees, the $z$-score that has an area to the *left* of 3% (or 0.0300) needs to be found (or, at least, as close as possible to 0.03).
:::
...
Using either the hard-copy or online tables, the appropriate $z$-value is $-2.75$ standard deviations *below* the mean; that is, $z = -2.75$ (Fig.&nbsp;\@ref(fig:DBHBackwards)).
...
\begin{align*}
	x &= \mu + (z\times\sigma) \\
		&= 8.8 + (-2.75 \times 2.7) = 1.375;
\end{align*}
that is, about 3% of trees have diameters less than about 1.4 inches.
...
text(xguess, 
     max(out$y) * 0.8,
     expression(italic(z)~somewhere~near~here), 
     pos = 3, 
     cex = 1)
...
Using the normal distribution tables (that give the *area to the left* of the $z$-score), the area to the left of $z = -0.68$ is 0.2483, or about 24.8%.
...
3. The probability that a tree has a diameter *greater* than `r diam1` inches is (as a *decimal value*):  
  `r if( knitr::is_html_output(exclude = "epub") ) {
	fitb( 1 - prob1,
		num = TRUE,
		tol = 0.01)}` 
4. A tree diameter of `r diam2` inches corresponds to a $z$-score (to two decimal places) of (as a *decimal value*):  
  `r if( knitr::is_html_output(exclude = "epub") ) {
	fitb(z2,
 		num = TRUE,
		tol = 0.01)}` 
...
```{r IQMatchDiagramsForwards, echo=FALSE, fig.cap="Match the diagram with the description", fig.align="center", out.width="90%", fig.height=3.75, fig.width=6.5}
par( mfrow = c(2, 2),
     mar = c(4, 1, 1, 2) + 0.1)
...
```{r IQMatchDiagramsBackwards, echo=FALSE, fig.cap="Match the diagram with the description", fig.align="center", out.width="90%", fig.height=3.75, fig.width=6.5}
par( mfrow = c(2, 2),
     mar = c(4, 1, 1, 2) + 0.1)
