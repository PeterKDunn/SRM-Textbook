...
```{r HeightsModel, echo=FALSE, out.width='100%', results='hide', fig.cap="A model for the heights of Australian adult males, showing 1, 2 qnd 3 standard deviations either side of the mean (left); the model for heights of Australian adult males, plus the histogram from one specific sample if size $n = 100$ of Australian adult males (right)", fig.align="center", fig.width=7.5, fig.height=2.5}
HT.mn <- 175
HT.sd <- 7
...
Since 68% of values are within one standard deviation of the mean, 32% are outside that range, smaller or larger (Fig. \@ref(fig:HtsExer1), left panel).
...
```{r HtsExer1, echo=FALSE, fig.cap="Left: What proportion of Australian adult males are taller than 182cm? Right: What proportion of Australian adult males are shorter than 161cm?", fig.align="center", fig.width=7.5, fig.height=2.5, out.width='100%'}
...
```{r, child = if (knitr::is_latex_output()) './children/LaTeX/UsingHardcopyTables.Rmd'}
```
```{r, child = if (knitr::is_html_output()) './children/HTML/UsingOnlineTables.Rmd'}
```
...
The number of standard deviations that 160cm is from the mean, using Equation \@ref(eq:zscores), is:
...
* 6 inches: $\quad  z = (6 - 8.8)/2.7 = -1.04$.
* 11 inches: $\quad z = (11 - 8.8)/2.7 = 0.81$.
...
```{r WorkingWithZ, echo=FALSE, fig.cap="Working with $z$-scores", fig.align="center", out.width='80%'}
...
```{r, child = if (knitr::is_latex_output()) './children/LaTeX/UsingHardcopyTablesBackwards.Rmd'}
```
```{r, child = if (knitr::is_html_output()) './children/HTML/UsingOnlineTablesBackwards.Rmd'}
```
...
Since an area is give, we need to work 'backwards' (Fig. \@ref(fig:DBHBackwards2), bottom panel), so the $z$-tables need to be used 'backwards' too.
...
A study @huhtanen2016effects of methane produced by animals modelled the retention time of food in sheep using a normal distribution, with the mean retention time as $\mu = 42.5$ hours, and the standard deviation of the retention time as $\sigma = 3.68$ hours.
...
Using the normal distribution tables (that give the *area to the left* of the $z$-score that we look up), the area to the left of $z = -0.68$ is 0.2483, or about 24.8%.
...
A retention time of 36 hours corresponds to $z = -1.77$, and, using the normal distribution tables, the area to the *left* of $z = -1.77$ is $0.0384$.
...
This is like a 'backwards problem', ad we need to find the $z$-score 'backwards' (Sect. \@ref(Unstandardising)).
...
The retention time $41.07$ hours.



```{r RetentionTime, echo=FALSE, fig.cap="Retention times of food in sheep", fig.align="center", fig.width=6, fig.height=2.5, out.width='60%'}
plot.norm(42.5, 
          sd = 3.68, 
          width = 5, 
	        height = 4,
