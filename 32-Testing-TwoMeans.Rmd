...
Two different groups of crayfish must be studied, so the data are *not paired*.
`r if (knitr::is_latex_output()) {
   'Part of the data are shown in Table\\ \\@ref(tab:SnakesDataTableTest).'
} else {
...
tb1 <- c( na.omit( SNF$SVL[ SNF$Crayfish == "Y" ] ) )
tb1 <- cbind (tb1[1:6],
              tb1[7:12],
              tb1[13:18],
              tb1[19:24],
...
tb2 <- c( na.omit( SNF$SVL[ SNF$Crayfish == "N" ] ) )
tb2 <- cbind (tb2[1:6],
              tb2[7:12],
              tb2[13:18],
              tb2[19:24],
  ...
  T1 <- knitr::kable( surroundMaths(tb1),
    ...
    add_header_above( c("Non-crayfish region" = 7),
                      bold = TRUE)
  ...
  T2 <- knitr::kable( surroundMaths(tb2),
                      ...
                      booktabs = TRUE) %>%
    row_spec(0, bold = TRUE) %>%
    add_header_above( c("Crayfish region" = 6),
                      bold = TRUE)
...
For these data, the differences are computed as the mean SVL for the females snakes living in non-crayfish regions ($N$), *minus* the SVL for the female snakes in not crayfish regions ($C$): $\mu_N - \mu_C$.
...
The parameter is $\mu_N - \mu_C$, the difference between the population SVL (using $N$ minus $C$).
...
Computing the difference as the mean SVL for snakes living in crayfish regions, *minus* the mean SVL for snakes living in non-crayfish regions is also correct.
...
All this information is found using software (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)), and can be compiled into a table (Table\ \@ref(tab:SnakesNumericalTest)).
The appropriate summary for graphically summarising the *data* is a boxplot (though a dot chart is also acceptable).
An error bar chart (Fig.\ \@ref(fig:SnakesErrorbar)), which compares the *sample means*, should also be produced.


```{r SnakesSummaryTestjamovi, fig.cap="jamovi output for the snakes data", fig.align="center", out.width="95%"}
...
The jamovi output (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)) can be used, but the $t$-score can also be computed manually:  
\begin{align*}
   t
  &= 
...
Again, the conclusions contains an *answer to the RQ*, the *evidence* leading to this conclusion ($t = 3.445$; two-tailed $P = 0.0011$), and some *sample summary statistics*, including a CI.
...
For the reaction-time data, both samples sizes are $41$ and $35$, so the results will be statistically valid.
