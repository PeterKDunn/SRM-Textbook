...
RT.DataTable10 <- cbind( head(RReactionTimePhoneIndT$Reaction[RReactionTimePhoneIndT$Group == "Phone"], 10), 
...
DeDecelerationcel$When <- factor(Deceleration$When,
                     levels = c("Before", 
                                "After"),
                     ordered = TRUE)
...
Speed.n <- tapply(Deceleration$Decel, 
                  list(Decel$When), 
                  "length") 
Speed.sd <- tapply(Deceleration$Decel, 
                   list(Decel$When), 
                   "sd") 

SignageSummaryDec[1:2, 1] <- round( tapply(Deceleration$Decel, 
