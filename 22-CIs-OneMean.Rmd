...
The standard deviation of this normal distribution is describing a sampling distribution, so is called a *standard error*: specifically, it is the standard error of the sample mean $\bar{x}$, written as $\text{s.e.}(\bar{x})$.
...
\begin{equation}
   ...
   (\#eq:stderrorxbar)
\end{equation}
when [certain conditions are met](#ValiditySampleMean), where $n$ is the size of the sample, and $s$ is the standard deviation describing the variation in the individual observations in the sample (that is, the *sample* standard deviation). 
:::
...
Another sample of 586 children would produce a different sample mean, so the sample mean varies from sample to sample; sampling variatin exists.
