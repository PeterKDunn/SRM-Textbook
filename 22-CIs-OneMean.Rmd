...
Suppose we roll a die $25$ times, to see how the sample mean varies in $25$ rolls
...
```{r RollDiceHistMeanFig, fig.align="center", fig.width=5, fig.height=3.5, fig.cap="Rolling dice: The mean of 25 rolls, for thousands of repetitions" }
if (knitr::is_latex_output()){
  set.seed(123456789)
  num.sims <- 2000
  num.rolls <- 25
...
The multiplier of is not actually a $z$-score here.
...
In these situations, the multiplier is actually a $t$-score.

However, the values of $t$- and $z$-multipliers are *very* similar, and (except for small sample sizes) using an approximate multiplier of\ $2$ is reasonable for computing *approximate* $95$% CIs in either case.
:::


Suppose we tossed a die $25$ times, and found $\bar{x} = 2.8$ and $s = 
...
(Loosely speaking, there is a $95$% chance that a $95$% CI straddles $\mu$.)


```{r CIrelationshipsMean, out.width='85%', fig.align="center", fig.cap="A CI gives a range of possible values of $\\mu$ for which it is reasonable to produce the observed value of $\\bar{x}$. The shaded regions represent the regions containing 95\\% of the values of $\\bar{x}$ for each value of $p$.", fig.width=9, fig.height=6.5}
...
```{r NHANEShistogram, fig.cap="Histogram of direct HDL cholesterol concentration", fig.align="center", fig.width=4.5, fig.height=3.0}
