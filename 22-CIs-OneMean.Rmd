...
```{r RollDiceMeanFig, fig.align="center", fig.width=7, fig.height=4, out.width="85%", fig.cap="Rolling dice: The average of 25 rolls, for some sets of 25 rolls" }
...
Since this distribution is describing how *sample means* vary, the *sampling mean* is denoted $\mu_{\bar{x}}$; it so happens that this is equal to the value of $\mu$.
...
* centred around a sampling mean denoted by $\mu_{\bar{x}}$, whose value is $\mu$,
...
OneMeanNotation[4, ] <- c("all possible samples",
                          "with sampling mean $\\mu_{\\bar{x}}$ and standard deviation $\\text{s.e.}(\\bar{x})$")
