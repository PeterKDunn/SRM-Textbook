...
We cannot be sure: the sample mean will vary from sample to sample (*sampling variation*).
...
An example of finding the mean after repeatedly rolling a die 25 times is
...
```{r RollDiceMeanFig, echo=FALSE, fig.align="center", cache=FALSE, fig.width=7, fig.height=4, out.width="85%", fig.cap="Rolling dice: The average of 25 rolls, for 10 sets of 25 rolls" }
...
Then, the best estimate of the *standard error* of the sample mean is $\displaystyle\text{s.e.}(\bar{x}) = \frac{s}{\sqrt{n}}$.
...
The multiplier of 2 is not a $z$-score here, since the *population* standard deviation is unknown.
Since we don't know the *population* standard deviation, and use the *sample* standard deviation instead, the multiplier is a $t$-score.
...
Another sample of 586 children would produce a different sample mean, so the sample mean varies from sample to sample.
...
From the sample information, the standard error of the mean is

\[
  \text{s.e.}(\bar{x}) = \frac{s}{\sqrt{n}} = \frac{51.1}{\sqrt{185}} = 3.756947,
\]
so the approximate 95% CI is from $84.9 - (2 \times 3.756947) = 77.38611$ to $84.9 + (2 \times 3.756947) = 92.41389$.
...
A histogram of the total radiation dose received (Fig.&nbsp;\@ref(fig:CTscanHistogram)) suggests this is very unlikely.
