...
```{r RollDiceMeanHTML, animation.hook="gifski", cache=TRUE, echo=FALSE, interval=0.5, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
```{r RollDiceHistMeanHTML, cache=TRUE, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.005, dev=if (is_latex_output()){"pdf"}else{"png"}}
if (knitr::is_html_output()){
  set.seed(123456789)
    num.sims <- 1000
    num.rolls <- 25
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-anna-shvets-4225923.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
