...
```{r RollDiceMeanHTML, animation.hook="gifski", cache=TRUE, echo=FALSE, interval=0.5}
if (knitr::is_html_output()){
  set.seed(99999)
    ...
    for (i in 1:num.sims){
      ...
      # Add x-bar heading
      mtext(expression(bar( italic(x) ) ), side=3, line=0, at=num.rolls+2 )
      ...
      #Add dividing line
      abline(v=num.rolls+1, col="grey")
      # Add line dividing roll sets
      abline(h=seq(0.5,10.5, by=1), col="gray")
    }
  }
```
...
```{r RollDiceMeanFig, echo=FALSE, fig.align="center", cache=TRUE, fig.width=7, out.width="85%", fig.cap="Rolling dice: The average of 25 rolls, for 10 sets of 25 rolls" }
...
<!-- ```{r RollDiceMeanHTML, echo=FALSE, fig.cap="Rolling dice: The average of 25 rolls, for 10 sets of 25 rolls", fig.align="center"} -->
<!-- htmltools::tags$iframe(title = "Rolling dice: The average of 25 rolls", src = "./Animations/RollDiceMeanMovie.html", frameborder=0, height=600, width=520)  -->
<!-- ``` -->


<!-- ```{r RollDiceMeanFig, echo=FALSE, fig.align="center", fig.width=5, out.width="85%", fig.cap="Rolling dice: The average of 25 rolls, for 10 sets of 25 rolls" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RollDiceMean10") -->
<!-- } -->
<!-- ``` -->
...
```{r RollDiceHistMeanHTML, cache=TRUE, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.005}
if (knitr::is_html_output()){
  set.seed(123456789)
    num.sims <- 1000
    num.rolls <- 25
    ...
    for (i in 1:num.sims){
      meanSingleRoll <- mean(sample(1:6, num.rolls, replace=TRUE))
      ...
      out <- hist(meanList,
                  ...
                  sub=paste("(For this set: mean roll is ", format(round(meanList[i], 2), nsmall=2), ")", sep="" ),
                  col=plot.colour,
                  las=1,
                  xlim=c(1.5, 5.5), right=FALSE,
                  ylim=c(0, 300),
      ...
      yy <- dnorm(xx, mean=die.mn, sd=sqrt(die.vr/num.rolls) )
      yy <- yy/max(yy) * max(out$count)
      
      lines(yy~xx, col="grey", lwd=2)  
    }
  }
```
...
```{r RollDiceHistMeanFig, echo=FALSE, fig.align="center", cache=TRUE, fig.width=5, fig.cap="Rolling dice: The mean of 25 rolls, for 1000 repetitions" }
if (knitr::is_latex_output()){
  set.seed(123456789)
    num.sims <- 1000
    num.rolls <- 25
...
<!-- ```{r RollDiceHistMeanHTML, echo=FALSE, fig.cap="Rolling dice: The mean of 25 rolls, for 1000 religions of 25 rolls", fig.align="center"} -->
<!-- #includeHTML("./Animations/RollDiceHistMovie.html") -->
<!-- htmltools::tags$iframe(title = "Rolling dice: The mean of 25 rolls, for 1000 repetitions of 25 rolls", src = "./Animations/RollDiceMeanHistMovie.html", frameborder=0, height=580, width=520)  -->
<!-- ``` -->



<!-- ```{r RollDiceHistMeanFig, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Rolling dice: The mean of 25 rolls, for 1000 repetitions" } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/images/RollDiceMeanHist1000") -->
<!-- } -->
<!-- ``` -->
...
<!-- * with a standard deviation of $\displaystyle \text{s.e.}(\bar{x}) = \frac{\sigma}{\sqrt{n}}$. (this is not obvious!) -->

<!-- For our dice example, -->
<!-- some fancy statistical theory,  -->
<!-- that's beyond this course, -->
<!-- shows us that the standard deviation of the mean of the 25 rolls  -->
<!-- will be $\text{s.e.}(\bar{x}) = `r round(die.se, 3)`$. -->
...
<!-- Remember, this standard deviation is called the *standard error*, -->
<!-- since it is the standard deviation of a sample statistic (the sample mean). -->


<!-- With this knowledge, -->
<!-- we can make some statements about what values of the sample mean -->
<!-- we are likely to expect in practice, -->
...
<!-- Of course, -->
<!-- in practice we almost never know the population mean (after all, we are taking a sample to estimate the mean...), -->
<!-- and almost never know the population standard deviation either. -->
<!-- But these ideas give us something to work with... -->
...
## One mean: Confidence intervals {#OneMeanCI}
...
include_graphics("Illustrations/pexels-pixabay-207697.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
```{lemma CIWidth, name="Width of CI"}         
Would a 99% CI for $\mu$ be *wider* or *narrower* than the 95% CI?  Why?         
```


```{r echo=FALSE}
Interpretation2 <- "A *wider* interval is needed to be *more* confident that the interval contains the population mean."
if( knitr::is_latex_output() ) {
Interpretation2 <- foldLaTeXText
}
```

(ref:Interpretation2r) `r Interpretation2`

```{block2, type="fold"}
(ref:Interpretation2r)
```
...
## One mean: Statistical validity conditions {#ValiditySampleMean}
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "30%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-anna-shvets-4225923.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
Since the sample size was $n=586$,
...
```{r NHANEShistogram, echo=FALSE, cache=TRUE, fig.cap="Histogram of direct HDL cholesterol concentration", fig.align="center", fig.width=4.5, fig.height=3.5}
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/tom-hermans-ZPfd3ZobOc0-unsplash.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
