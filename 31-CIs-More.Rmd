...
```{r, child = if (knitr::is_html_output()) {'./introductions/31-CIs-More-HTML.Rmd'} else {'./introductions/31-CIs-More-LaTeX.Rmd'}}
```
...
CIs in other contexts will also be studied (Chaps.\ \@ref(PairedCI) to\ \@ref(TestsOddsRatio)).
...
For instance, many CIs assume the sampling distribution has a normal distributions (so that for example, the $68$--$95$--$99.7$ rule can be used); the statistical validity conditions state the conditions under which the sampling distribution has an approximate normal distribution.
...
Besides checking the statistical validity conditions, the *internal validity* and *external validity* of the study should be discussed (Fig.\ \@ref(fig:ValiditiesCI)).
...
These statistical validity conditions are usually the same as, or similar to, those for the corresponding CIs (Sect.\ \@ref(ValidityHTs)).
...
> * Approximately $95$% of the amputeees walked between\ $96.4$ and\ $1\,488.6$\ms in two minutes.
