...
* centred around a sampling mean denoted by $\mu_{\bar{x}_P - \bar{x}_C}$, whose value is ${\mu_{P}} - {\mu_{C}} = 0$, the difference between the *population* means (from $H_0$);
...
* centred around the sampling mean $\mu_{\bar{x}_A - \bar{x}_B}$;
...
TwoMeansNotation[6, ] <- c("($\\bar{x}_A - \\bar{x_B}$) across all possible samples",
                          "with sampling mean $\\mu_{\\bar{X}_A - \\bar{x}_B}$ and standard deviation $\\text{s.e.}(\\bar{x}_A - \\bar{x}_B)$")
...
FacePlant <- array(dim = c(2, 10), # COL 6 is a gap
                   NA)
FacePlant[1, ] <- c( FF$LeanAngle[FF$Group == "Younger"][1:5],
                       FF$LeanAngle[FF$Group == "Older"][1:5] )
FacePlant[2, 1:5] <- FF$LeanAngle[FF$Group == "Younger"][6:10]
