...
(The data would be paired if *each* student's reaction time was measured twice: once using a phone, and once without using a phone; Sect.\ \@ref(PairedData).)
...
```{r PhoneDataTable}
data(ReactionTimePhoneInd)

if( knitr::is_latex_output() ) {
  tb1 <- cbind( ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][1:5],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][6:10],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][11:15],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][16:20],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][21:25],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][26:30],
                c(ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"][31:32], NA, NA, NA) )
  ...
  tb2 <- cbind( ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][1:5],
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][6:10], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][11:15], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][16:20], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][21:25], 
                ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][26:30], 
                c(ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"][31:32], NA, NA, NA))
...
if( knitr::is_html_output() ) {
  RT.DataTableHTML <- cbind( "Use phone" = ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Phone"], 
                           "Not using phone" = ReactionTimePhoneInd$Reaction[ReactionTimePhoneInd$Group == "Control"])
...
Since two groups are being compared, distinguishing between the two groups is important.
...
You **must** be clear about how differences are defined!
...
it measures how much the value of  $\bar{x}_P - \bar{x}_C$ varies across all possible samples.


## Summarising data


A suitable graphical summary of the data is a boxplot (Fig.\ \@ref(fig:PhonePlots)), which shows that the *sample* medians are slightly different, but the IQRs about the same; one large outlier is present for the phone-using group. 


```{r PhonePlots, fig.width=4.5, fig.height=3.5, out.width='45%', fig.alt="Boxplot of the two groups in the reaction-time data", fig.align="center", fig.cap="A boxplot of the reaction-time data"}
boxplot(ReactionTimePhoneInd$Reaction ~ ReactionTimePhoneInd$Group,
        col = plot.colour,
        las = 1,
        ylim = c(400, 1000),
...
The information can be found using jamovi (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)), then compiled into a table (Table\ \@ref(tab:PhoneNumerical)).
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(Phone.DataSummary2,
                      decDigits = c(2, 0, 2, 3)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
...
```{r ReactionPhoneSummaryjamovi, fig.cap="jamovi output for the phone reaction time data", fig.align="center", out.width='100%'}
...
Since the difference between the *population* means  $\mu_P - \mu_C$ is unknown, the difference is estimated using the *sample* means.
...
However, for the reaction-time data, computing differences as the reaction time for phone users, *minus*  the reaction time for non-phone users (controls) probably makes more sense: the differences then refer to how much greater (i.e., slower) the reaction times are, on average,  when students are using phones.
...
The *sampling distribution of the difference between two sample means* is, when the appropriate conditions are met (Sect.\ \@ref(ValidityTwoMeans)), described by:

* an approximate normal distribution;
* centred around a sampling mean whose value is ${\mu_{P}} - {\mu_{C}}$, the difference between the *population* means;
...
<!-- ```{r ReactionTimeSamplingDist, fig.cap="The sampling distribution of the difference between the reaction times in the phone and control groups (phone, minus control)", fig.align="center", fig.width=9.5, fig.height=2.75, out.width='95%'} -->
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
What does a *negative* differences between the means represent?\label{thinkBox:NegativeDifference}
...
<!-- ```{r NotationTwoMeansCI} -->

<!-- TwoMeansNotation <- array( dim = c(6, 2)) -->

<!-- TwoMeansNotation[1, ] <- c("Individual values in the population", -->
<!--                           "Group A: Vary with mean $\\mu_A$ and standard deviation $\\sigma_A$") -->
<!-- TwoMeansNotation[2, ] <- c("", -->
<!--                           "Group B: Vary with mean $\\mu_B$ and standard deviation $\\sigma_B$") -->

<!-- TwoMeansNotation[3, ] <- c("Individual values in a sample", -->
<!--                           "Group A: Vary with mean $\\bar{x}_A$ and standard deviation $s_A$") -->
<!-- TwoMeansNotation[4, ] <- c("", -->
<!--                           "Group B: Vary with mean $\\bar{x}_B$ and standard deviation $s_B$") -->

<!-- TwoMeansNotation[5, ] <- c("Difference between sample means", -->
<!--                           "Vary with approx. normal distribution (under certain conditions):") -->
<!-- TwoMeansNotation[6, ] <- c("($\\bar{x}_A - \\bar{x_B}$) across all possible samples", -->
<!--                           "sampling mean $\\mu_ - \\mu_B$; standard deviation $\\text{s.e.}(\\bar{x}_A - \\bar{x}_B)$") -->


<!-- if( knitr::is_latex_output() ) { -->
<!--   kable( TwoMeansNotation, -->
<!--          format = "latex", -->
<!--          booktabs = TRUE, -->
<!--          longtable = FALSE, -->
<!--          escape = FALSE, -->
...
<!--   kable_styling(font_size = 10) -->
<!-- } else { -->
<!--   TwoMeansNotation[5, 1] <- paste(TwoMeansNotation[5, 1],  -->
<!--                                  TwoMeansNotation[6, 1]) -->
<!--   TwoMeansNotation[5, 2] <- paste(TwoMeansNotation[5, 2],  -->
<!--                                  TwoMeansNotation[6, 2]) -->
<!--   TwoMeansNotation[6, ] <- NA -->
...
The sampling distribution describes how the values of $\bar{x}_P - \bar{x}_C$ are likely to vary from sample to sample. 
...
> Based on the sample, the difference between reactions times is $`r round(RT.mn, 2)`$\ ms, slower for those *using* a phone (mean: $585.19$; s.e.: $15.85$; $n = 32$) compared to those *not using* a phone (mean: $533.59$; s.e.: $11.55$; $n = 32$), with an *approximate* 95% CI for the difference between mean reaction times from $`r round(RT.mn - 2*RT.se, 2)`$ to $`r round(RT.mn + 2*RT.se, 2)`$\ ms.
...
The jamovi output (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)) shows *two* CIs.
*We will use the results from the second row*, as this row of output is more general, and makes fewer assumptions.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
jamovi gives *two* confidence intervals.
In this book, we will use the *second row* of information (the 'Welch's $t$' row) because it is more general and makes fewer assumptions.
...
From the output, the standard error is $\text{s.e.}(\bar{x}_P - \bar{x}_C) = 19.612$, and the exact $95$% CI is from $12.39$ to $90.80$.
...
As usual, these results apply under certain conditions (Example \@ref(exm:StatisticalValidityAnalogy)).
...
(An error bar chart can also be used to compare more than two groups.)


```{r PhoneErrorbar, fig.cap="Error bar chart comparing the mean reaction time for students using a mobile phone and not using a mobile phone (control)", fig.align="center", fig.width=4.5, fig.height=3.5, out.width='50%'}
...
axis(side = 1, 
     at = 1:2, 
     labels = levels(factor(ReactionTimePhoneInd$Group)), 
     las = 1) 
axis(side = 2, 
     las = 1)
box()
...
The data are summarised in Table\ \@ref(tab:SignageSummary) using the jamovi output (Fig.\ \@ref(fig:SpeedjamoviCI)).
Define $\mu$ at the mean speed (in km.h^-1^) on the exit ramp.
Then, the parameter is $\mu_B - \mu_A$, the *reduction* in the mean speed.
...
::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The *standard error must given* (e.g., the software output in Fig.\ \@ref(fig:SpeedjamoviCI)); you *cannot* easily calculate this from the other information; you are not expected to.
:::
...
```{r SignageErrorBar, fig.cap="Boxplot (left); error bar chart (right) showing the mean speed before and after the addition of extra signage, and the 95\\% CIs", fig.align="center", out.width='70%',fig.width=6.0, fig.height=3.25}
...
This means that, if many samples of size $38$ and $41$ were found, and the difference between the mean speeds were found, about $95$% of the CIs would contain the population difference ($\mu_A - \mu_A$).
Loosely speaking, there is a $95$% chance that our CI straddles the difference in the population means ($\mu_B - \mu_A$).

We could write:

> Based on the sample, the reduction in the mean speed after adding signage is $5.68$\ km.h^-1^ (before mean: $98.02$; s.e.: $2.140$; $n = 38$; after mean: $92.34$; s.e.: $2.051$; $n = 41$), with an approximate $95$% CI between $-0.24$\ km.h^-1^ (i.e., an increase of $0.24$\ km.h^-1^) and $11.6$\ km.h^-1^ (two independent samples).
...
Defining $\mu$ as the mean BHADP score, the parameter is $\mu_D - \mu_N$, the difference between the two population means (disability $D$, minus non-disability $N$).
The statistic is $\bar{x}_D - \bar{x}_N$.

In this case, only *summary* information is available (Table\ \@ref(tab:BHADPSummary)).
Nonetheless, an error bar chart can be produced by computing the CI for each group manually (Fig.\ \@ref(fig:BHADPErrorBar)).

The best estimate of the difference between the population means is the difference between sample means: $(\bar{x}_D - \bar{x}_N) = 6.76$.
The standard error for estimating this *difference* is $\text{s.e.}(\bar{x}_D - \bar{x}_N) = 0.80285$, as *given* in the table.
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(BHADPSummary,
                       decDigits = c(2, 2, 0, 5) ),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
```{r BHADPErrorBar, fig.cap="Error bar chart showing the mean BHADP score for people with and without a disability, and the 95\\% CIs", fig.align="center", fig.width=5.5, fig.height=3.25, out.width="55%"}
### Disability example
bit <- 0.05
...
An approximate $95$% CI for the difference in population mean BHADP scores  is $6.76 \pm (2 \times 0.80285)$, or from $5.15$ to $8.37$ (higher for those with a disability).
...
> Based on the sample, the difference between the mean BHADP scores is $6.76$, higher (i.e., worse) for those with a disability (mean: $31.83$; s.e.: $0.673$; $n = 132$) compared to those without (mean: $25.07$; s.e.: $0.410$; $n = 137$), with an approximate $95$% CI from $5.15$ to $8.37$.
...
However, a data summary is given in the article (motivating Table\ \@ref(tab:TGsummaryTable)), which can be used to create an error bar chart (Fig.\ \@ref(fig:ChamomilePlot)) to compare the mean reduction in each group.

The following relational RQ can be asked:

> For patients with T2DM, what is the difference between the mean reductions in TG, between those drinking chamomile tea and those who drink hot water?

Define $T$ and the tea group, and $W$ as the water group, and $\mu$ as the mean reduction in TG.
The *differences* between the TG reductions in the two groups can be defined either way, but we will define the differences as $\mu_T - \mu_W$.
Then, from Table\ \@ref(tab:TGsummaryTable), the estimate of the mean difference is $\bar{x}_T - \bar{x}_W = 45.74$\ mg.dl^-1^.
The standard error cannot be determined (easily) from the information in the data summary table; however, $\text{s.e.}(\bar{x}_T - \bar{x}_W) = 8.55$.
Hence, an approximate $95$% CI for the difference between the mean reduction is  
\[
  45.74 \pm (2\times 8.55),
\]
or from $28.64$ to $62.84$\ mg.dl^-1^, with a larger reduction for the tea group.
This is a CI for the *difference* between the mean reductions in each group.
We write:

> The difference between the mean *reductions* in TG is $45.74$\ mg.dl^-1^, greater for the tea-drinking group, with the approx. $95$% CI from $28.64$ to $62.84$\ mg.dl^-1^.
...
```{r ChamomilePlot, fig.align="center", fig.cap="The reduction in total glucose for the chamomile-tea drinking group, and the control group.", out.width='40%', fig.width=3, fig.height=3.5}
...
plot( x = c(0.75, 2.25),
      ...
      xlab = "Group",
      main = "Reductions in\ntotal glucose",
      sub = "(Bars are approx. 95% confidence intervals.)")
abline( h = 0,
        col = "grey")
...
A study [@data:barrett:echinacea] of the effectiveness of echinacea to treat the common cold compared, among other things, the duration of the cold for participants treated with echinacea or a placebo.
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(EchinaceaInfo,
                       decDigits = c(2, 2, 3, 0)),
         format = "latex",
         longtable = FALSE,
         booktabs = TRUE,
...
```{r DentaljamoviCI, fig.cap="jamovi output for the sugar-consumption data", fig.align="center", out.width="95%"}
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(SignageSummaryDec,
                     decDigits = c(3, 3, 5, 0)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
