...
kable(RT.DataTable,
      ...
      col.names = c("Using phone", 
                    "Not using phone"),
      caption = "Reaction times (in milliseconds) for students using, and not using, mobile phones. Only he first ten observations are shown, but 32 students are in each group",
      align = c("r", "r")) %>%
...
For the reaction-time data, we use the subscript $P$ is used for phone-users group, and $C$ for the control group.
...
For example, suppose Group A has 15 individuals, and Group B has 45 individuals, and we wish to study the difference $\mu_A - \mu_B$.
The sample size is not $45 - 15 = 30$... and certainly not $15 - 45 = -30$.
There are just two samples with given sizes.
...
```{r ReactionTimeSamplingDist, echo=FALSE, fig.cap="The sampling distribution of the difference between the reaction times in the phone and control groups (phone, minus control)", fig.align="center", fig.width=9.5, fig.height=2.75, out.width='90%'}
