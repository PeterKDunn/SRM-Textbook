...
# Confidence intervals for two independent means {#CITwoMeans}
...
```{r echo=FALSE}
POCI3 <- "**P**: Students (this is defined more specifically in the original study). **O**: Mean reaction time. **C**: Between those using and not using a mobile phone while driving. **I**: Yes; the use of a phone (or not) was decided by the researchers."
...
RT.DataTable     <- cbind( "Use phone"=head(RT$Reaction[RT$Group=="Phone"], 10), "Not using phone"=head(RT$Reaction[RT$Group=="Control"], 10))
RT.DataTableHTML <- cbind( "Use phone"=RT$Reaction[RT$Group=="Phone"], "Not using phone"=RT$Reaction[RT$Group=="Control"])
...
```{r echo=FALSE}
Phone.DataSummary <- Phone.DataSummary.rnd <- array(NA, dim=c(3, 4) )
...
rownames(Phone.DataSummary) <- c("Using phone", "Not using phone", "Differences")
```
...
## Two independent means: Notation {#TwoMeansNotationCI}
...
Phone.DataSummary2 <- array( dim=c(3, 4), data=Phone.DataSummary.rnd)
Phone.DataSummary2[3, 2] <- ""
Phone.DataSummary2[3, 3] <- ""
rownames(Phone.DataSummary2) <- c("Using phone", "Not using phone", "Difference")
...
kable(Phone.DataSummary2,
      ...
      longtable=FALSE,
      align="r",
      col.names=c("Mean", "Sample size", "Std dev", "Standard error"),
      digits=2,
      caption="The mean, median, standard deviation and standard error for the driving data (in ms)")
}
...
```{lemma StdDevStdError, name="Standard deviation and standard error"}
...
```{block2, type="fold"}
*Standard deviation*:  how much the individual reactions times vary from *person to person*.
...
```{definition name="Sampling distribution of the difference between two sample means"}
...
```{lemma UnderstandDiffs,  name="Understanding differences"}
...
Let's define the *difference* as the length of female gray whales *minus* the length of male gray whales.
...
## Conditions for statistical validity {#ValidityTwoMeans}
...
so the CI will be statisticall valid.
```
...
plot( range( c( ci.lo, ci.hi) ) ~ c(0.9,2.1), data=RT,
      type="n",
      ylim=c(500, 650),
     pch=19,
     axes=FALSE,
...
arrows(1:2, ci.lo[1:2], 1:2, ci.hi[1:2], 
  length=0.05, 
  angle=90, 
  code=3)
```    
...
```{lemma CompareGraphs, name="Comparing graphs"}
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-lucien-wanda-2827735.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-anderson-guerra-1171521.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
if( knitr::is_html_output() ) {
  kable(BHADPSummary,
    ...
    booktabs = TRUE,
    align=c("r", "r", "r", "l"),
    col.names = c("Sample mean", "Std deviation", "Sampe size", "Std error"),
    caption="The BHADP data summary"
  ) %>%
...
```{block2, type="rmdimportant"}
Remember:
clearly state which mean is larger.
```
