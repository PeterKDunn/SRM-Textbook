...
\[
  \text{s.e.}(\bar{x}_P - \bar{x}_C) = \sqrt{ \text{s.e.}(\bar{x}_P)^2 + \text{s.e.}(\bar{x}_C)^2},
\]
...
\]
the same as in the jamovi output (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)).
...
The standard error can be determined from the information in the first two rows of the data summary table: $\text{s.e.}(\bar{x}_T - \bar{x}_W) = 8.55$.
