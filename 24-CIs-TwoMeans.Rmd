...
A study examined the reaction times of students while driving [@data:Strayer2001:phones; @agresti2007statistics].
In one study, two different groups of students were used: one *used* a mobile phone, and one  *did not use* a mobile phone (a [*between individuals* comparison](#Comparison)).
...
Using this notation, the difference between population means (the parameter) is $\mu_A - \mu_B$.
Since the population values are unknown, this parameter is estimated using the statistic $\bar{x}_A - \bar{x}_B$.
...
For example, suppose Group A has 15 individuals, and Group B has 45 individuals, and we wish to study the difference $\bar{x}_A - \bar{x}_B$; what would the sample size be?
Certainly not $15 - 45 = -30$.

However, the *standard error* of the difference between the means does make sense:
it measures how much the value of  $\bar{x}_A - \bar{x}_B$ varies for all possible samples.
Each time the study is repeated, the means for each group are likely to be different (so have a standard error), and so the difference between the means is likely to be different.
...
For the reaction-time data, subscript $P$ is used for phone-users group, and $C$ for the control group.
The sample means are denoted as $\bar{x}_P$ and $\bar{x}_C$, and the difference between them as $\bar{x}_P - \bar{x}_C$.


## Summaries: Two independent means

A suitable graphical summary is a boxplot (Fig.&nbsp;\@ref(fig:PhonePlots)), which shows that the *sample* medians are slightly different, but the IQR about the same; one large outlier is present for the phone-using group. 


```{r PhonePlots, echo=FALSE, fig.width=3.5, fig.height=3.0, out.width='100%', fig.alt="Boxplot of the two groups in the reaction-time data", fig.align="center", fig.cap="A boxplot of the reaction-time data"}
...
The numerical summary should summarise both groups, and the *differences between the means* (since the RQ is about this difference).
...
#      digits=c(2, 0, 2, 3),
        col.names = c("Mean", 
                      ...
                      "Standard deviation", 
                      "Standard error"),
    caption = "FRED The mean, median, standard deviation and standard error for the driving data (in ms)") %>%
		row_spec(0, bold = TRUE)  %>%
...
```{r ReactionTimeSamplingDist, echo=FALSE, fig.cap="The sampling distribution of the difference between the reaction times in the phone and control groups (phone, minus control)", fig.align="center", fig.width=5, fig.height=2.25, out.width='70%'}
...
> Based on the sample, an *approximate* 95% CI for the difference in reaction time is from $`r round(RT.mn - 2*RT.se, 2)`$ to $`r round(RT.mn + 2*RT.se, 2)`$ milliseconds, higher for those using a phone compared to those *not* using a phone.
...
Define the *difference* as the mean length of female gray whales *minus* the mean length of male gray whales.
Then we wish to estimate the difference $\mu_F - \mu_M$ (where $F$ and $M$ represent female and male gray whales respectively); this is the *parameter* of interest.
The best estimate of this difference is $\bar{x}_F - \bar{x}_M = 4.66 - 4.60 = 0.06$.

This value is likely to vary from sample to sample, and hence it has a standard error.
Since we do not provide a formula to compute this standard error, it must be *given*: $\text{s.e.}(\bar{x}_F - \bar{x}_M) = 0.0929$.
Then the approximate 95% CI is from

\[
   0.06 - (2 \times 0.0929) \qquad \text{to}\qquad 0.06 + (2 \times 0.0929)
\]
so the CI is from $-0.12$&nbsp;m to $0.25$&nbsp;m.

One of these limits is a *negative* value. 
This does not mean a *negative* length for a whale; that would be silly.
This CI is for the *difference* between the mean lengths, and a *negative* length just says the mean length for males is greater than the mean length for females.
So we could say:
...
*We will use the results from the second row* in both cases, as this row of output is more general (and makes fewer assumptions^[The *first* row assumes that the standard deviations in the two groups are the same (in the population).]).
...
```{r PhoneErrorbar, echo=FALSE, fig.cap="Error bar chart comparing the mean reaction time for students using a mobile phone and not using a mobile phone (control)", fig.align="center", fig.width=4.5, fig.height=3.5, out.width='60%'}
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
What is different about the information displayed in the error bar chart (Fig.&nbsp;\@ref(fig:PhoneErrorbar)) and the boxplot (Fig.&nbsp;\@ref(fig:PhonePlots))?
