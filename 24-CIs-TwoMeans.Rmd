...
However, computing differences as the reaction time for phone users, *minus*  the reaction time for non-phone users (controls) probably makes more sense: the differences then refer to how much greater (i.e., slower) the reaction times are, on average, when students are using phones.
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
For those *using* a phone, what is the difference in meaning between the *standard deviation* and the *standard error* in the context of the reaction-time study?\label{thinkBox:SEsd}
...
A useful way to compare the means of two (or more) groups is to display the individual CIs in an *error bar chart*.
...
```{r PhoneErrorbar, fig.cap="Error bar chart comparing the mean reaction time for students using a mobile phone and not using a mobile phone (control)", fig.align="center", fig.width=4.5, fig.height=3.25, out.width='50%'}
...
\]
as in the jamovi output (Fig.\ \@ref(fig:ReactionPhoneSummaryjamovi)).
...
Choosing the second row assumes less, and is more widely applicable.
...
From the output, the exact $95$% CI is from $12.39$ to $90.80$\ ms.
The *approximate* CI and the *exact* (from software) CIs are only slightly different, as the samples sizes are not too small.
(Recall: software uses an *exact* multiplier, while the $t$-multiplier of $2$ is an approximation, based on the  [$68$--$95$--$99.7$ rule](#def:EmpiricalRule)).
...
The data are summarised in Table\ \@ref(tab:SignageSummary) using the jamovi output (Fig.\ \@ref(fig:SpeedjamoviCI2)).
Define $\mu$ at the mean speed (in km.h^-1^) on the exit ramp.
Then, the parameter is $\mu_B - \mu_A$, the *reduction* in the mean speed.


```{r SpeedjamoviCI2, fig.cap="jamovi output for the speed data", fig.align="center", out.width="100%"}
...
## Example: health promotion services {#BHADPCI}
...
The first two rows of information in Table\ \@ref(tab:BHADPSummary) appear in the original article.
The difference between the sample means: $(\bar{x}_D - \bar{x}_N) = 6.76$.
The standard error for estimating this *difference* is  
\[
  \text{s.e.}(\bar{x}_D - \bar{x}_N) = \sqrt{0.67280^2 +  0.41010^2} = 0.78794.
\]
...
An approximate $95$% CI for the difference in population mean BHADP scores  is $6.76 \pm (2 \times 0.78794)$, or from $5.18$ to $8.34$ (higher for those with a disability).
...
> Based on the sample, the difference between the mean BHADP scores is $6.76$, higher (i.e., worse) for those with a disability (mean: $31.83$; s.e.: $0.673$; $n = 132$) compared to those without (mean: $25.07$; s.e.: $0.410$; $n = 137$), with an approximate $95$% CI from $5.18$ to $8.34$.
...
Here, two separate groups are being compared:
the tea-drinking group ($T$), and the control water-drinking group ($W$).
Defining $\mu$ as the mean reduction in TG, we will define the *differences* between the TG reductions in the two groups as $\mu_T - \mu_W$.
...
```{r ChamomilePlot, fig.align="center", fig.cap="The reduction in total glucose for the chamomile-tea drinking group, and the control group.", out.width='45%', fig.width=3.5, fig.height=3.25}
...
Some summary data are given in Table\ \@ref(tab:WhaleInfo).
...
1. Construct an error bar chart.
1. Compute the standard error of this difference.
...
A numerical summary and jamovi output is given in Exercise\ \@ref(exr:CompareQuantExercisesNHANES).
...
```{r FallFowardTTestjamovi2, fig.cap="jamovi output for the lean-forward angles data", fig.align="center", out.width='95%'}
