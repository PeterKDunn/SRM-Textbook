...
Qualitative data, such as that in Table \@ref(tab:Bread), can also be summarised using *proportions* or *percentages*.
These can be given instead of, or (as in Table \@ref(tab:Bread)) together with, the counts.
...
::: {.example #ProportionsPercentages name="Porportions and percentages"}
Consider again the data in Table \@ref(tab:Bread), summarising results from a sample of 300 respondents.
...
\end{align*}
The *sample* proportion of successful procedures for *small* kidney stones is 0.882 (and the *sample* percentage is 88.2%).
...
For the *small* kidney stones (Table \@ref(tab:KS-Small)), the *column proportions* (Table \@ref(tab:KS-Small-colPC)) give the proportion of successes within each column (i.e., for successes and for failures), since the columns contain the procedure results.
...
<!--          tab1 ), -->
<!--     format = "latex", -->
<!--     caption = "DOUBLE TABLE CAPTION: TEST") %>% -->
...
Compute the success proportions for Method A and Method B when *small* and *large* stones are combined (Table \@ref(tab:KSAll)):\label{thinkBox:AllStonesSuccess}

* For small and large stones combined, the success proportion with Method A is: _____________
* For small and large stones combined, the success proportion with Method B is: _____________
...
That seems strange...
Method A performs better for small stones, and for large kidney stones... but Method B performs better when all stones are combined (ignoring size).


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
How can Method A be better when small and large stones are considered separately, but Method B be better when all stones are combined?\label{thinkBox:ExplainSimpson}
...
Consider again the **small** kidney stone data (Table \@ref(tab:KS-Small)).
The odds of a success using *Method B* is (Table \@ref(tab:KS-Small)) $234/36 = 6.52$.

Working with the corresponding proportions or percentages (Table \@ref(tab:KS-Small-rowPC)) rather than the numbers, the same value results: $86.7/13.3 = 6.52$.
:::
...
```{r OddsRatioKS, fig.cap="The odds ratio of success, comparing Method A to Method B, for the small kidney stones data", fig.align="center", fig.height=3, fig.width=7.5, out.width='75%'}
par( mar = c(0.05, 0.05, 0.05, 0.05))
openplotmat()

pos <- array(NA, 
             dim = c(4, 2))
pos[1, ] <- c(0.50, 0.75) # Odds success 
pos[2, ] <- c(0.50, 0.25) # Odds failure
pos[3, ] <- c(0.25, 0.50) # Odds failure
pos[4, ] <- c(0.75, 0.50) # Odds failure



straightarrow(from = pos[1, ], 
              to = pos[2,],
              arr.type = "none", # No arrow
              lcol = "black",
              lty = 1)
straightarrow(from = c(0.5, 0.5), 
              to = pos[3,], 
              lcol = "black",
              lty = 1)
...
The *odds ratio* (OR) comparing the odds of skipping breakfast, comparing females (Row 1) to males, is
...
The odds of skipping breakfast is different for females (0.560) and males (0.397), which may be due to eitehr (a) sampling variation; or (b) because a difference really exists in the population.
...
The *Southern Oscillation Index* (SOI) is a standardised measure of the air pressure difference between Tahiti and Darwin, and has been shown to be related to rainfall in some parts of the world [@climate:stone:1996], and especially Queensland, Australia [@climate:stone:1992; @mypapers:Dunn:bootstrap:2001].
