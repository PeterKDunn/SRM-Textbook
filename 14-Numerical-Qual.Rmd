...
For the modes of each variable:
...
* The *median* age group would be half-way between ordered observations 150 (age group 21 to 45) and 151 (age group 46 to 65); it is not clear what this means. Perhaps the median can be given as 45.5 years...
...
However, *ordinal* data **may be** [numerically summarised like quantitative data](#NumericalQuant) in **special** circumstance (for example, when the categories are considered 'equally spaced', a mid-point for each level is sensible, and the first level is fundamentally different than the others).


::: {.example #OrdinalMeans name="Means of ordinal data"}
Consider again the data in Table \@ref(tab:Bread).
Means are **not** sensible for 'Gender' as this variable is nominal.
...
The mean response to the statement **may** be sensible (and is used in the article) if the levels are considered equally spaced (that is, the distances between each of the five points on the scale are considered equal).
