...
In a study of the taste of bread with varying salt and fibre content [@gkebski2019impact], researchers recorded (Table \@ref(tab:Bread)), among other information from the 300 subjects:

* The gender of the subject;
* The age group of the subject;
* The subjects' place of residence; and
* The subjects' response to the statement 'Rolls with lower salt content taste worse than regular ones', on a five-point ordinal scale from 'Strongly Agree' to 'Strongly Disagree'.
...
Medians are not appropriate for 'Gender' (as it is nominal).
...
* the categories are considered 'equally spaced';
...
The aim of a medical study [@data:Charig:stones] was to compre two treatments for kidney stones to determine which was better.
...
The data can be displayed using a two-way table (Table \@ref(tab:KS-Small)), and by using a side-by-side or stacked bar chart, for example.
...
when computing percentages and proportions, we divide the relevant count by the *total number* relevant to the context.
...
So, perhaps the data for small (Table \@ref(tab:KS-Small)) and large kidney stones (Table \@ref(tab:KS-Large)) can therefore be combined, to produce a single two-way table of just Method and Result (Table \@ref(tab:KSAll)), ignoring size.
...
The size of the stone is related to success proportion (small stones have a greater success proportion) *and* the size of the stone is related to the method used
...
If the size of the kidney stone had not been recorded, size would have been a lurking variable, and the incorrect conclusion would have been reached.
...
With percentages and proportions, we divide the relevant number by the *total number* relevant to the context.
...
In fact, in the sample, the odds of success for Method A is $13.5\div 6.5 = 2.08$
