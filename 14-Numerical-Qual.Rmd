...
Because qualitative data has [*levels*](#def:Levels), all qualitative data (nominal; ordinal) can be numerically summarised by *counting* the number of observations in each level.
...
In a study of the taste of bread with varying salt and fibre content [@gkebski2019impact], researchers recorded (Table \@ref(tab:Bread)) information from the 300 subjects, including gender, age group, place of residence, and the subjects' responses to the statement 'Rolls with lower salt content taste worse than regular ones', on a five-point ordinal scale from 'Strongly Agree' to 'Strongly Disagree'.
...
* Likewise, both age groups contain 150 subjects. 
...
* The *median* age group would be half-way between ordered observations 150 (age group 21 to 45) and 151 (age group 46 to 65); perhaps the median can be given as 45.5 years.
...
```{r Bread}
BreadTable  <- array( dim = c(13, 3))

colnames(BreadTable) <- c("",
                          "Number",
                          "Percentage")
rownames(BreadTable) <- c("Gender",
                          "",
                          "Age",
                                 "",
                                 "Place of residence",
...
Commonly, qualitative data is numerically summarised using [*percentages*](#def:Percentage) and [*odds*](#def:Odds), which we study using an example.
The aim of a medical study [@data:Charig:stones] was to compare two treatments for kidney stones to determine which was better.
...
Treatment A was used from 1972--1980, and Treatment B from 1980--1985; that is, treatments were not *randomly* allocated, and so *confounding* may be an issue.
For this reason, the researchers also recorded the *size* of the kidney stone (as 'small' or 'large') as a possible confounding variable.

Firstly, consider just the **small stones** [@julious1994confounding].
The data can be displayed using a two-way table (Table \@ref(tab:KS-Small)), and a side-by-side or stacked bar chart, for example.
...
These can be computed as parts of [the *table as a whole*](#OverallPercentage), [by *rows*](#RowPercentages), or [by *columns*](#ColumnPercentages).
...
The size of the stone is related to success proportion (small stones have a greater success proportion) *and* the size of the stone is related to the method used (small stones are treated more often with Method B).
...
In this example, incorporating the size of the kidney stone is important, otherwise the wrong (opposite) conclusion is reached: one would think that Method B is better if the size of the stones was ignored, when the better method really is Method A.
...
The *0odds* are the proportion (or percentage, or number) of times that an event *happens*, divided by the proportion (or percentage, or number) of times that the event does *not happen*:
...
When computing odds, the count of interest is divided by the *remaining number*, which is different than how percentages are computed.
With percentages and proportions, the relevant number is divided by the *total number* relevant to the context.
...
Software usually computes odds by comparing either
...
Suppose one tutorial had 18 females and 5 non-females.
...
Consider again the **small** kidney stone data (Table \@ref(tab:KS-Small)).
The odds of a success using *Method B* can be found (Table \@ref(tab:KS-Small)):
...
\begin{align*}
  ...
  =\frac{234}{36} = 6.52.
\end{align*}
Working with the proportions (or percentages) (Table \@ref(tab:KS-Small-rowPC)) rather than the numbers, the same value results:
...
The odds and odds ratios are relative to the **first row** or **first column**.
:::
...
* **A difference does exist between the proportion (or percentage)** in the **population**, and this difference in the sample reflects this difference between the *population* proportion (or percentage).
