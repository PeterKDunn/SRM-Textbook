...
Because qualitative data has [*levels*](#def:Levels), all qualitative data (nominal; ordinal) can be numerically summarised by *counting* the number of observations in each level (or computing the percentages in each group).
...
In a study of the taste of bread with varying salt and fibre content [@gkebski2019impact], researchers recorded (Table \@ref(tab:Bread)) information from the 300 subjects, including gender, place of residence, and the subjects' responses to the statement 'Rolls with lower salt content taste worse than regular ones', on a five-point ordinal scale from 'Strongly Agree' to 'Strongly Disagree'.

'Gender' is *nominal* qualitative; all other variables are *ordinal* qualitative.
To summarise the data from each variable:

* Both listed genders contain 150 subjects. 
  This could be interpreted as having two modes, or having no mode, neither of which is very helpful.
  Better is simply to state there are equal numbers of females and males.
...
However, *ordinal* data **may be** [numerically summarised like quantitative data](#NumericalQuant) in *special* circumstances: when
...
Means are **not** sensible for 'Gender' since it is nominal.
Means are **not** suitable for 'Place of residence' either, as the categories are *not* equally spaced, and the final category is open-ended (so a mid-point for that level is not appropriate).

The mean response to the statement *may* be sensible (and is used in the original article) *if* the levels are considered equally spaced (that is, the distances between each of the five points on the scale are considered equal).
...
Commonly, qualitative data are numerically summarised using [*percentages*](#def:Percentage) or [*odds*](#def:Odds), which we study using an example.
...
For this reason, the researchers also recorded the *size* of the kidney stone ('small' or 'large') as a possible confounding variable.

Firstly, consider just the **small stones** [@julious1994confounding], displayed in Table \@ref(tab:KS-Small).
...
The data can be numerically summarised by computing *proportions* or *percentages*.
These can be computed as parts of the [*total number*](#OverallPercentage), [by *row totals*](#RowPercentages), or [by *column totals*](#ColumnPercentages).
These are demonstrated in sections below, and in a separate [Example](#ColumnPercentages).
...
Sample proportions are denoted using $\hat{p}$.
The *sample* proportion (a [*statistic*](#StatisticsAndParameters)) is an estimate of the unknown *population* proportion (a [*parameter*](#StatisticsAndParameters)), denoted $p$.


::: {.definition #Proportion name="Proportion"}
A *proportion* is a fraction out of a total, and are numbers between 0 and 1.
:::
...
When computing percentages and proportions, the relevant count is divided by the *total number* relevant to the context.
...
For the *small* kidney stones (Table \@ref(tab:KS-Small)), the *row proportions* (Table \@ref(tab:KS-Small-rowPC)) give the proportion of successes for each *Method*, since the rows represent the counts for Methods and B.
...
For the *small* kidney stones (Table \@ref(tab:KS-Small)), the *column proportions* (Table \@ref(tab:KS-Small-colPC)) give the proportion of successes within each method, since the columns contain the procedure results.
...
* $0.143$ (or 14.3% ) of *failures* came from using Method A.
...
For both small and large stones, the *success proportions* can be computed for Methods A and B (i.e., row percentages).
...
Compute the success proportions for the *large* stones too (Table \@ref(tab:KS-Large)):\label{thinkBox:LargeStonesSuccess}
...
So, could the data for small (Table \@ref(tab:KS-Small)) and large kidney stones (Table \@ref(tab:KS-Large)) could be combined, to produce a single two-way table of just Method and Result (Table \@ref(tab:KSAll)), ignoring size?
...
That seems strange...
Method A performs better for small, and for large kidney stones, but Method B performs better when all stones are combined (ignoring size).


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
How can Method A be better when small and large stones are considered separately, but Method B be better when they are combined?\label{thinkBox:ExplainSimpson}
...
In this example, incorporating a potential confounder (the size of the kidney stone) is important, otherwise the wrong (opposite) conclusion is reached: Method B would be considered better if the size of the stones was ignored, when the better method really is Method A.
...
Apart from proportions and percentages, another way to numerically summarise this information is to state that there are $81\div 6 = 13.5$ *times* as many successes than failures in the sample.
...
In a two-by-two table, software usually computes odds by comparing either
...
Based on Table \@ref(tab:KS-Small), the odds for the Methods computed by software will compare Method A compared to Method B (rather than Method B to Method A).
:::
...
The *population* odds of finding a female is $67 / (100 - 67) = 2.03$: about twice as many students are females compared to non-females.\smallskip

Suppose one specific class had 18 females and 5 non-females.
The *sample* odds of finding a female in this class is $18/5 = 3.60$.\smallskip
...
Consider again the **small** kidney stone data (Table \@ref(tab:KS-Small)).
The odds of a success using *Method B* is (Table \@ref(tab:KS-Small)) $\frac{234}{36} = 6.52$.\smallskip

Working with the proportions (or percentages) (Table \@ref(tab:KS-Small-rowPC)) rather than the numbers, the same value results: $\frac{86.7}{13.3} = 6.52$.
:::
...
This value is the **odds ratio** (OR); see Fig. \@ref(fig:OddsRatioKS).
...
jamovi and SPSS output can be interpreted in *either* of these ways:
...
```{r OddsRatioKS, fig.cap="The odds ratio for the small kidney stones data", fig.align="center", fig.height=3, fig.width=6.5, out.width='80%'}
par( mar = c(0.05, 0.05, 0.05, 0.05))
openplotmat()

pos <- array(NA, 
             dim = c(2, 2))
pos[1, ] <- c(0.17, 0.5) # Odds success 
pos[2, ] <- c(0.83, 0.5) # Odds failure
...
* Odds ratio is *greater* than one:  
...
Is there a difference in the *population* though?
...
(Notice that the number of top of the fraction in each case is from the *first* column.)
So, about 55.98 females *skip* breakfast for every 100 females who *eat* breakfast.
...
```{r NHANESBigSummary, cache=FALSE}
...
* Why is the percentage of females in the whole sample 50.2%, but the percentage of females is less than this for both non-smokers and smokers (43.8% and 43.5% respectively)?
...
With two qualitative variables, data can be compiled into a two-way table of counts, and then the data can be numerically summarised using *row* percentages, *column* percentages, *odds*, or *odds ratios*.
...
A study [@data:Alley2017:SocialMedia] examined social media (SM) use, using a representative sample of Queenslanders at least 18 years of age.
...
The *Southern Oscillation Index* (SOI) is a standardised measure of the pressure difference between Tahiti and Darwin, and has been shown to be related to rainfall in some parts of the world [@climate:stone:1996], and especially Queensland, Australia [@climate:stone:1992; @mypapers:Dunn:bootstrap:2001].

The rainfall at Emerald (Queensland) was recorded for Augusts between 1889 to 2002 inclusive [@mypapers:dunnsmyth:glms], for months when the monthly average SOI was positive, and for months when the monthly average SOI was non-positive (zero or negative); see Table \@ref(tab:SOItableCI).
...
- Sect. \ref{thinkBox:LargeStonesSuccess}:
Method A has a higher success proportion ($192/263 = 0.7300$) than Method B (0.6875), for large kidney stones.
-Sect. \ref{thinkBox:ExplainSimpson}:
Method B performs worse for small and large stones. 
Also, the success proportions are always better for small stones. 
Then see that Method A is used far more often for large stones, for which *both* methods perform poorly.
In other words, Method A is used more often in situations where both methods perform poorly.
