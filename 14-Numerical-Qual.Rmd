...
In a study of bread with varying salt and fibre content [@@gkebski2019impact], researchers recorded (Table \@ref(tab:Bread)), among other information from the 300 subjects:

* The age group of the subject;
* The subjects' place of residence; and
* The subjects' response that to the statement 'Rolls with lower salt content taste worse than regular ones', on a five-point scale.

All three variables are ordinal.

For the modes:

* Both age groups contain 150 subjects. 
...
The medians are located at ordered observation $(300 + 1) / 2 = 150.5$, or half-way between ordered observations 150 and 151:
...
The mean response to the statement *may* be sensible (and is used in the article) if the categories are considered equally spaced (that is, the distances between each of the five points on the scale are considered equal).

Then, assigning **1** to 'Strongly agree' up to **5** for 'Strongly disagree', the *mean* response is 3.02 (effectively 'Neutral').

The categories for 'Place of residence' are not equally spaced, and the final category is open-ended, so using a mean is **not** appropriate.
:::
