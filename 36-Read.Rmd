...
Rather than reading articles thoroughly from start to finish, first read the *Abstract* (also called a *Summary*, or *Overview*) to obtain an overview of the whole article, without becoming lost in the details.
...
When reading research, check the terminology being used if you are unsure!
:::
...
* *Asking* the RQ:\index{Research question!repeated-measures}
  ...
  * The *units of analysis* are the individual students in the study:\index{Units of analysis}\index{Units of observation}
    each person has four measurements.
  * The main outcome is the (average) 'walking velocity' (though others are mentioned in the article).
* *Designing* the study:
  * The sampling method is not stated, but likely to be voluntary-response.
  * The sample size is $n = 42$.
* *Analyse* the data:
  * A quantitative variable (walking velocity) is being compared *within* individuals, so paired $t$-test are the likely method of analysis (Chap.\ \@ref(AnalysisPaired)).
* *Report the results*:
...
The *Introduction* introduces the context, and establishes what is known about the topic.
It also states the *aim* of the study (p.\ 1):

> The aim of this study is to analyze how the use of a cell phone while walking at different velocities affects gait parameters, i.e., velocity... and stride length.

(Other response variable were also studied (cadence, stride width), but we will focus on walking velocity and stride length for brevity.)
...
* The *sample*\index{Sample} comprises students from the University School of Physical Education (Poland) studying a course in gait analysis.
  This sample may not represent any general population, though the impact on non-students and people not in Poland are likely to be similar.\index{External validity}
...
* The study was ethical,\index{Ethics} with permission sought from the students, and given approval by the Senate Committee on Research Ethics at the University School of Physical Education in Wroclaw.
...
person walks most naturally).
  ...
  3. Task\ 1 was repeated, but with the subjects continuously typing a sentence on a cell phone. 
  4. Task\ 2 was repeated, but with the subjects continuously typing a sentence on a cell phone.
...
* The software used, and the version, was stated: TIBCO Statistica\textregistered\ 13.3.0 (StatSoft Poland).
...
* The data are not immediately available (probably due to ethics concerns) but 'are available upon request from the corresponding author' (p.\ 7).
* Case-profile plots\index{Graphs!case-profile plot} were produced for the four tasks, but only showing the means for each Task rather than all for all\ $42$ individuals (much like Fig.\ \@ref(fig:RunningRM)). 
* Three response variables were used: left-side stride length, right-side stride length, and walking velocity.
* Correlations were computed between the left-side stride length, right-side stride length, and walking velocity.
  Since the relationship were non-linear, though, Spearman correlations\index{Correlation coefficient!Spearman} were used rather than the Pearson correlations studied in Chap.\ \@ref(CorrelationRegression).
  The $P$-value for these correlations (which were all positive) were all less than $0.05$, suggesting positive (non-linear) relationship between the variables.
* The following comparisons were made between the left-side stride lengths, right-side stride lengths and the walking velocities:
  1. Task\ 1 and Task\ 3 were compared: this explored the impact of texting on a cell phone when walking at a comfortable velocity.
  2. Task\ 2 and Task\ 4 were compared: this explored the impact of texting on a cell phone when walking at a fast velocity.
...
* A total of nine hypothesis tests were conducted: the three between-tasks comparison listed above, for the three response variables.
* For all nine hypothesis tests, the $P$-values were small 
* A small $P$-value was defined as 'smaller than $0.017$ rather than the common $0.05$.
  The reason for this is to reduce the chance of making a Type\ I error (Sect.\ \@ref(TypeErrors)),\index{Type\ I error} but the details are beyond the scope of this book.
* Mean walking velocity was smaller for Task\ 1 compared to Task\ 3, and for Task\ 2 compared to Task\ 4 (i.e., while texting).
* Mean stride length was smaller for Task\ 1 compared to Task\ 3, and for Task\ 2 compared to Task\ 4 (i.e., while texting).
* The researchers also made qualitative observations; for example:  they observed 'moments when the subject took their eyes off the phone in order to assess the direction of the path' (p.\ 5).
...
7. The two-tailed $P$-value for this test as given as $0.001$.
...
9. Construct the $2\times 2$ table for displaying these data.
...
3. The $P$-value for testing the slope is given as $P < 0.001$. 
...
if( knitr::is_latex_output() ) {
  kable(pad(seleniumTab,
        ...
        escape = FALSE,
        align = "c",
        caption = "Number of wells with dissolved Se above $2$ $\\mu$g.L$^{-1}$, and the geology within $5$\\kms\\ radius."
  ) %>%
...
> The results of a Welch two sample $t$-test showed that surviving *Cx. pipiens* control larvae were significantly larger than surviving *Ae. albopictus* control larvae ($\text{mean}\pm\text{SD}$: *Cx. pipiens*${} = 1.64 \pm 0.18$\mms, *Ae. albopictus*${} = 1.36 \pm 0.13$\mms; $\text{$p$-value} =< .0001$).
...
11. Construct the $2\times 2$ table summarising the data.
...
16. Test if sleep duration *increased* in the treatment group.
...
18. Test if sleep duration *increased* in the control group.
19. What do these results mean?
20. What could explain the *increase* in sleep duration, if the control group using an ineffective intervention? 
