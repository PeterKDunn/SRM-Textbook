...
Sometimes, the value for $s$ comes from:

* The results from a [pilot study](#protocols) are used for a value of $s$.
* The results from a similar study, where the value $s$ there can be used (see Example \@ref(exm:SampleSizePeanuts)).
...
Suppose we wanted to estimate the mean cadmium concentration in *Australian* peanuts, to give-or-take $0.005$&nbsp;ppm.
...
In the absense of any better data, we would use the vaklue for $s$ from the NHANES study,
which studies Americans. 
The value of $s$ for Americans would likely to be reasomnably similar to the value for Australians.
...
Exercise @ref(exr:CIOneProportionSnacking} discusses a study of the eating habits of university students in Canada
...
Suppose we wished to repeat the study, but for *Australian* university students.
...
In Exercise \@ref(CIOneMeanLungCapacityInChildren),
...
Suppose we wished to repeat the study, but for 11-year-old *Australian* girls.
