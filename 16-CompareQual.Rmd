# Comparing qualitative data {#CompareQualData}



<!-- Introductions; easier to separate by format -->
```{r, child = if (knitr::is_html_output()) {'./introductions/16-CompareQual-HTML.Rmd'} else {'./introductions/16-CompareQual-LaTeX.Rmd'}}
```
...
Since qualitative data is typically collated in a table, more than one qualitative variable can be combined into tables also.
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(KS.small2.full,
                      decDigits = 0),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE, 
...
For the data in Example\ \@ref(exm:SmallKidneyStones), a stacked bar chart can be created by producing a bar for each method, and *stacking* the successes and failures for each method (Fig.\ \@ref(fig:QualGraphsStones), top left panel).
...
Instead of stacking the bars within each season on top of each other, the bars can be placed *side-by-side* within each season (Fig.\ \@ref(fig:QualGraphsStones), right panel).

::: {.example #BarSideBySide name="Side-by-side bar charts"}
For the data in Example\ \@ref(exm:SmallKidneyStones), a side-by-side bar chart can be created by producing two bars for each method (one for failures; one for successes), and placing these side-by-side (Fig.\ \@ref(fig:QualGraphsStones), bottom left panel).
...
Instead of bars, dots (or other symbols) can be used in place of a side-by-side bar chart.
...
## Numerical summaries of two-way tables
...
### Summarising by rows {#RowPercentages}


::: {.example}
The data in Table\ \@ref(tab:KS-Small) can be numerically summarised by computing *proportions* or *percentages* by *row totals*.
The rows refer to the different Methods, this will compare the two methods.
...
This shows that the odds of a success is far greater for Method\ A.
:::
...
if( knitr::is_html_output() ) {
  kable(surroundMaths(KS.small.colPC2,
                      decDigits = 0),
               format = "html",
               longtable = FALSE,
               digits = 1,
...
if( knitr::is_latex_output() ) {
  T1 <- kable(surroundMaths(KS.small.rowPC2,
                      decDigits = 1),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
  ...
  T2 <- kable(surroundMaths(KS.small.colPC2,
                            decDigits = 1),
        format = "latex",
        longtable = FALSE,
        digits = 1,
...
### Summarising by columns {#ColumnPercentages}
...
* Successful operations: $81 \div (81 + 234) = 0.257$ (or $25.7$%) in the sample were with Method\ A; and
* Unsuccessful operations: $0.143$ (or $14.3$%) in the sample were with Method\ A.
...
### Summary tables {#CompareQualTables}
...
Remember, the data in Table\ \@ref(tab:KidneySmallSum) is for a *sample*, but the RQ is asking about the *population*.
:::
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(KidneySmall.Sum,
                      decDigits = c(1, 1, 0)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
```{r}
if( knitr::is_latex_output() ) {
  T1 <- kable(surroundMaths(KS.large2,
                            decDigits = 0),
        format  ="latex",
        longtable = FALSE,
        booktabs = TRUE,
  ...
  T2 <- kable(surroundMaths(KS.all2,
                            decDigits = 0),
        format = "latex",
        longtable = FALSE,
        digits = 1,
...
One of the main purposes of the study was to determine contributors to the incidence of diarrhoea in young children ($85$ households had children under\ $5$).
The comparison in Fig.\ \@ref(fig:WaterAccessQualCompare) include some categories with small sample sizes, so the percentages shown may not be precise estimates of the population values.
The summary table (Table\ \@ref(tab:WaterAccessQual)) may suggest a difference due to keeping livestock.

As usual, the data come one of countless possible samples, but the RQ is about the population, so making a definitive decisions is difficult.
...
rownames(WANumericalSummary) <- c("Does not have livestock",
                                  "Has livestock",
                                  "Odds ratio")
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(WANumericalSummary,
                      decDigits = c(1, 3, 0)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
The data can be displayed in a two-way table, then summarised by comparing proportions, percentages and odds.
Odds ratio can be used to compare odds in two different groups.



## Quick revision questions {#Chap16-QuickReview}
