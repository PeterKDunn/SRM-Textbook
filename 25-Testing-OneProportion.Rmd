...
I bought two packs!
However, there was no way to know *which* die was loaded.
...
So, I could roll one die, and see how often a `r include_graphics("Dice/die1.png", dpi=1500)` actually appeared.
...
> For this die, is the population proportion of rolls that show a `r include_graphics("Dice/die1.png", dpi=1500)` equal to $1/6$?


## Statistical hypotheses and notation

If the die was fair, about one-sixth of rolls would produce a `r include_graphics("Dice/die1.png", dpi=1500)`, but not necessarily *exactly* one-sixth due to *sampling variation*.
...
Then, reasonably expected values of the sample proportion can be compared to the observed value of $\hat{p}$ from just one of those possible samples.
...
When the population proportion of rolls that are a `r include_graphics("Dice/die1.png", dpi=1500)` really is $p = 1/6$, what values of the *sample* proportion are reasonable to expect, given sampling variation?
...
I decided to use $n = 100$ rolls.
So, if $p$ really was $1/6$, and if certain conditions are met (Sect.\ \@ref(ValidityProportionsTest)), the distribution of the possible values of the sample proportion across all possible samples of size $100$ would be described using:
...
* However, the formula for a hypothesis test uses the *population proportion* $p$ from the null hypothesis (see Eq.\ \@ref(eq:StdErrorPknownTest)), since hypothesis testing *assumes that the null hypothesis is true*, and hence the value of $p$ is known.
...
It means that the observed sample proportion is more than six standard deviations from the mean, which is *highly unusual* according to the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule).
...
The $P$-value measures how unusual an observation is, when $H_0$ is assumed to be true.
...
1. *Assumption*: 
   Write the *null hypothesis* and *alternative hypothesis* about the *parameter* (based on the RQ), where $p$ is the population proportion of rolls that are `r include_graphics("Dice/die1.png", dpi=1500)`: 
   * $H_0$: $p = 1/6$, and 
