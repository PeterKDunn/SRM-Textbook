...
if( knitr::is_latex_output() ) {
  tb1 <- cbind( ReactionTime$Reaction[ReactionTime$Group == "Phone"][1:6],
                ReactionTime$Reaction[ReactionTime$Group == "Phone"][7:12],
                ReactionTime$Reaction[ReactionTime$Group == "Phone"][13:18],
                ReactionTime$Reaction[ReactionTime$Group == "Phone"][19:24],
                ReactionTime$Reaction[ReactionTime$Group == "Phone"][25:30],
                c(ReactionTime$Reaction[ReactionTime$Group == "Phone"][31:32], NA, NA, NA, NA) )
  ...
  tb2 <- cbind( ReactionTime$Reaction[ReactionTime$Group == "Control"][1:6],
                ReactionTime$Reaction[ReactionTime$Group == "Control"][7:12], 
                ReactionTime$Reaction[ReactionTime$Group == "Control"][13:18], 
                ReactionTime$Reaction[ReactionTime$Group == "Control"][19:24], 
                ReactionTime$Reaction[ReactionTime$Group == "Control"][25:30], 
                c(ReactionTime$Reaction[ReactionTime$Group == "Control"][31:32], NA, NA, NA, NA))
...
1. If one or both sample sizes are $25$ or smaller, *and* the *populations* corresponding to both comparison groups have an approximate normal distribution.
     
The sample size of $25$ is a rough figure; some books give other (similar) values (such as $30$).
...
```{r ChamomilePlot, fig.align="center", fig.cap="The reduction in total glucose for the chamomile-tea drinking group, and the control group. The horizontal grey line represented no mean change in the groups", out.width='45%', fig.width=3.5, fig.height=3.25}
...
To compute a confidence interval (CI) for the difference between two means, compute the difference between teh two sample mean, $\bar{x}_A - \bar{x}_B$, and identify the sample sizes $n_A$ and $n_B$.
...
Then the CI is:  
\[
   (\bar{x}_1 - \bar{x}_B) \pm \left( \text{Multiplier}\times\text{standard error} \right).
\]
