...
would vary due to sampling variation, but would be approximately $\hat{p} = 1/6$.
...
* Make an *assumption* about the parameter (Sect.\ \@ref(Assumption)).
  That is: assume I have a fair die, so that $p = 1/6$, where $p$ is the population proportion of rolls that show a 
`r if (knitr::is_latex_output()) {
   '\\largedice{1}.'
} else {
   '<span class="larger-die">&#9856;</span>.'
}`
* Describe the *expectations* of the statistic (Sect.\ \@ref(ExpectationOf)).
  That is: describe what value of the *sample* proportion $\hat{p}$ could reasonably be expected from a fair die.
* Take the sample *observations* (Sect.\ \@ref(Observation)).
  That is: roll the die many times to find a value of $\hat{p}$.
...
For one specific die, I am asking the the decision-making RQ:
...
The process requires us to be able to describe what value of the *sample* proportion $\hat{p}$ could reasonably be expected from a fair die, with $p = 1/6$ (that is, Step\ 3 of the decision-making process).
...
## The expectations of $\hat{p}$: the sampling distribution of $\hat{p}$ {#SamplingDistributionKnownp}
...
That is, what will be the value of the *sample proportion* of numbers that show a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}?'
} else {
   '<span class="larger-die">&#9856;</span>?'
...
Suppose a fair die was rolled $50$\ times, and this random procedure \index{Random procedure} was repeated *thousands* of times, and the proportion of even rolls was recorded for every one of those thousands of sets of $50$\ rolls.
  ...
  for (i in 1:num.sims){
    ...
    if (print_Histo[i]){
      out <- hist( prop.even,
                   ...
                   xlab = "Proportion of the 50 rolls showing an even number",
...
(ref:DieRollSamplingDist) The proportion of rolls that show a 
`r if (knitr::is_latex_output()) {
   '\\largedice{1},'
} else {
   '<span class="larger-die">&#9856;</span>,'
}` 
$\hat{p}$, is not the same for every sample of $50$ rolls; it varies around a mean of $p = 1/6$ (shown by the dot). The solid line is the normal distribution used to model the sampling distribution.


The sampling distribution is an approximate normal distribution; it shows a model of how the proportion of rolls showing a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=7.5, out.width='85%', fig.cap="(ref:DieRollSamplingDist)" }
...
* The *sampling mean* has the value of $p = 1/6$, the mean value of $\hat{p}$.
* The standard deviation, called the *standard error* $\text{s.e.}(\hat{p})$, has the value\ $`r round(se.die, 5)`$.
  (The source of this number will be revealed later; see Eq.\ \@ref(eq:StdErrorExampleDie).)
...
(ref:DieRollStdError) The sampling distribution is an approximate normal distribution; it shows a model of how the proportion of rolls showing a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
```{r NormalDieTheory, fig.cap="(ref:DieRollStdError)", fig.align="center", fig.width=8.25, fig.height=3.25, out.width='100%'}
pop.p <- 1/6
n <- 50
...
For the die example, where $n = 50$\ rolls and the population proportion of rolls showing a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
...
is $p = 1/6$:
\begin{equation} 
	\text{s.e.} (\hat{p}) = \sqrt{\frac{1/6 \times (1 - 1/6)}{50}} = 0.05270\dots
...
## The observations and the decision {#TestpObsDecision}

Figure\ \@ref(fig:NormalDieTheory) show what values of the sample proportion $\hat{p}$ are expected when I roll a fair die.
Step\ 3 of the decision-making process (Fig.\ \@ref(DecisionFlowDice)) is to now roll the die.

When I rolled the die, I saw a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
}`
appear $19$\ times in my $50$\ rolls.
This represents a sample proportion of
$$
\hat{p} = \frac{19}{50} = 0.38.
$$
In this unusual, or something unexpected?
Comparing this value of $\hat{p}$ to the sampling distribution in Fig.\ \@ref(fig:NormalDieTheory) shows that a sample proportion of $\hat{p} = 0.38$ is *highly* unusual from a fair die with $p = 1/6$.
...
$$
  ...
  = 4.05,
$$
whuch is a *very* large $z$-score (based on the $68$--$95$--$99.7$ rule).\index{68@$68$--$95$--$99.7$ rule}
...
I concluded that the die I was rolling was the loaded die (that is, $p \ne 1/6$).
I may be incorrect (after all, it is not *impossible* to observe $\hat{p} = 0.38$), but the evidence is certainly quite convincing.
...
Formally, the hypothesis test above proceeds as follows. 


## The process of hypotheses testing: hypotheses {#TestpObsDecisionHypothesis}
...
This is called the *null hypothesis*,\index{Hypotheses!null} which is denoted by $H_0$.
We write:
$$
  \text{$H_0$:\ } p = 1/6.
$$
The null hypotheses states the value of $p$ is $1/6$; in other words, if the sample proportion $\hat{p}$ is not equal to $1/6$, the discrepancy is explained by sampling variation.
...
$$
  \text{$H_1$:\ } p \ne 1/6.
$$
These two hypotheses offer different explanations for the discrepancy between the values of the population proportion (the parameter) and the sample proportion.
...
An alternative hypothesis like $p > 1/6$ (the population proportion is *larger* than $1/6$) or $p < 1/6$  (the population proportion is *smaller* than $1/6$) is a *one-tailed* hypothesis.
...
## The process of hypotheses testing: sampling distribution for $\hat{p}$ {#TestpObsDecisionSamplingDist}

**Step\ 2** in the decision-making process is to describe what values of the statistic (i.e., $\hat{p}$) could be expected under the assumption about the parameter; that is, *when the null hypothesis is true*.
...
For the die example, the sampling distribution for $\hat{p}$ is (see Def.\ \@ref(def:SamplingDistProp))

* an approximate normal distribution,
* centred around the sampling mean whose value is\ $p = 1/6$,
* with a standard deviation, whose value is $\text{s.e.}(\hat{p}) = 0.05270\dots$

Drawing the picture of the sampling distribution (like Fig.\ \@ref(fig:NormalDieTheory)) using this information is not necessary, but may be helpful.


## The process of hypotheses testing: the test statistic $z$ {#TestpObsDecisiontestStat}
...
was observed in\ $19$ of the $50$\ rolls, a sample proportion of $\hat{p} = 0.38$.
Since the sampling distribution has a normal distribution, the corresponding $z$-score was also computed as $z = 4.05$.

In the context of hypothesis testing, the $z$-score is called the *test statistic*.\index{Test statistic}\index{Test statistic!z@$z$-score}
The test statistic measures how far, in relative terms, the sample proportion is from the assumed value of the parameter.


## The process of hypotheses testing: $P$-values {#TestpObsDecisionPvalues}

**Step\ 4** of the decision making process is to use the information to make a decision: is the sample statistic *consistent* with what was expected, or does it *contradict* what was expected?

For the die example, the decision is reasonably easy: $z = 4.05$ is *very* large and *very* unlikely to be observed if $p = 1/6$.
This means the sample evidence *contradicts* what was expected if the assumption was true: the evidence suggests the die is loaded.
However, more generally, the evidence is evaluated using a $P$-value.\index{P@$P$-values}
...
Then, the two-tailed $P$-value is the shaded area in Fig.\ \@ref(fig:OnePropTestP) (top left panel): about\ $32$%, based on the $68$--$95$--$99.7$ rule.
...
Similarly, suppose the $t$-score is not quite equal to $z = -2$; say $z = -1.9$.
...
In this die-rolling example, where $z = 4.05$, the tail area is *very* small (using `r if ( knitr::is_html_output()) { 'Appendix\\ \\@ref(ZTablesOnline)'} else {'Appendices\\ \\@ref(ZTablesNEG) and\\ \\@ref(ZTablesPOS)'}`),
...
* 'Big' $P$-values mean the sample statistic (i.e., $\hat{p}$) could reasonably have occurred through sampling variation in one of the many possible samples, if the assumption made about the parameter (stated in $H_0$) was true: 
   ...
   There *is no* compelling evidence to support the alternative hypothesis.
* 'Small' $P$-values mean the sample statistic (i.e., $\hat{p}$) is *unlikely* to have occurred through sampling variation in one of the many possible samples, if the assumption made about the parameter (stated in\ $H_0$) was true: 
   ...
   There *is* compelling evidence to support the alternative hypothesis.

What is meant by 'small' and 'big' in this context?
What represents compelling evidence to support the alternative hypothesis?
A $P$-value smaller than\ $5$% (or\ $0.05$) is usually considered 'small', and compelling evidence to support the alternative hypothesis.
In contrast, a $P$-value larger than\ $5$% (or\ $0.05$) is usually considered 'big', and *not* compelling evidence to support the alternative hypothesis.
However, the value of\ $0.05$ is *arbitrary*, and in some disciplines the distinction is made when $P = 0.01$ or $P = 0.10$ instead.
...
In this die-rolling example, where the $P$-value is *very* small, the data contradict the null hypothesis (that $p = 1/6$): compelling evidence exists to support the alternative hypothesis that $p \ne 1/6$.
...
This means that the observed value of\ $\hat{p}$ could easily be explained by chance, and is *not* compelling evidence to  support the alternative hypothesis (that the die is unfair).
...
* sample summary information (see Chap.\ \@ref(CIOneProportion)), summarising the data used to make the decision.

So for the die-rolling example, write:

> The sample provides very strong evidence ($z = 4.05$; two-tailed $P < 0.001$) that the proportion of
`r if (knitr::is_latex_output()) {
   '\\largedice{1}'
} else {
   '<span class="larger-die">&#9856;</span>'
}`
is not\ $1/6$ ($\hat{p} = 0.38$; $n = 50$ rolls) in the population.
...
`r if (knitr::is_latex_output()) {
   ...
   * $H_0$: $p = 1/6$ (i.e., sampling variation explains the discrepancy between\ $p$ and\ $\hat{p}$), and 
...
Firstly, $n\times p = 50 \times (1/6) = 8.666\dots$ (i.e., I would expect about $8.7$\ rolls to show a `r if (knitr::is_latex_output()) {
   '\\largedice{1})'
} else {
   '<span class="larger-die">&#9856;</span>)'
}`,
and $n\times (1 - p) = 41.666\dots$ (i.e., I would expect about $41.7$\ rolls to *not* show a 
`r if (knitr::is_latex_output()) {
   '\\largedice{1})'
} else {
   '<span class="larger-die">&#9856;</span>)'
}`.
*Both* comfortably exceed five, so we expect the normal distribution to be a good approximation for the sampling distribution.
This is what we observe from simulating the situation (Fig.\ \@ref(fig:StatValidp), left panel).
:::



::: {.example #StatisticalValidityDice2 name="Statistical validity"}
Suppose we only rolled the die $10$\ times rather than $50$\ times.
Then, $n\times p = 10 \times (1/6) = 1.666\dots$ and $n\times (1 - p) = 10 \times (1 - 1/6) = 8.333\dots$.
These do not *both* exceed five, so we expect the normal distribution to be a poor approximation for the sampling distribution.
...
(ref:StatValidp) The sampling distributions for two situations for rolling a die. Left: for sets of $10$ rolls, the sampling distribution does not have a normal distribution Right: for sets of 50 rolls, the sampling distribution does have an approximate nromal distributions. The solid lines show the approximate normal distributions, and the histograms show the distribution of the sample proportions over many sets of rolls. The solid dots are the value $p = 1/6$, the population proportion of rolls that show a
`r if (knitr::is_latex_output()) {
   '\\largedice{1}.'
} else {
   '<span class="larger-die">&#9856;</span>.'
...
```{r StatValidp, fig.cap="(ref:StatValidp)", fig.align="center", fig.width=10, fig.height=2.75, out.width='95%'}
...
This means that the sample result was not unusual if $p = 1/6$, and is certainly not compelling evidence to support the alternative hypothesis.
...
A discrepancy exists between the sample proportion ($\hat{p} = 0.8222$) and the sample proportion $p = 0.50$, but can this be explained by sampling variation?

Of course, every sample of $45$\ interactions would produce a different sample proportion.
...
$$
The test is statistically valid, since $n\times p = 45\times 0.5 = 22.5$ and $n\times (1 - p) = 22.5$; both exceed five (i.e., expect half of the $50$\ interactions to be won by each species).
...
This is compelling evidence to support the alternative hypothesis.
We write:

> *Very* strong evidence exists in the sample ($P < 0.001$; $z = 4.325$) that the interactions were not won equally by each species ($\hat{p} = 0.8222$ won by green-backed tits; $n = 45$) in the population.
...
1. True or false: The sample size is $n = 381$. \tightlist
...
1. True or false: The *alternative* hypothesis is\ $H_0$: $p = 0.8486$.
`r if( knitr::is_html_output() ) {torf(answer=FALSE)}`
...
1. True of false: We have compelling evidence to support the alternative hypothesis in this example.
`r if( knitr::is_html_output() ) {torf(answer=TRUE)}`
...
They compared the proportion of children susceptible to measles with the *World Health Organization* target proportion (for children aged\ $5$ to\ $9$ years old: $10$%).

The aim of the study was to test if the proportion of Korean children susceptible to measles in the *population* was $10$%\ or *lower* (i.e., better).
...
@maeda2013introducing introduced pedal machines on the first floor of Joyner Library for use by students at East Carolina University (ECU) to increase activity in library users.
...
The expectation was that $1$-in-$16$ of teh second-generation boreres would be resistant if the field borers were resistant.
...
1. The value of\ $105.5$ is effectively a population odds ratio of male-to-female births.
   Show that this is equivalent to stating the population proportion of male births is\ $0.51338$ (not including 'other' or 'not stated').
