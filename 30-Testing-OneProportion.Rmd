...
\begin{wrapfigure}[9]{R}{.30\textwidth}
  \centering%
...
Suppose I rolled a die a certain number of times (say, $n = 25$\ times).
Then, I could determine the value of the sample proportion.
But if the observed value of $\hat{p}$ was not exactly equal to $p = 1/6$, two possible reasons exist to explain this discrepancy:
...
* I was rolling the *loaded* die  (with $p \ne 1/6$), and the discrepancy between the *population* and *sample* proportion simply reflected this situation.
...
To do so, we need to be able to describe what value of the *sample* proportion $\hat{p}$ could reasonably be expected from a fair die, with $p = 1/6$ (that is, Step\ 3 of the decision-making process).
...
When a fair, six-sided die is rolled $25$\ times, what proportion of the rolls will produce an even number?
...
However, being more specific about the details of the normal distribution is useful.
...
```{r RollDiceHistFig, fig.align="center", fig.height=3, fig.width=7.5, out.width='85%', fig.cap="The proportion of rolls that are a one, $\\hat{p}$, is not the same for every sample of $50$ rolls; it varies around a mean of $p = 1/6$. The solid line is the normal distribution used to model the sampling distribution." }
...
* the *sampling mean* has the value of $p = 1/6$,
* the standard deviation, called the *standard error* $\text{s.e.}(\hat{p})$, has the value\ $`r round(se.die, 3)`$.
  (The source of this number will be revealed later, in Eq.\ \@ref(eq:StdErrorExampleDie).)
...
While we still don't know *exactly* what the next roll will produce, we have some idea of *how* the sample proportion varies in samples of $50$\ rolls.
For instance, values of\ $\hat{p}$  greater than about\ $0.35$ are unlikely to be observed from a fair die (with $p = 1/6$).


```{r NormalDieTheory, fig.cap="The sampling distribution is an approximate normal distribution; it shows a model of how the proportion of even rolls varies when a die is rolled $50$ times.", fig.align="center", fig.width=8.25, fig.height=3.25, out.width='100%'}
pop.p <- 1/6
n <- 50
...
## Sampling intervals: for $p$ known {#CIpKnownp}
...
Sometimes the value of\ $\hat{p}$ would be a bit smaller than\ $1/6$, and sometimes a bit larger, even if the value of\ $p$ really was\ $1/6$.
...
Since the mean of the distribution is\ $p$ and standard deviation of the distribution is $\text{s.e.}(\hat{p})$, the $z$-score\index{z@$z$-scores} is
\begin{align*}
   z 
...
In other words, what represents compelling evidence to support the alternative hypothesis?
...
This means that the sample result was not unusual if $p = 1/6$, and there is no compelling evidence to support the alternative hypothesis.
...
Are casino-goers just as likely to be a smokers as the general US population?
:::
