...
```{r, RollsSixesSD, fig.height=3.25, fig.width=8.75, out.width='95%', fig.align="center", fig.cap="The sampling distribution, showing the distribution of the sample proportion of ones when the population proportion is $1/6$, in $50$ die rolls"}
p <- 1/6
n <- 100
mu <- p
...
From Fig.\ \@ref(fig:RollsSixesSD), this is practically impossible *if the die was fair*: it almost never occurs when we look at all possible samples.
...
```{r, OnePropTestP, fig.cap="The two-tailed $P$-value is the combined area in the two tails of the distribution. Left panel: if $z = 1$ (or $z = -1$). Right panel: if $z = 2$ (or $z = -2$). (The one-tailed $P$-values are half the two-tailed $P$-values: $P = 0.16$ and $P = 0.025$.)", fig.width=10, fig.height=2.75, out.width='95%', fig.align="center"}
par(mfrow = c(1, 2), 
    mar = c(4, 1, 4, 1) + 0.1)
...
* 'Big' $P$-values mean the sample statistic (i.e., $\bar{p}$) could reasonably have occurred through sampling variation in one of the many possible samples, if the assumption made about the parameter (stated in $H_0$) was true: 
...
In this die-rolling example, where the $P$-value is *very* small, the data contradict the null hypothesis (that $p = 1/6$), suggesting that the die is very likely *not* fair.
...
This means that the sample result was not unusual if $p = 1/6$; there is *no evidence* to suggest the die is loaded.

This all suggests that the first die was the loaded die.
But now I need to decide how to remember which die is the loaded one...
...
Computing the $95$% CI for the proportion requires using the standard error computed with\ $\hat{p}$:
$$
   \text{s.e.}(\hat{p}) 
