...
The parameter is $\mu_d$, which we define as the population mean *reduction* in %N when sites are irrigated, compared to non-irrigated.
...
However, the differences could be computed as the %N in *irrigated* sites minus %N in *non-irrigated* sites.
...
(If the differences had been defined in the opposite way---as '%N in *irrigated* sites minus *non-irrigated* sites'--- then the alternative hypothesis would be $\mu_d < 0$, which has the same *meaning*.)


##  Describing the sampling distribution
\index{Sampling distribution!mean difference}

The *sample* mean %N reduction will vary depending on which one of the many possible samples is randomly obtained, *even if* the mean reduction in the population is zero.
...
A numerical summary can be produced from jamovi output (Fig.\ \@ref(fig:StressDescriptivesjamoviHT)).
