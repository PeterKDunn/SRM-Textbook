...
\[
  \hat{y} = -4  + 2x.
\]
Here, $x$ refers to the value of the explanatory variable, $y$ refers to the *observed* value of the response variable, and $\hat{y}$ refers to the *mean predicted* value of the response variable given a value of $x$.
...
$\hat{y}$ refers to *mean predicted* (not observed) values of $y$ for some given vaue of $x$.
:::
...
However, a rough approximation of the value of the *intercept* can be made by drawing a sensible-looking straight line through the data, and determining what that line predicts for the value of $y$ when $x = 0$.
...
To demonstrate, consider the scatterplot in 
`r if (knitr::is_latex_output()) {
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The regression equation has $\hat{y}$ and not $y$ on the left-hand side.
That is, the equation *predicts* values of $y$, which may not be equal to the observed values (which are denoted by $y$).
...
<!-- FOR LaTeX: Put this example data plot and the static rise/run in the same plot -->
```{r ExampleScatterLATEX, fig.cap="An example scatterplot", fig.align="center", fig.width=9, fig.height=4, out.width='100%'}
set.seed(5000)
x <- seq(0.5, 5, 
        length = 10)
mu <- 2 + 3 * x
...
However, one of those lines is the 'best' line in some sense^[For those interested: The 'line of best fit' has the sum of the *squared* vertical distances between the observations and the regression line (i.e, the predicted values) is as small as possible.].
...
Making predictions outside the range of the available data is called *extrapolation*, and *extrapolation* beyond the data can lead to nonsense predictions.
...
Equation \@ref(eq:RedDeerEquation) predicts a mean molar weight of $`r RD.b0`$g for deer zero years of age.
...
However, under certain conditions, the sampling distribution for the slope follows a normal distribution, whose mean is $\beta_1$ and whose standard deviation is $\text{s.e.}(b_1)$.
(A formula exists for finding $\text{s.e.}(b_1)$, but it is tedious to use and we will not give it.)
This distributions describes the possible values of the sample slope, through *sampling variation*.
The variation in the sample slope across all possible samples is described (Fig. \@ref(fig:RedDeerSlopeSampDistCI)) using:

* an approximate normal distribution,
* with a mean of $\beta_1 = 0$, and 
* a standard deviation, called the *standard error of the slope*, of $\text{s.e.}(b_1)$ (taken from software (jamovi: Fig. \@ref(fig:RedDeerRegressionjamovi); SPSS: Fig. \@ref(fig:RedDeerRegressionSPSS)).


```{r RedDeerSlopeSampDistCI, fig.cap="The distribution of sample slopes for the red deer data, if the population slope is 0", fig.align="center", fig.height=3.0, fig.width=8, out.width='85%'}
mn <- 0
se <- 0.0289
...
\]
The multiplier is two for an approximate 95% CI (from the 68--95--99.7 rule), so (using the standard error reported by the software), an approximate 95% CI is $-0.181 \pm (2\times 0.029)$, or $-0.181 \pm 0.058$, or from $-0.239$ to $-0.123$.
Software can produce *exact* CIs too (Fig. \@ref(fig:RedDeerRegressionjamoviSPSSCI)).
The *approximate* and *exact* 95% CIs are very similar.
We write:

> SOMETHING

```{r RedDeerRegressionjamoviSPSSCI, fig.cap="Output for the red-deer data, including the CIs. Top: jamovi; bottom: SPSS", fig.align="center", out.width=c("75%", "90%"), fig.show="hold"}
...
This hypothesis proposes that $b_1$ is not zero due to sampling variation.
...
The standard error is found using software (Fig. \@ref(fig:RedDeerRegressionjamoviSPSS)).


```{r RedDeerSlopeSampDist, fig.cap="The distribution of sample slopes for the red deer data, if the population slope is 0", fig.align="center", fig.height=3.5, fig.width=8, out.width='85%'}
mn <- 0
se <- 0.0289
...
\end{align*}
where the values of $b_1$ and $\text{s.e.}(b_1)$ are taken from the software output (Fig. \@ref(fig:RedDeerRegressionjamoviSPSS)).
The $t$-score is also reported by the software.


### Determining the $P$-value

To determine if the statistic is *consistent* with the null hypothesis, the $P$-value can be approximated using the [68--95--99.7 rule](#def:EmpiricalRule), or taken from software output (Fig. \@ref(fig:RedDeerRegressionjamoviSPSS)).
...
The data (extracted from Fig. 2 of @brunette1991correlation) are displayed in Fig. \@ref(fig:EDScatter).
...
The results for the CI and the test hold under [certain conditions to be met](#exm:StatisticalValidityAnalogy).
...
1. Using the 'rise over run' idea, the *slope* is approximately \tightlist
...
1. Use the plot (Fig. \@ref(fig:PuntPlot)) to approximate of the values of the intercept and slope.
