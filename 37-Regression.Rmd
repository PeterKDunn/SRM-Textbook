...
if (knitr::is_latex_output()){
  ...
  plot(y ~ x, 
     ...
     ylim = c(0, 20), 
     las = 1, 
     main = expression(An~example~scatterplot),
...
#  for (i in (NumPlots:NumPlots)){
    ...
    plot(y ~ x, 
         ...
         type = "n",
         las = 1, 
         main = expression(Approximating~italic(b)[0]~and~italic(b)[1]),
         xlab = expression(Explanatory~italic(x) ), 
         ylab = expression(Response~italic(y) ), 
         pch = 19)
    rug(1:20, 
...
Using the jamovi output (Fig. \@ref(fig:OSARegressionjamovi)), the correlation coefficientis $r = 0.264$ (and $R$-squared is about 7%), so the relationship is not very strong.
...
In the population, the regression equation is  x
\[
