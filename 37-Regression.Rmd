...
## Linear equations: A review {#RegressionEquationsReview}
...
```{r AnimateRiseRun, echo=FALSE, animation.hook="gifski", cache=FALSE, interval=0.85, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
The relationship between the number of cyclones $y$ in the Australian region each year from 1969 to 2005, and a climatological index called the *Ocean Nino Index* (ONI, $x$), is shown in Fig. \@ref(fig:ONIcyclones) [@mypapers:dunnsmyth:glms].
...
For the red deer data again (Fig&nbsp;\@ref(fig:RedDeerScatter)), part of the relevant output is shown in Fig.&nbsp;\@ref(fig:RedDeerRegressionjamoviraw) (using jamovi) and Fig.&nbsp;\@ref(fig:RedDeerRegressionSPSSraw) (using SPSS).
...
```{r RedDeerSlopeSampDist, echo=FALSE, fig.cap="The distribution of sample slope for the red deer data, if the population slope is 0", fig.align="center", fig.height=3.0, fig.width=8, out.width='85%'}
mn <- 0
se <- 0.0289
...
\captionof{table}{Part of the obstructive sleep apnoea data set\label{tab:OSAKable}}
...
\begin{minipage}{0.45\textwidth}%
\centering
```{r OSAscatter, echo=FALSE, fig.width=3.75, out.width='99%'}
plot( REI ~ Neck,
      data = OSA,
...
`r if (knitr::is_latex_output()) '<!--'`
```{r OSAscatter2, echo=FALSE, fig.align = "center", fig.cap = "Scatterplot of the neck circumference vs REI for Down Syndrome adults", fig.width=5, fig.height=4}
plot( REI ~ Neck,
      data = OSA,
      xlab = "Neck circumference (in cm)",
      ylab = "REI",
...
```{r PuntPlot, echo=FALSE, results='hide', fig.align="center", fig.cap="Punting distance and right leg strength", fig.width=5, fig.height=3.5}
...
* Question in Sect. \ref{thinkBox:Predictions}: 12 and 19861283.
* Question in Sect. \ref{thinkBox:RegressionCoefs}: $b_0 = 2.1$ and $b_1 = -0.0047$.
  Remember: $b_1$ is the number multiplied by the $x$, and that $b_0$ is the number by itself.
* Question in Sect. \ref{thinkBoxCI}: Recall that all CIs have the same general form.
* Question in Sect. \ref{thinkBoxRegressionConditions}: The relationship looks approximately linear; the variations seems somewhat constant for all values of $x$ (the ONI), but it is a little hard to be sure. 
