...
In general, the equation of a straight line is written
...
However, one of those lines is the 'best fitting line' in some sense^[For those interested: The 'line of best fit' is the line such that 
...
Equation \@ref(eq:RedDeerEquation) predicts a molar weight of $`r RD.b0`$ for a deer zero years of age, which is likely to be nonsense: it is *extrapolating* beyond the data (the youngest deer in the sample is aged 4.4 years).


::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The value of the intercept $b_0$ is often (but not always) meaningless.
...
However, we know that every sample is likely to be different, so the relationship may not actually be present in the population.
...
\]
The multiplier is two for an approximate 95% CI, so (using the standard error reported by the software), we obtain $-0.181 \pm (2\times 0.029)$, or $-0.181 \pm 0.058$, or from $-0.239$ to $-0.123$.
