...
The last chapter introduced *correlation*, which measures the *strength* of the *linear* relationship between two quantitative variables $x$ and $y$.
...
An *example* of a liner regression equation is

\[
  \hat{y} = -4  + 2x.
\]
Here, $x$ refers to the value of the explanatory variable, $y$ refers to the *observed* value of the response variable, and $\hat{y}$ refers to the *predicted* value of the response variable.
More generally, the equation of a straight line is

\[
   \hat{y} = {b_0} + {b_1} x
\]
where $b_0$ and $b_1$ are just (unknown) numbers.
...
It's not the *order* that's important.
...
We will use software to find the values of $b_0$ and $b_1$ (the formulas are complicated).
However, roughly guess of the values of the *intercept* can be made by drawing what looks like a sensible straight line through the data, and determining what that line predicts for the value of $y$ when $x = 0$.
...
With the red-deer data, the intercept is measured in 'grams' (the measurement units of the molar weight).
\smallskip
...
However, one of those lines is the 'best fitting line' in some sense^[For those interested: The 'line of best fit' has the sum of the *squared* vertical distances between the observations and the line is as small as possible.].
...
From the output, the *slope* (in the sample) is $b_1 = `r RD.b1`$, and the $y$-intercept is $b_0 = `r RD.b0`$: the values of $b_0$ and $b_1$ are in the column labelled  `Estimate` in jamovi, and the column labelled `B` in SPSS.
These are the values of the two *regression coefficients*; then
...
The regression equation for the red deer data $\hat{y} = `r RD.b0` - `r abs(RD.b1)` x$ can be used to make *predictions*, such as predicting the *average* molar weight for deer $10$ years old.
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
What is the predicted mean molar weight for:
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Using the output (jamovi: Fig. \@ref(fig:RedDeerRegressionjamovi); SPSS: Fig. \@ref(fig:RedDeerRegressionSPSS)), can you determine the *approximate* 95% CI for $\beta_1$?
:::
...
\begin{minipage}{0.45\textwidth}%
\centering
```{r OSAscatter, echo=FALSE, fig.width=5, out.width='95%'}
plot( REI ~ Neck,
      data = OSA,
