...
The regression line being estimated above is from the observed sample data.
...
\begin{equation}
   ...
   (\#eq:RedDeerEquation)
\end{equation}
What does thsi equation reveal about the relationship between $x$ and $y$?
...
\]
From the *sample*, the *estimate* of the *population* regression line is  
\[
	\hat{y} = {b_0} + {b_1} x.
\]
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Using the output (Fig. \@ref(fig:RedDeerRegressionjamoviSPSSraw)), can you determine the *approximate* 95% CI for $\beta_1$?\label{thinkBoxCI}
:::
...
* a standard deviation, called the *standard error of the slope* and denoted $\text{s.e.}(b_1)$, with a value of $0.029$ (from software; Fig. \@ref(fig:RedDeerRegressionjamoviSPSS)).
...
\begin{align*}
   t 
   &= \frac{\text{observed sample slope} - \text{assumed population slope}}{\text{standard error of the slope}}\\
...
To determine if the statistic is *consistent* with the null hypothesis, the $P$-value can be approximated using the [68--95--99.7 rule](#def:EmpiricalRule), or taken from software output (Fig. \@ref(fig:RedDeerRegressionjamoviSPSSCI)).
...
> The sample presents very strong evidence ($t = -6.27$; one-tailed $P < 0.001$) that the slope in the population between age of the deer and molar weight is not zero (slope: $-0.181$; 95% CI from $-0.239$ to $-0.124$).
...
```{r}
OSA <- OSAoriginal <- read.csv("Data/OSA.csv")
...
> Does the digestible energy of feed *increase* with dry matter digestibility percentage (and if so, what is the relationship)?

In this study, $x$ is the dry matter weight digestibility percentage, and $y$ is the digestible energy (in calories per gram).
`r if( knitr::is_html_output() ) {
...
`
Using the software output (Fig. \@ref(fig:RuminantRegressionjamoviSPSS)), the values of the slope and $y$-intercept in the *sample* are  $b_0 = -0.193$ and $b_1 = 0.047$.
The regression equation is 
...
```{r ruminantKable}
if( knitr::is_latex_output() ) {
  ruminantUse <- array(dim = c(11, 3))
  ruminantUse[1:5, ] <- as.matrix(head(ruminant, 5)) 
  ruminantUse[7:11, ] <- as.matrix(tail(ruminant, 5)) 
  ruminantUse <- format(ruminantUse, 
                        digits = 1, 
                        nsmall = 1)
  ruminantUse[6, ] <- rep("$\\vdots$", 3) 

  knitr::kable(ruminantUse,
               format = "latex",
               longtable = FALSE,
               booktabs = TRUE,
               linesep = "",
               escape = FALSE,
               align = "c",
               col.names = c("digestibility (in \\%)",
                             "digestibility (in \\%)",
                             "energy (in cal/gram)"),
              caption = "Part of the digestibility dataset ($n = 36$)") %>%
    row_spec(0 , bold = TRUE) %>%
    kable_styling(font_size = 10) %>%
    add_header_above(header = c("Dry matter " = 1,
                                "Energy" = 1,
                                "Digestible" = 1), 
                     bold = TRUE, 
                     line = FALSE,
...
```{r RuminantRegressionjamoviSPSS, fig.cap="Output for the sheep-feed data. Top: jamovi; bottom: SPSS", fig.align="center", out.width=c("55%", "80%"), fig.show="hold"}
knitr::include_graphics( "jamovi/Ruminant/Ruminant-Reg.png")
knitr::include_graphics( "SPSS/Ruminant/Ruminant-Reg-crop.png") 
```
...
ADD: 
**Gorilla chest beats? (Not sig) and also to correlation**
