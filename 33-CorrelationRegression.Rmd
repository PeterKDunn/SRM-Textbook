...
@da2023dog recorded the average lifespan and average weight of $73$\ dog breeds [@da2023dogDATA];
`r if( knitr::is_latex_output() ) {
  'see Table\\ \\@ref(tab:DogsLifeData).'
} else {
  'see Fig.\\ \\@ref(fig:dogslifeDataDT).'
}`
The dog breed is the unit of analysis (the 'individuals').\index{Units of analysis}
...
if( knitr::is_latex_output()){
  ...
  knitr::kable( pad( DogsLifeData,
                ...
                align = c("r", "c", "c"),
                linesep = "",
                caption = "Information about various dog breeds. The first five and last five breeds only are shown.",
                col.names = c( "Breed", 
                               "Breed weight (in kg)",
...
In the dog-breed study, only one of the countless possible samples of dogs has been studied to determine the average breed-weights and lifespans, and the value of\ $r$ (an estimate of $\rho$, the *parameter*) will vary from sample to sample.
...
The RQ (as given in Sect.\ \@ref(CorReg-Intro)) is a two-tailed RQ, so the alternative hypothesis is:
...
That is, for a 'run' of $80 - 0 = 80$, the 'rise' is $5.5 - 13.5 = -8$ (i.e., a drop of\ $8$), and so a rough estimate of the slope is $-8/80 = -0.1\ys$/kg.\index{Linear equations!rise over run}
...
mLifespan <- lm( Lifespan ~ Weight,
  ...
  plot(Lifespan ~ Weight, 
       ...
       xlab = expression(Breed~weight~italic(x)~"(in kg)" ), 
       ylab = expression(Breed~lifespan~italic(y)~"(in y)" ), 
       pch = 19)
...
However, one of those lines is the 'best' line in some sense,^[For those interested: the 'line of best fit' is the line for which the sum of the *squared* vertical distances between the observations\ $y$ and the predicted values\ $\hat{y}$ (i.e, the regression line) is as small as possible.] sometimes called the 'line of best fit'.\index{Regression!equation!line of best fit}
...
\end{equation}
These are close to the values obtained using the rough method in Sect.\ \@ref(RegressionEquationsReview) (which gave $b_0 = 13.5$ and $b_1 = -0.1$ approximately).
...
```{r DogsLifeRegression, fig.cap="The regression output for the dog-breed data.", fig.align="center", out.width=c("55%"), fig.show="hold"}
...
For example, the regression equation for the dog-breed data in Equation\ \@ref(eq:RedDogsLifeRegressionEqn) can be used to make *predictions* of the mean lifespan for a given breed-weight.
...
The model predicts that the *mean* lifespan for dogs with a breed-weight of\ $50\kgs$ will be about $`r round(DB.b0 + DB.b1 * 50, 2)`\ys$.
...
$$
  \hat{y} = 13.39 - (0.093\times 100) = 4.09,
$$
or about\ $4.09\ys$.
...
As an extreme example, a breed-age of\ $x = 150$ would be predicted to have a mean lifespan of $-0.56\ys$, which is clearly nonsense.
...
Using $x = 0$ in Equation\ \@ref(eq:DogsLifeEquation) predicts a mean lifespan of
$$
  \hat{y} = 13.39 - (0.093\times 0) = 13.39
$$
for breeds with a weight of zero.
Using $x = 0$ is *extrapolating*\index{Extrapolation} (the smallest breed-weight in the sample is $4.08\kgs$).
...
Specifically, each extra kilogram of breed-weight is associated with a mean change of $`r DB.b1`\ys$ in lifespan (from Equation\ \@ref(eq:DogsLifeEquation)); that is, a *decrease* in lifespan by a mean of $`r abs(DB.b1)`\ys$ for each extra kilogram of breed-weight.
The lifespan of some breeds will be shorter and some longer, and the lifespan of some individual dogs will be shorter and some longer; the value is a *mean* change in breed lifespan.

To demonstrate, consider the case where $x = 10$, and the regression equation predicts $\hat{y}= `r DB.b0 + 10 * DB.b1`\ys$.
For breeds one kilogram heavier than this (i.e., $x = 11$), the value of the prediction $\hat{y}$ will increase by an average of $`r DB.b1`\ys$ (or, equivalently, *decrease* by an average of $0.093\ys$).
That is, we would predict $\hat{y} = `r DB.b0 + 10 * DB.b1`  - `r abs(DB.b1)` = `r DB.b0 + 11 * DB.b1`\gs$.
This is the same prediction made by using $x = 11$ in Equation\ \@ref(eq:DogsLifeEquation).
...
A slope of zero means that a change in the value of\ $x$ is associated with a change of zero in the value of\ $y$.
...
Hence, the regression line in the population is
$$
...
The sampling distributions for $b_0$ and\ $b_1$ also have approximate normal distributions under certain conditions (Sect.\ \@ref(ValidityCorrelationRegression)).
...
For the dog-breed data then, the values of the sample slope across all possible samples is described (Fig.\ \@ref(fig:DogsLifeSlopeSampDistCI)) as, using Def.\ \@ref(def:Beta1SamplingDistn):
...
$$
  -0.093 \pm (2\times 0.0109)\qquad\text{or}\qquad -0.093 \pm 0.0218,
$$
which is from\ $-0.115$ to\ $-0.071\ys$/kg (after rounding).

Software can be used to produce *exact* CIs too; the exact $95$% CI is from\ $-0.115$ to\ $-0.071\gs$/year (Fig.\ \@ref(fig:DogsLifeRegressionCI)).\index{Software output!regression}
...
Since the regression line describing the relationship between\ $x$ and\ $y$ is computed from one of countless possible samples, any relationship between\ $x$ and\ $y$ observed in the sample may be due to sampling variation; possibly, no relationship actually exists in the population (i.e., $\beta_1 = 0$).
...
The *test statistic* is found using the usual approach when the sampling distribution has an approximate normal distribution:\index{Test statistic!t@$t$-score}
\begin{align*}
   t 
...
Since $t = -8.54$, the $P$-value will be very small; software shows the *two*-tailed $P$-value is $P < 0.0001$.
...
For the dog-breed data, the scatterplot (Fig.\ \@ref(fig:DogsLifeScatterjamovi2), left panel) shows that the relationship is approximately linear, so using a (Pearson) correlation coefficient and a regression line is appropriate.
...
```{r CorrelationRemovalEfficiency, fig.cap="The scatterplot showing the relationship between removal efficiency and inlet temperature.", fig.align="center", fig.width=7, fig.height=3.75, out.width="80%", fig.show="hold"}
data(Removal)
