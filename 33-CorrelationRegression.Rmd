...
Specifically, each extra kilogram of breed-weight is associated with a mean change of $`r DB.b1`$\ys in lifespan (from Eq.\ \@ref(eq:DogsLifeEquation)); that is, a *decrease* in lifespan by a mean of $`r abs(DB.b1)`$\ys for each extra kilogram of breed-weight.
...
For example, generalized linear models [@mypapers:dunnsmyth:glms] may be appropriate for some non-linear relationships and/or relationships with non-constant variation in\ $y$.
...
1. According to the article, a hypothesis test for testing\ $H_0$: $\beta_0 = 0$ produced a $P$-value *much* larger than\ $0.05$.
   What does this mean?
1. For people who spend spend $8$\mins applying sunscreen, how much sunscreen would they use, on average?
