...
> In dogs, do larger breeds live longer or shorter, in general?

@da2023dog recorded the lifespan and weight of at least\ $50$ dogs of numerous dog breeds [@da2023dogDATA]
`r if( knitr::is_latex_output() ) {
  '(Table\\ \\@ref(tab:DogsLifeData)).'
} else {
  '(Fig.\\ \\@ref(fig:dogslifeDataDT).'
}`
The dog breed is the unit of analysis;\index{Unit of analysis} the individual dog is the unit of observation.\index{Unit of observation}
...
if( knitr::is_latex_output()){
  knitr::kable( pad( head(DogsLife[, c(1, 2, 5)]),
                  surroundMaths = TRUE,
                  targetLength = c(0, 5, 5),
                  decDigits = c(0, 3, 2)),
         format = "latex",
         booktabs = TRUE,
         longtable = FALSE,
         ...
         align = c("r", "c", "c"),
         linesep = "",
         caption = "Information about various dog breeds (at least $50$\ individuals from each breed were used). The first five breeds only are shown.",
         col.names = c( "Breed", 
	                      "Breed weight (in kg)",
...
Knowing the breed-weight provides some information about the lifespan: a relationship between the variables seems evident.
The relationship also seems somewhat *linear*.
The *correlation coefficient* (Fig.\ \@ref(fig:DogsLifeScatterjamovi2), right panel)\index{Correlation coefficient (Pearson)} is $r = -0.712$, so $R^2 = (-0.712) ^2 = 50.7$%.
...
A hypothesis tests can also be conducted regarding the correlation coefficient in the *population*, as measured by the unknown *population* (Pearson) correlation coefficient.\index{Correlation}
...
This null hypothesis is the 'no relationship' position, which proposes that the population correlation coefficient is zero.
...
However, the output (Fig.\ \@ref(fig:DogsLifeScatterjamovi2), right panel)\index{Software output!correlation} contains the relevant $P$-value for the test.
...
::: {.example #CycloneCorrelation name="Correlation"}
The relationship between the number of cyclones\ $y$ in the Australian region each year from\ 1969 to\ 2005, and a unitless climatological index called the *Ocean `r readr::parse_character( c("Niño"), locale=locale(encoding="UTF-8"))` Index* (ONI,\ $x$), is shown in Fig.\ \@ref(fig:ONIcyclonesCorrelation) (left panel) [@mypapers:dunnsmyth:glms].

The relationship has a *negative* direction, so the relationship is *negative*.
For the software output, $r = -0.683$ with a $95$% CI from $-0.461$ to\ $-0.824$.
:::
...
(The relationship is *negative*, as the slope is *negative*.)
...
The relationship between the number of cyclones\ $y$ in the Australian region each year from\ 1969 to\ 2005, and a climatological index called the *Ocean `r readr::parse_character( c("Niño"), locale=locale(encoding="UTF-8"))` Index* (ONI,\ $x$), is shown in Fig.\ \@ref(fig:ONIcyclonesCorrelation) (left panel) [@mypapers:dunnsmyth:glms].

To make a guess of the regression coefficient, a sensible line can be drawn through the data (Fig.\ \@ref(fig:ONIcyclones)).
Then, when the value of\ $x$ is zero, the predicted value of\ $y$ is about\ $12$, so\ $b_0$ is about\ $12$\ cyclones.
Recall: the intercept is the predicted value of\ $y$ when $x = 0$\ cyclones, which is *not* at the left of the graph in Fig.\ \@ref(fig:ONIcyclones).
...
Hence, $b_1$ is approximately $-9/4 = -2.25$ cyclones per unit change in ONI.
...
```{r ONIcyclones, out.width="70%", fig.align="center", fig.cap="The number of cyclones in the Australian region each year from 1969 to 2005, and the ONI averaged over October, November, December. The cross ${}+{}$ is located on the line where $x = 0$. The ${}\\times{}$ are located to fine rise-over-run.", fig.width=6, fig.height=4}
...
For the dog-breed data (Fig.\ \@ref(fig:DogsLifeScatterjamovi2)), the relevant software output is shown in Fig.\ \@ref(fig:DogsLifeRegression).

In the output, the values of\ $b_0$ and\ $b_1$ are in the column labelled `Estimate`; the value of the sample $y$-intercept is $b_0 = `r DB.b0`$\ys, and the value of the sample *slope* is $b_1 = `r DB.b1`$\ys/kg.
...
However, the *size* of the slope cannot be deduced from the *size* of the correlation coefficient.
:::
...
The regression equation for the cyclone data (Fig.\ \@ref(fig:ONIcyclones)) is found from the software output (Fig.\ \@ref(fig:CyclonesRegressionjamoviCI)):
$$
  \hat{y} = 12.14 - 2.23x,
$$
where\ $x$ is the ONI (averaged over October, November, December) and\ $y$ is the number of cyclones; that is, $b_0 = 12.14$\ cyclones and $b_1 = -2.23$ cyclones per unit change in ONI.
...
For example, the regression equation for the dog-breed data in Eq.\ \@ref(eq:RedDogsLifeRegressionEqn) can be used to make *predictions* of the mean lifespan for a given breed-weight.
...
\begin{align*}
         ...
         &= `r DB.b0` - `r abs(DB.b1 * 50)` = `r round(DB.b0 + DB.b1 * 50, 2)`.
\end{align*}
Dog breeds with an average weight of $50$\kgs are predicted to have a *mean* lifespan of $`r  round(DB.b0 + DB.b1 * 50, 2)`$\ys.
...
The model predicts that the *mean* lifespan for breeds weighing\ $50$\kgs will be about $`r round(DB.b0 + DB.b1 * 50, 2)`$\ys.
...
As an extreme example, a breed-age of\ $x = 150$ would be predicted to have a mean lifespan of $-0.565$\ys, which is clearly nonsense.
...
Using $x = 0$ in Eq.\ \@ref(eq:DogsLifeEquation) predicts a mean lifespan of
$$
...
The slope\ $b_1$ is how much the value of $y$ changes (on average) when the value of\ $x$ *increases* by one (Sect.\ \@ref(Regression-Software)).
For the dog-breed data, $b_1$ is the average change in mean lifespan for each\ $1$\kgs increase in breed-weight.
...
For breeds one kilogram heavier than this (i.e., $x = 11$), the value of\ $y$ will increase by an average of $b_1 = `r DB.b1`$\ys (or, equivalently, *decrease* by an average $0.093$\ys).
...
## Regression: CIs and $t$-test for regression parameters {#RegressionCI}

### Introduction {#RegressionTests-Intro}
\index{Confidence intervals!regression parameters}

A regression equation exists in the *population* that connects the values of\ $x$ and\ $y$.
...
Every sample is likely to produce a slightly different value for both\ $b_0$ and\ $b_1$ (*sampling variation*),\index{Sampling variation} so both\ $b_0$ and\ $b_1$ have a sampling distribution and a standard error.
...
### CIs for the regression parameters {#SamplingDistributionSlopeCI}
...
```{r DogsLifeSlopeSampDistCI, fig.cap="The distribution of sample slope for the dog-breed data, around the population slope\\ $\\beta_1$.", fig.align="center", fig.height=2.9, fig.width=8.25, out.width='85%'}
mn <- 0
se <- 0.011
...
> For each increase of one kg in the breed-weight of dogs, the mean lifespan *increases* by\ $-0.093$ years per kg ($95$%\ CI: $-0.115$ to\ $-0.071$; $n = 73$).

Alternatively (and equivalently, but probably easier to understand):

> For each increase of one kg in the breed-weight of dogs, the mean lifespan *decreases* by\ $0.093$ years per kg ($95$%\ CI: $0.071$ to\ $0.115$; $n = 73$).
...
In this context, 'no relationship' means that the slope is zero (Sect.\ \@ref(RegressionForUnderstanding)), so the null hypotheses (about the *population*) is $H_0$: $\beta_1 = 0$.
...
The *observed* sample slope for the dog-breed data is $b_1 = -0.181$.
...
The test are effectively equivalent.
:::
...
$$
   \text{$H_0$: } \beta_1 = 0\quad\text{and}\quad\text{$H_1$: } \beta_1 \ne 0.
$$
From the output (Fig.\ \@ref(fig:CyclonesRegressionjamoviCI)), $t = -5.52$ and the $P$-value is small: $P < 0.001$.
We write:
...
::: {.example #StatisticalValidityDogsLife name="Statistical validity"}
...
For the hypothesis test, the variation in lifespans doesn't seem to be obviously getting consistently larger or smaller for heavier breeds (though two breeds with a lifespan near\ $6$\kgs with a breed-weight near\ $20$ seem unusual), and the sample size is also greater than\ $25$.
...
```{r CorrelationRemovalEfficiency, fig.cap="The relationship between removal efficiency and inlet temperature. Left: scatterplot. Right: software output.", fig.align="center", fig.width=5, fig.height=5, out.width=c("49%", "45%", "65%", "49%"), fig.show="hold"}
data(Removal)
...
The regression equation in the population is $\hat{y} = \beta_0 + \beta_1 x$.
To compute a confidence interval (CI) for the slope of a regression equation, software provides the standard error of\ $b_1$; then, the CI is
$$
...
@leblanc2005paramedic studied $n = 30$ paramedicine students, usin correlations to study the relationship between the amount of stress experienced while performing drug-dose calculations (measured using the State–Trait Anxiety Inventory, STAI), and length of work experience.
...
1. According to the article, a hypothesis test for testing\ $\beta_0 = 0$ produced a $P$-value *much* larger than\ $0.05$.
   What does this mean?
1. If someone spent $8$\mins applying sunscreen, how much sunscreen would they use, on average?
...
The scatterplot from $16$\ different soil samples from Assam, India, is shown in Fig.\ \@ref(fig:SoilPlotCor) (right panel).
...
```{r SoilPlotCor, fig.cap="The relationship between CBR and PI in sixteen soil samples. Left: scatterplot. Right: software output.", fig.align="center", out.width=c("50%","5%", "40%"), fig.width=c(4, 5), fig.height=3.75, fig.show="hold"}
data(Soils) ### Exercise
...
[*Dataset*: `Bitumen`]
@data:Panda2018:Bitumen made $n = 42$ observations of hot mix asphalt, and measured the volume of air voids and the bitumen content by weight (Fig.\ \@ref(fig:BitumenPlot), left panel).
The software output is shown in Fig.\ \@ref(fig:BitumenPlot) (right panel).
...
```{r BitumenPlot, fig.cap="Air voids in bitumen. Left: scatterplot. Right: software output", fig.align="center", fig.width=4.25, fig.height=3.5, out.width=c("45%", "54%"), fig.show="hold"}
...
They recorded, among other information, the body weight of the possums (in g) and their location, including the elevation of the location (in m; `DEM`), as shown in Fig.\ \@ref(fig:PossumPlot) (left panel).
The software output is shown in Fig.\ \@ref(fig:PossumPlot) (right panel).

1. The value of $R^2$ is $23.04$%.
   Determine, and interpret, the value of\ $r$.
1. Write down the regression equation.
1. Determine if there is a relationship between the possum weight and the elevation, using a test of the slope.
...
```{r PossumPlot, fig.cap="The relationship between weight of possums and the elevation of their location. Left: scatterplot. Right: software output.", fig.align="center", fig.width=4.25, fig.height=3.5, out.width=c("45%", "54%"), fig.show="hold"}
...
```{r GorillaPlotCorTest, fig.align = "center", fig.cap = "The chest-beating data. Left: a scatterplot. Right: software output.", fig.width=7, fig.height=3.15, out.width="80%", fig.show="hold"}
...
I restricted the data to cars less than\ $14$ years old at the time, removed one $13$-year-old Corolla advertised for sale for $$390\,000$ (assuming this was an error), then produced the scatterplot in Fig.\ \@ref(fig:CorollasPriceAgeYear) (left panel).
...
14. Compute an approximate $95$% CI for the population slope (use the software output in Fig.\ \@ref(fig:Corollasjamovi)).
15. I could have drawn a scatterplot with Price on the vertical axis and Year of manufacture on the horizontal axis (Fig. \@ref(fig:CorollasPriceAgeYear), right panel).
    For this graph:
...
Height (to the shoulder), which is easier to measure, may be a useful proxy for the mass of the elephant [@lalande2022sex].
Two scatterplots of some relevant data [@lalande2022sexDATA] are shown in Fig.\ \@ref(ElephantPlots).
...
1. For which sex is the height likely to be better for estimating mass? 
   Explain.
1. Use software to compute the regression equation for predicting mass from height, one line for each sex.
