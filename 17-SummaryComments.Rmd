...
* *Do* align numbers in the table by decimal point when possible, for easier reading.
* *Do* construct the table to allow readers to easily make the important comparisons, as far as possible.
* *Do not* use distracting colours and fonts.
* *Do not* use vertical lines (in general), and use *very few* horizontal lines.


## Example {#NHANESSummaryTable}

Studies usually involve many variables, some quantitative and some qualitative.
In addition, numerous comparisons are often of interest.
A table of summary information is often presented, separated by the comparison of most interest.

As an example, consider this RQ:

> Among Americans, is the mean direct HDL cholesterol different for current smokers and non-smokers?

Data to answer this RQ is available from the American NHANES study, which 
...
The direct HDL concentration can be summarised for all respondents, and for smokers and non-smokers (Table\ \@ref(tab:NHANESSummaryQuant)).
Notice that many respondents did not provide their smoking status, and so the total number of respondents with a HDL cholesterol concentration ($n = 8474$) is far less than the number whose smoking status is known ($1388 + 1668 = 3056$).
Similar information can be included in a summary table for other quantitative variables (such as age).

Relationships with qualitative data can also be included (Table\ \@ref(tab:NHANESSummaryQuant)), where the diabetes status is given.
Again, lots of data are missing.
Similar information can be included in a summary table for other quantitative variables (such as sex).


```{r NHANEDescriptivesBySmoking, fig.cap="Output for direct HDL cholesterol concentration and presence of diabetes, by current smoking status", fig.align="center",fig.show="hold", out.width=c('80%', '45%')}
knitr::include_graphics("jamovi/NHANES/NHANES-DirectHDL-Smoke-Explore-Horizontal.png")
knitr::include_graphics("jamovi/NHANES/NHANES-Smoke-Diabetes.png")
```


```{r}
NHANESComparison <- array(dim = c(8, 4))

NHANESComparison[1, ] <- c(NA,
                           "$n$",
                           "Mean",
                           "Std. dev.")
NHANESComparison[2, ] <- c("All",
                           realLength(NHANES$DirectChol),
                           mean(NHANES$DirectChol, na.rm = TRUE),
                           sd(NHANES$DirectChol, na.rm = TRUE) )

NHANESComparison[3, ] <- c("Smokers",
                           realLength(NHANES$DirectChol[NHANES$SmokeNow == "Yes"]),
                           mean(NHANES$DirectChol[NHANES$SmokeNow == "Yes"], na.rm = TRUE),
                           sd(NHANES$DirectChol[NHANES$SmokeNow == "Yes"], na.rm = TRUE) )

NHANESComparison[4, ] <- c("Non-smokers",
                           realLength(NHANES$DirectChol[NHANES$SmokeNow == "No"]),
                           mean(NHANES$DirectChol[NHANES$SmokeNow == "No"], na.rm = TRUE),
                           sd(NHANES$DirectChol[NHANES$SmokeNow == "No"], na.rm = TRUE) )

NHANES.DiabetesTab <- xtabs( ~ NHANES$Diabetes + NHANES$SmokeNow)
NHANES.DiabetesTab.Colsum <- colSums(NHANES.DiabetesTab)

NHANESComparison[5, ] <- c(NA,
                           "$n$",
                           "Percentage",
                           "Odds")

NHANESComparison[6, ] <- c("All",
                           realLength(NHANES$Diabetes),
                           sum(NHANES$Diabetes == "Yes", 
                               na.rm = TRUE)/realLength(NHANES$Diabetes) * 100,
                           NA)
NHANESComparison[7, ] <- c("Smokers",
                           NHANES.DiabetesTab.Colsum[2], 
                           realLength(NHANES$Diabetes[NHANES$SmokeNow == "Yes"])/realLength(NHANES$Diabetes) * 100,
                           NHANES.DiabetesTab[2, 2]/NHANES.DiabetesTab[1, 2])
NHANESComparison[8, ] <- c("Non-smokers",
                           NHANES.DiabetesTab.Colsum[1], 
                           realLength(NHANES$Diabetes[NHANES$SmokeNow == "No"])/realLength(NHANES$Diabetes) * 100,
                           NHANES.DiabetesTab[2, 1]/NHANES.DiabetesTab[1, 1])

if( knitr::is_latex_output() ) {

  kable(surroundMaths(NHANESComparison, 
                      decDigits = c(0, 0, 2, 3)),
        format = "latex",
        longtable = FALSE,
        escape = FALSE,
        #col.names = colnames(NHANESComparison),
        booktabs = TRUE,
        linesep = c("",
                    "",
                    "\\addlinespace"),
        align = "r",
        caption = "The direct HDL cholesterol and presence of diabetes, summarised for smokers and non-smokers. (Many respondents did not provide their smoking status.)") %>%
    kable_styling(font_size = 10) %>%
    row_spec(0, bold = TRUE) %>%
    row_spec(1, italic = TRUE) %>%
    row_spec(5, italic = TRUE) %>%
    pack_rows(  "Direct HDL cholesterol concentration",
                start_row = 1,
                end_row = 4,
                bold = TRUE,
              hline_after = TRUE) %>%
    pack_rows("Presence of diabetes",
              start_row = 5,
              end_row = 8,
              bold = TRUE,
              hline_after = TRUE,
...
Some respondents do not have their HDL concentration recorded either.
:::
...
> Is the percentage of people with diabetes the same for smokers and non-smokers?

The RQ again asks about the *population*.
Again, the RQ cannot be answered with certainty, since *we only have data from one of the many possible samples*, and every sample is likely to be different.
However, the *sample* percentages are clearly different.
Broadly, two possible reasons could explain why the *sample* percentage of diabetics is different for current smokers and non-smokers:

* *the **population** percentages are the same*. 
  The *sample* percentages are *different* simply because of the people selected in this sample.
  Another sample, with different people, might produce different sample percentages
