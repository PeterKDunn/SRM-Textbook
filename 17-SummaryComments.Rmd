...
In this chapter, we make some general comments for constructing graohs and tables.
Always remember:
...
In contrast, a $2$-D graph (a side-by-side bar graph; Fig.\ \@ref(fig:Bar3D2D), right panel) makes it clear whether each age group has more females or more males.\index{Bar chart!side-by-side}\index{Graphs!side-by-side bar chart}
:::
...
In one-dimensional cases (such as dot plots), points can be *stacked*\index{Overplotting!stacking}; see Sect.\ \@ref(DotChartsOneVar).
More generally, points may be jittered\index{Overplotting!jittering} by adding a small amount of randomness to the observations; see Example\ \@ref(exm:Dotchart2DGorillas).
...
This is a problem in graphs where the distance represented visually implies the frequency of those observations, as may be the case for bar charts, dot charts, or histograms.
This is *not* a problem in, for example, scatterplots, where the distance of points from zero do not visually imply any quantity of interest.


::: {.example #VerticalTruncation name="Truncating is not appropriate"}
...
```{r BarchartTruncated1, fig.align="center", fig.width=10, fig.height=4, out.width='95%', fig.cap="The same data presented in two bar charts, without truncating the vertical axis (left) and truncating the vertical axis (right)" }
...
The horizontal axis starts at $35.5$^o^C.
However, this does not create any problems as the *distances* from a temperature of $0$^o^C do not visual imply any quantity of interest.\index{Histogram}\index{Graphs!histogram}
...
```{r HistosTemp, fig.align="center", fig.width=10, fig.height=4, out.width='100%', fig.cap="Two histograms displaying the body temperature of $130$ people. Left: a well-constructed histogram. Right: A poorly-constructed histogram." }
data(BodyTemp)
...
\index{Bar chart}\index{Graphs!bar chart}\index{Pie chart}\index{Graphs!pie chart}


As noted in Sect.\ \@ref(PieCharts), using pie charts may present challenges since they are hard to read: humans compare *lengths* (bar and dot charts) better than *angles* (pie charts) [@data:Friel:Graphs].
...
```{r PieSmallCounts, fig.width=8.5, fig.height=4, out.width="100%", fig.align="center", fig.cap="Data with zero and small counts are easy to see in a bar chart (left panel) and dot chart, but difficult to see in a pie chart (right panel)."}
if( knitr::is_latex_output() ) {
...
2. Researchers measure the difference between each swimmers' fastest $100$\ m time and their fastest $200$\ m time.
