...
In the <span style="font-variant:small-caps;">bhadp</span> study [@data:NHANES3:Data], the age and sex of each participant were recorded.
...
(ref:NHANES) Two plots of the <span style="font-variant:small-caps;">bhadp</span> participants, divided by age group and sex. Left: a three-dimensional bar chart. Right: a side-by-side bar chart.
...
```{r BarchartTruncated1, fig.align="center", fig.width=10, fig.height=3.75, out.width='90%', fig.cap="The same data presented in two bar charts, without truncating the vertical axis (left) and truncating the vertical axis (right)." }
...
However, the horizontal axis starts at\ $35.5$^o^C, which does not create any problems as the *distances* from a temperature of\ $0$^o^C do not visually imply any quantity of interest.\index{Histogram}\index{Graphs!histogram}
...
[*Dataset*: `NHANES`]
Consider the <span style="font-variant:small-caps;">bhadp</span> data again.
