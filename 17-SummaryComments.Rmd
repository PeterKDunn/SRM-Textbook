...
colnames(WAkidsTable) <- c("$n$",
                           ...
                           "$n$",
                           "Summary")
rownames(WAkidsTable) <- c("Age (in years)$^a$",
                           "Household size$^a$",
                           "Under 5s in household$^a$")

quantStuff <- function(x, digits = c(2, 2)){
  c( paste( "$", pad(realLength(x), targetLength = 2), "$"),
     paste( "$", 
            pad( format( round( median(x, na.rm = TRUE), digits[1]), nsmall = digits[1]), targetLength = 5),
            "$\\ $(",
            pad( format( round( IQR(x, na.rm = TRUE), digits[2]), nsmall = digits[2]), targetLength = 5),
            ")$"),
     #
     paste( "$", pad(realLength(x[WAKids$Diarrhea == "N"]), targetLength = 2), "$"),
     paste( "$", 
            pad( format(round( median(x[WAKids$Diarrhea == "N"], na.rm = TRUE), digits[1]), nsmall = digits[1]), targetLength = 5),
            "$\\ $(",
            pad( format( round( IQR(x[WAKids$Diarrhea == "N"], na.rm = TRUE), digits[2]), nsmall = digits[2]), targetLength = 5),
            ")$"),
     #
     paste( "$", pad(realLength(x[WAKids$Diarrhea == "Y"]), targetLength = 2), "$"),
     paste( "$", 
            pad( format( round( median(x[WAKids$Diarrhea == "Y"], na.rm = TRUE), digits[1]),  nsmall = digits[1]), targetLength = 5),
            "$\\ $(",
            pad( format( round( IQR(x[WAKids$Diarrhea == "Y"], na.rm = TRUE), digits[2]), nsmall = digits[2]), targetLength = 5),
            ")$")
  )
}
...
qualStuff <- function(x, digits = c(2, 2)){
  ...
  temp[1:numLevels, 1] <- pad(rowSums(WAtab), targetLength = 2, surroundMaths = TRUE)
  temp[1:numLevels, c(3, 5, 4, 6)] <- array( c( paste("$", pad(WAtab, targetLength = 2), "$"),
                                      paste("$", 
                                            pad( format( round( prop.table(WAtab, 1) * 100, 1), nsmall = 1), targetLength=4),
                                            "$\\%") ),
                                   dim = c(numLevels, 4) )
  temp
}
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(rbind(WAkidsTable,
                             WAkidsTable2) ),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
    ...
    pack_rows( "Has livestock$^b$",
               ...
               end_row = 14,
               escape = FALSE) %>%
    add_footnote(c("Quantitative variables are summarised using medians and IQR", "Qualitative variables are summarised using counts and percentages"), notation = "alphabet")
...
That is, a difference is seen *in the sample*; but the interest is in the *population*.
