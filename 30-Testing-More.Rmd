...
* *Consistency* between the statistic and assumed parameter: computing $P$-values (Sect.\ \@ref(AboutFindingPvalues)).
...
Hypothesis testing always start by assuming that the null hypothesis is true.
...
Null hypotheses always contain an 'equals', because (as part of the [decision making process](#DecisionMaking)), a specific value must be assumed for the population parameter, so we know what we might expect from the sample.
...
The alternative hypothesis proposes that the value of the population parameter really is not the value claimed in the null hypothesis (but does not explain *why*).
...
Under certain circumstances, many sampling distributions have an approximate normal distribution, with a standard deviation called the *standard error*.
...
The sampling distribution describes what values the sample statistic can reasonably be expected to have, if the study was repeated for all possible samples.
Since the sampling distribution of the statistic often has an approximate normal distribution under certain conditions, the observed value of the sample statistic can be expressed as $z$-score or a $t$-score:  
\[
   \text{test statistic} = 
...
\[
   \frac{\text{a specific value} - \text{the mean of the distribution}}
...
As demonstrated in Sect.\ \@ref(OnePropTestP), often $P$-values can be *approximated* by using Tables or the  [68--95--99.7 rule](#def:EmpiricalRule).
...
A $P$-value is the likelihood of observing the value of the sample statistic (or something even more extreme) over repeated sampling, under the assumption that the null hypothesis is true.
...
3. *sample summary statistics* (such as sample means and sample sizes), including a CI (indicating the precision with which the statistic has been estimated).
...
Assuming the tests are statistically valid, use the [68--95--99.7 rule](#def:EmpiricalRule) to approximate the *two*-tailed $P$-value if:
...
Consider again the study to determine the mean body temperature (Chap.\ \@ref(TestOneMean)), where $\bar{x} = 36.8052^{\circ}\text{C}$.
