...
```{r RollDiceCIFig, fig.align="center", fig.width=8.5, out.width="100%", fig.cap="About $95$\\% of CIs contain the population proportion. In the $50$ samples, only three produced a CI that did not straddle $p = 0.5$. In practice, we only have one sample."}
if (knitr::is_latex_output()){
  set.seed(99999)
  num.rolls <- 25
  num.sims <- 50
...
Similarly, if the conditions for computing the confidence interval are not met, we can still perform the mathematical calculations and obtains values... but the results may be suspect. 
:::
...
This means there were $11$\ even rolls, and $14$\ odd rolls.
...
\]
The [statistical validity conditions](#ValidityProportions) should also be checked.
