...
We have seen that the value of the sample statistic often varies between sampleswith a normal distribution (whose standard deviation is called the *standard error*).
...
To better understand the sampling distribution for the proportion of even numbers in $25$ rolls of a die, statistical theory could be used... or thousands of repetitions of a sample of $25$ rolls could be performed... or a computer could *simulate* many samples of $25$ rolls.
...
if (knitr::is_latex_output()){
  ...
  plot( c(1, (num.rolls + 2)), 
        ...
        xlab = "",
        ylab = "",
        main = paste("The results from", num.sims, "samples of", num.rolls, "rolls each"),
        axes = FALSE)
...
In this example, the *population proportion* of even rolls is known to be $p = 0.5$ (using the classical approach to probability, since three of the six faces of the die are even numbers).
...
Suppose a fair die was rolled $25$ times, but this was repeated for *thousands* of samples (not just ten samples as in 
`r if (knitr::is_latex_output()) {
...
These thousands of sample proportions $\hat{p}$, one from every sample of rolls, could be graphed using a
`r if (knitr::is_latex_output()) {
...
The sampling distribution for $\hat{p}$ has:
...
We still don't know *exactly* what we'll find next roll... but we have some idea of *how* the sample proportion is likely to vary in samples of $25$ rolls.
For instance, values of $\hat{p}$ less than $0.2$, or greater than $0.8$ are unlikely to be observed.
...
The variation in $\hat{p}$ from sample to sample is $\text{s.e.}(\hat{p})$.
...
The *the standard error is the standard deviation of the distribution of a sample statistic*, that measures how much a sample estimate varies across all possible samples.
...
For example (Fig.\ \@ref(fig:NormalDieTheory)), about $68$% of the time, a sample of $25$ rolls will have a value of $\hat{p}$ between $0.5$ give-or-take *one* standard deviation (that is, give-or-take $`r round(se.die, 3)`$).
...
*all* ten samples of $25$ rolls (i.e., $100$%) had a sample proportion between $0.30$ and $0.70$.)
...
Since the sampling distribution has an approximate normal distribution (Def.\ \@ref(def:DEFSamplingDistributionPhat)), the $68$--$95$--$99.7$ rule can be used to compute the approximate give-or-take amount (using the ideas in Sect.\ \@ref(CIpKnownp)): the give-or-take amount, called the *margin of error*, is $\left(\text{multiplier}\times\text{s.e.}(\hat{p})\right)$.
This means that the interval would, more precisely, be  
\[
...
This confidence interval is an interval containing values of $p$ that could have reasonably produced the observed value of $\hat{p}$ (Fig.\ \@ref(fig:pProducingpHat)), with $95$% confidence.
...
```{r RollDiceCIFig, fig.align="center", fig.width=8.5, out.width="100%", fig.cap="About $95$\\% of CIs contain the population proportion. In the $50$ sampples, only three produced a CI that did not straddle $p = 0.5$. In practice, we only have one sample."}
if (knitr::is_latex_output()){
  set.seed(99999)
  num.rolls <- 25
  num.sims <- 50
...
Almost always, we ony have one sample.
:::


A confidence interval is an interval containing values of $p$ that could have reasonably produced the observed value of $\hat{p}$ (Fig.\ \@ref(fig:pProducingpHat)).
...
However, certain conditions must be true for the sampling distribution to be approximately normally distributed, or *statistically valid*.
...
If this condition is not met, the normal distribution may not approximate the sampling distribution well, so the $68$--$95$--$99.7$ rule may be inappropriate, and so the CI may also be slightly wrong.
...
For the die-throwing example in Sect.\ \@ref(SamplingDistributionUnknownp), $n = 25$ and $\hat{p} = 0.44$.
This means there were $11$\ even rolls, and $14$\ odds rolls.
...
\[
  \text{s.e.}(\hat{p})
               ...
               = 0.01977.
\]
An *approximate* $95$% CI is $0.16944 \pm (2 \times 0.01977)$, or $0.16944 \pm 0.03954$ (i.e., the *margin of error* is $0.03954$).
...
1. Find an approximate $95$% CI for the proportion of patients who receive pain medication.
1. Check if the statistical validity conditions are met or not.
