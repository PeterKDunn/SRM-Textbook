...
* *Consistency* between the statistic and parameter values: computing $P$-values (Sect.\ \@ref(AboutFindingPvalues)).
...
Null hypotheses always contain an 'equals', because (as part of the [decision making process](#DecisionMaking)), a specific value must be assumed for the population parameter, so we can describe what we might expect from the sample.
...
This normal distribution is the basis for computing $P$-values.
...
The sampling distribution describes what values the sample statistic can take over all possible samples of given size.
...
As demonstrated in Sect.\ \@ref(OnePropTestP), when the sampling distribution is a normal distribution, $P$-values can be *approximated* using Tables or the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule).
...
* A $P$-value *does not prove* anything (only one of many possible samples was studied).
...
In this context, a weight loss of at least $2.5$\ kg was considered to be of practical importance by the researchers.
:::
...
These conditions ensure that the mathematics behind computing the $P$-value is sound.
For instance, if the sampling distribution has an approximate normal distribution, the statistical validity conditions ensure that the approximation is sufficiently close to a normal distribution for the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule) rule to apply^[Not all sample statistics have normal distributions, but all the sample statistics in this book are either normally distributed or are closely related to normal distributions.].
...
These statistical validity conditions are usually (but not always) the same as for CIs (Sect.\ \@ref(ValidityCIs)).
...
however, the sample may not represent the more general population that is probably intended.
