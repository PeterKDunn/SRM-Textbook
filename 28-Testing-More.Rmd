...
Hypothesising, for example, that the *sample* mean body temperature (in Chap.\ \@ref(TestOneMean)) is equal to\ $37.0$^o^C is silly: the *sample* mean clearly is\ $36.8052$^o^C for the sample available, and its value will vary from sample to sample anyway.
...
3. *sample summary statistics* (such as sample means, with confidence intervals, and sample sizes).
...
## More details about practical importance and statistical significance {#PracticalSignificance}
...
**5.** True.
**6.** False.
**7.** c.\ There is no parameter given; perhaps $H_0$: $\mu = 37$.
:::
`r if (knitr::is_html_output()) '-->'`
