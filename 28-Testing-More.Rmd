...
A $t$-score and $z$-score both measure the number of standard deviations that a value is from the mean:
$$
...
Hypothesis testing concerns making a decision about a *population* using *sample* information.
...
In medical contexts, the terms *sensitivity* and *specificity* are often used rather than the terms *Type\ I* and *Type\ II errors*.
...
Suppose we test if the mean internal body temperature remains at $37.0$^o^C (Chap.\ \@ref(TestOneMean)).
Suppose we found *no evidence* that the internal body temperature had changed.
...
1. The alternative hypothesis is one-tailed if the sample statistic is larger than the hypothesised population mean.  
`r if( knitr::is_html_output() ) { torf( answer=FALSE )}`
...
1. Both $z$-scores and $t$-scores are test statistics.  
`r if( knitr::is_html_output() ) { torf( answer=TRUE )}`
