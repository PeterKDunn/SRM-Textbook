...
# Hypothesis tests for one mean {#TestOneMean}
...
include_graphics("Illustrations/pexels-anna-shvets-3987141.jpg")
```
`r if (knitr::is_latex_output()) '-->'`


The average internal body temperature is commonly believed to be $\mu= 37.0^\circ\text{C}$,
a guideline based on data over 150 years old
...
These two possible explanations are called *hypotheses*.\index{hypotheses}
More formally,
the two hypotheses above are:
...
subBodyT <- round(cbind(bodyt$BodyTempC[1:10], bodyt$BodyTempC[121:130] ), 2)
...
if( knitr::is_latex_output() ) {
  kable( subBodyT[1:10, ],
         ...
         booktabs = TRUE,
         longtable=FALSE,
         col.names=c("Gender", "Body temp (deg. C)"),
...
```{lemma ValuesXbar, name="Values of $\\bar{x}$"}
Given the sampling distribution shown in 
Fig. \@ref(fig:BodyTempSamplingDist),
use the [68--95--99.7 rule](#def:EmpiricalRule) to determine
how often will $\bar{x}$ be **larger** than 37.036 degrees C 
just because of sampling variation, 
if $\mu$ really is $37^\circ$C.
```
...
## The test statistic and $t$-scores: Observation {#Tscores}
...
#abline(v=z36)
arrows(z36, 0.1, z36, 0, length=0.15, angle=15)
text(z36, 0.1, "36.8 deg.",pos=4)
```
...
## $P$-values: Consistency with assumption? {#Pvalues}

This is [the decision-making progress](#DecisionMaking) 
so far:
...
```{lemma, Pvals, name="$P$-values"}
What do you think the $P$-value will be for $t=-5.45$
(using Fig. \@ref(fig:BodyTempSamplingDistT))?
```
...
## Hypothesis testing: A summary {#TestSummary}
...
<iframe src="https://h5p.org/h5p/embed/497798" width="1090" height="412" frameborder="0" allowfullscreen="allowfullscreen"></iframe><script src="https://h5p.org/sites/all/modules/h5p/library/js/h5p-resizer.js" charset="UTF-8"></script>
```
...
is statistically valid if the sample is somewhat representative,
...
```{block2, type="rmdthinkHTML"}
<iframe src="https://h5p.org/h5p/embed/488117" width="1090" height="403" frameborder="0" allowfullscreen="allowfullscreen"></iframe><script src="https://h5p.org/sites/all/modules/h5p/library/js/h5p-resizer.js" charset="UTF-8"></script>
```
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "30%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-anna-shvets-3846157.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
The hypotheses (**Step 1: Assumption**) 
...
RTimeArray <- array( c(RTime$RecTime, NA),  dim=c(2, 9))
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/ZbJ58Ag22Mw" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
A study 
[@imtiaz2017assessment]
compared the nutritional intake
of $n=50$ anaemic infants in Lahore (Pakistan)
with the amounts recommended.

The mean daily protein intake in the sample
was 14g, with a standard deviation of 3g.
...
`r if( knitr::is_html_output() ) {fitb(num=TRUE, tol=0.0001, answer=0.4243)}`
...
`r if( knitr::is_html_output() ) {mcq( c(
  ...
  "The population mean is 14g")  )}`
1. The test statistic (to two decimal places) is
`r if( knitr::is_html_output() ) {fitb(num=TRUE, tol=0.01, answer=2.35)}`
...
colnames(DataAndTargets) <- c("High level (Inst. 1)", "Mid level (Inst. 1)", "High level (Inst. 2)", "Mid level (Inst. 2)")
```
