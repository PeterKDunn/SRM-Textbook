...
$$
   \text{$H_1$:\ } \mu \ne 37.0.
$$
Now, we determine if evidence exists to contradict the assumption specified in $H_0$.
...
When the *population* standard deviation\ $\sigma$ is *known*, the value of the standard error happens to be (see Eq.\ \@ref(eq:stderrorxbarCI))
$$
...
The numerical summary, using software (Fig.\ \@ref(fig:BodyTempjamovi)), gives:
...
However, it is called a $t$-score.
...
Since $t$-scores and $z$-scores are very similar, the $P$-value can be *approximated* using the $68$--$95$--$99.7$ rule and a diagram (Sect.\ \@ref(ApproxP)), or *approximated* using $z$-tables 
(`r if ( knitr::is_html_output()) { 
  'Appendix\\ \\@ref(ZTablesOnline).'} else {
  'Appendices\\ \\@ref(ZTablesNEG) and\\ \\@ref(ZTablesPOS)'}`
).
Commonly, however, software is used to compute the $P$-value.
...
Usually, software is used to determine precise $P$-values for $t$-scores (Fig.\ \@ref(fig:BodyTempTestjamovi)).
...
* Some *sample summary information*: the sample mean (with the CI) and the sample size.
...
The sample is not a true random sample from the population of all GU students (the students are mostly first-year students enrolled in undergraduate psychological science). 
However, these students may be  somewhat representative of all GU student.
...
If we accept that this applies for university students too (and we do not know if it does), the standard error is $\text{s.e.}(\bar{x}) = \sigma/\sqrt{n} = 15/\sqrt{130} = 1.0022$, and the test-statistic is then a $z$-score:
$$
...
1. Use the software output (Fig.\ \@ref(fig:CherryRipes201720182019)) to perform a hypothesis test to determine if the mean weight is\ $14$\gs or not.
