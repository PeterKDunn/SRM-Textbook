...
Since then, the methods for measuring internal body temperature has changed substantially:
...
For this reason, the internal body temperature (recorded by newer instruments, in different locations) may have changed since the 1860s.
...
A *sample* of people can be taken to determine whether or not there is evidence that the *population* mean internal body temperature is $37.0$^o^C.\index{Mean!population}
...
This does not specifically ask if $\mu$ is smaller than  $37.0$^o^C, or larger than  $37.0$^o^C.
This means the alternative hypothesis is two-tailed:
$$
   \text{$H_1$:\ } \mu \ne 37.0.
$$
Now, we determine if compelling evidence exists to contradict the assumption specified in $H_0$.
...
**Step\ 2** of the decision-making process is to describe what values of the statistic (in this case, the sample mean $\bar{x}$) can be expected if the value of $\mu$ is assumed to $37.0$ (the value specified in $H_0$).
In other word, the *sampling distribution* of $\bar{x}$ needs to be described. 

The sample mean *varies* from sample to sample, and varies with a normal distribution (whose standard deviation is called the *standard error*) under certain conditions (Sect.\ \@ref(ValiditySampleMeanTest)).
...
$$
However, the value of the *population* standard deviation is almost never known.
In almost every case, the population standard deviation\ $\sigma$ is replaced by the best estimate available: the *sample* standard deviation\ $s$.
Then, the value of the standard error happens to be
$$
  \text{s.e.}(\bar{x}) = \frac{s}{\sqrt{n}}.
$$


::: {.definition #DEFSamplingDistributionXbarHT name="Sampling distribution of a sample mean for $\sigma$ unknown"}
When the *population* standard deviation is unknown, the *sampling distribution of the sample mean* is (when certain conditions are met; Sect.\ \@ref(ValiditySampleMeanTest)) described by:
...
Fortunately, @data:mackowiak:bodytemp gathered body-temperature data, which was collated by @data:Shoemaker1996:Temperature
`r if (knitr::is_latex_output()) {
   '(Table\\ \\@ref(tab:DataBodyTemp)).'
} else {
   '(Fig.\\ \\@ref(fig:DataBodyTemp)).'
}`
A graphical summary (Fig.\ \@ref(fig:BodyTempHist)) and a numerical summary, using software (Fig.\ \@ref(fig:BodyTempjamovi)), shows that:

* the *sample* mean is $\bar{x} = 36.8052$^o^C,\index{Mean!sample}
* the *sample* standard deviation is $s = 0.4073$^o^C, and
...
\captionof{table}{The body temperature data: the first twenty of the 130 ordered observations\label{tab:DataBodyTemp}.}
...
```{r BodyTempjamovi, fig.cap="The software output summary for the body temperature data.", fig.align="center", fig.height=3.0, out.width='65%'}
...
```{r BodyTempSamplingDist, fig.cap="The distribution of sample mean body temperatures, if the population mean is $37.0^\\circ$C and $n = 130$. The grey vertical lines are\\ $1$,\\ $2$ and\\ $3$ standard deviations from the mean.", fig.align="center", fig.height=2.75, fig.width=9, out.width='100%'}
mn <- 37.0
...
\index{Hypothesis testing!one mean}\index{Test statistic!t@$t$-score}


**Step\ 3** in the decision-making process is to make the observations.
The data have already been provided in 
`r if (knitr::is_latex_output()) {
   '(Table\\ \\@ref(tab:DataBodyTemp)),'
} else {
   '(Fig.\\ \\@ref(fig:DataBodyTemp)),'
}`
where $\bar{x} = 36.8052$ (Fig.\ \@ref(fig:BodyTempjamovi)).
Trying to locate this value on the sampling distribution (Fig.\ \@ref(fig:BodyTempSamplingDistT)) shows that this observed sample mean is, relatively speaking, *extremely* small: a sample mean this low is very unlikely to occur in any sample of $n = 130$ when $\mu = 37.0$.
...
This is compelling evidence that $\mu$ is not $37.0$.
...
Since $t$-scores and $z$-scores are very similar, the $P$-value can be *approximated* using the $68$--$95$--$99.7$ rule and a diagram (Sect.\ \@ref(ApproxP)), or using tables 
(`r if ( knitr::is_html_output()) { 
  'Appendix\\ \\@ref(ZTablesOnline).'} else {
  'Appendices\\ \\@ref(ZTablesNEG) and\\ \\@ref(ZTablesPOS)'}`
).
Commonly, however, software is used to compute the $P$-value (Sect.\ \@ref(SoftwareP)).
...
Since $t$-scores are similar to $z$-scores, the ideas in Sect.\ \@ref(TestpObsDecisionPvalues) can be used to *approximate* a $P$-value for a $t$-score.
...
That is, there is compelling evidence that the *population* mean body temperature is *not*\ $37.0$^o^C.
...
Communicating the results of any hypothesis test requires an *answer to the RQ*, a summary of the *evidence* used to reach that conclusion (such as the $t$-score and $P$-value, stating if it is a one- or two-tailed $P$-value), and some *sample summary information*.
For the body-temperature example, write:

> The sample provides very strong evidence ($t = -5.45$; two-tailed $P < 0.001$) that the population mean body temperature is *not*\ $37.0$^o^C ($\bar{x} = 36.81$; $n = 130$).
...
* Some *sample summary information*: $\bar{x} = 36.81$; $n = 130$.

The test is about the *mean* internal body temperature; *individual* people in the data have internal body temperatures ranging from\ $35.722$^o^C to\ $38.222$^o^C.\index{Range}
The difference between the value of $37.0$^o^C and the sample mean of $36.81$^o^C is quite small in absolute terms, and is probably of little practical importance.\index{Practical importance}
...
(The data, as displayed in Fig.\ \@ref(fig:BodyTempHist), do *not* need come from a population with a normal distribution.)
:::
...
Since the alternative hypothesis is *one-tailed*, and specifically asking if $\mu > 100$, the $P$-value is the area in the right-side tail of the distribution only (Fig.\ \@ref(fig:IQSamplingDistribution)); it will be extremely small.
This is compelling evidence to change our minds and support the alternative hypothesis.
...
text( x = xbar, 
      y = 0.9 * max(out$y), 
      expression( paste(italic(z) == "11.78")),
      pos = 3)
...
> Very strong evidence exists in the sample ($t = 11.78$; one-tailed $P < 0.001$) that the population mean IQ in students at Griffith University is greater than\ $100$ (mean $111.19$; $n = 224$).
...
If we accept that this applies for university students too (we do not know if it does), the standard error is $\text{s.e.}(\bar{x}) = \sigma/\sqrt{n} = 15/\sqrt{130} = 1.0022$, and the test-statistic is then a $z$-score:
$$
...
`r if( knitr::is_html_output() ) {longmcq( c(
  "The sample mean headway is 1.9s",
  "The sample mean is 1.1915s",
  "The population mean is 1.9s",
  answer = "The population mean is less than 1.9s",
  "The population mean is greater than 1.9s",
  "The population mean is 1.1915s")  )}`
...
`r if( knitr::is_html_output() ) {mcq( c(
  ...
  "0.05",
  "0.34")  )}`
1. True or false? There is no evidence to accept the *alternative* hypothesis (that the mean headway is less than $1.9$\secs).
`r if( knitr::is_html_output() ) {torf(answer=TRUE)}`
:::
...
Many dental associations recommend brushing teeth for *at least* two minutes. 
@data:Macgregor1979:BrushingDurationKids studied the brushing time for $85$ uninstructed school children from England ($11$ to\ $13$-years-old) and found the mean brushing time was\ $60.3$\secs, with a standard deviation of\ $23.8$\secs.
...
Health departments recommend that hot water be stored at $60$^o^C or higher, to kill *legionella* bacteria (for example, 
`r if (knitr::is_latex_output()) {
...
They found the mean temperature was\ $56.6$^o^C, with a standard error of\ $0.4$^o^C.\spacex
...
1. Use the software output (Fig.\ \@ref(fig:CherryRipes201720182019)) to perform a hypothesis test to determine if the mean weight really is\ $14$\gs or not.
...
When the actual amount of blood spilt on concrete was\ $\,1000$\mLs, the mean guess was\ $846.4$\mLs (with a standard deviation of\ $651.1$\mLs).

Is there evidence that the mean guess really is\ $1\,000$\mLs (the true amount)?
...
[*Dataset*: `BloodLoss`]
@feng2017application assessed the accuracy of two instruments from a clinical laboratory, by comparing the reported luteotropichormone (LH) concentrations to known, pre-determined values (Table\ \@ref(tab:QualityControlDataSummary)).
...
**1.** True.
**2.** $0.0436$.
**3** The population mean headway is\ $1.9$\secs.
**4** The population mean headway is *less than*\ $1.9$\secs.
**5.** $0.34$.
**6.** Big
