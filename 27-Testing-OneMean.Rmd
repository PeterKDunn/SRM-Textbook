...
This shows that, for example, the value of\ $\bar{x}$ will be *larger* than\ $37.0357$^o^C about\ $16$% of the time (using the $68$--$95$--$99.7$ rule) if\ $\mu$ really is\ $37.0$.
...
In general, when the sampling distribution has an approximate normal distribution and the *sample* standard deviation is used to compute the standard error, a $t$-score in hypothesis testing is 
\begin{equation}
   t 
   = 
...
To answer this RQ, @reilly2022gender studied $n = 224$ students at Griffith University (GU), and found the sample mean IQ was\ $111.19$, with a standard deviation of\ $14.21$. 
...
if( knitr::is_latex_output() ) {
  ...
  T1info <- rbind( head(LHconc[1:2], 5),
                   c("$\\vdots$", "$\\vdots$"),
  ...
  T2info <- rbind( head(LHconc[, 3:4], 5),
                   c("$\\vdots$", "$\\vdots$"),
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "The quality-control data: LH levels (in mIU/mL) for two instruments (only the first five of $36$\\ observations shown).") %>% 
    kable_styling(font_size = 8)
