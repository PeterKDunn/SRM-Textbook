...
```{r BodyTempSamplingDist, fig.cap="The distribution of sample mean body temperatures, if the population mean is $37.0^\\circ$C and $n = 130$. The grey vertical lines are\\ $1$,\\ $2$ and\\ $3$ standard deviations from the mean.", fig.align="center", fig.height=3.25, fig.width=9, out.width='95%'}
mn <- 37.0
...
se <- s/sqrt(n)
par(mar = c(3, 3, 1, 0.15))

out <- plotNormal(mn, 
                  s / sqrt(n),
                  #xlab = expression(Sample~means~from~samples~of~size~130),
                  round.dec = 4,
                  ylim = c(0, 18.25),
                  cex.tickmarks = 0.9)
...
```{r BodyTempSamplingDistT, fig.cap="The sample mean of $\\bar{x} = 36.8041^\\circ$C is very unlikely to be observed in any sample of size $n = 130$, if $\\mu = 37.0^\\circ$C.\\spacex The standard deviation of the distribution is $\\text{s.e.}(\\bar{x}) = 0.035724$.", fig.align="center", fig.width=8, fig.height=2.25, out.width='85%'}
mn <- 37.0
#These taken from Shoemaker's JSE data file
s <- 0.40732
n <- 130
...
out <- plotNormal(mn,
                  s / sqrt(n),
                  xlab = expression(Sample~mean~temperatures~"(in "*{}^o*"C)"),
                  round.dec = 3,
...
```{r BodyTempTestjamovi, fig.cap="Software output for conducting the $t$-test for the body temperature data.", fig.align="center", out.width="65%"}
...
```{r DecisionFlowTemps, fig.cap = "The decison-making process for the body-temperature data.", fig.align="center", out.width='100%', fig.width = 9.5, fig.height = 4}  
source("R/showDecisionMaking.R") 
showDecisionMaking(populationText = expression( atop(bold(Assumption)~about,
                                                     population*":"~italic(H)[0]*":"~mu==37.0) ),
                   expectationText = expression(atop(bar(italic(x))~has~normal~distn*";",
                                                     mean==37.0*";"~s.e.==0.0357) ),
                   oneSampleText = expression( atop(Sample,
                                                    italic(n)==130) ),
                   compareText = expression(atop(Assess~consistency*":",
                                                 italic(P)*"-"*value~bold(very)~small)),
                   oneStatisticText =  expression( bar(italic(x))==36.8052),
                   contradictText = expression(atop(Data~inconsistent,
                                                    with~assumption)),
                   supportText = expression(atop(Data~consistent,
                                                 with~assumption)), 
                   Decision = "Reject"     
)
```
...
```{r IQSamplingDistribution, fig.cap="The sampling distribution for the IQ data. The RQ is one-tailed so the $P$-value is the area in one tail.", fig.align="center", fig.width=9, fig.height=3, out.width='95%'}
mn.IQ <- 100
sd.IQ <- 14.21
sigma.IQ <- 15
n.IQ <- 100
...
out <- plotNormal(mn.IQ,
                  ...
                  round.dec = 2) 
# xlab, placed better
mtext(text = expression(Sample~mean~IQs),
      at = 107,
      side = 1,
      line = 1)
shadeNormal(out$x,
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "The quality-control data: LH levels (in mIU.mL$^{-1}$) for two instruments (only the first four of $36$\\ observations shown).") %>% 
    kable_styling(font_size = 8)
