...
A numerical summary, jamovi (Fig.\ \@ref(fig:BodyTempjamovi)), shows that:
...
Given the sampling distribution (Fig.\ \@ref(fig:BodyTempSamplingDist)), use the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule) to determine how often will $\bar{x}$ be *larger* than $37.0357^\circ$C, if $\mu$ really is $37$.
...
The sample size of $25$ is a rough figure here; some books give other values (such as $30$).
...
- $H_0$: $\mu = 100$ ($\mu$ equals $100$, but $\bar{x}$ is not $100$ due to sampling variation);
...
The *sample* mean is higher than $100$, but sample mean vary.
...
If we accept that this applies for university students too (we do know for for certain), the standard error becomes $\text{s.e.} = \sigma/\sqrt{n} = 15/\sqrt{130} = 1.0022$, and the test-statistic is  
\[
...
In traffic, the usual engineering recommendation is that the safe gap between travelling vehicles (a 'headway') is *at least* $1.9$\ s (often conveniently rounded to $2$\ s).
