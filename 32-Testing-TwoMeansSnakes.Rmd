...
> Morphological convergence in a Mexican garter snake associated with the ingestion of a novel prey Javier Manjarrez, Constantino Macías Garcia, Hugh Drummond
> https://datadryad.org/stash/dataset/doi:10.5061/dryad.mg152

Snakes which consume a range of prey may present intermediate morphology, such as that of Thamnophiine (Natricidae) which may be classified morphometrically across the soft-hard prey dietary boundary. In this study, we compared the dentition and head structure of populations of Thamnophis melanogaster that have entered the arthropod-crustacean (crayfish)-eating niche and those that have not, and tested for convergence between the former and two distantly related crayfish specialists of the genus Regina (R. septemvittata and R. grahamii). As a control, we included the congener T. eques. Multivariate analysis of jaw length, head length, head width, and number of maxillary teeth yielded three significant canonical variables that together explained 98.8 % of the variance in the size-corrected morphological data. The first canonical variable significantly discriminated between the three species. The results show that head 

Some Mexican garter snakes (*Thamnophis melanogaster*) have been observed eating crayfish, while some live in habitats with no crayfish.
Researchers were interested in whether the snakes in these two regions were different:

> For females Mexican garter snakes, is the snout-to-vent (SVL) different for those in regions with and without crayfish?

Two different groups if crayfish must be studied, so the data are *not paired*.
...
read.csv("Data/Snakes.csv")

SN <- read.csv("Data/Snakes.csv")
# Keep it small: Take vars of interest only


# Extract the two species of interest
SPtable <- table(SN$SPECIES)
SNsub <- subset(SN,
...
t.test( SVL..cm. ~ SPECIES, data = SNF)
tapply(SNF$SVL..cm., list(SNF$SPECIES), mean, na.rm=TRUE)
tapply(SNF$SVL..cm., list(SNF$SPECIES), sd, na.rm=TRUE)
tapply(SNF$SVL..cm., list(SNF$SPECIES), findStdError, na.rm=TRUE)
tapply(SNF$SVL..cm., list(SNF$SPECIES), realLength)
...
tb1 <- c( na.omit( SNF$SVL..cm.[ SNF$SPECIES == names(SPtable)[4] ] ) )
tb2 <- c( na.omit( SNF$SVL..cm.[ SNF$SPECIES == names(SPtable)[5] ] ) )
...
For these data, the differences are computed as the mean SVL for snakes eating crayfish ($C$), *minus* the SVL for snakes not eating crayfish ($N$): $\mu_C - \mu_N$.
By this definition, the differences refer to how much larger (on average) the SVL is for ???.
The parameter is $\mu_C - \mu_N$, the difference between the population SVL (using ??, minus *not* using ???).


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Here the difference is computed as the mean SVL for ?? ,  *minus* the mean SVL for ??.
Computing the difference as the mean SVL for ??, *minus* the mean SVL for ? is also correct.
...
Hence, the null hypothesis is that there is 'no difference' between the population means SVL of the two groups:
...
SN.summary[1:2, 1] <- aggregate(SVL..cm. ~ SPECIES, 
                                data = SNF, 
                                FUN = "mean")[, 2]
SN.summary[1:2, 2] <- aggregate(SVL..cm. ~ SPECIES, 
                                data = SNF, 
                                FUN = "sd")[, 2]
SN.summary[1:2, 3] <- aggregate(SVL..cm. ~ SPECIES, 
                                data = SNF, 
                                FUN = "realLength")[, 2]
SN.summary[1:2, 4] <- aggregate(SVL..cm. ~ SPECIES, 
                                data = SNF, 
                                FUN = "findStdError")[, 2]

SN.summary[3, 1] <- SN.summary[1, 1] - SN.summary[2, 1]
SN.summary[3, 4] <- sqrt( SN.summary[1, 3]^2/SN.summary[1, 2] +
                                 SN.summary[2, 3]^2/SN.summary[2, 2]  )

SN.summary[, 1] <- round( SN.summary[, 1], 
                                  2)
SN.summary[, 2] <- round( SN.summary[, 2], 
                                  0)
SN.summary[, 3] <- round( SN.summary[, 3], 
                                  2)
SN.summary[, 4] <- round( SN.summary[, 4], 
                                  3)

rownames(SN.summary) <- c("Eating crayfish", 
                                 "Not eating crayfish", 
                                 "???")

if( knitr::is_latex_output() ) {
  kable(surroundMaths(SN.summary,
                      decDigits = c(2, 0, 2, 3)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
        ...
        col.names = c("Mean", 
                      ...
                      "Standard deviation", 
                      "Standard error"),
        caption = "Numerical summaries of SVL (in cm for the snake data") %>%
   row_spec(0, bold = TRUE) %>%
   kable_styling(font_size = 10) 
}
if( knitr::is_html_output() ) {
...
The jamovi output (Fig.\ \@ref(fig:ReactionPhoneSummaryTestjamovi) can be used, but the $t$-score can also be computed manually:  
\begin{align*}
   t
  &= 
...
This is confirmed by the software (Fig.\ \@ref(fig:ReactionPhoneSummaryTestjamovi)): the two-tailed $P$-value is $0.0011$.
...
At one site studied (Ningxuan Freeway), speeds were recorded for $38$ vehicles before the extra signage was added, and then for $41$ vehicles after the extra signage was added (data with Sect.\ \@ref(SpeedSignage)).
...
Using the summary information in Table\ \@ref(tab:BHADPSummaryTest), the $t$-score is computed using  Eq.\ \@ref(eq:tscore):  
\[
  t 
...
In addition, the standard error of this difference is $\text{s.e.}(\bar{x}_F - \bar{x}_M) = 0.0929$.
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(ParaSummary,
                      decDigits = 0),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
