...
Unforeseen complications are not unusual, so often a *pilot study* (or a *practice run*) is conducted before the real data collection, to:\index{Pilot study}
...
2. how information is collected from the individuals (i.e., internal validity).
3. the analyses and software (including version) used.\index{Software}
...
The researchers also described *how the data was obtained* (p.\ 5):
...
* *Avoid leading questions*, which may lead respondents to answer a certain way.
  Imprecise question wording is the usual reason for leading questions.\index{Leading questions}
* *Avoid ambiguity*:
...
* *Ensure options are mutually exhaustive*, so answers fit into only one category.\index{Mutually exclusive}
...
* *Response bias* (Sect. \@ref(SelectionBias)):\index{Bias!response}
  ...
  Sometimes, respondents repeatedly provide the same answer to a series of multichoice questions.
