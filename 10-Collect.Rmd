...
include_graphics("Pics/iconmonstr-task-1-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
* **Avoid problems with confidentiality**, 
  ...
  Ethics committees usually look very carefully for question that are unethical.
...
```{lemma SurveyQns, name="Survey questions"}
Consider this survey question:
...
```{example MutuallyExclusive Qns, name="Miutually exclusive options"}
Consider this survey question
...
```{r echo=FALSE}
SurveyQ <- "There are two questions; it is *double-barrelled*. Better is to ask the two questions separately, one about *students* and one about *young professionals*. This allows us to separate the two components of the original survey question."
if( knitr::is_latex_output() ) {
SurveyQ <- foldLaTeXText
}
```

(ref:SurveyQr) `r SurveyQ`
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/G0ZZJXw4MTA" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
'Do have bromodosis?' (Possible answers are: Yes/No)
...
<!-- ####################### REVIEW QUESTIONS ##################################### -->
```{r, child = if (knitr::is_html_output()) './children/ReviewQuestions/ReviewQuestions-Chapter10.Rmd'}
```
