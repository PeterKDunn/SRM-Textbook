...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.5, message=FALSE, dev=if (is_latex_output()){"png"}else{"pdf"}}
...
```{r R2Plots, echo=FALSE, warning=FALSE, animation.hook="gifski", cache=TRUE, interval=0.5, dev=if (is_latex_output()){"png"}else{"pdf"}}
if (knitr::is_html_output()){
  set.seed(12345)
    library(MASS)
    num.obs <- 120
