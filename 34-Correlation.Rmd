...
The Pearson correlation coefficient only make sense 
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.5, message=FALSE}
  ...
  set.seed(12345)
    ...
    for (i in 1:num.plots){
      out <- MASS::mvrnorm(num.obs, 
                           mu = c(mean(xx), 10), 
                           Sigma = matrix(c(1, rho.list[i] , rho.list[i], 1), ncol = 2), 
                           empirical = TRUE)
      ...
      abline( coef(lm(out[,2] ~ out[,1]) ), lwd=2, col="grey")
    }
  }
```
...
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/CorrelationMovie.html", frameborder=0, height=480, width=350)  -->
<!-- ``` -->
...
In Plot A,
the correlation coefficient will be *positive*, and reasonably close to one.
In Plot B,
the correlation coefficient will be *negative*, but not that close to $-1$.
In Plot F,
the correlation coefficient will close to zero.
...
```{lemma Estimater, name="Estimate $r$"}
...
```{r echo=FALSE}
EstimateR <- "$r$ will be a positive number (since the scatterplot shows a positive linear relationship), and its value will be close to 1 as the relationship looks very strong."
if( knitr::is_latex_output() ) {
EstimateR <- foldLaTeXText
}
```

(ref:EstimateRr) `r EstimateR`
...
```{lemma Estimater2, name="Guess the value of $r$"}
...
```{r echo=FALSE}
GuessR <- "Not much relationship: the mean of the direct HDL cholesterol concentration is similar for any age. Perhaps describe the scatterplot as 'little relationship'. We cannot make good guess about the value of $r$, but it will be near zero."
if( knitr::is_latex_output() ) {
GuessR <- foldLaTeXText
}
```

(ref:GuessRr) `r GuessR`

```{block2, type="fold"}
(ref:GuessRr)
```

The web page [http://guessthecorrelation.com](http://guessthecorrelation.com) makes a game out of trying to guess the correlation!
...
if you enter `-0.5^2`, you will get `-0.25` in return.
This is correct, because the calculator interprets your input as meaning
`-(0.25^2)`.

What you need to do is to enter 
`(-0,5)^2`.
...
```{r R2Plots, echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.5}
if (knitr::is_html_output()){
  set.seed(12345)
    library(MASS)
    num.obs <- 120
    
    rho2.list <- seq(0.1, 1, by = 0.1)
    ...
    for (i in 1:num.plots){
      out <- mvrnorm(num.obs, mu = c(mean(xx), 10), Sigma = matrix(c(1, rho.list[i] , rho.list[i], 1), ncol = 2), empirical = TRUE)
...
<!-- htmltools::tags$iframe(title = "My embedded document", src = "./Animations/R2Movie.html", frameborder=0, height=480, width=350)  -->
<!-- ``` -->
...
```{lemma, InterpretR2, name="Interpreting $R^2$"}
...
```{block2, type="fold"}
$R^2 = (0.532)^2 = 0.283$:
about 28.3% of the variation in systolic BP
is due to age;
...
include_graphics("Illustrations/pexels-lisa-fotios-3264779.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/ZHwFJkpokMk" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
