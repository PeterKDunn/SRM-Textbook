...
Since the sampling distribution of the statistic often has an approximate normal distribution under certain conditions, the observed value of the sample statistic can be expressed as $z$-score or a $t$-score:\index{test statistic}  
\[
   \text{test statistic} = 
...
As demonstrated in Sect.\ \@ref(OnePropTestP), $P$-values can be *approximated* using Tables or the [$68$--$95$--$99.7$ rule](#def:EmpiricalRule) when the sampling distribution has a normal distribution.
...
* A small $P$-value *does not* mean the null hypothesis $H_0$ is false, or that $H_0$ is true.
...
## About making mistake in testing {#TypeErrors}
...
Two ways exist to make a mistake:

* *Incorrectly* concluding that that evidence supports the alternative hypothesis.
  ...
  This is called a *false positive*, or a *Type\ I error*.\index{Type\ I error}
* *Incorrectly* concluding that there is *no* evidence to support the alternative hypothesis.
  ...
  This is called a *false negative*, or a *Type\ II error*.\index{Type\ II error}

Ideally, neither of these errors would be made; however, sampling means that neither error can ever be completely eliminated.
...
A Type\ I error is often seen as analogous to declaring an innocent person as guilty.
Similarly, a Type\ II error is often seen as analogous to declaring a guilty person as innocent.
The law generally sees a Type\ I error as more grievous that a Type\ I error, just as in research.
...
This means that the difference in weight loss between placebo and intervention was unlikely to be explained by chance ($P < 0.001$, which is 'statistical significant'), but the difference was so small in size (a mean weight loss of $1.61$\ kg) that it was unlikely to be of any use in practice ('practical importance').
In this context, the researchers decidded that a weight loss of at least $2.5$\ kg was of practical importance.
:::
