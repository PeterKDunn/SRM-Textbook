...
Statistical hypotheses *are always about a parameter* (which can take different forms, depending on the context).
Hypothesising, for example, that the *sample* mean body temperature is equal to $37.0^\circ\text{C}$ is silly, because the *sample* mean clearly is $36.8052^\circ\text{C}$ (Chap.\ \@ref(TestOneMean)).
Tthe RQ is about the unknown *population*: the **P** in **P**OCI stands for **P**opulation.
...
* there is *no relationship* between the variables, as measured by a parameter value.
...
The sampling distribution often has an approximate normal distribution.
In these cases, the observed value of the *test statistic*\index{Test statistic} is  
\[
   \text{test statistic} = 
   \frac{\text{test statistic} - \text{centre of the sampling distribution}}
        {\text{standard deviation of the sampling distribution}}.
\]
This is called *test statistics*, since the values are based on sample data (so it is a *statistic*) and used in a hypothesis *test*.
This test statistic may be a $z$-scores or $t$-scores.
Other test statistics, with different forms, are used too (as in Chap.\ \@ref(TestsOddsRatio)).
...
As demonstrated in Sect.\ \@ref(OnePropTestP), $P$-values can be *approximated* (using the $68$--$95$--$99.7$ rule or tables) when the sampling distribution has a normal distribution.
...
A 'big' $P$-values mean that the sample statistic (i.e., $\bar{p}$) could reasonably have occurred through sampling variation in one of the many possible samples, if the assumption made about the parameter (stated in $H_0$) was true.
A 'small' $P$-values mean that the sample statistic (i.e., $\hat{p}$) is unlikely to have occurred through sampling variation in one of the many possible samples, if the assumption made about the parameter (stated in $H_0$) was true.
...
* *Incorrectly* concluding that evidence supports the alternative hypothesis.
  Of course, the researchers *do not know they are incorrect*... but the possibility of making this mistake is always present when concluding that the evidence supports the alternative hypothesis.
...
* *Incorrectly* concluding that there is *no* evidence to support the alternative hypothesis.
  Of course, the researchers *do not know they are incorrect*... but the possibility of making this mistake is always present when concluding that there is no evidence to support the alternative hypothesis.
...
> No evidence of change *does not* mean evidence of no change.
:::
...
This *does not* mean there is evidence of no difference between the mean IQ for football players and $100$.
:::
...
Hypothesis tests assess *statistical significance*, which answers the question: 'Can sampling variation explain the difference between the value of the statistic and the assumed value of the parameter?'
...
This means that the difference in weight loss between placebo and intervention was unlikely to be explained by chance ($P < 0.001$; i.e., 'statistical significant'), but the difference was so small (a sample mean weight loss of $1.61$\ kg) that it was unlikely to be of any use in practice ('practical importance').
...
For instance, the statistical validity conditions ensure that the approximation is sufficiently accurate for the $68$--$95$--$99.7$ rule to apply^[Not all sample statistics have normal distributions, but all the sample statistics in this book are either normally distributed or are closely related to normal distributions.].
If these conditions are *not* met, the sampling distribution may not be sufficiently close to a normal distribution, so the $P$-values (and hence conclusions) maybe inappropriate.
...
The statistic (the *estimate*) is then *observed*, and a *test statistic*, which often is a $z$- or  $t$-score,  is computed to describe this sample statistic.
