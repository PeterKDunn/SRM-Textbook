...
> In sorghum crops in Brazil, is there a relationship between borer infestation and yield?

`summary(lm(Yield ~ Infestation, data=subset(Borers, Hybrids=="AG1090")))`


@da2024potential recorded the borer infestation in sorghum crops, for crops treated with and without insecticide [@da2024potentialDATA], shown in 
`r if( knitr::is_latex_output() ) {
  'see Table\\ \\@ref(tab:BorersData).'
} else {
  'see Fig.\\ \\@ref(fig:BorersDataDT).'
}`
...
if( knitr::is_latex_output()){
  BorersData <- rbind( head(BorersSub, 5),
                       rep("$\\vdots$", 2), 
                       tail(BorersSub, 5) )
  knitr::kable( pad( BorersData,
                     surroundMaths = TRUE,
                     targetLength = c(5, 7),
                     decDigits = c(2, 2)),
                format = "latex",
                ...
                align = c("c", "c"),
                linesep = "",
                caption = "Sorghum yield and borer infestation.",
...
The *Pearson correlation coefficient* (Fig.\ \@ref(fig:BorersScatterjamovi2), right panel)\index{Correlation coefficient (Pearson)} is $r = -0.888, so $R^2 = (-0.888) ^2 = 78.8%.
...
```{r DogsLifeScatterjamovi2, fig.width=5.25, out.width=c('54%', '45%'), fig.align='center', fig.height=3.75, fig.cap="Sorghum yield against borer infestation. Left: scatterplot. Right: correlation output.", fig.show='hold'}
...
In the sorghum data, only one of the countless possible samples of sorghum crops has been studied to determine the yield and infestations, and the value of\ $r$ (an estimate of $\rho$, the *parameter*) will vary from sample to sample.
...
For the sorghum data, the $95$% CI for\ $\rho$ is from $-0.941$ to\ $-0.789$.
This CI is not symmetrical: the value of\ $r$ is not halfway between these limits.
We write:

> For soghum crops, the correlation coefficient between yield and borers infestation percentage is\ $-0.888$, with a $95$% CI from $-0.941$ to\ $-0.789$ ($n = 73$).

In other words, a population with a correlation coefficient\ $\rho$ between $-0.576$ and\ $-0.809$ could reasonably have produced a sample correlation coefficient of $r = -0.712$ from a sample of size $n = 36$.
...
> The sample presents very strong evidence (two-tailed $P < 0.0001$) that the sorghum yield is associated with corer infestation percentage ($r = -0.888$ with $95$% CI from $-941$ to $-0.789$; $n = 36$) in the population.
...
For the sorghum data (Fig.\ \@ref(fig:DogsLifeScatterjamovi2), left panel), a rough estimate of the regression line can be drawn on a scatterplot to estimate\ $b_0$ and\ $b_1$ (Fig.\ \@ref(fig:BorerRiseRun)).
...
Borers.lm <- lm(Yield ~ Infestation, 
            data = Borers)
DB.b0 <- round( coef(Borers.lm)[1], 2)
DB.b1 <- round( coef(Borers.lm)[2], 3)
```
...
For example, the equation can be used to predict the *average* lifespan of a breed with an average breed-weight of $50\kgs$.
...
The model predicts that the *mean* lifespan for dogs with an average breed-weight of\ $50\kgs$ will be about $`r round(DB.b0 + DB.b1 * 50, 2)`\ys$.
...
Suppose we were interested in dog breeds with an average breed-weight of $100\kgs$; the mean predicted lifespan is
$$
...
As an extreme example, a breed with an average breed-weight of\ $x = 150\kgs$ would be predicted to have a mean lifespan of $-0.56\ys$, which is clearly nonsense.
...
When the sampling distribution is an approximate normal distribution, CIs have the form
$$
...
The variation in the sample slope across all possible samples when $\beta_1 = 0$ is described (Fig.\ \@ref(fig:BorersSlopeSampDist)) using:

* an approximate normal distribution,
* with a sampling mean whose value is $\beta_1 = 0$ (from $H_0$), and 
* a standard deviation of $\text{s.e.}(b_1) = 9.011$ (from software; Fig.\ \@ref(fig:BorersRegressionCI)).


```{r BorersSlopeSampDist, fig.cap="The distribution of sample slopes for the dog-breed data, if the population slope is $\\beta_1 = 0$.", fig.align="center", fig.height=3.25, fig.width=8.25, out.width='85%'}
par( mar = c(5, 2, 1, 2))
mn <- 0
se <- 9.011
...
The *observed* sample slope for the sorghum data is $b_1 = -101.2$.
...
\begin{align*}
   ...
   &= \frac{ b_1 - \beta_1}{\text{s.e.}(b_1)} 
    = \frac{-101.2 - 0}{9011} = -11.23,
\end{align*}
where the values of\ $b_1$ and\ $\text{s.e.}(b_1)$ are taken from the software output (Fig.\ \@ref(fig:BorersLifeRegressionCI)).
...
Since $t = -11.23$, the $P$-value will be very small; software shows the *two*-tailed $P$-value is $P < 0.0001$.

We write:

> The sample presents very strong evidence ($t = -11.23$; two-tailed $P < 0.0001$) that, in the population, the yield of sorghum is related to the infestation percentage (slope: $-0.093$; $95$%\ CI from\ $-0.115$ to\ $-0.071$; $n = 73$).

Notice the three features of writing conclusions again:
an answer to the RQ; evidence to support the conclusion ($t = -8.54$; two-tailed $P < 0.0001$); and some sample summary information (slope: $-101.2$; $95$%\ CI from\ $-119.5$ to\ $-82.9$; $n = 36$).
...
For the sorghum data, the scatterplot (Fig.\ \@ref(fig:BorersScatterjamovi2), left panel) shows that the relationship is approximately linear, so using a (Pearson) correlation coefficient and a regression line is appropriate.
For the hypothesis test, the variation in yield doesn't seem to be obviously getting consistently larger or smaller for heavier infestations, and the sample size is greater than\ 365$.
...
These steps are used to test a hypothesis about a correlation between two variables,\ $\rho$.
...
* Use teh $P$-value to make a decision, and write a conclusion.
...
To compute a CI for the slope of a regression equation, software provides the standard error of\ $b_1$; then, the CI is
$$
...
```{r ThrottleAngle, fig.cap="Manifold air pressure plotted against throttle angle for an internal-combustion gas engine.", fig.align="center", fig.width=5, fig.height=3.25}
