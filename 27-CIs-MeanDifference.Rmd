...
The comparison is *within* individuals (Sect.\ \@ref(RQsRepeatedMeasures)); this is a *repeated-measures* RQ.
...
Computing the *differences* between the pairs of observations makes sense.
...
come from same person, with the same sex and age.
...
It is also easy to see that some students *lost* weight and some students *gained* weight from Week\ $1$ to Week\ $12$.
...
Otherwise, the notation is similar to that used in Chap.\ \@ref(OneMeanConfInterval).
...
Statistical software produces *exact* $95$%\ CIs, which may be slightly different than the *approximate* $95$%\ CI (the $68$--$95$--$99.7$ rule gives *approximate* multipliers).
...
Data for both species was collected at $25$ different sites, so the data are *paired* by site (Sect.\ \@ref(CompareWithinInvasivePlantsCI)).
