...
* *should not* promote personal opinions (such as describing resuts as *exciting*, and so on).
...
After completing a first draft, make sure you revise your document: remove unnecessary words, phrases and sentences; ensure consistency.
...
Presentations tend to focus on the *why* and the *what* of the research, rather than the *how*.
They may encourage audience members to learn more by reading your written documents (Sect.\ \@ref(WritingDocuments)).

Presentations also tend to use many graphs and images, use short sentences, and use minimal text.
...
Ensure your fonts are colours are readable from a distance!

Using bullet points on slides, while common, is not necessary; short sentences are fine.
Slides should *not* contain information that you just *read* to the audience; a good presenter adds important details around the structure provided by information on the slides.
...
*  *Background and Objectives*:
   List the study objectives (or the RQs, or the Aims, as necessary).
* *Methods*: Describe:
...
@data:Singh:lowerlimb examined the long-term effects of mortality after amputation.
...
Ensure your article is logical in its presentation.
...
Table should use very few horizonal lines, and no vertical lines.
...
* *Correlation*: In research, correlation describes the (usually linear) relationship between two *quantitative* variables (Sect.\ \@ref(CorrCoefficients)).
   In general use, 'correlation' is often used to mean 'association'.
* *Estimate*: In research, 'estimating' usually means to *calculate* a sample estimate for an unknown population parameter.
  In general use, 'estimate' often means to guess.
...
* *Regression*: In research, 'regression' refers to the mathematical (usually linear) relationship between two quantitative variables (Sect.\ \@ref(Regression)). 
...
* $\beta$: In research, $\beta$ refers to the regression parameters (Sect.\ \@ref(Regression-Software)).
* $\rho$: In research, $\rho$ refers to the population correlation coefficient (Sect.\ \@ref(CorrCoefficients)).
...
They should be written carefully, and words used precisely.
...
1. Presentations generally focus on the details of what was done.
`r if( knitr::is_html_output() ) {torf( FALSE )}`
...
In a student project at the university where I work, some students compared the mean reading speed for people when reading two different fonts.
Their RQ was:
