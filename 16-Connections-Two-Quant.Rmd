...
(Other correlation coefficients also exist, such as the *Spearman* correlation, which may be used for increasing-only or decreasing-only  non-linear relationships.
In this book, correlation coefficients are all Pearson correlation coefficients.)
Correlation coefficients only apply if the form is approximately *linear*, so checking the scatterplot first is important.
...
* $r = -1$ indicates a *perfect, negative* relationship:\index{Correlation coefficient!positive}
  by 'perfect', we mean that knowing the value of $x$ produces a perfect prediction of the value of $y$; the negative value means *larger* values of $y$ are associated with *smaller* values of $x$.
* Values of $r$ between $-1$ and $0$ indicates a *negative* relationship:\index{Correlation coefficient!negative}
...
* $r = 0$ indicates *no linear relationship* between the variables:\index{Correlation coefficient!zero}
...
* $r = +1$ indicates a *perfect, positive* relationship:
  by 'perfect', we mean that knowing the value of $x$ produces a perfect prediction of the value of $y$; the positive value means *larger* values of $y$ are associated with *larger* values of $x$.
...
plot(Weight ~ Age, 
     ...
     main = expression( atop(Molar~weight~and~age,
                             of~male~red~deer)),
#     main = expression("Molar weight and age\nof male red deer",
     xlab = "Age (in years)",
     ylab = "Molar weight (in g)")
axis(side = 2,
     las = 1)
