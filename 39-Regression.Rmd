...
To approximate the slope, the 'rise over run' idea is
`r if (knitr::is_latex_output()) {
   'used (Fig.\\ \\@ref(fig:ExampleScatterLATEX), right panel).'
} else {
   'used; the animation below may help explain.'
}`
...
As part of the decision-making process, the null hypothesis is initially *assumed* to be true.
...
The $t$-value is like a $z$-score, and so an approximate *$P$-value* can be estimated using the $68$--$95$--$99.7$ rule.
