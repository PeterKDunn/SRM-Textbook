...
$y$ refers to the observed data, whose values are observed from individuals.
...
* $b_0$ is called the *intercept* or the *$y$-intercept*.
...
A rough approximation of the *slope* $b_1$ can be made by computing  
\[
...
`r if (knitr::is_latex_output()) {
   'idea (Fig.\\ \\@ref(fig:ExampleScatterLATEX), right panel).'
} else {
   'idea; the animation below may help explain.'
}`
When the value of $x$ increases from $1$ to $5$ (an *increase* of $5 - 1 = 4$), the corresponding value of $y$ changes from $5$ to $17$ (a change of $17 - 5 = 12$).
So:  
\[
   \frac{\text{rise}}{\text{run}} 
   = \frac{17 - 5}{5 - 1}
...
This is a *negative* rise, since the value of $y$ is smaller as the value of $x$ increases.
...
#      ticksize = -0.02)
  ...
  points(Weight ~ Age,
    ...
    text( -0.75, 
          mean( c(y1, y2)) - 0.7, 
          labels = bquote( .(y2)-.(y1) == phantom() - .(abs(rise))), # To get a proper negative sign
          pos = 4 )  
```
...
So $\beta_0$ is usually promounced as 'beater-zero', and $\beta_1$ as 'beater-one'.
:::
...
```{r RedDeerRegressionjamoviSPSSraw, fig.cap="Output for the red-deer data", fig.align="center", out.width=c("55%"), fig.show="hold"}
...
```{r CyclonesRegressionjamoviSPSS, fig.cap="Output for the cyclone data", fig.align="center", out.width=c("55%"), fig.show="hold"}
...
\end{align*}
Male red deer aged $10$ years are predicted to have a *mean* molar weight of $`r  RD.b0 + RD.b1*10`$\ g.
...
Hence, the value of $\hat{y}$ is also depends on which one of the countless possible samples is used, so also has a standard error.)
...
Suppose we were interested in male red deer $20$ years of age; the mean predicted weight of the molars would be $4.398 - (0.181 \times 20) = 0.778$, or about $0.78$\ g.
...
Indeed, young deer are born with molars that are replaced as they grow. 
...
The regression line is computed from the *sample* data, assuming a linear relationship between $x$ and $y$ in the *population*.
...
Usually the slope of more interest than the intercept, because the slope explains the *relationship* between the two variables (Sect.\ \@ref(RegressionForUnderstanding)), so forming confidence intervals for $\beta_1$ is more common than for $\beta_0$, though the same ideas apply.
...
However, under certain conditions (Sect.\ \@ref(RegressionStatValidity)), the sampling distribution can be described .
...
This sampling distribution describes all possible values of the sample slope, through *sampling variation*.
...
Using the standard error reported by the software, an approximate $95$% CI is $-0.181 \pm (2\times 0.029)$, or $-0.181 \pm 0.058$, or from $-0.239$ to $-0.123$\ g/year.
...
Understanding may be easier this way: 
...
Since the regression line is computed from one of countless possible samples, there may be no relationship between $x$ and $y$ in the population; the only reason why a relation appears in the *sample* is due to sampling variation.
That is, a hypothesis test can be conducted.
...
For the red-deer data (Sect.\ \@ref(TwoQuant-Intro)), testing if a relationship exists between the age of the deer and the weight of their molars implies these hypotheses:  
\[
...
The alternative hypothesis is one-tailed, based on the RQ (see Sect.\ \@ref(CorrelationHT)).
...
*Assuming* the null hypothesis is true (that $\beta_1 = 0$), the possible values of the sample slope $b_1$ can be described, due to *sampling variation* (based on Def.\ \@ref(def:Beta1SamplingDistn) by setting $\beta_1 = 0$).
...
The $P$-value will be very small; software shows the one-tailed $P$-value is $P < 0.0005$.
...
> The sample presents very strong evidence ($t = -6.27$; one-tailed $P < 0.0005$) that, in the population, the mean weight of molars decreases as male red deer age (slope: $-0.181$; $95$% CI from $-0.239$ to $-0.124$).
...
> Very strong evidence exists ($t = -7.45$; two-tailed $P < 0.001$) of a relationship between the mean number of ED patients and the number of days after welfare distribution (slope: $-0.348$; $95$% CI: $-0.441$ to $-0.255$).
:::
...
The response variable was OSA severity: the average number of episodes of sleep disruption (according to specific criteria) per hour of sleep (called the Respiratory Event Index, REI).
...
Using the jamovi output (Fig.\ \@ref(fig:OSARegressionjamovi)), the correlation coefficient is $r = 0.264$ (and $R$-squared is about $7$%), so the relationship is not strong.
The value of the slope and intercept for the regression line in the *sample* are $b_0 = -24.47$ and $b_1 = 1.36$, so the regression equation is   
\[ 
  \hat{y} = -24.47 + 1.36x.
\]
...
Each sample will produce slightly different *sample* slopes, so the sample slopes have sampling variation.
Hence, we can test if the slope in the *population* is non-zero due to sampling variation, using a hypothesis test:  
\[
...
The scatterplot of the data (Fig.\ \@ref(fig:CorrelationRemovalEfficiency)) shows the relationship is approximateky linear.
