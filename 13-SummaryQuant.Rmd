...
In con-trast, the bars in histograms are butted together (except when an interval has acount of zero), as the variable-axis represent a numerical scale.
---
...
In this chapter, methods for summarising quantative variables numerically are described.
As described in Sect.\ \@ref(Statistics), numerical quantities are computed from a sample, even though the whole population is of interest.
...
The IQR is $Q_3 - Q_1$ = $2.9 - 1.5 = 1.4$, as shown in Fig.\ \@ref(fig:QuartilesYgorillas).
...
Selected answers are available in Sect.\ \@ref(SummariseQuantData-Answer).
