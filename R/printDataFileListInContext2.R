...
cat("filesPerChaper ", filesPerChapter,"\n")
cat("startNewChapter ", startNewChapter,"\n\n")

  # Initialise the list
  listDataFilesByChapter <- list()
  
  # Now order, with Exercises at end (and rest alpha?)
  for (j in 1 : numberChapters ) { # For each chapter with data used
    
    startSort <- startNewChapter[j]
    stopSort <-  startNewChapter[j+1] - 1 
    cat("start, stop", startSort, stopSort, "\n")
    
    if ( j == length(filesPerChapter)) {
      stopSort <- as.numeric(chapNumbersList[ length(chapterNumbers)] )
    }
...
#    filesUsedThisChapterRaw <- filesUsedThisChapterRaw[ !duplicated(filesUsedThisChapterRaw) ]
    ...
    # 
    cat("j = ", j, ":", filesUsedThisChapter, "\n")
    if (length(filesUsedThisChapter) > 2) {
      fileList <- paste(filesUsedThisChapter,
                        collapse = ", ") 
    } else {
      fileList = paste0("c(",
...
###
addHyperLinks() <- function(){
  
}


###
printDataFileList() <- function(){
