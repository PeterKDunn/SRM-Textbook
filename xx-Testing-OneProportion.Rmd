...
when [certain conditions are met](#LINK), where $n$ is the size of the sample, and $p$ is the population proportion.
...
So rather tha *us* rolling a die 20,000 times, we can use his data, where $n = 20,000$.

So, if $p$ really was $0.1666...$, and if certain conditions are true, the possible values of the sample proportion can be described using:
...
We can see that, in 20,000 rolls of a die, that a sample proportion of 6s as low as 0.165 is quite reasonable... but a sample proportion as low as 0.160 is starting to become quite unusual.
...
This is larger than the expected $p = 0.166...$... but not *highly* unusually so (Fig. \@ref(fig:RollsSixesSDWolf)).
...
Are Las Vegas visitors are more likely to smoke than the general U.S.
population ($p = 0.255$; based on data from the U.S. National Center for Health Statistics)?
