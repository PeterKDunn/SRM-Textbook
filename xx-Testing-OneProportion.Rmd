...
In 1882, R. Wolf tossed a die 20,000 times (@data:hand:handbook, Dataset 131; @wolf1882] and recorded the number of times each face appeared...
So, we would expect that each die face would appear about one-sixth of the time (based on [classical probability](#ProbClassical)),
so a `r include_graphics("Dice/dice6.png", dpi=2000)` would be expected to appear about one-sixth of the time.

Of course, we don't expect to see *exactly* one-sixth of the rolls turns up as a `r include_graphics("Dice/dice6.png", dpi=2000)`.
Sometimes, when we rolls a die 20,000 times, the proportions of `r include_graphics("Dice/dice6.png", dpi=2000)`s will be a bit smaller than $1/6$, and sometimes it will be a bit larger than $1/6$.

What would be a reasonable for the proportion of `r include_graphics("Dice/dice6.png", dpi=2000)`s rolled, from a fair die?
We could find a CI for the proportion, [as we did earlier](#SamplingDistributionKnownp), or use a simulation.

Using the [techniques from earlier](#SamplingDistributionKnownp), we would expect the sample proportions to vary from one set of 20,000 rolls to another set, but the average proportion would be $p = 1/6 = 0.166667$.
The amount of variation in each set is described by the *standard error*:

\[
  \text{s.e.}(\hat{p}) = \sqrt{\frac{p\times(1 - p)}{n}} = \sqrt{\frac{0.166667\times(1 - 0.166667)}{20,000}} = 0.002635231.
\]
So 95% of the time, the sample proportion $\hat{p}$ will lie in the range $0.166667\pm (2\times 0.002635231)$; that is, from 0.161 to 0.172.

When Wolf tossed his die, he found 3422  `r include_graphics("Dice/dice6.png", dpi=2000)`s, which is a sample proportion of  $\hat{p} = 3422/20,000 = 0.1711$.
This is just inside the range of proportions that we were expecting.

Alternatively, we can *simulate* what would happen:
We could use a computer to 'pretend' to roll a die 20,000 times and record the proportion of  `r include_graphics("Dice/dice6.png", dpi=2000)`s.
If we do this many, many times, we have a list of *sample* proportions.
We can plot a histogram of these sample proportions (Fig. \@ref(fig:RollsSixesHist)).


```{r, RollsSixesHist, fig.cap="The proportion of sixes in 20,000 rolls of a fair die varies from one set of 20,000 rolls to another."}
# SIMULATE dice rolls
set.seed(309147) # For reproducibility
...
numSixes <- array( dim = numSims)

for (i in (1:numSims)){
  
  roll <- rbinom(n, 1, p) # ^ or not 6 (or any other number of course)
  numSixes[i] <- sum(roll)
...
out <- hist( numSixes / n,
             ...
             ylab = "How often",
             main = "The proportion of 6s in 20,000 rolls",
             las = 1,
             xlim = c(0.155, 0.180), 
             col = "aliceblue")
...
What we see is interesting--the distribution looks like a normal distribution.
We also see that, in 20,000 rolls of a die, that a sample proportion of 6s as low as 0.165 is quite reasonable... but a sample proportion as low as 0.160 is starting to become quite rare.
A sample proportion as low as 0.155 almost never occurs.

So, if we did roll a die 20,000 time and saw the proportion of sixes was 0.155, we would have observed something rather unusual.

He wondered if the proportion of `r include_graphics("Dice/dice6.png", dpi=2000)` was too low or too high

A more formal approach to determining if the proportion of `r include_graphics("Dice/dice6.png", dpi=2000)`s was unusual is to perform a *hypothesis test*.

## Statistical hypotheses and notation: One proportion

The [decision-making process](#DecisionMaking) begins by assuming that the *population* proportion is $1/6 = 0.16667$, as one would expect for a fair die.
In other words, the reason why the *sample* proportion $\hat{p} = 3422/20,000 = 0.1711$ is not exactly $p = 0.166667$ is due to sampling variation.

The other explanation for why the *sample* proportion $\hat{p} = 3422/20,000 = 0.1711$ is not exactly $p = 0.166667$ is because the die is not fair.
...
We always begin the hypothesis testing process by assuming the null hypothesis is true.

Note that the alternative hypothesis asks if $p$ is $0.16667$ or not.
The value of $p$ may be smaller *or* larger than $0.16667$.
...
The decision-making process assumes that the population proportion is $p = 0.1667$ (as stated in the null hypothesis); that is, that the die is fair.
However, because of sampling variation, the value of $\hat{p}$ sometimes would be smaller than $0.16667$ and sometimes greater than $0.16667$.
How much variation in the value of $\hat{p}$ could be expected, simply due to sampling variation, when $p = 0.16667$?
This variation is described by the *sampling distribution*.

The sampling distribution of $\hat{p}$ was discussed in Sect. ?? (and Def. ?? specifically). 
From this, if $p$ really was 0.16667, and if certain conditions are true, the possible values of the sample proportion can be described using:

* An approximate normal distribution;
* With mean 0.16667;
* With standard deviation of $\text{s.e.}(\hat{p}) = \sqrt{\frac{p\times (1 - p)}{n}} = \sqrt{\frac{0.1711 \times(1 - 0.1711)}{n}} = 0.002635231$.
  This is the standard error of the sample proportions.
...
This enables questions to be asked about the likely values of $\hat{p}$ that would be found in the sample, when the population proportion is 
$p = 0.16667$.

```{r, RollsSixesSD, fig.cap="The sampling distribution, showing the distribution of the sample proportion when the *population* prooportion is 0.16667, in 20,000 die rolls"}
xx <- seq( min(breaks),
           max(breaks),
           len= 1000)
yy <- dnorm(xx, 
...
The sampling distributions describes what to expect from the sample proportion, assuming $p = 0.16667$.
The value of $\hat{p}$ that is observed, however, is $\hat{p} = 3422/20,000 = 0.1711$.
How likely is it that such a value could occur by chance?

The value of the observed sample proportion can be located the picture of the sampling distribution (Fig. ???). 
The value $\hat{p}$ = 0.1711$ is large... but is it *unusually* large?
More formally, about how many standard deviations is $\hat{p}$ away from $p$? 
In other words, what is the $z$-score?

The $z$-score is

\[
   z = \frac{\hat{p} - p}{\text{s.e.}(\text{sample estimate})} = \frac{0.1711 - 0.166667}{0.002635231} = 1.68.
\]

The sample proportion of $\hat{p} = 0.1711$ is larger than expected... but not unusually so.

(Remember that the standard deviation in Fig. 4.5 is the the standard error: the amount of variation in the sample means.)
...
1. **Assume** that the *population* proportion is $p = 0.16667$ (this is $H_0$).
2. Based on this assumption, describe what to **expect** from the sample proportions (Fig. ???).
3. The **observed statistic** is computed, relative to what is expected, using a $z$-score: $z = 1.68$.
...
```{r Pvalues, animation.hook="gifski",  interval=0.5, echo=FALSE, fig.cap="The strength of evidence: P-values", fig.height = 3, fig.align="center", dev=if (is_latex_output()){"pdf"}else{"png"}}
...
* The answer to the RQ: 'The sample provides little evidence... that the population proportion is not $1/6$.
...
The **conclusion** is that there is very strong evidence that $p$ is likely to be $0.16667$ based oin this evidence.
...
## Statistical validity conditions: One proportion
...
The hypothesis test regarding measles in Koprea is statistically valid since the number of...
...
275 iguanas with a SVL between 100 and 149mm were found in teh study, of which 146 were female.
...
The study of herbal medicines is complicated as *blinding* subjects is difficult: placebos are often easily-identifiable visually, by taste, or by small,
...
The palmaris longus tendon is visually *present* (PLA) in about 15% of the population.
