...
# Internal validity in experimental studies {#DesignExperiment}\index{Internal validity!experimental studies}\index{Study design!experimental|(}
...
A well-designed study is needed to draw solid conclusions (Def.\ \@ref(def:StudyDesign)): a study with high *internal validity* (Sect.\ \@ref(def:InternalValidity)) and hih *external validity* (Sect.\ \@ref(def:ExternalValidity))).
...
* Managing the placebo effect by using controls (Sect.\ \@ref(PlaceboEffectExperimental)).
...
For this chapter, consider this interventional RQ (based on @data:Bird2008:wholegrain):
...
## Managing confounding {#ExpManagingConfounding}\index{Internal validity!experimental studies!managing confounding}
...
Any difference detected between the two groups may not be because of the diets (Table\ \@ref(tab:ConfoundingGroups)): the study has very poor internal validity, due to poor study design.

These other variables are *confounding variables* (Def.\ \@ref(def:ConfoundingVariable)).\index{Variables!confounding}
For example, the age of the subject may be related to faecal weight (older people tend to eat less, and eat differently, than younger people), and the study design means that older people are more likely to be consuming the refined cereal.
This is an extreme case of *confounding* (Fig.\ \@ref(ConfoundingDiagram)); usually, confounding is more subtle (and more difficult to detect) than in this example.
...
* *Blocking*.\index{Confounding!experimental studies!blocking}
  Units of analysis are arranged into different groups that are similar to one another (see Sect.\ \@ref(PairedTTest-SoilN) for an example).
...
The fourth approach (*randomly allocating*) is superior when possible, because the chance of confounding is reduced for variables not even suspected as being confounders.
...
Individuals may be randomly allocated to groups (in true experiment).
...
## Random allocation vs random sampling {#Random-sampling-allocation}\index{Confounding!random allocation}\index{Sampling}
...
## Hawthorne effect and blinding individuals {#HawthorneEffectExperimental}\index{Hawthorne effect}\index{Blinding!individuals}
...
For example, a study aiming to increase fruit and vegetable intake in young adults [@clark2019educational] noted that the observed increases in intake 'could be explained by the Hawthorne effect' as they 'know they are being observed...'.
:::


The impact of the Hawthorne effect can be minimized by [blinding](#Blinding) the individuals in the experiment, so that:
...
Blinding *people* to knowing they are involved in a study is often difficult, as ethics often requires peoples' informed consent (Sect.\ \@ref(Common-Ethical-Issues)).
...
In a study to compare the efficacy of a new type of toothpaste [@lorenz2019effect], participants were given either a new or an existing toothpaste formulation, and evaluations of plaque remaining on the teeth were taken.
...
## Observer effect and blinding researchers {#ObserverEffectExperimental}\index{Observer effect}\index{Blinding!researchers}
...
## Placebo effect, controls and blinding {#PlaceboEffectExperimental}\index{Placebo effect}\index{Blinding!individuals}\index{Control group}
...
In the *Himalaya* study, the authors report that the individuals "were not told the identity of the test cereal in the foods provided" (@data:Bird2008:wholegrain, p.\ 1033).
...
## Carry-over effect and washouts {#CarryOverEffectExperimental}\index{Carry-over effect}\index{Washouts}
...
In the *Himalaya* study, the authors report that 'there was no washout period' (@data:Bird2008:wholegrain, p.\ 1033) since the response variable was only recorded after individuals spent four weeks on each diet.
...
In the *Himalaya* study, the authors report that "subjects were allocated randomly to [...] dietary treatments" (@data:Bird2008:wholegrain, p.\ 1033).
...
## Describing blinding {#DescribingBlinding}\index{Blinding!descriptions}\index{Blinding}
...
If *only* the individuals are blinded to the comparison groups or the connections that apply to them, the study is called *single blind*.\index{Blinding!single}
If *both* the researchers and participants are blinded to the comparison groups or connections that apply, the study is called *double blind*.\index{Blinding!double}
If the researchers, participants *and* the analyst are blinded to the comparison groups or connections that apply , the study is called *triple blind*.\index{Blinding!triple}
...
## Recording extraneous variables {#RecordExtraneous}\index{Variables!extraneous}
...
* To improve *internal validity*, by helping to manage confounding:
  ...
  * by explaining some variation in the response variable, using special methods of analysis (Sect.\ \@ref(ExtraneousVariables)).
  * by avoiding lurking variables (Sect.\ \@ref(ExtraneousVariables)).
  * by determining if the groups being compared are similar (Sect.\ \@ref(ExpManagingConfounding)).
...
Well-designed experimental studies also try to manage the *Hawthorne effect* (e.g., by blinding *participants*); the *observer effect* (e.g., by blinding the *researchers*); the *placebo effect* (e.g., by blinding, and using objective outcomes); and the *carry-over effect* (e.g., by using a washout, or randomly allocating the treatment order).
...
Often, however, not all of these issues can always be managed.
...
The impact of these issues are often reported as *limitations* in a journal article (Chap.\ \@ref(Interpretation)), perhaps part of the Discussion section (Sect.\ \@ref(WritingDiscussion)).
...
<!--                     "Behaviour of \\textbf{individuals} may change; what individuals report may change") -->
<!--   Biases[2, ] <-  c("Placebo effect", -->
<!--                     "", -->
<!--                     "Behaviour of \\textbf{individuals} may change; what individuals report may change") -->
<!--   Biases[3, ] <-  c("Observer effect", -->
<!--                     "\\textbf{Researcher} aware of explanatory variable value", -->
<!--                     "Behaviour of \\textbf{researcher} may change, unconsciously communicated to \\textbf{individuals}, who may act or report differently") -->


<!--   knitr::kable(Biases[c(1, 3, 2), ], -->
<!--                format = "latex", -->
<!--                longtable = FALSE, -->
<!--                booktabs = TRUE, -->
<!--                escape = FALSE, -->
...
<!--                     "Behaviour of **individuals** may change; what individuals report may change") -->
<!--   Biases[2, ] <-  c("[Placebo effect](#PlaceboEffectExperimental)", -->
<!--                     "", -->
<!--                     "Behaviour of **individuals** may change; what individuals report may change") -->
<!--   Biases[3, ] <-  c("[Observer effect](#ObserverEffectExperimental)", -->
<!--                     "**Researcher** aware of explanatory variable value", -->
<!--                     "Behaviour of **researcher** may change, unconsciously communicated to individuals, who may act or report differently") -->
...
`r if (knitr::is_html_output()) '<!--'`  
3. What is the the role of the variable 'The location of the bruising'?  
