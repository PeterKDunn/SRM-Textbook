...
So far, you have learnt to ask a RQ, identify studies, and soem parts of study design.
...
To draw solid conclusions from a study, good data are needed, and good data come from well-designed studies.
A well-designed study requires high [*internal validity*](#def:InternalValidity):
When studying the relationship between the response and explanatory variables, we would like to rule out---as far as possible---any other explanation for the changes in the values of the response variable, so any remaining changes we see can be attributed to just the explanatory variable of interest.
...
You usually get one chance to collect data, but you can analyse your data as many times as you like.
It is important to design the study properly the first time!
:::


::: {.example #InternalValidity name="Importance of internal validity"}
Researchers [@beaman2013profitability] describe an experiment where free fertilizer was provided to a sample of female farmers in Mali (at either the recommended amount per hectare, or at half the recommended amount per hectare).

Since all the farmers knew they were in the study (that is, they were not [*blinded*](#Blinding)), the farmers changed their farm management: they employed more hired labour and used more herbicide.
Consequently, the yields for all farmers changed.

However, knowing whether this change in yield was due to the amount of fertilizer applied is difficult.
...
* [Managing the observer-effect by blinding the researchers](#ObserverEffectExperimental).
...
In the *Himalaya* study [@data:Bird2008:wholegrain], the exclusion criteria were:
...
If a difference in faecal weight was found between the two groups, the difference may because:
...
* The *age* of the participants in each group (Group A is likely to be younger on average; Group B is likely to be older on average);
* The health and fitness levels in each group (Group A would generally be healthier than those in Group B).

If a difference is found between the *Himalaya 292* and refined cereal groups, it may not be because of the cereal (Table \@ref(tab:ConfoundingGroups)).
That is, the study has very poor internal validity due to poor study design.
...
An experiment to study the effect of using ginko to enhance memory [@data:Solomon2002:Ginko] compared two groups: one using ginko ($n = 111$), and one using a 
...
Potentially, many extraneous variables exist, but to demonstrate, we will consider just one: age.
How can we make sure that the age of the participants does not cause confounding?

Confounding can be *managed* by:

* [**Restricting**](#ExpConfoundingRestricting) the study to a certain group (for example, only people under 30).
...
* [**Analysing**](#ExpManagingConfoundingAnalysis) using special methods (after measuring the age of each subject).
...
A common theme is to ensure that any variables of potential concern are measured, observed, assessed or recorded, to ensure no *lurking variables* exist to compromise the results.

Of course, multiple approaches can be used, such as randomly allocating individuals to groups, but also measuring, observing, assessing or recording many other variables that can be managed through analysis (Example \@ref(exm:GinkoGroups)).
...
This means that, if differences are observed in the study between the DHOC and NDHOC groups, it is probably due to the treatment.
...
Blocking refers to separating the units of analysis into a small number of groups that are similar to one another, then studying those groups separately. 
...
Confounding variables can be accommodated in the analysis (using analysis methodology beyond this book), *provided those variables have been measured, observed, assessed or recorded*.
Because of this, *measuring, observing, assessing or recording all the information likely to be important for understanding the data* is important.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
*Measure, observe, assess or record* all the information that is likely to be important for understanding the data.
This may include information about

* the individuals in the study; and 
* the circumstances of the individuals in the study.
:::
...
In the *Himalaya 292* study, the sex, age, pre-study weight and pre-study BMI were also recorded for each individual.
:::
...
Random allocation may occur when randomly allocating individuals to groups (0rue experiment), and/or when randomly allocating treatments to groups (true or quasi-experiment).
...
* **Random sampling** impacts *external* validity, and concerns *how the sample is found* to study.
* **Random allocation** helps eliminate confounding issues, by evening-out possible confounders across treatment groups.
  *Random allocation* of treatments helps establish cause-and-effect, and impacts *internal* validity, and concerns *how the members of the chosen sample get the treatments*.
...
straightarrow(from = c(0.12, 0.33), 
              ...
              lcol = "black",
              lty = 1)
straightarrow(from = c(0.06, 0.38), 
              to = pos[1,], 
              lwd = 2,
              lcol = "black",
              lty = 1)
...
The impact of the carryover effect may be minimized by using a *washout* or similar.
For example, after tasting a food sample, participants may rinse their mouth with water before tasting another food sample.
For the *Himalaya* study, for example, the participants could spend four weeks between the two sepcial diets on their usual (before-study) diet.
This is often called a *washout period*.
...
In the *Himalaya 292* study, the authors report that 'there was no washout period' (@data:Bird2008:wholegrain, p. 1033).
No washout period was used, since the response variable was only recorded after four weeks on the diets.
:::


::: {.example #HimalayaWashout name="Washout"}
An engineering study [@miller2019behavioral] examined drivers' exposure to lane-keeping system on their driving performance.
...
> Subjects were allocated randomly to [...] dietary treatments according to a cross-over study design with each intervention phase lasting 4 weeks. 
> 
> --- @data:Bird2008:wholegrain, p. 1033
...
This is called the *Hawthorne effect* [@mccarney2007hawthorne].
...
For example, a study aiming to increase fruit and vegetable intake in young adults [@clark2019educational] noted that 
...
The impact of the Hawthorne effect can be minimized by [blinding](#Blinding) the individuals in the experiment so that they do not know:

* that they are in a study;
* the aims of the study; and/or 
* which treatment they are receiving. 

For example, if individuals do not know which treatment they receiving, they cannot behave differently according to the treatment they receive.
...
That is, the subjects knew they were in a study, which was an ethics requirement.
The Hawthorne effect may influence the results in this study.
...
In an experimental study [@lorenz2019effect] to compare the efficacy of a new type of toothpaste, participants were given two types of toothpaste to use (new type; exisiting type), and evaluations of plaque remaining on the teeth were taken.
The authors state that:

> ... a plaque-reducing effect was seen not only in the test group but also in the control group. 
> This phenomenon is due to the so-called Hawthorne effect that can lead to an overestimation of the effect and false positive results.
> 
> --- @lorenz2019effect, p. 5

Since participants knew they were being assessed after brushing their teeth, they brushed their teeth better than usual.
...
# abline seems to go outside margins...?
lines( x = c(0, 4),
       y = c(1, 1), 
       lty = 2,
       col = "grey")
...
What if the *researchers* assessing the outcomes *knew the diet* allocated to each patient, and were hoping that the new diet performed better than the refined cereal diet?
Could this lead to a misleading conclusion?
...
The impact of observer effect can be minimized by blinding the researchers so that they do not know which treatments the individuals are receiving.
The people *giving* the treatment and the people *evaluating* the treatment do not know what treatment has been given.
...
Later, the assistant tells the researchers whether Drug A or Drug B performed better, but only the researchers know what drugs the labels A and B refer to.
...
['Clever Hans'](https://en.wikipedia.org/wiki/Clever_Hans) was a horse that seemed to perform simple mental arithmetic.

After much study, Carl Stumpf realised that the horse was responding to involuntary (and unconscious) cues from the trainer.
This was discovered, in part, by using an experiment where the people interacting with the horse were blinded.
...
*Blinding of participants does not just apply to people*; it is also relevant with animals (Example \@ref(exm:CleverHans) about Clever Hans).
...
In a study comparing chest compressions with dominant and non-dominant hands of student paramedics [@cross2019impact], the article states that:
...
In a cropping study comparing yields from modern and traditional cowpea crops in Tanzania, the researchers wanted to use a double-blind study.
To do so:
...
Minimize the impact of these have been discussed (Fig. \@ref(fig:DesignConsiderations)), but is not always possible.
These effects are important to understand, so studies can be designed to manage or minimise their influence (to maximise internal validity).
This ensures that the results and conclusions from our studies are correctly interpreted.

Often, however, some (or all) of these issues cannot be well managed.
...
> First, data were collected over 15 years ago [...]
> Second, only college females were assessed and findings may not generalize to college males or to broader groups of young adults [...]
...
pos[6, ] <- c(0.45, 0.10) # Obs bias--removed
...
1. Extraneous variables are variables that are related to the response variable.  
