...
Since the farmers knew they were in the study, they changed their farm management: they employed more hired labour and used more herbicide.
...
* **Blocking**.
  Units of analysis are arranged into different groups that are similar to one another (see Sect.\ \@ref(PairedTTest-Intro) for an example).
...
The fourth approach (*randomly allocating*) is superior when possible, because the chance of confounding is reduced for variables not even suspected as being confounding variables.
...
*Blocking* could be used by first finding pairs of subjects with similar characteristics (for example, pairs of subjects of the same sex, and of similar age).
...
See Sect.\ \@ref(PairedTTest-Intro) for an example of blocking.
...
* To manage confounding:
  ...
  * To use the information in analysis (Sect.\ \@ref(ExpManagingConfounding)).
