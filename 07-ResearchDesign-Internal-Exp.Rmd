...
To demonstrate, we will consider just one: age.
...
is when the influence of past experience(s) of the individuals carry over to influence future experience(s) of the individuals.
:::
...
Sometimes, researchers can *randomly allocate* the  *order* in which the treatments (i.e., the diets) are used.
...
The impact of the carryover effect may be minimized by using a *washout period* or similar; 
for example, after finishing one diet, the participants spend four weeks on their usual (before study) diet, and then revert to the second diet being used.

In some studies, a *washout* is used. 
...
That is, subjects were randomly allocated to a diet: some subjects began the study on the *Himalaya 292* diet while others started on the refined cereal diet.
No washout period was used; however, since the response variable was recorded after four weeks on the diets, no washout period was considered necessary.
:::
...
People often behave differently (either positively or negatively) if they know (or think) they are in a study or are being watched.
...
is the tendency of individuals to change their behaviour if they know (or think) they are being observed.
:::
...
MOVED HERE START:
In general, making the *individuals* unaware ([blinding](#Blinding)) that they are in a study, or unaware of what explanatory variable values apply to them, reduces or eliminates bias.
Likewise, by making the *researchers* unaware ([blinding](#Blinding)) of what explanatory variable values apply to the individual reduces or eliminates bias.
MOVED HERE END
...
As is usual, this was an ethics requirement (in this case, from the Ethics Committee of the CSIRO).
...
# abline seems to go outside marhins...?
lines( x = c(0, 4),
       y = c(1, 1), 
       lty = 2,
       col = "grey")
...
Perhaps surprisingly,
the researchers' expectations or hopes for how the new diet will perform
may (unconsciously) influence how the researchers interact with the individuals, 
and perhaps (unconsciously) influence the behaviour of the individuals in the study.

This is called *observer effect*.\index{observer effect}
...
The impact of observer effect can be minimized by blinding the researchers so that they do not know
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The *observer effect* is about the observer *unconsciously* influencing the individuals; 
