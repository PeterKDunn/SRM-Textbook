...
**In this chapter**, you will learn how to ensure that the conclusions we can make are logical and sound in *experimental* studies. 
You will learn to:
...
we would like to be able to rule out---as much as possible---any other reason for the changes in the values of the response variable,
...
Remember the goal of study design is to maximise internal validity: 
...
or unaware of what explanatory variable values apply to them,
reduces or eliminates bias.

Likewise, by making the *researchers* unaware ([blinding](#Blinding)) of what explanatory variable values apply to the individual
reduces or eliminates bias.
...
(as older people tend to eat less, and eat differently, than younger people),
...
usually, confounding is more subtle (and hence more difficult to detect) than in this example.
...
(in the *Himalaya 292* example, the diet that they are given).
:::
...
compared two groups:
one using ginko ($n=111$), and one using a 
pretend, non-active supplement ($n=108$).

The authors randomly allocated participants to each group,
but also compared the two groups to ensure that no obvious differences 
...
Hence, any difference between the groups cannot be attributed to 
...
it is unlikely to be because the groups themselves are different in terms of age and sex of participants.
