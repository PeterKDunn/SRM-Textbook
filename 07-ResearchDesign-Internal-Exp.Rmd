...
other possible issuesthat might influences the value of the response variable should be eliminated.
...
This means that, if any difference are observed in the study between DHOC and NDHOC groups,
...
In this section, 
some specific design considerations are discussed to maximise [internally validity](#def:InternalValidity):
...
```{example InternalValidity, "Poor internal validity"}
A group of researchers [@beaman2013profitability]
describe an experiment where free fertilizer was provided to a sample of female farmers in Mali (at the recommended amount per hectare; or at half the recommended amount per heectare).
...
This could cmpromise the  [internally validity](#def:InternalValidity) of the study.
...
* [Blinding](#Blinding) of the participants: 
  Individual partipants maybe able to be *blinded* to being in the study, to the aims of the study, and/or which treatment they are receiving.  
  ...
  Sometimes, participants can be blinded to the real purpose of a study. 
  However, the participants will often  know that they are in a study 
...
* Blinding the researchers to which treatments have been applied to which individuals:
  That is,
...
```{example, BlindingCowpea, "Double-blinding"}
...
as discussed in Example \@ref(ex:BlindingParamedic),
