...
<!-- In the *Himalaya* study, many exclusion criteria were used, including "a history of diabetes, gastrointestinal, renal, hepatic and cardiovascular disease, an intolerance to cereal-based foods..." (@data:Bird2008:wholegrain, p. 1033). -->
<!-- ::: -->
...
That is, any difference detected may not be because of the cereal (Table \@ref(tab:ConfoundingGroups)): the study has very poor internal validity due to poor study design.
...
Potentially, many extraneous variables exist, but we will demonstrate just considering age.
...
A common theme is to ensure that any potential confounding variables are recorded, to ensure no *lurking variables* exist that may compromise the results.
Multiple approaches can be used, such as randomly allocating individuals to groups, but also recording other variables that can be managed through analysis (Example \@ref(exm:GinkoGroups)).
...
Confounding variables can be accommodated in the analysis (using analyses beyond this book), *provided those variables have been recorded*.
Because of this, *recording all the information likely to be important for understanding the data* is important.
...
Random allocation may occur when randomly allocating individuals to groups (true experiment), and/or when randomly allocating treatments to groups (true or quasi-experiment).
...
This is called the *carry-over effect*.
...
For the *Himalaya* study, the participants could spend four weeks between the two special diets on their usual (before-study) diet, which is called a *washout period*.
...
If individuals do not know which treatment they receiving, they cannot behave differently according to the treatment they receive.
...
The authors state that the reason was "the so-called Hawthorne effect" (@lorenz2019effect, p. 5) since participants knew they were being assessed after brushing their teeth.
:::
...
What is the control?\label{thinkBox:Control}
...
Minimize the impact of these have been discussed (Fig. \@ref(fig:DesignConsiderations)).
...
Often, however, some (or all) of these issues cannot always managed.
...
```{r DesignConsiderations, echo=FALSE, fig.cap='(ref:DesignConsiderations-Caption)', out.width='80%', fig.align="center", fig.height=7, fig.width=7}
par( mar = c(0.5, 0.5, 0.5, 0.5))
openplotmat()
