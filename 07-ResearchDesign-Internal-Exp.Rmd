...
%%%
\begin{minipage}{0.45\textwidth}%
```{r, fig.width=4.25, fig.height=2, out.width='95%'}
...
```{r, Washout, echo=FALSE, fig.align="center", fig.cap="Using a 'washout' period to minimize the carry-over effect", fig.width=6.0, out.width='70%', fig.height = 3}
...
However, the impact of the placebo effect can be minimized using a *control group*: units of analysis without the treatment applied, but *as similar as possible* in every other way to the units of analysis receiving the treatment.
This allows the effect of the treatment to be assessed, *over and above* the placebo effect.
In addition, recording objective data (rather than patient-reported outcomes) may reduce the placebo effect [@enck2013placebo] (Example\ \@ref(exm:PlaceboPrice)).
...
In an experimental study [@seo2020role] that examined the impact of an injection to alleviate post-operative umbilical pain, the authors stated (p. 392):
...
* the *observer effect* (e.g., by blinding the researchers to the applied treatment).
