...
To draw solid conclusions, a well-designed study is needed: a study with high [*internal validity*](#def:InternalValidity) to reduce bias.
This means that, when studying the relationship between the response and explanatory variables, we would like to rule out---as far as possible---any other explanation for the changes in the values of the response variable, so any remaining changes we see can be attributed to just the explanatory variable of interest.
...
```{r DesignConsiderations, echo=FALSE, fig.cap='(ref:DesignConsiderations-Caption)', out.width='70%', fig.align="center", fig.height=7}
par( mar = c(0.5, 0.5, 0.5, 0.5))
openplotmat()
...
**1.** Only the last statement is True.
**2.** Lurking and Confounding variables are types of extraneous variables.

**1.** True: second, third, seventh and eighth statements are true.
**2.** The sex of the patient; the initial weight of he patients.
:::
`r if (knitr::is_html_output()) '-->'`
