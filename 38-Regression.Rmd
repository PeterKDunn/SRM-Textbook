...
```{r, child = if (knitr::is_html_output()) {'./introductions/38-Regression-HTML.Rmd'} else {'./introductions/38-Regression-LaTeX.Rmd'}}
```
...
An example of a linear regression equation, that describes the relationship between the observed values of an explanatory variable $x$ and the observed values of a response variable $y$, is
\begin{equation}
...
Some individual girls aged\ $7$ will be taller than predicted, and some will be shorter.
...
In the graph, I have drawn a sensible line on the graph that seems to capture the relationship between $x$ and $y$.
...
A rough approximation of the value of the *intercept* $b_0$ is the value of $\hat{y}$ from the drawn line when $x = 0$.
...
Software calculates this 'line of best fit'.
...
The values of $b_0$ and $b_1$ are in the column labelled `Estimate` in the output; the value of the sample *slope* is $b_1 = `r RD.b1`$, and the value of the sample $y$-intercept is $b_0 = `r RD.b0`$: 
...
As an extreme example, deer aged $25$ would be predicted to have a mean molar weight of $-0.127$, which is clearly nonsense (molars cannot have a negative weight).
...
Consider again the red-deer regression equation (from Sect.\ \@ref(Regression-Software)):  
\begin{equation}
   \hat{y}
   = 
   `r RD.b0` 
...
Specifically, each extra year older is associated with an average change of $`r RD.b1`$\ g in molar weight (see Eq.\ \@ref(eq:RedDeerEquation)); that is, a *decrease* in molar weight by a mean of $`r abs(RD.b1)`$\ g.
...
To demonstrate, consider the case where $x = 10$, and $y$ is predicted to be $\hat{y}= `r RD.b0 + 10 * RD.b1`$\ g.
...
That is, the regression line that is estimated from one of the countless possible samples is an estimate of the regression line in the population.
In the population, the intercept is denoted by $\beta_0$ and the slope by $\beta_1$ (so the regression line is $\hat{y} = \beta_0 + \beta_x$ in the population).
...
Under certain conditions (Sect.\ \@ref(RegressionStatValidity)), the sampling distributions for $b_0$ and $b_1$ both have a normal distribution. 
...
Because of this, forming confidence intervals for $\beta_1$ is more common than for $\beta_0$, though the same ideas apply.
For this reason, we give the sampling distribution for the slope only, but the sampling distribution for the intercept is analagous.
...
Using the standard error reported by software, an approximate $95$% CI is $-0.181 \pm (2\times 0.029)$, or $-0.181 \pm 0.058$: from $-0.239$ to $-0.123$\ g/year.

Software can be used to produce *exact* CIs too (Fig.\ \@ref(fig:RedDeerRegressionjamoviSPSSCI)): $-0.239$ to $-0.124$\ g/year.
...
1. The relationship is approximately linear (this is necessary for the regression equation to be appropriate).
...
1. What weight is predicted for a rower who is $180$\ cm tall?
...
1. Using the software output (Fig.\ \@ref(fig:OSAscatter), right panel), determine the value of the correlation coefficient.
...
[*Dataset*: `EDpatients`]
@brunette1991correlation studied the relationship between the number of emergency department (ED) patients and the number of days following the distribution of monthly welfare monies, from 1986 to 1988 in Minneapolis,
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
out <- knitr::kables(list(T1, T2),
                     format = "latex",
                     label = "Data6MWTAge",
                     caption = "The six-minute walk test (6MWT) distance using a $30$\\ m walkway, and the subjects age. Ten of the $50$ observations are shown.") %>% 
  kable_styling(font_size = 8)
...
1. Construct an approximate $95$% for the slope.
1. Are the test and CI statistically valid?
