...
$\hat{y}$ is pronounced as 'why-hat'; the 'caret' above the $y$ is called a 'hat', and is used to denote a *predicted* value (in this case, a predicted value of $y$).
:::
...
The estimate of $b_1$, using the 'rise-over-run' idea, is $-2/10 = -0.2$.\index{Regression!equation!rise over run}
...
To demonstratcere, consider the case where $x = 10$, and $y$ is predicted to be $\hat{y}= `r RD.b0 + 10 * RD.b1`$\ g.
...
```{r RedDeerSlopeSampDistCI, fig.cap="The distribution of sample slopes for the red-deer data, around the population slope $\\beta_1$", fig.align="center", fig.height=3.5, fig.width=8.25, out.width='85%'}
mn <- 0
se <- 0.0289
...
Using the software output (Fig.\ \@ref(fig:CyclonesRegressionjamoviSPSS)) for the cyclone data, the approximate $95$%\ CI for the regression slope $\beta_1$ is
$$
...
Possibly, no relationship exists in the population; the only reason why a relationship appears in the *sample* is due to sampling variation.
...
\begin{align*}\index{Test statistic!t@$t$-score}
   t 
   &= \frac{\text{observed value} - \text{mean of the statistic's distribution}}{\text{standard dev. of the statistic's distribution}}\\
...
The third condition is only necessary for constructing CIs and performing tests.
...
1. What is teh value of the correlation coefficient?? 
   Explain.
