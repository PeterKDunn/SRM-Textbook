...
# Answers to even-numbered end-of-chapter exercises {#Answers}
...
**1.** Type of tourniquet; time to apply. 
**2.** **Quant**itative.
:::
...
**1.** Whether people get side-effects, or not after taking medication.
**2.** **Quant**itative.
:::
...
**1.** Type of diet (vegan; non-vegan).
**2.** Whether coffee caffeinated or not.
...
**1.** Whether vegetables frozen or fresh.
**2.** Type of fuel.
**3.** Size of city.
:::
`r if( !includeEvenAnswers) "-->"`
...
**Ex.\ \@ref(exr:RQsDogs).**
**1.** Correlational.
**2.** No sense assigning explanatory, response variables.
:::
...
**1.** P: Danish Uni students; O: *Average* resting diastolic BP; C: between students who regularly drive, ride their bicycles to uni.
**2.** No intervention.
**3.** Relational.
**4.** Decision-making.
**5.** *Conceptual*: 'regularly'; 'uni student' (on-campus? undergraduate? full-time?). *Operational*: how 'resting diastolic blood pressure' measured.
**6.** Resting diastolic blood pressure; whether they regularly drive, ride to uni.
**7.** Danish uni students; Danish uni students.
:::
...
**1.** Some elements not well defined; perhaps: P: Children aged under\ $3$ in a Peruvian peri-urban community; O: proportion children with diarrhoea; C: nutritional status; no intervention.
...
**5.** Whether or not each person takes Vit.\ C tablets or not.
**6.** Decision-making
**7.** One-tailed.
:::
`r if( !includeEvenAnswers) "-->"`
...
Animals in the same pen not independent: compete for same space, food, resources; similar environments.
**3.** Between diets.
:::
...
Outcome is *average* lung capacity.
Perhaps: 'For students that study at (Uni), do males have a larger *average* lung capacity than females?'
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** *Before*, *after* for each individual.
**3.** Operational.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** P: US adults; individuals: US adults.
...
Intervention (worms were used or not).
:::
`r if( !includeEvenAnswers) "-->"`
...
Brand allocated to car; each car gets only same tyre brand.
Tyres on one car exposed to the same day-to-day use, drivers, distances, conditions, etc.

Each unit of analysis (car) has four units of observations.
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:PoorRQs).**
**1.** Should be worded in terms of averages or means.
**2.** Everyone dies; time-frame is needed (i.e., within $12$ months after amputation). 
Compared to what?
**3.** No defined population (frozen beans? fresh butter beans? tinned kidney beans? jelly beans? can sizes?); 'amount' of salt should be, e.g., 'average' concentration (or amount per $100$\ g) or similar.
Anyway: look at label (unless, you wish to query those values).
**4.** Silly: elephants huge; joeys tiny; no need to test this. 
Also, RQ talks about 'weight' rather than 'mean weight'.
**5.** More specific! Perhaps: 'Is the mean reaction time different for males and females?' or similar.
:::
`r if( !includeEvenAnswers) "-->"`
...
**6.** Observational: researchers don't determine distance of lakes from mine.
:::
...
*Extraneous*: presumably *all* listed; that is why researchers obtained this information.
*None* can be lurking: researchers measure or observe them.

*Confounding* variable potentially related to *both* response *and* explanatory variables.
Hence, all extraneous variables could potentially be confounding.
:::
`r if( !includeEvenAnswers) "-->"`
...
*Response*: perhaps 'risk of developing a cancer of digestive system'.
*Explanatory*: 'whether or not participants drank green tea at least $3$ times a week'.
...
Older children more likely to be smokers, and larger in general: age is confounding.
Age easy to record, and usually *is* recorded in these types of studies: probably *not* lurking.
:::
`r if( !includeEvenAnswers) "-->"`


::: {.answer data-latex=""}
**Ex.\ \@ref(exr:FactorsInfluenceYRTEC).**
...
**1.** Between-individuals.
**2.** Relational if C is between individuals.
**3.** Yes.
**4.** Decision-making.
**5.** True experiment.
:::
...
**1.** Diet.
**2.** Change in body weight after $2$\ y.
...
**6.** Individuals: those from whom weight change is taken.
**7.** *Change* in body weight over $2$ years.
**8.** Type of diet.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** Researchers *do not intervene*: find subjects who *do* or *do not* already own a pet.
:::
...
**1.** Simple random sample: need list of all books held by library.
*May* be possible, but probably not for student or non-library staff member.
...
**3.** Cluster: each shelf a cluster; randomly select some shelves; determine number pages in each book on selected shelves.
**4.** Convenience: finding books in libraries easily accessible.
**5.** Multi-stage: random sample of campuses; random sample of shelves in selected libraries; random shelf from each one; small number of random book from each shelf.
...
**2.** Convenience; by approaching every $5$th person and going *every day* for a week, trying to make it more representative ( better than the first)
**3.** Self-selecting.
**4.** Convenience.
By going every day for two weeks, at different times and locations each week, approaching someone every $15$\ mins: sample more representative.

Fourth is  best, but still far from 'random'.
None truly random.
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** ...if the sample representative of all Santiago households.
...
Probably in case hive size is confounder.
:::


`r if( !includeEvenAnswers) "<!--"`
::: {.exercise #ResearchDesignSeptic}
The control, acting as base level.
Ethical.
:::
`r if( !includeEvenAnswers) "-->"`
...
**4.** If mean of both the response, explanatory variables was different for females and males, sex a *confounder*; would need to be included into  analysis.
**5.** Participants blinded to what is happening.
:::
...
**4.** To ensure participants do not change behaviour because of treatment received.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Randomly allocate type of water to subjects (or *order* in which subjects taste *each* drink.)
...
Observer effect: researcher directly contacting subjects; may unintentionally influence responses.
:::
...
Since data only collected at night, perhaps not *ecologically valid*.
:::
...
**1.** Observational: researchers cannot determine the *comparison* (not outcome).
**2.** A mix of both C ('smokers and non-smokers') and O ('the median serum cholesterol').
**3.** External validity only refers to whether sample represents target population (Australians); whether the results apply for entire world is irrelevant.
**4.** 'Serum cholesterol' not a variable; nothing here is varying. 
'Serum cholesterol' is just a type of cholesterol.
Variable is 'serum cholesterol concentration' or similar.
**5.** Not an experiment.
**6.** Data file has $2$ columns (variables): one to record smoking status (Yes/No), one to record serum cholesterol concentration.
**7.** A confounding variable has to be related to both response, explanatory variables.
**8.** Observer effect about how researchers might respond, not individuals under study.
:::
`r if( !includeEvenAnswers) "-->"`
...
First, second: only present option of 'owning'
Third: best question.
:::
`r if( !includeEvenAnswers) "-->"`
...
*Location* is only variable (observed from *individuals*).
*Number of people* and *percentage of people* who died is a *summary* of data from individuals.
...
Very right-skewed.
Average somewhere near $1$ or $2$\ kg perhaps.
...
Probably median as slightly skewed right, but with some outliers.
...
Maybe quote *both* mean and median ($14.90$ and $14.99$\ respectively).
:::
`r if( !includeEvenAnswers) "-->"`
...
*Outliers*: no outliers or unusual observations; observation between $35$ and $40$ *may* be outlier.
  I suspect it is *not*; a larger sample may have observations between $30$ and $35$.
:::
`r if( !includeEvenAnswers) "-->"`
...
Dataset\ B: more observations close to mean; average distance from mean smaller.
...
**2.** Bus.
**3.** No.
**4.** $3.44$; i.e., students $3.44$ times as likely to use motorised transport than active transport.
**5.** $0.141$; i.e., for every $100$ students that *do not walk* to campus, about $100\times 0.141 = 14.1$ *do walk* to campus.
**6.** Figure not shown.
The left panel shows specific methods, and right panel shows methods of transport grouped more coarsely.
:::
...
**1.** *Vomited*: $0.50$ beer then wine; $0.50$ wine.
*Didn't vomit*: $0.738$ beer then wine, $0.262$ wine. 
Prop. that drank various things, among those who did and didn't vomit.
**2.** *Beer then wine*: $8.8$% vomited, $91.2$% didn't; *Wine*: $21.4$% vomited, $78.6$% didn't. 
...
**7.** $0.354$.
**8.** $-0.176$.
**9.** Higher percentage vomited after beer-then-wine, compared to wine only.
:::
...
**5.** Odds ratio: $1.41$.
**6.** Odds females skipping are $1.41$ *times* odds males skipping. 
**7.** Not shown. 
:::
...
**4.** $0.0238$.
**5.** $6.39$.
**6.** Odds coffee drinker being a smoker is $6.39$ times odds of a non-coffee drinker being smoker.
**7.** Not shown.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Student type: nominal (three levels). 
       Number concussions: ordinal (three levels).
**2.** $18.75$% of all college students in sample received one concussion.
**3.** Odds(non-athlete receiving $2+$ concussions): $0.35$.
       Non-athletes $0.35$ times as likely to receive '$2+$ concussions' than 'fewer than two'. 
 Or: For every $100$ students with fewer than two concussions, $35$ with $2+$.
...
**2.** B: about $185$\ cm; N: about $175$\ cm.
**3.** B: about $14$\ cm; N: about $6$\ cm.
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** Small sample; hard to be certain.
...
$R^2 = (-0.682)^2 = 0.465$: about $46.5$% of the variation in number of cyclones explained by knowing value of ONI averaged over Oct. to Dec.; extraneous variables explain remaining $54.5$% of the variation in number of cyclones.
:::
...
*Graph*: black box hides median; vertical axis label unhelpful; horizontal axis unlabelled; units of measurement not given; title and/or caption helpful.
...
*Are* independent; what happens on die does not change what happens with cards.
$4/16 = 0.25$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** *Not independent*: A smoker far more likely to suffer from lung cancer than a non-smoker.
...
A positive test result may or may not mean the person has disease.
...
The reasoning assumes the outcomes (HH, TT, HT) *equally likely*: not true.
For example, consider tossing a $20$-cent coin (lower-case, normal font) and a $1$-dollar coin (capitals, **bold** font).
*Four* outcomes are:
h**H**,
h**T**,
t**H**
...
**1.** That the population mean is $12$\ inches, as claimed. We have no evidence to refute this.
**2.** *First*: *population* mean diameter $\mu = 12$ inches; *sample* mean not $12$\ inches due to sampling variation.
...
**4.** $\bar{x} = 11.25$ inches is further from $\mu = 12$ than $\bar{x} = 11.48$: claim probably not supported.
...
**4.** $z = 1.64$ (or $1.65$); $5$% *longer* than about $42.7$ weeks. 
**5.** $z$-score: $-1.28$; $10$% *shorter* than about $37.9$ weeks.
:::
...
About $z = 2.05$.
IQ: $130.75$.
IQ greater than about $130$ required to join Mensa.
:::
...
Based on sample, sample proportion of koalas that crossed at least one road in previous $30$ months is $0.353$ ($n = 51$), with an approximate $95$%\ CI from $0.219$ to $0.487$.
...
**1.** *Parameter*: population mean weight of American black bear, $\mu$.
**2.** $\text{s.e.}(\bar{x}) = 3.756947$.
**3.** Normal; mean $\mu$; std dev: $3.757$,
**4.** $77.4$ to $92.4$\ kg.
**5.** Approx. $95$% confident *population mean* weight of male American black bears between $77.4$ and $92.4$\ kg.
...
**3.** Normal; mean $\mu$; std dev: $0.03883$,
**4.** CI: $2.8\pm0.07766$.
**5.** $\bar{x} = 2.8$\ kg (s.e.: $0.0388$; $n = 586$); approx. $95$%\ CI from $2.72$ to $2.88$.
**6.** Stat.\ valid: $n \ge 25$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Approx. $95$%\ CI: $754.1$\ ml to $938.7$\ ml.
**2.** Not good at estimating (from article: guesses between $50$\ ml to $3000$\ ml).
**3.** Stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** *Samples* don't vary between values; *statistics* do. (CIs about populations anyway.)
...
**5.** $s$: variation in diameters of individual pizzas;
       s.e.: precision of sample mean to estimate population mean.
**6.** Almost certainly not same; probably close to $11.486$ inches.
...
**1.** Computing differences as `Before` minus the `After` measurements seems sensible: average blood pressure *decrease*, purpose of drug.
**2.** The diffs (*reductions*): $9$, $4$, $21$, $3$, $20$, $31$, $17$, $26$, etc..
...
**6.** Individuals in pairs have same sex, diet, age, etc.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** $-4.35$ to $9.71$\ mins.
'In population, mean difference between amount of vigorous PA by Spanish health students between $4.35$\ mins more *during* lockdown, and $9.71$\ mins more *before* lockdown.'
:::
...
**1.** Parameter: $\mu_M - \mu_F$.
       Estimate: $\bar{x}_M - \bar{x}_F = -0.06$\ m.
**2.** Plot not shown. <!-- See \@ref(fig:WhalesErrorbar). -->
**3.** $0.0928735$
**4.** Table not shown.
**5.** $-0.25$\ m to $0.13$\ m. 
'The population difference between mean lengths of female and male gray whales at birth has $95$% chance of being between $0.13$\ m, longer for male whales to $0.25$\ m longer for female whales.'
**6.** Stat.\ valid.
:::
...
**1.** Parameter: $\mu_N - \mu_S$.
       Estimate: $\bar{x}_N - \bar{x}_S = 0.085$\ m.
**2.** Graph not shown.
**3.** Table not shown.
**4.** Approx. $95$% CI: $0.055$ to $0.115$.
**5.** From jamovi, *exact* $95$%\ CI: $0.05438$ to $0.11501$ (bottom row).
       Exact $95$%\ CI for diff.\ between the mean direct HDL cholesterol concentrations: $0.054$ to $0.115$\ mm\ Hg, higher for non-smokers.
:::
`r if( !includeEvenAnswers) "-->"`
...
**5.** Placebo *minus* echinacea: diff.\ between the means show how much *longer* symptoms last with placebo, compared to echinacea.
**6.** $5.85$ to $6.83$ days.
**7.** Stat.\ valid.
...
The diff.\ between mean decelerations likely between $-0.0016$ m/s (i.e, a mean *acceleration* of $0.0016$\ m/s) to $0.0056$\ m/s.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Either direction fine; here $\mu_Y - \mu_O$: mean amount younger women can lean further forward than older.
**2.** Dot plot?
**3.** Table not shown.
...
**6.** Based on sample, a $95$%\ CI  for diff.\ between population mean one-step fall-recovery angle for healthy women between $9.1$ and $19.9$ degrees *greater* for younger women than for older women (two independent samples).
:::
...
**7.** A $95$% chance interval $-0.1542$ to $0.1477$ straddles population difference in proportions.
**8.** $0.429$.
**9.** $0.435$.
**10.** $0.985$.
...
**5.** $-0.0545$ to $0.130$.
**6.** $-0.0533$ to $0.1287$.
**7.** A $95$% chance interval $-0.0533$ to $0.1287$ straddles value of population proportion.
**8.** About $1.878$.
**9.** About $1.597$.
**10.** $1.1763$, as in output.
**11.** A few ways; for example, for every $100$ women with smooth scar, about $118$ men with smooth scar.
**12.** $0.7966$ to $1.7370$.
**13.** Table not shown.
**14.** (Graph not shown; use stacked or side-by-side bar chart.)
...
In testing, *assume* the value of $p$ as stated in $H_0$.
In CIs, have no value of $p$.
:::
...
Tests are *not* about sample value (we know value of $\hat{p}$) in our sample), but about unknown population value (i.e., $p$).
:::
...
2.\ The value $0.076$ appears to be sample value, but hypotheses should be formed before data collected.
:::
`r if( !includeEvenAnswers) "-->"`
...
Very strong evidence to support alternative hypothesis that people do better-than-guessing at identifying the placebo.
:::
...
$P$-value very small.
Strong evidence that proportion of females using machines was lower than proportion of females in uni.\ population.
:::
...
**7.** Very strong evidence mean speed of vehicles on this road greater than $90$\ km.h^$-1$^.
**8.** Stat.\ valid.
:::
...
$H_0$: $\mu = 120$ and $H_1$: $\mu \ne 120$ (two-tailed), where $\mu$ is mean time *in seconds*.
...
*Very* strong evidence ($P < 0.001$) that mean weight of a Fun Size *Cherry Ripe* bar not $14$\ g.
...
Very strong evidence that mean guess of blood volume is not $1000$\ ml, the actual value.
The sample much larger than $25$: test is stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
Hypotheses have form $H_0$: $\mu = \text{pre-determined target}$, and $H_1$: $\mu \ne \text{pre-determined target}$.
$t$-scores: $t_1 = 0.318$, $t_2 = 2.347$, $t_3 = -0.466$, $t_4 = -0.726$.
$P$-values large, except for second test.
No evidence that instruments are dodgy, except perhaps for first instrument for mid-level LH concentrations.
Stat.\ valid.
While assessing means is useful, how *variable* the measurements are is also useful (but beyond us).
:::
`r if( !includeEvenAnswers) "-->"`
...
But the diff.\ between $0.0501$ and $0.0499$ is trivial.
:::
...
But diff.\ between $0.011$ and $0.009$ is trivial.
:::
`r if( !includeEvenAnswers) "-->"`
...
**5.** Hypothesis written down *before* data collected.
...
**3.** Conclusion about the pop.\ **mean** energy intake.
**4.** $P$-value *is* $0.018$, not *less than* $0.018$.
:::
...
**1.** *No* evidence of a diff.\ in lifetime between two brands: $P$-value 'large'.
...
Common advertising trick: 'No other battery lasts longer!'... meaning "no evidence of a difference in means'.
**4.** Price!
:::
`r if( !includeEvenAnswers) "-->"`
...
How much longer task takes on PC for each child.
:::
...
**7.** No evidence ($t = -0.205$; two-tailed $P = 0.839$) of mean increase in weight of squash from dry to normal years (mean change: $2.230$\ g ($95$%\ CI from $-24.8$ to $20.3$\ g), heavier in normal year).
**8.* $n = 24$; probably.
:::
...
**1.** It is the blood pressure *reduction*; a reduction is what the drug meant to produce, so expect reductions to be positive.
**2.** Mmean reduction in blood pressure.
...
**8.** Very strong evidence ($P = 0.0005$) drug reduces average systolic blood pressure (mean reduction: $8.6$\ mm\ Hg) in population.
**9.** $n = 14$; maybe not.
:::
`r if( !includeEvenAnswers) "-->"`
...
The evidence *probably* doesn't support the $H_1$.
:::
...
$H_0$: $\mu_d = 0$ and $H_1$: $\mu_d > 0$, where *diffs.*\ positive when intention to smoke is reduced after exercise.
$t = 1.78$; $P$-value larger than $0.05$: evidence doesn't support $H_1$.
...
Test may not be stat.\ valid (histogram of data suggests population *might* have normal distribution), though $P$-value so large it probably makes little difference.
:::
...
$H_0$: $\mu_D = 0$ and $H_1$: $\mu_D > 0$, if diff.\ is beta-endorphin concentration $10$\ mins before surgery, minus concentration $12$--$14$ hours before.
$t = 2.48$; quite large.
Using $68$--$95$--$99.7$ rule, expect small **one-tailed** $P$-value (certainly less than $0.025$).
jamovi reports *one*-tailed $P = 0.0115$.
Conclude (CI found earlier) that strong evidence exists in sample (paired $t = 2.48$; one-tailed $P = 0.0115$) of population mean increase in beta-endorphin concentrations from $12$--$14$ hours before surgery to $10$\ mins before surgery ($95$%\ CI: $1.62$ to $13.78$\ fmol/mol.)
...
How much greater the mean braking distance is for cars with Type\ B brake pads compared to Type\ A.
:::
`r if( !includeEvenAnswers) "-->"`
...
**4.** No evidence ($t = 0.65$; two-tailed $P > 0.10$) in sample that mean length of adult gray whales is different in population for females (mean: $12.70$\ m; standard deviation: $0.611$\ m) and males (mean: $12.07$\ m; standard deviation: $0.705$\ m; $95$%\ CI for diff.: $-1.26$\ m to $0.246$\ m).
**5.** Stat.\ valid.
:::
...
**3.** Very strong evidence in sample ($P < 0.001$) that mean annual sugar consumption per person different for industrialised (mean: $41.8$\ kg/person/year) and non-industrialised (mean: $24.6$\ kg/person/year) countries ($95$%\ CI for difference $11.95$ to $22.54$).
:::
...
**1.** Either direction fine; amount by which younger ($Y$) women can lean further forward is $\mu_Y - \mu_O$.
**2.** One-tailed (from RQ).
**3.** $H_0$: $\mu_Y - \mu_O = 0$; $H_1$: $\mu_Y - \mu_O > 0$.
**4.** $t = 6.69$ (from *second row*); $P < 0.001/2$ as one-tailed; i.e., $P < 0.0005$.
**5.** Very strong evidence in sample ($t = 6.691$;  one-tailed $P < 0.0005$) that population mean one-step fall recovery angle for healthy women is *greater* for young women (mean: $30.7$^o^C; std. dev.: $2.58$^o^C; $n = 10$) compared to older women (mean: $16.20$^o^C; std. dev.: $4.44$^o^C; $n = 5$; $95$%\ CI for diff.: $9.1$^o^C to $19.9$^o^C).
:::
...
**4.** Strong evidence in sample ($t = 8.58$; two-tailed $P < 0.001$) that people with disability (mean: $31.83$; $n = 132$; standard deviation: $7.73$) and people without disability (mean: $25.07$; $n = 137$; standard deviation: $4.80$) have different population mean access to health promotion services ($95$%\ CI for diff.: $5.18$ to $8.34$), as measured by BHADP scores.
:::
`r if( !includeEvenAnswers) "-->"`
...
Sample size small; test may not be stat.\ valid.
...
4. The *positive* value of $2.76$ means coeliacs have a mean of $2.76$ more DMFT.  
   The *negative* value of $-2.32$ means *non*-coeliacs have a mean of $2.32$ more DMFT.
:::
...
**1.** Table not shown.
**2.** Graph no shown.
**3.** Moderate evidence that *population* odds of finding a male sandfly in eastern Panama is different at $3$\ ft above ground compared to $35$\ ft above ground.
**4.** Stat.\ valid.
:::
...
One option: $H_0$: pop.\ OR is one; $H_1$: pop.\ OR not one.
...
**9.** Strong evidence ($P < 0.001$; $\chi^2 = 33.763$; $n = 752$) that odds of wearing hat is different for males (odds: $0.257$) and females (odds: $0.0640$; OR: $0.249$, $95$%\ CI from $0.151$ to $0.408$).
**10.** Yes.
:::
...
$P < 0.001$.
The *sample* provides very strong evidence ($\chi^2 = 22.374$; two-tailed  $P < 0.001$) that odds in the *population* of having a pet bird is not the same for people with lung cancer (odds: $0.695$) and for people without lung cancer (odds: $0.308$; OR: $2.26$; $95$%\ CI from $1.6$ to $3.2$).
:::
...
**2.** Pop. OR, comparing odds of being B12 deficient, for veg. to non-vegs.
**3.** $\chi^2 = 4.707$.
**4.** Equivalent to $z = 2.17$; small-ish $P$; $P = 0.030$ from output.
**5.** Sample provides *moderate evidence* ($\chi^2 = 4.707$; $P = 0.030$) that  odds in the population of being Vit.\ B12 deficient is different for vegetarian women (odds: $0.3077$) compared to non-vegetarian women (odds: $0.0976$; OR: $3.2$; $95$%\ CI: $1.08$ to $9.24$).
...
**7.** Smallest exp. count: $7.75$; stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
**6.** Very strong evidence in sample that proportion bringing a bag not the same for all three age groups.
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:Method4).**
Summary of two groups (over $30$; $30$ and under), plus diff.\ between means; boxplot and error-bar chart.
$t$-test and CI for diff.\ between two means.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** Stat.\ valid only if relationship approx. linear, and variation in STAI does not change for different levels of work experience.
$n \ge 25$.
:::
...
$P$-value is $0.190$: no evidence of relationship.
:::
`r if( !includeEvenAnswers) "-->"`
...
**6.** $t = 2.09$ and $P = 0.041$: slight evidence of relationship.
**7.** Stat.\ valid.
:::
...
**4.** $H_0$: $\beta = 0$; $H_1$: $\beta \ne 0$ (could be in terms of correlations).
RQ two-tailed, but testing if stronger legs *increase* kicking distance seems sensible.
**5.** $t = 6.16$: huge; $P = 0.0001$ (two-tailed).
**6.** $0.70$ to $1.4$\ ft.
**7.** Very strong evidence in sample ($t = 6.16$; $P = 0.0001$ (two-tailed)) that punting distance is related to leg strength (slope: $1.0427$; $n = 13$).
:::
...
*Intercept*: dodgy (extrapolation); in principle $0$% bitumen content by weight, the percentage air voids by volume about $17.47$%.
...
Might be poor prediction, since extrapolation.
:::
...
**3.** $\hat{y} = 18$\ g.
**4.** About $64$% reduction in unknown variation by using time spent on application.
...
**1.** Perhaps use *substantial* rather than *significant*.
**2.** The data *are*...'.
:::
...
**1.** *Correlation* is the wrong word; consider *relationship* or *association*.
**2.** 'The group *was*', or 'The group members *were*...'.
:::
`r if( !includeEvenAnswers) "-->"`
...
Perhaps: Sample provided evidence that mean reading speeds different ($P = ???$), when comparing text in Georgia font (mean: ???) and Calibri font (mean: ???; $95$%\ CI for difference: ??? to ???).
:::
...
No units of measurement; jump-heights given to $0.001$ of centimetre.; table could also summarise information for each individual jump type; numerical summary shouldn't include $P$-value, $t$-score, or CI.
:::
`r if( !includeEvenAnswers) "-->"`
...
Variables *qualitative*: means inappropriate; use odds ratio; values almost certainly refer to CI for the OR.
Without more information, can't be sure what the OR means.
:::
...
**7.** $P$ small; evidence that mean difference in step-count between the methods cannot be explained by chance: likely is a difference.
**8.** Stat.\ valid.
:::
...
**14.** Evidence mean not same in all three groups.
**15.** $19.6$ to $20.0$.
**16.** $0.20488$.
...
**2.** Comparing two groups; age a possible confounder.
**3.** Two-sample $t$-test.
**4.** $0.03376$.
...
**12.** Observational, so no cause-and-effect; no confounders noted; very restricted population.
:::
...
**1.** About $27$% of variation in Se concentration explained by electrical conductivity.
**2.** $r = 0.52$.
...
**6.** $5.61$.
**7.** Very small.
**8.** Very strong evidence proportion of wells with Se concentration less than  $2$\ $\mu$g.L^$-1$^ different for areas with and without Pliocene rocks within $5$\ km.
:::
`r if( !includeEvenAnswers) "-->"`
...
**7.** Yes.
**8.** Yes.
**9.** For *Cx.*, evidence mean sizes at two temperatures were different; for *Ae.*, no evidence mean sizes at two temperatures were different.
...
**1.** Stratified?
**2.** Two-sample $t$-test.
**3.** Strong evidence mean number of actinomycetes are different.
...
**4.** To ensure two comparison groups included women of similar age (to help manage confounding).
**5.** $0.92208$.
**6.** $t = 0.542$.
**7.** Yes.
**8.** No evidence of difference in mean age.
**9.** Yes.
**10.** To ensure two comparison groups included women in similar rooms (to manage confounding).
**11.** Not shown.
**12.** $z - 0.592$.
**13.** Yes.
