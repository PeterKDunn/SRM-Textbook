...
**6.** Danish university students.
**7.** Danish university students.
:::
...
**2.** Perhaps something like: 'In children aged under\ $3$ in a Peruvian peri-urban community, is there a a relationship between diarrhoea status and nutritional status?'.
**3.** Relational.
**4.** 'Diarrhoea status'; 'nutritional status'; probably others.
...
**1.** How *individual* people are using their phones.  
('Talking on the phone' and 'texting on the phone' are not *variables*.)
**2.** *Response variable* is the walking speed.
...
**4.** *Between*-intervals comparison.
...
*Unit of observation*: the *animal* (animals are weighed).
*Unit of analysis*: the *pen*, as food is allocated to the animals in the whole pen.
...
::: {.answer data-latex=""}
**Answer to Exercise\ \@ref(exr:DescribeClassifying12).**
...
```{r CPalsyPlots, fig.align="center", fig.cap="Some graphs from the cerebral palsy data", fig.width=7, fig.height=2.0, out.width='100%'}
NumP <- 15
Gender <- rep("M", 
              NumP)
...
```{r PanelsBoxplot, fig.align="center", fig.cap="The boxplot for the productivity data", fig.width=5, fig.height=3}
Panels <- array(NA, 
                dim = c(7, 3))
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(TypingSummary,
                      decDigits = c(3, 2, 1, 0)),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
```{r, fig.cap="The sampling distribution of the proportion of males in samples of $864$ people with hiccups", fig.width=6, fig.height=3.25, fig.align="center"}
z <- seq(-3.5, 3.5, 
         length = 100)
mn <-  0.8194444
sdev <- 0.01308604
...
if( knitr::is_latex_output() ) {
  kable( surroundMaths(BrocolliSummaryTable,
                       decDigits = c(1, 1, 2)),
         format="latex",
         booktabs = TRUE,
         align = "r",
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(NHANESSummaryTable,
                      decDigits = c(3, 4, 4, 0)), 
        format = "latex",
        longtable = FALSE,
        escape = FALSE,
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(FF.Summary2,
                      decDigits = c(1, 2, 2, 0)),
      format = "latex",
      booktabs = TRUE,
      align = "r",
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(TurbineCITable,
                      decDigits = 0),
        format = "latex",
        longtable = FALSE,
...
TurbineCISumTable[3, ] <- c("0.389", 
  ...
  kable(surroundMaths(TurbineCISumTable,
                      decDigits = c(4, 2, 0)),
        format = "latex",
        longtable = FALSE,
...
```{r FerritinGraph, fig.align="center", fig.cap="A histogram of the change in ferritin concentration", out.width='70%', fig.width=6, height=2}
...
if( knitr::is_latex_output() ) {
  kable(surroundMaths(round(chisq.test(PhoneTable)$expected, 2),
                      decDigits = 2),
        format = "latex",
        longtable = FALSE,
        booktabs = TRUE,
...
1. Bus.
1. No.
1. $220 / (284 - 220) = 3.44$; that is, students are $3.44$ times as likely to use motorised transport than to use active transport.
1. $35/(284 - 35) = 0.141$; that is, for every $100$ students that *do not walk* to campus, about $100\times 0.141 = 14.1$ *do walk* to campus.
1. Fig.\ \@ref(fig:TransportPlots). (The colours in the two plots are related:  white refers to active methods of transport.)
...
4. $t = (\bar{x} - \mu)/\text{s.e.}(\bar{x}) = (96.56 - 90)/0.6937 = 9.46$.
5. The $t$-score is a *huge* value; the (one-tailed) $P$-value is very small.
6. Write (remembering the alternative hypothesis is one-tailed):
