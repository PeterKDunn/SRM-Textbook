...
**1.** Some elements not well defined, but perhaps: P: Children aged under\ $3$ in a Peruvian peri-urban community; O: proportion of children with diarrhoea; C: nutritional status; no intervention.
...
<!-- **3.** Hard to be sure since "nutritional status" is not well defined. Perhaps it is amount intake per day? Then, correlational probably.
<!-- **4.** Decision-making.**5.** 'Diarrhea status'; 'nutritional status'; probably others.
...
**1.** *Where*: nominal (respondents can select more than one option, so each option is effectively a Yes/No variable).
...
**3.** *Where*: home, work most common (modes).
       ...
       *Time*: $3$\ mins is mode and median among those who know.
       All counts can be turned to percentages and odds too.
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** Assuming fair die, would *not* expect a `r if (knitr::is_latex_output()) {
   '\\Largedice{6}'
} else {
...
*Standard error of the mean* describes how the sample mean varies from sample to sample.
:::
...
CIs gives interval for the population *mean*, not *data*.
:::
...
**1.** *Difference*: mean length of males (M) *minus* females (F). 
**2.** $\mu_M - \mu_F$.
...
**2.** $0.563$
**3.** Splinting *minus* exercise: the difference are how much greater the pain is with splinting.
**4.** $-0.826$ to $1.426$: $0.826$ greater pain with exercise to $1.426$ greater pain with splinting.
**5.** $0.404$ to $1.796$.
**6.** Sample sizes are small; CIs may not be statistically valid, roughly correct only.
:::
`r if( !includeEvenAnswers) "-->"`
...
**5.** Probably not statistically valid.
...
**4.** Expensive (time *and* money): $10\ 000$ and $2500$ probably unrealistic.
:::
...
$t = (56.6 - 60)/0.4 = -8.5$; very small $P$-value.
...
For both boys and girls, very strong evidence that the mean sleep time one weekend is less than ten hours.
:::
...
Test may not be statistically valid; the histogram of data suggests population *might* have a normal distribution), though $P$-value is so large it probably makes little difference.
:::
::::::
...
(The small difference is because the expected counts are only given to three decimal places).
:::
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:Method1).**
Summary of mean *differences*; histogram of differences.
Paired samples $t$-test; CI for mean difference.
:::
...
**3.** Statistically valid only if the relationship approx. linear, and variation in STAI does not change for different levels of work experience.
$n > 25$.
:::
...
<!-- **7.** Null: mean HQL score same for females and males.**8.** $\text{s.e.}(\text{diff}) = 0.2375$.
...
<!-- **15.** $19.6$ to $20.0$.<!-- **16.** $0.20488$.<!-- **17.** $t = 3.90$; small $P$, as in table.:::
`r if( !includeEvenAnswers) "-->"`
...
<!-- **3.** $R^2 = 29$%; about $29$% of variation in MOO explained by height.**4.** Slope: Each extra $10$\ cm of height associated with $3.6$\ mm greater MMO, on average.
**5.** $\hat{y} = 54.29$.
**6.** Two-sample $t$-test.
**7.** Extremely small.
