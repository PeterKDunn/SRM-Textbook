...
**1.** *Between*-individuals.
       Outcome: percentage of people wearing hats.
**2.** *Between*-intervals.
...
**Ex.\ \@ref(exr:RQsDogs).**
**1.** Correlational.
**2.** No sense assigning one variable as explanatory, and another as response.
:::
...
**2.** No intervention.
**3.** Relational.
**4.** *Conceptual*: 'regularly'; 'university student' (on-campus and online? undergraduate *and* postgraduate? full-time and part-time?). *Operational*: how 'resting diastolic blood pressure' will be measured.
...
(levels: 'Talking on the phone' and 'texting on the phone').
**4.** Walking speed.
...
The $10$ adults is the sample.
Unclear how many fonts being compared (or which fonts).
...
**3.** Response: number of steps recorded for individuals.
       Explanatory: the location of accelerometer.
...
Tyres on each car do not operate independently: exposed to the same day-to-day use, same drivers, have driven almost identical distances, under the same conditions, etc.
...
Observer effect.
Researcher directly contacting the subjects, so may unintentionally influence responses.
:::
...
Because the data was only collected at night, perhaps not *ecologically valid*.
:::
...
Best: second.
First is *leading*: *concerned* cat owners...
Third is *leading* (Do you *agree*...)
:::
...
Shape: skewed *left*.
Outliers: none, though 'bump' at lower ages.
:::
...
A pie chart inappropriate: more than one option can be selected.
:::
...
**5.** $14/40$; $0.35$.
**6.** $7/53$; about $0.132$.
**7.** $0.35/0.132$,; about $2.7$.
**8.** The odds of no August rainfall in Emerald $2.7$ times higher in months with non-positive SOI.
:::
...
Perhaps use medians and IQRs for summarising (mean and std dev. probably OK for `mTS`).
...
**1.** *Form*: starts straight-ish, but then hard to describe.
...
Relationship prob. linear... some observations at top right look different. 
Variation increase a bit as Age increases.
Some observations in top right seem to not follow the linear relationship.
:::
...
Between biomass and origin: *boxplot*.
Between biomass and other variables: *scatterplot*.
...
Average speed, accuracy vary by age but not sex.
...
**1.** Expect $100\times 0.99 = 99$ people to return a positive result.
**2.** Expect $900\times (1 - 0.98) = 18$ people to return a positive result.
...
Only $50$--$50$ chance if events equally likely... they are not.
:::
::::::
...
**2.** Assuming fair die, I would *not* expect a `r if (knitr::is_latex_output()) {
   '\\Largedice{6}'
} else {
...
It describes precision of $\bar{x}$ for estimating $\mu$
:::
::::::
...
**1.** $z = -0.30$; about $38.2$%.
**2.** $z = 0.07$; about $47.2$%.
**3.** The $z$-scores: $-0.67$ and $0.44$; $0.6700 - 0.2514 = 0.4186$, or about $41.9$%. (Diagram!)
**4.** $z$-score about $1.04$; tree diameter $x = 8.8 + (1.04\times 2.7) = 11.608$; about $11.6$\ inches.
:::
...
**1.** $z = -0.61$; $1 - 0.2709 = 0.7291$, or about $72.9$%.
**2.** $z = -1.83$; about $3.4$%.
...
Std error: $\text{s.e.}(\hat{p}) = 0.01309$; approx. $95$%\ CI: $0.819 \pm (2\times 0.0131)$.
Statistically valid.
:::
...
$\hat{p} = 8/154 = 0.05194805$; $\text{s.e.}(\hat{p}) = 0.0017833$; approx. $95$%\ CI: $0.0519\pm 0.0358$.
Statistically valid.
:::
...
```{r, fig.width=3, fig.height=1.5, out.width='30%'}
z <- seq(-3.5, 3.5, 
         length = 100)
mn <-  0.8194444
sdev <- 0.01308604
