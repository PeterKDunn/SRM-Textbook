...
$\text{s.e.}(\bar{x}) = 5.36768$, so the approximate 95% CI is $61.3\pm (2 \times 5.36768)$, which is $61.3\pm 10.74$s (or 50.56s to 72.04s).
...
Mean taste preference between preferring it better *with* dip by up to 11.2mm on the 100mm visual analogue scale, or preferring it *without* dip by a little (up to -0.9mm on the 100mm visual analogue scale.
...
**6.** Based on the sample, a 95% CI  for the difference between population mean one-step fall-recovery angle for heathy women is between $9.1$ and $19.9$ degrees *greater* for younger women than for older women (two independent samples).
:::
...
1. $H_0$: $p = 0.2$; $H_a$: $p > 0.2$.
1. One-tailed.
1. $\text{s.e.}(\hat{p}) = 0.044444$; so draw a normal distribution centred on $0.2$, with a standard deviation of $\text{s.e.}(\hat{p}) = 0.0444$.
1. $\hat{p} = 50/81 = 0.6173$; $z = (0.6173 - 0.2)/0.04444 = 9.39$.
   This is *very* large, so the $P$-value will be *very small*.
   There is very strong evidence to support the alternative hypothesis that people do better-than-guessing at identifying the placebo.
:::
...
$H_0$: $p = 0.5$ and $H_a$: $p > 0.5$ (which is *one*-tailed).
...
**1.** Either direction is fine: $\mu_Y - \mu_O$, the difference between the population mean lean-forward angle (younger women, minus older women).
**2.** One-tailed according to the RQ.
**3.** $H_0$: $\mu_{\text{Y}} - \mu_{\text{O}} = 0$ (or $\mu_{\text{Y}} = \mu_{\text{O}}$).
       $H_1$: $\mu_{\text{Y}} - \mu_{\text{O}} > 0$ (or $\mu_{\text{Y}} > \mu_{\text{O}}$).
4. $t = = 6.691$; the *two*-tailed $P$-value is $0.001$, so the *one-tailed* $P$-value is $0.001\div 2 = 0.0005$.
