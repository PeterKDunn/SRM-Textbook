...
Sample not random; the researchers (rightly) state results may not *generalise* to all hospitals.
Data only collected at night, so perhaps not *ecologically valid*.
:::
...
*Skewness makes no sense* (only for *quant.*\ variables).
:::
`r if( !includeEvenAnswers) "-->"`
...
Based on sample, sample proportion of koalas that crossed at least $1$\ road in previous $30$\ months is $0.353$ ($n = 51$), with approx. $95$%\ CI from $0.219$ to\ $0.487$.
Stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
Std\ error about population mean diameter, not individual trees.
:::
`r if( !includeEvenAnswers) "-->"`
...
Tests *not* about sample value (we know value of $\hat{p}$ in sample), but about unknown population value (i.e., $p$).
:::
...
Very strong evidence to support $H_1$: people do better-than-guessing at identifying placebo.
:::
...
**5.** Very strong evidence in sample ($z = -4.51$; one-tailed $P < 0.001$) that pop.\ proportion less than target of $p = 0.10$ (Korean sample proportion: $\hat{p} = 0.0566$; $n = 972$; approx. $95$%\ CI from $0.042$ to $0.071$).
...
Using normal-distribution tables, one-tailed $P$-value is $0.0281$; slight evidence of home-ground advantage in the pop.
:::
`r if( !includeEvenAnswers) "-->"`
...
$P$-value very small.
Strong evidence that proportion of females using machines lower than proportion of females in uni pop.
:::
...
$P$-value very small.
Strong evidence exists that majority like breadfruit pasta (for pop. represented by sample anyway).
:::
...
$P$-value very small.
Strong evidence exists proportion people with CTS with a PL tendon absent is different for people with CTS.
:::
`r if( !includeEvenAnswers) "-->"`
...
$\hat{p} = 0.139535$ and $\text{s.e.}(\hat{p}) = 0.018457$; $z = 26.3$: *massive*; $P$-value incredibly small.
...
$H_0$: $p = 0.5$; $H_1$: $p \ne 0.5$.
$\hat{p} = 0.5576923$; $\text{s.e.}(\hat{p}) = 0.06933752$, giving $z = -0.832$.
$P$-value 'large'.
...
**7.** Very strong evidence the mean speed of vehicles on road greater than $90$\kms.h^$-1$^.
**8.** Stat.\ valid.
:::
...
$H_0$: $\mu = 120$ and $H_1$: $\mu \ne 120$ (two-tailed), where $\mu$ is mean time *in secs*.
Std\ error: $\text{s.e.}(\bar{x}) = 2.581472$.
$t$-score: $-23.13$, *huge*; $P$-value *very* small.
...
$t$-score: $10.35$: *huge*; $P$-value very small.
...
SE: The variation in sample means for samples of size\ $67$.
:::
...
$H_0$: $\mu = 1000$ and $H_1$: $\mu \ne 1000$ ($\mu$ is pop.\ mean guess of spill volume).
Std\ error: $46.15526$.
...
$P$-values large, except for $2$nd test.
...
The difference between $0.0501$ and\ $0.0499$ trivial though.
:::
...
$P$-value *just larger* than $0.05$; 'moderate evidence' to support $H_1$.
$P$-value *just smaller* than $0.05$; 'strong evidence' to support $H_1$.
Diff between $0.011$ and\ $0.009$ trivial though.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** The conclusion about pop.\ **mean** energy intake.
...
**1.** *No* evidence of a diff in lifetime between two brands: $P$-value is 'large'.
**2.** No; diff is $0.29$\hs, or about $17$\mins.
A diff of $17$\mins in over $5$\hs of use: trivial.
**3.** Conclusion: no evidence of a diff between mean lifetimes; cumbersome for advertising!
Common advertising trick: 'No other battery lasts longer!'.
**4.** Price!
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** That pop. mean is $12$\ inches, as claimed.
Have no evidence to refute this.
**2.** *First*: *population* mean diameter is $\mu = 12$ inches; *sample* mean not $12$\ inches due to sampling variation.
*Second*: *population* mean diameter isn't $12$\ inches, reflected in sample.
**3.** $11.48$ is $0.52$\ inches from target of $12$; seems unlikely sample mean would be that far from $12$ inches through sampling variation alone.
...
**12.** No evidence ($t = -0.205$; two-tailed $P = 0.839$) of mean increase in weight of squash from dry to normal years (mean change: $2.230$\gs ($95$%\ CI from $-24.8$ to $20.3$\gs), heavier in normal year).
:::
...
**3.** $t = 1.699$; *approx.* one-tailed $P$-value between $16$% and\ $2.5$%; not sure if $P$ larger than $0.05$, but likely ($t$-score quite a distance from $z = 1$).
...
**2.** $t = 1.78$; $P$-value larger than $0.05$: evidence doesn't support $H_1$.
...
Test may not be stat.\ valid; histogram of data suggests population *might* have normal distribution); $P$-value so large probably makes little difference.
:::
...
**1.** $t = 2.48$; quite large; **one-tailed** $P$-value certainly less than $0.025$.
...
**3.** $t = 0.762$; $P$-value is large; no evidence of change; $-4.35$ to $9.71$\mins.
:::
...
$t = -0.175$; very large $P$-value; no evidence of increase.
:::
`r if( !includeEvenAnswers) "-->"`
...
**13.** $z = (-0.0033 - 0)/0.0832097 = -0.040$; very small $P$-value; no evidence of diff.
...
**13.** $z = (0.0377 - 0)/0.04611 = 0.818$; very small $P$-value; no evidence of diff.
...
Either of the others *may* be true.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** Stat.\ valid only if relationship approx. linear, and variation in STAI does not change for diff. levels of work experience.
$n \ge 25$.
:::
...
$P$-value *very* small; very strong evidence of correlation in pop.
:::
`r if( !includeEvenAnswers) "-->"`
...
**6.** $0.70$ to\ $1.4$\ ft.
**7.** Very strong evidence in sample ($t = 6.16$; $P = 0.0001$ (two-tailed)) that punting distance related to leg strength (slope: $1.0427$; $n = 13$).
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** *Slope*: an increase in bitumen weight by one percentage point *decreases* average percentage air voids by volume by $2.59$ percentage points.
*Intercept*: dodgy (extrapolation); in principle $0$% bitumen content by weight, percentage air voids by volume is about $17.47$%.
...
**13.** $-0.958 \pm (2\times 0.070)$, or $-0.958\pm 0.140$, or from $-1.098$ to $-0.818$.
...
Perhaps better: For students, is mean reading speed for text in Georgia font the same as for text in Calibri font?
**Abstract** statement poor (*fonts* are not fast or slow).
Perhaps: Sample provided evidence that mean reading speeds were different ($P = ???$), when comparing text in Georgia font (mean: ???) and Calibri font (mean: ???; $95$%\ CI for the diff: ??? to ???).
:::
...
No units of measurement; jump-heights given to $0.001$ of cm.; table could also summarise information for each individual jump type; numerical summary shouldn't include $P$-value, $t$-score, or CI.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Convenience; self-selected. 
       Those in study *may* record different accuracies than people not in study.
...
**2.** Observational.
**3.** A random method used: results should be generalisable to population (students at that uni).
**4.** Double-barrelled.
...
**11.** Yes.
**12.** *Null*: mean score same in all three groups (no diff').
...
**6.** Evidence of diff.
**7.** Probably, given standard errors rounded.
**8.** Conceptual.
**9.** Table not shown.
**10.** $\chi^2$.
...
**2.** No evidence of difference in survival rates at temps.
**3.** Evidence that surviving\ *Cx.* had larger mean size compared to surviving\ *Ae.*.
**4.** Two-sample $t$-test.
**5.** $0.010628$.
**6.** $t = 26.3$; very small $P$; very strong evidence of diff in mean lengths.
**7.** Yes.
**8.** Yes.
**9.** For *Cx.*, evidence mean sizes at the temps different; for *Ae.*, no evidence mean sizes at the temps different.
...
**15.** About $8.7$% of variation in predation efficient can be explained by value of the predator--size ratio.
**16.** $r = 0.294$.
:::
