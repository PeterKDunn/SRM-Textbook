...
Numerical information to be answered, such as the *average time* taken to apply the tourniquets.
**Quant**itative.
:::
...
**3.** Number of iron tablets per day.
:::
...
**2.** *Between*-intervals comparison.
       Outcome: average yield (measured in kg/plant, tomatoes/plant, kg/hectare, etc).
...
**Ex.\ \@ref(exr:RQsDogs).**
**1.** Correlational.
**2.** It makes no sense to assign one variable as explanatory, and another as response.
:::
...
<!-- **1.** Correlational. -->
<!-- **2.** Age probably impacts typing speed; age is explanatory variable, and typing speed the response variable. -->
<!-- ::: -->


::: {.answer data-latex=""}
**Ex.\ \@ref(exr:RQsBloodPressure)**:
**1.** P: Danish University students; O: *Average* resting diastolic blood pressure; C: between students who regularly drive to university and who regularly ride their bicycles.
**2.** No intervention.
**3.** Relational.
...
<!-- **3.** Hard to be sure since "nutritional status" is not well defined. Perhaps it is amount intake per day? 
<!-- Then, correlational probably. -->
...
**2.** Probably not.
**3.** How *individual* people are using their phones
(with levels 'Talking on the phone' and 'texting on the phone').
**4.** Walking speed.
...
The *population* is not $10$ adults; that is the sample.
Unclear how many fonts are being compared (or which fonts).
...
**1.** Japanese adults.
**2.** Between those who take Vitamin\ C tablets and those who do not.
**3.** *Average* cold duration'.
**4.** Duration of the cold symptoms for each person.
**5.** Whether or not each person take Vitamin\ C tablets or not.
**6.** Decision-making
**7.** One-tailed.
:::
...
**1.** Units of analysis: the person; unit of observation: individual nose hairs.
...
Unclear if intervention, but seems unlikely.
:::
...
<!-- ::: {.answer data-latex=""} -->
<!-- **Ex.\ \@ref(exr:InterventionalRQBetween).** -->
<!-- **1.** Relational; estimation. -->
<!-- **2.** Relational; decision-making. -->

<!-- There is an intervention (whether worms were or not).  -->
<!-- ::: -->
...
*Unit of observation*: the *tyre* (amount of wear is measured on tyres).
*Unit of analysis*: the *car*.
Tyre brand allocated to the car; each car gets only the same brand of tyre.
Tyres on any one car do not operate independently; they all 'live their life together': exposed to the same day-to-day use, same drivers, have driven almost identical distances, under the same conditions, etc.
...
**3.** Most likely.
**4.** Estimation.
**5.** Since an intervention: experiment.
       Either true or quasi, but most likely a true experiment.
:::
...
<!-- **3.** Yes. -->
<!-- **4.** Decision-making. -->
<!-- **5.** Expoerimental. True experiment. -->
<!-- ::: -->
...
Researchers decide which beams go into Group\ A and into Group\ B.
Researchers allocate treatments to the groups, and decide treatment applied to each group:
*true experiment*.
:::
...
<!-- **4.** Experimental. -->
<!--        Since the Program was given to paramedics in certain cities, this is quasi-experimental (researchers cannot tell paramedics where to work). -->
<!-- ::: -->
...
The researchers cannot decide who was in hospital at the time, so could not allocate patients to the groups (overlay; mattress): *quasi-experiment*.
:::
...
**3.** Experimental: diets have been *manipulated* and *imposed* by the researchers.
**4.** Probably a true experiment.
**5.** Individuals: the diets are allocated to individuals.
...
**1.** Every seventh day is same day of the week.
...
Stratified: zones are the strata.
:::
...
In Stage\ 2, selection of farms is *not* random.
:::


::: {.answer data-latex=""}
**Ex.\ \@ref(exr:NotClusterSampling).**
In Stage\ 1, selection of schools is *not* random.
:::
::::::
...
*Response*: something like 'risk of developing a cancer of the digestive system'.
*Explanatory*: 'whether or not the participants drank green tea at least three times a week'.
*Lurking*: 'health consciousness of the participants', because the researchers don't seem to have measured or observed this.
:::
...
**1.** A group receiving a pill like Treatment\ A and\ B, but has no effective ingredient.
**2.** Blinding participants.
**3.** To ensure that participants do not change their behaviour because of the treatment they were receiving.
:::
...
**1.** *Response*: amount of sunscreen used; *explanatory*: time spent on sunscreen application.
...
*Blinding*: students are not aware of which water they drink; in an observational study, probably infeasible.
*Double blinding*: neither the students nor the researchers know which type of water the students are drinking; probably infeasible.
...
### Chap.\ \@ref(Interpretation): Study design limitations {.unlisted .unnumbered}
...
External validity refers to whether the results apply to other members of *the target population*.
:::
...
Sample not random; the researchers are (rightly) state that results may not *generalise* to all hospitals.
...
Best: second.
First is *leading*: should *concerned* cat owners...
Third is *leading* also (Do you *agree*...)
:::
...
First seems fine, but 'seldom' (for instance) may mean different things to different people; possible recall bias.
...
*Kangaroo response*: qual. ordinal (or perhaps nominal?).
*Drone height*: quant.; but with just four values used, probably treated as qual. ordinal.
...
Most common social group has many females plus offspring.
...
A pie chart *not* be appropriate: respondents could select more than one option.
:::
...
**5.** Table not shown. <!-- See Table\ \@ref(tab:ReclaimedWaterPercent) for the table of percentages. -->
**6.** $40/(231 - 40) = 0.209$.
**7.** $40/(12 + 21) = 1.21$.
:::
::::::
...
The proportion that drank various things, among those who did and didn't vomit.
...
**1.** *Proportion* of F skipped breakfast: $\hat{p}_F = 0.359$;
**2.** *Proportion* of M skipped breakfast: $\hat{p}_M = 0.284$.
**3.** $\text{Odds}(\text{Skips breakfast, among F}) = 0.5598$;
**4.** $\text{Odds}(\text{Skips breakfast, among M}) = 0.3966$.
**5.** *Odds ratio*: $0.5598/0.3966 = 1.41$.
**6.** Odds of females skipping breakfast are $1.41$ *times* the odds of males skipping breakfast.
...
Perhaps best to use medians and IQRs for summarising (mean and standard deviation probably OK for `mTS`).
...
**1.** *Form*: starts off straight-ish, but then seems hard to describe.
...
Relationship probably linear... a few observations at the top right look a bit different. 
Variation increase a little as the Age increases.
...
Individual variables: *bar chart* for the origin,; *histogram* for the others.
...
Graph is *inappropriate*: both variables are *qualitative*.
...
Variable is the 'Sport' (*qual. nominal*).
...
*Accuracy* and *Age*: hard to see the relationship; no older people are very slow.

Average speed and accuracy seems to vary by age but not sex.
Ask: 
How were data collected (self-reported, or measured how?). 
How were students obtained: a random, somewhat representative or self-selecting sample?
:::
::::::
...
False; true; it depends on the first card.
:::
...
**1.** Yes! Problem seems likely (we can't be sure).
**2.** Assuming fair die, I would *not* expect to get a `r if (knitr::is_latex_output()) {
   '\\Largedice{6}'
} else {
...
:::::: {.ChapAnswers data-latex=""}
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:StdErrorOrStdDeviation)**:
...
The *standard error of the mean* describes how the sample mean is likely to vary from sample to sample.
Or, it describes how precisely the sample mean estimates the (unknown) population mean.
:::
::::::
...
**1.** $z = 0.2962$, or $z = -0.30$.
From tables, prob. is $0.3821$, or about $38.2$%.
**2.** $z = 0.07$; prob. is $1 - 0.52379 = 0.4721$, or about $47.2$%.
**3.** The $z$-scores are $z_1 = -0.67$ and $z_2 = 0.44$; the prob. is $0.6700 - 0.2514 = 0.4186$, or about $41.9$%. (Draw a diagram!)
**4.** Using the tables 'backwards': $z$-score is about $1.04$; corresponding tree diameter is $x = 8.8 + (1.04\times 2.7) = 11.608$; about $11.6$\ inches.
...
**1.** $z = -0.6097561$, or $z = -0.61$. 
Using tables: prob. *less than* this value is $0.2709$, so $1 - 0.2709 = 0.7291$, or about $72.9$%.
**2.** $z = (37 - 40)/1.64 = -1.83$; prob. is $0.0336$, about $3.4$%.
**3.** The two $z$-scores: $z_1 = -4.878$ and $z_2 = -1.83$. 
Drawing a diagram, prob. is $0.0336 - 0 = 0.0336$, or about $3.4$%.
...
An IQ score lower than about $80.8$ leads to a rejection.
:::
...
Std error: $\text{s.e.}(\hat{p}) = 0.01308604$; approximate 95%\ CI is $0.8194444 \pm (2\times 0.01308604)$.
Statistically valid.
:::
...
$\hat{p} = 8/154 = 0.05194805$; $\text{s.e.}(\hat{p}) = 0.0017833$; approx. $95$%\ CI is $0.05194 \pm (2 \times 0.0017833)$, or $0.0519\pm 0.0358$, equivalent to $0.016$ to $0.088$.
Statistically valid.
:::
...
The *approx.* $95$%\ CI is from $0.219$ to $0.263$.
:::
::::::
...
**4.** Approx. $95$% confident the *population mean* weight of male American black bears is between $77.4$ and $92.4$\ kg.
**5.** Statistically valid ($n$ much larger than $25$.
...
Std error: $\text{s.e.} = s/\sqrt{n} = 0.43/\sqrt{45} = 0.06410062$.
*Approx.* $95$%\ CI: $2.85 \pm(2\times 0.06410062)$, or $2.85\pm 0.1282012$, or from $2.72$\ L to $2.98$\ L.
:::
...
Approx. $95$%\ CI for the mean brushing time: $29.9$\ s to $36.1$\ s.
:::
...
<!-- **3.** The sample size is much larger than $25$; the CI is statistically valid. -->
...
*None* are acceptable.
**1.** CIs are not about how individual observations vary; they are about how a *statistic* varies.
In addition, CIs are about populations and not samples.
**2.** CIs are not about how individual observations vary; they are about how a *statistic* varies.
...
**6.** We know *exactly* what the sample mean is ($\bar{x} = 1.3649$\ mmol/L: Why give an interval?
**7.** We know *exactly* what the sample mean is ($\bar{x} = 1.3649$\ mmol/L: Why give an interval?
:::
...
$\text{s.e.}(\bar{x}) = 5.36768$; approx. $95$%\ CI is $61.3\pm (2 \times 5.36768)$, which is $61.3\pm 10.74$s (or $50.56$\ s to $72.04$\ s).
Statistically valid.
:::
::::::
...
*Approx.* $95$%\ CI: $5.2 \pm (2\times 3.06)$, or $5.2\pm 6.12$, from $-0.92$ to $11.22$.
(Understanding how the *differences* are defined is needed to know where this came from.)
...
**1.** *Differences* defined as *during* minus *before*, so *positive* differences means the *during* value is higher.
...
The CI is for the population mean *increase* during compared to before lockdown, so a negative value means that the *before* values are higher than the *during* values on average. 
The CI means: 'In the population, the mean difference between the amount of vigorous PA by Spanish health students is between $4.35$\ mins more *during* lockdown, and $9.71$\ mins more *before* lockdown.'
:::
::::::
...
**1.** Define the *difference* as the mean length of female *minus* the mean length of male gray whales. 
(Either direction is fine, but be consistent.)
...
**5.** Sample sizes are large; the CIs statistically valid.

The difference between the means is an average of $0.53$ days; about half a day (quicker on echinacea). 
Probably not practically important when a cold last for almost seven days.
:::
...
**2.** The approx. CI is $-0.00162$ to $0.00562$\ m/s.
...
**4.** Approx. CI: (No answer yet). Exact CI: From the *second row* of the output: $9.10$ to $19.90$.
...
**7.** All *expected* counts are larger than five; the CI is statistically valid:
:::
...
*Or*: the odds are the odds of *not* having an ear infection; OR compares these odds for beach swimmers to non-beach swimmers.
:::
...
Numerical summary: Table\ \@ref(tab:BirdsNumericalSummary) (some parts computed *manually* from the data).
Graphical summary: not shwon <!-- (Fig.\ \@ref(fig:BirdsGraphs)). -->
...
The CI is statistically valid as all expected counts exceed five.
:::
::::::
...
:::::: {.ChapAnswers data-latex=""}
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:SampleSizeProp1)**:
**1.** Use $n = 1/0.04^2 = 625$; at least $625$.
**2.** Use $n = 1/0.02^2 = 2500$; at least $2500$ (i.e., *four times as many*).
**3.** Use $n = 1/0.01^2 = 10,00$; at least $10\ 000$ (i.e., *sixteen times as many*.
...
Use $s = 0.43$.
**1.** Use $n = \left( (2\times 0.43)/0.02 \right)^2 = 1849$; at least $1849$.
**2.** Use $n = 295.85$; at least $296$.
**3.** Use $n = 73.96$; at least $74$.
...
**3.** One-tailed.
**4.** $\text{s.e.}(\hat{p}) = 0.044444$; draw a normal distribution centred on $0.2$, with a std deviation of $\text{s.e.}(\hat{p}) = 0.0444$.
**5.** $\hat{p} = 50/81 = 0.6173$; $z = (0.6173 - 0.2)/0.04444 = 9.39$.
This is *very* large; $P$-value will be *very small*:
...
Very large $z$-score; $P$-value will be very small.
...
Very large; $P$-value will be very small.
...
$H_0$: $p = 0.5$ and $H_1$: $p \ne 0.5$.
Use $n = 52$ and so $\hat{p} = 29/52 = 0.5576923$ and $\text{s.e.}(\hat{p}) = 0.06933752$, giving $z = -0.8320503$, which is small; the $P$-value will be 'large'.
No evidence to suggest that choice in non-random.
:::
::::::
...
Very strong evidence ($t = 9.46$; one-tailed $P < 0.001$) that the mean speed of vehicles on this road (sample mean: $96.56$ (approx. $95$%\ CI: $95.17$ to $97.95$); standard deviation: $13.874$) is greater than $90$\ km.h^-1^.
:::
...
<!-- ::: {.answer data-latex=""} -->
<!-- **Ex.\ \@ref(exr:TestOneMeanExercisesQualityOfLife)**: -->
...
The answers are *half* the values given to **Ex.\ \@ref(exr:MoreAboutExercisesApproximatingPValues)**.
...
$P$-value is *just larger* than $0.05$; 'slight evidence' to support $H_1$.
$P$-value is *just smaller* than $0.05$; 'moderate evidence' to support $H_1$.
...
**1.** The conclusion is about the population **mean** energy intake.
...
$t = (5.2 - 0)/3.06 = 1.699$; the *approx.* one-tailed $P$-value (using the $68$--$95$--$99.7$ rule) is between $16$% and $2.5$%. 
...
Since $n < 25$, the test may not be statistically valid (the histogram of data (not shown) suggests that the population *might* have a normal distribution), though the $P$-value is very large so it probably makes little difference.
:::
::::::
...
**1.** The mean length of female *minus* the mean length of male gray whales.
(Either direction is fine,, but be consistent.)
**2.** $H_0: \mu_F - \mu_M = 0$ and $H_1: \mu_F - \mu_M \ne 0$.
**3.** $t = [(\bar{x}_F - \bar{x}_M) - (\mu_F - \mu_M)]/{\text{s.e.}(\bar{x}_F - \bar{x}_M)} = (0.06 - 0)/0.0929 = 0.65$.
*Very* small $t$-score; the $P$-value will be very large.
...
Missing entries: Odds: $1.15$; Percentage: $58.1$%.
...
From software: $\chi^2 = 22.374$, approx. equivalent to a $z$-score of $\sqrt{22.374/1} = 4.730$, which is very large.
Hence, a small $P$-value.
...
**2.** $P$-value suggests no evidence of a linear relationship.
**3.** No scatterplot provided, so the test is statistically valid only if the relationship is approximately linear, and if the variation in STAI scores does not change for different levels of work experience.
...
$r$ is not relevant: relationship non-linear.
...
**4.** From output: test statistic is $t = -7.45$, which is very large;  two-tailed $P$-value is very small: $P < 0.001$.
:::
...
No units of measurement given; jump-heights are given to $0.001$ of a centimetre... which seems rather optimistic; table gives information for the *differences*, which is great but it could also provide numerical summary information for each individual jump type too; a numerical summary shouldn't include a $P$-value, $t$-score, or confidence interval.
:::
...
Variables are *qualitative*, so means inappropriate; appropriate summary is an odds ratio, so the values almost certainly refer to the CI for the OR.
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:WriteExerciseBaughman)**:
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:ReadExerciseSoftDrinks).**
No answer (yet).
:::
::::::
