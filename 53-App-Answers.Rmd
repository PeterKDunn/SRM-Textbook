...
**2.** 1. The parameter: $\mu_F - \mu_M$ (where $F$ and $M$ represent female and male gray whales respectively).
The estimate: $\bar{x}_F - \bar{x}_M = 4.66 - 4.60 = 0.06$.
...
```{r WhalesErrorbar, echo=FALSE, fig.cap="Error bar chart comparing the lengths of female and male gray whales", fig.align="center", fig.width=4.5, fig.height=3.5}
...
With $n = 172$, find $\hat{p} = 24/172 = 0.1395349$ and $\text{s.e.}(\hat{p}) = 0.01845701$ and so $z = 26.3$, wich is *massive*; the $P$-value will be incredibly small.
...
$H_0$: $\mu_D = 0$ and $H_1$: $\mu_D > 0$, provided we define the difference as the beta-endorphin concentration 10 minutes before surgery, minus the concentration 12--14 hours before surgery.
...
Conclude (where the CI was [found earlier](#StressSurgery)) that strong evidence exists in the sample (paired $t = 2.48$; one-tailed $P = 0.0115$) of a population mean increase in beta-endorphin concentrations from 12--14 hours before surgery to 10 minutes before surgery (95% CI: 1.62 to 13.78 fmol/mol.)
...
**3.** $t = [(\bar{x}_F - \bar{x}_M) - (\mu_F - \mu_M)]/{\text{s.e.}(\bar{x}_F - \bar{x}_M)} = (0.63 - 0)/0.07079 = 8.90$.
This is a *very* large $t$-score, so the $P$-value will be very small.
**4.** `Very strong evidence ($t = 8.90$; two-tailed $P < 0.001$) exists in the sample that the mean length of adult gray whales is different in the population for females (mean: 12.70&nbsp;m; standard deviation: 0.611&nbsp;m) and males (mean: 12.07&nbsp;m; standard deviation: 0.705&nbsp;m; 95% CI for the difference: 0.48&nbsp;m to 0.77&nbsp;m).
'
