...
The second question has options that overlap: Both $1$\ h and $2$\ h are in two categories.
:::
...
```{r CPalsyPlots, fig.align="center", fig.cap="Some graphs from the cerebral palsy data", fig.width=7, fig.height=2.25, out.width='100%'}
NumP <- 15
Gender <- rep("M", 
              NumP)
...
What do the plotting symbols mean?
Axis labels not helpful.
The vertical axis goes to $35$, but could stop at $20$.
See Fig.\ \@ref(fig:MinersGoodCrabs) (left panel).
:::

```{r MinersGoodCrabs, fig.cap="Left: The number of noisy miners and the number of eucalyptus trees. Right: The colour of female horseshoe crabs and the condition of their spines. There are no missing values", fig.align="center", out.width='100%', fig.width=9.25, fig.height=2.75}
