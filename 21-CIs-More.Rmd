...
Similarly, most people interpret a CI as an interval with a certain chance of containing the value iof the population parameter, even though it is technically incorrect.
...
In addition, CIs also require that the sample size is less than 10% of the population size; this is almost always the case.
...
1. What is the missing word: A CI gives an interval in which we are fairly sure that the value of the ______ is within.
`r if( knitr::is_html_output(exclude = "epub") ) {longmcq( c(
           "Sample statistic",
