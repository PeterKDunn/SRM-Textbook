...
The paper states that participants
...
3. Why did the authors discuss the changes in sample size for some parts of the study?
...
7. While walking at $2.5$\ km.h^-1^, the above test resulted in $t = 2.95$.
...
@mohammadpoorasl2018prevalence studied the relationship between hearing loss, and headphone and earphone use among Iranian students, using a non-directional study.
...
5. Compute the $95$% confidence interval for the proportion of students who had used earphones. (>>>>>>>>>>>>>>>745/866)

Some of the results are presented in Table\ \@ref(tab:HearingLossTable), including a comparison of the mean HLQ scores for males and females.
...
13. What is the sample size for this comparison only $791$ and not $845$?
...
7. The research give the (two-tailed) $P$-value for this test as $0.001$.
...
13. What limitations can you identify for this study?
:::
...
4. The $P$-score for testing the slope is given as $<0.001$. 
...
6. What hypotheses are being tested by the table?
7. The article states that $\chi^2 = 31.5$.
   What is the equivalent $z$-score for the test?
8. What is the approximate $P$-value for the test?
9. Interpret what this means in this context.
:::
...
if( knitr::is_latex_output() ) {
  kable(pad(seleniumTab,
        ...
        escape = FALSE,
        align = "c",
        caption = "Number of wells with dissolved Se above $2$ $\\mu$g.L$^{-1}$, and the geology within $5$\ km radius of the well"
  ) %>%
...
5. Would the test be statistically valid?

The length of the surviving larvae from both species were compared for the two temperatures also (p.\ 5):

> Within *Cx. pipiens*, surviving control larvae were significantly larger from replicates held at $15$^o^C, relative to those from $25$^o^C ($\text{mean} \pm \text{SD}$: $15$^o^C${} = 1.66 \pm 0.01$\ mm, $25$^o^C${}= 1.60 \pm 0.02$\ mm; $\text{$p$-value} = .0065$). For *Ae. albopictus* surviving control larvae, there was no significant difference in length due to temperature ($\text{mean}\pm\text{SD}$: $15$^o^C${}= 1.35 \pm 0.01$\ mm, $25$^o^C${} = 1.36 \pm 0.01$\ mm; $\text{$p$-value} = .4343$).

6. How would these results be interpreted?
7. What type of test would probably have been used?

The linear association between predation efficiency ($y$; no units) and predator--prey size-ratio ($x$; no units) was found (using $n = 45$) to be    
\[
   \hat{y} = -19.56 + 31.64x.
\]
The standard errors of the two regression coefficients were $17.92$ (intercept) and $13.88$ (slope).

8. Find an approximate $95$% confidence interval for each regression parameter.
9. Estimate the $P$-value for testing if the population slope is zero or not. 
   Interpret what this means.
10. Is this test statistically valid?
11. Interpret the meaning of the slope.
12. The value of $R^2$ was given as $0.087$ (i.e., $8.7$%); interpret this value,
13. Find the value of the correlation coefficient, $r$.
:::
...
The researchers (p.\ 1100):

> [...] sampled tomato fields between April and Sep- tember in 1989 and 1990, using a stratified random design [...] 
> Each of these sampling areas was divided into $20$ sections, and $1$\ m of the tomato row was selected randomly within each section...
p.\ 1103:

1. Explain *why* the sampling is stratified, but also convenience.

One important measure of the soil health was the number of actinomycetes.
Researchers found (p.\ 1103):
...
Thus, two groups were used: one using eye-masks and earplugs (treatment group, T; $n = 29$) and one using headband (control or placebo group, P; $n = 27$).

1. What was the purpose of using 'sham' headbands?
2. What type of study is this: experimental or observational?
   Explain.

Sleep duration was measured in Week\ 1 (no intervention) and again in Week\ 2 (with the allocated intervention) using a 'wrist actigraphy monitor'.
...
6. Is the result statistically valid?

Another comparison was the room 'condition': in the treatment group, $13$ had a room with a fan (the others had air conditioning), while in the control group $10$ women had a fan (and $17$ air conditioning).
The $P$-value for the comparison was given as $P = 0.60$.

7. *Why* was this comparison made?
8. Construct the two-way table summarising the data.
9.  What do these results mean?
10. Is the result statistically valid?
...
11. Conduct the appropriate statistical test to determine if sleep duration *increased* in the treatment group.
12. What do these results mean?
...
13. Conduct the appropriate statistical test to determine if sleep duration *increased* in the treatment group.
14. What do these results mean?
15. What could explain the *increase* in sleep duration, even though the control group was using an ineffective intervention? 

The *increase* in sleep duration can be compared for the two groups.

16. Compute the standard error for difference between the mean increases $\text{s.e.}( \bar{x}_T - \bar{x}_T)$.
17. Construct an approximate $95$% confidence interval for the *difference* in the increase in sleep duration for the two groups.
18. Conduct a hypothesis test to compare the increase in sleep duration for the two groups.
    Interpret what this means.
