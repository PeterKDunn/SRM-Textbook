...
For example, *explanatory variables* can be called, among other terms, *independent variables*, *predictors*, *regressors*, *covariates* [@dunn2016learning].
Check the terminology being used if unsure!
:::


The six steps of the research process (Sect.\ \@ref(SixStepsOfResearch)) can be used as a guide to critiquing the research:\index{Research!six steps}
...
>
> The mean total PSQI score before intervention was $6.83\pm 2.73$ (bad), while after the intervention the same score was statistically significant reduced to $3.93 \pm 1.68$ (good)...  
>
...
* *Asking* the RQ:
  ...
  * The *units of analysis* and *observation* are the individual students in the study.\index{Units of analysis}\index{Units of observation}
    There are two units of observation for each unit of analysis: each person has two measurements (before and after using the 'intervention').
...
* *Designing* the study:
  ...
  * The sample size is not given (which is unusual for an *Abstract*).
* *Analyse* the data:
...
* *Report the results*:
  ...
  * No $P$-values or confidence intervals for the change are given (unusual for an *Abstract*).

Reading more of the article obviously provides more information.

### Details

The *population* comprises students of medicine from the University of Nis, Helsinki aged $20$ to $22$ (p.\ 336).
The results will only apply to these people.
Other students of a similar age are probably not obviously different in this regard; however, the results may not apply to older people.

The *inclusion* criteria are given as 'owning and daily usage of mobile phone with Android operating system and AMOLED screen' (p.\ 336).\index{Inclusion criteria}
This suggests that results may not apply to users of iPhones.
The *exclusion* criteria are given as 'sleep disorders, usage of sedative drugs, psychoactive substances, usage of phone apps or glasses that reduce blue light during the night, recent stressful situations'(p.\ 336).\index{Exclusion criteria}\index{Inclusion criteria}


The *intervention* is the use of a 'free Android application Twilight [...] on mobile phones of participants [which] automatically decreases brightness and content of emitted blue color from the screen during the night time' (p.\ XXXXXXXXXXX).
...
An excellent case-profile plot is shown of the data (Fig.\ 1), and the data usefully summarised in Table\ 2. 
...
That is, the *Hawthorne effect* may impact the results, and this is difficult to detect since a control group was not used.
...
1. The best way to read an article is thoroughly, from start to finish.\tightlist
`r if( knitr::is_html_output() ) {torf( FALSE)}`
1. The six steps of research are a useful guide for critiquing an article.
`r if( knitr::is_html_output() ) {torf( TRUE) }`
1. Critiquing an article means to only find the problems.
`r if( knitr::is_html_output() ) {torf( FALSE)}`
:::
