...
@lian2024effect studied alleviating post-operative thirst experienced by patients admitted to the intensive care unit, by comparing standard procedures with the use of ice-water spray.
To use random allocation of patients to the two groups, the researchers used random numbers:

> Subjects were sequentially assigned unique numbers from\ $1$ to\ $56$ according to their admission order. 
> [...] consecutive two-digit numbers were read from the random number table's rows and columns, generating random values that were matched with the respective admission numbers [...]
> Serial numbers\ $1$ to\ $28$ represented the control group, while serial numbers\ $29$ to\ $56$ constituted the experimental group.
:::
...
For this reason, the researchers used trees on which water was applied as the control group.
This is a better control, since trees in both groups  (faeces, water) had been approached by humans.
Now, if a difference was found between the faeces and water-sprayed trees, this explanation has been eliminated.
:::
...
Hence, the researchers may decide to use an *inclusion criteria*,\index{Inclusion criteria} restricting the study to people aged from\ $30$ to\ $50$.
...
For instance, coeliacs have an autoimmune disorder which results in an intolerance to gluten (found in wheat, barley and rye).
...
Exclusion and inclusion criteria clarify which individuals are explicitly included or excluded from the population for the purposes of the study, and should be explained when their purpose is not obvious.
...
That is, each person is paired with another person of the same sex and of a similar age; one of the pair is given the *Himalaya\ 292* diet, and the other is given the refined cereal diet. 
...
:::{.exampleExtra  data-latex=""}
An experiment to study the effect of using gingko to enhance memory [@data:Solomon2002:Gingko] compared two groups: one using gingko ($n = 111$), and one using a fake, non-active supplement ($n = 108$).
gThe authors randomly allocated participants to each group, then compared the two groups to ensure that no obvious differences initially existed between the groups that might explain differences in the response variable (Table\ \@ref(tab:GinkoDemographics)).
...
```{r GingkoDemographics}
Gingko <- array( dim = c(3, 3) )
colnames(Gingko) <- c("Characteristic", 
                      "Group A (Gingko)", 
		                  "Group B (Fake)")

Gingko[1, ] <- c("Average age (in years)", 
                 "68.7", 
                 "69.9")
Gingko[2, ] <- c("Men (number; percentage)", 
                 "46 (41%)", 
		             "45 (42%)")
Gingko[3, ] <- c("Average years of education", 
                 "14.4", 
		             "14.0")
...
if( knitr::is_latex_output() ) {
  kable(Gingko,
        ...
        caption = "Comparing the two groups in the gingko-memory study.",
        align = c("r", "c", "c"))   %>%
...
The researchers also recorded extraneous variables such as 'whether the farm was organic', 'elevation of the orchard' and 'whether general fungicides were used'.
...
For instance, in an observational study where subjects' blood pressure is measured, subjects clearly know they are being observed, which has the potential to alter the subjects' behaviour (for example, people become tense, called 'white-coat hypertension').
...
Using masks and hand sanitizer were recommended by the Danish Health Authority, but the adherence to the safety measures were very different.
...
To manage the placebo effect, researchers should record *objective*\index{Response variable}\index{Data!objective} data rather than patient-reported outcomes when possible [@enck2013placebo].
...
Note that the outcome is subjective:\index{Data!subjective} a *patient*-reported outcome.\index{Response variable!subjective}
:::
...
Researchers were assessing the relaxation percentage of the students while undertaking the compressionfdoible bls (a relaxation percentage of about\ $50$% is ideal).
...
Each of the paramedics began the series of tasks at a random task, to mitigate the carry-over effect.
A washout period between tasks (i.e., a rest time) was also used.
:::


The carry-over effect also is a potential concern to internal validity in observational studies involving a within-individuals comparison.
...
In observational studies, blinding individuals *may* be (but not always) easier than in experimental studies (Sect.\ \@ref(HawthorneEffect)).
...
Well-designed studies manage the *Hawthorne effect* (e.g., by blinding *participants*); the *observer effect* (e.g., by blinding the *researchers*); the *placebo effect* (experimental studies only; e.g., by using controls, objective outcomes and blinding subjects); and the *carry-over effect* (e.g., by using a washout, or randomly allocating the treatment order).
...
@doosti2016development wanted to determine the relationship between the depth of bruising on apples and the size of the impact force.
...
`r if( knitr::is_html_output() ) {torf( TRUE)}`
6. The *carry-over* effect is likely to be a big problem in this study.  
...
The researchers also recorded the size of the hive, among other things.
...
In Study\ B, the farmers were blinded to which seeds were new and which were traditional (both seed were the same size and colour), and the researchers interacting with the farmers also did *not* know the types of seed allocated.
...
@greier2021objective studied $36$\ Austrian $8$th\ grade students, recording the time spent performing moderate-to-vigorous physical activity (<span style="font-variant:small-caps;">mvpa</span>).
...
> The participants and the field researchers were blinded to the group status of the participants [...] the cleaning was done in the evening after the employees had left the building.
