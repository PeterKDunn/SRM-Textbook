...
This is true for identified potential confounders (such as age), and also for variables *not* even considered as confounders, or variables that are hard to measure or observe (such as genetic conditions).
...
These variables are a type of *control variable* (Def.\ \@ref(def:ControlVariables)).\index{Confounding!control variables}
...
Using masks and hand sanitiser were recommended by the Danish Health Authority, but the adherence to the safety measures were very different.
...
Since faecal weight was not measured until the *end* of the four-week periods, the carry-over effect is essentially irrelevant.
:::
