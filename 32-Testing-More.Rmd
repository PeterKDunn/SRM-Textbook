...
**Statistical hypotheses**\index{Hypotheses!statistical} refer to statements made about a parameter to explain sample statistics.
...
The alternative hypothesis $H_1$ (or $H_a$) offers another possible reason why the value of the statistic (such as the sample proportion) is not the same as the proposed value of the parameter (such as the population proportion): the value of the parameter really is not the value claimed in the null hypothesis (but does not explain *why*).
...
That is, does the data provide compelling evidence to stop supporting the null hypothesis (and support the alternative hypothesis)?
...
* Null hypothesis always have the form 'no difference, no change, no relationship' (i.e., sampling variation explains the difference between the parameter and statistic).
...
(Many people think the threshold should be much smaller.)
...
However, in truth, the mean internal body may not have changed, and it still $37.0^\circ\text{C}$ (i.e., the null hypotheses actually is true).
...
Of course, since the value of $\mu$ is unknown, we do not know if we have made a Type\ I error.
:::
...
This *does not* mean there is evidence that the mean IQ is\ $100$.
