...
That is, we retain the null hypothesis unless compelling evidence exists to change our mind (and hence adopt the alternative hypothesis).
...
* there is *no change* in the value of the parameter compared to an established or accepted value (for descriptive RQs), such as in the body-teperature example in Chap.\ \@ref(TestOneMean);
...
* Null hypothesis always have the form 'no difference, no change, no relationship' (i.e., sampling variation explains the difference between the values of the parameter and statistic).
...
(for example, for odds ratios in Chap.\ \@ref(TestsOddsRatio)).
...
Some software (such as SPSS) always reports two-tailed $P$-values.\index{Computers and software!statistical}
:::
...
Some software (e.g., SPSS) reports very small $P$-values as '$P = 0.000$' (i.e., zero to three decimal places).
...
For the body-temperature example (Chap.\ \@ref(TestOneMean)), the conclusion was that the sample provided very strong evidence that the population mean body temperature was *not* $37.0$^o^C (i.e., very strong evidence to reject the null hypothesis).
...
The sample we studied was just highly unusual.
...
This *does not* mean there is evidence that the mean IQ for football players is\ $100$.
...
This means that the difference in mean weight loss between the placebo and intervention groups was unlikely to be explained by chance ($P < 0.001$; i.e., 'statistical significant'), but the difference was so small that it was unlikely to be of any use in practice ('practical importance').
...
If the statistical validity conditions are *not* met, the $P$-values (and hence conclusions) may be inappropriate.
