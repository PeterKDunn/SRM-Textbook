...
* Describe the *expectations* of the statistic (Sect.\ \@ref(ExpectationOf)): describe what value of the *sample* proportion $\hat{p}$ could reasonably be expected from a fair die in $50$\ rolls.
* Take the sample *observations* (Sect.\ \@ref(Observation)): roll the die $50$\ times to find a value of $\hat{p}$.
...
showDecisionMaking(populationText = expression( atop(bold(Assume)~the~die,
                                                     is~fair*":"~italic(p)=="1/6")),
                   expectationText = expression(atop(bold(Expect)~to~find~the,
                                                     value~of~hat(italic(p))~near~"1/6")),
...
(ref:DieRollStdError) The sampling distribution is an approximate normal distribution; it shows a model of how the proportion of rolls showing a `r if (knitr::is_latex_output()) {
   ...
   '<span class="larger-die">&#9856;</span>'
}` varies, when a die is rolled $50$ times. The cross represents the observed sample proportion, $\hat{p} = 0.38$.
...
```{r PvaluesAnimation, animation.hook="gifski", dev=if (is_latex_output()){"pdf"}else{"png"}, fig.height = 3}
zList <- c( seq(0.5,
                1,
                by = 0.1),
            seq(1, 3.5, 
...
`r if (knitr::is_latex_output()) {
   ...
   '<span class="larger-die">&#9856;</span>'
}` (Fig.\ \@ref(fig:DecisionFlowDieRoll)):
...
```{r DecisionFlowDieRoll, fig.cap = "The decison-making process for the die-rolling data.", fig.align="center", out.width='100%', fig.width = 9.5, fig.height = 4}  
source("R/showDecisionMaking.R")
showDecisionMaking(populationText = expression( atop(bold(Assumption)~about,
                                                     population*":"~italic(H)[0]*":"~italic(p)==1/6) ),
                   expectationText = expression(atop(hat(italic(p))~has~normal~distn*";",
                                                     mean==1/6*";"~s.e.==0.05270) ),
                   oneSampleText = expression( atop(Sample,
                                                    italic(n)==50) ),
                   compareText = expression(atop(Assess~consistency*":",
                                                 italic(P)*"-"*value~bold(very)~small)),
                   oneStatisticText =  expression( hat(italic(p))==0.38),
