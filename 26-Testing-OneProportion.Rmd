...
## The expectations of $\hat{p}$: the sampling distribution of $\hat{p}$ {#SamplingDistributionKnownpHT}
...
The mean of this distribution is called the *sampling mean*, and the standard deviation for this sampling distribution is called the *standard error*, denoted\ $\text{s.e.}(\hat{p})$ (see Fig.\ \@ref(fig:NormalDieTheoryHT)).

More specifically, the *values* of the mean and standard deviation of the normal distribution
`r if (knitr::is_latex_output()) {
   'in Fig.\\ \\@ref(fig:RollDiceHistFigHT)'
} else {
   'the animation above'
}`
can be determined:
...
* The standard deviation, called the *standard error* $\text{s.e.}(\hat{p})$, has the value\ $`r round(se.die, 5)`$.
  (The source of this number will be revealed later; see Eq.\ \@ref(eq:StdErrorExampleDieHT).)
...
```{r NormalDieTheoryHT, fig.cap="(ref:DieRollStdError)", fig.align="center", fig.width=8.25, fig.height=3.25, out.width='100%'}
pop.p <- 1/6
n <- 50
...
\end{equation}
This standard error is the standard deviation of the normal distribution in Fig.\ \@ref(fig:NormalDieTheoryHT).


## The observations and the decision {#TestpObsDecision}

Figure\ \@ref(fig:NormalDieTheoryHT) show what values of the sample proportion $\hat{p}$ are expected when I roll a fair die.
...
Comparing this value of $\hat{p}$ to the sampling distribution in Fig.\ \@ref(fig:NormalDieTheoryHT) shows that a sample proportion of $\hat{p} = 0.38$ is *highly* unusual from a fair die with $p = 1/6$.
...
and zero to four decimal places (Fig.\ \@ref(fig:RollDiceHistFigHT)).
