...
The sampling distribution will always be approximately a normal distribution when certain conditions are met (Sect.\ \@ref(ValidityProportions)).
The mean of this distribution is called the *sampling mean*, and the standard deviation for this sampling distribution is called the *standard error*, denoted\ $\text{s.e.}(\hat{p})$ (Fig.\ \@ref(fig:NormalDieTheoryHT)).
...
varies, when a die is rolled $50$ times.
The cross represents the observed sample proportion.
...
More generally, the sampling distribution can be described in the next sections.
...
This is called the *null hypothesis*,\index{Hypotheses!null}, by $H_0$:
$$
  \text{$H_0$:\ } p = 1/6.
$$
...
This measures how unusual the calculated $z$-score is.
...
* sample summary information (see Chap.\ \@ref(CIOneProportion)), summarising the data used to make the decision.
  This statement usually includes a confidence interval for the parameter.
...
A discrepancy exists between the sample proportion ($\hat{p} = 0.8222$) and the sample proportion $p = 0.50$.
...
> *Very* strong evidence exists in the sample ($P < 0.001$; $z = 4.325$) that the interactions were not won equally by each species ($\hat{p} = 0.8222$ won by green-backed tits; approx.\ $95$% CI: $0.708$ to\ $0.936$; $n = 45$) in the population.
...
1. True of false: We have evidence to support the alternative hypothesis in this example.
`r if( knitr::is_html_output() ) {torf(answer=TRUE)}`
...
The expectation was that $1$-in-$16$ of the second-generation boreres would be resistant if the field borers were resistant.
