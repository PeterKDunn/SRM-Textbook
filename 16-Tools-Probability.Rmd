...
An *event* is then defined as any combination of these elements of the sample space.
...
`r include_graphics("Dice/dice1.png", dpi=3000)`, `r include_graphics("Dice/dice2.png", dpi=3000)`, `r include_graphics("Dice/dice3.png", dpi=3000)`, `r include_graphics("Dice/dice4.png", dpi=3000)`, `r include_graphics("Dice/dice5.png", dpi=3000)` and `r include_graphics("Dice/dice6.png", dpi=3000)`.
...
* Rolling a `r include_graphics("Dice/dice4.png", dpi=3000)`: 
  This event includes one element of the sample space: `r include_graphics("Dice/dice4.png", dpi=3000)`.

* Rolling a even number: 
  This event includes three elements of the sample space: `r include_graphics("Dice/dice2.png", dpi=3000)`, `r include_graphics("Dice/dice4.png", dpi=3000)` and `r include_graphics("Dice/dice6.png", dpi=3000)`

* Rolling a number larger than `r include_graphics("Dice/dice2.png", dpi=3000)`: 
  This event includes four elements of the sample space:   `r include_graphics("Dice/dice3.png", dpi=3000)`, `r include_graphics("Dice/dice4.png", dpi=3000)`, `r include_graphics("Dice/dice5.png", dpi=3000)` and `r include_graphics("Dice/dice6.png", dpi=3000)`.
:::


<div style="float:right; width: 222x; border: 1px; padding:10px">
<img src="Illustrations/cricket-724622_640.jpg" width="200px"/>
</div>
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/cricket-724622_640.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
We do not know beforehand what distance your throw will be, but we can describe the *sample space* for this procedure:
the distance could be anywhere between (say) 0 and 200 metres (and of course, some of those distances are fairly unlikely to occur...).
...
* The probability of receiving some rain in Sydney in the next decade is one. 
  It is certain.
* The probability of receiving rain on any given day in Sydney... is between 0 and 1.
:::
...
What is the probability of rolling a `r include_graphics("Dice/dice4.png", dpi=3000)` on a die?
The sample space has six possible outcomes (listed in Example \@ref(exm:SampleSpaceDie)) that are *equally likely*, and the event ('rolling a `r include_graphics("Dice/dice4.png", dpi=3000)`') comprises just *one* of those.
Thus,
...
\[
   ...
   = \frac{1}{6}.
\]
This approach to computing probabilities is called the  *classical* approach to probability.

We say that 'the probability of rolling a `r include_graphics("Dice/dice4.png", dpi=3000)` is 1/6', or 'the probability of rolling a `r include_graphics("Dice/dice4.png", dpi=3000)` is 0.1667'.
The answer can also be expressed as a *percentage* ('the probability of rolling a `r include_graphics("Dice/dice4.png", dpi=3000)` is 16.7%').
The answer could also be interpreted as 'the *expected* proportion of rolls that are a `r include_graphics("Dice/dice4.png", dpi=3000)` is 0.167'.

The chance of rolling a `r include_graphics("Dice/dice4.png", dpi=3000)` in the future is 0.1667, but a roll of the die either will or will not produce a `r include_graphics("Dice/dice4.png", dpi=3000)`... and we don't know which will occur.
...
What is the *probability* of rolling a `r include_graphics("Dice/dice2.png", dpi=3000)` on a die?
What are the *odds* of rolling a `r include_graphics("Dice/dice2.png", dpi=3000)` on a die?
...
\]
So the probability is $\frac{1}{6} = 0.1667$, or about 16.7%.
Also, since the six possible outcomes are *equally likely*:
...
The probability of rolling a `r include_graphics("Dice/dice1.png", dpi=3000)`  *or* a `r include_graphics("Dice/dice2.png", dpi=3000)` is $2/6$ (or 33.3%).
:::
...
This approach is fine *if* boys and non-boys are *equally likely* to be born.
But are they?

In [Australia in 2015](http:www.abs.gov.au/ausstats/abs@.nsf/0/B8865D71D84F5210CA2579330016754C?opendocument), 305&nbsp;377 live births occurred, with 157&nbsp;088 male births and 148&nbsp;289 non-male births.
Then, the *proportion* of boys born in 2015 is

\[
   \frac{157\,088}{305\,377} = 0.514,
\]
or about 51.4%.
An *estimate* of the probability that the next birth will be a boy is is about 0.514 (or 51.4%).
This is the *relative frequency* approach to calculating probabilities: based on past proportions.
...
According to the [ABS](http:www.abs.gov.au/ausstats/abs@.nsf/0/B8865D71D84F5210CA2579330016754C?opendocument):
...
The  data in Table \@ref(tab:OWeekTable) concern students enrolling in a library introductory session in O-Week.
(SSE is the School of Science and Engineering; SHSS is the School of Health and Sport Science.)
...
Library2 <- cbind( Library, 
                   "Total" = rowSums(Library))
Library2 <- rbind( Library2, 
                   "Total" = colSums(Library2))
...
> What is the probability that California will experience a Category 1 cyclone next year?

In this case, only a *subjective probability* can be given.

'Subjective' probabilities may sound like they are 'made up; however, it actually means the probability is estimated by considering all the relevant issues that may impact the probability (and may, for example, be based on mathematical models that incorporate information from numerous inputs).
...
Weather forecasts are one example: weather forecasts incorporate data from sea surface temperatures, topography, air pressures, air temperatures and so on.
Different models use different inputs, and then may coombine these inputs differently to produce different (subjective) forecast probabilities.
...
Many farmers, based on many years of experience, can give a subjective probability of the chance of receiving rainfall in the coming month.
:::
...
<div style="float:right; width: 222x; border: 1px; padding:10px">
<img src="Illustrations/cards-416960_640.jpg" width="200px"/>
</div>
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/cards-416960_640.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
In summary, the probability of drawing an **Ace** for the second card *depends* on whether or not an **Ace** was drawn for the first card.
...
1 Suppose **Event A** is defined as '*Rolling a `r include_graphics("Dice/dice1.png", dpi=3000)` or a `r include_graphics("Dice/dice2.png", dpi=3000)` on a fair die*'.
...
1. Consider these three events, then answer the questions that follow:  
...
1. Suppose I roll a standard six-sided die. What is the **probability** that I will roll a number larger than 2?
`r if( knitr::is_html_output(exclude = "epub") ) {mcq( c(
	"0",
	"1/6",
	"2/6",
  ...
  "4/6",
	"5/6",
	"1"))}`
2. Suppose I roll a standard six-sided die. What are the **odds** of rolling a number smaller than 6?
`r if( knitr::is_html_output(exclude = "epub") ) {mcq( c(
	"0",
	"1/6",
	"1/5",
...
1. Suppose I draw one card from the pack and roll a six-sided die.
   Are the events 'Draw a **Jack** from the pack of cards' and 'Roll a `r include_graphics("Dice/dice5.png", dpi=3000)` on the die' independent events? 
:::
