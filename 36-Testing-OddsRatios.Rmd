...
As seen in Sect.\ \@ref(OddsRatioIntro), @data:Mann12017:UniStudents examined the relationship between where students usually ate, and where the student lived (Table\ \@ref(tab:MealsDataTableHT)).
...
For the data in Table\ \@ref(tab:MealsDataTableHT), the software output (Fig.\ \@ref(fig:UniMealsTestOutputHTjamovi)) can be interpreted in *either* of these ways (i.e., both are correct): 

* The *odds* compare eating most meals *off-campus* (Row\ 1) to *on-campus* (Row\ 2; on the bottom of the fraction):
  * for students living with their parents (Column\ 1): $52/2 = 26$;
  * for students not living with their parents (Column\ 2): $105/24 = 4.375$. 
  
  The OR is $26/4.375 = 5.943$ (Column\ 2 on bottom of the fraction), as in the output.

* The *odds* compare the odds of living with parents (Column\ 1) to not living with parents (Column\ 2; on the bottom of the fraction):

  * for those eating most meals off-campus: $52/105 = 0.49524$;
  * for those eating most meals on-campus: and $2/24 = 0.083333$.
  
  The OR is $0.49524/0.083333 = 5.943$ (Row\ 2  on bottom), as in the output.

Both approaches produce an OR of $5.943$.
:::


The RQ can be written as comparing proportions, comparing odds, or about odds ratios directly.
...
The hypotheses can also be written in terms of differences in percentages (or proportions), though using odds or odds ratios is preferred for consistency, since the software output is usually expressed in terms of odds.
...
From Table\ \@ref(tab:MealsDataTableHT), $157$ students out of $183$ ate most meals off-campus, so that $157\div 183 \times 100 = 85.79$% of the students in the entire sample ate most of their meals off-campus.
...
(These were also found in Sect.\ \@ref(ValidityConditionsORCI).)
...
Those are the *expected* counts if the percentage was exactly the same in each group (Table\ \@ref(tab:MealsDataTableHTExpected)), if the null hypothesis (the assumption) was true.
...
In previous hypothesis tests, the *sampling distribution* had an approximate normal distribution (whose standard deviation is called the *standard error*).
...
We will use software output instead.
...
In a $2\times 2$ table of counts (when the 'degrees\index{Degrees of freedom} of freedom'^[For those interested: the degrees of freedom in a two-way table is the number of rows of data less one, times the number of columns of data less one. 
For a $2\times 2$ table, $\text{df} = (2 - 1) \times (2 - 1) = 1$.], or `df`, is equal to 1, as in the computer output), the *square root* of the $\chi^2$ value is equivalent to a $z$-score of about $\sqrt{6.934} = 2.63$.
...
As usual, a very small $P$-value ($0.008$ to three decimals) means strong evidence exists to supporting $H_1$: the evidence suggests a difference in the *population* odds in the two groups.
We write:
...
A study [@candan2021first] compared the proportion of infected nests relocated nest due to the risk of tidal inundation, and non-relocated nests 
`r if (knitr::is_latex_output()) {
...
```{r TurtleNestsOutputjamoviHT, fig.show="hold", fig.cap="The jamovi output for the turtle-nesting data", fig.align="center", out.width=c("48%", "49%")}
...
Then, *expected counts can be computed.
...
The value of $\sqrt{\chi^2/\text{df}}$ is like a $z$-score,  where 'df' is the 'degrees of freedom' reported by software, and so an approximate $P$-value can be estimated using the $68$--$95$--$99.7$ rule.
Software reports the $P$-value to assess whether the data are consistent with the assumption.
...
`r if( knitr::is_html_output() ) {longmcq(
  c("One-tailed",
    ...
    "There is not enough information")  )}`
1. True or false:
There is *no* evidence of a difference in the odds of buying a car in the next 10 years, between those with and without post-graduate study.
...
A forward-direction observational study in Western Australia compared the heights of scars from burns received [@data:Wallace2017:Sunburn].
