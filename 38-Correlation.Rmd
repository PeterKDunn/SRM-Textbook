...
Our main example in the next chapters is a study [@data:Holgate1965:StraightLine] examining the relationship between the age of $n = 78$ male red deer and the weight of their molars (Sect.\ \@ref(TwoQuant-Intro)).
...
## Numerical summaries: linear relationships {#LinearRelationshipsR}
...
Here, the *Pearson* correlation coefficient is discussed, which is appropriate for describing *linear* relationships between *quantitative* data^[Other correlation coefficients also exist, such as the *Spearman* correlation, which may be used for monotonic, non-linear relationships.].
...
* Values of $r$ between $-1$ and $0$ mean a *negative* relationship:
...
the relationship is approximately linear, and the output from jamovi shows that $r = -0.584$.
...
```{r RedDeerCorrelationjamoviSPSS, fig.cap="jamovi correlation output for the red-deer data", fig.align="center", out.width=c("40%", "55%"), fig.show="hold"}
...
For the red-deer data (Sect.\ \@ref(TwoQuant-Intro); Sect.\ \@ref(CorrCoefficients)), the *population* correlation coefficient between the weight of molars $y$ and age of the deer $x$ is unknown, and denoted by $\rho$.
...
### Hypothesis testing {#CorrelationHT}
...
In the context of the red-deer data, a *one-tailed* RQ is asked:
...
The output (Fig.\ \@ref(fig:RedDeerCorrelationjamoviSPSS)) contains the relevant $P$-value.
The *two*-tailed $P$-value for the test is less than $0.001$, so the *one*-tailed $P$ value will be less than $0.0005$.
There is *very strong evidence* exists to support $H_1$ (that the correlation in the population is less than zero).
We write:

> The sample presents very strong evidence (one-tailed $P < 0.0005$) that teh mean molar weight decreases as the age of the male red deer increases ($r = -0.584$; $n = 78$) in the population.
...
\]
The article gives the correlation coefficient as $r = 0.346$ and $P = 0.18$.
...
## Example: Phu Quoc ridgeback dogs {#CorTestDogs}
...
Each sample of dogs could produce a different value for $r$, so sampling variation exists.
...
```{r GorillaCorrelationjamovi, fig.cap="Output for the gorilla data", fig.align="center", out.width=c("48%", "48%"), fig.show="hold"}
...
- Sect. \ref{thinkBox:EstimateR}: These are *very* hard to estimate! You are not expected to be very accurate (you cannot be). 
  The actual values (from software) are $r = 0.71$ and $r = -0.13$.
  Realistically, the best you can probably do is 'a reasonably high positive $r$ value' and 'a slightly negative $r$ value '.
