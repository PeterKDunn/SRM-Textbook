...
**1.** Whether Egyptians experience side effects, from many people, after medication.
**2.** **Quant**itative.
:::
...
**7.** Observation: person.
       Analysis: individual people not in a group, since people in a group may not walk independently of each other (i.e., they keep pace with each other). 
:::
...
Perhaps: 'Among Australian adults, is average time taken to read passage of text diff when Arial font is used compared to Times Roman font?'
:::
...
Ten.
Ten.
Within-board variation smaller (except first board).
:::
...
Control: perhaps farm size (e.g., farms over certain size).
:::
...
**4.** If mean of both response, explanatory variables different for F and M, sex a *confounder*; would need to be factored into the analysis.
**5.** Participants blinded.
:::
...
Average near $1.5\mmols$/L.\spacex
Most between\ $3$, $4\mmols$/L.\spacex
Some large outliers.
Probably median (slightly skewed right; outliers).
...
**2.** Gender: modes are F, M.  Place: city $> 100\,000$ residents. Response: agree.
...
**5.** $0.613$: respondents about $0.61$\ times as likely to agree, strongly disagree than choose other option.
**6.** $1$: respondents as likely to be\ M as\ F.
:::
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:FEVplots).**
*Age*, *FEV*, *Height*: histogram.
*Gender*: bar or dot; mode: M ($51.4$%; odds: $1.06$).
...
**1.** *Vomited*: $0.50$ beer, wine; $0.50$ wine only.
*Didn't vomit*: $0.738$ beer, wine, $0.262$ wine only. 
Prop.\ drank various drinks, among those who did, didn't vomit.
**2.** *Beer, wine*: $8.8$% vomited, $91.2$% didn't; *Wine only*: $21.4$% vomited, $78.6$% didn't. 
...
**2.** Assuming fair coin, four consecutive heads doesn't seem highly unusual.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Mean diameter $12$\ inches.
**2.** Sampling variation incorrect.
...
**5.** $t = 9.46$.
**6.** $t$-score *huge*; (one-tailed) $P$ very small: very strong evidence mean speed of vehicles on road greater than $90\kms$.h^$-1$^.
**7.** Stat.\ valid.
:::
...
**8.** $t = -2.12$.
**9.** $P$ large; from software, $P = 0.046$.
**10.** $0.11\gs$ to $3.81\gs$ lighter in 2015.
**11.** Probably stat.\ valid; $n$ just less than $25$.
**12.** Slight evidence ($t = -2.12$; two-tailed $P = 0.046$) of mean increase in weight of squash from dry to normal years (mean change: $1.96\gs$ ($95$%\ CI $0.11$ to $3.81\gs$), heavier in normal year).
:::
...
**1.** $t = 2.48$; quite large; **one-tailed** $P$ certainly less than $0.025$.
       Strong evidence of a population mean increase in beta-endorphin concentrations from $12$--$14\hs$ before surgery to $10\mins$ before surgery 
**2.** $1.5$ to\ $13.9$.
...
Normal; mean $\mu_B - \mu_A$; std\ dev.: $2.96\kms$.h^$-1$^.
:::
...
**1.** Parameter: $\mu_M - \mu_F$ (amount M longer than F, on average).
...
**8.** No evidence ($t = 0.65$; two-tailed $P > 0.10$) in sample that mean length of adult gray whales is diff in pop. for F (mean: $12.70\ms$; std dev.: $0.611\ms$) and M (mean: $12.07\ms$; std dev.: $0.705\ms$; $95$%\ CI for the diff: $-1.26\ms$ to $0.246\ms$).
**9.** Yes.
:::
...
**4.** Very strong evidence in sample ($P < 0.001$)  mean annual sugar consumption per person diff for industrialised (mean: $41.8\kgs$/person/year) and non-industrialised (mean: $24.6\kgs$/person/year) countries ($95$%\ CI for the diff $11.95$ to\ $22.54$).
**5.** Yes.
:::
...
Moderate evidence ($P = 0.024$) mean internal body temperature diff for F (mean: $36.886$^o^C) and M (mean: $36.725$^o^C).
...
**1.** $r$ moderate, positive; $\hat{y} = 4 + 1.4x$
...
**3.** $\hat{y}$, not $y$; slope, intercept values *swapped*.
**4.** Very dodgy.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** Stat.\ valid if approx. linear; variation in STAI same for diff levels of experience.
$n \ge 25$.
:::
...
**4.** $\hat{y} = 18\gs$.
**5.** $64$% reduction in unexplained variation using application time.
...
**5.** Negative; cannot deduce value from given info, but moderate.
:::
`r if( !includeEvenAnswers) "-->"`
...
**8.** Sample sizes, linearity OK; for Sex\ B, perhaps hint of increasing variation.
:::
...
**3.** $\rho$ possibly zero.
**4.** Not shown.
**4.** Sex a confounding variable; need separate reports for men's, women's.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** No: sample ORs do not have an approx.\ normal sampling distribution.
:::
...
**1.** Yes.
**2.** No: $r$ does not have an approx.\ normal sampling distribution.
**3.** Yes.
:::
`r if( !includeEvenAnswers) "-->"`
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:Method3).**
Comparing two odds: ORs; stacked, side-by-side bar chart.
CI for ORs.
:::
...
**1.** *to*.
**2.** *its*.
**3.** One sample with $50$ individuals; use 'mean' or 'median', not 'average'; units of age is 'years'.
...
**1.** Convenience; self-selected. 
       Those in study *may* record different accuracies than those not in.
...
**8.** Yes.
**9.** To ensure the two comparison groups included women in similar rooms (to manage confounding).
**10.** Not shown.
