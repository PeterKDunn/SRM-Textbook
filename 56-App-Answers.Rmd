...
# Answers to odd-numbered exercises {#Answers .unnumbered}
...
**1.** $R^2 = 0.881^2 = 77.6$%. 
Unknown variance reduced by $77.6$% using leg strength.
**2.** $H_0$: $\rho = 0$ and $H_1$: $\rho \ne 0$. 
$P$ *very* small; very strong evidence of correlation in pop.
:::
`r if( !includeEvenAnswers) "-->"`
...
Rise-over-run: slope about $(190 - 110)/(180 - 110) = 1.14$.
**2.** $\hat{y} = -3.69 + 1.04x$, where $y$ is punting distance (feet), and $x$ is right-leg strength (pounds).
**3.** For each extra pound of leg strength, punting distance increases, on average, by about $1$\ ft.
...
**6.** $0.70$ to\ $1.4$\ ft.
**7.** Very strong evidence in sample ($t = 6.16$; $P = 0.0001$ (two-tailed)) punting distance related to leg strength (slope: $1.0427$; $n = 13$).
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** $\hat{y} = 18$\gs.
**4.** About $64$% reduction in explained variation using time spent on application.
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}::: {.answer data-latex=""}
