...
Stratified by zone; then maybe convenience (though unclear).
:::
...
Data collected at night; not *ecologically valid*?
:::
...
First fine; 'seldom' (for instance) may have different meanings to different people; possible recall bias.
...
*<span style="font-variant:small-caps;">gmfcs</span>*: qual.\ ordinal.
:::
...
*Fertiliser dose*: quant.\ continuous.
*Soil nitrogen*: quant.\ continuous.
*Fertiliser source*: qual.\ nominal.
:::
`r if( !includeEvenAnswers) "-->"`
...
Shape: skewed *left*.
Average: perhaps $70$--$100$?
Variation: most between $30$, $80$.
...
**1.** Probably median (but mean probably OK).
**2.** Slightly right skewed; average near $1.5\mmols$/L; most between\ $3$, $4\mmols$/L; some large outliers.
:::
...
<!-- rideFatalities <- c(2,4,3,4,7,6,1,3,2,5) -->
**1.** $3.7$.
**2.** $3.5$.
**3.** $1.888562$.
**4.** $5 - 2 = 3$.
...
<!-- SOI <- c(0.8, 4.6, -19.8, 9.8, 2.1, 5.3, -8.2, -14.6, -1.8) -->
Plot not shown.
**1.** $-2.42$.
**2.** $0.8$.
**3.** $29.6$ ($-19.8$ to\ $9.8$).
**4.** $9.831172$; about $9.83$.
...
**3.** Unlikely (but only one of many samples).
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** Don't know.
**3.--5.** Not shown.
**6.** Hard to describe with standard language; approx. symmetric?.
:::
...
In seconds: Average about $10$?; variation $0$ to\ $30$ perhaps; skewed right.
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:JeansIQR1).**
**1.** Men's: about $50$%; women's: about $100$%. 
**2.** Men's: about  $0$%; women's: about $50$%.
:::


`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:JeansIQR2).**
...
**1.** Graph not shown; no commonly-observed social group include\ M.
**2.** Mode: many F plus offspring; median inappropriate.
...
**4.** *Where*: mode is 'Home'. 
       *Temp* (ignoring unknowns): mode and median is $100$^o^C.
       *Time* (ignoring unknowns): mode and median is About $3\mins$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Gender: nominal; others ordinal.
**2.**--**4.** 
Gender: modes are F, M; no median.
Place: mode is city $> 100\,000$ residents; median is city $20\,000$ to\ $100\,000$ residents
Response: mode is 'Agree'; median is 'Neutral.
**5.** $5.12$: respondents about $5$\ times more likely to come from city than rural.
**6.** $0.613$: respondents about $0.61$\ times as likely to agree, strongly disagree than choose other option.
**7.** $1$: respondents as likely to be\ M as\ F.
:::
...
**6.** $0.209$.
**7.** $1.21$.
**8.** First includes farmers not identifying disadvantages. 
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** Bus.
**3.** No.
**4.** $0.102$, $0.123$; $0.246$, $0.412$, $0.116$.
**5.** $3.44$; i.e., students $3.44$ times as likely to use motorised transport than active.
**6.** $0.141$; i.e, for every $100$ students that *do not walk*, $100\times 0.141 = 14.1$ *do walk*.
**7.** Not shown.
:::
...
**3.** $40.9$% F, $59.1$ M.
**4.** $0.692$; a birth less likely to be F than\ M.
**5.** Not shown.
:::
`r if( !includeEvenAnswers) "-->"`
...
**Ex.\ \@ref(exr:FEVplots).**
**1.** Not shown.
**2.** Gender: M; smoking: Non-smoking.
**3.** Gender: percentage, odds M: $51.4$%, $1.06$; smoking: percentage, odds non-smoking: $9.9$%, $0.11$.
:::
...
*Gender*: bar or dot: mode: M. F: $20$%; odds: $0.25$.
...
**1.** In general, DB smaller cost over-runs.
**2.** Tricky: DB: $2\cms$; DBB: $3\cms$.
**3.** Tricky: DB: $2\cms$; DBB: $3\cms$. 
:::
...
**1.** 
*Vomited*: $0.50$ beer, wine; $0.50$ wine only.
*Didn't vomit*: $0.738$ beer, wine, $0.262$ wine only. 
Prop.\ drank various drinks, among those who did, didn't vomit.
**2.** 
*Beer, wine*: $8.8$% vomited, $91.2$% didn't.
*Wine only*: $21.4$% vomited, $78.6$% didn't. 
...
**7.** $0.354$.
**8.** $-0.176$.
**9.** Higher percentage vomited after beer+wine, compared to beer only.
:::
...
**6.** $1.22$.
**7.** $0.0453$.
**10.** Percentage F among adults slightly higher than among young.
...
**5.** OR: $1.41$.
**6.** Odds F skipping $1.41$ *times* odds M skipping. 
**7.** Not shown. 
:::
...
**4.** $0.0238$.
**5.** $6.39$.
**6.** Odds coffee drinker being smoker: $6.39$ times odds of non-coffee drinker being smoker.
**7.** Not shown.
:::
`r if( !includeEvenAnswers) "-->"`
...
$\text{OR(W; home)} = 4/6  = 0.667$; $\text{OR(W; away)} = 7/4 = 1.75$.
...
```{r PossumRoadkillNumbers, , eval=includeGraphsTables, fig.align="center", fig.cap="Dot chart for the roadkill data", fig.height=4, fig.width=8, include=FALSE, out.width='80%'}
...
**1.** Approximately linear; positive relationship; variation larger for more cases.
**2.** A delivery.
**3.** Non-constant variation: no.
:::
...
$R^2 = (-0.682)^2 = 0.465$: about $46.5$% of the unknown variation in number cyclones explained by knowing value of ONI.
:::
...
*Fertiliser* (quant.): histogram (response).
*Soil nitrogen* (quant.): histogram (explanatory).
*Source* (qual.\ nominal): bar chart (explanatory).
*Relationships*: between fertiliser dose, soil nitrogen: scatterplot. 
...
**1.** Response: *change* in <span style="font-variant:small-caps;">madrs</span> (quant.\ cont.).
...
*Sex*: about twice as many F as M.
...
**3.** Picture cards.
**4.** $16/52 = 4/13$.
**5.** **Ace**, **King**, **Queen**, **Jack** of $\spadesuit$.
**6.** $4/52 = 1/13$.
...
**3.** $1/2$.
**4.** $1/2$.
**5.** HH, HT, TH, TT (Coin\ A listed first).
:::
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:ProbabilityDie).**
**1.** $4/6$.
**2.** $5$. 
**3.** Yes: die outcome won't change coin outcome.
**4.** $1/2$.
**5.** $1/6$.
**6.** $1/3$.
:::
...
**2.** Equally-likely outcomes, so $1/2$.
**3.** $1/2$.
**4.** Yes.
:::
...
**3.** *Not independent*: if it rains, I won't water garden.
:::
...
<!-- \pagebreak MANUAL LATEX PAGE BREAK ADDED --> 
...
**4.** Close to one.
:::
<!-- round(pnorm( c(1.84, -2.09, -5.34, 4.25) ), 4) -->
...
**1.** $0.6844$.
**2.** $0.1515$.
**3.** $0.99997$, using note at bottom of table.
**4.** $0.5$.
:::
`r if( !includeEvenAnswers) "-->"`
<!-- 1 - round(pnorm( c(-0.48, 1.03, -4, 0) ), 4) -->


::: {.answer data-latex=""}
**Ex.\ \@ref(exr:SamplingDistributionsGrowthChartA).**
About $2.5$% of girls under $100\cms$ tall.
:::


`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:SamplingDistributionsGrowthChartB).**
About $2.5$% of girls taller than $120\cms$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**4.** About $z = 1.04$; diameter about $11.6$\ inches.
**5.** About $z = -0.67$; diameter about $7.0$\ inches.
:::
...
**4.** $z = 1.64$ (or $1.65$); $5$% *longer* than $42.7$ weeks. 
**5.** $z$-score: $-1.28$; $10$% *shorter* than $37.9$ weeks.
:::
...
$\hat{p} = 0.81944$, $n = 864$.
$\text{s.e.}(\hat{p}) = 0.01309$; approx. $95$%\ CI: $0.819 \pm (2\times 0.0131)$ or $0.793$ to\ $0.845$.
Stat.\ valid.
...
:::
```{r eval = FALSE}
phat <- 8/154; se <- sqrt( (phat *(1 - phat))/ 154)
phat; se;
phat + c(-2, 2) * se
```
...
Approx. $95$%\ CI $0.219$ to\ $0.487$.
Stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
phat <- 18/51; se <- sqrt( (phat *(1 - phat))/ 51)
phat; se;
phat + c(-2, 2) * se
```
...
:::
```{r eval = FALSE}
phat <- 2182/6882; se <- sqrt( (phat *(1 - phat))/ 6882)
phat; se;
phat + c(-2, 2) * se
```
...
$3\,000\hs$: $\hat{p} = 0.2143$; $\text{s.e.}(\hat{p}) = 0.06331$.
CI: $0.088$ to\ $0.341$.
Stat.\ valid.
$400\hs$: $\hat{p} = 0$; $\text{s.e.}(\hat{p}) = 0$.
CI: $0$ to\ $0$: silly (no sampling variation?): *not* stat.\ validity conditions.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
phat <- 9/42; se <- sqrt( (phat *(1 - phat))/ 42)
phat; se;
phat + c(-2, 2) * se

phat <- 0/39; se <- sqrt( (phat *(1 - phat))/ 39)
phat; se;
phat + c(-2, 2) * se
```
...
:::
```{r eval = FALSE}
phat <- 365/1516; se <- sqrt( (phat *(1 - phat))/ 1516)
phat; se;
phat + c(-2, 2) * se
```
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:OnePropCIAlcohol).**
$\hat{p} = 0.49310$.
$\text{s.e.}(\hat{p}) =  0.015700$.
Approx. $95$%\ CI: $0.462$ to\ $0.524$.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
phat <- 500/1014; se <- sqrt( (phat *(1 - phat))/ 1014)
phat; se;
phat + c(-2, 2) * se
```
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:OnePropCIAI).**
$\hat{p} = 0.3182$.
$\text{s.e.}(\hat{p}) =  0.0702175$.
Approx. $95$%\ CI: $0.178$ to\ $0.459$.
:::
```{r eval = FALSE}
phat <- 14/44; se <- sqrt( (phat *(1 - phat))/ 44)
phat; se;
phat + c(-2, 2) * se
```
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:OnePropCICheating).**
$\hat{p} = 0.064356$.
$\text{s.e.}(\hat{p}) =  0.017265$.
Approx. $95$%\ CI: $0.0298$ to\ $0.0989$.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
phat <- 13/202; se <- sqrt( (phat *(1 - phat))/ 202)
phat; se;
phat + c(-2, 2) * se
```

::: {.answer data-latex=""}
**Ex.\ \@ref(exr:OnePropCIWearHats).**
$\hat{p} = 0.13431$.
$\text{s.e.}(\hat{p}) = 0.012434$.
Approx. $95$%\ CI: $0.109$ to\ $0.159$.
:::
```{r eval = FALSE}
phat <- 101/752; se <- sqrt( (phat *(1 - phat))/ 752)
phat; se;
phat + c(-2, 2) * se
```
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:OnePropCIWearSunglasses).**
$\hat{p} = 0.33112$.
$\text{s.e.}(\hat{p}) =  0.017162$.
Approx. $95$%\ CI: $0.297$ to\ $0.365$.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
phat <- 249/752; se <- sqrt( (phat *(1 - phat))/ 752)
phat; se;
phat + c(-2, 2) * se
```
...
Approx. $95$%\ CI: $29.9$ to\ $36.1\secs$.
:::
```{r eval = FALSE}
xbar <- 33.0; n <- 60; s <- 12.0
se <- s/sqrt(n); se
xbar + c(-2, 2) * se
```
...
**2.** Not good at estimating (from article: guesses between $50$ to $3\,000\mLs$).\spacex
**3.** Stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval = FALSE}
xbar <- 846.4; n <- 199; s <- 651.1
se <- s/sqrt(n); se
xbar + c(-2, 2) * se
```
...
*Neither* correct.
For variation in *individuals* trees, use *std deviation* not std error.
CIs about pop. mean diameter, not individual trees.
:::
`r if( !includeEvenAnswers) "-->"`
...
$\text{s.e.}(\bar{x}) = 5.36768$; approx. $95$%\ CI: $50.56$ to\ $72.04\secs$.
Stat.\ valid.
:::
```{r eval = FALSE}
xbar <- 61.3; n <- 30; s <- 29.4
se <- s/sqrt(n); se
xbar + c(-2, 2) * se
```
...
**9.** Not given.
**10.** $n \ge 25$, or pop. has normal distribution; stat.\ valid.
**11.** $\mu$ probably not $12$ inches.
:::
`r if( !includeEvenAnswers) "-->"`
...
**4.** CI allows for sampling variation.
**5.** $44.850$ to\ $44.950$.
**6.** Possibly lying; not certain.
**7.** Sample mean; $\bar{x} = 44.9$.
**8.** Population mean; value unknown, but *claimed* to be $\mu = 45$.
:::
...
**1.** CIs are intervals for unknown *parameters*, not known *statistics*. 
...
**1.** Yes; very likely (can't be sure).
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:MakingDecisionsCoin)**.
**1.** No; seem reasonable (can't be sure).
**2.** Assuming fair coin, two consecutive heads is not unusual.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Mean diameter $12$\ inches (as claimed).
**2.** Difference due to sampling variation; difference because mean not really $12$\ inches.
**3.** Perhaps unsupported?
**4.** Very likely unsupported.
**5.** Yes.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** $p = 0.36$.
**2.** $\hat{p} = 0.433$; probably not.
**3.** $\hat{p} = 0.1$; probably.
:::
::::::
...
'The null hypothesis equal to $0.076$' makes no sense; mean: 'the value of some *parameter* is equal to $0.076$'.
Also, $0.076$ appears to be sample value, but hypotheses should be formed before data collected.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** One-tailed.
**4.** Normal; mean $0.2$, std\ deviation $\text{s.e.}(\hat{p}) = 0.0444$.
...
Using tables, one-tailed $P = 0.0281$; slight evidence of home-ground advantage in pop.
:::
`r if( !includeEvenAnswers) "-->"`
...
$P$ very small.
Strong evidence prop. F using machines lower than prop. F in uni pop.
:::
...
**5.** $t = 9.46$.
**6.** $t$-score *huge*; (one-tailed) $P$ very small: very strong evidence mean speed of vehicles on road greater than $90\kms.\xhs^{-1}$.
**7.** Stat.\ valid.
:::
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:TestOneMeanExercisesSlalom).**
$H_0$: $\mu = 38.8$; $H_1$: $\mu \ne 38.8$ (two-tailed), where $\mu$ mean distance in metres.
Std error: $\text{s.e.}(\bar{x}) = 0.37792$.
$t$-score: $-5.98$, *huge*; $P$ *very* small.

*Very* strong evidence ($P < 0.001$) that GPS is not recording distance correctly.
Since $n > 25$, stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
$H_0$: $\mu = 1\,000$ and $H_1$: $\mu \ne 1\,000$ ($\mu$: pop.\ mean guess of spill volume).
Std\ error: $46.15526$.
$t = -3.33$: $P$ very small.
Very strong evidence mean guess of blood volume not $1\,000\mLs$, the actual value.
Stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
...
**7.** $t = -23.3$.
**8.** $P$ *really* small.
**9.** Very strong evidence mean diameter not $12$\ inches.
...
**1.** Very small; certainly less than $0.0015$.
**2.** Very small; bit bigger than $0.0015$.
**3.** Bit smaller than $0.025$.
...
**1.** Bit smaller than $0.16$.
**2.** Between $0.16$ and $0.025$; closer to $0.32$.
...
**3.** A diff of $17\mins$ in over $5\hs$ of use: trivial.
**4.** Conclusion: no evidence of diff between mean lifetimes; cumbersome for advertising!
...
How much longer task takes on the PC.
:::
...
How much larger pH is downstream compared to upstream.
:::
`r if( !includeEvenAnswers) "-->"`
...
CI calculated as: $-3.24$ to\ $0.52$ days.
...
CI calculated as: $-49.36$ to $-27.88\mgs$.dL$^{-1}$.
...
**3.** Not shown.
**4.** Not shown.
**5.** Mean increase in fruit weight from normal (2015) to dry (i.e., normal minus dry). 
**6.** $H_0$: $\mu_d = 0$ and $H_1$: $\mu_d \ne 0$.
**7.** Not shown.
**8.** $t = 0.205$.
**9.** $P$ large; from software, $P = 0.839$.
**10.** $19.53\gs$ to $23.99\gs$ heavier in normal year (2015).
**11.** Probably stat.\ valid; $n$ just less than $25$.
**12.** No evidence ($t = 0.205$; two-tailed $P = 0.839$) of mean increase in weight of squash from dry to normal years (mean change: $2.23\gs$ ($95$%\ CI $-19.53$ to $23.99\gs$), heavier in normal year).
:::
```{r eval = FALSE}
2.23 + c(-2, 2)*10.88
```
...
**1.** $t = 1.78$; $P$ larger than $0.05$: evidence doesn't support $H_1$.
**2.** *Approx.* $95$%\ CI for *reduction*: $-0.08$ to\ $1.4$:.
**3.** Probably; $n$ just less than $25$.
:::
`r if( !includeEvenAnswers) "-->"`
...
$\bar{d} = -424.25$; $\text{s.e.}(\bar{d}) = 467.9404$; $n = 20$: $t = -0.907$.
$P = 0.376$: evidence doesn't support $H_1$.
$95$% CI: $-1403.7$ to\ $555.2$.
Test perhaps not stat.\ valid $n < 25$); but histogram of data suggests population *might* have normal distribution); $P$ so large probably makes little difference.
:::
```{r eval = FALSE}
data(Ferretin)
t.test(Ferretin$Reduction)
```
...
**1.** $t = 2.48$; quite large; **one-tailed** $P$ certainly less than $0.025$.
       Strong evidence of population mean increase in beta-endorphin concentrations. 
**2.** $1.5$ to\ $13.9$.
**3.** $1.18$ to $14.22\fmols$/mol.
...
**1.** *Diffs* are *during* minus *before*: *positive* diffs means *during* value higher.
**2.** $\text{s.e.}(\bar{d}) = 3.5150$.
...
data(SoilCN)
with(SoilCN, 
     t.test(IrrigatedN - NonirrigatedN))
```


::: {.answer data-latex=""}
**Ex.\ \@ref(exr:PairedCIJumping).**
Exact CI: $1.204$ to\ $0.069\cms$ greater barefoot.
:::
```{r eval=FALSE}
data(Jumping)
with(Jumping, 
...
Exact $95$%\ CI: $-0.01$ to\ $0.047\secs$ faster without racquet.
Perhaps not stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval=FALSE}
data(WCTennis)
with(WCTennis, 
     t.test(PTwith - PTwithout))
```
...
How much greater mean braking distance is (worse) for cars with Type\ B brake pads compared to Type\ A pads.
:::
`r if( !includeEvenAnswers) "-->"`
...
Normal; mean $\mu_B - \mu_A$; std\ dev.: $2.965\kms$.h^$-1$^.
:::
...
Normal; mean at $\mu_T - \mu_W$; std\ dev.: $8.42\mgs.\xdLs^{-1}$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Parameter: $\mu_F - \mu_M$.
       Estimate: $\bar{x}_F - \bar{x}_M = -0.06\ms$.
**2.** Not shown. <!-- See \@ref(fig:WhalesErrorbar). -->
**3.** $0.092487$
**4.** Not shown.
**5.** $-1.94$\ to $0.24\ms$. 
**6.** $H_0: \mu_F - \mu_M = 0$; $H_1: \mu_F - \mu_M \ne 0$.
**7.** $t = 0.65$; $P$ very large.
**8.** No evidence ($t = 0.65$; two-tailed $P > 0.10$) in sample that mean length of adult gray whales is diff in pop. for F (mean: $4.66\ms$; std dev.: $0..38\ms$) and M (mean: $4.60\ms$; std dev.: $0.30\ms$; $95$%\ CI for the diff: $-1.94\ms$ to $0.24\ms$).
**9.** Yes.
:::
```{r eval=FALSE}
se1 <- 0.38/sqrt(26)
...
**1.** Parameter: $\mu_N - \mu_YS$.
       Estimate: $\bar{x}_N - \bar{x}_Y = 0.085$ mmol.L^$-1$^.
**2.** Not shown.
**3.** Not shown.
**4.** $0.055$ to $0.115\mmols.\xLs^{-1}$.
**5.** $0.054$ to $0.115$ (bottom row).
**6.** $H_0$: $\mu_N - \mu_Y = 0$; $H_1$: $\mu_N - \mu_Y \ne 0$.
**7.** $0.015$.
...
**8.** $0.3665054$.
**9.** $t = 1.45$; one-tailed $P$ between $0.025$ and $0.32$; using $z$-tables, $P$ approx. $0.074$.
**10.** Slight evidence of diff.
**11.** Not given.
**12.** Yes.
**13.** Probably not practically important (diff $0.53$ days).
:::
```{r eval=FALSE}
se1 <- 3.62/sqrt(176)
se2 <- 3.31/sqrt(183)
...
**2.** Exercise: $0.4427$; splinting: $0.3479$.
**3.** $0.563$
**4.** Not shown.
**5.** $-0.826$ to $1.426$: $0.826$ greater pain with exercise to $1.426$ greater pain with splinting.
**6.** $0.404$ to $1.796$.
**7.** $H_0$: $\mu_S - \mu_E = 0$; $H_1$: $\mu_S - \mu_E \ne 0$.
**8.** $t = 0.53$; very large $P$.
**9.** No evidence of difference.
**10.** Sample sizes small; probably not stat.\ valid: roughly correct only.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** Amount of DMFT greater in non-industrialised countries, as upper table has negative mean.
**2.** $H_0$: $\mu_I - \mu_{NI} = 0$; $H_1$: $\mu_I - \mu_{NI} \ne 0$.
**3.** $11.9$ to $22.5$, greater for industrialised.
**4.** Very strong evidence in sample ($P < 0.001$) mean annual sugar consumption per person diff for industrialised (mean: $41.8\kgs$/person/y) and non-industrialised (mean: $24.6\kgs$/person/y) countries ($95$%\ CI for the diff $11.9$ to\ $22.5$).
**5.** Yes.
:::
...
Perhaps: $\mu_{\text{After}} - \mu_{\text{Before}}$, *increase* in mean deceleration.
Approx. CI: $-0.025$ to $0.021\ms.\xsecs$.
...
xbar <- (-0.0020)
se <- 0.01143
se.me <- sqrt( (0.0494^2)/38 + (0.0521^2)/41 )
t <- xbar/se
c(t, pnorm( -abs(t)))
xbar + c(-2, 2)*se
```
...
**2.** Boxplot.
**3.** Not shown.
**4.** Approx.: $10.2$ to\ $18.8$^o^C.\spacex
       Exact CI (Row 2): $9.1$ to\ $19.9$^o^C.\spacex
       Different: sample sizes small.
**5.** One.
**6.** $H_0$: $\mu_Y - \mu_O = 0$; $H_1$: $\mu_Y - \mu_O > 0$.
**7.** $t = 6.69$ (*second row*); $P < 0.001/2$ as one-tailed; i.e., $P < 0.0005$.
**8.** Very strong evidence in sample ($t = 6.69$;  one-tailed $P < 0.0005$) that pop. mean one-step fall recovery angle for healthy women *greater* for young women (mean: $30.7$^o^C; std\ dev.: $2.58$^o^C; $n = 10$) compared to older women (mean: $16.20$^o^C; std\ dev.: $4.44$^o^C; $n = 5$; $95$%\ CI for the diff: $9.1$^o^C to $19.9$^o^C).
...
From output, $t = -2.29$; (two-tailed) $P = 0.024$.
Moderate evidence ($P = 0.024$) mean internal body temperature diff for F (mean: $36.9$^o^C) and M (mean: $36.7$^o^C).
Diff between the means ($0.2$ of degree) of little *practical* importance.
:::
```{r eval=FALSE}
xbar <- -0.161
se <- 0.0703
...
**2.** $t = 1.51$; $P$ large; no evidence of diff.
**3.** $t = 2.70$; $P$ small; evidence of diff.
Sample sizes small; may not be stat.\ valid.
:::
`r if( !includeEvenAnswers) "-->"`

```{r eval=FALSE}
ses <- c(8, 9, 10, 11) / sqrt( c(18, 11, 18, 11))
mns <- c(51, 56, 36, 47)
sediffs1 <- sqrt( ses[1]^2 + ses[2]^2)
sediffs2 <- sqrt( ses[3]^2 + ses[4]^2)
c(sediffs1, sediffs2)
...
**1.** $2.76\kgs$.
**2.** CB: $0.227$ to $5.79\kgs$; Control: $-3.68$ to $2.78\kgs$. 
**3.** $-0.68$ to $7.59\kgs$, greater for CB.
**4.** $t = 1.67$; Two-tailed $P = 0.095$.
       Slight evidence of diff.
:::
```{r eval=FALSE}
data(Anorexia)
Anorexia$Gains <- with(Anorexia, 
                       After - Before)
df1 <- subset(Anorexia, 
              Anorexia$Treatment=="CB")
t.test(df1$Gain)
df2 <- subset(Anorexia, 
              Anorexia$Treatment=="Control")
t.test(df2$Gain)
se1 <- findStdError(df1$Gain)
se2 <- findStdError(df2$Gain)
sediff <- sqrt(se1^2 + se2^2)
c(se1, se2, sediff)
xbar <- (mean(df1$Gain) - mean(df2$Gains)) 
xbar + c(-2, 2)*sediff
t <- xbar/sediff
2 * pnorm( -abs(t))
```
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:Coeliac).**
**1.** Researchers used exact $95$%CI.
**2.** True.
**3.** False.
**4.** $2.76$ means coeliacs have mean $2.76$ more DMFT.\spacex  
       $-2.32$ means *non*-coeliacs have a mean $2.32$ more DMFT.
:::
`r if( !includeEvenAnswers) "-->"`
...
$t = 2.631$; one-tailed $P = 0.0055$; evidence of diff.
:::
```{r eval=FALSE}
data(ReactionTime)
with(ReactionTime,
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:TwoMeansCIExercisesBMIanova).**
Table and graph not shown.
$F = 13.64$; $P < 0.001$; small.
Strong evidence that not all means are the same.
:::
`r if( !includeEvenAnswers) "-->"`
```{r eval=FALSE}
data(BMI)
with(BMI,
     anova( lm( exercise ~ factor(smoke) ) ) )
```
::::::
...
**1.** No-PG proportion (Row\ 1), minus PG (Row\ 2).
**2.** $H_0$: $p_{\text{With}} - p_{\text{W'out}} = 0$; $H_1$: $p_{\text{With}} - p_{\text{W'out}} \ne 0$.
**3.** $z = 1.14$; $P = 0.253$; no evidence of diff.
...
**7.** $\chi^2 = 1.31$; $P = 0.253$; no evidence of diff.
**8.** OR between $0.68$, $4.28$.
**9.** Smallest expected: $10.6$; yes.
:::
...
**2.** $H_0$: $p_{\text{S}} - p_{\text{L}} = 0$; $H_1$: $p_{\text{S}} - p_{\text{L}} \ne 0$.
...
**1.** Not shown.
**2.** Diff: $-0.1184$; individual std errors: $0.03446$ and $0.063315$; for diff: $0.07208575$.
**3.** $0.389$.
**4.** $-0.263$ to\ $0.026$.
**5.** $-0.260$ to\ $0.023$.
**6.** Not given.
...
cap.tab <- "Odds and percentage that people bring their own shopping bags by age groups. The ORs are computed relative to those 'Over $40$'."
...
**4.** Halve width: $4$ times as many units.
**5.** Quarter width: $16$ times as many units.
**6.** More needed for greater precision.
:::
`r if( !includeEvenAnswers) "-->"`
...
**3.** Info not relevant to goldfish.
:::
...
Answers *very* approximate.
**1.** $r$ moderate, positive; $\hat{y} = 4 + 1.5x$
**2.** $r$ reasonably strong, positive; $\hat{y} = 6 + 2.3x$.
**3.** $r$ not apt: variation in $y$ increases as $x$ increases.
...
**2.** Inappropriate: non-linear.
**3.** Inappropriate; $\hat{y}$, not $y$; slope, intercept values *swapped*.
**4.** Very dodgy.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** $H_0$: $\rho = 0$; $H_1$: $\rho \ne 0$.
**2.** No evidence of relationship.
**3.** Approx. linear; variation in STAI same for diff levels of experience.
$n \ge 25$.
:::
...
Non-linear for larger gestational ages; variation not constant.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** $b_0$: *no* time spent on application, mean $0.27\gs$ applied; nonsense.
       $b_1$: each extra minute adds average of $2.21\gs$ sunscreen.
**2.** Slope: g/min; intercept: g.
**3.** $\beta_0$ could be zero; makes sense.
**4.** $\hat{y} = 18\gs$.
**5.** $64$% reduction in unexplained variation using time.
**6.** $r = 0.8$; strong positive correlation.
:::
...
**1.** Negative, moderate.
**2.** $R^2 = 67.07$%; unexplained variation in CBR reduces by about $67$% using PI.
**3.** From $-0.54$ to\ $-0.94$.
...
**4.** $t = -7.45$; two-tailed $P$ very small: $P < 0.00001$.
**5.** Negative; not sure of value.
:::
`r if( !includeEvenAnswers) "-->"`
...
**2.** $r = -\sqrt{0.9929} = -0.9964$; must be negative.
**3.** $\hat{y} = 17.47 - 2.59x$: $x$ is percentage bitumen by wt; $y$ is percentage air voids by volume.
**4.** *Slope*: increase in bitumen wt by one percentage point *decreases* average percentage air voids by volume by $2.59$ percentage points.
       *Intercept*: extrapolation: $0$% bitumen content by wt, percentage air voids by volume $17.47$%.
**5.** $t = -74.9$: massive; extremely strong evidence ($P < 0.001$) of relationship.
**6.** $P < 0.001$, as for slope.
**7.** $\hat{y} = 4.5027$, or $4.5$%; good prediction, as relationship strong.
**8.** $\hat{y} = 1.909$, or $1.9$%; perhaps poor: extrapolation.
**9.** Yes.
:::
...
**1.** $r = \sqrt{0.2304} = 0.48$.
**2.** $\hat{y} = 110.6 + 0.021x$.
**3.** $t = 4.92$; $P < 0.001$; strong evidence of relationship.
**4.** $P < 0.001$, same as for slope.
**5.** Each extra m of elevation associated with mean increase in weight of $0.021\gs$.
**6.** $131.6\gs$; fine.
**7.** $114.8\gs$; extrapolation.
:::
`r if( !includeEvenAnswers) "-->"`
...
**1.** $r = 0.271$; $R^2 = 7.3$%.
**2.** $t = 1.35$; $P = 0.190$; no evidence of relationship.
**3.** $\hat{y} = -5.647 + 0.123x$.
:::
...
**9.** $\hat{y} = 16.41 - 0.96x$; $y$ price in thousands of dollars, $x$ age in years.
**10.** $r = -0.922$,\; $R^2 = 85$%.
**11.** $\hat{y} = 16.41 - (0.96\times 20) = -2.79$: extrapolating.
...
**1.** Left probably F.
**2.** Sex\ B.
**3.** A: $r = 0.600$; B: $r = 0.815$.
**4.** B (M).
**5.** A: $\hat{y} = -2289 + 21.31x$; B: $\hat{y} = -3621 + 27.63x$.
**6.** Both: $P < 0 .0001$ (probably use one-tailed test). 
**7.** A: $2\,503\kgs$; B: $2\,596\kgs$.
**8.** Sample sizes, linearity OK; for Sex\ B, perhaps increasing variation.
:::
...
**3.** $\rho$ possibly zero.
**4.** Not shown.
**5.** Sex confounding variable; need separate reports for men's, women's.
:::
`r if( !includeEvenAnswers) "-->"`
...
```{r eval=FALSE}
data(Jeans)
mean(Jeans$MaxHeightFront, na.rm=TRUE)
mean(Jeans$MaxWidthFront, na.rm=TRUE)
tapply( Jeans$MaxHeightFront, list(Jeans$Sex), mean, na.rm=TRUE)
tapply( Jeans$MaxWidthFront, list(Jeans$Sex), mean, na.rm=TRUE)
```



::: {.answer data-latex=""}
**Ex.\ \@ref(exr:RegressionDogsLife).**
$r = -0.712$ and $P < 0.001$.
Regression: $\hat{y} = 13.39 - 0.093x$.
:::
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:SamplingDistributionA).**
Only **3.**
**2.** Almost certainly; $n = 24$ very close to $n = 25$.
:::


`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:SamplingDistributionB).**
Only **2.**
:::
`r if( !includeEvenAnswers) "-->"`
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:Method3).**
Comparing two proportions (or ORs); stacked, side-by-side bar chart.
CI for diff in proportions (or CI for OR).
:::
...
### Chap.\ \@ref(WritingResearch): Writing and reporting research {.unlisted .unnumbered}
...
**1.** *to*.
**2.** *its*.
**3.** One sample of $50$ individuals; use 'mean' or 'median', not 'average'; units of age is 'years'.
**4.** Should all be one sentence.
:::
...
**1.** Ambiguous; sound like cage is M; passive.
...
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:WriteAmbiguous3).**
**1.**  'Substantial' if a *large* change is expected (quote statistics (e.g., $P$-value) if *statistically* significant intended).
**2.** 'The data *are*...'
:::
...
`r if( !includeEvenAnswers) "<!--"`
::: {.answer data-latex=""}
**Ex.\ \@ref(exr:WriteAmbiguous4}
1. Correlation inappropriate for nominal data like sex.
2. 'Group' is singular.
:::
`r if( !includeEvenAnswers) "-->"`
...
Variables *qualitative*: means inappropriate; use OR; values almost certainly refer to CI for OR.\spacex
...
**1.** Convenience; self-selected. 
       Those in study *may* be different than those not in.
**2.** Inclusion criteria.
**3.** Ethical (drop-outs happen); accurate description of study.
**4.** Not ecologically valid.
**5.** Paired $t$-test.
**6.** Null: no mean diff between counts on phone, manually counted; alternative: a diff.
**7.** $P$ small; evidence mean diff in step-count between methods cannot be explained by chance: likely a diff.
**8.** Valid.
:::
...
**2.** Observational.
**3.** Random method used (but small proportion did not participate): results should generalise to population (students at that uni).
**4.** Double-barrelled.
**5.** $\hat{p} = 0.8603$; $\text{s.e.}(\hat{p}) = 0;011781$; $0.837$ to\ $0.884$.
**6.** $t$-test comparing two means.
**7.** Null: mean HQL score same for F, M.
       Alt: different mean scores.
**8.** $\text{s.e.}(\text{diff}) = 0.2375$.
**9.** $t = -2.61$; expect small $P$; evidence mean diff for F, M.
**10.** $0.145$ to\ $1.085$, higher for M.
**11.** Yes.
...
**3.** Two-sample $t$-test.
**4.** $0.03376$.
**5.** $t = 2.07$; small $P$; evidence of diff.
**6.** Probably, given std errors rounded.
**7.** Conceptual.
**8.** Not shown.
**9.** $\chi^2$.
**10.** $z = 1.75$; $P$ between $5$% and\ $32$%: not helpful.
**11.** Observational: not cause-and-effect; no confounders noted; very restricted population.
:::
...
**6.** $5.61$.
**7.** Very small; very strong evidence proportion of wells with Se concentration less than  $2$\ $\mu$g.L^$-1$^ different for areas with, without Pliocene rocks within $5\kms$.
:::
`r if( !includeEvenAnswers) "-->"`
...
**7.** Yes.
**8.** Yes.
**9.** *Cx.*: evidence mean sizes at temps diff; *Ae.*: no evidence mean sizes at temps diff.
...
**14.** When predator--size ratio increases by one, predation efficiency increases by $31.64$ percentage points.
...
**1.** No; patients may have tried to open their mouth wider than usual.
...
**9.** Very strong evidence M have larger mean MMO than F.
**10.** Yes; relationship between height and MMO may be because M are taller on average.
...
**1.** Stratified?
**2.** Strong evidence the mean number of actinomycetes diff.
**3.** Possibly not; sample sizes small.
**4.** Very strong evidence mean number higher in CNV.
**5.** Larger actinomycetes numbers linearly associated with lower corky root severity.
...
**1.** Control, so any increase has a reference value.
**2.** Experimental.
**3.** Objective measures; aid managing placebo effect.
**4.** To ensure comparison groups included women of similar age (to help manage confounding).
**5.** $0.92208$.
**6.** $t = 0.542$.
**7.** Yes; no evidence of diff in mean age.
**8.** Yes.
**9.** To ensure comparison groups included women in similar rooms (to manage confounding).
**10.** Not shown.
**11.** $z = 0.592$; no evidence of diff in percentage of room with air-con.
**12.** Yes.
**13.** Yes.
**14.** Paired $t = 8.93$; very small $P$; very strong evidence of mean increase.
**15.** Paired $t = 5.33$; small $P$; very strong evidence of mean increase.
**16.** Hawthorne effect.
**17.** $\text{s.e.}( \bar{x}_T - \bar{x}_T) = 4.378$.
**18.** $-2.16$ to $15.36\mins$, greater for treatment group.
        $t = 1.51$; one-tailed $P$ not very small; no diff between mean increase in sleep times.
**19.** Yes.
:::
`r if( !includeEvenAnswers) "-->"`
