...
Each group should be summarised too.
...
```{r SnakesSummaryTestjamovi, fig.cap="Software output for the snakes data", fig.align="center", out.width=c("60%","100%"), fig.show='hold'}
...
For the garter-snake data, define the differences as the mean for females snakes living in non-crayfish regions ($N$), *minus* the mean for female snakes in crayfish regions ($C$): $\mu_N - \mu_C$.
...
## Tests for comparing two means: $t$-test
...
The jamovi output (Fig.\ \@ref(fig:SnakesSummaryTestjamovi)) gives the $t$-score, but the $t$-score can also be computed using the information in Table\ \@ref(tab:SnakesNumericalTest): \index{Hypothesis testing!difference between two means}\index{Test statistic!$t$-score} 
\begin{align*}
   t
  &= 
...
A small $P$-value suggests the observations are *inconsi)stent* with the assumption of no difference (Table\ \@ref(tab:PvaluesInterpretation)), and the difference between the sample means could *not* be reasonably explained by sampling variation.
...
* When *both* samples have $n > 25$, the test is statistically valid provided *neither* distribution is highly skewed.
...
[*Dataset*: `BMI`]
@johnson2021association collected data from hospital outpatients patients at an Irish hospital (Table\ \@ref(tab:BMIsummary)).
...
An error bar chart can be used to display the three groups (Fig.\ \@ref(fig:BMIerrorbar)).

As per Sect.\ \@ref(AboutHypotheses), the null hypothesis is 'no difference' between the means:  
\[
...
(The *test statistic* here is an $F$-score;\index{Test statistic!$F$-score} we don't discuss these further.)
...
One option might seem to be to compare all *pairs* of groups (i.e., $\mu_D$ vs $\mu_O$; $\mu_D$ vs $\mu_N$; $\mu_O$ vs $\mu_N$) using three two-sample $t$-tests.
However, this approach increases the probability of *incorrectly* declaring a difference between the means (increasing the chance of a Type\ I error;\index{Type I error} Sect.\ \@ref(TypeErrors))
...
```{r BMIerrorbar, fig.align="center", fig.cap="The error bar chart for comparing the number of days per week on which people do more than $30$\ mins of exercise, for different smoking status", fig.width=5, out.width='60%', fig.height=3.25}
mns <- BMItab[, 1]
ses <- BMItab[, 3]
ci.lo <- mns - ses*2
ci.hi <- mns + ses*2
...
ANOVA is a general tool that can be extended, and used in many and varied context for analysis of data.
...
At one site (Ningxuan Freeway), speeds were recorded for $38$ vehicles *before* the extra signage was added (denoted $B$), and then for $41$ different vehicles *after* the extra signage was added (denoted $A$); the data are shown in Sect.\ \@ref(SpeedSignageCI).
...
The data are summarised in Table\ \@ref(tab:SignageSummaryHT), where
\[
...
if( knitr::is_latex_output() ) {
  kable(pad(SignageSummary,
        ...
        col.names = c("Mean", 
                      ...
                      "Standard error",
                      "Sample size"),
        caption = "The signage data summary (in km.h$^{-1}$), before and after extra signage added"
  ) %>%
  row_spec(0, bold = TRUE) %>%
  row_spec(3, italic = TRUE) %>%
  kable_styling(font_size = 8)
...
1. Compute the *standard error* for the difference.
`r if( knitr::is_html_output() ) {mcq( c(
  "0.3391",
  "0.3928",
