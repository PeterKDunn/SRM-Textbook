...
A study examined the Australian's acceptance to eating insects [@hopkins2023food].
The researchers asked participants about their levels of education and whether they had previously eaten insects to cross-classified the $n = 601$ respondents.

Since both variables are qualitative, *means are not appropriate* for summarising the data.
A two-way table of counts is appropriate (Table\ \@ref(tab:EatInsectsTableHT)), called a *contingency table*.
...
A *graphical summary* is shown in Fig.\ \@ref(fig:EatInsectsGraphsCI) (left panel), and a *numerical summary* in Table\ \@ref(tab:EatInsectsNumericalSummaryHT).
(The details of the computations appear in Sect.\ \@ref(OddsRatioIntro) NO THEY DO NOT!!).
...
EatInsectsNumericalSummary[3, ] <- c(round( (EatInsects.tab.Counts[1, 1] / EatInsects.tab.Counts[1, 2] ) / (EatInsects.tab.Counts[2, 1] / EatInsects.tab.Counts[2, 2]), 3), 
                                 NA,
                                 NA)
rownames(EatInsectsNumericalSummary) <- c("Uni education",
                                          "No uni eduication",
                                          "Odds ratio")
...
For the data in Table\ \@ref(tab:EatInsectsDataTableHT), the software output can be interpreted in *either* of these ways (i.e., both are correct): 
...
* The *odds* are the odds of eating most meals *off-campus* (Row\ 1) compared to *on-campus* (Row\ 2; on the bottom of the fraction)
  ...
  * for students not living with their parents (Column\ 2): $105/24 = 4.375$. 
  ...
  So the OR is $26/4.375 = 5.943$ (Column\ 2 on bottom of the fraction), as in the output (Fig.\ \@ref(fig:EatInsectsTestOutputHTjamovi)).
...
* The *odds* are the odds of living with parents (Column\ 1) compared to not living with parents (Column\ 2; on the bottom of the fraction):
  ...
  * for those eating most meals on-campus: and $2/24 = 0.083333$.
  ...
  So the OR is $0.49524/0.083333 = 5.943$ (Row2  on bottom of the fraction), as in the output (Fig.\ \@ref(fig:EatInsectsTestOutputHTjamovi)).
...
From Table\ \@ref(tab:EatInsectsDataTableHT), $157$ students out of $183$ ate most meals off-campus, so that  
\[
...
Those are the *expected* numbers if the percentage was exactly the same in each group (Table\ \@ref(tab:EatInsectsDataTableHTExpected)), if the null hypothesis (the assumption) was true.


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider the expected counts in Table\ \@ref(tab:EatInsectsDataTableHTExpected).
...
How do those *expected values* (Table\ \@ref(tab:EatInsectsDataTableHTExpected)) compare to what was *observed* (Table\ \@ref(tab:EatInsectsDataTableHT))?
...
```{r EatInsectsDataTableHTExpected}
...
EatInsect.ExpCounts <- rbind( EatInsects.ExpCounts, 
                               "Total" = round( colSums(EatInsects.ExpCounts)) )

if( knitr::is_latex_output() ) {
  kable( surroundMaths(EatInsects.ExpCounts,
                       decDigits = c(3, 3, 0)),
        format = "latex",
        booktabs = TRUE,
        longtable = FALSE,
...
The [decision-making process](#DecisionMaking) compares what is *expected* if the null hypothesis about the parameter is true (Table\ \@ref(tab:EatInsectsDataTableHTExpected)) to what is **observed** in the sample (Table\ \@ref(tab:EatInsectsDataTableHT)).
...
From the software (Fig.\ \@ref(fig:EatInsectsTestOutputHTjamovi)), $\chi^2 = 1.15$.
...
```{r EatInsectTestOutputHTjamovi, fig.show="hold", fig.cap="The jamovi output for computing a CI and conducting a test", fig.align="center", out.width="50%"}
