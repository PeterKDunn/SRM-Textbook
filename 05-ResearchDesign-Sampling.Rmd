...
This is cheaper than [simple random sampling](#SRS),
...
Often the best we can do is to select
a sample that is diverse enough to be somewhat *representative*\index{representative samples}
...
:::{.exampleExtra #SamplingChildCare name="Sampling"}
...
Even so, the impact of this on the conclusions shoud be noted (that is, in discussing the limitations of the study).
...
The problem is that the responses from those who *do not* respond may be different than the responses who *do* respond.
...
`r if( knitr::is_html_output(exclude = "epub") ) {longmcq( c(
	"It is easier.",
	"It is more likely to produce an internally-valid study.",
...
Which of the following statements are true?  
   ...
   * The sample size is too small for the study results to be useful for the general population.
   `r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
   * This study is likely to have a high *internal* validity.
   `r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = TRUE) }`
