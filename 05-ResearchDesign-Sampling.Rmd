...
```{r DealCards, echo=FALSE, animation.hook="gifski", cache=FALSE, interval=0.20}
library(png)
...
showCardSampling <- function(i, numSamples, numCardsPerSample){
  ...
  sampleGapWidth <- out$gapWidth
    ...
    # so we use the same card repeatedly.
    jitterPack <- -0.01
...
if (knitr::is_html_output()) {
   for (iteration in 1:( (numSamples * numCardsPerSample) + 20)){
     ...
     points(-2.1, -2.25 + iteration/1000, 
            col = "white",
            cex = 0.5) # To keep getting a new plot, so the final plot holds to be looked art
   }
}
```
...
```{example name="Precision and accuracy"}
...
if( knitr::is_html_output() ) {
   ...
   SampleTypes[1, 5] <- "![](./Pics/iconmonstr-chart-22-240.png){#id .class height=auto width=90px}"
   SampleTypes[2, 5] <- "![](./Pics/iconmonstr-layer-17-240.png){#id .class height=auto width=90px}"
   SampleTypes[3, 5] <- "![](./Pics/iconmonstr-view-5-240.png){#id .class height=auto width=90px}"
   SampleTypes[4, 5] <- "![](./Pics/iconmonstr-view-4-240.png){#id .class height=auto width=90px}"
...
Suppose a sample of 40 students in this course is needed.
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.4}
# SRS
if (knitr::is_html_output()){
...
```{example name="Systematic sampling"}
Suppose for a particular study,
a sample of 40 students in this course is needed.
...
```{example name="Stratified sampling"}
To select students in this course again,
20 of the females and 20 of the males could be selected.
The sample is stratified by *sex* of the person.

Alternatively,
since about 67% of USC students are females,
two-thirds of the sample could be chosen to be females 
(around 26.7, say 27) 
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.2}
...
if (knitr::is_html_output()){
  ...
  for (i in (1:sample.female)){
    ...
    mtext(paste("Select", sample.female,"females"),   side=1, at= separation/2)
    mtext(paste("Select", sample.male,"males"),   side=1, at= separation + (num.x-separation)/2)
...
<!-- ```{r Stratified5050, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 20 female and 20 male students." } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/SamplingStratifiedEqual41") -->
<!-- } -->
<!-- ``` -->



<!-- ```{r Stratified6733, echo=FALSE, fig.align="center", fig.width=5, fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 27 female and 13 male students." } -->
<!-- if (knitr::is_latex_output()){ -->
<!--   include_graphics("./Animations/SamplingStratifiedProp41") -->
<!-- } -->
<!-- ``` -->
...
<!-- ```{r echo=FALSE} -->
<!-- htmltools::tags$iframe(title = "Systematic Sampling, equal numbers", src = "./Animations/SamplingStratifiedEqualMovie.html", frameborder=0, height=640, width=520)  -->
<!-- ``` -->
...
```{r echo=FALSE, animation.hook="gifski", cache=TRUE, interval=0.75}
 if (knitr::is_html_output()) {
    # CLUSTER
  ...
  plot( c(1, max.tutorial), c(1, num.tutorials),
        ...
        xlab="",
        ylab="")
  axis(side=2, at=1:num.tutorials, labels=paste("Tut.", 1:num.tutorials), las=1)
...
To find a sample of USC students at Sippy Downs,
students at Cafe C could be approached
...
* Students at Cafe C on Monday at 8am;
* Students at the Brasserie on Tuesday at 11:30am; and
...
For example,
the sex and age of a sample of USC students may be recorded;
if the proportion of females in the sample,
and the average age of students in the sample,
are similar to those of the whole USC population,
then the
...
```{r animation.hook="gifski", cache=TRUE, echo=FALSE, interval=1.5}
if (knitr::is_html_output()){
   for (i in 1:4){
...
in a book in the USC library (including all campuses),
...
```{exercise SamplingParking}
Suppose we needed a sample of 80 vehicles parking at USC (Sippy Downs), 
