...
The challenge of research is to learn about a population from studying just one of the many possible samples.
...
A study (based on @data:aspirin:headaches) of the effectiveness of aspirin in treating headaches cannot possibly study every single human alive who may one day wish to take aspirin.
...
Which do you think correctly predicted the winner of the election, and why?
:::
...
The first question addresses *accuracy*, while the second addresses *precision* (Fig. \@ref(fig:PrecisionAccuracy)).
...
A **non-random** sample requires some personal input.
...
For example, the poll could have been completed multiple times, and by non-Australians as well as Australians.
...
For example, finding a simple random sample of wombats [@yang2018wombats] would require having a list and location of all wombats, so some could be selected using random numbers.
...
Every individual in the population must be known to be in one, and only one, cluster.
...
For example, to obtain a random sample from a country, here is one approach:
...
This is cheaper than [simple random sampling](#SRS), as data collectors can be employed in a small number of cities (only those chosen in Stage 1).
:::
...
1. Suppose a student is randomly selected and sent a postal survey, but the student has moved address and so never receives the survey.
...
Suppose you need a sample of 20 residents from apartments in a large residential apartment complex, comprising 20 floors with 30 apartments on each floor.
