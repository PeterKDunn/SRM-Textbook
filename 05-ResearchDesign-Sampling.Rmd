...
A RQ implies that every member of the population should be studied (the **P** in **P**OCI stands for 'population').
...
This is only possible if the sample faithfully represents the whole population.
...
A study of the effectiveness of aspirin in treating headaches cannot possibly study every single human who may one day wish to take aspirin.
...
The second issue is studied later (Chap. \@ref(EstimatingSampleSize)), after learning more about the implications of studying samples rather than populations.
...
`r if (knitr::is_latex_output()) {
   'Consider a fair pack of cards, where 50% of cards are red. The percentage of red cards is not the same in every sample of ten cards.'
} else {
   'The animation below shows how the estimates calculated from a sample vary from sample to sample. We know that 50% of cards in a fair, shuffled pack are red, but each sample of ten cards can produce a different percentage of red cards (and not always 50%).'
}`
...
The first question addresses *accuracy* of any population quantity estimated using a sample, while the second addresses *precision* with which the population quantity is estimated using a sample.
...
One key to obtaining accurate estimates about the population from the sample is to ensure that the sample is representative of the population of interest (that is, to ensure the study is externally valid).
...
::: {.example #SimpleRandomSample name="Selecting a simple random sample"}
Suppose budget and time constraints mean only $40$ students (out of $441$) can be selected for the study above.

One way to select a simple random sample using the course enrolment list is to first place all $441$ enrolled students into a spreadsheet.
