...
Since a sample doesn't include every member of the population, any conclusions made from samples cannot be *certain* to apply to the whole population.



::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The challenge of research is to learn about a population from an imperfect sample.
...
A study (based on @data:aspirin:headaches) of the effect of aspirin in treating headaches cannot possibly study every single human alive who may one day wish to take aspirin.
...
The second issue must be left until later (Sect. \@ref(EstimatingSampleSize)), after learning more about the implications of studying a sample rather than a population.
...
`r if (knitr::is_latex_output()) {
   'Consider a fair pack of cards, where 50% of cards are red. Figure \\@ref(fig:SampleCardsRed) shows five samples of 10 cards each. The percentage of red cards is not the same in every sample (and not always 50%).'
} else {
   'The animation below shows how the estimates calculated from a sample vary from sample to sample.  We know that 50% of cards in a fair pack are red, but each sample of 10 cards can produce a different percentage of red cards (and not always 50%).'
}`



```{r DealCards, echo=FALSE, animation.hook="gifski", cache=TRUE, fig.height = 4, interval=0.20, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
```{r SampleCardsRed, echo=FALSE, cache = FALSE, fig.align="center", fig.width=7,out.width='70%',  fig.cap="The sample of 10 cards do not always produce the same sample percentage of red cards" }
...
Which do you think predicted correctly the winner of the election, and why?
:::
...
As an analogy, consider an archer shooting at a target.
The shots can be accurate, or precise... or ideally both (Fig. \@ref(fig:PrecisionAccuracy)).
An estimate that is not accurate is called [*biased*](#def:Bias).


```{r PrecisionAccuracy, echo=FALSE,fig.align="center", fig.cap="Precision and accuracy: Each coloured dot is where a shot lands, and is like a sample estimate of the population value (shown by the black central dot)", fig.width=5.5, out.width='70%'}
par( mfrow = c(2,2),
     mar = c(1, 1, 1, 1),
...
The sample gives a precise and accurate answer to a *different* question: 'What is the average age of Canadian school children?'
:::
...
In a *random sample*, each individual in the population can be selected on the basis of impersonal chance (for example, using a random number generator, or a table of random numbers).
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Using a non-random sample means that the results probably do not generalise to the intended population: they probably do not produce externally valid studies.
:::
...
For example, the poll could have been completed by non-Australians as well as Australians.
...
While this sample was not a *truly* random sample of Australians, it was close to a random sample:

> ... researchers made efforts to ensure the sample included individuals representing a wide range of demographics (e.g., age, gender, location, income, political preferences, religiosity).
>
...
For example,  finding a simple random sample of wombats [@yang2018wombats] would require having a list of all wombats, so some could be selected using random numbers.
This is absurd; other random sampling methods, such as 
`r if (knitr::is_html_output()){
...
Describe *how* to use the course enrolment list to find a simple random sample of 40 students.\label{thinkBox:SRS40Students}
...
```{r SRS40, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5,  out.width='70%', out.width='70%', fig.cap="Taking a simple random sample of size 40" } 
# SRS
if (knitr::is_latex_output()){
...
```{r Systematic40, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5,  out.width='70%', fig.cap="Taking a systematic random sample of size 40, by starting at student number 9 and then talking every 11th person" } 
# Systematic
if (knitr::is_latex_output()){
   set.seed(456780)
  sample.size <- 40
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Care needs to be taken when using systematic samples to ensure a pattern is not hidden in the population.
For example, consider taking a systematic sample of every 10th residence on a long street.
In Australia, odd numbers are usually on the same side of the street, and even numbers usually all on the other side of the street.
Selecting every 10th house (for example) would include houses all on the same side of the street, and hence with similar exposure of the sun and traffic.
:::
...
Every individual in the population must be known to be in one, and only one, stratum.
...
```{r Stratified5050, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5, out.width='70%', fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 20 female and 20 male students." } 
# STRATIFIED,  By M/F: 20:20
if (knitr::is_latex_output()){
  set.seed(34567)
...
```{r Stratified6733, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5,  out.width='70%', fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 27 female and 13 male students." }
# STRATIFIED, By M/F: 30:10-ish
if (knitr::is_latex_output()){
  set.seed(34567)
  sample.size <- 40
...
```{r Cluster40, echo=FALSE, fig.align="center", fig.width=7, fig.height=6.5,  out.width='70%', fig.cap="Taking a cluster random sample of size 40, using the tutorials as clusters" }
if (knitr::is_latex_output()) {
    # CLUSTER
...
```{r Multistage40, echo=FALSE, fig.align="center", fig.width=7, fig.height=6.5,  out.width='70%', fig.cap="Taking a multistage random sample of size 40, by randomly selecting 10 classes at random in Stage 1, then randomly selecting students from each class in Stage 2" }
if (knitr::is_latex_output()){
   ### MULTI-STAGE
...
For example, to obtain a random sample from a country, one procedure is:
...
This is cheaper than [simple random sampling](#SRS), as data collectors can be employed in a smaller number of cities (only those chosen in Stage 1).
:::
...
In practice, sometimes the best compromise is to select a sample diverse enough to be somewhat *representative* of the diversity in the population; that is, where those *in* the sample are not likely to be different than those *not in* the sample, at least for the variables of interest, are preferred.
...
Probable (but not certainly) their views would be similar to those of all Australians.
...
To determine if the sample is somewhat representative of the population, sometimes information can be recorded from those in the sample and compared to information about the population.
...
The sample is [biased](#def:Bias) because those *in* the sample may be different than those *not in* the sample (even if this is not obvious).
...
Suppose you need to estimate the average number of pages in a book in a university library (including all five campuses), using a sample of 200 books.
...
Suppose you need a sample of 20 residents from apartments in a large residential apartment complex, comprising 20 floors with 30 apartments in each floor.
You plan to survey the residents of these apartments.
For each of the possible sampling schemes given below:

1. Describe the sampling scheme.
2. Determine which methods are good, and which are poor (explaining your answers).
...
1. Describe these sampling methods:
...
b. The study is likely to have high *external* validity.
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
c. The sample size is too small for the study results to be useful for the general population.
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
:::
...
- Sect. \ref{thinkBox:SRS40Students}: Place the list of all 441 students enrolled in the course into a spreadsheet, and then use random numbers to select 40 at random (without repeating numbers) between 1 and 441 inclusive.
Every student that was chosen using this method would be part of the study.
If a student could not be contacted, more students could be chosen at random to ensure 40 students participated.
- Sect. \ref{thinkBox:ComputerBias}: **Non-response bias**: Those who respond are different than those who do (less likely to own a computer).
- Sect. \ref{thinkBox:TypesOfSamples}:
There are many correct answers; here are some: 1. The percentage of people that own a telephone. 2. The percentage of people that are shift-workers 3. The percentage of people studying at university.
