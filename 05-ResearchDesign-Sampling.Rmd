...
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:SamplesA), top right panel).'
} else {
   '(see animation below).'
}`
:::
...
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:SamplesA), bottom left panel).'
} else {
   '(see animation below).'
}`
...
`r if (knitr::is_latex_output()) {
   '(Fig.\\ \\@ref(fig:SamplesA), bottom right panel).'
} else {
   '(see animation below).'
}`
...
```{r SamplesB, fig.align="center", fig.width=8, fig.height=5,  out.width='100%', fig.cap="Two more methods for taking a random sample of size $40$. Left: a cluster sample; right: A multistage sample.  Classes shown in bold represent classes selected to be in the sample in the first stage. In all cases, circles represent students in the population not chosen for the sample, and the filled squares represent the students in the population who were chosen in the sample." } 
...
For the study in Example\ \@ref(exm:Typing), a *simple random sample* of ten of the many small-roup classes could be selected (Stage\ 1), and then four students are *randomly* selected from each of these $10$ selected tutorials (Stage 2)
`r if (knitr::is_latex_output()) {
