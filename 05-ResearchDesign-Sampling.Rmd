...
```{r DealCards, echo=FALSE, animation.hook="gifski", cache=TRUE, fig.height = 4, fig.width=7, out.width='60%', interval=0.20, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
The first question addresses *accuracy*, while the second addresses *precision*.
...
```{r PrecisionAccuracy, echo=FALSE,fig.align="center", fig.cap="Precision and accuracy: Each dot is where a shot lands, and is like a sample estimate of the population value (shown by the black central dot)", fig.width=5.0, out.width='60%'}
par( mfrow = c(2,2),
     mar = c(1, 1, 1, 1),
...
Describe *how* to use the course enrolment list to find a simple random sample of 40 students (Fig. \@ref(fig:SRS40)).\label{thinkBox:SRS40Students}
...
```{r SRS40, echo=FALSE, fig.align="center", fig.width=5.5, fig.height = 5.5,  out.width='50%', fig.cap="Taking a simple random sample of size 40" } 
# SRS
if (knitr::is_latex_output()){
...
```{r Systematic40, echo=FALSE, fig.align="center", fig.width=5.5, fig.height = 5.5,  out.width='50%', fig.cap="Taking a systematic random sample of size 40, by starting at student number 9 and then talking every 11th person" } 
# Systematic
if (knitr::is_latex_output()){
   set.seed(456780)
  sample.size <- 40
...
`r if (knitr::is_latex_output()) {
   'Figure \\@ref(fig:StratifiedTwoWays) (left panel) shows a diagram of selecting a stratified random sample of size 40, by randomly selecting 20 female and 20 male students; the right panel shows a diagram of selecting a stratified random sample of size 40, by randomly selecting 27 female and 13 male students. (The online version has animations.)'
} else {
...
if (knitr::is_html_output()){
  ...
  for (i in (1:sample.female)){
    ...
    sample.colF[selected.female[1:i]] <- plotDark
  ...
  for (i in (1:sample.male)){
    ...
    sample.colF <- rep("grey", length=num.female*num.y)
    sample.colF[selected.female] <- plotDark
    ...
    sample.colM[selected.male[1:i]] <- plotDark
...
```{r StratifiedTwoWays, echo=FALSE, fig.align="center", fig.width=9.5, fig.height = 5.5, out.width='100%', fig.cap="Selecting a stratified random sample of size 40, by (left) randomly selecting 20 female and 20 male students, and (right) by selecting 27 females and 13 males." } 
# STRATIFIED,  By M/F: 20:20
...
#  points( EGmale$Var1, EGmale$Var2)
  ...
  for (i in (sample.female)){
    ...
    sample.cexF <- rep(1, 
                       length = num.female*num.y)
    sample.cexF[selected.female[1:i]] <- 1.3
  ...
  for (i in (sample.male)){
    ...
    sample.cexF <- rep(1, 
                       length = num.female*num.y)
    sample.cexF[selected.female] <- 1.3
...
#  points( EGmale$Var1, EGmale$Var2)
  ...
  for (i in (sample.male:sample.male)){
    ...
    sample.cexM <- rep(1, 
                       length = num.male*num.y)
    sample.cexM[selected.male[1:i]] <- 1.3
...
```{r echo=FALSE, animation.hook="gifski", fig.height = 6.5, cache=FALSE, interval=0.75, dev=if (is_latex_output()){"pdf"}else{"png"}}
 if (knitr::is_html_output()) {
  ...
  for (i in (1:num.tutorials)){
    ...
    sample.col <- ifelse( i %in% selected.tutorials, plotDark, "grey")
...
```{r Cluster40, echo=FALSE, fig.align="center", fig.width=5.5, fig.height=5.5,  out.width='50%', fig.cap="Taking a cluster random sample of size 40, using the tutorials as clusters" }
if (knitr::is_latex_output()) {
    # CLUSTER
  ...
  for (i in (1:num.tutorials)){
    sample.pch <- ifelse( i %in% selected.tutorials, 15, 1)
    sample.col <- ifelse( i %in% selected.tutorials, plotDark, "grey")
    sample.cex <- ifelse( i %in% selected.tutorials, 1.3, 1)
    sample.lwd <- ifelse( i %in% selected.tutorials, 2, 1)
...
if (knitr::is_html_output()){
  ...
  for (i in (1:num.tutorials)){
    ...
    if ( i %in% selected.tutorials){
      ...
      sample.col[selected.students] <- plotDark
      sample.cex[selected.students] <- 1.3
...
```{r Multistage40, echo=FALSE, fig.align="center", fig.width=5.5, fig.height=5.5,  out.width='50%', fig.cap="Taking a multistage random sample of size 40, by randomly selecting 10 classes at random in Stage 1, then randomly selecting students from each class in Stage 2" }
if (knitr::is_latex_output()){
   ### MULTI-STAGE
...
1. Suppose a student is randomly selected and sent a postal survey from their university, but the student has moved and so never receives the survey.
...
a. This is a voluntary response sample. `r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = TRUE) }`
b. The study is likely to have high *external* validity. `r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
c. The sample size is too small for the study results to be useful for the general population. `r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
:::
