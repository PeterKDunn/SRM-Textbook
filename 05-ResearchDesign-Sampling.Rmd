...
That is, ideally the sample that is studied is representative of the population, so the the results from the sample generalise to the population.
...
The study is externally valid if the sample is representative of all Californian university students.
...
The differences in the samples, and the results from each sample, are called *sampling variation*.\index{Sampling variation}
...
In a 'population' as small as $100$, the number of possible samples of size $25$ is more than twice the number of people living on earth.
:::


This is the challenge of research: *making decisions about populations, using just one of the many possible samples*.
A lot can be learnt about the population if the task of selecting a sample is approached correctly.\index{Making decisions}
...
The first question addresses the *accuracy*\index{Accuracy} in of a sample value to estimate a population value. 
The second addresses the *precision*\index{Precision} with which the population value is estimated using a sample.
...
would be used instead (e.g., @manly2014introduction).
...
A simple random sample could be found using the course enrolment list, placing all $441$ student names into rows of a spreadsheet (ordered by name, student ID, or any way).
Then, use random numbers to select $40$ rows at random (without repeating numbers) between $1$ and $441$ inclusive.
...
```{r SamplesA, fig.align="center", fig.width=8, fig.height=5,  out.width='100%', fig.cap="Two methods for taking a random sample of size $40$ from a class of $441$. Left: a simple random sample. Right: a systematic sample. $\\bigcirc$ represent older students, triangles $\\bigtriangledown$ represent younger students, and filled shapes represent those individuals selected in the sample." } 
...
@alary1991risk studied households in Quebec to determine if their hot water systems kept their water sufficiently hot avoid Legionellae bacteria.
They used a systematic random sample to select households to study (p.\ 2361):
...
```{r SamplesB, fig.align="center", fig.width=8, fig.height=5,  out.width='100%', fig.cap="Two more methods for taking a random sample of size approximately $40$. Left: a cluster sample. Right: a multistage sample.  Classes shown in bold represent classes selected to be in the sample in the first stage.  $\\bigcirc$ represent older students, triangles $\\bigtriangledown$ represent younger students, and filled shapes represent those individuals selected in the sample." } 
...
\index{Sampling!random!multistage}
...
::: {.example name="Multistage sampling"}
Multistage sampling is often used by national statistical agencies.
...
@egbue2017mass studied of the adoption of electric vehicles (EVs) by Americans, using a sample of $121$ people found through social media (such as Facebook) and professional engineering channels.
...
The sample is biased because those *in* the sample may be different than those *not in* the sample (and this may not always be obvious).
...
This may be intentional (for example, when respondents lie) or non-intentional (for example, if the question is poorly written, is personal, or is misunderstood).


::: {.example #PoorSampling name="Poor sampling"}
Obtaining data using a telephone survey only includes people who own a telephone, who answer the phone, who do not hang up, who volunteer to complete the survey, and who finish the whole survey.

Obtaining data using a TV stations call-in at about $6$:$15$pm only includes people watching that channel, at that time, and who are sufficiently motivated to call.

Sampling students at your university, because it is easier than finding a random sample of all people in your country, will only generalise to students at that university.
:::
...
Consider this (qualitative) RQ [@walters2018factors]: 
...
**1.** Non-response.
**2.** False.
**3.** Stratified; simple random; cluster; multistage.
:::
`r if (knitr::is_html_output()) '-->'`
