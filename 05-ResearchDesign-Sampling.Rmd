...
So far, you have learnt to ask a RQ, and identify a study design.
\smallskip

**In this chapter**, you will learn how to obtain the sample to study.
You will learn to:
...
However, doing so is very rare because of cost, time, ethics, logistics or practicality.
Usually, a subset of the population (a *sample*) is studied, comprising some *individuals*  (or *cases*, or (if the individuals are people) *subjects*) from the population.
...
However, since a sample doesn't include every member of the population, any conclusions made from samples cannot be certain to apply to the whole population.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
In research, the goal is to learn about the *population*, but only a *sample* can be studied.

This book is essentially about how to learn about a population, based on data from just one of the many possible samples.
:::
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-gerd-altmann-23180.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
A study (based on @data:aspirin:headaches) of the effect of aspirin in treating headaches cannot possibly study every single human alive who might one day wish to take aspirin.

Not only would this be prohibitively expensive, time-consuming, and impractical, but such a study would not even study humans yet to be born who might use aspirin.
(That is, using the whole target population is *impossible*.)
...
The second issue must be left [until later](#EstimatingSampleSize), after learning more about the implications of studying a sample rather than a population.

Using a sample instead of the entire population presents challenges.
*Every sample is likely to be different*, so what is learnt from a sample depends on which individuals happen to be in the sample studied.
This is called *sampling variation*.
That is, each sample hsa different infividuals, so produces different data, and so may lead to different answers to the RQ.

This is the challenge of research: *How to make decisions about populations, using an imperfect sample information.*
...
*Every sample is likely to be different*, and hence the *results from every sample are likely to be different*.
This is called *sampling variation*.
...
```{r DealCards, echo=FALSE, animation.hook="gifski", cache=FALSE, fig.height = 4.5, interval=0.20, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
Two surveys were conducted before the 1936 presidential election in the USA to predict the winner [@data:Bryson1976:Poll], summarised in Table \@ref(tab:USAVoting).
...
`r if (knitr::is_latex_output()) '<!--'`
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
<iframe src='https://www.ferendum.com/en/embeded.php?pregunta_ID=879051&sec_digit=620571698&embeded_digit=541828783' style='width:100%; height:400px; overflow: auto; background: #badaff;' frameBorder='0'></iframe><BR />
...
* *how many* individuals should be in the sample be.

These two questions address two different aspects of sampling: **precision** and **accuracy** (Fig. \@ref(fig:PrecisionAccuracy)).
Accuracy is related to the statistical concept of **bias**.
...
*Precision* refers to how close the sample estimates from different samples are likely to be to each other (that is, how much variation is likely in the sample estimates).
:::


Using this language:

* The *method* of sampling (i.e., how the sample is selected) impacts the *accuracy* of the sample estimate.
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-iqwan-alif-1206101.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
To estimate the average age of *all Queenslanders*, we could ask 9000 Queensland school children (a large sample indeed!).

This will give a *precise* answer because the sample is large, but an *inaccurate* answer because the sample is not representative of *all* Queenslanders.
The sample may give a precise and accurate answer to a *different* question: 'What is the average age of Queensland school children?'
:::
...
In a **random sample**, each individual in the population can be selected on the basis of impersonal chance.
...
If the soup is stirred, we don't need to taste the whole pot of soup to taste the soup.

The same principle applies in research: If we use a random sample (analogous to the stirring the soup), we don't need to study *every* member of the population.
If we don't use a random sample (that is, we don't stir the soup), we do not get an *overall* impression of the population (or the soup).


### Non-random sampling methods {#NonRandomSamples}

A **non-random** sample requires some kind of personal input.
Examples of non-random samples include:

* *Judgement sample*:
  Individuals are selected, based on the researchers' judgement (for example, as the individuals may be agreeable, supportive, or helpful.
  For example, researchers may select rats that are less aggressive.
* *Convenience sample*:
...
* *Voluntary* response (self-selecting) sample:
  ...
  For example, researchers may ask peple to volunteer for a response survey.
...
During the COVID-19 pandemic in 2020, [a Facebook poll](https://www.abc.net.au/news/2020-07-03/coronacheck-anti-vaxxers-flood-online-poll-pete-evans/12415860) asked the question:
...
However, this sample was a *voluntary response sample*, not a random sample, so the results may not be *accurate*.
For example, many anti-vaccination groups instructed their members to flood the poll with 'No' responses, and the poll could have been completed by non-Australians as well as Australians.
...
While this sample was not a truly *random* sample, the researchers made efforts to sample a *representative* cross-section of Australians:

> While not a random sample of the Australian [adult] population, researchers made efforts to ensure the sample included individuals representing a wide range of demographics (e.g., age, gender, location, income, political preferences, religiosity).
>
...
Finding a simple random sample requires a list of all members of the population, called the *sampling frame*, from which to make the selections.
...
For example, to find a simple random sample of wombats [@yang2018wombats] would require having a list of all wombats, so some could be selected using random numbers.
This is clearly absurd, and other random sampling methods, such as [special ecological sampling methods](http://www.countrysideinfo.co.uk/what_method.htm), would be used instead [@manly2014introduction].

Selecting a simple random sample from the *sampling frame* is performed using *random numbers* (e.g., Appendix \@ref(AppendixRandomNumbers) or 
`r if (knitr::is_html_output()){
...
Obtaining the *sampling frame* is feasible here (lecturers have access to this information for grading).
:::


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Suppose budget and time constraints mean only 40 (from 441) students can be selected for the study above.
Describe *how* to use the course enrolment list to find a simple random sample of 40 students.

`r webexercises::hide()`
We could place the list of all 441 students enrolled in the course into a spreadsheet, and then use random numbers to select 40 at random (without repeating numbers) between 1 and 441 inclusive.
...
```{r echo=FALSE, animation.hook="gifski", cache=FALSE, interval=0.4, fig.height = 6.5, dev=if (is_latex_output()){"pdf"}else{"png"}}
# SRS
if (knitr::is_html_output()){
...
```{r SRS40, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5, fig.cap="Taking a simple random sample of size 40" } 
# SRS
if (knitr::is_latex_output()){
...
Forty students could be randomly selected by choosing a number at random between 1 and $441/40$ (approximately 11) as a starting point; suppose the random number selected is 9.
The first student selected is the 9th person in the list (which may be ordered alphabetically, by student ID, or any other means).
...
```{r echo=FALSE, animation.hook="gifski", cache=FALSE, fig.height = 6.5, interval=0.4, dev=if (is_latex_output()){"pdf"}else{"png"}}
# Systematic
if (knitr::is_html_output()){
   set.seed(456780)
  sample.size <- 40
...
```{r Systematic40, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5, fig.cap="Taking a systematic random sample of size 40, by starting at student number 9 and then talking every 11th person" } 
# Systematic
if (knitr::is_latex_output()){
   set.seed(456780)
  sample.size <- 40
...
For example, consider a study where residents in a large, eight-level residential accommodation complex are to be visited and a survey administered.
Each floor of the building has a similar layout (Fig. \@ref(fig:ApartmentLayout)), with 10 apartments per level.
A systematic sample of every *tenth* apartment would select the same apartment on each floor.

For example, suppose Apartments 1-10, 2-10, 3-10,... 8-10 were chosen, which are all larger than the other apartments.
The residents of these apartments may be wealthier than the other residents, so the systematic sample will not be a representative sample of all residents.
:::
...
for (i in 1:(Num_Apartments_On_Each_Side-1)){
   ...
   lines( y = c(Bottom, 
                Apt_Height),
          x = c(Top_Wall + Elevator_Width, 
                Top_Wall + Elevator_Width) )
   
}


# Extra wall for Elevator
lines( x = c(Elevator_Width, 
             Elevator_Width),
       y = c(Bottom, 
             Bottom + Apt_Height))

# Extra wall for Stairs
lines( x = c(Left - Stair_Width, 
             Left - Stair_Width),
       y = c(Bottom, 
             Top - Apt_Height),
       lwd = 2)
...
text(x = Elevator_Width/2,
     ...
     srt = 90)
# "Stairs" text
text(x = mean( c(Left, Left - Stair_Width) ),
     y = Bottom + (Apt_Height/2),
     "Stairs",
     srt = 90)
...
polygon(x = c(Left - Stair_Width, 
        ...
        y = c(Bottom, 
              ...
              Bottom),
        col = plot.colour )
# Colour stairs again, overlaying the two colours as for Elevator 
polygon(x = c(Left - Stair_Width, 
              Left - Stair_Width,
              Left,
              Left),
...
polygon( x = c(0, 
               ...
               Right, 
               Right),
         y = c(Bottom + Apt_Height, 
               Bottom + Apt_Height + Hall_Height, 
               Bottom + Apt_Height + Hall_Height, 
               Bottom + Apt_Height),
         col = "white")
...
For example, if the RQ is about comparing the percentage of females and males who wear hats at midday, a *stratified sample* of size 100 is **not** obtained by selecting 50 females and 50 males, for example.
...
To select 40 students in a large course at a particular university, 20 of the females and 20 of the males could be selected.
The sample is stratified by *sex* of the person.

At the university where I work, about 67% of the students are females.
So, I could ensure that two-thirds of the sample comprised females (around 26.7, say 27) and about one-third males (about 13.3, say 13). 
:::
...
```{r, echo=FALSE, animation.hook="gifski", cache=FALSE, interval=0.2, fig.height = 6.5, dev=if (is_latex_output()){"pdf"}else{"png"}}
...
if (knitr::is_html_output()){
  ...
  for (i in (1:sample.female)){
    ...
    sample.cexF[selected.female[1:i]] <- 1.4
  ...
  for (i in (1:sample.male)){
    ...
    sample.cexM[selected.male[1:i]] <- 1.4
...
```{r Stratified5050, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5, fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 20 female and 20 male students." } 
# STRATIFIED,  By M/F: 20:20
if (knitr::is_latex_output()){
  set.seed(34567)
...
#  points( EGmale$Var1, EGmale$Var2)
  ...
  for (i in (sample.female)){
    ...
    points( expand.grid(1:num.female, 
                        1:num.y))
    EGmale <- expand.grid(1:num.male, 
                          1:num.y)
    EGmale$Var1 <- EGmale$Var1 + separation
    ...
    # Plot the selected **Females**
    sample.pchF <- rep(1, 
                       length = num.female * num.y)
    sample.pchF[selected.female[1:i]] <- 15
    
    sample.colF <- rep("grey", 
                       length = num.female*num.y)
    sample.colF[selected.female[1:i]] <- "darkolivegreen4"
  ...
  for (i in (sample.male)){
    ...
    sample.cexF <- rep(1, 
                       length = num.female*num.y)
    sample.cexF[selected.female] <- 1.4
    ...
    # Plot the selected **Males**
    sample.pchM <- rep(1, 
                       length = num.male*num.y)
    sample.pchM[selected.male[1:i]] <- 15
    
    sample.colM <- rep("grey", 
                       length = num.male*num.y)
    sample.colM[selected.male[1:i]] <- "darkolivegreen4"
...
if (knitr::is_html_output()){
  ...
  selected.female <- sample(1:(num.female*num.y), 
                            sample.female)
  selected.male   <- sample(1:(num.male*num.y  ), 
                            sample.male)
...
```{r Stratified6733, echo=FALSE, fig.align="center", fig.width=7, fig.height = 6.5, fig.cap="Selecting a stratified random sample of size 40, by randomly selecting 27 female and 13 male students." }
# STRATIFIED, By M/F: 30:10-ish
if (knitr::is_latex_output()){
  set.seed(34567)
  sample.size <- 40
  ...
  selected <- sample(1:population.size, 
                     sample.size)
...
```{r echo=FALSE, animation.hook="gifski", fig.height = 6.5, cache=FALSE, interval=0.75, dev=if (is_latex_output()){"pdf"}else{"png"}}
 if (knitr::is_html_output()) {
    # CLUSTER
  ...
  num.tutorials <- 20
  num.students.in.tutorials <- array( NA,
                                      dim = num.tutorials) 
  ...
  for (i in (1:num.tutorials)){
    points(1:num.students.in.tutorials[i], 
           rep(i, num.students.in.tutorials[i]), 
           col = "grey" )
  }
  ...
  for (i in (1:num.tutorials)){
    ...
    sample.cex <- ifelse( i %in% selected.tutorials, 1.4, 1)
    sample.lwd <- ifelse( i %in% selected.tutorials, 2, 1)
...
```{r Cluster40, echo=FALSE, fig.align="center", fig.width=7, fig.height=6.5, fig.cap="Taking a cluster random sample of size 40, using the tutorials as clusters" }
if (knitr::is_latex_output()) {
    # CLUSTER
...
In **multistage sampling**, large groups are selected using a *simple random sample*, then smaller groups *within* those large groups are selected using a *simple random sample*.
The simple random sampling can continue for as many levels as necessary.
...
```{r echo=FALSE, animation.hook="gifski", cache=FALSE, interval=0.75, fig.height = 6.5, dev=if (is_latex_output()){"pdf"}else{"png"}}
if (knitr::is_html_output()){
   ### MULTI-STAGE
  ...
  axis(side = 2, 
       ...
       labels = paste("Tut.", 1:num.tutorials), 
       las = 1)
  # BOLD the chosen tutorial classes
  axis(side = 2, 
       at = selected.tutorials, 
       labels=paste("Tut.", selected.tutorials), 
       las = 1, 
  ...
  for (i in (1:num.tutorials)){
    if ( i %in% selected.tutorials){
      # Chosen tutorials have thick, black points for all students
      points(1:num.students.in.tutorials[i], 
             rep(i, num.students.in.tutorials[i]), 
             col = "black",
             lwd = 2)
    } else {
      # Not-chosen tutorials have grey, thinner points for all students
      points(1:num.students.in.tutorials[i], 
             rep(i, num.students.in.tutorials[i]), 
             col = "grey" )
    }
  ...
  for (i in (1:num.tutorials)){
    ...
    sample.lwd <- rep(1, (num.students.in.tutorials[i]))
    
    
    if ( i %in% selected.tutorials){
      sample.col <- rep("black", (num.students.in.tutorials[i]))
      selected.students <- sample(1 : num.students.in.tutorials[i], 
                                  num.students.selected.each.tutorial)
      sample.pch[selected.students] <- 15
      sample.col[selected.students] <- "darkolivegreen4"
      sample.cex[selected.students] <- 1.4
      sample.lwd[selected.students] <- 2
  ...
  title(xlab = paste("Selected classes:", 
                     paste( sort(selected.tutorials), collapse = ", ")))
  title(sub = "Stage 1: Select classes.  Stage 2: Select students in those classes")
}
```


```{r Multistage40, echo=FALSE, fig.align="center", fig.width=7, fig.height=6.5, fig.cap="Taking a multistage random sample of size 40, by randomly selecting 10 classes at random in Stage 1, then randomly selecting students from each class in Stage 2" }
if (knitr::is_latex_output()){
   ### MULTI-STAGE
  ...
  for (i in (1:num.tutorials)){
    ...
    if ( i %in% selected.tutorials){
      ...
      sample.pch[selected.students] <- 15
      sample.col[selected.students] <- "darkolivegreen4"
      sample.cex[selected.students] <- 1.5
      sample.lwd[selected.students] <- 2
...
::: {.example name="Multistage sampling"}
Multistage sampling is often used by the Australian Bureau of Statistics (ABS).
For example, to obtain a random sample of Queenslanders, one procedure is:
...
This is cheaper than [simple random sampling](#SRS), as data collectors can be employed in a smaller number of Queensland cities (only those chosen in Stage 1).
:::
...
In practice, sometimes the best we can do is select a sample diverse enough to be somewhat *representative* of the diversity in the population.
Even so, the results from any non-random sample *may not generalise* to the intended population.
The results will generalise to the population which the sample *does* represent.

Ideally, even if obtaining a random sample is impossible, we prefer a sample where those *in* the sample are not likely to be different than those *not in* the sample, at least for the variables of interest.
...
It is probable (but not certain) that their views would be similar to those of all Australians.
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-juergen-striewski-301048.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-lina-kivaka-3395280.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
Sometimes, information may be recorded from those in the sample, and used to make some comment about whether our sample seems reasonably representative.
...
\begin{wrapfigure}{R}{.25\textwidth}
  \begin{center}
    \includegraphics[width=.20\textwidth]{Illustrations/pexels-rathaphon-nanthapreecha-3846205.jpg}
  \end{center}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`
...
A study of the adoption of electric vehicles (EVs) by Americans [@egbue2017mass] used a sample of $n = 121$ found through social media (such as Facebook) and professional engineering channels.
This is not a random sample.

The authors compared some characteristics of the sample with the American population from the 2010 census (Table \@ref(tab:EVAsdoptCompare)), stating:
   
> The sample has a higher representation of males and individuals in the 18--44 age group [...] compared to the US population.
> In addition, the sample has a higher representation of [...] wealthier individuals. 
>
...
The sample may not be representative of the population for many reasons, all of which compromise how well the sample represents the population (i.e., compromises *external* validity).
This is called *sampling bias*.
Biased samples are less likely to produce externally valid studies.
Selection bias is less common in studies with [forward directionality](ForwardsStudies), compared to studies that are [non-directional](#NondirectionalStudies) or have [backward directionality]#(BackwardsStudies).
...
The sample is [biased](#def:Bias) because those *in* the sample may be different than those *not in* the sample (but this may not be obvious).
...
Using people only from Queensland and the Northern Territory in the sample is using the wrong sampling frame: the sampling frame does not represent the target population ('Australians').
...
People who work a large amount of overtime are likely to be too busy to answer the survey.
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
One (true) survey in 2001 concluded (@data:Heiger:Homebuyer, cited in @bock2010stats, p. 283):
...
The article later reveals the survey was conducted *on-line* (and recall the survey was done in 2001...).

What type of bias is apparent?

`r webexercises::hide()`
**Non-response bias**: Those who respond are different than those who do (less likely to own a computer).
`r webexercises::unhide()`
:::
...
3. Asking students at your university because it is easier than finding a random sample.
...
3. The percentage of Australians that are studying at university.
`r webexercises::unhide()`
:::
...
For a [*systematic sample*](#SystematicSampling), we take:
...
For a [*multi-stage sample*](#MultistageSampling) we select some columns at random, then select some pixels in those columns at random.
...
For a [*stratified sample*](#StratifiedSampling), we select :

* a simple random sample from the background greenery; and
...
Conclusions made from random samples can usually be generalized to the population (that is, they are externally valid).
...
1. Suppose we randomly select a student and send them a postal survey, but the student has moved address and so never receives the survey.
...
1. Select all the sampling methods that are random sampling methods.
   ...
   * Judgement sampling
   `r if( knitr::is_html_output(exclude = "epub") ) {mcq( c(answer = "Not random", "Random"))}`
   * Stratified sampling
   `r if( knitr::is_html_output(exclude = "epub") ) {mcq( c("Not random", answer = "Random"))}`
...
Suppose we needed to estimate the average number of pages in a book in a university library (including all five campuses), using a sample of 200 books.
...
Suppose we need a sample of 20 residents from apartments in a large residential apartment complex, comprising 20 floors with 30 apartments in each floor.
We plan to interview the residents of these apartments.

1. One approach to obtaining a sample is to randomly select five floors, then randomly select four apartments from each of those five floors, and interview the oldest resident of that apartment.
   What type of sampling scheme is this?
1. Another approach is to select one floor at random, and select the first 20 apartments on that floor then interview the oldest resident of that apartment.
   What type of sampling scheme is this?
1. Another approach is to wait at the ground-floor elevator, and ask people who emerge to participate in our interview.
   What type of sampling scheme is this?
1. Another approach is to select five floors at random, then wait by the elevator and interview residents as they arrive at the elevator.
   What type of sampling scheme is this?
1. Which of the above sampling methods are good, and which are poor?
...
Suppose a researcher needs a sample of customers who shop at a large, local shopping centre to complete a survey.

1. The researcher stations themselves outside the supermarket at the shopping centre one morning, and approaches every 10th person who walks past. 
   What is the sampling method?
1. The researcher waits at the main entrance for 30 minutes at 8am every morning for a week, and approaches every 5th person. 
   What is the sampling method?
1. The researcher leaves a pile of survey forms at an unattended booth in the shopping centre, and a locked barrel in which to place completed surveys.
   What is the sampling method?
1. The researcher goes to the shopping centre every day for two weeks, at a different time and location each day, and approaches someone every 15 minutes.
   What is the sampling method?
...
Suppose researchers randomly sampled four schools from a list of Brisbane state schools, and invited every family at each of those four schools to complete a survey. 
...
A study comparing two new prototype vaccines for malaria recruited a sample of 200 Americans who had contracted malaria.
...
Patients were free to volunteer for the study or not.
...
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
* The sample size is too small for the study results to be useful for the general population.
`r if( knitr::is_html_output(exclude = "epub") ) {torf(answer = FALSE) }`
:::
...
Suppose I classify a natural forest region into two zones, which are quite different: Region A is mostly dunes and lightly vegetated, and is on the coastal side of a ridge; Region B is more densely vegetated and on the inland side of the ridge. 
...
**1.** Non-response bias.
**2.** It is more likely to produce an externally-valid study.
**3.** It is more likely to produce precise estimates.
**4.** False.
**5.** Stratified sampling; simple random sampling; cluster sampling; multi-stage sampling.
:::
`r if (knitr::is_html_output()) '-->'`
