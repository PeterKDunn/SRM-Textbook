...
The first question addresses the *accuracy* of a population value when estimated using a sample. The second addresses the *precision* with which the population value is estimated when using a sample.
...
So, I could ensure that two-thirds of the sample of size $40$ comprised female, and onbe-third males; this is about $27$ females and $13$ males
`r if (knitr::is_latex_output()) {
...
In *cluster sampling*, the population is split into a *large* number of *small* groups called *clusters*, then a *simple random sample* of clusters is selected and *every* member of the cluster is part of the sample.
Every individual in the population must be in one, and only one, cluster.


::: {.example name="Cluster sampling"}
For the study in Example \@ref(exm:Typing), a simple random sample of (say) three of the many tutorials could be selected, and *every* student enrolled in those selected tutorials constitute the sample
`r if (knitr::is_latex_output()) {
...
```{r SamplesB, fig.align="center", fig.width=8, fig.height=5,  out.width='100%', fig.cap="Two more methods for taking a random sample of size $40$. Left: a cluster sample; right: A multistage sample.  Classes shown in bold italics represent classes selected to be in the sample in the first stage. In all cases, hollow circles represent students in the population not chosen for the sample, filled circles represent student in the chosen classes who are not selected for the sample, and the filled squares represent the students in the population who were chosen in the sample." } 
...
For the study in Example \@ref(exm:Typing), a *simple random sample* of ten of the many tutorials could be selected (Stage 1), and then four people *randomly* selected from each of these $10$ selected tutorials (Stage 2)
`r if (knitr::is_latex_output()) {
...
To select the days in which to collect data, she decides (by using random numbers) to start data collection on a Tuesday, and then every 7th day thereafter.

1. What problem is evident in this sampling scheme? 
2. Is this problem likely to impact accuracy or precision, or both?
3. What suggestions would you give to improve the sampling?
:::
