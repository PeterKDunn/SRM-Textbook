...
In a toy store one day (for my children...), I saw packs of two dice for sale that proclaimed 'One loaded \& one normal'.
I bought two packs!
...
actually appeared.
Then, using the decision-making process (Sect.\ \@ref(DecisionMaking)), I could decide if that die was the fair die.
...
Even if the die was fair, the value of $\hat{p}$ would not necessarily be *exactly* one-sixth, due to *sampling variation*.
...
However, if I assumed the value of $p$ is $1/6$, the possible values of the *sample* proportion from all possible rolls of the fair die could be determined; that is, the *sampling distribution* could be described.
The sampling distribution would show what values of $\hat{p}$ could reasonably be expected from a die with $p = 1/6$.

If we find that the value of $\hat{p}$ is not *exactly* $1/6$, two explanations exist:

* The *population* proportion *really is* $1/6$.  
  However, $\hat{p}$ is not exactly $1/6$ due to sampling variation.
* The *population* proportion *really is not* $1/6$.  
  That is, $\hat{p}$ is not exactly $1/6$ because the die is not fair.

These two possible explanations are called *statistical hypotheses*.
If $p$ is defined as the population proportion of twos, then the hypotheses above are written as:
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The alternative hypothesis depends on what the research question asks, *not the data*.
:::
...
This is how the values of $\hat{p}$ would vary if $p$ really was $1/6$, and if certain conditions are met (Sect.\ \@ref(ValidityProportionsTest)); see Fig.\ \@ref(fig:RollsSixesSD).
...
The *mean* of this distribution (the *sampling mean*) is the *mean* of all possible values of $\hat{p}$; the value of that mean is $p$.
Similarly, the standard deviation of this distribution is the *standard error*, denoted $\text{s.e.}(\hat{p})$, and is the standard deviation of all possible values of the statistic $\hat{p}$.
...
From Fig.\ \@ref(fig:RollsSixesSD), this is practically impossible (almost never occurs in any possible sample) *if the die was fair*.
...
The observed sample proportion is more than six standard deviations from the mean, which is *highly unusual* according to the $68$--$95$--$99.7$ rule.


## Determining $P$-values {#OnePropTestP}

The value of the $z$-score shows that the observed value of $\hat{p}$ is highly unusual... but how unusual?
...
Similarly, suppose the $t$-score is a bit *smaller* than $z = 2$, say $z = 1.8$.
...
```{r OnePropTestP2, fig.cap="The two-tailed $P$-value is the combined area in the two tails of the distribution. Left panel: when $z = 1.2$ (or $z = -1.2$). Right panel: when $z = 1.8$ (or $z = -1.8$).", fig.align="center", fig.width=10, fig.height=2.75, out.width='90%'}
par( mfrow = c(1, 2))
...
out <- plotNormal(mu = 0,
           sd = 1,
           main = expression( atop(The~two*"-"*tailed~italic(P)*"-value"~when~italic(z)==1.8*":",
...
But now I need to decide how to distinguish the two dice...
...
is not unusual... but perhaps the due is loaded in some other way (i.e., to produce more-than-expected rolls of a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{1}).'
} else {
   '<span class="larger-die">&#9856;</span>).'
...
Here, the $P$-value is (much) less than $0.001$, so we that there is very strong evidence exists that $p$ is *not* $1/6$.
...
All hypothesis tests have underlying conditions to be met so that the results are statistically valid; that is, $P$-values are appropriate because the sampling distribution is an approximate normal distribution.
...
To test if the population proportion of interactions could be equally shared, the hypotheses are:  
\[
   \text{$H_0$: } p = 0.5\quad\text{and}\quad\text{$H_1$: } p \ne 0.5 \text{ (two-tailed)}.
\]
The test is statistically valid, since $n\times p = 45\times 0.5 = 22.5$ and $n\times (1 - p) = 22.5$; both exceed five (i.e., expected half of the $50$ rolls to be won by each species).
...
`r if( knitr::is_html_output() ) {torf(answer=FALSE)}`
1. What is the value of $z$-score?
`r if( knitr::is_html_output() ) {
...
@loyeung2018experimental studied if subjects could identify potential placebos at *better* rate than just guessing.
...
The aim of the study was to test if the proportion of Korean children with the measles antibody in the *population* was $10$% or better (lower).
...
In the 2019/2020 English Premier League (EPL), the home team won $91$ out of $208$ games, while the away team won $67$; $50$ games were draws.
...
@koenen1995analysis found that, in 1995, $88$ of the $357$ visitors to Las Vegas casinos were smokers.
...
@siegfried2014estimating studied resistance of some commercial corn varieties to the European corn borer. Borers were collected from corn in Iowa and Nebraska.
