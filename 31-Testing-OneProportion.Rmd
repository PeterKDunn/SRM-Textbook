...
```{r, child = if (knitr::is_html_output()) {'./introductions/31-Testing-OneProportion-HTML.Rmd'} else {'./introductions/31-Testing-OneProportion-LaTeX.Rmd'}}
```
...
\begin{wrapfigure}[8]{R}{.25\textwidth}
  ...
  \includegraphics[width=.23\textwidth]{OtherImages/SmiffyDice-OneLoaded.png}
\end{wrapfigure}
`r if (knitr::is_html_output()) '-->'`


In a toy store one day (for my children...), I saw 'loaded dice'.
The packaging claimed 'One loaded \& one normal'.
I bought two packs!
However, there was no indication as to *which* die was loaded.
How could I determine which was 'loaded'?
...
actually appeared.
Using the decision-making process (Sect.\ \@ref(DecisionMaking)), I could decide if that die was the fair die.
...
Even if the die was fair, the proportion of rolls that showed a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
}`
would not necessarily be *exactly* one-sixth, due to *sampling variation*.
Sometimes the value of $\hat{p}$ would be a bit smaller than $1/6$, and sometimes a bit larger, even if $p$ really was $1/6$.

By initially assuming the population proportion of twos is $1/6$, the possible values of the *sample* proportion from all possible rolls of the fair die could be determined; that is, the *sampling distribution* could be described.
...
If the sample proportion of rolls that are
`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
}`
is not *exactly* $1/6$, two explanations exist:

* The *population* proportion *really is* $1/6$, and $\hat{p}$ is not exactly $1/6$ due to sampling variation; or
* The *population* proportion *really is not* $1/6$; that is, $\hat{p}$ is not exactly $1/6$ because the die is not fair.

These two possible explanations are called *statistical hypotheses*.
If $p$ is defined as the population proportion of twos, then the hypotheses above are:
...
When the population proportion of rolls that are a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
}`
really is $p = 1/6$, what values of the *sample* proportion are reasonable to expect, given sampling variation?
The answer depends on the sample size.
In *one* roll of a die, rolling a 
`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
...
In Chap.\ \@ref(CIOneProportion), the sampling distribution of a sample proportion was given when $p$ is a known value (Sect.\ \@ref(SamplingDistributionKnownp)).
If I decide to use $n = 100$ die rolls, the sampling distribution can be described as:
...
This is how the values of $\hat{p}$ would vary if $p$ really was $1/6$, and if certain conditions are met (Sect.\ \@ref(ValidityProportionsTest)).
...
* The standard error for a *confidence interval* uses the *sample proportion* $\hat{p}$ (see Eq.\ \@ref(eq:stderrorphat)), since we only have sample information when forming a confidence interval.
...
out <- plotNormal(mu = mu, 
                  ...
                  main = "Sampling distribution of the sample proportion of twos\nin 100 rolls ")
...
text(y =  0.75 * max(out$y),
     ...
     cex = 0.8,
     pos = 3,
     labels = expression( atop(41~twos~"in"~100~rolls,
...
## Computing the value of the test statistic: $z$-scores {#OnePropTestStatistic}
...
The observed sample proportion is more than six standard deviations from the mean, which is *highly unusual* according to the $68$--$95$--$99.7$ rule (or Tables).
...
* *If* the calculated $z$-score was $z = 2$, the two-tailed $P$-value would be the shaded area shown in Fig.\ \@ref(fig:OnePropTestP) (right panel): about $5$%, based on the $68$--$95$--$99.7$ rule.
...
```{r, OnePropTestP, fig.cap="The two-tailed P-value is the combined area in the two tails of the distribution; left panel: if $z = 1$ (or $z = -1$). Right panel: if $z = 2$ (or $z = -2$). (The one-tailed $P$-values are half the two-tailed $P$-values: $P = 0.16$ and $P = 0.025$.)", fig.width=10, fig.height=2.75, out.width='90%', fig.align="center"}
par(mfrow = c(1, 2), 
    mar = c(4, 1, 4, 1) + 0.1)
...
```{r OnePropTestP2, fig.cap="The two-tailed P-value is the combined area in the two tails of the distribution; left panel: when $z = 1.2$ (or $z = -1.2$). Right panel: when $z = 1.9$ (or $z = -1.9$).", fig.align="center", fig.width=10, fig.height=2.75, out.width='90%'}
par( mfrow = c(1, 2))
...
$P$-values are never exactly zero, so we write $P < 0.001$ (that is, the $P$-value is *less than* $0.0001$).
...
A $P$-value smaller than $1$% (or $0.01$) is usually considered 'small', and a $P$-value larger than $10$% (or $0.10$) is usually considered 'big'.
Between the values of $1$% and $10$% is often a 'grey area'.
Commonly, however, a $P$-value less than $5$% (or $0.05$) is often considered 'small'.
...
The die may be fair but, due to sampling variation, the sample we observed simply produced an unusually high proportion of 
`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
...
This means that the sample result was not unusual if $p = 1/6$.
...
is not unusual... but perhaps the due is loaded in some other way (i.e., to produce more-than-expected numbers of `r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
   '<span class="larger-die">&#9857;</span>'
}`
...
*Be careful interpreting the results!*
*A large $P$-value does not necessarily mean that the die produces is fair!*
...
Hence, the result is interpreted as 'there is no evidence that the die is fair'.
...
All hypothesis tests have underlying conditions to be met so that the results are statistically valid; that is, $P$-values can be found accurately because the sampling distribution is an approximate normal distribution.
...
\]
The test is statistically valid, since $n\times p = 45\times 0.5 = 22.5$ and $n\times (1 - p) = 22.5$; both exceed five.
...
An approximate *$P$-value* can be estimated using the $68$--$95$--$99.7$ rule, or using tables.
...
In the USA general population, the percentage obese or overweight is $65$%.
...
A study of the measles-rubella vaccination in Korea [@kim2004sero] wished to compare the proportion of children with measles antibodies to the World Health Organization (WHO) target proportion (for children aged $5$ to $9$ years old: $10$%).
...
In a study to increase activity in library users [@maeda2013introducing], pedal machine were introduced on the first floor of Joyner Library for use by students at East Carolina University, where $60.2$% of all students were females.
