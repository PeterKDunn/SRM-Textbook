...
In my $100$ rolls of one die, $41$ showed a `r include_graphics("Dice/die1.png", dpi=1500)`, a sample proportion of $\hat{p} = 41/100 = 0.41$.
...
The die may be fair but, due to sampling variation, the sample we observed simply produced an unusually high proportion of `r include_graphics("Dice/die1.png", dpi=1500)` rolls.
...
```{r PvaluesAnimation, animation.hook="gifski", interval=0.20, progress=TRUE, fig.height = 2.75, fig.align="center", dev=if (is_latex_output()){"pdf"}else{"png"}}
...
In $100$ rolls of the *other* die, I found a `r include_graphics("Dice/die1.png", dpi=1500)` on $15$ rolls, so that $\hat{p} = 0.15$.
...
It only means that the proportions of rolls that produce a `r include_graphics("Dice/die1.png", dpi=1500)` is not unusual... but perhaps the due is loaded in some other way (i.e., to produce more-than-expected numbers of `r include_graphics("Dice/die5.png", dpi=1500)` rolls).


*Be careful interpreting the results!*
*A large $P$-value does not necessarily mean that the die produces is fair!*
The die may indeed be loaded to produce a larger-than-expected numbers of `r include_graphics("Dice/die1.png", dpi=1500)` rolls... but, due to sampling variation, the sample we observed simply did not provide evidence to make that conclusion.
