...
That is, different individuals are in the comparison groups. 
This is what is meant by "between individuals".
...
Consider a study to compare the average blood pressure (the Outcome) in Australians (the Population), to see if the average blood pressure in the right arm is the same as the average blood pressure in the left arm.

**There is no comparison between individuals**: the individuals are not divided into separated groups and so that  average blood pressure is compared; every person is treated the same way.
This is a **"within individuals" comparison**.

The blood pressure is measured twice on *every* member of the population.
The outcome might be best described as 'the mean *difference* between right- and left-arm blood pressure'.
...
The research team needs to interact with the participants and use a machine to measure blood pressure, but there is *no* intervention.
Using the sphygmomanometer is just a way to measure blood pressure, to *obtain* the data.
The sphygmomanometer is not used *with the intent of changing the outcome*.
...
Sometimes, the RQ does not make it clear if an intervention is present or not.
If you are writing an interventional RQ, you should try to make it clear when an intervention is used.
...
However, different *types* of RQ emerge depending on whether the RQ also has a comparison/connection (C) or intervention (I).
...
In this RQ, the *Population* is 'Canadian males between 18 and 35 years of age', and the *Outcome* is '*Average* heart rate'.
Notice that the Outcome is a numerical summary of the Outcome across the population (the *average* heart rate).
...
Usually, *relationships* are more interesting than just descriptions; relational RQs explore existing relationships.
...
In this RQ, the *Population* is 'Australians between 18 and 35 years of age', the *Outcome* is 'average heart rate', and the (between individuals) *Comparison* is 'between females and males'.
...
Age (the C) is *not* allocated by the researchers, so there is *no intervention*.
:::
...
In this RQ, identify the **P**opulation, the **O**utcome, and the **C**onnection.
:::
...
Consider a study of blood pressure in Australians (the Population), comparing right- and left-arm blood pressures.

This is a *descriptive* RQ.
There is *no* (between subjects) comparison, since there are not two subsets of the population being compared.
...
*Interventional RQs* explore relationships where the comparison/connection is determined or allocated by the researchers.
They identify:
...
Interventional RQs may look like relational RQs, except that the comparison or connection is imposed by the researchers.

Sometimes it is not clear if the comparison or connection has been imposed by the researchers in an interventional RQ.
When writing interventional RQs, make efforts to make it clear, if possible, when the RQ is interventional.
...
In this RQ, the *Population* is 'Australians between 18 and 35 years of age', the *Outcome* is 'average heart rate', and the *Comparison* is between two groups: 'between those taking the new pill, and those taking the existing pill'.
...
In this RQ, the Population is 'children with acute atitis media', the Outcome is 'average duration of symptoms', and the Connection is between two groups. depending on the type of drug (comparing 'cefuroxime' and 'amoxicillin').
...
It is very probable that there is an intervention.
:::
...
*Interventional RQs* (when possible to answer) are the most interesting: they can be used to test theories or models, or to establish cause-and-effect relationships (Table \@ref(tab:ThreeTypesOfRQs)).
...
* **Descriptive**: What proportion of Indonesian adults are coeliacs [@data:Cook2000:Coeliac]?
* **Relational**: Among Indonesian adults, is the proportion of females who are coeliacs the same as the proportion of males who are coeliacs [@data:Cook2000:Coeliac]?
* **Interventional**:  Among Indonesian adult coeliacs, is the percentage with adverse symptoms the same for those given a diet *without* oats and those given a diet *with* oats [@data:Janatuinen2002:Coeliac; @data:Lundin2003:Coeliac]?
...
`r webexercises::hide()`
1. **Relational**: A comparison exists (comparing amputees with transradial and transhumeral amputations), but this would not be **manipulated** by the researchers, or **imposed** upon subjects by the researchers.
1. **Interventional**:
...
* For [**estimation** (*confidence intervals*)](#TwoApproachesCI):
  These RQs are concerned with, for example, estimating a value in a population.
...
* For [**making decisions** (*hypothesis testing*)](#TwoApproachesHT):
  These RQs are concerned with making a decision about an unknown population value: for example, is the percentage the same in two different groups of the population?
...
Sometimes, the RQ concerns how precisely a *value* in the population is estimated by the sample.
...
Based on the sample, the hypothesis best supported by the data is to be chosen.
...
In practice the answer is rarely clear cut, and instead *how much* evidence there is in the sample to support a particular hypothesis about the *population* is reported.
...
Ideally, a well-written RQ [@anastasiadis2015framing] should be:
...
* **N**ovel: 
  The RQ should be original (the RQ should 'seek  to  confirm,  refute  or  extend  previous  findings, and potentially reveal new findings' (@anastasiadis2015framing, p. 410). 
...
Given the nature of a course, and the short timelines, these RQs don't necessarily *need* to be Interesting, Novel or Relevant.
...
This is not a criticism: the operational definitions can be provided elsewhere.
...
RQs emerge from observations, which leads to asking questions, and the need for evidence to answer that question [@tully2014articulating].
...
For example, suppose you notice that many people take echinacea when they get a cold; it is reasonable to ask if there is evidence that echinacea helps with a cold in any way.
...
RQs often start as a basic idea, which can be refined by clarifying the POCI elements.
...
We could decide to **compare** an outcome across different groups, or **connect** it to something else.
...
Furthermore, we could decide to **intervene** or not.
Whether we decide to include an intervention or not has implications for *how* the study is conducted and how the results are interpreted.
...
The outcome is a numerical summary across subsets of the *population*, not of *individuals*.
