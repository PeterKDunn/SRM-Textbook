...
Depending on the purpose of the study, descriptive RQs have either of these forms:
...
Answering *estimation* descriptive RQs is studied in Chaps.\ \@ref(CIOneProportion) and\ \@ref(CIOneMean)).
...
POINT xref ComparisonBetweenInd to xref RQsRelational, or to the DEF
...
AAIN, redirect xref ComparisonWithinInd
...
Repeated-measures RQs with only two comparisons are often called *paired* studies.\index{Data!paired}
...
In some studies, the comparison naturally occurs (e.g., age of people), and in some studies the comparison is manipulated by researchers (and called an *intervention*).
...
In other studies, the comparison can be *manipulated* by the researchers, or *imposed* on the individuals by the researchers, and are called *treatments*
...
The researchers treated each neuron as an independent observation, giving a sample size of $n = 160$.
...
* Studying the relationship between the ???????? PLANT and the number of hours of sunlight per day.
...
One correlational RQ they studied was relationship between the size of trees (using DBH, diameter at breast height; the outcome), and the pH of the soil (the connection) was explored.
...
A RQ exploring the relationship between the daily water intake and the daily sun exposure  for many individuals would be a correlational RQ.

A RQ studying the daily sun exposure for many individuals over two days has a *within*-individuals comparison, so is a repeated-measures RQ.
:::
