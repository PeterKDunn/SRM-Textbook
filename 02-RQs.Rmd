...
In research, asking clear and answerable **research questions** (RQs) is important.
In addition, the data (evidence) that must be collected depends on the RQ.
...
* A *conceptual definition* explains precisely *what* is being measured, observed or assessed (i.e., what a word or a term *means* in the study).
...
However, **most variables will require an operational definition** to ensure consistent data collection, by removing any ambiguity about how a variables is measured.
...
However, sometimes commonly-accepted definitions exist, so should be used unless there is a good reason to use a different definition.
...
An *operational definition* is needed (which the student did not provide...).
:::
...
::: {.exampleExtra data-latex=""}
A study of snacking in Australia [@data:Fayet2017:Snacks] used this conceptual definition of an 'eating occasion' to explain what the term *means*:
...
An *operational* definition would explain *how* the data were obtained from the participants (i.e., using a food diary).
:::
...
A RQ must be *written* carefully so it can be properly *answered*.
In this section, the four potential components of a RQ are studied:
...
Indeed, *all* elements of the population may not be accessible in practice.
...
A *sample* is a subset of the population of interest which is actually studied, and from which data are collected.
:::
...
Both exclusion and inclusion criteria are not necessarily *needed*; none, one or both may be used.
...
A study of a certain bird species may only include sites where there has been a confirmed sighting within the last two years.
...
* **I**nteresting: 
  ...
  For example, no-one cares about comparing the percentage of people who prefer drinking tea in blue cups rather than green cups... 
* **N**ovel: 
...
RQs often start as basic ideas, which are refined by clarifying the POCI elements.
...
* *Percentage* of people who take days off work or school.
...
No relationship could be found if we gathered information from just two teenagers.
...
Is there a difference between the cold durations *in the population* (not just between those two teenagers)?
...
Each piece of information that we gather from or about the individuals is called a *variable*, because its values can *vary* from individual to individual.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
A variable is a single aspect of characteristic that can vary from *individual to individual*.

*Your* city of birth may not change, but 'city of birth' is a variable because it can vary from *individual* to *individual*.
...
The '*average* duration of cold symptoms' is the *outcome*, numerically summarising many individuals' cold durations.
:::
...
* The **explanatory variable** measures, assesses, describes or records information to determine the comparison or connection (Table \@ref(tab:RQsPopulationIndividuals)).
