...
Clear and answerable *research questions* (RQs) are important, as the RQ impacts all other components of the research.
Quantitative research summarises and analyses data using numerical methods (like averages or percentages), so the RQ must be *written* carefully so they can be *answered* effectively.
In this book, four different types of RQs are studied:
...
*All RQs* identify a large group of interest to be studied (a *population*; Sect.\ \@ref(Population)),\index{Population} and study something *about* that population (the *outcome*; Sect.\ \@ref(Outcome)).\index{Outcome}
...
A *population* is a group of *individuals* from which the total set of observations of interest *could* be made, and to which the results will (hopefully) generalise.
:::


Populations comprise many *individuals*, sometimes called *cases*.\index{Individuals}\index{Cases}
If the individuals are people, individuals are also called *subjects*.\index{Subjects}
...
The data are rarely taken from all the individuals in the *population*: *all* individuals are rarely accessible in practice.
...
The *sample* comprised $28$ active and $28$ sedentary American college women, from which data are collected.
...
Exclusion and inclusion criteria clarify which individuals are explicitly included or excluded from the population.
Exclusion and inclusion criteria should be explained when their purpose is not obvious.
Exclusion and inclusion criteria are not *necessary*; none, one or both may be used.
...
Because the RQ concerns a large group (the population), the outcome summarises a group of individuals.
Hence, the outcome is a numerical quantity (such as an *average* or *percentage*) that can summarise a group of individuals.
...
We can now introduce a descriptive RQ.


::: {.definition #DescriptiveRQ  name="Descriptive RQ"}
Descriptive RQs have population and an outcome.
:::


Descriptive RQs can have one of these forms:

* *Estimation* RQs: Among {*the population*}, what is {*the outcome*}?\index{Research questions!Estimation}
* *Decision-making* RQs: Among {*the population*}, is {*the outcome*} equal to {*a given value*}?\index{Research questions!Decision making}
...
@data:mackowiak:bodytemp  examined the 'body temperature of $148$ healthy men and women' aged from $18$ to $40$; this is the *population*.
...
They also studied a *decision-making* descriptive RQ (where $98.6^\circ$F ($37.0^\circ$C) is a commonly-accepted value):
...
*Relational RQs* compare the outcome for different groups of individuals in the population.
These comparisons are called *between-individuals* comparisons,\index{Comparison!between individuals} as they compare the outcome *between* (or among) different groups of *individuals*.
<!-- The groups being compared may be either naturally different (e.g., different sub-species), or are manipulated by the researchers to be different (e.g., different doses of a drug). -->
...
The *between-individuals comparison* in a RQ identifies the small number of different groups of individuals for which the outcome is compared.
:::


:::{.example #BetweenHimalaya name="Between-individuals comparison"}
@data:Bird2008:wholegrain gave one group of participants a diet using refined flour, and gave another group of participants a diet using a new flour variety.
Faecal weight was compared for the two groups.
The type of diet is the *between-individuals* comparison.
:::


::: {.definition #RelationalRQ name="Relational RQ"}
Relational RQs have population, outcome, and a *between*-individuals comparison.
:::
...
These comparisons are called *within-individuals* comparisons,\index{Comparison!within individuals} as they compare the outcome *within the same individuals*, not across different groups of individuals.
...
* Comparing the average thickness of the cornea in eyes with glaucoma and eyes without glaucoma, in people with one glaucoma eye.
...
The *within-individuals comparison* in the RQ identifies the small number of different, distinct situations for which the outcome is compared multiple times for each individual.
:::
...
A *between*-individuals comparison would compare the left- and right-leg strengths *between different* groups of footballers: one group would have their left-leg strength measured, and the other would have their right-leg strength measured.
This study has a *between*-individuals comparison.

In contrast, both left- and right-leg strength could be recorded on the *same* individuals.
This study examines *within*-individuals changes: the differences between the left- and right-leg strength *within* individuals.
In this study, *no between-individuals comparison* exists, as different groups are not being compared.
:::
...
* *Estimation* RQ: Among {*the population*}, what is the change in {*the outcome*} for {*the alternatives being compared*}? 
* *Decision-making* RQ: Among {*the population*}, is {*there a change in the outcome*} for {*the alternatives being compared*}?


::: {.example #WithinRelationalRQ name="Within-individuals relational RQ"}
To understand tree-dwelling marsupials, a study compared the temperature in the *same* tree hollows in both summer and winter [@rowland2017comparing]:
...
Repeated-measures RQs with only two comparisons are often called *paired*.\index{Data!paired}
...
An intervention is present when *researchers* can manipulate (or impose) the *comparison* to determine the impact on the outcome. 
:::


Comparisons that are *not* manipulated by the researchers are called *conditions*.
Comparison that are *manipulated* by the researchers, or *imposed* on the individuals by the researchers, are called *treatments*

*Between*- or *within*-individuals comparisons can be an intervention.
Within any type of comparison, the *analysis* is the same whether an intervention is used or not.
However, the *interpretation* of the results depend on whether an intervention is used or not (Sect.\ \@ref(CompareStudyTypes)).
...
A study by @data:Bird2008:wholegrain *supplied* one group of participants a diet using refined flour, and *supplied* another group of participants a diet using a new flour variety.
The type of diet is the between-individuals comparison.
...
The research team interacts with the participants and uses the machine to measure blood pressure, but there is *no* intervention.
...
What is the outcome, comparison (if any), and intervention (if any)?\label{thinkBox:POCIWomen}
...
Together, the **P**opulation, **O**utcome, **C**omparison and **I**ntervention form the POCI acronym (sometimes written as the PICO acronym) to aid remembering the elements of RQs.
...
*Estimation RQs* ask how precisely a *value* in the *population* is estimated by using the *sample*, and are answered using *confidence intervals*.
Answering estimation RQs are discussed in this book in Chaps.\ \@ref(CIOneProportion) to\ \@ref(OddsRatiosCI),  and Sect.\ \@ref(RegressionCI).

*Decisions-making RQs* are concerned with making a decision about a population, and are answered using *hypothesis testing*.
Answering decision-making RQs are discussed in this book in Chaps.\ \@ref(TestOneProportion) to\ \@ref(TestsOddsRatio), plus Sects.\ \@ref(RegressionHT) and\ \@ref(CorrelationTesting).


::: { .example #TypesOfRQS name="Decision-making RQs"}
A study [@data:Thane2004:ZincVitA] of 'British young people aged $4$--$18$' asked and answered numerous RQs.
One relational RQ was:
...
For the example above: the average zinc intake either *is* the same for boys and girls, or *is not* the same for boys and girls (Fig.\ \@ref(fig:ZincRQ)).
...
For the zinc-intake study above (Fig.\ \@ref(fig:ZincRQ)), the RQ could ask if:
...
The first two are *two-tailed RQs* (and are essentially asking the same thing), since they ask about a difference either way: the average zinc intake could be higher for girls or higher for boys; we are just interested in whether any difference is present.
The last two are *one-tailed RQ*, since they ask specifically about a difference in just one direction: girls greater than boys, or boys greater than girls.
...
One of the last two would only be adopted if there was a specific reason why a one-tailed direction was suspected before the data were collected.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
In general, RQs should be written as two-tailed RQs, unless a good (and justifiable) reason exists for asking a one-tailed question before the data are collected.
:::



## Writing RQs: an example {#Writing-RQs}
\index{Research questions!writing}
...
The outcome could be *compared* between groups (such as those taking echinacea and those who do not).
A within-individuals comparison seems unsuitable for this RQ.
...
If the study *did have an intervention*, various interventions would be imposed by the researchers; the dose *frequency* or the doses *amounts* could be imposed, for instance.
...
Based on the above, consider this RQ (based on @data:barrett:echinacea):

> Among Australian teenagers with a common cold, is the average duration of cold symptoms shorter for teens given a daily dose of echinacea compared to teens taking no echincea?

This is an one-tailed relational RQ.
...
While *your* city of birth may not change, 'city of birth' is a variable because it varies from *individual* to *individual*.
:::
...
* The *response variable*, which records information to determine the outcome.\index{Variables!response}
* The *explanatory variable*, which records information to determine the comparison.\index{Variables!explanatory}
...
The RQ cannot be answered without information about the response and explanatory variables.
...
Similarly, the values of the explanatory variable measured on the individuals distinguish between the values of the *comparison* (Table\ \@ref(tab:RQsPopulationIndividualsExamplesComparison) and\  \@ref(tab:RQsPopulationIndividualsExamplesComparisonWithin)) being made.



```{r POCIVariables, fig.cap="The POCI elements, and two important variables", fig.align="center", fig.height=2, fig.width=6, out.width='55%'}
source("R/showPOCI.R")
...
PopInd3[2, ] <- c("Between $3$\\ kg/ha, $4$\\ kg/ha fertilizer rates", 
                  "", 
...
if( knitr::is_latex_output() ) {
  kable(PopInd3,
        ...
        escape = FALSE, # For latex to work in \rightarrow
        linesep  =  c( "\\addlinespace"), # Add a bit of space between all rows
        caption = "Examples of the between-individuals comparison and corresponding explanatory variable",
        align = c("r", "c", "l"))   %>%
    kable_styling(full_width = FALSE, 
                  font_size = 8) %>%
    row_spec(0, bold = TRUE) %>% # Columns headings in bold
...
if( knitr::is_html_output() ) {
  kable(PopInd3,
        ...
        align = c("r", "c", "l"),
        longtable = FALSE,
        caption = "Examples of the between-individuals comparison and the corresponding explanatory variable",
        booktabs = TRUE)    
}
```
...
PopInd3[2, ] <- c("Between left and right arms", 
                  "", 
                  "Which arm is used")
PopInd3[3, ] <- c("Between fore legs and hind legs", 
                  "", 
...
if( knitr::is_latex_output() ) {
  kable(PopInd3,
        ...
        escape = FALSE, # For latex to work in \rightarrow
        linesep  =  c( "\\addlinespace"), # Add a bit of space between all rows
        caption = "Examples of the within-individuals comparison and corresponding explanatory variable",
        align = c("r", "c", "l"))   %>%
    kable_styling(full_width = FALSE, 
                  font_size = 8) %>%
    row_spec(0, bold = TRUE) %>% # Columns headings in bold
...
if( knitr::is_html_output() ) {
  kable(PopInd3,
        ...
        align = c("r", "c", "l"),
        longtable = FALSE,
        caption = "Examples of the within-individuals comparison and the corresponding explanatory variable",
        booktabs = TRUE)    
}
```
...
The *outcome* is the *average* lead concentration in the surfaces over many playgrounds; the *response variable* is the lead concentration in the *individual playground surfaces.

The between-individuals *comparison* is between the four types of surfaces (rubber, soil, sand, mulch).
...
A different type of RQ is a correlational RQ.
*Correlational RQs* are not concerned with summarising outcomes in groups.
Instead, correlational RQs explore relationships between two variables that are measured or observed on the individuals.
Typically, correlational RQs have the form:

* *Estimation* RQ: Among {*the population*}, how strong is the relationship between {*the outcome*} and {*something else*}?
* *Decision making* RQ: Among {*the population*}, is {*the outcome*} related to {*something else*}?
...
::: {.example #CorrelationalRQ name="Correlational RQs"}
The study in Example\ \@ref(exm:RelationalRQ), about Cubans and heart rates, could also have asked:

> Among Cubans between $13$ and $20$ years of age, is heart rate related to age?

This RQ is a *decision-making*, correlational RQ, since it asks if the heart rate is related to age.
An *estimation* correlational RQ might be:

> Among Cubans between $13$ and $20$ years of age, how strong is the relationship between  heart rate and age?
:::



:::{.example #WithinBetweenConnection name="Correlational RQs"}
The Wollemi pine was discovered by science in 1994.
@offord2023home studied the growth of these rare plants.

One correlational RQ they studied was the relationship between the size of trees (using DBH, diameter at breast height), and the pH of the soil.
The two variables are the DBH and pH, both recorded for many trees. 

They also studied the relationship between the DBH for each tree at at various times after the planting date. 
Each tree has the DBH measured over time, for many time points. 
Time is the *within*-individuals comparison. 
:::


In some (but not all) situations, one variable can be considered as perhaps influencing the value of the other variable.
...
:::{.example #WithinBetweenConnections name="Correlational RQs"}
Consider studying marathon runners.
A RQ exploring the relationship between the daily water intake and the daily amount of sun exposure for individuals would be a correlational RQ.

The daily amount of sun exposure is the explanatory variable, and the daily water intake is the response variable.
It seems possible that the amount of sun exposure would influence the water intake.
:::
...
To answer this question, the thickness of individual hair strands need to measured.
The 'thing' from which measurements are taken are called *units of observation*.
...
Suppose we have $100$ strands of hair from which we can record the thickness.
These hair strands could be obtained in different ways.
For example, two possibilities are to (1)\ measure $100$ strands of hair from *one* man; or (2)\ measure one strand of hair from each of $100$ different men.
While each approach gives $100$ measurements of hair-strand thickness, these two approaches are *very* different.

In the first, only one man is represented.
In the second, $100$ men are represented.
The difference is related to the concept of *unit of analysis*.
The purpose of the study is to make conclusions about 'men'.
Since the RQ is asking about 'men', the 'man' is the unit of analysis.
In the second possibility, each *unit of analysis* provides $100$ observations.


::: {.definition #UnitOfAnalysis name="Unit of analysis"}
The *unit of analysis* is the smallest collection of units of observations (and perhaps the units of observations themselves) about which conclusions are made; the smallest *independent* elements of the population for which information is analysed.
:::


All the hair strands from the same man have essentially 'lived their life together': they are washed at the same time, with the same shampoo, exposed to the same amount of sunlight and exercise, share the same genetics, etc.
However, different men would potentially use different shampoo, exercise differently, have different genetics, and so on.
The hair on different men tends to be different, and is independent of the hairs on other men.
...
reported a study where researchers examined '$10$ neurons from each of the $16$ mice' in the study.
...
The unit of analysis is the mouse, and the unit of observation is the neuron.
...
The *units of observation* are the individual students, as the fitness measurements are taken from each student.
The *units of analysis* are also the individual students, as the students using the different programs are being compared.
In addition, the PA program was *allocated* to each student individually, and each student has their own sport, family routines and activities, etc.
...
\index{Software!data entry}\index{Software!statistical}\index{Software!jamovi}
...
```{r DataPrepjamovi, fig.show="hold", out.width=c('37%', '2%', '51%'), fig.align="center", fig.cap="jamovi prepared for the data, with some data entered, and the variable names as the column headers. Left: a between-individuals comparison. Right: a within-individuals comparison.", fig.align="center", fig.width=3, fig.height=4}
...
<!-- ## Writing RQs NEEDED??????????????????????? {#TypesOfRQs} -->
<!-- \index{Research question!types} -->

<!-- All RQs have a population (P) and an outcome (O). -->
<!-- Different *types* of RQ emerge depending on the type of comparison/connection (C), if any: -->

<!-- * *Descriptive RQs* have a population and an outcome. -->

<!-- * *Relational RQs* have a population, an outcome, and a *between-individuals* comparison (and perhaps an intervention). -->

<!-- * *Repeated measures RQs* have a population, an outcome, and a *within-individuals* comparison (and perhaps an intervention). -->

<!-- * *Correlational RQs* have a population, an outcome, and a *connection* (and perhaps an intervention). -->

<!-- All RQ types can also be written to elicit different information, depending on the purpose of the research (Sect.\ \@ref(TwoPurposesOfRQs)): -->

<!-- * *Estimation*: -->
<!--   These RQs ask about estimating an unknown *value* in the *population*, using the *sample* data. -->
<!-- * *Making decisions*: -->
...
<!-- RQs may or may not have an intervention. -->
<!-- Sometimes, the use of an intervention is unclear from the RQ; clarify if an intervention is used if possible. -->
...
1. What *type* of *comparison* is this: *between* or *within* individuals? \tightlist 
`r if( knitr::is_html_output()) {
longmcq( c(
...
longmcq( c(
   ...
   "Estimation") )}`
1. If decision-making, is this RQ one- or two-tailed?
`r if( knitr::is_html_output()) {
longmcq( c(
   answer = "Two-tailed",
   "This is an estimation RQ.",
...
1. Is the definition given for SCL (p. 2) *conceptual* or *operational*? 
...
For example, @data:Thane2004:ZincVitA studied 'British young people aged $4$--$18$' and answered numerous RQs.

a. What is the average zinc intake of the children? 
b. Does the average zinc intake meet recommended dietary guidelines? 
c. What is the strength of the association between plasma zinc and retinol concentrations?
d. Is the average zinc intake the same for boys and girls? 
...
The study also explored the relationship between the daily sodium excretion and the systolic blood pressure.
...
The size of the subjects' lymph nodes (removed through surgery) were then measured.
Each subjects' specimen (p.\ 6)
...
**1.** Between defence, attacking players.
**2.** *Between* individuals.
**3.** Average number knee injuries per season.
**4.** Number knee injuries per player, per season.
**5.** The elite netball players.
**6.** The elite netball players.
**7.** Relational.
**8.** Decision-making.
**9.** Two-tailed.
:::
