...
Asking clear and answerable **research questions** (RQs) is important for many reasons; one is that the data (evidence) that must be collected depends on the RQ.

Quantitative research summarises and analyses the data using numerical methods (such as averages or percentages), so the RQ must be appropriate for analysis using these methods.

For this reason, writing the RQ appropriately is important.
The RQ drives all other aspects of the research (Fig. \@ref(fig:RQLeadsTo)).


```{r RQLeadsTo, echo=FALSE, fig.cap="The RQ drives all aspects of the study", fig.align="center", fig.height=1.75, fig.width=5.5, out.width = '80%'}
...
pos[2, ] <- c(0.15, 0.175) # Why
pos[3, ] <- c(0.85, 0.175) # How
pos[4, ] <- c(0.50, 0.825) # What
...
This goal has many component RQs (Fig. \@ref(fig:PermPavements)), and each can be answered using separate studies.
:::

```{r PermPavements, echo=FALSE, fig.cap="A study of permeable pavements (PPs) may have many sub-questions", fig.align="center", fig.height=2, fig.width=7, out.width = '100%'}
par( mar = c(0.10, 0.15, 0.10, 0.15))
openplotmat()
...
textrect( pos[2, ], 
           ...
           rady = 0.20, 
           shadow.size = 0,
           box.col = "darkolivegreen3",
           lcol = "darkolivegreen3")
textrect( pos[3, ], 
           lab = "Do PPs improve\nrun-off quality?", 
           radx = 0.13, 
           rady = 0.20, 
...
* A *conceptual definition* explains precisely *what* is being identified, measured, observed or assessed (i.e., what a word or a term *means* in the study).
* An *operational definition* defines exactly *how* something will be identified, measured, observed or assessed.

In most cases, any data being collected will need a clear **operational definition**, to ensure repeatability and consistent data collection, by removing any ambiguity about how a variables is measured.


::: {.definition #ConceptualDefinition name="Conceptual definition"}
A *conceptual definition* articulates precisely *what* is being identified, measured, observed or assessed in a study.
:::
...
This is especially important here, since stress cannot be measured directly (like height, for example).

'Stress' could be *measured* using a survey (like the Perceived Stress Scale (PSS) [@cohen1983global]); the level of stress is the score on the ten-question PSS.
...
Sometimes the definitions themselves aren't important; a clear definition is simply needed.
However, sometimes commonly-accepted definitions exist, and should be used unless good reasons exist for using a different definition.
For example, in criminal law, an 'adult' in Australia is someone 
...
Additionally, how *shoulder flexibility* is being *measured* is not clear. 
An *operational definition* is needed (which was not provided...).
:::
<!-- Zac R., SEM1 2020 -->
...
A RQ must be *written* carefully to be properly *answered*.
In this section, the four potential components of a quantitative RQ are studied:
...
All quantitative RQs study a *population*: the larger group of interest in the study.
...
The *population* is the group of *individuals* (or *cases*; or *subjects* when the individuals are people) from which the total set of observations of interest could be made, and to which the results will (hopefully) generalise.
:::


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
To fully understand *individuals*, you should also read about [**units of analysis and units of observation**](#UnitsObsAnalysis).

The *individuals* are the *units of analysis*.
:::


The population is *any* group of interest; for example:
...
The *population* is not the individuals from which the data are actually *obtained*.
Indeed, *all* elements of the population are rarely accessible in practice.

The population represents all the 'individuals' to which the results are to be generalised.
For example, when testing a new drug, the aim is to see if it works on people in general, including people not yet born. 
The population is 'all people', **not** just those studied.


::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The **population** in a RQ is *not* just those we study; it is the whole group to which our results would generalise.
:::


In contrast, a *sample* is a *subset* of the population that we actually end up studying, from which data are obtained.
...
A *sample* is a subset of the population which is actually studied, and from which data are collected.
:::
...
Completely and precisely defining the population [@banerjee2010statistics] sometimes requires *refining* or *clarifying* the population, using *exclusion* and/or *inclusion criteria*.
...
A study on the influenza vaccine [@kheok2008efficacy] listed the **P**opulation as 'health-care workers' [@kheok2008efficacy, p. 466], and the sample they studied was:
...
All RQs study something *about* the population, which is called the *outcome*.

Because the RQ concerns a population, the outcome describes a population (not individuals).
Hence, the outcome is usually an *average*, *percentage*, or *general* numerically quantity summarising the population (or subsets of the population).
...
The *outcome* in a RQ is the result, output, consequence or effect of interest in a study, numerically summarising the population (or subsets of the population).   
:::
...
That is, the comparison is between different groups of individuals: a **between-individuals comparison**.
...
The distinction between *within*-individuals comparisons and *between*-individuals comparisons (as here)  is important.
...
In contrast, *within-individual comparisons* make comparison within the individuals.
:::
...
A *between*-individuals comparison would compare different groups: one group would have their left leg strength measured, and the other have their right-leg strength measured.

In contrast, a *within*-individuals comparison would measure both the right and left strength on the same individuals: the comparison is within each individual.
:::


The outcome may be *compared* between two or more separate subsets of the population; for example:

* Comparing the average amount of wear in floor boards (O) between two groups in the population: standard wooden flooring materials and bamboo flooring.
* Comparing the average heart rates (O) across three subsets of the population: those who received no dose of a drug, those who received a daily dose of the drug, and those who received a dose of the drug twice daily.
...
This definition requires a *between-individuals comparison*, that may be *imposed* (for example, one group is given one dose of fertilizer per day, and another given two doses of fertilizer per day) or *existing* (for example, one group of people aged under 30, and another aged 30 or over).

If all individuals are treated in the same way, or do not have existing difference that allow them to be divided into groups to be compared, there is no *comparison* according to this definition.
:::
...
**There is no comparison between individuals**: the individuals are not divided into separated groups to compare average blood pressure; every person is treated the same way.
...
A study comparing the average blood pressure between (a) people aged under 40, and (b) people aged 40 or over *does* have a comparison: two subsets (under 40; 40 and over) of the population (Australians) are compared.
...
That is, the researchers have *imposed* the intervention upon those in the study to see the change in the outcome.
:::
...
A study comparing the average blood pressure (O) in female and male (C) Australians (P) measured blood pressure using a blood pressure machine (a sphygmomanometer).
...
**There is no intervention**, since the *comparison* is between females and males, which cannot be manipulated or imposed on the individuals by the researchers.
:::
...
Different *types* of RQ emerge depending on whether the RQ has a comparison/connection (C) or intervention (I).
...
> Among Canadian males between 18 and 35 years of age, what is the average heart rate?

The *Population* is 'Canadian males between 18 and 35 years of age', and the *Outcome* is '*Average* heart rate'.
Notice that the Outcome describes a numerical summary in the population (the *average* heart rate).

This is a *descriptive RQ*: there is no connection with, or comparison between, the average heart rate and anything else.
:::
...
Usually, *relationships* are more interesting than just descriptions.
*Relational RQs* explore existing relationships, and identify:

* The **P**opulation.
* The **O**utcome.
* The **C**omparison or **C**onnection.
		
Relational RQs have no intervention; the connection or comparison is manipulated by, nor imposed by, the researchers.

Typically, relational RQs with a *comparison* look like this:
...
The *Population* is 'Australians between 18 and 35 years of age', the *Outcome* is 'average heart rate', and the (between individuals) *Comparison* is 'between females and males'.

This is a *relational RQ* with a *comparison*.
Notice that the average heart rate (Outcome) is numerically summarised the two groups being compared (females; males).

The sex of the individual (the C) is *not* manipulated by, or imposed by, the researchers, so there is *no intervention*.
:::
...
The *Population* is 'Australians between 18 and 35 years of age', the *Outcome* is 'average heart rate', and the *Connection* is with 'age'.

This is a *relational RQ* with a *connection*.
:::


::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider this RQ (based on @data:Brown2000:WarningLights):
...
(A [noisy miner](https://en.wikipedia.org/wiki/Noisy_miner) is a type of bird.)
Identify the **P**opulation, **O**utcome, and **C**onnection.
:::
...
Consider a study of blood pressure in Australians (the Population), by measuring right- and left-arm blood pressures of each individual.

This is a *descriptive* RQ.
There is *no* (between subjects) comparison, since subsets of the population are not being compared.

The blood pressure is measured twice on each member of the population: every member of the population is treated in the same way.
The outcome is 'the average *difference* between right- and left-arm blood pressure'.
This is a *descriptive* RQ (and has a within-individuals comparison).

In contrast, a study comparing the average blood pressure between females and males is a *relational* RQ.
There *is* a (between subjects) comparison: the two subsets of the population (Australians) being compared are females and males.
:::
...
*Interventional RQs* explore relationships where the comparison/connection is manipilated by, or imposed by, the researchers.
They identify:
...
Interventional RQs are like relational RQs, except that the comparison or connection is manipulated by the researchers.

Sometimes, the RQ is not clear whether an intervention is used or not.
When writing an interventional RQ, try to make it clear when an intervention is used.
...
The *Population* is 'Australians between 18 and 35 years of age', the *Outcome* is 'average heart rate', and the *Comparison* is between two groups: 'between those taking the new pill, and those taking the existing pill'.

There is an *Intervention*: the researchers *allocate* one of the pills to each subject.
...
The Population is 'children with acute atitis media', the Outcome is 'average duration of symptoms', and the Connection is between two groups (taking 'cefuroxime' or 'amoxicillin').
...
### Comparing the RQs {#RQsCompare}

*Descriptive RQs* are usually used when a research topic is in its infancy; descriptive RQs set the platform for asking relational questions.
...
*Interventional RQs* (when possible) are the most interesting: they can be used to test theories or models, or to establish cause-and-effect relationships (Table \@ref(tab:ThreeTypesOfRQs)).
...
Research often develops through these stages of RQs as knowledge grows and develops.
For example:

* **Descriptive**: What proportion of adults are coeliacs [@data:Cook2000:Coeliac]?
* **Relational**: Among adults, is the proportion of females who are coeliacs the same as the proportion of males who are coeliacs [@data:Cook2000:Coeliac]?
* **Interventional**:  Among adult coeliacs, is the percentage with adverse symptoms the same for those given a diet *without* oats and those given a diet *with* oats [@data:Janatuinen2002:Coeliac; @data:Lundin2003:Coeliac]?
...
RQs can be asked in one of two ways:

* For [**estimation** (*confidence intervals*)](#TwoApproachesCI):
  These RQs are concerned with estimating a value in a population.
  This value may be the *size* of a difference (probably a RQ with a Comparison), or *strength* of a relationship (probably a RQ with a Connection).
* For [**making decisions** (*hypothesis testing*)](#TwoApproachesHT):
  These RQs are concerned with making a decision about a population: for example, is the percentage the same in two different groups of the population?


`r if (knitr::is_latex_output()) '<!--'`
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
<iframe src='https://www.ferendum.com/en/embeded.php?pregunta_ID=1249718&sec_digit=1903872648&embeded_digit=1192051453' style='width:100%; height:420px; overflow: auto; background: #badaff33' frameBorder='0'></iframe><BR>
<A href='https://www.ferendum.com' target='_blank'>Free Online Poll Maker</A>

<iframe src='https://www.ferendum.com/en/embeded.php?pregunta_ID=1249722&sec_digit=1963858267&embeded_digit=1156913995' style='width:100%; height:420px; overflow: auto; background: #badaff33' frameBorder='0'></iframe><BR>
<A href='https://www.ferendum.com' target='_blank'>Free Online Poll Maker</A>

`r webexercises::hide()`
1. **Estimation**: The RQ is about estimation *what* the difference is.
1. **Decision-making**: The RQ needs an answer: Is there a relationship, or not?
`r webexercises::unhide()`
:::
`r if (knitr::is_latex_output()) '-->'`
...
Some RQs ask how precisely a *value* in the population is estimated by the sample.
This value may measure a *difference*, or the *strength* of a relationship.
These RQs are studied in Chapters \@ref(CIIntro) to \@ref(OddsRatiosCI), and in Sect. \@ref(RegressionCI).


::: {.example #EstimationRQ name="Estimation RQs"}
Consider this RQ (based on @data:barrett:echinacea):

> Among New Zealander teens with a common cold, *how much shorter* are cold symptoms, on *average*, for teens taking a daily dose of echinacea compared to teens taking no medication?

This RQ asks about size of the *difference* in the average duration of cold symptoms for two groups of teens.
:::
...
Some RQs ask if a difference or a relationship exists in the *population*.

These RQs often are associated with *hypotheses*: *statements* that suggest possible answers to the RQ.
Based on the sample, the hypothesis best supported by the data is to chosen.

These RQs are studied in Chapters \@ref(HTIntro) to \@ref(TestsOddsRatio), and in Sect. \@ref(RegressionHT).


::: {.example #DecisionsWithSamples name="Making decisions with samples"}
Consider this RQ (based on @data:barrett:echinacea):

> Among New Zealander teens with a common cold, is the average duration of cold symptoms shorter for teens taking a daily dose of echinacea compared to teens taking no medication?

This is a decision-type RQ, with two possible answers (Fig. \@ref(fig:EchinaceaRQ)): Either echinacea *does* result in shorter average cold durations, or it *doesn't*.
In practice, answers are rarely clear cut.
...
Ideally, a well-written RQ [@anastasiadis2015framing] is:

* **F**easible: 
  Answering the RQ should be possible practically; sufficient personnel, time, resources, and money should be available to complete the study properly.
* **I**nteresting: 
  The RQ should be interesting. 
  For example, no-one cares about comparing the percentage of people who prefer drinking tea in blue cups to green cups... 
* **N**ovel: 
  The RQ should be original, so as to [not waste resources, time or money](#Common-Ethical-Issues). 
* **E**thical: 
  The RQ must be able to be answered ethically (Chap. \@ref(Ethics)).
...
In undergraduate university courses, a Project RQ **must** be feasible and ethical.

However, the short timelines and knowledge of teh stduents mean that these RQs don't necessarily *need* to be Interesting, Novel or Relevant.
:::
...
* General poor writing: 
  A round bracket is started but never closed, and the bracket is followed by a comma.
...
* The RQ does not have a clear Outcome that numerically summarises the population, such as a proportion or a mean.
* The comparison is not clear: between morning and afternoons: Or between English and Australian chocolates? Or both?

This is a better RQ:
  
> For UniSC students at the Sippy Downs campus, is the percentage of people who can correctly identify English or Australians chocolates the same in the mornings as in the afternoons?

Written this way:
  
* P: UniSC students at Sippy Downs campus.
* O: The *percentage* correctly identifying English or Australian chocolates.
* C: Between mornings and afternoons.
* I: No intervention.
	
We cannot decide if a particular time of day (teh comparison) is morning or afternoon.

This RQ is **F**easible and **E**thical, but probably not really **I**nteresting (except to the project group), **N**ovel or **R**elevant... but that's OK for a student project.
...
RQs emerge from observations, which lead to questions, and the need for evidence to answer that question [@tully2014articulating].
...
For example, suppose you notice that many people take echinacea when they get a cold.
You may wonder: is there evidence that echinacea helps with a cold in any way?
This may lead to an initial RQ (based on @data:barrett:echinacea):

> Is it better to take echinacea when you have a cold?

*This RQ is clearly poor, but serves as a starting point.*

This RQs can be refined by clarifying the POCI elements.
...
The initial RQ cannot be answered because 'better' is ambiguous: better than what?
We could **compare** an outcome across different groups, or **connect** the outcome to something else.
For example, the *comparison* could be:
...
We could also decide whether to **intervene** or not, which has implications for *how* the study is conducted and how the results are interpreted.

If we decided *not* to intervene, the subjects in the study would decide for themselves how to treat their cold.
If we did decide to intervene, various interventions could be used:
...
This RQ is much better, but it is *still not correct*.
The outcome should be a numerical summary of groups of individuals.
So consider this revised RQ (based on @data:barrett:echinacea):
...
The Outcome describes the population in general, and so Outcomes are often worded in terms of averages or percentages or similar.
For example, consider this RQ seen above:
...
Consider this: suppose a cold lasts for 6 days for a teenager who *does* take echinacea, and a cold lasts for 5 days for a teenager who *does not* take echinacea.
Is there a difference between the cold durations *in the population*?
We have no way of knowing: Only two teenagers were studied.
To explore the relationship using teenagers in general, data from *many* teenagers is needed.

RQs concern numerical summaries about *populations*, but the data to answer the RQ come from *individuals* in the population.
(As with the word 'population', the word 'individual' does not only refer to people.)
...
is a single aspect or characteristic associated with each of a group of individuals under consideration, whose values can vary from individual to individual.
:::
...
*Your* city of birth may not change, but 'city of birth' is still a variable because it can vary from *individual* to *individual*.
...
'Duration of cold symptoms' is a variable, as it is obtained from individuals, and its value can vary from individual to individual.
...
* The **response variable**\index{response variable}
  measures, assesses, describes or records information to determine the outcome; and
* The **explanatory variable**\index{explanatory variable}
  measures, assesses, describes or records information to determine the comparison or connection (Table \@ref(tab:RQsPopulationIndividuals)).
...
is the variable used to measure, assess or describe the outcome on each individual in the population.
:::


The *outcome* refers to the numerical summary of the values of the response variable (Table \@ref(tab:RQsPopulationIndividualsExamplesOutcome)).
...
is a variable of interest from the individuals in the study which (potentially) causes changes in, or is related to, the response variable.
:::

The explanatory variable is a formal description of what *C* measures, observes, assesses or describes in each individual member of the population (Table \@ref(tab:RQsPopulationIndividualsExamplesComparison)).
...
PopInd3[2, ] <- c("Between beech, tallowwood, and jarrah floor boards", 
		  "", 
		  "Type of floorboard in each \\emph{individual} home")
PopInd3[3, ] <- c("Between 350kg/ha and 400kg/ha fertilizer rates", 
	          "", 
	          "Application rate in each \\emph{individual} paddock")
...
For the final RQ for the echinacea study (Sect. \@ref(Writing-RQs)), the *response variable* would be the length of cold symptoms, and the *explanatory variable* is the type of medication (echinacea; or none).
...
`r webexercises::hide()`
Importantly, note that the *Population* is 'carrots grown in Buderim' 8 weeks after planting.

From these carrots, all we need to collect is information about *Whether or not Thrive was applied* and the *weight of the carrots 8 weeks after planting*.

That is: The *response* variable is 'the weight of each individual carrot 8 weeks after planting', and the *explanatory* variable is 'whether or not Thrive was used on each carrot'.

('The number of carrots planted' is not even a variable. Can you see why?)
`r webexercises::unhide()`
:::
`r if (knitr::is_latex_output()) '-->'`
...
(This would be found by measuring their weight *before* and *after* three weeks on the diets.)
...
> For Australian adults, is the average number of steps per day the same for those using a waist and wrist accelerometer?

The **population** is the group being studied.
We cannot study *every* Australian adult, but the population is *all* Australian adults because that is the group that we wish to understand.

The **outcome** is what is of interest in the population, and describes an entire group. 
In this case, we might talk about the *average* number of steps per day.

The **response variable** describes the individuals in the population.
In this case, from each individual we would record the number of steps per day; this is the response variable.

The **comparison** refers to the groups for which the outcome is compared.
Here, the comparison here is "Between those wearing a waist accelerometer and those wearing a wrist accelerometer".

The comparison compares two separate groups within the population that are different, or have been treated differently.
Comparisons usually start with "Between..." or something similar.

The **explanatory variable** is what varies from person to person.
In this case, what *varies* is "Where the subjects wear the accelerometer", so this is the explanatory variable.

This RQ may or may not have an **intervention**.

If the researchers can manipulate (i.e., tell) the people where to wear the accelerometer, there *is* an intervention.

If the individuals have chosen themselves where to wear the accelerometer, there is no intervention.
:::
...
> In Australian 20-something men, is the average thickness of head hair strands the same for [blond men](https://www.cjr.org/analysis/blondes_brunettes_chaperone.php) and [brunet men](https://www.dictionary.com/browse/brunet?s=t) [@vaughn2009comparison]?

What is the problem with comparing 100 hair strands from one blond man, to 100 hair strands from one brunet man?

`r webexercises::hide()`
We only have *one* example of a blond man, and *one* of a brunet man. 
The sample size is just 2.

All the hairs from one man will be very similar: 
The hair gets washed at the same time with the same shampoo, is exposed to the sun for the same length of time, has the same genetics.

We don't have a good cross-section of what the hair strands of blonds and brunets, in general, are like.
`r webexercises::unhide()`
:::

In this study, *only one man of each hair colour is represented*.
There are 200 observations, but only two people are compared, so little is learnt about 20-something men *in general*.

We learn a lot about two men specifically.
The Population is represented by just two men... so we don't learn much about the population of men *in general*.
...
But each blond hair comes from the same man, so each of those hairs have essentially lived their life together: They are washed at the same time, with the same shampoo, exposed to the same amount of sunlight and exercise, share genetics, etc.
However, different people do their own thing and have their own genetics.


::: {.definition #UnitOfObservation name="Unit of observation"}
*Unit of observation*:\index{unit of observation}
The 'who' or 'what' which are observed, from which measurements are taken and data collected.
:::
...
In the example above, each man is a *unit of analysis*.
...
Importantly, the size of sample in the study is the number units of analysis; so here, *there are only two examples of the population in the study*.
...
However, the unit of *analysis* is the *person*: the hair strands from each man share a lot in common.
The men themselves would share little in common, and we are interested in comparing men.
:::
...
People in a  *group* at the beach will probably not be operating 'independently': groups of people tend to behave similarly (but perhaps not identically).
For example, a couple will often *both* be either wearing or *not* wearing sunglasses.
...
The *unit of analysis* is the *car*: the brand of tyre is allocated to the car and all wheels on the car get the same tyre.
Tyres on any one car 'live their life together': They all are exposed to the same day-to-day use, the same drivers, have driven very similar distances,  under the same conditions, etc.
:::
...
>
> The research team had looked at 10 neurons from each of the 16 mice in the experiment, a practice that in itself was unproblematic. 
> But in the statistical analysis, the researchers had analyzed each neuron as if it were an independent [individual observation]. 
...
There were 16 units of analysis (mice) so the sample size is 16 mice, but the authors treated the $16\times 10 = 160$ neurons as the sample size.
The 10 neurons from each mouse have a lot in common: the genetic information was the same for all 10 neurons from each mouse.
...
However, they are sometimes different too, and it is *crucial* to be able to identify these situations.
...
Each of 44 children (whose parents agreed for their children to participate in the study) were allocated to one of two PA programs.
The improvement in children's fitness was measured for every student in the study after six months.

The *units of observation* are the individual students, as the the fitness measurements are taken from the students.
The *units of analysis* are also the individual students, as the PA program was allocated to each student individually.
:::
...
The students in any one school are exposed to the same administrative policies, similar weather and physical conditions, teachers and school-based teaching philosophies, etc. 
...
Most statistical software (including [jamovi](https://www.jamovi.org/) [@Software:jamovi] and [SPSS](https://www.ibm.com/products/spss-statistics) [@Software:SPSS]) uses the same approach for collating the data^[Though there are exceptions for some types of analyses.]:

* Each *row* represents one unit of analysis.
  Hence, the *number* of rows will equal the *number* of units of analysis.
* Each *column* represents one variable.
  Hence, the *number* of columns will equal the *number* of variables.
...
In Sect. \@ref(PopToIndividuals), this RQ was posed:
  
> Among Australian teenagers with a common cold, is the average duration of cold symptoms shorter for teens given a daily dose of echinacea compared to teenagers given no medication?

For this RQ, the *variables* are (Examples \@ref(exm:Variables) and \@ref(exm:Variables2)):
...
* The number of *columns* would be two:
  one column to record the duration of each individual's cold symptoms, and the other to record whether the individual received a dose of echinacea or received no medication.  
  
  In addition, there may be a column recording the name or ID of each individual.  
  
The variable names (say, `Duration` and `Treatment`) would not be in a row of their own; they would be the columns names (Fig \@ref(fig:DataPrepjamovi)).
:::
...
While spreadsheets (such as Excel) can be used for analysing data, [significant problems can, and do, emerge with using spreadsheets](#Software-In-Research).
...
In this chapter, we have learnt about writing and understanding **research questions**.
	
* Research questions (RQs) are always about an **outcome** (O) in some **population** (P).
* Some RQs have a **comparison** or **connection** (C).
* Some also have an **intervention** (I): When the C can be manipulated by the researchers.

By comparison, here we mean "between individuals" comparisons.

RQs may be:

* **Descriptive**, with only a P and O;
* **Relational**, with a P, O and C; or
* **Interventional**, with a P, O, C and I.
	
We have also learnt about two approaches to research questions:

* **Estimation**-type RQs; or 
* **Decision**-type RQs.

For *quantitative* research questions,  the outcome *numerically* summarises the population (or subsets of the population), so is usually worded in terms of percentages, averages, etc.
...
```{r Chap2Summary, animation.hook="gifski",  interval=1.5, echo=FALSE, fig.cap="Chapter 2 summary", fig.height = 3, fig.align="center", dev=if (is_latex_output()){"pdf"}else{"png"}}
if (knitr::is_html_output()) {
...
> In female elite netball players, do players in defence positions have a greater average number of knee injuries (per player per season) than players in attacking positions?
  ...
  a. What is the **comparison** in this RQ?
`r if( knitr::is_html_output()) {
longmcq( c(
...
`r if (!knitr::is_html_output()) '<!--'`
`r {webexercises::hide()}`
Remember that the **outcome** describes a group of netballers: We could talk about the *average* for the group.

In contrast, the response *variable* is measured on each individual: The number of knee injuries for that player.
`r webexercises::unhide()`
`r if (!knitr::is_html_output()) '-->'`
...
> For Australian adults with a common cold, do people who take Vitamin C tablets have an average shorter cold duration than people who do not take any Vitamin C tablets?

In this RQ, the population is
`r if( knitr::is_html_output() ) {mcq( c("Australians", answer = "Australian adults", "Australians who take Vitamin C tablets") )} else {"________________"}`

The outcome is
`r if( knitr::is_html_output() ) {mcq( c("The cold duration", answer = "The average cold duration", "The cold") )}  else {"________________"}`

The `r if( knitr::is_html_output() ) {mcq( c("intervention", answer = "comparison", "unit of analysis") )} else {"________________"}` is between those who take Vitamin C tablets and those who do not.

The `r if( knitr::is_html_output() ) {mcq( c("explanatory", answer = "response") )} else {"________________"}` variable is the duration of the cold symptoms for each individual person. 

The `r if( knitr::is_html_output() ) {mcq( c(answer = "explanatory", "response") )} else {"________________"}` variable is whether or not each person take Vitamin C tablets or not.

It is not clear whether there is an intervention.
If there *is* an intervention, this would be `r if( knitr::is_html_output() ) {mcq( c(answer = "an interventional", "a relational") )} else {"________________"}` RQ, otherwise it would be
`r if( knitr::is_html_output() ) {mcq( c("an interventional", answer = "a relational") )} else {"________________"}` RQ.
...
1. Waking speed probably depends on (or *responds to*) how *individual* people are using their phone: the *response variable* is the walking speed.
...
4. A research study was comparing the average size of Blue Gum eucalypt leaves in two areas of Queensland.  
A student takes 40 leaves from each of ten trees in Area A, and 40 leaves from each of ten trees in Area B.  
Which of the following statements are true?

  a. The unit of analysis is the individual leaf.
`r if( knitr::is_html_output() ) { torf( FALSE )}`
  b. The unit of observation is the individual leaf.
`r if( knitr::is_html_output() ) { torf( TRUE )}`
  c. The unit of analysis is the tree.
`r if( knitr::is_html_output() ) { torf( TRUE )}`
  d. The size of the sample in the study is 80.
`r if( knitr::is_html_output() ) { torf( FALSE )}`
  e. The size of the sample in the study is 20.
`r if( knitr::is_html_output() ) { torf( TRUE )}`
  f. The size of the sample in the study is 800
`r if( knitr::is_html_output() ) { torf( FALSE )}`
...
In a study of public acceptance of alternative water supplies [@hurlimann2016public], various water sources are defined.
In Table \@ref(tab:RQsWaterDefinitionsData), match the term with the appropriate conceptual definition.
:::
...
1. For this RQ, identify the **C**omparison, if any.
...
<!-- SEM 1 2019: Danielle S.; Matt A.; Nicholas R.eeves-->
<!-- ```{exercise ProjectRQ1} -->
...
> Of students that study at UniSC, Sippy Downs, do males have a larger lung capacity than females? 
:::
