...
::: {.example #DefinitionsTemp name="Operational and conceptual definitions" data-latex=""}
...
*Comparison*: between highly active and sedentary women. 
...
There is *no* comparison, since there are not two subsets of the population being compared.
...
There *is* a comparison: the two subsets of the population (Australians) being compared are females and males.
:::
...
1. **Interventional**:
   The RQ has a comparison ('comparing trees planted in a concrete sidewalk and a grassed sidewalk'), and the wording sounds like these trees were planted here intentionally.
...
```{r Chap1Summary, animation.hook="gifski",  interval=3.0, echo=FALSE, fig.cap="?????????????????", fig.align="center", dev=if (is_latex_output()){"pdf"}else{"png"}}
if (knitr::is_html_output()) {

for (i in (1:9)){
  
  par( mar = c(0.2, 0.25, 0.25, 0.25), # Number of margin lines on each side
       pin = c(5, 5)) # Current plot dimensions]
  ...
  if (i <=  2){
    textrect( colMeans( pos[1:2, ] )  + c(0, 0.1),
              lab = "Descriptive RQ",
              radx = 0.15,
              rady = 0.03,
              shadow.size = 0,
              ...
              cex = 1.0)
    if (i == 2) {
      textrect( colMeans( pos[1:2, ] ) + c(0, 0.2),
  ...
  if ( (i == 3) | ( i == 4) ){
    textrect( colMeans( pos[1:3, ] ) + c(0, 0.1),
               lab = "Relational RQ",
              radx = 0.15,
              rady = 0.03,
              shadow.size = 0,
              ...
              cex = 1.0)
    if (i == 4) {
      textrect( colMeans( pos[1:3, ] ) + c(0, 0.2),
  ...
  if ( (i >= 5) & ( i <= 7) ){
    textrect( colMeans( pos[1:4, ] ) + c(0, 0.1),
              ...
              cex = 1.0)
    if (i >= 6) {
      textrect( colMeans( pos[1:4, ] ) + c(0, 0.2),
  ...
  if ( i == 8 ) {
    straightarrow( from = colMeans( pos[1:4, ] ) + c(-0.1, -0.1),
                   to = pos[2, ],
                   lwd = 2)
  }
  if ( i == 9 ) {
  ...
  if ( i == 8 ) {
    textrect( colMeans( pos[1:4, ] ) + c(-0.1, -0.1),
