...
<!-- ```{r echo=FALSE, fig.cap="", fig.align="center", fig.width=3} -->
...
```{r RQLeadsTo, echo=FALSE, fig.cap="The RQ drives all aspects of the study", fig.align="center", fig.height=3.25, fig.width=4}
par( mar=c(0.15, 0.15, 0.15, 0.15))
openplotmat()

pos <- array(NA, dim=c(4, 2))
pos[1, ] <- c(0.50, 0.3) # RQ
...
curvedarrow(from=pos[1,], to=pos[2,], 
            curve=0, 
            lty=1, 
            lwd=2)
curvedarrow(from=pos[1,], to=pos[3,], 
            curve=0, 
            lty=1)
curvedarrow(from=pos[1,], to=pos[4,], 
            curve=0, 
            lty=1)
...
## Conceptual and operational definitions {#OperationDefinitions}

Research studies
usually include terms that must be carefully and precisely defined,
...
Loosely speaking,
a *conceptual definitions* explains *what* to measure or observe (what a word or a term *means* for your study),
and an *operational definitions* defines exactly *how* to measure or observe it.

For example,
in a study of stress in students during a university semester.
...
Sometimes,
a commonly-accepted definition does not exist,
so the definition being used should be clearly articulated.
...
> Several common features that incorporate clinical, pathologic and biomechanical injury constructs 
> that may be utilised in defining the nature of a
...
include_graphics("Illustrations/pexels-oladimeji-ajegbile-2696299.jpg")
```
`r if (knitr::is_latex_output()) '-->'`


```{example, Definitions2, name="Operational definitions"}
Consider a study measuring stress in first-year university students.

Stress cannot be measured directly,
but could be assessed using a survey
(like the Perceived Stress Scale (PSS)
[@cohen1983global]).

The *operational definition* of stress is the 
score on the ten-question PSS.
Other means of measuring stress are also possible
...
People may be classified as stutters by some
...
used this operational definition of 'snacking':

> ...an eating occasion that occurred between meals based on time of day.
>
> --- @data:Fayet2017:Snacks (p. 3)
...
> ... people in the Northeast walk faster, make change faster, talk faster 
> and are more likely to wear a watch...
>
...
```{lemma DefinitionsSmoker, name="Definitions"}
Define a 'smoker'.
```
...
include_graphics("Pics/iconmonstr-friend-5-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
*Individuals* or *cases* do not just refer to people.,
though the words may be commonly used that way.

Similarly,
*population* does not just mean people.
In this context,
a population is *any* group of interest;
for example:
...
The *population* is not just those individuals 
from which the data are actually *obtained*.
Indeed,
*all* these elements of the population 
may not be accessible in practice.

The population represents all the 'individuals' to which
the results are to be generalised.
For example,
when testing a new drug,
the aim is to see if it works on people in general,
...
that we actually end up studying,
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-sitemap-20-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
Consider a study of American college women,
which aimed to:
...
The *sample* comprises 28 'active' women and 
28 'sedentary' American college women,
from which data are collected.

The *population* is *all* 'active and sedentary' American college women,
not just the 56 in the study.
The group of 56 subjects is the 
*sample*.\index{sample}
```

Completely defining the population
[@banerjee2010statistics]
sometimes requires refining the population,
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-checkbox-4-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
may only included sites where there has been a confirmed sighting within the last two years.
```


```{example InclusionCriteriaEG2, name="Inclusion criteria"}
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-checkbox-14-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
```{example ExclusionCriteriaEG2, name="Exclusion criteria"}
...
```{block2, type="exampleExtra"}
A study 
[@data:Guirao2017:amputees]
of the walking abilities of amputees used
...
Because the RQ concerns a population,
the outcome describes a population as a whole;
hence, the outcome is usually an 
*average*, *percentage*, or *general* quantity 
numerically summarising the population
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-process-1-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
small number of distinct subsets of the population
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-tree-16-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
    
```{definition, Comparison, name="Comparison"}
The *comparison*\index{comparison}
in the RQ identifies the small number of distinct subsets of the population between which the outcome is compared.
```
...
in two subsets of the population;
...
'the *difference* between right- and left-arm blood pressure'.

In contrast, a study comparing the average blood pressure 
between females and males *does* have a comparison:
two subsets (females and males) 
...
the value of the outcome (potentially) changes:
...
In addition to having a population (P), an outcome (O), 
and possibly a connection or comparison (C),
some RQs also have an *intervention*.
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-medical-14-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
is 
a comparison or connection that the 
researchers have *imposed* upon those in the study,
...
* explicitly applying wear testing loads to two different 
  flooring materials.
...
include_graphics("Pics/iconmonstr-medical-6-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
foldPOCI <- c("*Outcome*: 'average iron status' (which would need an *operational definition*.) *Comparison*: between highly active and sedentary women. *Intervention*: Probably none; an intervention would mean the *researchers* tell each individual woman to be highly active or sedentary, which seems unlikely.") ### Seems to only work if all one line
} 
if( knitr::is_latex_output() ) {
foldPOCI <- foldLaTeXText
}
```

(ref:foldPOCIr) `r foldPOCI`
...
<!-- ```{block2, type="fold"} -->
<!-- The *Outcome* is 'average iron status'. -->
<!-- (This would need to be defined using an *operational definition*.) -->
<!-- The *Comparison* is between highly active and sedentary women. -->
<!-- (That is, the iron status is being *compared* between these two groups.) -->
<!-- There is probably no *Intervention*. An intervention would mean that -->
<!-- the *researchers* tell each individual woman to be highly active or sedentary, -->
...
depending on whether the RQ also has an 
...
```{lemma POCICoeliacs, name="POCI"}
Consider this RQ:

> Among Australian adults, what proportion are coeliacs?

For this RQ, identify
the **P**opulation and the **O**utcome.

``` 

```{r echo=FALSE}
POCIfold <- "*Population*: 'Australian adults'. *Outcome*: 'the proportion that are coeliacs'. (Note the *proportion* describes the population in general.)"
if( knitr::is_latex_output() ) {
POCIfold <- foldLaTeXText
}
```

(ref:POCIfold) `r POCIfold`
...
Notice that the average heart rate (Outcome) 
is a numerical summary across the two
population sub-groups being compared (females; males).

The sex of the individual (the C) is not allocated by the researchers,
...
Identify
the **P**opulation, the **O**utcome,
and the **C**omparison.
```

```{r echo=FALSE}
POCIfoldAmbo <- "*Population*: 'Calls to the Queensland Ambulance Service'; the Outcome and the Comparison information come from these. *Outcome*: 'average response time'. *Comparison*: 'between weekdays and weekends'."
if( knitr::is_latex_output() ) {
POCIfoldAmbo <- foldLaTeXText
}
```

(ref:POCIfoldr) `r POCIfoldAmbo`
...
```{lemma, POCIMiners, name="POCI"}
Consider this RQ 
(based on @data:Maron:eucthreshold):
...
```{r echo=FALSE}
POCIfold <- "*Population*: 'Queensland state forests'.*Outcome*: 'the number of noisy miners'. *Connection*: 'the number of eucalypts'. Since the number of eucalypts would probably *influence* the number of birds, the *Outcome* would be the number of noisy miners."
if( knitr::is_latex_output() ) {
POCIfold <- foldLaTeXText
}
```

(ref:POCIr) `r POCIfold`
...
colnames(RQtypes) <- c("RQ type", "P", "O", "C", "I")

RQtypes[1, ] <- c("Descriptive (D)",    "Yes", "Yes", "",    "")
RQtypes[2, ] <- c("Relational (R)",     "Yes", "Yes", "Yes", "")
RQtypes[3, ] <- c("Interventional (I)", "Yes", "Yes", "Yes", "Yes")
...
```{r echo=FALSE}
RQType2 <- "1: Relational (using a Comparison). 2: Relational (using a Comparison) or Interventional, depending on whether the researchers plant the trees explicitly. 3: Descriptive. 4: Relational (using a Connection)."
if( knitr::is_latex_output() ) {
RQType2 <- foldLaTeXText
}
```

(ref:RQType2r) `r RQType2`
...
```{r echo=FALSE}
RQType <- "1: Decision making; 2: Estimation; 3: Estimation; 4: Decision making."
if( knitr::is_latex_output() ) {
RQType <- foldLaTeXText
}
```

(ref:RQTyper) `r RQType`
...
### Estimation: Confidence intervals questions {#TwoApproachesCI}
...
### Making decisions: Hypothesis testing questions {#TwoApproachesHT}
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "10%", out.extra='style="float:left; padding:10px"'}
include_graphics("Pics/iconmonstr-share-4-240.png")
```
`r if (knitr::is_latex_output()) '-->'`
...
## Writing good RQs {#WritingGoodRQs}
...
include_graphics("Illustrations/pexels-pixabay-65882.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
Here is a RQ submitted by a student group (including typos):
...
```{r echo=FALSE}
POCI2 <- "*Population*: Australian teens with a common cold. *Outcome*: *Average* duration of the cold. *Comparison*: Between teens taking echinacea to those not taking echinacea. *Intervention*: Probably, since the medications are described as being *given* to the teenagers."
if( knitr::is_latex_output() ) {
POCI2 <- foldLaTeXText
}
```
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/rgR073gyYXk" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
if( knitr::is_latex_output() ) {
  ...
  colnames(PopInd) <- c("Population", 
                        "",
                        "Individuals")
  PopInd[1, ] <- c("Outcome:", "$\\rightarrow$", "Response variable")
  PopInd[2, ] <- c("Comparison/Connection:", "$\\rightarrow$", "Explanatory variable")
...
if( knitr::is_html_output() ) {
  ...
  PopInd[1, ] <- c("![](./Pics/iconmonstr-friend-5-240.png){#id .class height=60px}",
                   "",
                    "![](./Pics/iconmonstr-generation-16-240.png){#id .class height=60px}")
  PopInd[2, ] <- c("Population", "", "Indivduals")


  PopInd[3, ] <- c("Outcome:", "$\\rightarrow$", "Response variable")
  PopInd[4, ] <- c("Comparison/Connection:", "$\\rightarrow$", "Explanatory variable")
  ...
  kable(PopInd,
      ...
      caption="The relationship between the population and the individuals",
      booktabs=TRUE) %>%
  row_spec(2,bold=TRUE) # Columns headings in bold
...
```{r echo=FALSE, RQsPopulationIndividualsExamplesOutcome}
PopInd2 <- array( dim=c(7, 3) )


if( knitr::is_latex_output() ) {
  PopInd2[1, ] <- c("![](./Pics/iconmonstr-friend-5-240.png){#id .class height=60px}",
                     "",
                      "![](./Pics/iconmonstr-generation-16-240.png){#id .class height=60px}")
  PopInd2[2, ] <- c("Population", "", "Indivduals")
  
  
  PopInd2[3, ] <- c("\\emph{Average} increase in diastolic blood pressure", "", "Increase in diastolic blood pressure of \\emph{individuals} before and after exercise")
  PopInd2[4, ] <- c("\\emph{Percentage} of seedings that sprout", "", "Whether or not an \\emph{individual} seedling sprouts")
  PopInd2[5, ] <- c("\\emph{Proportion} owning iPad", "", "Whether or not an \\emph{individual} owns an iPad")
  PopInd2[6, ] <- c("\\emph{Average} cold duration", "", "Cold duration for \\emph{individuals}")
  PopInd2[7, ] <- c("\\emph{Percentage} of concrete cylinders having fissures", "", "Whether or not an \\emph{individual} cylinder has fissures")
...
if( knitr::is_html_output() ) {
  ...
  PopInd2[3, ] <- c("*Average* increase in diastolic blood pressure", "", "Increase in diastolic blood pressure of *individuals*() before and after exercise")
  PopInd2[4, ] <- c("*Percentage* of seedings that sprout", "", "Whether or not an *individual* seedling sprouts")
  PopInd2[5, ] <- c("*Proportion* owning iPad", "", "Whether or not an *individual* owns an iPad")
  PopInd2[6, ] <- c("*Average* cold duration", "", "Cold duration for *individuals*")
  PopInd2[7, ] <- c("*Percentage* of concrete cylinders having fissures", "", "Whether or not an *individual* cylinder has fissures")
...
*response variable* would be length of cold symptoms,
and the 
...
```{lemma VarsCarrots, name="Variables"}
Consider this RQ:
...
2. What data is *needed* from each element of the population
...
## Units of observation and units of analysis {#UnitsObsAnalysis}

**Units of observation** and **units of analysis**
are important, but similar,
concepts that need to be distinguished.

```{lemma DesignsHair, name="Designs"}
Consider this RQ:

> In Australian 20-something men,
> is the average thickness of head hair strands the same
> for [blonds](https://www.cjr.org/analysis/blondes_brunettes_chaperone.php) 
> and [brunets](https://www.dictionary.com/browse/brunet?s=t)
...
`r if (knitr::is_latex_output()) '<!--'`
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-moose-photos-1036627.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
each *person* a *unit of analysis*.
Importantly,
...
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/pexels-nappy-936018.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
```{r, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
include_graphics("Illustrations/winter-tires-4664205_640.jpg")
```
`r if (knitr::is_latex_output()) '-->'`
...
```{lemma,UnitsSchools,  name="Units of analysis and observation"}
A study compared two school physical activity (PA) programs.
...
## Preparing software for data entry {#DataEntry}
...
* Each *row* represent one unit of analysis.
  Hence,
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/rgR073gyYXk" frameborder="10" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe> 
...
<!-- ####################### REVIEW QUESTIONS ##################################### -->
```{r, child = if (knitr::is_html_output()) './children/ReviewQuestions/ReviewQuestions-Chapter2.Rmd'}
```
...
> the same for students who regularly drive to USC
...
Consider this actual Project Report RQ.
