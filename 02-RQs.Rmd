...
This is a relational, *estimation*-type relational RQ.
:::
...
Comparison can are *manipulated* by the researchers, or *imposed* on the individuals by the researchers, are called *treatments*

Either *between*- or *within*-individuals comparison can have an intervention.
...
The intervention may be present when the researchers: 
...
Answering the question by measuring $100$ hair strands from one man is problematic: *only one man is represented*.
While there are $100$ observations, all the data come from one man; little is learnt about $20$-something men *in general*.
Instead, a lot is learnt about one specific man.
The population is represented by just one man.
...
Since each hair comes from the same man, each of those hairs have essentially 'lived their life together': they are washed at the same time, with the same shampoo, exposed to the same amount of sunlight and exercise, share genetics, etc.
...
In the example above, the man is a *unit of analysis*, and provides $100$ observations.
...
In the hair-thickness study, a *person* is a *unit of analysis*, so the sample size is just one.
Only one example of the population of men is in the study.
*Each unit of analysis (man) has $100$ units of observation (hair strands).*


::: {.tipBox .tip data-latex="{iconmonstr-info-6-240.png}"}
The number of units of *analysis* in a study is the size of the sample.\index{Sample size}
:::
...
reported a study where researchers 'looked at $10$ neurons from each of the $16$ mice' in the study.
...
* An *operational definition* defines *how* something will be observed or measured.\index{Definitions!operational}
...
An *explanatory variable* is a variable that records the values of the comparison on individuals in the population. 
:::
...
Repeated-measures RQs compare the *same* outcome when measured on teh same individuals multiple times (a within-individuals comparison). 
