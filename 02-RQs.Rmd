...
For example, a couple will often (but not always) *both* be wearing or both *not* wearing hats; friends often behave in similar ways.
...
* each *row* represents one unit of analysis.\index{Unts of analysis}
...
An additional column of identifying information may also appear.


In *long* format:\index{Computers and software!long format}

* each unit of analysis is repeated *multiple* times, one row for each time the within-individuals variable\index{Variables!within-individuals} is recorded.
* the columns for each between-individual variable must be repeated for each row too. 
* each within-individuals variable is recorded in a separate *column*.
...
```{r DataPrepNormaljamovi, fig.show="hold", out.width='37%', fig.align="center", fig.cap="Software prepared for data with no within-individuals variable.", fig.align="center", fig.width=3, fig.height=4}
knitr::include_graphics( "jamovi/DataPrep/DataPrep.png" )
```


```{r DataPrepWithinjamovi, fig.show="hold", out.width=c('51%', '5%', '42%'), fig.align="center", fig.cap="Software prepared for data with a within-individuals variable; the same data is shown in both panels. Left: in wide format. Right: in long format. Both include a column of identifying information.", fig.align="center", fig.width=3, fig.height=4}
knitr::include_graphics( "jamovi/DataPrep/WideFormat.png" )
knitr::include_graphics( "OtherImages/SPACER.png" )
knitr::include_graphics( "jamovi/DataPrep/LongFormat.png" )
```
