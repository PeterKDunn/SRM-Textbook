...
```{r RQLeadsTo, echo=FALSE, fig.cap="The research question drives all aspects of the study", fig.align="center", fig.height=1.75, fig.width=5.5, out.width = '60%'}
...
```{r PermPavements, echo=FALSE, fig.cap="A study of permeable pavements (PPs) may have many sub-questions", fig.align="center", fig.height=2, fig.width=7, out.width = '80%'}
par( mar = c(0.10, 0.15, 0.10, 0.15))
openplotmat()
...
In most cases, any data being collected will need a clear *operational definition*, to ensure repeatability and consistent data collection, by removing any ambiguity about how data are measured.
...
This is important, since stress cannot be measured directly (like height, for example).\smallskip
...
* Young: 8--15 years;
* Competitive tennis players: Participants were some of the best players in their age category in France, and were members of a French tennis centre of excellence.
...
Consider a study requiring water temperature to be measured.\smallskip

An *operational definition* would explain *how* the temperature is measured: the thermometer type, how the thermometer was positioned, how long was it left in the water, and so on.\smallskip
...
For example, when testing a new drug, the aim is to see if it is effective on people in general, including people not yet born. 
...
In contrast, a *sample* is the *subset* of the population from which data are obtained (see Chap. \@ref(Sampling)).
...
Concrete test cylinders with fissure cracks may be excluded from tests of concrete strength.\smallskip
...
To answer this RQ, what is the problem with comparing 100 hair strands from one blond-haired man, to 100 hair strands from one brunet-haired man?

`r if (knitr::is_latex_output()) '<!--'`
`r webexercises::hide()`
The study includes only *one* example of a blond-haired man, and *one* of a brunet-haired man, so it is hard to make generalisation about blond- and brunette-haired men in general.
...
We don't have a good cross-section of what the hair strands of blonds and brunets from men, in general, are like.
`r webexercises::unhide()`
`r if (knitr::is_latex_output()) '-->'`
:::
...
Importantly, the size of sample in the study is the number units of analysis; so here, *only two examples of the population of men are in the study*.
...
Four tyres of Brand A are allocated to each of Cars 1--5, and four tyres of Brand B are allocated to each of Cars 6--10.\smallskip

After 12 months, the amount of wear is recorded on each tyre.
The *unit of observation* is the *tyre*: the amount of wear is measured on each tyre.\smallskip

The tyres on any one car do not operate independently; the four tyres on a single car 'live their life together'.
They all are exposed to the same day-to-day use, the same drivers, have driven almost identical distances, under the same conditions, etc.\smallskip
...
Consider comparing the percentage of females and males wearing sunglasses at a specific beach.\smallskip
...
For example, a couple will often *both* be either wearing or *not* wearing sunglasses; families will often all be wearing sunglasses or not wearing sunglasses.\smallskip
...
The improvement in children's fitness was measured for every student in the study over six months.\smallskip

The *units of observation* are the individual students, as the the fitness measurements are taken from the students.
The *units of analysis* are also the individual students, as the PA program was allocated to each student individually, and each student has their own sport, routines, etc.
:::
...
* *average* increase in heart rates before and after exercise.
...
In addition to having a population (P) and an outcome (O), some RQs may *compare* the outcome between a small number of different, distinct subsets of the population (that is, different *groups of individuals*), or may explore a *connection* between the outcome and some other quantity that varies with a large number of possible values.
...
:::{.example #WithinBetweenComparison name="Between- and within-individual comparisons/connections"}
Consider studying the strength of left and right legs of football players.\smallskip

A *between*-individuals comparison would compare the left and right leg strengths of different groups: one group would have their left-leg strength measured, and the other their right-leg strength measured.\smallskip

In contrast, a *within*-individuals comparison would measure both the right and left strength on the same individuals: the comparison is within each individual.\smallskip
...
A study comparing the average blood pressure (O) in female and male (C) Australians (P) measured blood pressure using a blood pressure machine (a sphygmomanometer).\smallskip
...
Using the sphygmomanometer is just a way to measure blood pressure, to *obtain* the data.\smallskip
...
Typically, descriptive RQs have the form:
...
A study of farming with multiple livestock species [@schanz2022single] asks the question:

> In co-grazing farms in Germany, how often do young cattle and chickens interact?

This is a descriptive RQ, with a population ('co-grazing farms in Germany') and an outcome ('how often do young cattle and chickens interact', which is slightly vague); no relationship with anything else is suggested.
:::
...
Usually, *relationships* are more interesting than just describing a population.
...
Typically, relational RQs with a *comparison* has the form:
...
> Among Cubans between 18 and 35 years of age, is the average heart rate the same for females and males?
