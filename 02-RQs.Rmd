...
Quantitative RQs must be *written* carefully so they can be *answered* effectively.
...
*All RQs* identify a large group of interest to be studied, called a *population* (Sect.\ \@ref(Population)).\index{Population}
Descriptive RQs study something *about* the population, called the *outcome* (Sect.\ \@ref(Outcome)).\index{Outcome}
...
The population is 'all people', *not* just those people studied.
...
Descriptive RQs study something *about* the population, called the *outcome*.
Because the RQ concerns a large group (the population), the outcome must be suitable for describing a group (not individuals).
...
* *Estimation*: Among {*the population*}, what is {*the outcome*}?\index{Research questions!Estimation}
* *Decision-making*: Among {*the population*}, is {*the outcome*} equal to {*a given value*}?\index{Research questions!Decision making}
...
A study examined the 'body temperature of $148$ healthy men and women' [@data:mackowiak:bodytemp] aged between $18$ to $40$; this is the *population*.
...
A *decision-making* descriptive RQ they also studied was whether the average body temperature was the value that had been commonly accepted by medical professionals:
...
These comparisons are called *between-individuals* comparisons,\index{Comparison!between individuals} as they compare the outcome *between* different groups of *individuals*.
...
::: {.definition #ComparisonBetween name="Comparisons (between individuals)"}
...
This study examines *within*-individuals changes: the differences between the left- and right-leg strength *within* each individual.
...
Instead, researchers decide *how much* sample evidence support a particular hypothesis about the *population*.\index{Hypotheses}
...
If the study *did have an intervention*, various interventions could be imposed by the researchers; the dose *frequency* or the doses *amounts* could be imposed, for instance.
...
PopInd3[1, ] <- c("Between jarrah, beech and bamboo floor boards", 
                  "", 
                  "Type of floorboard in \\emph{individual} homes")
PopInd3[2, ] <- c("Between $30$\\ kg/ha and $40$\\ kg/ha fertilizer rates", 
                  "", 
...
The study also explored the the relationship between the daily sodium excretion and the systolic blood pressure.
