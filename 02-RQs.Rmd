...
RQ are about *populations*.
However, the data to answer a RQ come from *individuals* in that population.
...
:::{.example #CorrelationalRunners name="Correlational RQs"}
...
@gonzalez2024length studied the size of $39$ demersal fish species, recorded the length and weight of $14\ 040$ fish.
There are two variables (fish length; fish weight), but it makes no sense to identify a response variable and explanatory variable.
:::



:::{.example #CorrelationalPine name="Correlational RQs"}
The Wollemi pine was discovered by science in 1994.
@offord2023home studied the growth of these rare plants.

One correlational RQ they studied was the relationship between the diameter of trees at breast height (response variable), and the pH of the soil (explanatory variable).
...
Sometimes, the explanatory variable naturally occurs (e.g., age group, or the age, of people), and sometimes the explanatory variable is manipulated by researchers and called an *intervention* (e.g., the amount of fertilizer applied).


::: {.definition #Intervention name="Intervention"}
An intervention is present when *researchers* can manipulate (or impose) the *explanatory variable* on the individuals to determine the impact on the response variable.
:::


Explanatory variables that are *not* manipulated by the researchers are called *conditions*.
Explanatory variables that are *manipulated* by the researchers, or *imposed* on the individuals by the researchers, are called *treatments*
The *analysis* is the same whether an intervention is used or not.
However, the *interpretation* of the results depend on whether or not an intervention is used (Sect.\ \@ref(CompareStudyTypes)).
...
Often, in studies with a comparison, one comparison groups is the *control group*.
...
For this RQ, the *variables* are 'Duration of cold symptoms' (response), and 'Type of treatment' (explanatory); see Example\ \@ref(exm:Variables).
