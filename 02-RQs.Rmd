...
RQs can also be written in one of two different forms, which have different purposes:

* For *estimation* (or forming *confidence intervals*):
  These RQs ask how precisely a *value* in the *population* is estimated by using the *sample*.
...
* For *making decisions* (or *hypothesis testing*):
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
These are **not** 'recipes', but a guidelines.
:::
...
A *decision-making* RQ they also studied was whether the average body temperature was the value that had been commonly accepted:

> Is the mean body temperature 98.6^o^F?
:::
...
* *Estimation*: Among {the population}, what is the difference between {the outcome} for {the groups being compared}? 
...
This RQ is a *decision-making RQ*, since it asks if the average heart rate is the same or not.

An estimation-type relational RQ would be:

> Among Cubans between 13 and 20 years of age, what is difference in the average heart rate between females and males?
:::
...
An *estimation-type RQ* could be used to estimate the composting time.
A *decision making-type RQ* could be used to compare the composting times for waste with and without earthworms added. 
:::
...
3. What is the strength of the association between plasma zinc and retinol concentrations?
  This is an *estimation* RQ, estimating the strength of the relationship.
4. Is the average zinc intake the same for boys and girls? 
  This is an *decision-making* RQ.
  
The fourth RQ is a decision-type RQ, with two possible answers (Fig. \@ref(fig:ZincRQ)): either the zinc intake *is* the same for females and males, or *is not* the same for females and males.
...
textrect( pos[2,], 
           lab = "YES: the average zinc intake\nis the same for boys and girls", 
           radx = 0.20, 
           rady = 0.1, 
           shadow.size = 0,
