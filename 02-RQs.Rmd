...
A *comparison* refers to an attribute that can be recorded for a small number of distinct groups, and the outcome compared.
For example, crop yield (an outcome) may be compared for some fields where a fertilizer *has* been used, and some other fields where a fertilizer *has not* been used (the two comparison groups).
...
The *control group* comprises units of analysis *as similar as possible* to the other units of analysis (called *controls*), but without the receiving the treatment being studied, or without having the condition being studied.
...
Ideally, controls would be people who, like the treatment group, report to a doctor and receive an injection... however, they injection is ineffective (a placebo).
:::
...
*Between-individuals comparisons or connections* mean that the comparison or connection is between different individuals.
...
The rare Wollemi pine was discovered by science in 1994.
Conservation efforts have included a commercial propagation program focusing on home gardeners around the world.
...
Each tree has the DBH measured over time. 
:::
...
* *Estimation*:
  These RQs ask about an estimate of a *value* in the *population*, using the *sample* data.
* *Making decisions*:
...
*Relational RQs* explore existing relationships, and state the **P**opulation, the **O**utcome, and the *between-individuals* **C**omparison or **C**onnection.
...
In relational RQs, the comparison or connection explicitly refers to a *between-individuals comparison or connection*.
:::
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider this RQ:\label{thinkBox:POCIMiners}
\vspace{-2ex}
