...
```{r PermPavements, echo=FALSE, fig.cap="A study of permeable pavements (PPs) may have many sub-questions", fig.align="center", fig.height=2, fig.width=7, out.width = '95%'}
par( mar = c(0.10, 0.15, 0.10, 0.15))
openplotmat()
...
The implication is that the comparison or connection *may* influence the value of the outcome.
...
In contrast, *within-individual comparisons* make comparison *within* individual.
:::
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
A study of American college women [@@data:woolf:ironstatus] aimed to measure 'iron status' in highly active women (>12 hr purposeful physical activity per week) and sedentary women (<2 hr purposeful physical activity per week).

In this study, what is the Outcome; Comparison or Connection (if any), and Intervention (if any)?\label{thinkBox:POCIWomen}
...
`r if (knitr::is_latex_output()) {
   ...
   '[noisy miner](https://en.wikipedia.org/wiki/Noisy_miner) '
}`
is a bird.)
...
*Interventional RQs* (when possible) are the most interesting: they can be used to test theories or models, or to establish cause-and-effect relationships (i.e., causality); see Table \@ref(tab:ThreeTypesOfRQs).
...
Examples include the duration of cold symptoms, gender, place of birth, amount of tyre wear, or hair colour.
...
While many variables can be measured on individuals, two essential variables are (Table \@ref(tab:RQsPopulationIndividuals)):
...
The implication is that the values of the explanatory variable *may* influence the value of the response variable.
...
```{r POCIVariables, echo=FALSE, fig.cap="The POCI elements", fig.align="center", fig.height=2, fig.width=6, out.width='70%'}
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider this RQ:\label{thinkBox:Hairs}
...
Most statistical software (including jamovi [@Software:jamovi] and SPSS [@Software:SPSS]) uses the same approach for collating the data (though there are exceptions for some types of analyses):
...
1. What is the **comparison** in this RQ, if any?
`r if( knitr::is_html_output()) {
longmcq( c(
...
longmcq( c(
   ...
   answer = "Relational",
   "Interventional") )}`
6. What *approach* is taken by this RQ?
`r if( knitr::is_html_output()) {
longmcq( c(
...
`r if( knitr::is_html_output() ) { torf( TRUE )}`
  d. The size of the sample in the study is
...
*O*: The proportion that are coeliacs.
- Sect. \ref{thinkBox:Hairs}: We only have *one* example of a blond-haired man, and *one* of a brunet-haired man, so it is hard to make generalisation about blond- and brunette-haired men in general.  
All the hairs from one man will be very similar:  Their hair is washed at the same time with the same shampoo, is exposed to the sun for the same length of time, has the same genetics.  
We don't have a good cross-section of what the hair strands of blonds and brunets, in general, are like.
