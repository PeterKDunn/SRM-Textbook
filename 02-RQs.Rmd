...
* repeated-measures RQs (Sect.\ \@ref(RQsRepeatedMeasures)); and
...
The *exclusion* criteria includes people under $18$ years of age and over $40$ years of age; alternatively, the *inclusion* criteria are people aged between $18$ and $40$ years of age.
...
They also studied a *decision-making* descriptive RQ (where $98.6$^o^F ($37.0$^o^C) is a commonly-accepted value for the internal body temperature):
...
*Relational RQs* compare the outcome for groups of different individuals in the population, or compare two different populations.
...
@data:Williams2022:Possums compared the average weight of a sample of female and male Leadbeater's possums.
'Sex of the possum' is the *between-individuals* comparison.
:::
...
A *between*-individuals comparison would compare the strengths of the dominant and non-dominant legs *between different* groups of footballers: one group would have their dominant-leg strength measured, and the other would have their non-dominant-leg strength measured.
...
This study examines *within*-individuals changes: the differences between the strengths of the dominant and non-dominant legs *within* the same individuals.
...
For instance, a study may examine the *change* in each individuals blood pressure (the within-individuals comparison), for two drugs given to two different groups (the between-groups comparison).
...
This is a repeated-measures, estimation-type
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
(ref:CompareBetween) Examples of the *between-individuals* comparison and corresponding explanatory variable.
...
(ref:CompareWithin) Examples of the *within-individuals* comparison and corresponding explanatory variable.
...
A different type of RQ is a correlational
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
A RQ exploring the relationship between the individuals' water intake on the day before the race and the individuals' race times would be a correlational
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
One correlational RQ they studied was the relationship between the diameter of trees at breast height (DBH; response variable), and the pH of the soil (explanatory variable).
...
Nonetheless, a correlational RQ can still be asked: 
...
Sometimes, the explanatory variable naturally occurs (e.g., the height of people, or the pH of forest soil) without manipulation by the researchers.
Sometimes, however, the explanatory variable is manipulated by researchers (e.g., the dose of fertilizer applied; the dose of drug given); this is called an *intervention*.
...
To have an intervention, the *researchers* would need to tell each individual woman to be highly active or sedentary.
...
Answering decision-making RQs is discussed in Chaps.\ \@ref(TestOneProportion) to\ \@ref(TestsOddsRatio), plus Sects.\ \@ref(CorrelationTesting) and\ \@ref(RegressionHT).
...
These two options are hypotheses: potential answers to the\index{Hypotheses} 
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
For the zinc-intake study above (Fig.\ \@ref(fig:ZincRQ)), the RQ could ask:
...
These $100$ hair strands could be obtained in different ways:

* $100$ hair strands, all from the same man; or
* one hair strand, one each from $100$ different men.
...
U OF A:
* The unit of analysis is often determined by the research question: what is being compared (relational only?!)?
* It can be a single UoO (e.g., individual) or a collection of UoO (e.g., group or organization)
* Understanding the unit of analysis is crucial for designing and executing research studies
*  It is the smallest unit a researcher can use to identify and describe a phenomenon—the 'what' or 'who' the researcher wants to study.
* Must be independent, or nearly so.?


https://dovetail.com/research/unit-of-analysis/:
...
Ecological fallacy (PUT IN LIMITATIONS???? AND POINT BACK HERE???)
This mistake occurs when researchers use data from a higher-level unit of analysis to make claims about one lower-level unit of analysis.
It usually occurs when only group-level data is collected, but the researcher makes claims about individuals.

(Example: DFMT data?? That's country-level. In non-industrialised:

> with(Dental, plot( DMFT ~ Sugar, data=subset(Dental, Indus=="No")))
> with(Dental, abline(lm( DMFT ~ Sugar, data=subset(Dental, Indus=="No"))))

$y = 1.16 + 0.04698 x$.

But pop-level: Cannot learn about individuals.
For iinstance: maybe those those *not* eating sugar are responsible for the high DMFT.
...
Example/exercise: 
Take DBP from 30 patients to compare two roups (such as 40+ and under 40): 15 in each roup.

The researchers decide to take measurements from both L and R arms, doublin the number of BDP measuremenst to 60.
...
In addition, the PA program was *allocated* to each student individually, and each student has their own sport, family routines and activities, etc. and lives separate lives.
...
For example, a couple will often (if not always) *both* be wearing or both *not* wearing hats.
...
A within-individuals comparison seems unsuitable for this
`r if (knitr::is_latex_output()) {
  '\\RQperiod'
} else {
  'RQ.'
...
```{r DataPrepjamovi, fig.show="hold", out.width=c('37%', '2%', '51%'), fig.align="center", fig.cap="Software prepared for the data, with some data entered, and the variable names as the column headers. Left: a between-individuals comparison. Right: a within-individuals comparison.", fig.align="center", fig.width=3, fig.height=4}
...
The size of all the subjects' lymph nodes (removed through surgery) were then measured.
...
5. Suppose the measurements were all taken from $10$ *different* places on the *same* board (rather than from five different boards).
...
**1.** Between players in defensive, attacking positions.
**2.** *Between* individuals.
**3.** Average number knee injuries per player, per season.
