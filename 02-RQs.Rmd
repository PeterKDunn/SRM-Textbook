...
* An *operational definition* defines *how* something will be studied.
...
In many cases, a clear *operational definition* will be needed to describe how data will be collected to ensure repeatability and consistent data collection, by removing any ambiguity about how data are measured.
...
An *operational definition* would describe *how* 'stress' would be *measured*, since stress cannot be measured directly (like height, for example).
...
The *population* is rarely the individuals from which the data are actually *obtained*.
Indeed, *all* elements of the population are rarely accessible in practice.
For example, testing if a new drug is effective cannot possibly study *all* people (especially people not yet born who might use the drug). 
...
::: {.example #ExclusionAmoutees name="Population and exclusion criteria"}
...
'Men' operate separately, but the hairs on each man are not separate entities.
:::
...
Each of 44 children (with parental agreement) were allocated to one of two PA programs.
...
The values of the comparison or connection may be *imposed* on the individuals by the researchers, or may already *exist* in the individuals.
...
A study by @data:Bird2008:wholegrain *gave* participants a diet using refined flour, or a diet using a new flour variety (*Himalaya 292*).
...
Different *types* of RQ emerge depending on whether the RQ has a comparison/connection (C) and whether this can be manipulated (an intervention (I)).
...
This is a descriptive RQ, with a population ('co-grazing farms in Germany') and an outcome ('how often do young cattle and chickens interact'); no association with anything else is suggested.
:::
...
Typically, relational RQs with a *comparison* have the form:
...
The *population* and *outcome* are the same as for Example \@ref(exm:RelationalRQ).
The *connection* is with 'age', which cannot be manipulated by the researchers.
This is a *relational RQ* with a *connection*, but no *intervention*.
:::
...
*Relational RQs* then examine relationships, and provide an understanding of how the outcome is related to certain groups in the population, which may set the platform for asking interventional questions.
...
:::{.example #CoeliacStudies name="Diferent study types"}
...
Initially, how many individuals suffered from coeliac disease was unknown (e.g., @kenwright1972coeliac).
...
## Two forms of RQs {#TwoApproaches}

RQs can take different forms:
...
In practice, answers are rarely clear.
...
You may wonder: is there evidence that echinacea helps with a cold?
...
The outcome could be *compared* between groups (such as those taking echinacea and those who do not), or *connected* to something else (such as the daily *dose* of echinacea).
...
After making some decisions about P, O, C and I, a revised RQ (based on @data:barrett:echinacea) is:
...
Examples of variables include: the duration of cold symptoms, gender, place of birth, amount of tyre wear, or hair colour.
...
* Each *column* represents one variable: the *number* of columns equals the *number* of variables.
  (A column of identifying information may also appear, such as the person's name, or concrete batch number.)
