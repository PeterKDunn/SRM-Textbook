...
`r webexercises::hide()`
1. **Estimation**: The RQ is about estimating *what* the difference is.
...
This is an interventional RQ about a *population*.
The data needed to answer this RQ must come from the *individuals* in the population.

Each piece of information that we gather from individuals is called a *variable*, because the values can *vary* from individual to individual.


::: {.definition #Variable name="Variable"}
A *variable* is a single aspect or characteristic associated with each individual under consideration, whose values can vary from individual to individual.
:::
...
*Your* city of birth may not change, but 'city of birth' is a variable because it varies from *individual* to *individual*.

*Your* city of birth may not change, but that is not relevant.
:::
...
The '*average* duration of cold symptoms' is the *outcome*, a numerical summary of many individuals' cold durations in the population.
:::


While many variables can be measured on individuals, two essential variables are:
...
A *response variable* is a variable used to identify, measure, assess or describe the outcome on individuals in the population.
:::
...
An *explanatory variable* is a variable used to identify, measure, assess or describe the comparison or connection on individuals in the population.
:::


The explanatory variable allows the comparison or connection to be identified (Table \@ref(tab:RQsPopulationIndividualsExamplesComparison)).
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
*Response variables* are also called *dependent variables*, and *explanatory variables* are also called *independent variables*.
:::
	

The value of the *response* variable may change in *response* to the vcalue of the explanatory variable.
The value of the *explanatory* variable may *explain* the value of the response variable.


::: {.example #Variables2 name="Variables"}
For the final RQ for the echinacea study (Sect. \@ref(Writing-RQs)), 'the duration of cold symptoms' is the *response variable*, and 'type of medication (echinacea; or none)' is the *explanatory variable*.
...
(This would be found by measuring their weight *before* and *after* three weeks on the diets: a within-individuals comparison.)
...
> For Australian adults, is the average number of steps per day the same for those using a waistc, and those using a wrist accelerometer?

The **population** is the group being studied: *all* Australian adults.

The **outcome** is what is of interest in the population: the *average* number of steps per day.
The **response variable** is the number of steps per day.

The **comparison** refers to the groups for which the outcome is compared: "Between those wearing a waist accelerometer and those wearing a wrist accelerometer".
The **explanatory variable** is what varies from person to person; the **explanatory variable** is "The location of the accelerometer".

This RQ may or may not have an **intervention**.
If the researchers manipulate (i.e., tell) people where to wear the accelerometer, there *is* an intervention.
...
> In Australian 20-something men, is the average thickness of head hair strands the same for [blond-haired men](https://www.dictionary.com/browse/blonde) and [brunet-haired men](https://www.dictionary.com/browse/brunet?s=t) [@vaughn2009comparison]?

What is the problem with comparing 100 hair strands from one blond-haired man, to 100 hair strands from one brunet-haired man?

`r webexercises::hide()`
We only have *one* example of a blond-haired man, and *one* of a brunet-haired man, so it is hard to make generalisation about blond- and brunette-haired men in general.

All the hairs from one man will be very similar:  Their hair is washed at the same time with the same shampoo, is exposed to the sun for the same length of time, has the same genetics.
...
Instead, we learn a lot about two specific men.
...
However, different men would use different shampoo, exercise differently, have different genetics, and so on.
...
The RQ is comparing blond *men* with brunet *men*; *men* are being compared.
...
Units of analysis do not typically share a common underlying source.
:::
...
Different men would share little in common, and we are interested in comparing men.
:::
...
The tyres on any one car do not operate independently; they 'live their life together'.
They all are exposed to the same day-to-day use, the same drivers, have driven very similar distances, under the same conditions, etc.

The *unit of analysis* is the *car*: the brand of tyre is allocated to the carx, and all wheels on the car get the same tyre.
:::
...
People in a *group* at the beach will probably not be operating 'independently': people with similarities tend to group toegther.
...
* Use the people *groups* as the *unit of analysis* (some of which will be groups of one), and record data from just *one* person in any group.  
  Ideally, the researchers would specify before-hand from which group member to take data (e.g., the person closest to the researchers when the group is spotted).
...
However, they are sometimes different, and identifing these situations is *crucial*.
...
* Each *column* represents one variable.
  ...
  (There may also be a column of identifying information (such as the person's name, or concrete batch number).)
...
RQs may adopt one of two approaches:
