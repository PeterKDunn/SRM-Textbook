...
Some RQs ask about the value of some unknown population quantity (such as: what is the average internal body temperature?); these are called *estimation* RQs.
Some RQs ask require *making a decisions* about the population (such as: is the average internal body temperature the same for females and males?); these are called *decision-making* RQs.
...
Examples of variables include: the duration of cold symptoms, sex, rate of tree growth, response to a survey question (Yes; Maybe; No), city of birth, or hair colour.
...
The values of the explanatory variable, as measured on the individuals, may distinguish between the values of the *comparison* (Table\ \@ref(tab:RQsPopulationIndividualsExamplesComparison) and\ \@ref(tab:RQsPopulationIndividualsExamplesComparisonWithin)) being made.\index{POCI}
...
* the height of plants and the number of hours of sunlight per day.
* heart rate and the number of grams of caffeine consumed that day.
...
They also studied the relationship between the DBH for each tree at at various times after the planting date (a repeated-measure RQs). 
...
Sometimes, the explanatory variable naturally occurs without manipulation by the researchers (e.g., the height of people, or the pH of forest soil).
...
\index{Conditions}
The *conditions* are the values of the explanatory variable that those in the observational study experience, but are not manipulated or imposed by the researchers.
:::
...
*Estimation RQs* ask how precisely a *value* in the *population* is estimated by the *sample*; these are answered using *confidence intervals*.
Answering estimation RQs is discussed in Chaps.\ \@ref(CIOneProportion) to\ \@ref(OddsRatiosCI), and Sect.\ \@ref(RegressionCI).

*Decisions-making RQs* require a decision to be made about the population, and are answered using *hypothesis tests*.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
In general, RQs should be two-tailed RQs, unless a good (and justifiable) reason exists for asking a one-tailed question *before data are collected*.
:::
...
The units of *observation* are the 'slices': the type of bread and the amount of fibre are taken from the slice.
...
In addition, the PA program was *allocated* to each student individually, and each student has their own family routines and activities, etc. and lives separate lives.
