...
Asking clear and answerable *research questions* (RQs) is important as the RQ impacts all other components of the research (Fig. \@ref(fig:RQLeadsTo)).
...
```{r RQLeadsTo, echo=FALSE, fig.cap="The RQ drives all aspects of the study", fig.align="center", fig.height=1.75, fig.width=5.5, out.width = '75%'}
...
* A *conceptual definition* explains precisely *what* is being studied (i.e., what a word or a term *means* in the study).
* An *operational definition* defines exactly *how* something will be studied.
...
This is important here, since stress cannot be measured directly (like height, for example).

'Stress' could be *measured* using a questionnaire (like the Perceived Stress Scale (PSS) [@cohen1983global]); the level of stress is the score on the ten-question PSS.
Other means of measuring stress are also possible, and all have advantages and disadvantages.
:::
...
However, when commonly-accepted definitions exist they should be used unless good reasons exist for using a different definition.
...
In contrast, a *sample* is the *subset* of the population that is actually studied, from which data are obtained (see Sect. \@ref(Sampling)).
...
A study on the influenza vaccine [@kheok2008efficacy] listed the **P**opulation as 'health-care workers' [@kheok2008efficacy, p. 466], and the sample they studied was the healthcare workers at two specific hospitals.
...
:::{.exampleExtra  data-latex=""}
A study [@data:Guirao2017:amputees] of the walking abilities of amputees used inclusion *and* exclusion criteria (@data:Guirao2017:amputees, p. 27). 
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Consider this RQ (based on @vaughn2009comparison):\label{thinkBox:Hairs}
...
Each unit of analysis gives 200 observations.
...
Each unit of analysis (car) produces four observations.
:::
...
For example, a couple will often *both* be either wearing or *not* wearing sunglasses; families will often all be wearing sunglasses or not.
...
The implication is that a change in the value of the comparison or connection may result in a change in the value of the outcome (though it *may* or *may not* be a cause-and-effect relationship).
...
The groups being compared have either differences *imposed* on the individuals, or have *existing* differences in the individuals.
:::


The distinction between *within*-individuals comparisons and *between*-individuals comparisons (being discussed here) is important.
...
A *between*-individuals comparison would compare the left and right leg strengths of different groups: one group would have their left-leg strength measured, and the other their right-leg strength measured.
...
The **C** refers to *between*-individual comparisons.
:::
...
* Comparing the average heart rates (O) across three subsets: those who received no dose of a drug, those who received a daily dose of the drug, and those who received a twice-dailty dose of the drug.
...
**There is no (between-individuals) comparison**: the individuals are not divided into separated groups to compare average blood pressure; every person is treated the same way.
...
The *connection* in the RQ identifies another quantity of interest that varies *between-individuals*, that may be related to the outcome. 
:::
...
That is, the researchers *impose* the intervention upon those in the study to see the change in the outcome.
:::
...
A study by @data:Bird2008:wholegrain *gave* participants either a diet using refined flour, or a diet using a new variety (*Himalaya 292*).
...
*Descriptive RQs* are the most basic RQs, and identify the **P**opulation to be studied, and the **O**utcome of interest about this population.
...
*Relational RQs* explore existing relationships, and identify the **P**opulation, the **O**utcome, and the **C**omparison or **C**onnection.
...
*Interventional RQs* explore relationships where the comparison/connection is manipulated by, or imposed by, the researchers.
They identify the **P**opulation, the **O**utcome, the **C**omparison or **C**onnection, and use an **I**ntervention.
...
```{r EchinaceaRQ, echo=FALSE, fig.align="center", fig.cap="Two possible answers to the RQ about echinacea", out.width='80%', fig.width=6}
...
However, the short timelines and knowledge of the students mean that these RQs don't necessarily *need* to be Interesting, Novel or Relevant.
:::
...
We cannot decide if a particular time of day (the comparison) is morning or afternoon.
...
Many options exist: All Australians, or Australians adults with a specific "cold".
Some of these may not be practical.

What *outcome* could be used to to determine echinacea's effectiveness?
Again, options exist such as: *Average* cold duration, or the *percentage* of people who take days off work.

The initial RQ cannot be answered, because 'better' is ambiguous: better than what?
We could *compare* an outcome across different groups (such as those taking echinacea and those who do not), or *connect* the outcome to something else (the daily *dose* of echinacea).
...
If we did decide to intervene, various interventions could be used; we could impose how *frequently* the dose was taken; and/or imposing what *doses* of echinacea to take.

After making some decisions about P, O, C and I, consider this revised RQ (based on @data:barrett:echinacea):
...
Notice that the outcome is a numerical summary of groups of individuals: *average* duration of cold symptoms.
...
Each piece of information that we gather from each individuals is called a *variable*, because the values can *vary* from individual to individual.
...
PopInd3[2, ] <- c("Between beech and bamboo floor boards", 
		  "", 
		  "Type of floorboard in each \\emph{individual} home")
PopInd3[3, ] <- c("Between 300kg/ha, 350kg/ha and 400kg/ha fertilizer rates", 
	          "", 
...
if( knitr::is_latex_output() ) {
  kable(PopInd3,
   ...
   row_spec(0, bold = TRUE) %>% # Columns headings in bold
   column_spec(column = 1, width = "82mm") %>% 
   column_spec(column = 3, width = "61mm")
}
...
The **comparison** refers to the separate groups for which the outcome is compared: "Between those wearing a waist accelerometer and those wearing a wrist accelerometer".
...
Most statistical software (including jamovi and SPSS) uses the same approach for collating the data (though there are exceptions for some types of analyses):
...
In statistical software, the *names* of the variables are not placed in a separate row (say, in Row 1 above the data itself), which might happen when using a spreadsheet.\smallskip
...
For this RQ, the *variables* are (Examples \@ref(exm:Variables) and \@ref(exm:Variables2)) 'Duration of cold symptoms' (response), and 'Type of treatment' (explanatory).
...
While spreadsheets (such as Excel) can be used for analysing data, significant problems can, and do, emerge with using spreadsheets (Sect. \@ref(Software-In-Research)).
Care is needed when using spreadsheets for data analysis!
...
Some RQs have a **comparison** or **connection** (C); some also have an **intervention** (I): When the C can be manipulated by the researchers.
...
RQs may adopt one of two approaches: *Estimation*-type RQs; or  *Decision-making*-type RQs.
...
`r if( knitr::is_html_output() ) {mcq( c("Australians", answer = "Australian adults", "Australians who take Vitamin C tablets") )} else {"________________"}`, and the *outcome* is
