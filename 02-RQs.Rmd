...
The RQ directs all other components of the research, so writing clear and answerable *research questions* (RQs) is important.
...
Completely and precisely defining the population sometimes requires refining or clarifying the population, using *exclusion*\index{Exclusion criteria} and/or *inclusion criteria*.\index{Inclusion criteria}

Exclusion and inclusion criteria clarify which individuals are explicitly included or excluded from the population for the purposes of the study.
...
In a strength study study where the population is 'concrete test cylinders', cylinders with severe cracks may be *excluded* from strength tests.
...
Because the RQ concerns a large group (the population), the outcome describes a *group* of individuals (not single individuals).
Hence, the outcome is a numerical quantity (such as an *average* or *percentage*) summarising a group of individuals.
...
* Comparing the average amount of wear in floor boards *between* two *groups*: standard wooden floor boards, and bamboo floor boards.
* Comparing the average heart rates *across* three *groups* of people: those who received no dose of a drug, those who received a daily dose of the drug, and those who received a weekly dose of the drug.
...
Average faecal weight (the outcome) was compared for the two groups.
'Type of diet' is the *between-individuals* comparison.
:::
...
Consider comparing left- and right-leg strengths of professional football players.
...
In contrast, the left- and right-leg strength could be recorded on the *same* individuals.
...
To understand tree-dwelling marsupials, a study compared the temperature in the *same* tree hollows in summer and winter [@rowland2017comparing]:
...
The RQ cannot be answered without values for the response and explanatory variables.
...
A RQ exploring the relationship between the individuals' water intake on the day before the race and the individuals' race times would be a correlational RQ.
...
Sometimes, the explanatory variable naturally occurs (e.g., the height of people, or the age of trees) without manipulation by the researchers.
Sometimes, however, the explanatory variable is manipulated by researchers (e.g., the dose of fertilizer applied); this is called an *intervention*.
...
Explanatory variables *not* manipulated by the researchers are called *conditions*.
...
The *analysis* is the same whether an intervention is used or not, but the *interpretation* of the results depend on whether or not an intervention is used (Sect.\ \@ref(CompareStudyTypes)).
...
The *comparison* is between females and males, which cannot be manipulated or imposed on the individuals by the researchers, so *there is no intervention*.
:::
...
To have an intervention, the *researchers* would need to tell each individual woman to be highly active or sedentary; this seems unlikely, so the study does not have an intervention.
:::
...
The control group is like a benchmark for detecting changes in the outcome (Sect.\ \@ref(PlaceboEffectExperimental)).
...
Some patients are assigned to the *control group*, and do not get the drug injection.
The controls ideally would also report to a doctor and receive an injection (like those receiving the drug); the injection, however, would be ineffective (a placebo).
:::


Together, the **P**opulation, **O**utcome, **C**omparison and **I**ntervention form the POCI acronym\index{POCI} (sometimes written as PICO) to aid remembering the elements of RQs.
...
*Estimation RQs* ask how precisely a *value* in the *population* is estimated by using the *sample*; these are answered using *confidence intervals*.
Answering estimation RQs is discussed in Chaps.\ \@ref(CIOneProportion) to\ \@ref(OddsRatiosCI),  and Sect.\ \@ref(RegressionCI).

*Decisions-making RQs* require a decision to be made about the population, and are answered using *hypothesis testing*.
Answering decision-making RQs is discussed in Chaps.\ \@ref(TestOneProportion) to\ \@ref(TestsOddsRatio), plus Sects.\ \@ref(RegressionHT) and\ \@ref(CorrelationTesting).
...
We are just interested in whether any difference is present.
...
* we may have $100$ hair strands, all from the same single man; or
* we may have one hair strand, one each from $100$ different men.

While each approach gives $100$ measurements of hair-strand thickness, these two approaches are *very* different.
Only one man is represented in the first scenario, but $100$ different men are represented in the second.
...
All the hair strands from the same man have essentially 'lived their life together': they are washed together with the same shampoo, exposed to the same amount of sunlight and exercise, share the same genetics, etc.
However, different men potentially use different shampoo, exercise differently, have different genetics, and so on.
The hair on different men tends to be independent of the hairs on other men.
...
reported a study where researchers examined '$10$ neurons from each of the $16$ mice' in the study (November 2022).
...
For example, a couple will often *both* be wearing or both *not* wearing hats.
