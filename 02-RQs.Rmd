...
This is important, since stress cannot be measured directly (like height, for example).
...
For example, a couple will often *both* be either wearing or *not* wearing sunglasses; families will often all be wearing sunglasses or not wearing sunglasses.
...
*Descriptive RQs* are the most basic RQs, and state the **P**opulation to be studied, and the **O**utcome of interest about this population.
...
The Outcome (the average heart rate) numerically summarises the two groups being compared (females; males).
...
The *Population* is and **O**utcome are the same as for Example \@ref(exm:RelationalRQ).
The *Connection* is with 'age'.
...
*Interventional RQs* explore relationships where the comparison/connection can be manipulated by, or imposed by, the researchers.
Interventional RQs state the **P**opulation, the **O**utcome, the **C**omparison or **C**onnection, and use an **I**ntervention.

Interventional RQs are like relational RQs, except that the comparison or connection is manipulated by the researchers (i.e., has an intervention).
Sometimes, whether an intervention is used or not is unclear from the RQ.
...
*Descriptive RQs* are usually used when little is known about a research topic.
*Relational RQs* then explore relationships, and provide an understanding of how the outcome of interest is related to certain sub-groups of the population; they may set the platform for asking interventional questions.
*Interventional RQs* (when possible) can be used to test theories or models, or to establish cause-and-effect relationships (i.e., causality); see Table \@ref(tab:ThreeTypesOfRQs).
...
Initially, how many individuals suffered from coeliac disease was unknown (e.g., @kenwright1972coeliac: "there are no adequate data on the prevalence of coeliac disease").
Later *descriptive* RQs were answered that began to make estimates (e.g., @data:Cook2000:Coeliac).

Then, *relational* RQs asked questions about the proportion of females and males who were coeliacs (e.g., @data:Cook2000:Coeliac).
Then, *interventional* RQs were posed to understand the diease further; for example, if the percentage with adverse symptoms is the same for those given a diet *without* oats and those given a diet *with* oats (e.g., @data:Janatuinen2002:Coeliac; @data:Lundin2003:Coeliac).
:::
...
RQs can take different approaches:

* For **estimation** (*confidence intervals*) (Sect. \@ref(TwoApproachesCI)):
...
* For **making decisions** (*hypothesis testing*) (Sect. \@ref(TwoApproachesHT)):
...
Some RQs ask how precisely a *value* in the *population* is estimated by using the *sample*.
This value may measure a *difference*, or the *strength* of a relationship.
These RQs are studied in Chaps. \@ref(CIIntro)--\@ref(OddsRatiosCI) (for RQs with a *comparison*), and in Sect. \@ref(RegressionCI) (for RQs with a *connection*).


::: {.example #EstimationRQ name="Estimation RQs"}
Consider this RQ (based on @data:barrett:echinacea):

> Among New Zealand teens with a common cold, *how much shorter* are cold symptoms, on *average*, for teens taking a daily dose of echinacea compared to teens taking no medication?
...
Some RQs ask if a difference or a relationship exists in the *population*, based on what is observed in the *sample*.
These RQs often are associated with *hypotheses*: *statements* that suggest possible answers to the RQ.

These RQs are studied in Chaps. \@ref(HTIntro)--\@ref(TestsOddsRatio) (for RQs with a *comparison*), and in Sect. \@ref(RegressionHT) (for RQs with a *connection*).


::: {.example #DecisionsWithSamples name="Making decisions with samples"}
Consider this RQ (based on @data:barrett:echinacea):

> Among New Zealand teens with a common cold, is the average duration of cold symptoms shorter for teens taking a daily dose of echinacea compared to teens taking no medication?
...
```{r EchinaceaRQ, echo=FALSE, fig.align="center", fig.cap="Two possible answers to the RQ about echinacea", out.width='95%', fig.width=8.25, fig.height=4}
...
RQs emerge from observations, which lead to questions, and the need for evidence to answer that question.
For example, suppose you notice that some people take echinacea when they get a cold.
...
Many options exist: All Australians, or Australian adults with a specific "cold".
Some of these may not be practical (i.e., a sample cannot easily be obtained from the population).

What *outcome* could be used to to determine echinacea's effectiveness?
Many options exist, such as: *Average* cold duration, or the *percentage* of people who take days off work.

The initial RQ is also vague: better than what?
The outcome could be *compared* in two groups (such as those taking echinacea and those who do not), or *connected* to something else (such as the daily *dose* of echinacea).
...
The data to answer this RQ come from a sample of *individuals* in the population.
...
The implication is that changes in the values of the explanatory variable *may* be associated with changes in the value of the response variable.


The *outcome* refers to the numerical summary of the values of the response variable (Table \@ref(tab:RQsPopulationIndividualsExamplesOutcome)) from many individuals.
...
```{r POCIVariables, echo=FALSE, fig.cap="The POCI elements", fig.align="center", fig.height=2, fig.width=6, out.width='55%'}
...
Most statistical software (including jamovi and SPSS) uses the same approach for organising the data (though there are exceptions for some types of analyses):

* Each *row* represents one unit of analysis: the *number* of rows equals the *number* of units of analysis.
* Each *column* represents one variable:
  the *number* of columns equals the *number* of variables.
  (There may also be a column of identifying information, such as the person's name, or concrete batch number.)


::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In statistical software, the variable *names* are not placed in a separate row (say, in Row 1 above the data itself), which might happen when using a spreadsheet.
...
* The number of *columns* is two:
  ...
  There may be a column recording the name of each individual.  
  
The variable names (say, `Duration` and `Treatment`) are the columns names (Fig \@ref(fig:DataPrepjamovi)).
:::


```{r DataPrepjamovi, echo=FALSE, fig.show="hold", out.width='42%', fig.align="center", fig.cap="jamovi (left) and SPSS (right) prepared for the data, with some data entered, and the variable names as the column headers", fig.align="center", fig.width=3, fig.height=4}
...
Are the following statements true or false?
  ...
  a. The unit of analysis is the individual leaf.\tightlist
