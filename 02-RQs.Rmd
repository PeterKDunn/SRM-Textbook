...
* What is the cost of using PPs?
* Do PPs improve run-off quality?
...
Concrete test cylinders with fissure cracks may be excluded from strength tests.
...
Comparing $100$ hair strands from one blond-haired man, to $100$ hair strands from one brunet-haired man, is problematic: *only one man of each hair colour is represented*.
...
People in a *group* at the beach will probably not be operating independently: people in groups tend to be similar.
...
A *comparison* refers to an attribute that can be recorded for a small number of alternatives of interest, the outcome compared.
...
The *control group* is a comparison group *not* receiving the treatment being studied, or without having the condition being studied, but *as similar as possible* to the other units of analysis in all other ways.
...
In contrast, *within-individual comparisons or connections* make comparison *within* the *same* individuals.
:::
...
In contrast, consider a similar study where both right and left strength are measured on the same individuals.
...
Conservation efforts include a commercial propagation program focusing on home gardeners around the world.
...
Both DBH and pH are recorded on many different trees. 
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
A study of American college women [@data:woolf:ironstatus] measured iron status in highly active women and sedentary women.
...
Make it clear which is being used!
:::
...
Furthermore, the comparison is *between individuals*.
...
To understand tree-dwelling marsupials, a study compared the temperature in the *same* tree hollows during both summer and winter:

> For tree hollows in the Strathbogie Ranges, Australia, what is this mean temperature difference between summer and winter?
...
::: {.example #InterventionalRQ  name="Interventional RQ (between-individuals comparison)"}
...
::: {.example #InterventionalRQ  name="Interventional RQ (within-individuals comparison)"}
A study introduced an intervention, to determine if the intervebtion reduced the incidence of musculoskeletal disorders in Iranian bus drivers [@ghasemi2019effectiveness].
...
* *Estimation*:
  ...
  - descriptive RQs (Chaps.\ \@ref(CIOneProportion) to\ \@ref(OneMeanConfInterval));
...
* *Making decisions*:
  ...
  - descriptive RQs (Chaps.\ \@ref(TestOneProportion) to\ \@ref(TestOneMean));
  - relational RQs with a *within*-individuals comparison (Chap.\ \@ref(TestPairedMeans)), where the decision is about the changes within individuals;
...
Decision-making RQs can be asked in different ways.
...
The last two are *one-tailed RQs*, since they are ask specifically about a difference in just one direction: boys greater than girls, or girls greater than boys.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
In general, RQs should be written as two-tailed RQs, unless a good reason exists for asking a one-tailed question.
:::
...
Many terms need defining: What is meant by 'echinacea' (fresh? tablet form? as a tea); 'cold' (self-diagnosed? diagnosed by a doctor?), and so on.
...
The between-individuals *comparison* is "Between those wearing a waist accelerometer and those wearing a wrist accelerometer".
...
> In elite female netball players, do players in defence positions the same average number of knee injuries (per player, per season) than players in attacking positions?
...
1. What *type* of *comparison* appears in this RQ? \tightlist 
`r if( knitr::is_latex_output()) {'One- or two-tailed?'} else  {
longmcq( c(
