# Quantitative data: Summaries for changes within individuals {#SummariseWithin}



<!-- Introductions; easier to separate by format -->
```{r, child = if (knitr::is_html_output()) {'./introductions/16-CompareWithin-HTML.Rmd'} else {'./introductions/16-CompareWithin-LaTeX.Rmd'}}
```


## Introduction

Sometimes the same variable is measured on each individual more than once (i.e., *within*-individual comparisons for each unit of analysis) but only a small number of times.
...
*Within*-individuals comparisons require different methods than *between*-individuals comparisons.
...
Sometimes the same variable is measured on each unit of analysis twice. 
(Within-individuals data that is measured twice is also called 'paired' data.)
...
if( knitr::is_latex_output() ) {
  ...
  T1 <- kable(pad(IgE[1:6, ],
    ...
    add_header_above( c("IgE (before)" = 1,
                        "IgE (after)" = 1,
                        "Reduction" = 1),
                      bold = TRUE,
                      line = FALSE) 
...
```{r PairedGraphCaseProfileLATEX, fig.cap="The IgE data. Left: A histogram of the differences. Right: A case-profile plot. Each line represents one subject, joining that person's pre-intervention to their post-intervention measurement.", fig.align="center", fig.width=9.25, fig.height=3.25, out.width='100%'}
...
As seen in the last example, the case-profile plot is hard to read with large numbers of individuals.
In those situations, sometimes just the mean (or median, as appropriate) is shown, often with some measure of the variation of the observations (Fig.\ \@ref(fig:RunningRM) shows the minimum and maximum values, for instance).
...
For the data in Example\ \@ref(exm:CaseProfileRunning), the numerical summary table is shown in Table\ \@ref(tab:IgETable).
...
if( knitr::is_latex_output() ) {
 ...
 T1 <- knitr::kable(pad(Flowering[1:9, c(5, 1, 2)],
                        surroundMaths = TRUE,
 ...
 T2 <- knitr::kable(pad( rbind(Flowering[10:17, c(5, 1, 2)],
                                         c(NA, NA)),
                        surroundMaths = TRUE,
                        targetLength = c(2, 3, 3),
                        digits = 0),
  ...
  T3 <- knitr::kable(pad( rbind(Flowering[18:25, c(5, 1, 2)],
                                          c(NA, NA)),
                        surroundMaths = TRUE,
                        targetLength = c(2, 3, 3),
                        digits = 0),
...
When the number of individuals is large, sometimes just the means are shown (the black lines and points in the figure).
...
Quantitative data measured within individuals can be summarised using a histogram of differences (with two comparisons) or a case-profile plot (with two or more comparisons).
A summary table should show the numerical summaries for the quantitative variable at each comparison and, if appropriate, the within-individual differences. 
...
1. A histogram of the differences is only appropriate for showing within-individuals comparisons measured twice. \tightlist
`r if( knitr::is_html_output() ) {torf(answer = TRUE)}`
2. A case-profile plot is only appropriate for showing within-individuals comparisons measured twice.
