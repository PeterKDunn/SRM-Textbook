...
One approach to studying this is to record some students' weights as they begin university, and then obtain *the same* students' weight at some later time.
The comparison is *within* individuals (Sect.\ \@ref(Comparison)), and the RQ is a *descriptive RQ*: the *Outcome* is the mean *change* in weight, and the response variable is the weight *change* for each individual student.
Each student has a *pair* of weight measurements, and the study produces *paired data*, the topic of this chapter.
	
Alternatively, the researchers could take a sample of students who are beginning university and measure their weight, and then a *different* sample of students some later time and measure their weight.
This is comparing *between* individuals.
This second (between-individuals) design requires comparing the means of two *different* groups of students, the topic of the next chapter.
...
* Blood pressure is recorded on the same individuals *before* and *after* giving a drug.
...
```{r ComparePairedBoxplots, fig.align="center", out.width='80%', fig.cap="Plots of the weight-loss data. Left: Treating the data incorrectly as not paired; right: a boxplot of the weight gains when the data are treated as paired.", fig.height = 3.25, fig.width = 7}
data(StudentWt)
...
The question asked in Sect.\ \@ref(PairedIntro) was studied by Prof. David Levitsky (@levitsky2004freshman, @DASL:WeightChange):

> For Cornell University students, what is the *mean weight change* in students after $12$ weeks at university?

Data were obtained data to answer this RQ 
`r if (knitr::is_latex_output()) {
   '(Table\\ \\@ref(tab:DataWeightChange)).'
} else {
   '(shown below).'
...
tb1 <- array( cbind( StudentWt$Student[1:5 ],
                     StudentWt$Week1[1:5 ],
...
tb2 <- array( cbind( StudentWt$Student[(SWlen - 4):SWlen ],
...
For the weight-gain data, the sample mean differences $\bar{d}$ are described by:
...
Is the difference between the two treatments statistically significant?
...
Young girls ($n = 29$) with anorexia received cognitive behavioural treatment (@data:hand:handbook, Dataset 285), and their weight before and after treatment were recorded [Data: `Anorexia`].
In summary:
...
In Table\ \@ref(tab:FruitsData), the mean changes refer to the *mean increase* in fruit weight from 2014 (dry) to 2015 (normal).
(Computing the changes as the 2015 (normal) values less the 2014 (dry) values is fine, provided this is used consistently throughout the analysis.)
The parameter is $\mu_d$, the population mean *increase* in average fruit weight (in g) from 2014 to 2015.
...
a numerical summary *must* summarise the differences, as the RQ is about the mean difference.
The dry-year and normal-year weights can be summarised too.
...
> The mean increase in average fruit weight from 2014 (dry year) to 2015 (normal year) is $2.230$\ g ($\text{s.e.} = 10.879$; $n = 23$), with an approximate $95$% CI between $24.79$\ g lighter in 2014 to $20.33$\ g higher in 2015.

The sample size ($n = 23$) is only *slightly* smaller than $25$.
Since Fig.\ \@ref(fig:FruitPlots) shows the *sample* differences are approximately symmetric, so the CI should be reasonably statistically valid.
:::
...
::: {.thinkBox .think data-latex="{iconmonstr-light-bulb-2-240.png}"}
Is there a mean difference in average fruit weight from 2014 to 2015 *in the population*?\label{thinkBox:MeanDifferenceFruit}
