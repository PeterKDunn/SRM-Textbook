...
If one of the qualitative variable has two levels,\index{Levels} the RQ may be worded in terms of odds.
...
> Are the odds of correctly identifying a rip the same for each age group?
:::
...
Because one variable has two levels, the hypotheses can be worded in terms of comparing odds:
...
For tables larger than $2\times 2$ more generally, the hypothesis are usually worded in terms of associations (but *not* correlations):
...
The value of $\chi^2$ compares the observed counts to the expected counts (Sect.\ \@ref(CompareTwoOdds)); the expected counts are found in the same way as in Sect.\ \@ref(ExpectedValues).

For two-way tables larger than $2\times 2$, the parameter describing the association between the variables is the $\chi^2$ value itself.
...
```{r Ripsjamovi, fig.show="hold", fig.cap="Software output for the hypothesis test about knowledge of ocean rips", fig.align="center", out.width=c("45%", "50%"), }
...
The software output is shown in Fig.\ \@ref(fig:BurrosChisqjamovi), and a graphical summary in Fig.\ \@ref(fig:BurrosSidebysideBar).
...
###
 ...
 T2 <- kable(pad(BurroSummary,
    ...
    out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "BurrosData",
                       caption = "Left: the health and reproductive status of female burros. Right: summary of the burro data. The odds ratios are relative to burros lactating. (Burros pregnant and lactating were counted in the lactating data only.)") %>% 
    kable_styling(font_size = 8)
...
if( knitr::is_html_output() ) {
  kable(pad(BurroData,
        ...
        align = "c",
        longtable = FALSE,
        caption = "The health and reproductive status of female burros. (Burros pregnant and lactating were counted in the lactating data only.)",
        booktabs = TRUE)
}
```
...
if( knitr::is_html_output() ) {
  kable(pad(BurroSummary,
        ...
        align = "c",
        longtable = FALSE,
        caption = "Summary of the burro data. The odds ratios are relative to burros lactating.",
        booktabs = TRUE)
}
```
...
* For barren burros: the odds of excellent health is $16/(21 + 38) = 0.271$.
* For pregnant (non-lactating) burros: the odds of excellent health is $14/(53 + 62) = 0.122$.
* For lactating burros: the odds of excellent health is $4/(29 + 78) = 0.0374$.

(We could have chosen to compute the odds of Fair or Poor health, rather than Excellent health.)
For computing the odds ratios, Row\ 3 is on the bottom of the fraction (as the reference level):\index{Reference level}

* The OR of excellent health, comparing barren burros to lactating burros, is: $0.271/0.037 = 7.25$.
* The OR of excellent health, comparing pregnant burros to lactating burros, is: $0.122/0.037 = 3.30$.

That is, the odds of excellent health for barren burros is $7.25$ times the odds of excellent health compared to lactating burros.
Similarly, the odds of excellent health for pregnant burros is $3.26$ times the odds of excellent health compared to lactating burros.

The hypothesis are usually worded in terms of associations (but *not* correlations):
...
From the software output (Fig.\ \@ref(fig:BurrosChisqjamovi)), $\chi^2 = 23.585$.
...
> The sample provides very strong evidence ($\chi^2 = 23.585$) of an association between reproductive status and overall health of female burros.
...
Comparing all possible *pairs* of groups increases the probability of *incorrectly* declaring a difference between the odds (increasing the chance of a Type\ I error;\index{Type I error} Sect.\ \@ref(TypeErrors))
The correct approach requires methods beyond this book.

All *expected* values exceed $5$ (Fig.\ \@ref(fig:BurrosChisqjamovi)), so the results are statistically valid.
...
```{r SunglassesOutput, fig.show="hold", fig.cap="Software output for the hats data", fig.align="center", out.width=c("52%", "47%"), fig.show='hold'}
...
The software output is shown in Fig.\ \@ref(fig:BagsChisqjamovi).
