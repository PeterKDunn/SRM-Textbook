...
For example, the parameter could be difference between population proportion of students eating most meals *off*-campus, comparing students living with their parents, to students *not* living with their parents.
...
We use $N$ to refer to students *not* living with their parents, and $L$ for students living with their parents.
Then, following Table\ \@ref(tab:ORHT), the *parameter* in the RQ is the difference between the population proportions: $p_L - p_N$.
...
$$
  z = \frac{ (\hat{p}_L - \hat{p}_N) - (p_L - p_N) }{\text{s.e.}(\hat{p}_A - \hat{p}_B)} \
    ...
    = 2.633.
$$
Since the test-statistic is a $z$-score, the approximate $P$-value can be computed from normal distributions (Sect.\ \@ref(ExactAreasUsingTables)) or from software output (Fig.\ \@ref(fig:UniMealsTestOutputHTjamovi)).
...
$$
  z = \sqrt{\chi^2}\qquad\text{for a $2\times 2$ table only}.
$$
...
knitr::kable( pad(RipSummary,
       ...
       align = "c",
       format = "html",
       caption = "Identifying rips: a summary table. The odds ratio is relative to the $51$ to $65$ age group.",
       booktabs = TRUE) %>%
     row_spec(0, bold = TRUE) %>%
    add_header_above( c(" " = 1, 
                        "Correctly identifying rips" = 3,
...
Because odds can be computed, the hypothesis can be worded in terms of comparing odds:
...
This hypothesis encompasses many possibilities: for example, that the three odds are *all* different from each other, or that the odds for the first age group is different than the other two (which are the same).

For tables larger than $2\times 2$, the hypothesis are usually worded in terms of associations (but *not* correlations):

* $H_0$: In the *population*, there *is no association* between bringing a shopping bag and age grpup;
* $H_1$: In the *population*, there *is an association* between bringing a shoopingbag and age grop.
...
$$
where $\text{df}$ is the *degrees of freedom*,\index{Degrees of freedom} and
$$
...
$$
which is quite small, so we expected a large (two-tailed) $P$-value.
...
```{r TurtleNestsOutputjamoviHT, fig.show="hold", fig.cap="The software output for the turtle-nesting data", fig.align="center", out.width=c("46%", "51%")}
...
## Example: health of female burros {#asxhaiuhpiauhapxhpxuihpaxhsap}

johnson1987demography studied $631$ introduced donkeys (burros) in the Mojave Desert (California) to understand management processes. 
One RQ was:
...
The software output is shown in Fig.\ \@ref(fig:BurrosChisquareTest), and a graphical summary in Fig.\ \@ref(fig:BurrosSidebysideBar).
...
BurroSummary[1, 1] <- BurroTab[1, 1] / BurroTab[1, 3]
BurroSummary[2, 1] <- BurroTab[2, 1] / BurroTab[2, 3]
BurroSummary[3, 1] <- BurroTab[3, 1] / BurroTab[3, 3]

BurroSummary[1, 2] <- BurroTab[1, 1] / sum( BurroTab[1, ]) * 100
BurroSummary[2, 2] <- BurroTab[2, 1] / sum( BurroTab[2, ]) * 100
BurroSummary[3, 2] <- BurroTab[3, 1] / sum( BurroTab[3, ]) * 100

BurroSummary[, 3] <- rowSums(BurroTab)
...
###
 ...
 T2 <- kable(pad(BurroSummary,
    ...
    out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "BurrosData",
                       caption = "Left: the health and reproductive status of female burros. Right: summary of the burro data.  The odds are relative to burros lactating. (Pregnant and lactating burros are counted in the lactating data.)") %>% 
    kable_styling(font_size = 8)
...
if( knitr::is_html_output() ) {
  kable(pad(BurroData,
        ...
        align = "c",
        longtable = FALSE,
        caption = "The health and reproductive status of female burros.  (Pregnant and lactating burros are counted in the lactating data.)",
        booktabs = TRUE)
}
```
...
if( knitr::is_html_output() ) {
  kable(pad(BurroSummary,
        ...
        align = "c",
        longtable = FALSE,
        caption = "Summary of the burro data",
        booktabs = TRUE)
}
```
...
```{r BurrosChisqjamovi, fig.show="hold", fig.cap="Software output for the burro-health data", fig.align="center", out.width=c("43%", "56%")}
...
* For barren burros: the *odds* of excellent health (compared to poor health) is $16/38 = 0.421$.
* For pregnant (but not lactating) burros: the *odds* of excellent health (compared to poor health) is $14/62 = 0.226$.
* For lactating burros: the odds of *excellent* health (compared to poor health) is $4/78 = 0.0513$.
...
The hypothesis can be worded in terms of odds, but for tables larger than $2\times 2$, the hypothesis are usually worded in terms of associations (but *not* correlations):
...
From the software output (Fig.\ \@ref(fig:BurroChisquareTest)), $\chi^2 = 23.585$ and $\text{df} = 4$.
Software reports $P < 0.001$, which suggests very strong evidence in the sample that a relationship exists between reproductive status and overall health.

The conclusion could be written as

> The sample provides very strong evidence ($\chi^2 = 23.585$; $\text{df} = 4$) of an association between reproductive status and overall health of female burros.
...
`r if (knitr::is_latex_output()) {
   '(Table\\ \\@ref(tab:BurrosSummary), right table).'
} else {
   '(Table\\ \\@ref(tab:BurrosSummaryHTML)).'
}`
...
All *expected* values exceed $5$ (Fig.\ \@ref(fig:BurrosExpected)), so the results are statistically valid.
...
Use the software output (Fig.\ \@ref(fig:Sandfliesjamovi)) to evaluate the evidence, complete Table\ \@ref(tab:SandfliesHTNumerical), and write a conclusion.
:::
...
```{r B12jamoviOutputHT, fig.show="hold", fig.cap="Software output for the B12 data", fig.align="center", out.width=c("49%", "49%", fig.hold=TRUE)}
...
```{r BagsChisqjamovi, fig.show="hold", fig.cap="Software output for the shopping-bags data", fig.align="center", out.width=c("43%", "56%")}
...
From the software output (Fig.\ \@ref(fig:BagsChisqjamovi)), $\chi^2 = 16.24$.
Software reports $P < 0.001$.
