...
The difference between the two proportions will vary from sample to sample too, and hence have a sampling distribution; under certain conditions (Sect.\ \@ref(Validity-Test-ChiSq)), this sampling distributiomn will have a normal distribution.
...
$$
  z = \frac{ (\hat{p}_L - \hat{p}_N) - (p_L - p_N) }{\text{s.e.}(\hat{p}_A - \hat{p}_B)} \
    ...
    = 2.633.
$$
Since the sampling distribution has an approximate normal distribution, the approximate $P$-value can be computed from normal distributions (Sect.\ \@ref(ExactAreasUsingTables)), approximated using the $68$--$95$--$99.7$ rule, or from software output (Fig.\ \@ref(fig:UniMealsTestOutputHTjamovi)).
...
From the software output (Fig.\ \@ref(fig:TurtleNestsOutputjamoviHT), left panel), the $\chi^2$-value is $0.777$.
This is like a $z$-score of $z = \sqrt{0.777/1} = 0.88$, which is very small, so expect a large $P$-value.
Notice that this the value of the $z$-score shown in Fig.\ \@ref(TurtleNestsOutputjamoviHT) (left panel).
...
We assume the null hypothesis is true, so we state how strong the evidence is to change outr mind (and hence support the alternative hypothesis).
...
```{r TurtleNestsOutputjamoviHT, fig.show="hold", fig.cap="The software output for the turtle-nesting data", fig.align="center", out.width=c("50%", "49%")}
...
###
 ...
 T2 <- kable(pad(BurroSummary,
    ...
    out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "BurrosData",
                       caption = "Left: the health and reproductive status of female burros. Right: summary of the burro data. The odds ratios are relative to burros lactating. (Burros pregnant and lactating were counted with the lactating burros only.)") %>% 
    kable_styling(font_size = 8)
...
if( knitr::is_html_output() ) {
  kable(pad(BurroTab,
        ...
        align = "c",
        longtable = FALSE,
        caption = "The health and reproductive status of female burros. (Burros pregnant and lactating were counted with the lactating burros only.)",
        booktabs = TRUE)
}
```
