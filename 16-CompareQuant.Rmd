...
## Summarising the comparison: difference between means {#CompareQuantTables}
\index{Quantitative data!compare \textit{between} individuals!summary tables}


The best way to compare the two groups is to summarise the *difference between the two means*.
A numerical summary table can be constructed summarising both group, and reporting the difference between the two means.
...
Gorilla2 <- cbind( CBRyounger[1:3], 
                   CBRyounger[4:6],
                   CBRyounger[7:9],
                   CBRyounger[10:12],
                   c(CBRyounger[13:14], NA),
                   CBRolder[1:3],
                   CBRolder[4:6],
                   CBRolder[7:9],
                   c(CBRolder[10:11], NA) )
...
if( knitr::is_latex_output() ) {
  kable( pad(Gorilla2,
         ...
         booktabs = TRUE,
         escape = FALSE,
         caption = "The chest-beating rate of gorillas (in beats per $10$\\ hours)",
         linesep = c("", "", "", "\\addlinespace"),
         align = c("c","c")) %>%
         #col.names = c("Treatment group", "Control group"),
    add_header_above(header = c("Younger " = 5, 
                                "Older" = 4), 
                     bold = TRUE, 
                     align = "c")
} else {
...
Disadvantage are that only two groups can be compared, and not all data work well with stemplots.




::: {.example #GorillaData name="Back-to-back stemplots"}
A back-to-back stemplot for comparing the chest-beating rate of gorillas (Fig.\ \@ref(fig:krillEggsDoubleStem) has the leaves for younger gorillas go from right-to-left, and the leaves for older gorillas go left-to-right, sharing the same stems.
...
```{r krillEggsDoubleStem, fig.width=7.0, fig.height=3, out.width='60%', fig.cap="Stemplot for the chest-beating rate for gorillas", fig.align="center"}
...
Many observations are the same, so some points would be *overplotted*\index{Overplotting} if points were not *stacked*\index{Overplotting!stacking} (left panel), or some randomness added in the vertical direction (a 'jitter'),\index{Overplotting!jittering} (right panel).
:::
...
A boxplot is a picture of the quantiles (Sect.\ \@ref(VariationIQR)) of each group.
...
The values of $Q_1$, the medians, and $Q_3$ can be used to compare the distributions.
...
```{r BoxplotGorillasExplain, fig.cap="Explaining the boxplots for the chest-beating data", fig.align="center",  fig.height=4.25, fig.width=10, out.width='100%'}
...
Computer software has identified this observation as a potential (large) *outlier* using the IQR rule (Sect.\ \@ref(OutliersIQRrule)), and has plotted this point separately.
...
The graphs (Fig.\ \@ref(fig:WaterAccessCompareQuantFigs)) and summary (Table\ \@ref(tab:WaterAccessCompareQuantTabs)) show that households in which diarrhoea was found in the last two weeks in children had older coordinators, more people in the household, and more children under\ $5$ in the household. 
...
```{r WaterAccessCompareQuantFigs, fig.align="center", fig.cap="Some plots for the water access data in 85 households ($59$ household reported no diarrhoea in children under\\ $5$; $26$ reported diarrhoea in children under\\ $5$). ", out.width = '100%', fig.width=6, fig.height=2.25}
data(WaterAccess)
par( mfrow = c(1, 3),
...
if( knitr::is_latex_output() ) {
    kable( pad(WACompareQual,
        ...
        linesep = c( "", "", ""), 
        caption = "A summary of the quantitative variables in the water-access study, according to whether diarrhoea had been observed in the last two weeks in children under $5$, for those household with children under $5$",
        align = c("r", "c", "c", "c", "c", "c"))   %>%
   kable_styling(full_width = FALSE, 
                 font_size = 8) %>%
...
A summary table should show the numerical summaries for the quantitative variable at each comparison and the between-group differences. 
...
@data:Hale2009:ProjectDelivery studied different engineering project delivery methods (Fig.\ \@ref(fig:AISfemalesportEng), left panel).
...
Sketch a graphs to explore the *relationship* between the children's weight and GMGCS.
:::
...
Use the software output (Fig.\ \@ref(fig:NHANESTwoSample))\index{jamovi output!comparing two means} to construct an appropriate table showing the numerical summary relevant to the RQ.
:::

```{r NHANESTwoSample, fig.cap="Software output for the NHANES data", fig.align="center", out.width='100%'}
...
1. Using the software output in Fig.\ \@ref(fig:SpeedjamoviCI), summarise the data numerically, then construct a summary table.
...
4. Which box in the boxplot represents the Dangar Island jellyfish (shown in Fig.\ \@ref(fig:JellyfishHist), left panel)?
:::
